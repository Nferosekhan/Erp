...
if (isset($_POST['submitcustomizes'])) {
	...
	$timefrom = htmlspecialchars($_POST['timefrom'], ENT_QUOTES, 'UTF-8');
	$timeto = htmlspecialchars($_POST['timeto'], ENT_QUOTES, 'UTF-8');
	$reportperiod = htmlspecialchars($_POST['reportperiod'], ENT_QUOTES, 'UTF-8');

	$sqlreportmoduless=$con->prepare("SELECT * FROM pairreportmodules WHERE types='saledetails' ORDER BY id ASC");
	...
	foreach ($newmodules as $newmoduleskey) {
		...
		$modcolumncol=$coltypemod."saledetails";
	...
	$sqlupreport = $con->prepare("UPDATE pairreports SET rowcolumns=? WHERE franchiseid=? AND createdid=? AND types='saledetails'");
	...
	$sqlreportfav = $con->prepare("UPDATE pairreportfavourites SET reporturl=? WHERE franchisesession=? AND createdid=? AND reportnames='saledetails'");
	$sqlreportfav->bind_param("sss", $newReportUrl, $_SESSION['franchisesession'], $companymainid);
	$sqlreportfav->execute();
	$sqlreportfav->close();

	$sqlreport = $con->prepare("UPDATE pairreports SET reportperiod=?,filtername=?,companyname=?,dateprepare=?,timeprepare=?,gstrule=?,payterm=? WHERE franchiseid=? AND createdid=? AND types='saledetails'");
	$sqlreport->bind_param("sssssssss", $reportperiod, $filtername, $companyname, $dateprepare, $timeprepare, $gstrule, $payterm, $_SESSION['franchisesession'], $companymainid);
	$sqlreport->execute();
	$sqlreport->close();

	header("Location:reportsaledetails.php?datefrom=".date($dateformatforurlredirection,strtotime($from))."&dateto=".date($dateformatforurlredirection,strtotime($to))."&timefrom=".$timefrom."&timeto=".$timeto."&payterm=".$payterm."&productid=".$_GET['productid']."");
}

$sqlreportviews=$con->prepare("SELECT * FROM pairreports WHERE franchiseid=? AND createdid=? AND types='saledetails'");
									...
									<div class="row">
										<div class="col-lg-6">
											<p class="mb-3" style="color:black;font-size: 20px;margin-top: -8px;">
												...
												&gt;
												<a href="reportsales.php?datefrom=<?=$_GET['datefrom']?>&dateto=<?=$_GET['dateto']?>&timefrom=<?=$_GET['timefrom']?>&timeto=<?=$_GET['timeto']?>&payterm=<?=$_GET['payterm']?>" style="color: #1878F1">
													Sales
												</a>
												&gt;
												Sales Details
																			...
																			<div id="customizerowcolumns" class="accordion-collapse collapse show" aria-labelledby="customizerowcolumn">
																				<div class="accordion-body text-sm">
																					<div class="row justify-content-center">
																						...
																						<?php
																							$newans=array();
																							$sqlreportaccesss=$con->prepare("SELECT * FROM pairreports WHERE franchiseid=? AND createdid=? AND types='saledetails'");
																								...
																								<div class="col-lg-6 my-1" style="padding-left: 0px !important;">
																									<div class="custom-control custom-checkbox mr-sm-2" style="padding: 0px 12px !important;">
																										<div class="input-group input-group-sm" id="flaghead">
																											...
																											<input type="checkbox" class="custom-control-input" name="<?= $coltypemod; ?>saledetails" id="<?= $coltypemod; ?>saledetails" <?=((($newmoduleskey=='Sno')||($newmoduleskey=='Payment Term'))?(((in_array($newmoduleskey, $newans)))?'checked':''):'checked')?> <?=((($newmoduleskey=='Sno')||($newmoduleskey=='Payment Term'))?'':'disabled')?>>
																											<label class="custom-control-label custom-label" for="<?= $coltypemod; ?>saledetails">
																							...
																							<div class="row">
																								...
																								<div class="col-lg-6 automodalchecking">
																									...
																									<?php
																											...
																											$sqlis->bind_param("s", $_SESSION['franchisesession']);
																											$sqlis->execute();
											...
											<div class="tab-pane fade" id="nav-profile" role="tabpanel" aria-labelledby="nav-profile-tab">
												<div class="table-responsive m-3" id="histables">
													<table class="table table-bordered">
														...
														<tbody>
														<?php
															$sqluse=$con->prepare("SELECT * FROM pairusehistory WHERE usetype = 'REPsaledetails' AND useid = ? ORDER BY createdon DESC");
																		...
																		<tr style="height:1px;">
																			<td width="100%" style="padding:10px;">
																				<table id="print-are1" style="border: 1px solid #eee;" width="100%">
																					<thead>
																						<tr>
																							<td class="text-uppercase" style="<?=(((in_array('Sno', $newans)))?'':'display: none;')?>width:7%;border:1px solid #eee;padding-left: 10px;">
																								<span style="font-size:13px;color:black;">
																									#
																								</span>
																							</td>
																							<td class="text-uppercase" style="width:<?=(((in_array('Payment Term', $newans)))?'20%':'30%')?>;border:1px solid #eee;padding-left: 10px;">
																								<span style="font-size:13px;color:black;">
																									Product Name
																								</span>
																							</td>
																							<td class="text-uppercase" style="text-align: left !important;width: 16%;border:1px solid #eee;padding-left: 10px;">
																								<span style="font-size:13px;color:black;">
																									Invoice Number
																								</span>
																							</td>
																							<td class="text-uppercase" style="text-align: left !important;width: <?=(((in_array('Payment Term', $newans)))?'11%':'14%')?>;border:1px solid #eee;padding-left: 10px;">
																								<span style="font-size:13px;color:black;">
																									Invoice Date
																								</span>
																							</td>
																							<td class="text-uppercase" style="text-align: right !important;width: 10%;border:1px solid #eee;padding-right: 10px;">
																								<span style="font-size:13px;color:black;">
																									Quantity
																								</span>
																							</td>
																							...
																							<td class="text-uppercase" style="<?=(((in_array('Payment Term', $newans)))?'width:13%;':'display: none;')?>border:1px solid #eee;padding-left: 10px;">
																								<span style="font-size:13px;color:black;">
																									Term
																								</span>
																							</td>
																					...
																					<?php
																						$count=1;
																						$sqlsels=$con->prepare("SELECT invoiceterm,productname,SUM(quantity) AS quantity,SUM(productvalue) AS productvalue,invoiceno,invoicedate FROM pairinvoices WHERE franchisesession=? AND createdid=? AND (invoicedate>=? AND invoicedate<=?) AND (invoicetime>=? AND invoicetime<=?) AND productid=? ".(($sqlviewreport['payterm']=='all')?' ':' AND invoiceterm="'.$sqlviewreport['payterm'].'"')." GROUP BY invoiceno,invoicedate LIMIT 36");
																						$sqlsels->bind_param("sssssss", $_SESSION['franchisesession'], $companymainid, $datefrom, $dateto, $timefrom, $timeto, $_GET['productid']);
																						$sqlsels->execute();
																						$sqlsel = $sqlsels->get_result();
																						while($fetsql=$sqlsel->fetch_array()){
																					?>
																						<tr style="height: 30px;">
																							<td  data-label="#" style="<?=(((in_array('Sno', $newans)))?'':'display: none;')?>border: 1px solid #eee;padding-left: 10px;font-size: 13px;">
																								<?=$count?>
																							</td>
																							<td  data-label="Product Name" style="border: 1px solid #eee;padding-left: 10px;font-size: 13px;">
																								<?=$fetsql['productname']?>
																							</td>
																							<td  data-label="Invoice Number" style="border: 1px solid #eee;padding-left: 10px;font-size: 13px;text-align: left;">
																								<?=$fetsql['invoiceno']?>
																							</td>
																							<td  data-label="Invoice Date" style="border: 1px solid #eee;padding-left: 10px;font-size: 13px;text-align: left;">
																								<?=date($datemainphp,strtotime($fetsql['invoicedate']))?>
																							</td>
																							<td  data-label="Quantity" style="border: 1px solid #eee;padding-right: 10px;font-size: 13px;text-align: right;">
																								<?=$fetsql['quantity']?>
																							</td>
																							<td  data-label="Amount" style="border: 1px solid #eee;padding-right: 10px;font-size: 13px;text-align: right;">
																								<?=number_format($fetsql['productvalue'],2,'.',',')?>
																							</td>
																					...
																					<?php
																						...
																						if (mysqli_num_rows($sqlsel)>0) {
																							...
																							$sqltotlistpages->bind_param("sssssss", $_SESSION['franchisesession'], $companymainid, $datefrom, $dateto, $timefrom, $timeto, $_GET['productid']);
										...
										<table id="print-are3" style="border: 1px solid #eee;display: none;" width="100%">
											<thead>
												<tr>
													<td class="text-uppercase" style="<?=(((in_array('Sno', $newans)))?'':'display: none;')?>width:7%;border:1px solid #eee;padding-left: 10px;"><span style="font-size:13px;color:black;">Sno</span></td>
													<td class="text-uppercase" style="width:<?=(((in_array('Payment Term', $newans)))?'20%':'30%')?>;border:1px solid #eee;padding-left: 10px;"><span style="font-size:13px;color:black;">Product Name</span></td>
													<td class="text-uppercase" style="text-align: left !important;width: 16%;border:1px solid #eee;padding-left: 10px;"><span style="font-size:13px;color:black;">Invoice Number</span></td>
													<td class="text-uppercase" style="text-align: left !important;width: <?=(((in_array('Payment Term', $newans)))?'11%':'14%')?>;border:1px solid #eee;padding-left: 10px;"><span style="font-size:13px;color:black;">Invoice Date</span></td>
													<td class="text-uppercase" style="text-align: right !important;width: 10%;border:1px solid #eee;padding-right: 10px;"><span style="font-size:13px;color:black;">Quantity</span></td>
													<td class="text-uppercase" style="text-align: right !important;width: 13%;border:1px solid #eee;padding-right: 10px;"><span style="font-size:13px;color:black;">Amount</span></td>
													<td class="text-uppercase" style="<?=(((in_array('Payment Term', $newans)))?'width:13%;':'display: none;')?>border:1px solid #eee;padding-left: 10px;"><span style="font-size:13px;color:black;">Term</span></td>
												</tr>
											</thead>
											<tbody id="myTables">
											</tbody>
...
const pagechanges = (page, pageSize) => {
	...
	ajaxRequestview = $.ajax({
		type: "GET",
		url: 'reportviewsearch.php?term='+perpages+'&limitings='+($('#limitforpagenum').val())+'&datefrom=<?=$datefrom?>&dateto=<?=$dateto?>&dif=saledetails&lastornone='+lastornone+'&productid=<?=$_GET['productid']?>&timefrom=<?=$timefrom?>&timeto=<?=$timeto?>&payterm=<?=$_GET['payterm']?>',
...
$(function() {
	...
	function(start, end, label) {
		...
		window.open("reportsaledetails.php?datefrom="+datefrom+"&dateto="+dateto+"&timefrom="+timefromonspots+"&timeto="+timetoonspots+"&payterm=<?=$_GET['payterm']?>&productid=<?=$_GET['productid']?>","_self");
	});
...
function converHTMLFileToPDF() {
	...
	var finaltimer = setTimeout(function(){
		...
		doc.autoTable({
			...
			columnStyles: { 
			<?php
				if ((in_array('Sno', $newanscheck))&&(in_array('Payment Term', $newanscheck))){
			?>
				0: { halign: 'left', cellWidth: 39},
				1: { halign: 'left', cellWidth: 101},
				2: { halign: 'right', cellWidth: 70},
				3: { halign: 'right', cellWidth: 70},
				4: { halign: 'right', cellWidth: 87},
				5: { halign: 'right', cellWidth: 87},
				6: { halign: 'right', cellWidth: 60}
			<?php
				}
				elseif (in_array('Sno', $newanscheck)) {
			?>
				0: { halign: 'left', cellWidth: 39},
				1: { halign: 'left', cellWidth: 161},
				2: { halign: 'right', cellWidth: 70},
				3: { halign: 'right', cellWidth: 70},
				4: { halign: 'right', cellWidth: 87},
				5: { halign: 'right', cellWidth: 87}
			<?php
				...
				elseif (in_array('Payment Term', $newanscheck)) {
			?>
		...
		$.ajax({
			type: "GET",
			url: 'reporthistory.php?types=saledetails&titles=Sales Details&printdownpdfcsv=Printed PDF&fromto='+fromtopdf+'',
			success: function (result) {
				console.log(result);
			},
			error: function (error) {
...
function downloadaspdf() {
	...
	var finaltimer = setTimeout(function(){
		...
		$.ajax({
			type: "GET",
			url: 'reporthistory.php?types=saledetails&titles=Sales Details&printdownpdfcsv=Downloaded PDF&fromto='+fromtopdf+'',
			success: function (result) {
				console.log(result);
			},
			error: function (error) {
		...
		doc.save('Sales-Details-<?=$datefrom?>-To-<?=$dateto?>');
...
function downloadascsv() {
	...
	var finaltimer = setTimeout(function(){
		...
		link.setAttribute("download", "Sales-Details-<?=$datefrom?>-To-<?=$dateto?>");
		document.body.appendChild(link);
		link.click();
		$.ajax({
			type: "GET",
			url: 'reporthistory.php?types=saledetails&titles=Sales Details&printdownpdfcsv=Downloaded CSV&fromto='+fromtopdf+'',
			success: function (result) {
				console.log(result);
			},
			error: function (error) {
...
document.addEventListener('DOMContentLoaded', function() {
	...
	function makeAjaxRequest() {
		ajaxRequest = $.ajax({
			type: "GET",
			url: 'check.php?term=saledetails&datefrom=<?=$datefrom?>&dateto=<?=$dateto?>&limitfrom=0&limitto=10000&timefrom=<?=$timefrom?>&timeto=<?=$timeto?>&payterm=<?=$_GET['payterm']?>&productid=<?=$_GET['productid']?>',
			success: function (result) {
				if (result != mytabprev) {
