...
if(isset($_POST['adjustmentno']))
{
	...
	$adjustmentamount=mysqli_real_escape_string($con, $_POST['adjustmentamount']);
	$paidamount=mysqli_real_escape_string($con, $_POST['paidamount']);
	$paymentterm=mysqli_real_escape_string($con, $_POST['paymentterm']);
	$sql=mysqli_query($con, "update pairadjustments set cancelstatus='$cancelstatus' where adjustmentdate='$adjustmentdate' and adjustmentno='$adjustmentno'");
	/* $sql2=mysqli_query($con, "update pairadjustmentspayments set cancelstatus='$cancelstatus' where receiptdate='$adjustmentdate' and receiptno='$adjustmentno'"); */
	if($sql)
	{
...
$sqlp=mysqli_query($con, "select productid, quantity from pairadjustments where createdid='$companymainid' and  adjustmentno='$adjustmentno' and adjustmentdate='$adjustmentdate'");
	...
	if($paidamount!='0')
{
$sqlq=mysqli_query($con, "select customerid, customername from pairadjustments where createdid='$companymainid' and  adjustmentno='$adjustmentno' and adjustmentdate='$adjustmentdate'");	
...
if($sql)
{
	...
	if($sqle)
	{
if((float)$adjustmentamount==(float)$paidamount)
{
	$paidstatus='1';
}
else
