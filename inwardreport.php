...
if (isset($_POST['submitcustomizes'])) {
	...
	$sqlreportmoduless=$con->prepare("SELECT * FROM pairreportmodules WHERE types='inw' ORDER BY id ASC");
	...
	$sqlupreport = $con->prepare("UPDATE pairreports SET rowcolumns=? WHERE franchiseid=? AND createdid=? AND types='inw'");
	...
	$sqlreportfav = $con->prepare("UPDATE pairreportfavourites SET reporturl=? WHERE franchisesession=? AND createdid=? AND reportnames='inward'");
	$sqlreportfav->bind_param("sss", $newReportUrl, $_SESSION['franchisesession'], $companymainid);
	$sqlreportfav->execute();
	$sqlreportfav->close();

	$sqlreport = $con->prepare("UPDATE pairreports SET reportperiod=?,filtername=?,companyname=?,dateprepare=?,timeprepare=?,gstrule=? WHERE franchiseid=? AND createdid=? AND types='inw'");
	$sqlreport->bind_param("ssssssss", $reportperiod, $filtername, $companyname, $dateprepare, $timeprepare, $gstrule, $_SESSION['franchisesession'], $companymainid);
	$sqlreport->execute();
	$sqlreport->close();

	$sqlreportcon = $con->prepare("UPDATE pairreports SET gstrule=? WHERE franchiseid=? AND createdid=? AND (types='inwreg' OR types='inwcon')");
	$sqlreportcon->bind_param("sss", $gstrule, $_SESSION['franchisesession'], $companymainid);
	$sqlreportcon->execute();
	$sqlreportcon->close();

	header("Location:inwardreport.php?datefrom=".date($dateformatforurlredirection,strtotime($from))."&dateto=".date($dateformatforurlredirection,strtotime($to))."");
}

$sqlreportviews=$con->prepare("SELECT * FROM pairreports WHERE franchiseid=? AND createdid=? AND types='inw'");
									...
									<div class="row">
										...
										<div class="col-lg-6">
											<span style="float:right">
																			...
																			<div id="customizerowcolumns" class="accordion-collapse collapse show" aria-labelledby="customizerowcolumn">
																				<div class="accordion-body text-sm">
																					<div class="row justify-content-center">
																						...
																						<?php
																							$newans=array();
																							$sqlreportaccesss=$con->prepare("SELECT * FROM pairreports WHERE franchiseid=? AND createdid=? AND types='inw'");
																								...
																								<div class="col-lg-6 my-1" style="padding-left: 0px !important;">
																									<div class="custom-control custom-checkbox mr-sm-2" style="padding: 0px 12px !important;">
																										<div class="input-group input-group-sm" id="flaghead">
																											...
																											<input type="checkbox" class="custom-control-input" name="<?= $coltypemod; ?>inw" id="<?= $coltypemod; ?>inw" checked disabled>
																											<label class="custom-control-label custom-label" for="<?= $coltypemod; ?>inw">
																												<?= str_replace(" or ", " / ",str_replace("Bill",$infomainaccessuserbill['modulename'],$newmoduleskey)) ?>
																											</label>
																										</div>
																									</div>
																								</div>
												...
												<a style="margin:4.5px 4.5px !important;" id="dropdownMenuButton1" data-bs-toggle="dropdown" aria-expanded="false" class="btn btn-primary btn-sm btn-custom-grey" onclick="downloadpdf()">
											...
											<div class="tab-pane fade" id="nav-profile" role="tabpanel" aria-labelledby="nav-profile-tab">
												<div class="table-responsive m-3" id="histables">
													<table class="table table-bordered">
														...
														<tbody>
      												<?php
															$sqluse=$con->prepare("SELECT * FROM pairusehistory WHERE usetype = 'REPinw' AND useid = ? ORDER BY createdon DESC");
												...
												<div class="container mt-1 mb-3">
													<div class="row d-flex justify-content-center mt-5">
														<div class="col-lg-8 col-md-12 justify-content-center">
																			...
																			<td width="100%" style="padding:10px;">
																				<div style="height: 975px;">
																					<table id="print-are1" style="border: 1px solid #eee;" width="100%">
																						<thead>
																							<tr>
																								<td class="text-uppercase" style="width:40%;border:1px solid #eee;padding-left: 10px;">
																									<span style="font-size:13px;color:black;">
																										Description
																									</span>
																								</td>
																								<td class="text-uppercase" style="text-align: right !important;width: 14%;border:1px solid #eee;padding-right: 10px;">
																									<span style="font-size:13px;color:black;">
																										IGST Amount
																									</span>
																								</td>
																								<td class="text-uppercase" style="text-align: right !important;width: 15%;border:1px solid #eee;padding-right: 10px;">
																									<span style="font-size:13px;color:black;">
																										CGST Amount
																									</span>
																								</td>
																								...
																								<td class="text-uppercase" style="text-align: right !important;width: 16%;border:1px solid #eee;padding-right: 10px;">
																									<span style="font-size:13px;color:black;">
																										<?=$infomainaccessuserbill["modulename"]?> Total
																									</span>
																								</td>
																							</tr>
																						</thead>
																						<tbody id="myTable">
																						<?php
																							$sqlregisters=$con->prepare("SELECT * FROM pairbills WHERE franchisesession=? AND createdid=? AND (billdate>=? AND billdate<=?) AND cancelstatus='0' AND ".(($sqlviewreport['gstrule']=='0')?'gstrtype="Registered Business - Regular"':'gstrtype="Registered Business - Regular" and gstno!="" ')." GROUP BY billdate, billno");
										      											$sqlregisters->bind_param("ssss", $_SESSION['franchisesession'], $companymainid, $datefrom, $dateto);
										      											$sqlregisters->execute();
										      											$sqlregister = $sqlregisters->get_result();
																							$registerigst = 0;
																							$registercgst = 0;
																							$registersgst = 0;
																							$registerbilltotal = 0;
																							while($register=$sqlregister->fetch_array()){
																								$anstax = $register['tax'];
																							...
																							if($registerbilltotal!=0){
																						?>
																							<tr onclick="window.open('inwardreportregistered.php?datefrom=<?=$datefrom?>&dateto=<?=$dateto?>','_self')">
																								<td data-label="DESCRIPTION" style="border:1px solid #eee;padding-left: 10px;">
																									<span style="display: inline-flex;max-width: 295px;min-width: 295px;white-space: nowrap;overflow: hidden;color: royalblue;">
																										Purchases Received From Registered taxpayers
																									</span>
																								</td>
																								<td data-label="IGST AMOUNT" style="text-align: right !important;border:1px solid #eee;padding-right: 10px;">
																									<span style="display: inline-flex;max-width: 90px;min-width: 90px;white-space: nowrap;overflow: hidden;justify-content: end;<?= (($registerigst>0)?'color:royalblue;':'color:black;') ?>">
																										<?= (($registerigst==0)?number_format((float)0,2,'.',','):number_format((float)$registerigst,2,'.',',')) ?>
																									</span>
																								</td>
																								<td data-label="CGST AMOUNT" style="text-align: right !important;border:1px solid #eee;padding-right: 10px;">
																									<span style="display: inline-flex;max-width: 90px;min-width: 90px;white-space: nowrap;overflow: hidden;justify-content: end;<?= (($registercgst>0)?'color:royalblue;':'color:black;') ?>">
																										<?= (($registercgst==0)?number_format((float)0,2,'.',','):number_format((float)$registercgst,2,'.',',')) ?>
																									</span>
																								</td>
																								<td data-label="SGST AMOUNT" style="text-align: right !important;border:1px solid #eee;padding-right: 10px;">
																									<span style="display: inline-flex;max-width: 90px;min-width: 90px;white-space: nowrap;overflow: hidden;justify-content: end;<?= (($registersgst>0)?'color:royalblue;':'color:black;') ?>">
																										<?= (($registersgst==0)?number_format((float)0,2,'.',','):number_format((float)$registersgst,2,'.',',')) ?>
																									</span>
																								</td>
																								<td data-label="<?=$infomainaccessuserbill["modulename"]?> TOTAL" style="text-align: right !important;border:1px solid #eee;padding-right: 10px;">
																									<span style="display: inline-flex;max-width: 90px;min-width: 90px;white-space: nowrap;overflow: hidden;justify-content: end;<?= (($registerbilltotal>0)?'color:royalblue;':'color:black;') ?>">
																										<?= (($registerbilltotal==0)?number_format((float)0,2,'.',','):number_format((float)$registerbilltotal,2,'.',',')) ?>
																									</span>
																								</td>
																							</tr>
																						<?php
																							}
																							$sqlconsumers=$con->prepare("SELECT * FROM pairbills WHERE franchisesession=? AND createdid=? AND (billdate>=? AND billdate<=?) AND cancelstatus='0' AND ".(($sqlviewreport['gstrule']=='0')?'gstrtype="Consumer"':'gstrtype="Registered Business - Regular" and gstno="" ')." GROUP BY billdate, billno");
										      											$sqlconsumers->bind_param("ssss", $_SESSION['franchisesession'], $companymainid, $datefrom, $dateto);
										      											$sqlconsumers->execute();
										      											$sqlconsumer = $sqlconsumers->get_result();
																							$consumerigst = 0;
																							$consumercgst = 0;
																							$consumersgst = 0;
																							$consumerbilltotal = 0;
																							while($consumer=$sqlconsumer->fetch_array()){
																								$anstax = $consumer['tax'];
																							...
																							if($consumerbilltotal!=0){
																						?>
																							<tr onclick="window.open('inwardreportconsumer.php?datefrom=<?=$datefrom?>&dateto=<?=$dateto?>','_self')">
																								<td data-label="DESCRIPTION" style="border:1px solid #eee;padding-left: 10px;">
																									<span style="display: inline-flex;max-width: 295px;min-width: 295px;white-space: nowrap;overflow: hidden;color: royalblue;">
																										Purchases Received From Unregistered (Consumer) taxpayers
																									</span>
																								</td>
																								<td data-label="IGST AMOUNT" style="text-align: right !important;border:1px solid #eee;padding-right: 10px;">
																									<span style="display: inline-flex;max-width: 90px;min-width: 90px;white-space: nowrap;overflow: hidden;justify-content: end;<?= (($consumerigst>0)?'color:royalblue;':'color:black;') ?>">
																										<?= (($consumerigst==0)?number_format((float)0,2,'.',','):number_format((float)$consumerigst,2,'.',',')) ?>
																									</span>
																								</td>
																								<td data-label="CGST AMOUNT" style="text-align: right !important;border:1px solid #eee;padding-right: 10px;">
																									<span style="display: inline-flex;max-width: 90px;min-width: 90px;white-space: nowrap;overflow: hidden;justify-content: end;<?= (($consumercgst>0)?'color:royalblue;':'color:black;') ?>">
																										<?= (($consumercgst==0)?number_format((float)0,2,'.',','):number_format((float)$consumercgst,2,'.',',')) ?>
																									</span>
																								</td>
																								<td data-label="SGST AMOUNT" style="text-align: right !important;border:1px solid #eee;padding-right: 10px;">
																									<span style="display: inline-flex;max-width: 90px;min-width: 90px;white-space: nowrap;overflow: hidden;justify-content: end;<?= (($consumersgst>0)?'color:royalblue;':'color:black;') ?>">
																										<?= (($consumersgst==0)?number_format((float)0,2,'.',','):number_format((float)$consumersgst,2,'.',',')) ?>
																									</span>
																								</td>
																								<td data-label="<?=$infomainaccessuserbill["modulename"]?> TOTAL" style="text-align: right !important;border:1px solid #eee;padding-right: 10px;">
																									<span style="display: inline-flex;max-width: 90px;min-width: 90px;white-space: nowrap;overflow: hidden;justify-content: end;<?= (($consumerbilltotal>0)?'color:royalblue;':'color:black;') ?>">
																										<?= (($consumerbilltotal==0)?number_format((float)0,2,'.',','):number_format((float)$consumerbilltotal,2,'.',',')) ?>
																									</span>
																								</td>
																							</tr>
																						<?php
																							}
																							$sqlhsns=$con->prepare("SELECT * FROM pairbills WHERE franchisesession=? AND createdid=? AND (billdate>=? AND billdate<=?) AND cancelstatus='0' GROUP BY billdate, billno");
										      											$sqlhsns->bind_param("ssss", $_SESSION['franchisesession'], $companymainid, $datefrom, $dateto);
										      											$sqlhsns->execute();
										      											$sqlhsn = $sqlhsns->get_result();
																							$hsnigst = 0;
																							$hsncgst = 0;
																							$hsnsgst = 0;
																							$hsnbilltotal = 0;
																							while($hsn=$sqlhsn->fetch_array()){
																								$anstax = $hsn['tax'];
																								$anscgst = $hsn['cgst'];
																								$anssgst = $hsn['sgst'];
																								$ansigst = $hsn['igst'];
																							...
																							if($hsnbilltotal!=0){
																						?>
																							<tr onclick="window.open('inwardreporthsn.php?datefrom=<?=$datefrom?>&dateto=<?=$dateto?>','_self')">
																								<td data-label="DESCRIPTION" style="border:1px solid #eee;padding-left: 10px;">
																									<span style="display: inline-flex;max-width: 295px;min-width: 295px;white-space: nowrap;overflow: hidden;color: royalblue;">
																								...
																								<td data-label="IGST AMOUNT" style="text-align: right !important;border:1px solid #eee;padding-right: 10px;">
																									<span style="display: inline-flex;max-width: 90px;min-width: 90px;white-space: nowrap;overflow: hidden;justify-content: end;<?= (($hsnigst>0)?'color:royalblue;':'color:black;') ?>">
																										<?= (($hsnigst==0)?number_format((float)0,2,'.',','):number_format((float)$hsnigst,2,'.',',')) ?>
																									</span>
																								</td>
																								<td data-label="CGST AMOUNT" style="text-align: right !important;border:1px solid #eee;padding-right: 10px;">
																									<span style="display: inline-flex;max-width: 90px;min-width: 90px;white-space: nowrap;overflow: hidden;justify-content: end;<?= (($hsncgst>0)?'color:royalblue;':'color:black;') ?>">
																										<?= (($hsncgst==0)?number_format((float)0,2,'.',','):number_format((float)$hsncgst,2,'.',',')) ?>
																									</span>
																								</td>
																								<td data-label="SGST AMOUNT" style="text-align: right !important;border:1px solid #eee;padding-right: 10px;">
																									<span style="display: inline-flex;max-width: 90px;min-width: 90px;white-space: nowrap;overflow: hidden;justify-content: end;<?= (($hsnsgst>0)?'color:royalblue;':'color:black;') ?>">
																										<?= (($hsnsgst==0)?number_format((float)0,2,'.',','):number_format((float)$hsnsgst,2,'.',',')) ?>
																									</span>
																								</td>
																								<td data-label="<?=$infomainaccessuserbill["modulename"]?> TOTAL" style="text-align: right !important;border:1px solid #eee;padding-right: 10px;">
																									<span style="display: inline-flex;max-width: 90px;min-width: 90px;white-space: nowrap;overflow: hidden;justify-content: end;<?= (($hsnbilltotal>0)?'color:royalblue;':'color:black;') ?>">
																										<?= (($hsnbilltotal==0)?number_format((float)0,2,'.',','):number_format((float)$hsnbilltotal,2,'.',',')) ?>
																									</span>
																								</td>
																							</tr>
																						<?php
																							...
																							if ($registerbilltotal==0) {
																						?>
																							<div style="text-align: center;position: relative;top: 75px;margin-bottom: -25px;">
																								There are no transactions in registered during the selected date range
																							</div>
																						<?php
																							}
																							if ($consumerbilltotal==0) {
																						?>
																							<div style="text-align: center;position: relative;top: 106px;margin-bottom: -25px;">
																								There are no transactions in consumer during the selected date range
																							</div>
																						<?php
																							}
																							if ($hsnbilltotal==0) {
																						?>
																							<div style="text-align: center;position: relative;top: 139px;margin-bottom: -25px;">
																								There are no transactions in hsn during the selected date range
																							</div>
																						<?php
																							}
																						?>
															...
															<input type="hidden" value="1" id="totalpagenums">
										...
										<table id="print-are3" style="border: 1px solid #eee;display: none;" width="100%">
											<thead>
												<tr>
													...
													<td class="text-uppercase" style="text-align: right !important;width: 16%;border:1px solid #eee;padding-right: 10px;"><span style="font-size:13px;color:black;"> <?=$infomainaccessuserbill["modulename"]?> Total</span></td>
												</tr>
											</thead>
											<tbody id="myTables">
											</tbody>
...
const pagechanges = (page, pageSize) => {
	...
	ajaxRequestview = $.ajax({
		type: "GET",
		url: 'reportviewsearch.php?term='+perpages+'&limitings='+($('#limitforpagenum').val())+'&datefrom=<?=$datefrom?>&dateto=<?=$dateto?>&dif=inward',
...
$(function() {
	...
	function(start, end, label) {
		...
		window.open("inwardreport.php?datefrom="+datefrom+"&dateto="+dateto+"","_self");
	});
});
...
function converHTMLFileToPDF() {
	...
	var finaltimer = setTimeout(function(){
		...
		var header = function(data) {
			...
			var headtext1 = "Summary of Inward Supplies (GSTR-2)";
			var fontSize = doc.internal.getFontSize();
			var pageWidth = doc.internal.pageSize.width;
		...
		$.ajax({
			type: "GET",
			url: 'reporthistory.php?types=inw&titles=Summary of Inward Supplies (GSTR-2)&printdownpdfcsv=Printed PDF&fromto='+fromtopdf+'',
			success: function (result) {
				console.log(result);
			},
			error: function (error) {
...
function downloadaspdf() {
	...
	var finaltimer = setTimeout(function(){
		...
		$.ajax({
			type: "GET",
			url: 'reporthistory.php?types=inw&titles=Summary of Inward Supplies (GSTR-2)&printdownpdfcsv=Downloaded PDF&fromto='+fromtopdf+'',
			success: function (result) {
				console.log(result);
			},
			error: function (error) {
		...
		doc.save('Inward-Reports-<?=$datefrom?>-To-<?=$dateto?>');
...
function downloadascsv() {
	...
	var finaltimer = setTimeout(function(){
		...
		data.push(["Summary of Inward Supplies (GSTR-2)"]);
		data.push([fromtopdf]);
		...
		link.setAttribute("download", "Inward-Reports-<?=$datefrom?>-To-<?=$dateto?>");
		document.body.appendChild(link);
		link.click();
		$.ajax({
			type: "GET",
			url: 'reporthistory.php?types=inw&titles=Summary of Inward Supplies (GSTR-2)&printdownpdfcsv=Downloaded CSV&fromto='+fromtopdf+'',
			success: function (result) {
				console.log(result);
			},
			error: function (error) {
...
document.addEventListener('DOMContentLoaded', function() {
	...
	function makeAjaxRequest() {
		ajaxRequest = $.ajax({
			type: "GET",
			url: 'check.php?term=inward&datefrom=<?=$datefrom?>&dateto=<?=$dateto?>&limitfrom=0&limitto=10000',
			success: function (result) {
				if (result != mytabprev) {
