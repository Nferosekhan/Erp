...
$sqlismainaccessusercuss=$con->prepare("SELECT * FROM pairmainaccess WHERE franchiseid=? AND moduletype='Vendors' ORDER BY id ASC");
...
if (isset($_POST['submitcustomizes'])) {
	...
	$sqlreportmoduless=$con->prepare("SELECT * FROM pairreportmodules WHERE types='paymade' ORDER BY id ASC");
	...
	foreach ($newmodules as $newmoduleskey) {
		...
		$modcolumncol=$coltypemod."paymade";
	...
	$sqlupreport = $con->prepare("UPDATE pairreports SET rowcolumns=? WHERE franchiseid=? AND createdid=? AND types='paymade'");
	...
	$customername = '';
	if($names!='all'){
		$sqlicusnames=$con->prepare("SELECT customername FROM paircustomers WHERE franchisesession=? AND createdid=? AND moduletype='Vendors' AND id=? ORDER BY customername ASC");
	...
	$sqlreportfav = $con->prepare("UPDATE pairreportfavourites SET reporturl=? WHERE franchisesession=? AND createdid=? AND reportnames='paymade'");
	$sqlreportfav->bind_param("sss", $newReportUrl, $_SESSION['franchisesession'], $companymainid);
	$sqlreportfav->execute();
	$sqlreportfav->close();

	$sqlreport = $con->prepare("UPDATE pairreports SET cusvenname=?,reportperiod=?,filtername=?,companyname=?,dateprepare=?,timeprepare=?,names=?,payterm=? WHERE franchiseid=? AND createdid=? AND types='paymade'");
	...
	header("Location:reportpaymade.php?names=".$names."&datefrom=".date($dateformatforurlredirection,strtotime($from))."&dateto=".date($dateformatforurlredirection,strtotime($to))."&timefrom=".$timefrom."&timeto=".$timeto."&payterm=".$payterm."");
}
																			...
																			<div id="customizerowcolumns" class="accordion-collapse collapse show" aria-labelledby="customizerowcolumn">
																				<div class="accordion-body text-sm">
																					<div class="row justify-content-center">
																						...
																						<?php
																							$newans=array();
																							$sqlreportaccesss=$con->prepare("SELECT * FROM pairreports WHERE franchiseid=? AND createdid=? AND types='paymade'");
																								...
																								<div class="col-lg-6 my-1" style="padding-left: 0px !important;">
																									<div class="custom-control custom-checkbox mr-sm-2" style="padding: 0px 12px !important;">
																										<div class="input-group input-group-sm" id="flaghead">
																											...
																											<input type="checkbox" class="custom-control-input" name="<?= $coltypemod; ?>paymade" id="<?= $coltypemod; ?>paymade" <?=((($newmoduleskey=='Sno')||($newmoduleskey=='Payment Term')||($newmoduleskey=='City')||($newmoduleskey=='Vendor Name')||($newmoduleskey=='Balance Due'))?(((in_array($newmoduleskey, $newans)))?'checked':''):'checked')?> <?=((($newmoduleskey=='Sno')||($newmoduleskey=='Payment Term')||($newmoduleskey=='City')||($newmoduleskey=='Vendor Name')||($newmoduleskey=='Balance Due'))?'':'disabled')?>>
																											<label class="custom-control-label custom-label" for="<?= $coltypemod; ?>paymade">
																								...
																								<div class="col-lg-6 my-1">
																									<div class="custom-control custom-checkbox mr-sm-2">
																										...
																										<label class="custom-control-label custom-label" for="filtername">
																											<?=$infomainaccessusercus['modulename']?>(PRIVATE)
																										</label>
																									</div>                  
																								</div>
																								<div class="col-lg-6 automodalchecking">
																									<select class="form-control form-control-sm select4 reportvensearch" id="names" name="names">
																									<?php
																									...
																									<?php
																											...
																											$sqli = mysqli_query($con, "SELECT id, customername, billcity, mobile, workphone FROM paircustomers WHERE franchisesession='".$_SESSION["franchisesession"]."' and (createdid='$companymainid' and moduletype='Vendors') and id='".$sqlviewreport['names']."' order by customername asc");
											...
											<div class="tab-pane fade" id="nav-profile" role="tabpanel" aria-labelledby="nav-profile-tab">
												<div class="table-responsive m-3" id="histables">
													<table class="table table-bordered">
														...
														<tbody>
														<?php
															$sqluse=$con->prepare("SELECT * FROM pairusehistory WHERE usetype = 'REPpaymade' AND useid = ? ORDER BY createdon DESC");
																				...
																				<table id="print-are1" style="border: 1px solid #eee;" width="100%">
																					<thead style="background-color: #e1e1e1;">
																						<tr>
																							...
																							<td class="text-uppercase" style="border: 1px solid #eee;padding-left: 10px;max-width: 73px !important;min-width: 73px !important;white-space: nowrap;overflow: hidden;">
																								<span style="font-size:13px;color:black;">
																									Number
																								</span>
																							</td>
																							<td class="text-uppercase" style="<?=(((in_array('Vendor Name', $newans)))?'':'display: none;')?>border: 1px solid #eee;padding-left: 10px;max-width: 100px !important;min-width: 100px !important;white-space: nowrap;overflow: hidden;<?=((($sqlviewreport['filtername']=='1')||($sqlviewreport['filtername']=='0'))?'':'display:none;')?>">
																								<span style="font-size:13px;color:black;">
																					...
																					<tbody id="myTable">
																					<?php
																						$sqlinfos=$con->prepare("SELECT * FROM pairpurchasepayments WHERE franchisesession=? AND createdid=? AND (receiptdate>=? AND receiptdate<=?) ".(($sqlviewreport['names']=='all')?' ':' AND vendorid="'.$sqlviewreport['names'].'"')."".(($sqlviewreport['payterm']=='all')?' ':' AND paymentmode="'.$sqlviewreport['payterm'].'"')." ORDER BY id DESC LIMIT 36");
																					...
																					?>
																						<tr style="height: 30px;">
																							...
																							<td data-label="<?=$infomainaccessusercus['modulename']?> Name" style="<?=(((in_array('Vendor Name', $newans)))?'':'display: none;')?>border: 1px solid #eee;padding-left: 10px;font-size: 13px;<?=((($sqlviewreport['filtername']=='1')||($sqlviewreport['filtername']=='0'))?'':'display:none;')?>">
																							...
																							<td data-label="Payment Term" style="<?=(((in_array('Payment Term', $newans)))?'':'display: none;')?>border: 1px solid #eee;padding-left: 10px;font-size: 13px;">
																								<?=$info['paymentmode']?>
																							</td>
																						</tr>
																					<?php
																						...
																						if(mysqli_num_rows($sqlinfo)<36){
																							echo '<tr style="height: '.((36-mysqli_num_rows($sqlinfo))*30).'px;"><td data-label="Date" style="border: 1px solid #eee;padding-left: 10px;font-size: 13px;">&nbsp;</td><td data-label="Number" style="border: 1px solid #eee;padding-left: 10px;font-size: 13px;">&nbsp;</td><td data-label="'.($infomainaccessusercus['modulename']).' Name" style="'.((((in_array('Vendor Name', $newans)))?'':'display: none;')).'border: 1px solid #eee;padding-left: 10px;font-size: 13px;'.(((($sqlviewreport['filtername']=='1')||($sqlviewreport['filtername']=='0'))?'':'display:none;')).'">&nbsp;</td><td data-label="Total" style="border: 1px solid #eee;padding-right: 10px;text-align: right;">&nbsp;</td><td data-label="Payment Term" style="'.((((in_array('Payment Term', $newans)))?'':'display: none;')).'border: 1px solid #eee;padding-left: 10px;font-size: 13px;">&nbsp;</td></tr>';
																						}
																						$sqltotlistpages=$con->prepare("SELECT * FROM pairpurchasepayments WHERE franchisesession=? AND createdid=? AND (receiptdate>=? AND receiptdate<=?) ".(($sqlviewreport['names']=='all')?' ':' AND vendorid="'.$sqlviewreport['names'].'"')."".(($sqlviewreport['payterm']=='all')?' ':' AND paymentmode="'.$sqlviewreport['payterm'].'"')." ORDER BY id DESC");
																								...
																								?>
																									<td style="vertical-align:middle !important;text-align: center !important;padding-top: 7px !important;">
																										<div style="text-align:center;margin-top: -10px !important;font-size: 12px !important;<?=(($sqlviewreport['timeprepare']=='1'||$sqlviewreport['dateprepare']=='1')?'':'display: none;')?>">
																											<span>
																												Printed On : 
																												<?php echo date($date,strtotime($dates))?>
																											</span>
																										</div>
										...
										<table id="print-are3" style="border: 1px solid #eee;display: none;" width="100%">
											<thead>
												<tr>
													...
													<td class="text-uppercase" style="<?=(((in_array('Vendor Name', $newans)))?'':'display: none;')?>border: 1px solid #eee;padding-left: 10px;max-width: 100px !important;min-width: 100px !important;white-space: nowrap;overflow: hidden;<?=((($sqlviewreport['filtername']=='1')||($sqlviewreport['filtername']=='0'))?'':'display:none;')?>"><span style="font-size:13px;color:black;"><?=$infomainaccessusercus['modulename']?> Name</span></td>
													<td class="text-uppercase" style="border: 1px solid #eee;padding-right: 10px;max-width: 76px !important;min-width: 76px !important;white-space: nowrap;overflow: hidden;text-align: right;"><span style="font-size:13px;color:black;">Amount</span></td>
...
const pagechanges = (page, pageSize) => {
	...
	ajaxRequestview = $.ajax({
		type: "GET",
		url: 'reportviewsearch.php?term='+perpages+'&limitings='+($('#limitforpagenum').val())+'&datefrom=<?=$datefrom?>&dateto=<?=$dateto?>&dif=paymade&names=<?=$_GET['names']?>&timefrom=<?=$timefrom?>&timeto=<?=$timeto?>&payterm=<?=$_GET['payterm']?>&lastornone='+lastornone+'',
...
$(function() {
	...
	function(start, end, label) {
		...
		window.open("reportpaymade.php?names=<?=$_GET['names']?>&datefrom="+datefrom+"&dateto="+dateto+"&timefrom="+timefromonspots+"&timeto="+timetoonspots+"&payterm=<?=$_GET['payterm']?>","_self");
	});
...
function converHTMLFileToPDF() {
	...
	var finaltimer = setTimeout(function(){
		...
		doc.autoTable({ 
			...
			didDrawPage: header, 
		<?php
			if ((in_array('Vendor Name', $newans))&&(in_array('Payment Term', $newans))){
		?>
			columnStyles: { 
				0: { halign: 'left', cellWidth: 49},
				1: { halign: 'left', cellWidth: 80},
				2: { halign: 'left', cellWidth: 236},
				3: { halign: 'left', cellWidth: 60},
				4: { halign: 'left', cellWidth: 100}
			}
		<?php
			}
			elseif ((!in_array('Vendor Name', $newans))&&(in_array('Payment Term', $newans))){
		?>
			columnStyles: { 
		...
		?>
			columnStyles: { 
				0: { halign: 'left', cellWidth: 155},
				1: { halign: 'left', cellWidth: 190},
		...
		$.ajax({
			type: "GET",
			url: 'reporthistory.php?types=paymade&titles=Payments Made&printdownpdfcsv=Printed PDF&fromto='+fromtopdf+'',
			success: function (result) {
				console.log(result);
			},
			error: function (error) {
...
function downloadaspdf() {
	...
	var finaltimer = setTimeout(function(){
		...
		$.ajax({
			type: "GET",
			url: 'reporthistory.php?types=paymade&titles=Payments Made&printdownpdfcsv=Downloaded PDF&fromto='+fromtopdf+'',
			success: function (result) {
				console.log(result);
			},
			error: function (error) {
		...
		doc.save('Payments-Made-<?=$datefrom?>-To-<?=$dateto?>');
...
function downloadascsv() {
	...
	var finaltimer = setTimeout(function(){
		...
		link.setAttribute("download", "Payments-Made-Reports-<?=$datefrom?>-To-<?=$dateto?>");
		document.body.appendChild(link);
		link.click();
		$.ajax({
			type: "GET",
			url: 'reporthistory.php?types=paymade&titles=Payments Made&printdownpdfcsv=Downloaded CSV&fromto='+fromtopdf+'',
			success: function (result) {
				console.log(result);
			},
			error: function (error) {
...
document.addEventListener('DOMContentLoaded', function() {
	...
	function makeAjaxRequest() {
		ajaxRequest = $.ajax({
			type: "GET",
			url: 'check.php?term=paymade&datefrom=<?=$datefrom?>&dateto=<?=$dateto?>&limitfrom=0&limitto=10000&names=<?=$sqlviewreport['names']?>&timefrom=<?=$timefrom?>&timeto=<?=$timeto?>&payterm=<?=$sqlviewreport['payterm']?>',
			success: function (result) {
				if (result != mytabprev) {
