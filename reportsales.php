...
if (isset($_POST['submitcustomizes'])) {
	...
	$sqlreportmoduless=$con->prepare("SELECT * FROM pairreportmodules WHERE types='sales' ORDER BY id ASC");
	...
	$sqlupreport = $con->prepare("UPDATE pairreports SET rowcolumns=? WHERE franchiseid=? AND createdid=? AND types='sales'");
	...
	$sqlreportfav = $con->prepare("UPDATE pairreportfavourites SET reporturl=? WHERE franchisesession=? AND createdid=? AND reportnames='sales'");
	$sqlreportfav->bind_param("sss", $newReportUrl, $_SESSION['franchisesession'], $companymainid);
	$sqlreportfav->execute();
	$sqlreportfav->close();

	$sqlreport = $con->prepare("UPDATE pairreports SET reportperiod=?,filtername=?,companyname=?,dateprepare=?,timeprepare=?,gstrule=?,payterm=? WHERE franchiseid=? AND createdid=? AND types='sales'");
	...
	header("Location:reportsales.php?datefrom=".date($dateformatforurlredirection,strtotime($from))."&dateto=".date($dateformatforurlredirection,strtotime($to))."&timefrom=".$timefrom."&timeto=".$timeto."&payterm=".$payterm."");
}

$sqlreportviews=$con->prepare("SELECT * FROM pairreports WHERE franchiseid=? AND createdid=? AND types='sales'");
																			...
																			<div id="customizerowcolumns" class="accordion-collapse collapse show" aria-labelledby="customizerowcolumn">
																				<div class="accordion-body text-sm">
																					<div class="row justify-content-center">
																						...
																						<?php
																							$newans=array();
																							$sqlreportaccesss=$con->prepare("SELECT * FROM pairreports WHERE franchiseid=? AND createdid=? AND types='sales'");
																								...
																								<div class="col-lg-6 my-1" style="padding-left: 0px !important;">
																									<div class="custom-control custom-checkbox mr-sm-2" style="padding: 0px 12px !important;">
																										<div class="input-group input-group-sm" id="flaghead">
																											...
																											<input type="checkbox" class="custom-control-input" name="<?= $coltypemod; ?>sales" id="<?= $coltypemod; ?>sales" <?=((($newmoduleskey=='Sno')||($newmoduleskey=='Payment Term'))?(((in_array($newmoduleskey, $newans)))?'checked':''):'checked')?> <?=((($newmoduleskey=='Sno')||($newmoduleskey=='Payment Term'))?'':'disabled')?>>
																											<label class="custom-control-label custom-label" for="<?= $coltypemod; ?>sales">
											...
											<div class="tab-pane fade" id="nav-profile" role="tabpanel" aria-labelledby="nav-profile-tab">
												<div class="table-responsive m-3" id="histables">
													<table class="table table-bordered">
														...
														<tbody>
														<?php
															$sqluse=$con->prepare("SELECT * FROM pairusehistory WHERE usetype = 'REPsales' AND useid = ? ORDER BY createdon DESC");
																				...
																				<table id="print-are1" style="border: 1px solid #eee;" width="100%">
																					<thead>
																						<tr>
																							...
																							<td class="text-uppercase" style="width:42%;border:1px solid #eee;padding-left: 10px;">
																								<span style="font-size:13px;color:black;">
																									Product Name
																								</span>
																							</td>
																							...
																							<td class="text-uppercase" style="<?=(((in_array('Payment Term', $newans)))?'':'display: none;')?>width:20%;border:1px solid #eee;padding-left: 10px;">
																								<span style="font-size:13px;color:black;">
																									Term
																								</span>
																							</td>
																					...
																					<?php
																						$count=1;
																						$totalamount = 0;
																						$sqlsels=$con->prepare("SELECT invoiceterm,productid,productname,SUM(quantity) AS quantity,SUM(productvalue) AS productvalue FROM pairinvoices WHERE franchisesession=? AND createdid=? AND (invoicedate>=? AND invoicedate<=?) AND (invoicetime>=? AND invoicetime<=?) ".(($sqlviewreport['payterm']=='all')?' ':' AND invoiceterm="'.$sqlviewreport['payterm'].'"')." GROUP BY productid LIMIT 36");
																						$sqlsels->bind_param("ssssss", $_SESSION['franchisesession'], $companymainid, $datefrom, $dateto, $timefrom, $timeto);
																						$sqlsels->execute();
																						$sqlsel = $sqlsels->get_result();
																						while($fetsql=$sqlsel->fetch_array()){
																							$totalamount += $fetsql['productvalue'];
																					?>
																						<tr style="height: 30px;" onclick="window.open('reportsaledetails.php?datefrom=<?=$datefrom?>&dateto=<?=$dateto?>&productid=<?=$fetsql['productid']?>&timefrom=<?=$timefrom?>&timeto=<?=$timeto?>&payterm=<?=$_GET['payterm']?>','_self')">
																							...
																							<td  data-label="Payment Term" style="<?=(((in_array('Payment Term', $newans)))?'':'display: none;')?>border: 1px solid #eee;padding-left: 10px;font-size: 13px;">
																								<?=(($sqlviewreport['payterm']=='all')?'All':$fetsql['invoiceterm'])?>
																							</td>
																						</tr>
																					<?php
																							$count++;
																						...
																						<tr style="height: 30px;">
																							<td  data-label="Total" style="border: 1px solid #eee;padding-left: 10px;font-size: 13px;" colspan="<?=(((in_array('Sno', $newans)))?'3':'2')?>">
																								Total
																							</td>
																							<td  data-label="Total" style="border: 1px solid #eee;padding-left: 10px;font-size: 13px;text-align: right;">
																								<?=number_format($totalamount,2,'.',',')?>
																							</td>
																							<td  data-label="Total" style="<?=(((in_array('Payment Term', $newans)))?'':'display: none;')?>border: 1px solid #eee;padding-left: 10px;font-size: 13px;"></td>
																						</tr>
																						<?php
																						}
																						$chekcs=0;
										...
										<table id="print-are3" style="border: 1px solid #eee;display: none;" width="100%">
											<thead>
												<tr>
													...
													<td class="text-uppercase" style="width:42%;border:1px solid #eee;padding-left: 10px;"><span style="font-size:13px;color:black;">Product Name</span></td>
													<td class="text-uppercase" style="text-align: right !important;width: 14%;border:1px solid #eee;padding-right: 10px;"><span style="font-size:13px;color:black;">Quantity</span></td>
													<td class="text-uppercase" style="text-align: right !important;width: 15%;border:1px solid #eee;padding-right: 10px;"><span style="font-size:13px;color:black;">Amount</span></td>
													<td class="text-uppercase" style="<?=(((in_array('Payment Term', $newans)))?'':'display: none;')?>width:20%;border:1px solid #eee;padding-left: 10px;"><span style="font-size:13px;color:black;">Term</span></td>
												</tr>
											</thead>
											<tbody id="myTables">
											</tbody>
...
const pagechanges = (page, pageSize) => {
	...
	ajaxRequestview = $.ajax({
		type: "GET",
		url: 'reportviewsearch.php?term='+perpages+'&limitings='+($('#limitforpagenum').val())+'&datefrom=<?=$datefrom?>&dateto=<?=$dateto?>&dif=sales&timefrom=<?=$timefrom?>&timeto=<?=$timeto?>&payterm=<?=$_GET['payterm']?>&lastornone='+lastornone+'',
...
$(function() {
	...
	function(start, end, label) {
		...
		window.open("reportsales.php?datefrom="+datefrom+"&dateto="+dateto+"&timefrom="+timefromonspots+"&timeto="+timetoonspots+"&payterm=<?=$_GET['payterm']?>","_self");
	});
...
function converHTMLFileToPDF() {
	...
	var finaltimer = setTimeout(function(){
		...
		$.ajax({
			type: "GET",
			url: 'reporthistory.php?types=sales&titles=Sales&printdownpdfcsv=Printed PDF&fromto='+fromtopdf+'',
			success: function (result) {
				console.log(result);
			},
			error: function (error) {
...
function downloadaspdf() {
	...
	var finaltimer = setTimeout(function(){
		...
		$.ajax({
			type: "GET",
			url: 'reporthistory.php?types=sales&titles=Sales&printdownpdfcsv=Downloaded PDF&fromto='+fromtopdf+'',
			success: function (result) {
				console.log(result);
			},
			error: function (error) {
		...
		doc.save('Sales-<?=$datefrom?>-To-<?=$dateto?>');
...
function downloadascsv() {
	...
	var finaltimer = setTimeout(function(){
		...
		link.setAttribute("download", "Sales-<?=$datefrom?>-To-<?=$dateto?>");
		document.body.appendChild(link);
		link.click();
		$.ajax({
			type: "GET",
			url: 'reporthistory.php?types=sales&titles=Sales&printdownpdfcsv=Downloaded CSV&fromto='+fromtopdf+'',
			success: function (result) {
				console.log(result);
			},
			error: function (error) {
...
document.addEventListener('DOMContentLoaded', function() {
	...
	function makeAjaxRequest() {
		ajaxRequest = $.ajax({
			type: "GET",
			url: 'check.php?term=sales&datefrom=<?=$datefrom?>&dateto=<?=$dateto?>&limitfrom=0&limitto=10000&timefrom=<?=$timefrom?>&timeto=<?=$timeto?>&payterm=<?=$_GET['payterm']?>',
			success: function (result) {
				if (result != mytabprev) {
