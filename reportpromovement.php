...
if (isset($_POST['submitcustomizes'])) {
	...
	$sqlreportmoduless=$con->prepare("SELECT * FROM pairreportmodules WHERE types='promovement' ORDER BY id ASC");
	...
	foreach ($newmodules as $newmoduleskey) {
		...
		$modcolumncol=$coltypemod."promovement";
	...
	$sqlupreport = $con->prepare("UPDATE pairreports SET rowcolumns=? WHERE franchiseid=? AND createdid=? AND types='promovement'");
	...
	$sqlreportfav = $con->prepare("UPDATE pairreportfavourites SET reporturl=? WHERE franchisesession=? AND createdid=? AND reportnames='promovement'");
	$sqlreportfav->bind_param("sss", $newReportUrl, $_SESSION['franchisesession'], $companymainid);
	$sqlreportfav->execute();
	$sqlreportfav->close();

	$sqlreport = $con->prepare("UPDATE pairreports SET proid=?,proname=?,reportperiod=?,filtername=?,companyname=?,dateprepare=?,timeprepare=?,gstrule=? WHERE franchiseid=? AND createdid=? AND types='promovement'");
	$sqlreport->bind_param("ssssssssss", $proid, $proname, $reportperiod, $filtername, $companyname, $dateprepare, $timeprepare, $gstrule, $_SESSION['franchisesession'], $companymainid);
	$sqlreport->execute();
	$sqlreport->close();

	header("Location:reportpromovement.php?datefrom=".date($dateformatforurlredirection,strtotime($from))."&dateto=".date($dateformatforurlredirection,strtotime($to))."&proid=".$proid."");
}

$sqlreportviews=$con->prepare("SELECT * FROM pairreports WHERE franchiseid=? AND createdid=? AND types='promovement'");
																			...
																			<div id="customizerowcolumns" class="accordion-collapse collapse show" aria-labelledby="customizerowcolumn">
																				<div class="accordion-body text-sm">
																					<div class="row justify-content-center">
																						...
																						<?php
																							$newans=array();
																							$sqlreportaccesss=$con->prepare("SELECT * FROM pairreports WHERE franchiseid=? AND createdid=? AND types='promovement'");
																								...
																								<div class="col-lg-6 my-1" style="padding-left: 0px !important;">
																									<div class="custom-control custom-checkbox mr-sm-2" style="padding: 0px 12px !important;">
																										<div class="input-group input-group-sm" id="flaghead">
																											...
																											<input type="checkbox" class="custom-control-input" name="<?= $coltypemod; ?>promovement" id="<?= $coltypemod; ?>promovement" checked disabled>
																											<label class="custom-control-label custom-label" for="<?= $coltypemod; ?>promovement">
											...
											<div class="tab-pane fade" id="nav-profile" role="tabpanel" aria-labelledby="nav-profile-tab">
												<div class="table-responsive m-3" id="histables">
													<table class="table table-bordered">
														...
														<tbody>
														<?php
															$sqluse=$con->prepare("SELECT * FROM pairusehistory WHERE usetype = 'REPpromovement' AND useid = ? ORDER BY createdon DESC");
																		...
																		<tr>
																			<td width="100%" style="height:10px !important;">
																				<table width="100%" style="text-align: center;font-weight: bold;">
																					...
																					<tr>
																						<td>
																							<?=(($sqlviewreport['proid']=='all')?'All':$sqlviewreport['proname'])?>
																						</td>
																					</tr>
																					<tr>
																						<td id="datefromto"></td>
																				...
																				<table id="print-are1" style="border: 1px solid #eee;" width="100%">
																					<thead>
																						<tr>
																							...
																							<td class="text-uppercase" style="width:21%;border:1px solid #eee;padding-left: 10px;">
																								<span style="font-size:13px;color:black;">
																									<?=$infomainaccessuserinvoice['modulename']?> Number
																								</span>
																							</td>
																							<td class="text-uppercase" style="width:28%;border:1px solid #eee;padding-left: 10px;">
																								<span style="font-size:13px;color:black;">
																					...
																					<?php
																						...
																						$purtotal = 0;
																						$allthetotal = 0;
																						$seldatas=$con->prepare("SELECT quantity AS quantities,'invoices' AS types,invoiceno AS numbers,invoicedate AS dates,productname AS productname,customername AS names FROM pairinvoices WHERE franchisesession=? AND createdid=? AND (invoicedate>=? AND invoicedate<=?) AND cancelstatus='0' ".(($sqlviewreport['proid']=='all')?' ':' AND productid="'.$sqlviewreport['proid'].'"')." UNION SELECT quantity AS quantities,'bills' AS types,billno AS numbers,billdate AS dates,productname AS productname,vendorname AS names FROM pairbills WHERE franchisesession=? AND createdid=? AND (billdate>=? AND billdate<=?) AND cancelstatus='0'".(($sqlviewreport['proid']=='all')?' ':' AND productid="'.$sqlviewreport['proid'].'"')." UNION SELECT newquantity AS quantities,'adjustments' AS types,privateid AS numbers,adjustmentdate AS dates,productname AS productname,createdby AS names FROM pairadjustments WHERE franchisesession=? AND createdid=? AND (adjustmentdate>=? AND adjustmentdate<=?) AND cancelstatus='0'".(($sqlviewreport['proid']=='all')?' ':' AND productid="'.$sqlviewreport['proid'].'"')." ORDER BY dates,numbers ASC LIMIT 35");
										      										$seldatas->bind_param("ssssssssssss", $_SESSION['franchisesession'], $companymainid, $datefrom, $dateto, $_SESSION['franchisesession'], $companymainid, $datefrom, $dateto, $_SESSION['franchisesession'], $companymainid, $datefrom, $dateto);
										      										$seldatas->execute();
																					...
																					?>
																						<tr style="vertical-align: middle; height: 30px !important;">
																							<td style="text-align: left !important; font-size:12px; color:black; border:1px solid #eee; padding-left: 10px;">
																								<?= date($datemainphp, strtotime($info['dates'])) ?>
																							</td>
																							<td style="text-align: left !important; font-size:12px; color:black; border:1px solid #eee; padding-left: 10px;">
																								<?= $info['numbers'] ?>
																							</td>
																							...
																							<td style="text-align: right !important; font-size:12px; color:black; border:1px solid #eee; padding-right: 10px;">
																								<?= (($info['types']=='adjustments')?$info['quantities']:'0') ?>
																							</td>
																							<td style="text-align: right !important; font-size:12px; color:black; border:1px solid #eee; padding-right: 10px;">
																								<?= (($info['types']=='invoices')?$info['quantities']:'0') ?>
																							</td>
																							<td style="text-align: right !important; font-size:12px; color:black; border:1px solid #eee; padding-right: 10px;">
																								<?= (($info['types']=='bills')?$info['quantities']:'0') ?>
																							</td>
																						</tr>
																					<?php
																						}
																						...
																						$sqltotlistpages=$con->prepare("SELECT quantity AS quantities,'invoices' AS types,invoiceno AS numbers,invoicedate AS dates,productname AS productname,customername AS names FROM pairinvoices WHERE franchisesession=? AND createdid=? AND (invoicedate>=? AND invoicedate<=?) AND cancelstatus='0' ".(($sqlviewreport['proid']=='all')?' ':' AND productid="'.$sqlviewreport['proid'].'"')." UNION SELECT quantity AS quantities,'bills' AS types,billno AS numbers,billdate AS dates,productname AS productname,vendorname AS names FROM pairbills WHERE franchisesession=? AND createdid=? AND (billdate>=? AND billdate<=?) AND cancelstatus='0'".(($sqlviewreport['proid']=='all')?' ':' AND productid="'.$sqlviewreport['proid'].'"')." UNION SELECT newquantity AS quantities,'adjustments' AS types,privateid AS numbers,adjustmentdate AS dates,productname AS productname,createdby AS names FROM pairadjustments WHERE franchisesession=? AND createdid=? AND (adjustmentdate>=? AND adjustmentdate<=?) AND cancelstatus='0'".(($sqlviewreport['proid']=='all')?' ':' AND productid="'.$sqlviewreport['proid'].'"')." ORDER BY dates,numbers ASC");
																						$sqltotlistpages->bind_param("ssssssssssss", $_SESSION['franchisesession'], $companymainid, $datefrom, $dateto, $_SESSION['franchisesession'], $companymainid, $datefrom, $dateto, $_SESSION['franchisesession'], $companymainid, $datefrom, $dateto);
																						...
																						if ($pagetotnumpage==1) {
																							$seldataanothers=$con->prepare("SELECT quantity AS quantities,'invoices' AS types,invoiceno AS numbers,invoicedate AS dates,productname AS productname,customername AS names FROM pairinvoices WHERE franchisesession=? AND createdid=? AND (invoicedate>=? AND invoicedate<=?) AND cancelstatus='0' ".(($sqlviewreport['proid']=='all')?' ':' AND productid="'.$sqlviewreport['proid'].'"')." UNION SELECT quantity AS quantities,'bills' AS types,billno AS numbers,billdate AS dates,productname AS productname,vendorname AS names FROM pairbills WHERE franchisesession=? AND createdid=? AND (billdate>=? AND billdate<=?) AND cancelstatus='0'".(($sqlviewreport['proid']=='all')?' ':' AND productid="'.$sqlviewreport['proid'].'"')." UNION SELECT newquantity AS quantities,'adjustments' AS types,privateid AS numbers,adjustmentdate AS dates,productname AS productname,createdby AS names FROM pairadjustments WHERE franchisesession=? AND createdid=? AND (adjustmentdate>=? AND adjustmentdate<=?) AND cancelstatus='0'".(($sqlviewreport['proid']=='all')?' ':' AND productid="'.$sqlviewreport['proid'].'"')." ORDER BY dates,numbers ASC");
																							$seldataanothers->bind_param("ssssssssssss", $_SESSION['franchisesession'], $companymainid, $datefrom, $dateto, $_SESSION['franchisesession'], $companymainid, $datefrom, $dateto, $_SESSION['franchisesession'], $companymainid, $datefrom, $dateto);
																							$seldataanothers->execute();
																							$seldataanother = $seldataanothers->get_result();
																							while($infoanother=$seldataanother->fetch_array()){
										...
										<table id="print-are3" style="border: 1px solid #eee;display: none;" width="100%">
											<thead>
												<tr>
													...
													<td class="text-uppercase" style="text-align: right !important;width: 15%;border:1px solid #eee;padding-right: 10px;"><span style="font-size:13px;color:black;white-space: nowrap;overflow: hidden;">Adjustment <?=$access['txtqtyinv']?></span></td>
													<td class="text-uppercase" style="text-align: right !important;width: 13%;border:1px solid #eee;padding-right: 10px;"><span style="font-size:13px;color:black;white-space: nowrap;overflow: hidden;">Sales <?=$access['txtqtyinv']?></span></td>
													<td class="text-uppercase" style="text-align: right !important;width: 13%;border:1px solid #eee;padding-right: 10px;"><span style="font-size:13px;color:black;white-space: nowrap;overflow: hidden;">Purchase <?=$access['txtqtyinv']?></span></td>
												</tr>
											</thead>
											<tbody id="myTables">
											</tbody>
...
const pagechanges = (page, pageSize) => {
	...
	ajaxRequestview = $.ajax({
		type: "GET",
		url: 'reportviewsearch.php?term='+perpages+'&limitings='+($('#limitforpagenum').val())+'&datefrom=<?=$datefrom?>&dateto=<?=$dateto?>&dif=promovement&proid=<?=$_GET['proid']?>&lastornone='+lastornone+'',
...
$(function() {
	...
	function(start, end, label) {
		...
		window.open("reportpromovement.php?datefrom="+datefrom+"&dateto="+dateto+"&proid=<?=$_GET['proid']?>","_self");
	});
});
...
function converHTMLFileToPDF() {
	...
	var finaltimer = setTimeout(function(){
		...
		var header = function(data) {
			...
			var headtext1 = "Product Movement";
			var fontSize = doc.internal.getFontSize();
			var pageWidth = doc.internal.pageSize.width;
		...
		$.ajax({
			type: "GET",
			url: 'reporthistory.php?types=promovement&titles=Product Movement&printdownpdfcsv=Printed PDF&fromto='+fromtopdf+'',
			success: function (result) {
				console.log(result);
			},
			error: function (error) {
...
function downloadaspdf() {
	...
	var finaltimer = setTimeout(function(){
		...
		$.ajax({
			type: "GET",
			url: 'reporthistory.php?types=promovement&titles=Product Movement&printdownpdfcsv=Downloaded PDF&fromto='+fromtopdf+'',
			success: function (result) {
				console.log(result);
			},
			error: function (error) {
		...
		doc.save('Product-Movement-<?=$datefrom?>-To-<?=$dateto?>');
...
function downloadascsv() {
	...
	var finaltimer = setTimeout(function(){
		...
		link.setAttribute("download", "Product-Movement-<?=$datefrom?>-To-<?=$dateto?>");
		document.body.appendChild(link);
		link.click();
		$.ajax({
			type: "GET",
			url: 'reporthistory.php?types=promovement&titles=Product Movement&printdownpdfcsv=Downloaded CSV&fromto='+fromtopdf+'',
			success: function (result) {
				console.log(result);
			},
			error: function (error) {
...
document.addEventListener('DOMContentLoaded', function() {
	...
	function makeAjaxRequest() {
		ajaxRequest = $.ajax({
			type: "GET",
			url: 'check.php?term=promovement&datefrom=<?=$datefrom?>&dateto=<?=$dateto?>&limitfrom=0&limitto=10000&proid=<?=$_GET['proid']?>',
			success: function (result) {
				if (result != mytabprev) {
