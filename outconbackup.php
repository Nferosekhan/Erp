...
if (isset($_POST['submitcustomizes'])) {
  ...
  $sqlreportmodules=mysqli_query($con, "select * from pairreportmodules where types='ouwcon' order by id  asc");
...
$modcolumncol=$coltypemod."ouwcon";
...
$sqlupreport = "update pairreports set rowcolumns='$modcolumncolchanges' where franchiseid='".$_SESSION['franchisesession']."' and types='ouwcon' and createdid='$companymainid'"; 
$sqlreportup = mysqli_query($con, $sqlupreport);
  ...
  $sqlreport = mysqli_query($con,"update pairreports set reportperiod='$reportperiod',filtername='$filtername',companyname='$companyname',dateprepare='$dateprepare',timeprepare='$timeprepare' where franchiseid='".$_SESSION['franchisesession']."' and types='ouwcon' and createdid='$companymainid'");
  header("Location:outwardreportconsumer.php?invfrom=".date($datenxt,strtotime($from))."&invto=".date($datenxt,strtotime($to))."");
}
...
<?php
$newans=array();
$sqlreportaccess=mysqli_query($con, "select * from pairreports where franchiseid='".$_SESSION['franchisesession']."' and types='ouwcon' and createdid='$companymainid' order by id  asc");
...
<input type="checkbox" class="custom-control-input" name="<?= $coltypemod; ?>ouwcon" id="<?= $coltypemod; ?>ouwcon" checked disabled>
<label class="custom-control-label custom-label" for="<?= $coltypemod; ?>ouwcon"> <?= str_replace(" or ", " / ",$newmoduleskey) ?></label>
</div>
</div>                  
</div>
</div>
...
<div class="tab-pane fade" id="nav-profile" role="tabpanel" aria-labelledby="nav-profile-tab">
  <div class="table-responsive m-3" id="histables">
    <table class="table table-bordered">
      ...
      <tbody>
      <?php
      $sqluse=mysqli_query($con, "select * from pairusehistory where usetype='REPouwcon' and useid='$companymainid' order by createdon desc");
...
<tr>
<td>Taxable outward supplies to consumer</td>
</tr>
<tr>
<td id="datefromto"></td>
</tr>
...
<thead>
<tr>
<td class="text-uppercase" style="text-align: left !important;border:1px solid #eee;padding-left: 10px;"><span style="font-size:12px;color:black;"> DESCRIPTION</span></td>
<td class="text-uppercase" style="text-align: left !important;border:1px solid #eee;padding-left: 10px;"><span style="font-size:12px;color:black;"> <?=$infomainaccessuserinv["modulename"]?> DATE</span></td>
<td class="text-uppercase" style="text-align: right !important;border:1px solid #eee;padding-right: 10px;"><span style="font-size:12px;color:black;"> <?=$access["txttaxableinv"]?></span></td>
...
$pagetotnumpage = ceil($chekcspage/15);
}
...
const App = () => (React.createElement(Pagination, { total: <?=ceil($chekcs)?>, showSizeChanger: false, showQuickJumper: true, showTotal: (total, range) => `Total ${range[0]} - ${range[1]} of ${total} items`,
        onChange: pagechanges,defaultPageSize: 15 }));
...
var totalpagesnumber = <?=ceil($chekcs/15)?>;
if ((page=='')||(page==0)) {
var isthisval = 1;
}
...
$.ajax({
type: "GET",
url: 'reportviewsearch.php?term='+perpages+'&limitings='+($('#limitforpagenum').val())+'&invfrom=<?=$invfrom?>&invto=<?=$invto?>&dif=outwardcon',
...
$(function() {
  ...
  $('#calendars').daterangepicker({
    ...
    let invto = $("#invto").val();
    window.open("outwardreportconsumer.php?invfrom="+invfrom+"&invto="+invto+"","_self");
  });
});
</script>
<script type="text/javascript">
...
if (mysqli_num_rows($sql)==0) {
?>
$("#myTables").html('<tr style="vertical-align: middle;"><td data-label="DESCRIPTION" style="text-align: left !important;font-size:12px;color:black;border:1px solid #eee;padding-left: 10px;"><span></span><br><span>Place of Supply : <span></span></span><br><span><?=$infomainaccessuserinv["modulename"]?> Number : </span><br><span>Amount : <?=number_format((float)0,2,'.',',')?></span></td><td data-label="<?=$infomainaccessuserinv["modulename"]?> DATE" style="text-align: left !important;font-size:12px;color:black;border:1px solid #eee;padding-left: 10px;"></td><td data-label="<?=$access["txttaxableinv"]?>" style="text-align: right !important;font-size:12px;color:black;border:1px solid #eee;padding-right: 10px;"><?=number_format((float)0,2,'.',',')?></td><td data-label="IGST AMOUNT" style="text-align: right !important;font-size:12px;color:black;border:1px solid #eee;padding-right: 10px;"><?=number_format((float)0,2,'.',',')?></td><td data-label="CGST AMOUNT" style="text-align: right !important;font-size:12px;color:black;border:1px solid #eee;padding-right: 10px;"><?=number_format((float)0,2,'.',',')?></td><td data-label="SGST AMOUNT" style="text-align: right !important;font-size:12px;color:black;border:1px solid #eee;padding-right: 10px;"><?=number_format((float)0,2,'.',',')?></td><!-- <td data-label="CESS AMOUNT " style="text-align: right !important;">0</td> --></tr>');
<?php
}
$sqlbranch=mysqli_query($con, "select franchisename from pairfranchises where id='".$_SESSION['franchisesession']."' ");
$branch=mysqli_fetch_array($sqlbranch);
$sqllimit = mysqli_query($con,"select * from pairinvoices where franchisesession='".$_SESSION['franchisesession']."' and createdid='$companymainid' and ".(($sqlviewreport['gstrule']=='0')?'gstrtype="Consumer"':'(gstrtype="Consumer" or gstrtype="Registered Business - Regular") and gstno="" ')." and (invoicedate>='".$invfrom."' and invoicedate<='".$invto."') and cancelstatus='0' GROUP BY invoicedate, invoiceno");
$countoftotal=0;
...
$.ajax({
type: "GET",
url: 'reporthistory.php?types=ouwcon&titles=Taxable outward supplies to consumer&printdownpdfcsv=Printed PDF&fromto='+fromtopdf+'',
success: function (result) {
console.log(result);
},
error: function (error) {
...
$.ajax({
type: "GET",
url: 'check.php?term=outwardcon&invfrom=<?=$invfrom?>&invto=<?=$invto?>&limitfrom='+sIndex+'&limitto='+dt+'',
success: function (result) {
    if (result!=mytabprev) {
...
$.ajax({
type: "GET",
url: 'reporthistory.php?types=ouwcon&titles=Taxable outward supplies to consumer&printdownpdfcsv=Downloaded PDF&fromto='+fromtopdf+'',
success: function (result) {
console.log(result);
},
error: function (error) {
console.log(error);
    ...
    doc.save('Outward-Consumer-Reports-<?=$invfrom?>-To-<?=$invto?>');
...
$.ajax({
type: "GET",
url: 'reportcsv.php?term=outwardcon&from=<?=$invfrom?>&to=<?=$invto?>&branch=<?=(($sqlviewreport['companyname']=='1')?$branch['franchisename']:'')?>&type=Taxable outward supplies to consumer&fromto='+fromtopdf+'',
success: function (result) {
if(checkdown){
...
$.ajax({
type: "GET",
url: 'reporthistory.php?types=ouwcon&titles=Taxable outward supplies to consumer&printdownpdfcsv=Downloaded CSV&fromto='+fromtopdf+'',
success: function (result) {
console.log(result);
},
error: function (error) {
