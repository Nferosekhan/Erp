...
if (isset($_POST['submitcustomizes'])) {
	...
	$sqlreportmoduless=$con->prepare("SELECT * FROM pairreportmodules WHERE types='ouwcon' ORDER BY id ASC");
	...
	$sqlupreport = $con->prepare("UPDATE pairreports SET rowcolumns=? WHERE franchiseid=? AND createdid=? AND types='ouwcon'");
	...
	$sqlreport = $con->prepare("UPDATE pairreports SET reportperiod=?,filtername=?,companyname=?,dateprepare=?,timeprepare=? WHERE franchiseid=? AND createdid=? AND types='ouwcon'");
	...
	header("Location:outwardreportconsumer.php?datefrom=".date($dateformatforurlredirection,strtotime($from))."&dateto=".date($dateformatforurlredirection,strtotime($to))."");
}

$sqlreportviews=$con->prepare("SELECT * FROM pairreports WHERE franchiseid=? AND createdid=? AND types='ouwcon'");
																			...
																			<div id="customizerowcolumns" class="accordion-collapse collapse show" aria-labelledby="customizerowcolumn">
																				<div class="accordion-body text-sm">
																					<div class="row justify-content-center">
																						...
																						<?php
																							$newans=array();
																							$sqlreportaccesss=$con->prepare("SELECT * FROM pairreports WHERE franchiseid=? AND createdid=? AND types='ouwcon'");
																								...
																								<div class="col-lg-6 my-1" style="padding-left: 0px !important;">
																									<div class="custom-control custom-checkbox mr-sm-2" style="padding: 0px 12px !important;">
																										<div class="input-group input-group-sm" id="flaghead">
																											...
																											<label class="custom-control-label custom-label" for="<?= $coltypemod; ?>ouwcon">
																												<?= str_replace(" or ", " / ",str_replace("Invoice",$infomainaccessuserinv['modulename'],str_replace("Taxable Amount",$access['txttaxableinv'],$newmoduleskey))) ?>
																											</label>
																										</div>
																									</div>
																								</div>
											...
											<div class="tab-pane fade" id="nav-profile" role="tabpanel" aria-labelledby="nav-profile-tab">
												<div class="table-responsive m-3" id="histables">
													<table class="table table-bordered">
														...
														<tbody>
      												<?php
															$sqluse=$con->prepare("SELECT * FROM pairusehistory WHERE usetype = 'REPouwcon' AND useid = ? ORDER BY createdon DESC");
																				...
																				<table id="print-are1" style="border:1px solid #eee;height: 975px;" width="100%">
																					<thead>
																						<tr>
																							...
																							<td class="text-uppercase" style="text-align: left !important;border:1px solid #eee;padding-left: 10px;">
																								<span style="font-size:12px;color:black;">
																										<?=$infomainaccessuserinv["modulename"]?> DATE
																								</span>
																							</td>
																					...
																					<tbody id="myTable">
																					<?php
																						$sqls=$con->prepare("SELECT * FROM pairinvoices WHERE franchisesession=? AND createdid=? AND (invoicedate>=? AND invoicedate<=?) AND cancelstatus='0' AND ".(($sqlviewreport['gstrule']=='0')?'gstrtype="Consumer"':'(gstrtype="Registered Business - Regular" OR gstrtype="Consumer") AND gstno="" ')." GROUP BY invoicedate, invoiceno ORDER BY invoicedate ASC, invoiceno ASC LIMIT 15");
																					...
																					<?php
																						...
																						$sqltotlistpages=$con->prepare("SELECT * FROM pairinvoices WHERE franchisesession=? AND createdid=? AND (invoicedate>=? AND invoicedate<=?) AND cancelstatus='0' AND ".(($sqlviewreport['gstrule']=='0')?'gstrtype="Consumer"':'(gstrtype="Registered Business - Regular" OR gstrtype="Consumer") AND gstno="" ')." GROUP BY invoicedate, invoiceno");
										...
										<table id="csvprint-are3" style="border: 1px solid #eee;display: none;" width="100%">
											<thead>
												<tr>
													...
													<td class="text-uppercase" style="text-align: left !important;border:1px solid #eee;padding-left: 10px;"><span style="font-size:12px;color:black;"> Place of Supply</span></td>
													<td class="text-uppercase" style="text-align: right !important;border:1px solid #eee;padding-right: 10px;"><span style="font-size:12px;color:black;"> Total <?=($access["txttaxableinv"])?></span></td>
...
const pagechanges = (page, pageSize) => {
	...
	ajaxRequestview = $.ajax({
		type: "GET",
		url: 'reportviewsearch.php?lastornone='+lastornone+'&term='+perpages+'&limitings='+($('#limitforpagenum').val())+'&datefrom=<?=$datefrom?>&dateto=<?=$dateto?>&dif=outwardcon',
...
$(function() {
	...
	function(start, end, label) {
		...
		window.open("outwardreportconsumer.php?datefrom="+datefrom+"&dateto="+dateto+"","_self");
	});
});
...
document.addEventListener('DOMContentLoaded', function() {
	...
	function makeAjaxRequest() {
		ajaxRequest = $.ajax({
			type: "GET",
			url: 'check.php?term=outwardcon&datefrom=<?=$datefrom?>&dateto=<?=$dateto?>&limitfrom=0&limitto=10000',
			success: function (result) {
				if (result != mytabprev) {
		...
		ajaxCsvRequest = $.ajax({
			type: "GET",
			url: 'reportcsv.php?term=outwardcon&datefrom=<?=$datefrom?>&dateto=<?=$dateto?>',
			success: function (result) {
