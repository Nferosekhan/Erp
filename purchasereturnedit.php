...
if((isset($_GET['purchasereturnno']))&&(isset($_GET['purchasereturndate']))){
	 $purchasereturnno=htmlspecialchars($_GET['purchasereturnno'], ENT_QUOTES, 'UTF-8');
	 $purchasereturndate=htmlspecialchars($_GET['purchasereturndate'], ENT_QUOTES, 'UTF-8');
	 $sql=$con->prepare("SELECT * FROM pairpurchasereturns WHERE franchisesession=? AND createdid=? AND purchasereturnno=? AND purchasereturndate=? ORDER BY id ASC");
	 $sql->bind_param("ssss", $_SESSION['franchisesession'], $companymainid, $purchasereturnno, $purchasereturndate);
	 $sql->execute();
	 $sql->store_result();
	 if($sql->num_rows>0){
		  $sql->close();
		  ...
		  if(isset($_POST['grandtotal'])){
				...
				$oldpurchasereturnno=htmlspecialchars($_GET['purchasereturnno'], ENT_QUOTES, 'UTF-8');
				$oldpurchasereturndate=htmlspecialchars($_GET['purchasereturndate'], ENT_QUOTES, 'UTF-8');
				$purchasereturnno=htmlspecialchars($_POST['purchasereturnno'], ENT_QUOTES, 'UTF-8');
				...
				for($i=0; $i<count($_POST['oldproductname']); $i++){
					 ...
					 if($productname!=''){
						  $sql = $con->prepare("DELETE FROM pairpurchasereturns WHERE createdid=? AND franchisesession=? AND purchasereturnno=? AND purchasereturndate=?");
						  $sql->bind_param("ssss", $companymainid, $_SESSION["franchisesession"], $oldpurchasereturnno, $oldpurchasereturndate);
						  $sql->execute();
						  $sqldelledger = $con->prepare("DELETE FROM pairledgers WHERE createdid=? AND franchisesession=? AND ledgerno=? AND ledgerdate=? AND (type='purchasereturn' OR type='purchasereturn payment')");
						  $sqldelledger->bind_param("ssss", $companymainid, $_SESSION["franchisesession"], $oldpurchasereturnno, $oldpurchasereturndate);
						  $sqldelledger->execute();
						  $sqls = $con->prepare("DELETE FROM pairpurchasereturnpayments WHERE receiptno=? AND receiptdate=?");
						  $sqls->bind_param("ss", $oldpurchasereturnno, $oldpurchasereturndate);
						  $sqls->execute();
						  $sqls->close();
						  $sqlsant = $con->prepare("DELETE FROM pairpurchasereturnpayhistory WHERE purchasereturnno=? AND purchasereturndate=? AND type='purchasereturn'");
						  $sqlsant->bind_param("ss", $oldpurchasereturnno, $oldpurchasereturndate);
						  $sqlsant->execute();
						  $sqlsant->close();
						  ...
						  $sqlmarginsdel->bind_param("iiss",$companymainid, $_SESSION["franchisesession"], $oldpurchasereturnno, $oldpurchasereturndate);
				...
				$sql2->bind_param("iiss", $_SESSION['franchisesession'], $companymainid, $oldpurchasereturnno, $oldpurchasereturndate);
				$sql2->execute();
				$sql2->store_result();
				...
				if($sql2->num_rows==0){
					 ...
					 if($purchasereturnterm=='CASH'){
						  if ($validpaidamount!=0) {
								...
								$sqlselects=$con->prepare("SELECT id FROM pairpurchasereturnpayments WHERE receiptno=? AND receiptdate=?");
								$sqlselects->bind_param("ss", $purchasereturnno, $purchasereturndate);
								$sqlselects->execute();
								if ($sqlselects->num_rows==0) {
									 $sqlselectsql = $sqlselects->get_result();
									 $sqlselect=$sqlselectsql->fetch_array();
									 $sqlpurchasepayins = $con->prepare("INSERT INTO pairpurchasereturnpayments (createdid,createdby,franchisesession,createdon,term,type,vendorname,vendorid,receiptno,receiptdate,amount,paymentmode,notes,publicid,privateid) VALUES (?, ?, ?, ?, 'RECEIPT', 'purchasereturn', ?, ?, ?, ?, ?, ?, '-', ?, ?)");
									 $sqlpurchasepayins->bind_param("ssssssssssss", $companymainid, $_SESSION["unqwerty"], $_SESSION["franchisesession"], $createdon, $vendorname, $vendorid, $purchasereturnno, $purchasereturndate, $validpaidamount, $purchasereturnterm, $publiccode, $privatecode);
									 ...
									 $sqlbatchupd = $con->prepare("UPDATE pairpurchasereturnpayments SET createdid=?, createdby=?, franchisesession=?, createdon=?,  term='RECEIPT', type='purchasereturn', vendorname=?, vendorid=?, receiptno=?, receiptdate=?, amount=?, paymentmode=?, notes='-' WHERE receiptno=? AND receiptdate=?");
									 $sqlbatchupd->bind_param("ssssssssssss", $companymainid, $_SESSION["unqwerty"], $_SESSION["franchisesession"], $createdon, $vendorname, $vendorid, $purchasereturnno, $purchasereturndate, $validpaidamount, $purchasereturnterm, $purchasereturnno, $purchasereturndate);
									 $sqlbatchupd->execute();
									 if($sqlbatchupd){
										  $sqlbatchupd->close();
										  $sqle = $con->prepare("UPDATE pairpurchasereturnpayhistory SET amount=? WHERE purchasereturnno=? AND purchasereturndate=? AND type='purchasereturn'");
										  $sqle->bind_param("sss", $validpaidamount, $purchasereturnno, $purchasereturndate);
										  $sqle->execute();
										  $sqle->close();
									 }
								...
								$sqlbatchupdant = $con->prepare("UPDATE pairpurchasereturnpayments SET createdid=?, createdby=?, franchisesession=?, createdon=?,  term='RECEIPT', type='purchasereturn', vendorname=?, vendorid=?, receiptno=?, receiptdate=?, amount=?, paymentmode=?, notes='-' WHERE receiptno=? AND receiptdate=?");
								$sqlbatchupdant->bind_param("ssssssssssss", $companymainid, $_SESSION["unqwerty"], $_SESSION["franchisesession"], $createdon, $vendorname, $vendorid, $purchasereturnno, $purchasereturndate, $validpaidamount, $purchasereturnterm, $purchasereturnno, $purchasereturndate);
								$sqlbatchupdant->execute();
								if($sqlbatchupdant){
									 $sqlbatchupdant->close();
									 $sqleant = $con->prepare("UPDATE pairpurchasereturnpayhistory SET amount=? WHERE purchasereturnno=? AND purchasereturndate=? AND type='purchasereturn'");
									 $sqleant->bind_param("sss", $validpaidamount, $purchasereturnno, $purchasereturndate);
									 $sqleant->execute();
									 $sqleant->close();
								}
					 ...
					 elseif($purchasereturnterm=='CREDIT'){
						  $sqls = $con->prepare("DELETE FROM pairpurchasereturnpayments WHERE receiptno=? AND receiptdate=?");
						  $sqls->bind_param("ss", $purchasereturnno, $purchasereturndate);
						  $sqls->execute();
						  $sqls->close();
						  $sqlsant = $con->prepare("DELETE FROM pairpurchasereturnpayhistory WHERE purchasereturnno=? AND purchasereturndate=? AND type='purchasereturn'");
						  $sqlsant->bind_param("ss", $purchasereturnno, $purchasereturndate);
						  $sqlsant->execute();
						  $sqlsant->close();
						  //FOR UPDATE THE PAYMENT STATUS UNPAID
						  $sqldelledgerpay = $con->prepare("DELETE FROM pairledgers WHERE createdid=? AND franchisesession=? AND ledgerno=? AND ledgerdate=? AND type='purchasereturn payment'");
						  $sqldelledgerpay->bind_param("ssss", $companymainid, $_SESSION["franchisesession"], $purchasereturnno, $purchasereturndate);
					 ...
					 if($sql){
						  ...
						  while($infopurchasereturn=$sqlipurchasereturn->fetch_array()){
								...
								$paidamount=0;
								$sqlpurchasespays=$con->prepare("SELECT amount FROM pairpurchasereturnpayhistory WHERE franchisesession=? AND createdid=? AND purchasereturnno=? AND purchasereturndate=? AND vendorid=? ORDER BY id DESC");
						  ...
						  $cussqlup->bind_param("sssss", $purchasereturnamount, $balanceamount, $currentamount, $overdueamount, $vendorid);
						  $cussqlup->execute();
						  $cussqlup->close();
						  if(isset($_POST['submit1'])){
								...
								echo '<script> window.location.href="purchasereturns.php?remarks=Updated Successfully";</script>'; 
						  }
						  else{
								$sqluse = $con->prepare("INSERT INTO pairusehistory (usetype,createdon,createdby,useid,uniqueid,useremarks) VALUES ('PURCHASERETURNS', ?, ?, ?, ?, '<span style=\"color:green;\" id=\"prohisfromtospan\">Updated Successfully</span>')");
								$sqluse->bind_param("ssss", $times, $_SESSION["unqwerty"], $purchasereturnno, $purchaseid);
								$sqluse->execute();
								$sqluse->close();
								echo '<script> window.location.href="purchasereturnview.php?id='.$purchaseid.'&purchasereturnno='.$purchasereturnno.'&purchasereturndate='.$purchasereturndate.'&remarks=Updated Successfully";</script>';
						  }
					 }
				...
				if ($sql) {
					 $oldespurchasereturnno = htmlspecialchars($_POST['oldespurchasereturnno'], ENT_QUOTES, 'UTF-8');
					 $oldesinvnumber = htmlspecialchars($_POST['oldesinvnumber'], ENT_QUOTES, 'UTF-8');
					 $oldespurchasereturndate = htmlspecialchars($_POST['oldespurchasereturndate'], ENT_QUOTES, 'UTF-8');
					 ...
					 $oldespurchasereturnterm = htmlspecialchars($_POST['oldespurchasereturnterm'], ENT_QUOTES, 'UTF-8');
					 ...
					 $dateformats->close();
					 //FOR DATE FORMATING
					 if($purchasereturnno!=$oldespurchasereturnno){
						  if($purchasereturninfoch!=''){
								$purchasereturninfoch.='<br> '.$infomainaccessuser['modulename'].' Number <span style="color:green;" id="prohisfromtospan">( From '.$oldespurchasereturnno.' To '.$purchasereturnno.' ) </span>';
						  }
						  else{
								$purchasereturninfoch.='<span style="color:royalblue;">'.$infomainaccessuser['modulename'].' Information</span> <br> '.$infomainaccessuser['modulename'].' Number <span style="color:green;" id="prohisfromtospan">( From '.$oldespurchasereturnno.' To '.$purchasereturnno.' ) </span>';
						  }                   
					 }
					 if($invnumber!=$oldesinvnumber){
						  if($purchasereturninfoch!=''){
								$purchasereturninfoch.='<br> Invoice Number <span style="color:green;" id="prohisfromtospan">( From '.$oldesinvnumber.' To '.$invnumber.' ) </span>';
						  }
						  else{
								$purchasereturninfoch.='<span style="color:royalblue;">'.$infomainaccessuser['modulename'].' Information</span> <br> Invoice Number <span style="color:green;" id="prohisfromtospan">( From '.$oldesinvnumber.' To '.$invnumber.' ) </span>';
						  }                   
					 }
					 if($purchasereturndate!=$oldespurchasereturndate){
						  if($purchasereturninfoch!=''){
								$purchasereturninfoch.='<br> '.$infomainaccessuser['modulename'].' Date <span style="color:green;" id="prohisfromtospan">( From '.date($date,strtotime($oldespurchasereturndate)).' To '.date($date,strtotime($purchasereturndate)).' ) </span>';
						  }
						  else{
								$purchasereturninfoch.='<span style="color:royalblue;">'.$infomainaccessuser['modulename'].' Information</span> <br> '.$infomainaccessuser['modulename'].' Date <span style="color:green;" id="prohisfromtospan">( From '.date($date,strtotime($oldespurchasereturndate)).' To '.date($date,strtotime($purchasereturndate)).' ) </span>';
						  }                   
					 }
					 if($reference!=$oldesreference){
						  if($purchasereturninfoch!=''){
								$purchasereturninfoch.='<br> Reference <span style="color:green;" id="prohisfromtospan">( From '.$oldesreference.' To '.$reference.' ) </span>';
						  }
						  else{
								$purchasereturninfoch.='<span style="color:royalblue;">'.$infomainaccessuser['modulename'].' Information</span> <br> Reference <span style="color:green;" id="prohisfromtospan">( From '.$oldesreference.' To '.$reference.' ) </span>';
						  }                   
					 }
					 if($saleperson!=$oldessaleperson){
						  if($purchasereturninfoch!=''){
								$purchasereturninfoch.='<br> Sale Person <span style="color:green;" id="prohisfromtospan">( From '.$oldessaleperson.' To '.$saleperson.' ) </span>';
						  }
						  else{
								$purchasereturninfoch.='<span style="color:royalblue;">'.$infomainaccessuser['modulename'].' Information</span> <br> Sale Person <span style="color:green;" id="prohisfromtospan">( From '.$oldessaleperson.' To '.$saleperson.' ) </span>';
						  }                   
					 }
					 if($preparedby!=$oldespreparedby){
						  if($purchasereturninfoch!=''){
								$purchasereturninfoch.='<br> Prepared By <span style="color:green;" id="prohisfromtospan">( From '.$oldespreparedby.' To '.$preparedby.' ) </span>';
						  }
						  else{
								$purchasereturninfoch.='<span style="color:royalblue;">'.$infomainaccessuser['modulename'].' Information</span> <br> Prepared By <span style="color:green;" id="prohisfromtospan">( From '.$oldespreparedby.' To '.$preparedby.' ) </span>';
						  }                   
					 }
					 if($checkedby!=$oldescheckedby){
						  if($purchasereturninfoch!=''){
								$purchasereturninfoch.='<br> Checked By <span style="color:green;" id="prohisfromtospan">( From '.$oldescheckedby.' To '.$checkedby.' ) </span>';
						  }
						  else{
								$purchasereturninfoch.='<span style="color:royalblue;">'.$infomainaccessuser['modulename'].' Information</span> <br> Checked By <span style="color:green;" id="prohisfromtospan">( From '.$oldescheckedby.' To '.$checkedby.' ) </span>';
						  }                   
					 }
					 if($invgrandtotal!=$oldesinvgrandtotal){
						  if($purchasereturninfoch!=''){
								$purchasereturninfoch.='<br> Grand Total <span style="color:green;" id="prohisfromtospan">( From '.$oldesinvgrandtotal.' To '.$invgrandtotal.' ) </span>';
						  }
						  else{
								$purchasereturninfoch.='<span style="color:royalblue;">'.$infomainaccessuser['modulename'].' Information</span> <br> Grand Total <span style="color:green;" id="prohisfromtospan">( From '.$oldesinvgrandtotal.' To '.$invgrandtotal.' ) </span>';
						  }                   
					 }
					 ...
					 if($vendorname!=$oldesvendorname){
						  if($purreturnveninfoch!=''){
								$purreturnveninfoch.='<br> '.$infomainaccessuserven['modulename'].' Name <span style="color:green;" id="prohisfromtospan">( From '.$oldesvendorname.' To '.$vendorname.' ) </span>';
						  }
						  else{
								$purreturnveninfoch.=''.(($purchasereturninfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessuserven['modulename'].' Information</span> <br> '.$infomainaccessuserven['modulename'].' Name <span style="color:green;" id="prohisfromtospan">( From '.$oldesvendorname.' To '.$vendorname.' ) </span>';
						  }                   
					 }
					 if($area!=$oldesarea){
						  if($purreturnveninfoch!=''){
								$purreturnveninfoch.='<br> PURCHASERETURNING STREET <span style="color:green;" id="prohisfromtospan">( From '.$oldesarea.' To '.$area.' ) </span>';
						  }
						  else{
								$purreturnveninfoch.=''.(($purchasereturninfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessuserven['modulename'].' Information</span> <br> PURCHASERETURNING STREET <span style="color:green;" id="prohisfromtospan">( From '.$oldesarea.' To '.$area.' ) </span>';
						  }                   
					 }
					 if($city!=$oldescity){
						  if($purreturnveninfoch!=''){
								$purreturnveninfoch.='<br> PURCHASERETURNING CITY/TOWN <span style="color:green;" id="prohisfromtospan">( From '.$oldescity.' To '.$city.' ) </span>';
						  }
						  else{
								$purreturnveninfoch.=''.(($purchasereturninfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessuserven['modulename'].' Information</span> <br> PURCHASERETURNING CITY/TOWN <span style="color:green;" id="prohisfromtospan">( From '.$oldescity.' To '.$city.' ) </span>';
						  }                   
					 }
					 if($state!=$oldesstate){
						  if($purreturnveninfoch!=''){
								$purreturnveninfoch.='<br> PURCHASERETURNING STATE <span style="color:green;" id="prohisfromtospan">( From '.$oldesstate.' To '.$state.' ) </span>';
						  }
						  else{
								$purreturnveninfoch.=''.(($purchasereturninfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessuserven['modulename'].' Information</span> <br> PURCHASERETURNING STATE <span style="color:green;" id="prohisfromtospan">( From '.$oldesstate.' To '.$state.' ) </span>';
						  }                   
					 }
					 if($district!=$oldesdistrict){
						  if($purreturnveninfoch!=''){
								$purreturnveninfoch.='<br> PURCHASERETURNING PIN <span style="color:green;" id="prohisfromtospan">( From '.$oldesdistrict.' To '.$district.' ) </span>';
						  }
						  else{
								$purreturnveninfoch.=''.(($purchasereturninfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessuserven['modulename'].' Information</span> <br> PURCHASERETURNING PIN <span style="color:green;" id="prohisfromtospan">( From '.$oldesdistrict.' To '.$district.' ) </span>';
						  }                   
					 }
					 if($pincode!=$oldespincode){
						  if($purreturnveninfoch!=''){
								$purreturnveninfoch.='<br> PURCHASERETURNING COUNTRY/REGION <span style="color:green;" id="prohisfromtospan">( From '.$oldespincode.' To '.$pincode.' ) </span>';
						  }
						  else{
								$purreturnveninfoch.=''.(($purchasereturninfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessuserven['modulename'].' Information</span> <br> PURCHASERETURNING COUNTRY/REGION <span style="color:green;" id="prohisfromtospan">( From '.$oldespincode.' To '.$pincode.' ) </span>';
						  }                   
					 }
					 if($sarea!=$oldessarea){
						  if($purreturnveninfoch!=''){
								$purreturnveninfoch.='<br> SHIPPING STREET <span style="color:green;" id="prohisfromtospan">( From '.$oldessarea.' To '.$sarea.' ) </span>';
						  }
						  else{
								$purreturnveninfoch.=''.(($purchasereturninfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessuserven['modulename'].' Information</span> <br> SHIPPING STREET <span style="color:green;" id="prohisfromtospan">( From '.$oldessarea.' To '.$sarea.' ) </span>';
						  }                   
					 }
					 if($scity!=$oldesscity){
						  if($purreturnveninfoch!=''){
								$purreturnveninfoch.='<br> SHIPPING CITY/TOWN <span style="color:green;" id="prohisfromtospan">( From '.$oldesscity.' To '.$scity.' ) </span>';
						  }
						  else{
								$purreturnveninfoch.=''.(($purchasereturninfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessuserven['modulename'].' Information</span> <br> SHIPPING CITY/TOWN <span style="color:green;" id="prohisfromtospan">( From '.$oldesscity.' To '.$scity.' ) </span>';
						  }                   
					 }
					 if($sstate!=$oldessstate){
						  if($purreturnveninfoch!=''){
								$purreturnveninfoch.='<br> SHIPPING STATE <span style="color:green;" id="prohisfromtospan">( From '.$oldessstate.' To '.$sstate.' ) </span>';
						  }
						  else{
								$purreturnveninfoch.=''.(($purchasereturninfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessuserven['modulename'].' Information</span> <br> SHIPPING STATE <span style="color:green;" id="prohisfromtospan">( From '.$oldessstate.' To '.$sstate.' ) </span>';
						  }                   
					 }
					 if($sdistrict!=$oldessdistrict){
						  if($purreturnveninfoch!=''){
								$purreturnveninfoch.='<br> SHIPPING PIN <span style="color:green;" id="prohisfromtospan">( From '.$oldessdistrict.' To '.$sdistrict.' ) </span>';
						  }
						  else{
								$purreturnveninfoch.=''.(($purchasereturninfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessuserven['modulename'].' Information</span> <br> SHIPPING PIN <span style="color:green;" id="prohisfromtospan">( From '.$oldessdistrict.' To '.$sdistrict.' ) </span>';
						  }                   
					 }
					 if($spincode!=$oldesspincode){
						  if($purreturnveninfoch!=''){
								$purreturnveninfoch.='<br> SHIPPING COUNTRY/REGION <span style="color:green;" id="prohisfromtospan">( From '.$oldesspincode.' To '.$spincode.' ) </span>';
						  }
						  else{
								$purreturnveninfoch.=''.(($purchasereturninfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessuserven['modulename'].' Information</span> <br> SHIPPING COUNTRY/REGION <span style="color:green;" id="prohisfromtospan">( From '.$oldesspincode.' To '.$spincode.' ) </span>';
						  }                   
					 }
					 if($twoworkphone!=$oldestwoworkphone){
						  if($purreturnveninfoch!=''){
								$purreturnveninfoch.='<br> WORK PHONE <span style="color:green;" id="prohisfromtospan">( From '.$oldestwoworkphone.' To '.$twoworkphone.' ) </span>';
						  }
						  else{
								$purreturnveninfoch.=''.(($purchasereturninfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessuserven['modulename'].' Information</span> <br> WORK PHONE <span style="color:green;" id="prohisfromtospan">( From '.$oldestwoworkphone.' To '.$twoworkphone.' ) </span>';
						  }                   
					 }
					 if($twomobilephone!=$oldestwomobilephone){
						  if($purreturnveninfoch!=''){
								$purreturnveninfoch.='<br> MOBILE PHONE <span style="color:green;" id="prohisfromtospan">( From '.$oldestwomobilephone.' To '.$twomobilephone.' ) </span>';
						  }
						  else{
								$purreturnveninfoch.=''.(($purchasereturninfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessuserven['modulename'].' Information</span> <br> MOBILE PHONE <span style="color:green;" id="prohisfromtospan">( From '.$oldestwomobilephone.' To '.$twomobilephone.' ) </span>';
						  }                   
					 }
					 if($gstrtype!=$oldesgstrtype){
						  if($purreturnveninfoch!=''){
								$purreturnveninfoch.='<br> GST TREATMENT <span style="color:green;" id="prohisfromtospan">( From '.$oldesgstrtype.' To '.$gstrtype.' ) </span>';
						  }
						  else{
								$purreturnveninfoch.=''.(($purchasereturninfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessuserven['modulename'].' Information</span> <br> GST TREATMENT <span style="color:green;" id="prohisfromtospan">( From '.$oldesgstrtype.' To '.$gstrtype.' ) </span>';
						  }                   
					 }
					 if($gstno!=$oldesgstno){
						  if($purreturnveninfoch!=''){
								$purreturnveninfoch.='<br> GSTIN <span style="color:green;" id="prohisfromtospan">( From '.$oldesgstno.' To '.$gstno.' ) </span>';
						  }
						  else{
								$purreturnveninfoch.=''.(($purchasereturninfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessuserven['modulename'].' Information</span> <br> GSTIN <span style="color:green;" id="prohisfromtospan">( From '.$oldesgstno.' To '.$gstno.' ) </span>';
						  }                   
					 }
					 if($pos!=$oldespos){
						  if($purreturnveninfoch!=''){
								$purreturnveninfoch.='<br> PLACE OF SUPPLY <span style="color:green;" id="prohisfromtospan">( From '.$oldespos.' To '.$pos.' ) </span>';
						  }
						  else{
								$purreturnveninfoch.=''.(($purchasereturninfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessuserven['modulename'].' Information</span> <br> PLACE OF SUPPLY <span style="color:green;" id="prohisfromtospan">( From '.$oldespos.' To '.$pos.' ) </span>';
						  }                   
					 }
					 if($dlno20!=$oldesdlno20){
						  if($purreturnveninfoch!=''){
								$purreturnveninfoch.='<br> DL No 20 <span style="color:green;" id="prohisfromtospan">( From '.$oldesdlno20.' To '.$dlno20.' ) </span>';
						  }
						  else{
								$purreturnveninfoch.=''.(($purchasereturninfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessuserven['modulename'].' Information</span> <br> DL No 20 <span style="color:green;" id="prohisfromtospan">( From '.$oldesdlno20.' To '.$dlno20.' ) </span>';
						  }                   
					 }
					 if($dlno21!=$oldesdlno21){
						  if($purreturnveninfoch!=''){
								$purreturnveninfoch.='<br> DL No 21 <span style="color:green;" id="prohisfromtospan">( From '.$oldesdlno21.' To '.$dlno21.' ) </span>';
						  }
						  else{
								$purreturnveninfoch.=''.(($purchasereturninfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessuserven['modulename'].' Information</span> <br> DL No 21 <span style="color:green;" id="prohisfromtospan">( From '.$oldesdlno21.' To '.$dlno21.' ) </span>';
						  }                   
					 }
								...
								if($productname!=''){
									 if($productname!=$oldesproductname){
										  if($purchasereturniteminfoch!=''){
												$purchasereturniteminfoch.='<br> '.$infomainaccessuserpro['modulename'].' Name <span style="color:green;" id="prohisfromtospan">( From '.$oldesproductname.' To '.$productname.' ) </span>';
										  }
										  else{
												$purchasereturniteminfoch.=''.(($purchasereturninfoch!=''||$purreturnveninfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> '.$infomainaccessuserpro['modulename'].' Name <span style="color:green;" id="prohisfromtospan">( From '.$oldesproductname.' To '.$productname.' ) </span>';
										  }                   
									 }
									 if($manufacturer!=$oldesmanufacturer){
										  if($purchasereturniteminfoch!=''){
												$purchasereturniteminfoch.='<br> '.$access['txtnamecategory'].' <span style="color:green;" id="prohisfromtospan">( From '.$oldesmanufacturer.' To '.$manufacturer.' ) </span>';
										  }
										  else{
												$purchasereturniteminfoch.=''.(($purchasereturninfoch!=''||$purreturnveninfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> '.$access['txtnamecategory'].' <span style="color:green;" id="prohisfromtospan">( From '.$oldesmanufacturer.' To '.$manufacturer.' ) </span>';
										  }                   
									 }
									 if($producthsn!=$oldesproducthsn){
										  if($purchasereturniteminfoch!=''){
												$purchasereturniteminfoch.='<br> HSN Code <span style="color:green;" id="prohisfromtospan">( From '.$oldesproducthsn.' To '.$producthsn.' ) </span>';
										  }
										  else{
												$purchasereturniteminfoch.=''.(($purchasereturninfoch!=''||$purreturnveninfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> HSN Code <span style="color:green;" id="prohisfromtospan">( From '.$oldesproducthsn.' To '.$producthsn.' ) </span>';
										  }                   
									 }
									 if($productdescription!=$oldesproductdescription){
										  if($purchasereturniteminfoch!=''){
												$purchasereturniteminfoch.='<br> Description <span style="color:green;" id="prohisfromtospan">( From '.$oldesproductdescription.' To '.$productdescription.' ) </span>';
										  }
										  else{
												$purchasereturniteminfoch.=''.(($purchasereturninfoch!=''||$purreturnveninfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Description <span style="color:green;" id="prohisfromtospan">( From '.$oldesproductdescription.' To '.$productdescription.' ) </span>';
										  }                   
									 }
									 if($batch!=$oldesbatch){
										  if($purchasereturniteminfoch!=''){
												$purchasereturniteminfoch.='<br> Batch <span style="color:green;" id="prohisfromtospan">( From '.$oldesbatch.' To '.$batch.' ) </span>';
										  }
										  else{
												$purchasereturniteminfoch.=''.(($purchasereturninfoch!=''||$purreturnveninfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Batch <span style="color:green;" id="prohisfromtospan">( From '.$oldesbatch.' To '.$batch.' ) </span>';
										  }                   
									 }
									 if($expdate!=$oldesexpdate){
										  if($purchasereturniteminfoch!=''){
												$purchasereturniteminfoch.='<br> Expiry <span style="color:green;" id="prohisfromtospan">( From '.date($date,strtotime($oldesexpdate)).' To '.date($date,strtotime($expdate)).' ) </span>';
										  }
										  else{
												$purchasereturniteminfoch.=''.(($purchasereturninfoch!=''||$purreturnveninfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Expiry <span style="color:green;" id="prohisfromtospan">( From '.date($date,strtotime($oldesexpdate)).' To '.date($date,strtotime($expdate)).' ) </span>';
										  }                   
									 }
									 if($productrate!=$oldesproductrate){
										  if($purchasereturniteminfoch!=''){
												$purchasereturniteminfoch.='<br> Rate <span style="color:green;" id="prohisfromtospan">( From '.$oldesproductrate.' To '.$productrate.' ) </span>';
										  }
										  else{
												$purchasereturniteminfoch.=''.(($purchasereturninfoch!=''||$purreturnveninfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Rate <span style="color:green;" id="prohisfromtospan">( From '.$oldesproductrate.' To '.$productrate.' ) </span>';
										  }                   
									 }
									 if($mrp!=$oldesmrp){
										  if($purchasereturniteminfoch!=''){
												$purchasereturniteminfoch.='<br> Mrp <span style="color:green;" id="prohisfromtospan">( From '.$oldesmrp.' To '.$mrp.' ) </span>';
										  }
										  else{
												$purchasereturniteminfoch.=''.(($purchasereturninfoch!=''||$purreturnveninfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Mrp <span style="color:green;" id="prohisfromtospan">( From '.$oldesmrp.' To '.$mrp.' ) </span>';
										  }                   
									 }
									 if($quantity!=$oldesquantity){
										  if($purchasereturniteminfoch!=''){
												$purchasereturniteminfoch.='<br> Quantity <span style="color:green;" id="prohisfromtospan">( From '.$oldesquantity.' To '.$quantity.' ) </span>';
										  }
										  else{
												$purchasereturniteminfoch.=''.(($purchasereturninfoch!=''||$purreturnveninfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Quantity <span style="color:green;" id="prohisfromtospan">( From '.$oldesquantity.' To '.$quantity.' ) </span>';
										  }                   
									 }
									 if($unit!=$oldesunit){
										  if($purchasereturniteminfoch!=''){
												$purchasereturniteminfoch.='<br> Unit <span style="color:green;" id="prohisfromtospan">( From '.$oldesunit.' To '.$unit.' ) </span>';
										  }
										  else{
												$purchasereturniteminfoch.=''.(($purchasereturninfoch!=''||$purreturnveninfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Unit <span style="color:green;" id="prohisfromtospan">( From '.$oldesunit.' To '.$unit.' ) </span>';
										  }                   
									 }
									 if($noofpacks!=$oldesnoofpacks){
										  if($purchasereturniteminfoch!=''){
												$purchasereturniteminfoch.='<br> Pack <span style="color:green;" id="prohisfromtospan">( From '.$oldesnoofpacks.' To '.$noofpacks.' ) </span>';
										  }
										  else{
												$purchasereturniteminfoch.=''.(($purchasereturninfoch!=''||$purreturnveninfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Pack <span style="color:green;" id="prohisfromtospan">( From '.$oldesnoofpacks.' To '.$noofpacks.' ) </span>';
										  }                   
									 }
									 if($productvalue!=$oldesproductvalue){
										  if($purchasereturniteminfoch!=''){
												$purchasereturniteminfoch.='<br> Taxable Value <span style="color:green;" id="prohisfromtospan">( From '.$oldesproductvalue.' To '.$productvalue.' ) </span>';
										  }
										  else{
												$purchasereturniteminfoch.=''.(($purchasereturninfoch!=''||$purreturnveninfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Taxable Value <span style="color:green;" id="prohisfromtospan">( From '.$oldesproductvalue.' To '.$productvalue.' ) </span>';
										  }                   
									 }
									 if($prodiscount!=$oldesprodiscount){
										  if($purchasereturniteminfoch!=''){
												$purchasereturniteminfoch.='<br> '.$access['txtprodispurchasereturn'].' <span style="color:green;" id="prohisfromtospan">( From '.$oldesprodiscount.' To '.$prodiscount.' ) </span>';
										  }
										  else{
												$purchasereturniteminfoch.=''.(($purchasereturninfoch!=''||$purreturnveninfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> '.$access['txtprodispurchasereturn'].' <span style="color:green;" id="prohisfromtospan">( From '.$oldesprodiscount.' To '.$prodiscount.' ) </span>';
										  }                   
									 }
									 if($prodiscounttype!=$oldesprodiscounttype){
										  if($purchasereturniteminfoch!=''){
												$purchasereturniteminfoch.='<br> Discounted By <span style="color:green;" id="prohisfromtospan">( From '.(($oldesprodiscounttype==0)?'%':'?').' To '.(($prodiscounttype==0)?'%':'?').' ) </span>';
										  }
										  else{
												$purchasereturniteminfoch.=''.(($purchasereturninfoch!=''||$purreturnveninfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Discounted By <span style="color:green;" id="prohisfromtospan">( From '.(($oldesprodiscounttype==0)?'%':'?').' To '.(($prodiscounttype==0)?'%':'?').' ) </span>';
										  }                   
									 }
									 if($taxvalue!=$oldestaxvalue){
										  if($purchasereturniteminfoch!=''){
												$purchasereturniteminfoch.='<br> Tax Value <span style="color:green;" id="prohisfromtospan">( From '.$oldestaxvalue.' To '.$taxvalue.' ) </span>';
										  }
										  else{
												$purchasereturniteminfoch.=''.(($purchasereturninfoch!=''||$purreturnveninfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Tax Value <span style="color:green;" id="prohisfromtospan">( From '.$oldestaxvalue.' To '.$taxvalue.' ) </span>';
										  }                   
									 }
									 if($vat!=$oldesvat){
										  if($purchasereturniteminfoch!=''){
												$purchasereturniteminfoch.='<br> Gst <span style="color:green;" id="prohisfromtospan">( From '.$oldesvat.' % ('.$oldesicsgsthis.') To '.$vat.' % ('.$ansforsepgstval.') ) </span>';
										  }
										  else{
												$purchasereturniteminfoch.=''.(($purchasereturninfoch!=''||$purreturnveninfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Gst <span style="color:green;" id="prohisfromtospan">( From '.$oldesvat.' % ('.$oldesicsgsthis.') To '.$vat.' % ('.$ansforsepgstval.') ) </span>';
										  }                   
									 }
									 if($productnetvalue!=$oldesproductnetvalue){
										  if($purchasereturniteminfoch!=''){
												$purchasereturniteminfoch.='<br> Amount <span style="color:green;" id="prohisfromtospan">( From '.$oldesproductnetvalue.' To '.$productnetvalue.' ) </span>';
										  }
										  else{
												$purchasereturniteminfoch.=''.(($purchasereturninfoch!=''||$purreturnveninfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Amount <span style="color:green;" id="prohisfromtospan">( From '.$oldesproductnetvalue.' To '.$productnetvalue.' ) </span>';
										  }                   
									 }
									 if($salequantity!=$oldessalequantity){
										  if($purchasereturniteminfoch!=''){
												$purchasereturniteminfoch.='<br> '.($access['purreturntxtsqty']).' <span style="color:green;" id="prohisfromtospan">( From '.$oldessalequantity.' To '.$salequantity.' ) </span>';
										  }
										  else{
												$purchasereturniteminfoch.=''.(($purchasereturninfoch!=''||$purreturnveninfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> '.($access['purreturntxtsqty']).' <span style="color:green;" id="prohisfromtospan">( From '.$oldessalequantity.' To '.$salequantity.' ) </span>';
										  }                   
									 }
									 if($productwithouttax!=$oldesproductwithouttax){
										  if($purchasereturniteminfoch!=''){
												$purchasereturniteminfoch.='<br> Sale Rate/Unit(Without Gst) <span style="color:green;" id="prohisfromtospan">( From '.$oldesproductwithouttax.' To '.$productwithouttax.' ) </span>';
										  }
										  else{
												$purchasereturniteminfoch.=''.(($purchasereturninfoch!=''||$purreturnveninfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Sale Rate/Unit(Without Gst) <span style="color:green;" id="prohisfromtospan">( From '.$oldesproductwithouttax.' To '.$productwithouttax.' ) </span>';
										  }                   
									 }
									 if($productwithtax!=$oldesproductwithtax){
										  if($purchasereturniteminfoch!=''){
												$purchasereturniteminfoch.='<br> Sale Rate/Unit(With Gst) <span style="color:green;" id="prohisfromtospan">( From '.$oldesproductwithtax.' To '.$productwithtax.' ) </span>';
										  }
										  else{
												$purchasereturniteminfoch.=''.(($purchasereturninfoch!=''||$purreturnveninfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Sale Rate/Unit(With Gst) <span style="color:green;" id="prohisfromtospan">( From '.$oldesproductwithtax.' To '.$productwithtax.' ) </span>';
										  }                   
									 }
								}
						  }
						  ...
						  for($i=0; $i<count($_POST['oldesproductname']); $i++){
								...
								if($productname!=''){
									 ...
									 if($oldesproductname!=''){                          
										  if($oldesproductname!=''){
												if($purchasereturniteminfoch!=''){
													 $purchasereturniteminfoch.='<br> '.$infomainaccessuserpro['modulename'].' Name <span style="color:red;" id="prohisfromtospan">( From '.$oldesproductname.' To &nbsp; ) </span>';
												}
												else{
													 $purchasereturniteminfoch.=''.(($purchasereturninfoch!=''||$purreturnveninfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> '.$infomainaccessuserpro['modulename'].' Name <span style="color:red;" id="prohisfromtospan">( From '.$oldesproductname.' To &nbsp; ) </span>';
												}
										  }
										  if($oldesmanufacturer!=''){
												if($purchasereturniteminfoch!=''){
													 $purchasereturniteminfoch.='<br> '.$access['txtnamecategory'].' <span style="color:red;" id="prohisfromtospan">( From '.$oldesmanufacturer.' To &nbsp; ) </span>';
												}
												else{
													 $purchasereturniteminfoch.=''.(($purchasereturninfoch!=''||$purreturnveninfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> '.$access['txtnamecategory'].' <span style="color:red;" id="prohisfromtospan">( From '.$oldesmanufacturer.' To &nbsp; ) </span>';
												}
										  }
										  if($oldesproducthsn!=''){
												if($purchasereturniteminfoch!=''){
													 $purchasereturniteminfoch.='<br> HSN Code <span style="color:red;" id="prohisfromtospan">( From '.$oldesproducthsn.' To &nbsp; ) </span>';
												}
												else{
													 $purchasereturniteminfoch.=''.(($purchasereturninfoch!=''||$purreturnveninfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> HSN Code <span style="color:red;" id="prohisfromtospan">( From '.$oldesproducthsn.' To &nbsp; ) </span>';
												}
										  }
										  if($oldesproductdescription!=''){
												if($purchasereturniteminfoch!=''){
													 $purchasereturniteminfoch.='<br> Description <span style="color:red;" id="prohisfromtospan">( From '.$oldesproductdescription.' To &nbsp; ) </span>';
												}
												else{
													 $purchasereturniteminfoch.=''.(($purchasereturninfoch!=''||$purreturnveninfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Description <span style="color:red;" id="prohisfromtospan">( From '.$oldesproductdescription.' To &nbsp; ) </span>';
												}
										  }
										  if($oldesbatch!=''){
												if($purchasereturniteminfoch!=''){
													 $purchasereturniteminfoch.='<br> Batch <span style="color:red;" id="prohisfromtospan">( From '.$oldesbatch.' To &nbsp; ) </span>';
												}
												else{
													 $purchasereturniteminfoch.=''.(($purchasereturninfoch!=''||$purreturnveninfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Batch <span style="color:red;" id="prohisfromtospan">( From '.$oldesbatch.' To &nbsp; ) </span>';
												}
										  }
										  if($oldesexpdate!=''){
												if($purchasereturniteminfoch!=''){
													 $purchasereturniteminfoch.='<br> Expiry <span style="color:red;" id="prohisfromtospan">( From '.date($date,strtotime($oldesexpdate)).' To &nbsp; ) </span>';
												}
												else{
													 $purchasereturniteminfoch.=''.(($purchasereturninfoch!=''||$purreturnveninfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Expiry <span style="color:red;" id="prohisfromtospan">( From '.date($date,strtotime($oldesexpdate)).' To &nbsp; ) </span>';
												}
										  }
										  if($oldesproductrate!=''){
												if($purchasereturniteminfoch!=''){
													 $purchasereturniteminfoch.='<br> Rate <span style="color:red;" id="prohisfromtospan">( From '.$oldesproductrate.' To &nbsp; ) </span>';
												}
												else{
													 $purchasereturniteminfoch.=''.(($purchasereturninfoch!=''||$purreturnveninfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Rate <span style="color:red;" id="prohisfromtospan">( From '.$oldesproductrate.' To &nbsp; ) </span>';
												}
										  }
										  if($oldesmrp!=''){
												if($purchasereturniteminfoch!=''){
													 $purchasereturniteminfoch.='<br> Mrp <span style="color:red;" id="prohisfromtospan">( From '.$oldesmrp.' To &nbsp; ) </span>';
												}
												else{
													 $purchasereturniteminfoch.=''.(($purchasereturninfoch!=''||$purreturnveninfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Mrp <span style="color:red;" id="prohisfromtospan">( From '.$oldesmrp.' To &nbsp; ) </span>';
												}
										  }
										  if($oldesquantity!=''){
												if($purchasereturniteminfoch!=''){
													 $purchasereturniteminfoch.='<br> Quantity <span style="color:red;" id="prohisfromtospan">( From '.$oldesquantity.' To &nbsp; ) </span>';
												}
												else{
													 $purchasereturniteminfoch.=''.(($purchasereturninfoch!=''||$purreturnveninfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Quantity <span style="color:red;" id="prohisfromtospan">( From '.$oldesquantity.' To &nbsp; ) </span>';
												}
										  }
										  if($oldesunit!=''){
												if($purchasereturniteminfoch!=''){
													 $purchasereturniteminfoch.='<br> Unit <span style="color:red;" id="prohisfromtospan">( From '.$oldesunit.' To &nbsp; ) </span>';
												}
												else{
													 $purchasereturniteminfoch.=''.(($purchasereturninfoch!=''||$purreturnveninfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Unit <span style="color:red;" id="prohisfromtospan">( From '.$oldesunit.' To &nbsp; ) </span>';
												}
										  }
										  if($oldesnoofpacks!=''){
												if($purchasereturniteminfoch!=''){
													 $purchasereturniteminfoch.='<br> Pack <span style="color:red;" id="prohisfromtospan">( From '.$oldesnoofpacks.' To &nbsp; ) </span>';
												}
												else{
													 $purchasereturniteminfoch.=''.(($purchasereturninfoch!=''||$purreturnveninfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Pack <span style="color:red;" id="prohisfromtospan">( From '.$oldesnoofpacks.' To &nbsp; ) </span>';
												}
										  }
										  if($oldesproductvalue!=''){
												if($purchasereturniteminfoch!=''){
													 $purchasereturniteminfoch.='<br> Taxable Value <span style="color:red;" id="prohisfromtospan">( From '.$oldesproductvalue.' To &nbsp; ) </span>';
												}
												else{
													 $purchasereturniteminfoch.=''.(($purchasereturninfoch!=''||$purreturnveninfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Taxable Value <span style="color:red;" id="prohisfromtospan">( From '.$oldesproductvalue.' To &nbsp; ) </span>';
												}
										  }
										  if($oldesprodiscount!=''){
												if($purchasereturniteminfoch!=''){
													 $purchasereturniteminfoch.='<br> Discount <span style="color:red;" id="prohisfromtospan">( From '.$oldesprodiscount.' To &nbsp; ) </span>';
												}
												else{
													 $purchasereturniteminfoch.=''.(($purchasereturninfoch!=''||$purreturnveninfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Discount <span style="color:red;" id="prohisfromtospan">( From '.$oldesprodiscount.' To &nbsp; ) </span>';
												}
										  }
										  if($oldestaxvalue!=''){
												if($purchasereturniteminfoch!=''){
													 $purchasereturniteminfoch.='<br> Tax Value <span style="color:red;" id="prohisfromtospan">( From '.$oldestaxvalue.' To &nbsp; ) </span>';
												}
												else{
													 $purchasereturniteminfoch.=''.(($purchasereturninfoch!=''||$purreturnveninfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Tax Value <span style="color:red;" id="prohisfromtospan">( From '.$oldestaxvalue.' To &nbsp; ) </span>';
												}
										  }
										  if($oldesvat!=''){
												if($purchasereturniteminfoch!=''){
													 $purchasereturniteminfoch.='<br> Gst <span style="color:red;" id="prohisfromtospan">( From '.$oldesvat.' % ('.$oldesicsgsthis.') To &nbsp; ) </span>';
												}
												else{
													 $purchasereturniteminfoch.=''.(($purchasereturninfoch!=''||$purreturnveninfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Gst <span style="color:red;" id="prohisfromtospan">( From '.$oldesvat.' % ('.$oldesicsgsthis.') To &nbsp; ) </span>';
												}
										  }
										  if($oldesproductnetvalue!=''){
												if($purchasereturniteminfoch!=''){
													 $purchasereturniteminfoch.='<br> Amount <span style="color:red;" id="prohisfromtospan">( From '.$oldesproductnetvalue.' To &nbsp; ) </span>';
												}
												else{
													 $purchasereturniteminfoch.=''.(($purchasereturninfoch!=''||$purreturnveninfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Amount <span style="color:red;" id="prohisfromtospan">( From '.$oldesproductnetvalue.' To &nbsp; ) </span>';
												}
										  }
										  if($oldessalequantity!=''){
												if($purchasereturniteminfoch!=''){
													 $purchasereturniteminfoch.='<br> '.($access['purreturntxtsqty']).' <span style="color:red;" id="prohisfromtospan">( From '.$oldessalequantity.' To &nbsp; ) </span>';
												}
												else{
													 $purchasereturniteminfoch.=''.(($purchasereturninfoch!=''||$purreturnveninfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> '.($access['purreturntxtsqty']).' <span style="color:red;" id="prohisfromtospan">( From '.$oldessalequantity.' To &nbsp; ) </span>';
												}
										  }
										  if($oldesproductwithouttax!=''){
												if($purchasereturniteminfoch!=''){
													 $purchasereturniteminfoch.='<br> Sale Rate/Unit(Without Gst) <span style="color:red;" id="prohisfromtospan">( From '.$oldesproductwithouttax.' To &nbsp; ) </span>';
												}
												else{
													 $purchasereturniteminfoch.=''.(($purchasereturninfoch!=''||$purreturnveninfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Sale Rate/Unit(Without Gst) <span style="color:red;" id="prohisfromtospan">( From '.$oldesproductwithouttax.' To &nbsp; ) </span>';
												}
										  }
										  if($oldesproductwithtax!=''){
												if($purchasereturniteminfoch!=''){
													 $purchasereturniteminfoch.='<br> Sale Rate/Unit(With Gst) <span style="color:red;" id="prohisfromtospan">( From '.$oldesproductwithtax.' To &nbsp; ) </span>';
												}
												else{
													 $purchasereturniteminfoch.=''.(($purchasereturninfoch!=''||$purreturnveninfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Sale Rate/Unit(With Gst) <span style="color:red;" id="prohisfromtospan">( From '.$oldesproductwithtax.' To &nbsp; ) </span>';
												}
										  }
									 }
								}
					 ...
					 elseif ($curans>$oldans) {
						  for($i=0; $i<count($_POST['productname']); $i++){
								...
								if($oldesproductname!=''){
									 ...
									 if($expdate!=''){
										  if($purchasereturniteminfoch!=''){
												$purchasereturniteminfoch.='<br> Expiry <span style="color:green;" id="prohisfromtospan">( '.date($date,strtotime($expdate)).' ) </span>';
										  }
										  else{
												$purchasereturniteminfoch.=''.(($purchasereturninfoch!=''||$purreturnveninfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Expiry <span style="color:green;" id="prohisfromtospan">( '.date($date,strtotime($expdate)).' ) </span>';        
										  }
									 }
									 if($productrate!=''){
										  if($purchasereturniteminfoch!=''){
					 ...
					 if($totalitems!=$oldestotalitems){
						  if($purchasereturniteminfoch!=''){
								$purchasereturniteminfoch.='<br> Total Items <span style="color:green;" id="prohisfromtospan">( From '.$oldestotalitems.' To '.$totalitems.' ) </span>';
						  }
						  else{
								$purchasereturniteminfoch.=''.(($purchasereturninfoch!=''||$purreturnveninfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Total Items <span style="color:green;" id="prohisfromtospan">( From '.$oldestotalitems.' To '.$totalitems.' ) </span>';
						  }
					 }
					 if($totalquantity!=$oldestotalquantity){
						  if($purchasereturniteminfoch!=''){
								$purchasereturniteminfoch.='<br> Total Qty <span style="color:green;" id="prohisfromtospan">( From '.$oldestotalquantity.' To '.$totalquantity.' ) </span>';
						  }
						  else{
								$purchasereturniteminfoch.=''.(($purchasereturninfoch!=''||$purreturnveninfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Total Qty <span style="color:green;" id="prohisfromtospan">( From '.$oldestotalquantity.' To '.$totalquantity.' ) </span>';
						  }
					 }
					 if($totalamount!=$oldestotalamount){
						  if($purchasereturniteminfoch!=''){
								$purchasereturniteminfoch.='<br> Sub Total <span style="color:green;" id="prohisfromtospan">( From '.$oldestotalamount.' To '.$totalamount.' ) </span>';
						  }
						  else{
								$purchasereturniteminfoch.=''.(($purchasereturninfoch!=''||$purreturnveninfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Sub Total <span style="color:green;" id="prohisfromtospan">( From '.$oldestotalamount.' To '.$totalamount.' ) </span>';
						  }
					 }
					 if($discountamount!=$oldesdiscountamount){
						  if($purchasereturniteminfoch!=''){
								$purchasereturniteminfoch.='<br> Discount <span style="color:green;" id="prohisfromtospan">( From '.$oldesdiscountamount.' To '.$discountamount.' ) </span>';
						  }
						  else{
								$purchasereturniteminfoch.=''.(($purchasereturninfoch!=''||$purreturnveninfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Discount <span style="color:green;" id="prohisfromtospan">( From '.$oldesdiscountamount.' To '.$discountamount.' ) </span>';
						  }
					 }
					 if($totalvatamount!=$oldestotalvatamount){
						  if($purchasereturniteminfoch!=''){
								$purchasereturniteminfoch.='<br> Total Tax <span style="color:green;" id="prohisfromtospan">( From '.$oldestotalvatamount.' To '.$totalvatamount.' ) </span>';
						  }
						  else{
								$purchasereturniteminfoch.=''.(($purchasereturninfoch!=''||$purreturnveninfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Total Tax <span style="color:green;" id="prohisfromtospan">( From '.$oldestotalvatamount.' To '.$totalvatamount.' ) </span>';
						  }
					 }
					 if($roundoff!=$oldesroundoff){
						  if($purchasereturniteminfoch!=''){
								$purchasereturniteminfoch.='<br> Round Off <span style="color:green;" id="prohisfromtospan">( From '.$oldesroundoff.' To '.$roundoff.' ) </span>';
						  }
						  else{
								$purchasereturniteminfoch.=''.(($purchasereturninfoch!=''||$purreturnveninfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Round Off <span style="color:green;" id="prohisfromtospan">( From '.$oldesroundoff.' To '.$roundoff.' ) </span>';
						  }
					 }
					 if($grandtotal!=$oldesgrandtotal){
						  if($purchasereturniteminfoch!=''){
								$purchasereturniteminfoch.='<br> Grand Total <span style="color:green;" id="prohisfromtospan">( From '.$oldesgrandtotal.' To '.$grandtotal.' ) </span>';
						  }
						  else{
								$purchasereturniteminfoch.=''.(($purchasereturninfoch!=''||$purreturnveninfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Grand Total <span style="color:green;" id="prohisfromtospan">( From '.$oldesgrandtotal.' To '.$grandtotal.' ) </span>';
						  }
					 }
					 if($description!=$oldesdescription){
						  if($purchasereturniteminfoch!=''){
								$purchasereturniteminfoch.='<br> Description <span style="color:green;" id="prohisfromtospan">( From '.$oldesdescription.' To '.$description.' ) </span>';
						  }
						  else{
								$purchasereturniteminfoch.=''.(($purchasereturninfoch!=''||$purreturnveninfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Description <span style="color:green;" id="prohisfromtospan">( From '.$oldesdescription.' To '.$description.' ) </span>';
						  }
					 }
					 if($notes!=$oldesnotes){
						  if($purchasereturniteminfoch!=''){
								$purchasereturniteminfoch.='<br> Notes <span style="color:green;" id="prohisfromtospan">( From '.$oldesnotes.' To '.$notes.' ) </span>';
						  }
						  else{
								$purchasereturniteminfoch.=''.(($purchasereturninfoch!=''||$purreturnveninfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Notes <span style="color:green;" id="prohisfromtospan">( From '.$oldesnotes.' To '.$notes.' ) </span>';
						  }
					 }
					 if($terms!=$oldesterms){
						  if($purchasereturniteminfoch!=''){
								$purchasereturniteminfoch.='<br> Terms And Conditions <span style="color:green;" id="prohisfromtospan">( From '.$oldesterms.' To '.$terms.' ) </span>';
						  }
						  else{
								$purchasereturniteminfoch.=''.(($purchasereturninfoch!=''||$purreturnveninfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Terms And Conditions <span style="color:green;" id="prohisfromtospan">( From '.$oldesterms.' To '.$terms.' ) </span>';
						  }
					 }
					 if($fileattach!=$oldesfileattach){
						  if($purchasereturniteminfoch!=''){
								$purchasereturniteminfoch.='<br> Attach <span style="color:green;" id="prohisfromtospan">( Changed ) </span>';
						  }
						  else{
								$purchasereturniteminfoch.=''.(($purchasereturninfoch!=''||$purreturnveninfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Attach <span style="color:green;" id="prohisfromtospan">( Changed ) </span>';
						  }
					 }
					 if($purchasereturnterm!=$oldespurchasereturnterm){
						  if($purchasereturniteminfoch!=''){
								$purchasereturniteminfoch.='<br> Payment Term <span style="color:green;" id="prohisfromtospan">( From '.$oldespurchasereturnterm.' To '.$purchasereturnterm.' ) </span>';
						  }
						  else{
								$purchasereturniteminfoch.=''.(($purchasereturninfoch!=''||$purreturnveninfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Payment Term <span style="color:green;" id="prohisfromtospan">( From '.$oldespurchasereturnterm.' To '.$purchasereturnterm.' ) </span>';
						  }
					 }
					 if($validpaidamount!=$oldesvalidpaidamount&&$purchasereturnterm=='CASH'){
						  if($purchasereturniteminfoch!=''){
								$purchasereturniteminfoch.='<br> Paid Amount <span style="color:green;" id="prohisfromtospan">( From '.$oldesvalidpaidamount.' To '.$validpaidamount.' ) </span>';
						  }
						  else{
								$purchasereturniteminfoch.=''.(($purchasereturninfoch!=''||$purreturnveninfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Paid Amount <span style="color:green;" id="prohisfromtospan">( From '.$oldesvalidpaidamount.' To '.$validpaidamount.' ) </span>';
						  }
					 }
					 if($validbalance!=$oldesvalidbalance&&$purchasereturnterm=='CASH'){
						  if($purchasereturniteminfoch!=''){
								$purchasereturniteminfoch.='<br> Balance Due <span style="color:green;" id="prohisfromtospan">( From '.$oldesvalidbalance.' To '.$validbalance.' ) </span>';
						  }
						  else{
								$purchasereturniteminfoch.=''.(($purchasereturninfoch!=''||$purreturnveninfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Balance Due <span style="color:green;" id="prohisfromtospan">( From '.$oldesvalidbalance.' To '.$validbalance.' ) </span>';
						  }
					 }
					 if($duedates!=$oldesduedates&&$purchasereturnterm=='CREDIT'){
						  if($purchasereturniteminfoch!=''){
								$purchasereturniteminfoch.='<br> Due Term <span style="color:green;" id="prohisfromtospan">( From '.$oldesduedates.' To '.$duedates.' ) </span>';
						  }
						  else{
								$purchasereturniteminfoch.=''.(($purchasereturninfoch!=''||$purreturnveninfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Due Term <span style="color:green;" id="prohisfromtospan">( From '.$oldesduedates.' To '.$duedates.' ) </span>';
						  }
					 }
					 if($duedate!=$oldesduedate&&$purchasereturnterm=='CREDIT'){
						  if($purchasereturniteminfoch!=''){
								$purchasereturniteminfoch.='<br> Due Date <span style="color:green;" id="prohisfromtospan">( From '.date($date,strtotime($oldesduedate)).' To '.date($date,strtotime($duedate)).' ) </span>';
						  }
						  else{
								$purchasereturniteminfoch.=''.(($purchasereturninfoch!=''||$purreturnveninfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Due Date <span style="color:green;" id="prohisfromtospan">( From '.date($date,strtotime($oldesduedate)).' To '.date($date,strtotime($duedate)).' ) </span>';
						  }
					 }
					 //FOR PURCHASERETURN OTHER INFORMATIONS LIKE PAYMENT AND TERMS AND DESCRIPTION......
					 $purchasereturntotinfoch = $purchasereturninfoch.$purreturnveninfoch.$purchasereturniteminfoch;
					 ...
					 $sqlusehisupdqry->bind_param("sss", $purchaseid, $purchasereturnno, $_GET['id']);
...
?>
	 ...
	 <?php
		  ...
		  if((isset($_GET['purchasereturnno']))&&(isset($_GET['purchasereturndate']))){
				$purchasereturnno=htmlspecialchars( $_GET['purchasereturnno'], ENT_QUOTES, 'UTF-8');
				$purchasereturndate=htmlspecialchars( $_GET['purchasereturndate'], ENT_QUOTES, 'UTF-8');
				$sql=$con->prepare("SELECT * FROM pairpurchasereturns WHERE franchisesession=? AND createdid=? AND purchasereturnno=? AND purchasereturndate=? ORDER BY id ASC LIMIT 5");
				$sql->bind_param("ssss", $_SESSION['franchisesession'], $companymainid, $purchasereturnno, $purchasereturndate);
				$sql->execute();
				$count=1;
					 ...
					 <div class="col-12">
						  <div class="card mb-4 mt-5">
								...
								?>
									 ...
									 <!-- VENDOR ADD MODAL -->
									 <form autocomplete="off" id="purchasereturnform" action="" onsubmit="return checkvalidate()" method="post" enctype="multipart/form-data" class="form-horizontal" role="form">
									 ...
									 <?php
										  $oldarray=array( 'purchasereturnno','invnumber','purchasereturndate','reference','saleperson','preparedby','checkedby','invgrandtotal','vendorname','area','city','state','district','pincode','sarea','scity','sstate','sdistrict','spincode','workphone','mobile','gstrtype','gstno','pos','salequantity','dlno20','dlno21' );
									 ...
									 ?>
										  ...
										  <input type="hidden" value="<?=$rows[0]['purchasereturnterm']?>" name="oldespurchasereturnterm">
										  ...
										  <input type="hidden" name="oldpurchasereturnno" value="<?=$rows[0]['purchasereturnno']?>">
										  <input type="hidden" name="oldpurchasereturndate" value="<?=$rows[0]['purchasereturndate']?>">
										  <input type="hidden" name="purchasereturnterm" value="<?=$rows[0]['purchasereturnterm']?>">
										  <div class="row">
										  <?php
																		  ...
																		  <div class="row">
																				<div class="col-lg-12">
																					 <div class="form-group row" style="align-items: center !important;">
																						  <div class="col-lg-5">
																								<label for="purchasereturnno" class="custom-label">
																						  ...
																						  <div class="col-lg-7">
																								<input type="text" class="form-control  form-control-sm" id="purchasereturnno" name="purchasereturnno" required  value="<?=$rows[0]['purchasereturnno']?>" readonly>
																				...
																				<div class="col-lg-12">
																					 <div class="form-group row" style="align-items: center !important;">
																						  <div class="col-lg-5">
																								<label for="purchasereturndate" class="custom-label">
																									 <span class="text-danger">
																						  ...
																						  <?php
																								...
																								$dateformats->close();
																						  ?>
																								<input type="date" class="form-control  form-control-sm" id="purchasereturndate" name="purchasereturndate" required value="<?=$rows[0]['purchasereturndate']?>">
																						  </div>
																					 </div>
																				</div>
																		  <?php
										  ...
										  ?>
														  ...
														  <div class="accordion-item mb-1">
																		  ...
																		  <div class="row" style="width:100% !important;">
																				...
																				?>
																					 ...
																					 $(document).ready(function () {
																						  ...
																						  <?php
																								if ($access['purreturnbtwocnamerequired']=='Yes') {
																						  ?>
																								$("#twovendorname").removeAttr("required");
																						  <?php
																								}
																								if ($access['purreturnbtwocwphonerequired']=='Yes') {
																						  ?>
																...
																<div id="collapseOne" class="accordion-collapse collapse show" aria-labelledby="headingOne" style="padding-left:7px !important;padding-right: 7px !important;">
																	 ...
																	 <?php
																		  ...
																		  $sqlismainaccessuser->close();
																		  $sqlismainaccessusers->close();
																		  if ($infomainaccessuser['purreturnveninfo']=='one'||(($infomainaccessuser['purreturnveninfo']=='defone')||($infomainaccessuser['purreturnveninfo']=='deftwo'))) {
																	 ?>
																		  <div id="one">
																				<div class="row">
																					 <div class="col-lg-12">
																				...
																				<div class="row">
																					 ...
																					 <div class="col-lg-9">
																						  <div class="row mb-1" id="custaddressdiv" style="background-color:#fbfafa; color:#777777; display:none;margin: 0px !important;">
																								...
																								<div class="col-lg-6" style="padding-left: 3px !important;font-size: 13px !important;<?=(in_array('Billing Address', $fieldedit))?'':'display:none;'?>">
																									 <div id="ember532" class="popovercontainer address-group ember-view">
																										  <span class="font-small" style="color:#777777;">
																												...
																												<span id="purchasereturningaddressspan" class="text-blue cursor-pointer" href="#" data-bs-toggle="modal" data-bs-target="#purchasereturningaddressmodel" style="font-size:13px !important;">
																													 Change
																												</span>
																												<address id="purchasereturningaddressdiv" class="font-small ember-view" style="color:green;margin-bottom: 1px !important;">
																								...
																								<div class="modal fade" id="purchasereturningaddressmodel" tabindex="-1" role="dialog" aria-labelledby="purchasereturningaddressmodelLabel" aria-hidden="true">
																									 <div class="modal-dialog modal-dialog-centered" role="document">
																										  <div class="modal-content">
																												<div class="modal-header">
																													 <h5 class="modal-title" id="purchasereturningaddressmodelLabel">
																														  Billing Address
																													 </h5>
																												...
																												<div class="modal-footer" style="margin-bottom: -9px !important;margin-top: 34.5px !important;">
																													 <button   onclick="funpurchasereturningaddress()" class="btn btn-primary btn-sm btn-custom arlina-button expand-left" type="button"  name="submitpurchasereturning" value="Save">
																														  <span class="label">Save</span>
																														  <span class="spinner"></span>
																													 </button>
																								...
																								<script>
																								function funpurchasereturningaddress(){
																									 ...
																									 document.getElementById("purchasereturningaddressmodel").classList.remove("show");
																									 document.getElementById("purchasereturningaddressmodel").style.display="none";
																									 document.getElementById("purchasereturningaddressmodel").removeAttribute("aria-modal");
																									 document.getElementById("purchasereturningaddressmodel").removeAttribute("role");
																									 document.getElementById("purchasereturningaddressmodel").setAttribute("aria-hidden", "true");
																	 ...
																	 <?php
																		  ...
																		  if ($infomainaccessuser['purreturnveninfo']=='two'||(($infomainaccessuser['purreturnveninfo']=='defone')||($infomainaccessuser['purreturnveninfo']=='deftwo'))) {
																	 ?>
																		  <script type="text/javascript">
																		  function twoposans() {
																				...
																				<div class="row">
																					 <div class="col-lg-3">
																						  <label for="customername" class="custom-label">
																								<?= ($access['purreturnbtwocnamerequired']=='Yes')? '<span class="text-danger">'. $infomainaccessuserven['modulename'] . ' Name * </span>': $infomainaccessuserven['modulename'] . ' Name ' ?>
																						  </label>
																					 </div>
																					 <div class="col-lg-9">
																						  <input type="text" name="twovendorname" id="twovendorname" class="form-control form-control-sm" placeholder="<?= $infomainaccessuserven['modulename'] ?> Name" <?= ($access['purreturnbtwocnamerequired']=='Yes')? 'required': ' ' ?> value="<?= $rows[0]['vendorname'] ?>">
																						  ...
																						  <div class="row mb-0" id="custaddressdiv" style="background-color:#fbfafa; color:#777777; display:block;margin: 0px !important;padding: 9px 9px 0px 9px !important;">
																								...
																								<div class="row" style="padding:0px !important;margin: 0px !important;">
																									 <div class="col-lg-6" <?=(in_array('Billing Address', $fieldedit))?'':'style="display:none;"'?>>
																										  <label for="twopurchasereturning" class="custom-label" style="margin:0px !important;font-size: 13px !important;color: #777777;">
																												PURCHASERETURNING ADDRESS
																										  </label>
																										  <div class="row justify-content-center">
																												<div class="col-lg-12">
																														  ...
																														  <div class="col-sm-12">
																																<div class="input-group input-group-sm">
																																	 ...
																																	 function twobillpincodeiop(xpin) {
																																		  twopurchasereturnpinans = xpin.value;
																																		  twopurchasereturnpinlen = twopurchasereturnpinans.length;
																																		  let twoshoworhide = document.getElementById('twosameasbilling');
																																		  if (twoshoworhide.checked==true) {
																																				if (twopurchasereturnpinlen>=0) {
																																					 twoshippincode.value=twopurchasereturnpinans;
																																				}
																																		  }
																																	 }
																								...
																								<div class="row" style="padding:0px !important;margin: 0px !important;">
																									 <div class="col-lg-6" <?=(in_array('Work Phone', $fieldedit))?'':'style="display:none;"'?>>
																										  <label for="twoworkphone" class="custom-label" style="margin:0px !important;font-size: 12px !important;color: #777777;">
																												<?= ($access['purreturnbtwocwphonerequired']=='Yes')? '<span class="text-danger"> WORK PHONE * </span>': ' WORK PHONE ' ?>
																										  </label>
																										  <input type="text" class="form-control form-control-sm" id="twoworkphone" name="twoworkphone" placeholder="WORK PHONE" value="<?=$rows[0]['workphone']?>" <?= ($access['purreturnbtwocwphonerequired']=='Yes')? 'required': ' ' ?>>
												...
												// FOR B2C SAME AS PURCHASERETURNING CHECKBOX
												</script>
												<hr class="p-0 mb-1 mt-1">
										  <?php
												// }
														  ...
														  <div id="collapseOne" class="accordion-collapse collapse show" aria-labelledby="headingOne">
																<div class="accordion-body text-sm" style="padding-bottom: 0px !important;padding-top: 3px">
																	 <div class="table-responsive" style="overflow-y: hidden;">
																		  <table class="table table-bordered" id="purchasetable">
																				<thead>
																					 <tr>
																						  ...
																						  <th <?=((in_array('Item Details', $fieldedit))||($access['batchexpiryval']==1)||(in_array('Rate', $fieldedit))||(in_array('Quantity', $fieldedit))||(in_array('Taxable Value', $fieldedit))||(in_array('Tax Value', $fieldedit))||(in_array('Amount', $fieldedit))||(in_array('Sale Quantity', $fieldedit)))?'':'style="display:none !important;"'?>>
																						  </th>
																					 <?php
																					 ...
																					 ?>
																						  <th style="width: 87px !important;text-align: right !important;">
																								<?=($access['txtqtypurchasereturn'])?><span class="text-danger"> *</span>
																						  </th>
																					 <?php
																						  }
																					 ...
																					 ?>
																						  <th style="text-align: right !important;">
																								<?=($access['txttaxablepurchasereturn'])?>
																						  </th>
																					 <?php
																						  }
																		  ...
																		  ?>
																				...
																				?>
																					 ...
																					 <tr>
																						  ...
																						  <td class="tdmove" <?=((in_array('Item Details', $fieldedit))||($access['batchexpiryval']==1)||(in_array('Rate', $fieldedit))||(in_array('Quantity', $fieldedit))||(in_array('Taxable Value', $fieldedit))||(in_array('Tax Value', $fieldedit))||(in_array('Amount', $fieldedit))||(in_array('Sale Quantity', $fieldedit)))?'':'style="display:none !important;"'?>>
																						  ...
																						  <td data-label="RATE" <?=(in_array('Rate', $fieldedit))?'':'style="display:none !important;"'?>>
																								...
																								<div>
																									 <div class="dvi invpurchasereturngets" id="invpurchasereturngets<?=$i?>" style="margin-top:-22px;display: none;width: 250px;border-radius: 0px !important;">
																										  <table width="100%">
																												<tr style="border-bottom: 1px solid #cccccc;margin-bottom: 0px;">
																													 ...
																													 <td align="center" style="width: 50% !important;display: inline-block !important;text-align: center;">
																														  <span onclick="othersgetfun(<?=$i?>)" id="purchasereturnonoff<?=$i?>">
																																OTHERS
																														  </span>
																													 </td>
																						  ...
																						  <td data-label="<?=($access['txtqtypurchasereturn'])?>" <?=(in_array('Quantity', $fieldedit))?'':'style="display:none !important;"'?>>
																								<div>
																						  ...
																						  <td data-label="<?=($access['txttaxablepurchasereturn'])?>" <?=(in_array('Taxable Value', $fieldedit))?'':'style="display:none;"'?>>
																								<div>
																									 <span style="font-size:15px !important;">
																										  <?php echo $resmaincurrencyans; ?>
																									 </span>
																								...
																								<div <?=(in_array('Discount', $fieldedit))?'':'style="display:none !important;"'?>>
																									 <span id="productprodiscountspan<?=$i?>" style=" font-size:11px;white-space: nowrap !important;">
																										  <?=$access['txtprodispurchasereturn']?>:
																						  ...
																						  <td data-label="<?=$access['purreturntxtsqty']?>" <?=(in_array('Sale Quantity', $fieldedit))?'':'style="display:none;"'?>>
																								<div>
																		  ...
																		  .invpurchasereturngets span:hover{
																				background-color: #1BBC9B !important;
																				color: #fff !important;
																		  }
																...
																<div class="table-responsive mt-1" id="gsttablediv" <?=(in_array('Tax Informations', $fieldedit))?'':'style="display:none;"'?>>
																	 <table class="table table-bordered" id="gsttable" style="font-size:12px;">
																		  ...
																		  <?php
																				...
																				$sqlitaxes=$con->prepare("SELECT * FROM pairpurchasereturns WHERE franchisesession=? AND createdid=? AND purchasereturnno=? AND purchasereturndate=? ORDER BY id ASC");
																				$sqlitaxes->bind_param("ssss", $_SESSION["franchisesession"], $companymainid, $purchasereturnno, $purchasereturndate);
																				$sqlitaxes->execute();
																...
																<div class="col" style="width:34px !important;margin-top: 1px !important;margin-left: -9px !important;">
																	 <div class="form-group row">
																		  <div class="col-7 col-sm-6">
																				...
																				<a class="btn btn-primary btn-sm btn-custom-grey" href="purchasereturns.php" style="margin-top:-3px !important;">
																					 Cancel
																				</a>
																		  </div>
													 ...
													 <div class="modal-body" style="height: max-content;">
														  ...
														  <table class="table notforfinaloutputsave showhideload">
																<tr>
																	 ...
																	 <td>
																		  <input type="text" name="validpurchasereturnno" id="validpurchasereturnno" class="form-control form-control-sm finalsubmitrequired" readonly required>
																	 </td>
																</tr>
																<tr>
																	 <th style="text-align:left !important">
																	 ...
																	 <td>
																		  <input type="date" name="validpurchasereturndate" id="validpurchasereturndate" class="form-control form-control-sm finalsubmitrequired" readonly required>
																	 </td>
																</tr>
																<tr>
																	 <th style="text-align:left !important">
																	 ...
																	 <td>
																		  <input type="number" name="validpurchasereturnamount" id="validpurchasereturnamount" class="form-control form-control-sm finalsubmitrequired" readonly required>
																	 </td>
																</tr>
																<tr>
																	 ...
																	 <td>
																		  <div class="col-lg-12">
																				<select class="select2-field form-control  form-control-sm finalsubmitrequired" name="purchasereturnterm" id="purchasereturnterm" required onchange="ihaveblock()">
																				<?php
																				...
																				?>
																					 <option value="<?= $info['term'] ?>" <?=($info['term']==$rows[0]['purchasereturnterm'])?'selected':''?>>
																						  <?= $info['term'] ?>
																					 </option>
																				<?php
																						  }
																...
																<tr id="validpaidblock">
																	 ...
																	 <td>
																		  ...
																		  function paidamountcalc(){
																				...
																				if((validpurchasereturnamount!='')&&(validpaidamount!='')){
																					 var balance=parseFloat(validpurchasereturnamount)-parseFloat(validpaidamount);
																...
																<tr id="duedateblock">
																	 ...
																	 <td>
																		  ...
																		  function funduedates(){
																				...
																				var today="";
																				if(duedates!=''){
																					 // console.log(purchasereturndate);
																					 // console.log(duedates);
																					 today=addDays(purchasereturndate, duedates);
														  ...
														  <script type="text/javascript">
														  var ihaveterm = document.getElementById("purchasereturnterm").value;
...
$.ajax({
type: "GET",
url: 'editfetch.php?term=' + sIndexEdit + '&limitings=5&purchasereturnno=<?=$_GET['purchasereturnno']?>&purchasereturndate=<?=$_GET['purchasereturndate']?>&type=purchasereturn&i='+ivalue+'',
success: function (result) {
...
<?php
		  ...
		  header("Location:purchasereturns.php?error=No Information Found");
	 }
}
else{
	 header("Location:purchasereturns.php?error=No Information Found");  
