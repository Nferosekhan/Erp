...
if(isset($_POST['grandtotal'])){
	...
	$salesordernos = htmlspecialchars($_POST['salesorderno'], ENT_QUOTES, 'UTF-8');
	$salesorderdates = htmlspecialchars($_POST['salesorderdate'], ENT_QUOTES, 'UTF-8');
		...
		for($i=0; $i<count($_POST['productname']); $i++){
			...
			if($productname!=''){
				...
				if($sql->execute()){
					...
					$updsaleorderstatus = $con->prepare("UPDATE pairsalesorders SET convertstatus=1,invoiceno=?,invoicedate=? WHERE salesorderno=? AND salesorderdate=? AND createdid=? AND franchisesession=? AND tdelete=0");
    				$updsaleorderstatus->bind_param("ssssss", $invoiceno, $invoicedate, $salesordernos, $salesorderdates, $companymainid, $_SESSION["franchisesession"]);
    				$updsaleorderstatus->execute();
    				$updsaleorderstatus->close();
    				// if($access['definvpay']=='split'){}
    				// else{
						if($invoiceterm=='CASH'){
							...
							if($accountname!=''){
								...
								$sqlaccdefault = $con->prepare("INSERT INTO pairledgers (createdon,createdid,createdby,franchisesession,ledgerdate,ledgerno,chartaccountname,chartaccountid,customerid,customername,ledgerdebit,ledgercredit,subledgerdebit,subledgercredit,totalledgerdebit,totalledgercredit,balanceledgerdebit,balanceledgercredit,publicid,privateid,type) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, '0', ?, '0', ?, '0', ?, '0', '0', ?, ?, 'invoice')");
			    				$sqlaccdefault->bind_param("sisisssssssssss", $times, $companymainid, $_SESSION["unqwerty"], $_SESSION["franchisesession"], $invoicedate, $invoiceno, $defaccountname, $defaccountid, $customerid, $customername, $productrate, $productrate, $grandtotal, $publicidmanual, $privateidmanual);
			    				$sqlaccdefault->execute();
			    				$sqlaccdefault->close();
						...
						if (strpos($runtheans[$ans], '|||')!==false) {
							...
							if($selbillmargins->num_rows>0){
								...
								$reduceqty = $runtheansnxt[4];
									...
									$sqlsellingmargins = $con->prepare("INSERT INTO pairmargins (createdon,createdid,franchisesession,type,billername,billerid,billingno,billingdate,productid,productname,batch,expiry,quantity,mrp,rate,nowstatus,taxablevalue,discountvalue) VALUES (?, ?, ?, 'selling', ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'added', ?, ?)");
    								$sqlsellingmargins->bind_param("siisississssssss", $times, $companymainid, $_SESSION["franchisesession"], $runtheansnxt[1], $runtheansnxt[0], $runtheansnxt[2], $runtheansnxt[3], $productid, $productname, $batch, $expdate, $reduceqty, $mrp, $productrate, $productvalue, $prodiscount);
	...
	<div class="container-fluid py-4 bg-body">
		...
		<?php
			...
			if((isset($_GET['salesorderno']))&&(isset($_GET['salesorderdate']))){
				...
				$sql=$con->prepare("SELECT * FROM pairsalesorders WHERE franchisesession=? AND createdid=? AND salesorderno=? AND salesorderdate=? AND tdelete=0 ORDER BY id ASC");
					...
					<div class="card mb-4 mt-5">
						<div class="card-body p-3" style="font-family:'Myriad Set Pro','Helvetica Neue',Helvetica,Arial,sans-serif;">
							...
							<form autocomplete="off" id="invoiceform" action="" onsubmit="return checkvalidate()" method="post" enctype="multipart/form-data" class="form-horizontal" role="form">
								<input type="hidden" name="salesorderno" id="salesorderno" value="<?=$_GET['salesorderno']?>">
								<input type="hidden" name="salesorderdate" id="salesorderdate" value="<?=$_GET['salesorderdate']?>">
																...
																<div class="form-group row" style="align-items: center !important;">
																	...
																	<div class="col-lg-7">
																		<select class="select2-field form-control form-control-sm" name="saleperson" id="saleperson">
																			...
																			?>
																				<option <?=($rows[0]['saleperson']==$info['saleperson'])?'selected':''?> value="<?= $info['saleperson'] ?>"><?= $info['saleperson'] ?></option>
																			<?php
																				}
																				$sqli->close();
																				$sqlis->close();
																	...
																	<div class="row mb-0" id="custaddressdiv" style="background-color:#fbfafa; color:#777777; display:block;margin: 0px !important;padding: 9px 9px 0px 9px !important;">
																		...
																		<div class="row" style="padding:0px !important;margin: 0px !important;">
																			<div class="col-lg-6" <?=(in_array('Work Phone', $fieldadd))?'':'style="display:none;"'?>>
																				...
																				<input type="text" class="form-control form-control-sm" id="twoworkphone" name="twoworkphone" placeholder="Work Phone" value="<?=$rows[0]['workphone']?>" <?= ($access['invbtwocwphonerequired']=='Yes')? 'required': ' ' ?>>
																...
																<tr>
																	...
																	<td data-label="BATCH" <?=($access['batchexpiryval']==1)?'':'style="display:none;"'?>>
																		...
																		?>
																			...
																			<input type="date" <?= ($access['invbatchdef']=='availdrop')?'readonly':'' ?> <?=(($access['invbatexprequired']=='Yes')?'required':'')?> name="expdate[]" id="expdate<?=$i?>" class="form-control form-control-sm proitemselect" style="display:none;width: 94px !important;padding: 0px !important;height: 18px !important;" value="<?=$row['expdate']?>" onChange="productcalc(<?=$i?>)">
												...
												<div class="col" style="width:34px !important;margin-top: 1px !important;margin-left: -9px !important;">
													<div class="form-group row">
														<div class="col-7 col-sm-6">
															<button class="btn btn-primary btn-sm btn-custom arlina-button expand-left" style="margin-left: 10px !important;margin-top: -3px !important;" type="submit" id="trigggerbutton" name="triggerbutton" value="Submit"  <?=(($access['invconvertmode']=='defmode')?'onClick="triggerpayment(\'\',\'\',\'\',\'1\',\'1\')"':'onClick="this.form.submit()"')?>>
																<span class="label"><?=(($access['invconvertmode']=='defmode')?'NEXT':'SAVE')?></span>
																<span class="spinner"></span>
															</button>
												...
												<div class="modal-body" style="height: max-content;">
													...
													<table class="table notforfinaloutputsave showhideload">
														...
														<tr style="<?=(($access['definvpay']=='split')?'display: none;':'')?>">
															...
															<td style="background-color: #e9ecef;">
																<div class="col-lg-12">
																	...
																	?>
																				<option value="<?= $info['term'] ?>" <?=($info['term']==(($access['invconvertmode']=='defmode')?'CASH':'CREDIT'))?'selected':''?>>
																					<?= $info['term'] ?>
																				</option>
																	<?php
																			}
														...
														<tr id="validpaidblock">
															...
															<td style="background-color: #e9ecef;">
																<input type="number" name="validpaidamount" id="validpaidamount" class="form-control form-control-sm finalsubmitrequired" placeholder="0.00" required oninput="paidamountcalc()" value="0.00">
																<script>
...
// payment confirm
<?php
	if($access['invconvertmode']=='defmode'){
?>
setTimeout(function() {
	triggerpayment('','','','1','1');
},1500);
...
function checkvalidate() {
	if (isValidFinal) {
		...
		return <?=(($access['invconvertmode']=='defmode')?'false':'true')?>;
	}
}
// FOR FINAL SUBMIT
// payment confirm
