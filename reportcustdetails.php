...
if (isset($_POST['submitcustomizes'])) {
	...
	$sqlreportmoduless=$con->prepare("SELECT * FROM pairreportmodules WHERE types='custdetails' ORDER BY id ASC");
	...
	foreach ($newmodules as $newmoduleskey) {
		...
		$modcolumncol=$coltypemod."custdetails";
	...
	$sqlupreport = $con->prepare("UPDATE pairreports SET rowcolumns=? WHERE franchiseid=? AND createdid=? AND types='custdetails'");
	...
	$sqlreportfav = $con->prepare("UPDATE pairreportfavourites SET reporturl=? WHERE franchisesession=? AND createdid=? AND reportnames='custdetails'");
	$sqlreportfav->bind_param("sss", $newReportUrl, $_SESSION['franchisesession'], $companymainid);
	$sqlreportfav->execute();
	$sqlreportfav->close();

	$sqlreport = $con->prepare("UPDATE pairreports SET reportperiod=?,filtername=?,companyname=?,dateprepare=?,timeprepare=?,gstrule=? WHERE franchiseid=? AND createdid=? AND types='custdetails'");
	...
	header("Location:reportcustdetails.php?datefrom=".date($dateformatforurlredirection,strtotime($from))."&dateto=".date($dateformatforurlredirection,strtotime($to))."&customerid=".$_GET['customerid']."");
}

$sqlreportviews=$con->prepare("SELECT * FROM pairreports WHERE franchiseid=? AND createdid=? AND types='custdetails'");
									...
									<div class="row">
										<div class="col-lg-6">
											<p class="mb-3" style="color:black;font-size: 20px;margin-top: -8px;">
												...
												&gt;
												<a href="reportcustbalance.php?datefrom=<?=$_GET['datefrom']?>&dateto=<?=$_GET['dateto']?>" style="color: #1878F1">
													Balance
												</a>
												&gt;
												Balance Details
																			...
																			<div id="customizerowcolumns" class="accordion-collapse collapse show" aria-labelledby="customizerowcolumn">
																				<div class="accordion-body text-sm">
																					<div class="row justify-content-center">
																						...
																						<?php
																							$newans=array();
																							$sqlreportaccesss=$con->prepare("SELECT * FROM pairreports WHERE franchiseid=? AND createdid=? AND types='custdetails'");
																								...
																								<div class="col-lg-6 my-1" style="padding-left: 0px !important;">
																									<div class="custom-control custom-checkbox mr-sm-2" style="padding: 0px 12px !important;">
																										<div class="input-group input-group-sm" id="flaghead">
																											...
																											<input type="checkbox" class="custom-control-input" name="<?= $coltypemod; ?>custdetails" id="<?= $coltypemod; ?>custdetails" checked disabled>
																											<label class="custom-control-label custom-label" for="<?= $coltypemod; ?>custdetails">
											...
											<div class="tab-pane fade" id="nav-profile" role="tabpanel" aria-labelledby="nav-profile-tab">
												<div class="table-responsive m-3" id="histables">
													<table class="table table-bordered">
														...
														<tbody>
														<?php
															$sqluse=$con->prepare("SELECT * FROM pairusehistory WHERE usetype = 'REPcustdetails' AND useid = ? ORDER BY createdon DESC");
																		...
																		<tr style="height:1px;">
																			<td width="100%" style="padding:10px;">
																				<table id="print-are1" style="border: 1px solid #eee;" width="100%">
																					<thead>
																						<tr>
																							<td class="text-uppercase" style="width:23%;border:1px solid #eee;padding-left: 10px;">
																							...
																							<td class="text-uppercase" style="text-align: left !important;width: 14%;border:1px solid #eee;padding-left: 10px;">
																								<span style="font-size:13px;color:black;">
																									Invoice Date
																								</span>
																							</td>
																							...
																							<td class="text-uppercase" style="width:15%;border:1px solid #eee;padding-left: 10px;">
																								<span style="font-size:13px;color:black;">
																									Over Due
																								</span>
																							</td>
																					...
																					<tbody id="myTable">
																					<?php
																						$sqliinvoices=$con->prepare("SELECT 'invoice' AS type,pi.customername, pi.customerid, pi.invoiceamount AS amount, pi.invoiceno AS noteno, pi.invoicedate AS notedate, pi.duedate, COALESCE(SUM(psh.amount), 0) AS paidamount,(pi.invoiceamount - COALESCE(SUM(psh.amount), 0)) AS balanceamount FROM pairinvoices pi LEFT JOIN pairsalespayhistory psh ON pi.invoiceno = psh.invoiceno AND pi.invoicedate = psh.invoicedate WHERE pi.franchisesession = ? AND pi.createdid = ? AND (pi.invoicedate >= ? AND pi.invoicedate <= ?) AND pi.customerid = ? GROUP BY pi.invoiceno, pi.invoicedate UNION SELECT 'creditnote' AS type,pcn.customername, pcn.customerid, pcn.creditnoteamount AS amount, pcn.creditnoteno AS noteno, pcn.creditnotedate AS notedate, pcn.duedate,COALESCE(SUM(pcnph.amount), 0) AS paidamount,(pcn.creditnoteamount - COALESCE(SUM(pcnph.amount), 0)) AS balanceamount FROM paircreditnotes pcn LEFT JOIN paircreditnotepayhistory pcnph ON pcn.creditnoteno = pcnph.creditnoteno AND pcn.creditnotedate = pcnph.creditnotedate WHERE pcn.franchisesession = ? AND pcn.createdid = ? AND (pcn.creditnotedate >= ? AND pcn.creditnotedate <= ?) AND pcn.customerid = ? GROUP BY pcn.creditnoteno, pcn.creditnotedate ORDER BY notedate ASC LIMIT 36");
																						$sqliinvoices->bind_param("ssssssssss", $_SESSION['franchisesession'], $companymainid, $datefrom, $dateto, $_GET['customerid'], $_SESSION['franchisesession'], $companymainid, $datefrom, $dateto, $_GET['customerid']);
																					...
																					?>
																						<tr style="height: 30px;">
																							<td  data-label="Customer Name" style="border: 1px solid #eee;padding-left: 10px;font-size: 13px;">
																								<?=$infoinvoice['customername']?>
																							</td>
																							<td  data-label="Invoice Number" style="border: 1px solid #eee;padding-left: 10px;font-size: 13px;">
																								<?=$infoinvoice['noteno']?>
																							</td>
																							<td  data-label="Invoice Date" style="border: 1px solid #eee;padding-left: 10px;font-size: 13px;">
																								<?=date($datemainphp,strtotime($infoinvoice['notedate']))?>
																							</td>
																							<td  data-label="Invoice Amount" style="border: 1px solid #eee;padding-right: 10px;font-size: 13px;text-align: right;">
																								<?=number_format($infoinvoice['amount'],2,'.',',')?>
																							</td>
																							<td  data-label="Balance" style="border: 1px solid #eee;padding-right: 10px;font-size: 13px;text-align: right;">
																								<?=number_format($balanceamount,2,'.',',')?>
																							</td>
																							<td  data-label="Over Due" style="border: 1px solid #eee;padding-left: 10px;font-size: 13px;">
																								<?=(($balanceamount>0)?calculateOverdueDays($infoinvoice['duedate'], date("Y-m-d")):'No Due')?>
																							</td>
																						</tr>
																					<?php
																						}
																						if(mysqli_num_rows($sqliinvoice)<36){
																							echo '<tr style="height: '.((36-mysqli_num_rows($sqliinvoice))*30).'px;"><td  data-label="Customer Name" style="border: 1px solid #eee;padding-left: 10px;font-size: 13px;"> </td><td  data-label="Invoice Number" style="border: 1px solid #eee;padding-left: 10px;font-size: 13px;"> </td><td  data-label="Invoice Date" style="border: 1px solid #eee;padding-left: 10px;font-size: 13px;"> </td><td  data-label="Invoice Amount" style="border: 1px solid #eee;padding-right: 10px;font-size: 13px;text-align: right;"> </td><td  data-label="Balance" style="border: 1px solid #eee;padding-right: 10px;font-size: 13px;text-align: right;"> </td><td  data-label="Over Due" style="border: 1px solid #eee;padding-left: 10px;font-size: 13px;"> </td></tr>';
																					?>
																						<tr style="height: 30px;font-weight: bold;">
																							<td style="border: 1px solid #eee;padding-left: 10px;" colspan="3">
																								Total
																							</td>
																					...
																					<?php
																						...
																						$pageinitnumpage = 1;
																						$pagetotnumpage = 1;
																						if (mysqli_num_rows($sqliinvoice)>0) {
																							$sqltotlistpages=$con->prepare("SELECT 'invoice' AS type,pi.customername, pi.customerid, pi.invoiceamount AS amount, pi.invoiceno AS noteno, pi.invoicedate AS notedate, pi.duedate, COALESCE(SUM(psh.amount), 0) AS paidamount,(pi.invoiceamount - COALESCE(SUM(psh.amount), 0)) AS balanceamount FROM pairinvoices pi LEFT JOIN pairsalespayhistory psh ON pi.invoiceno = psh.invoiceno AND pi.invoicedate = psh.invoicedate WHERE pi.franchisesession = ? AND pi.createdid = ? AND (pi.invoicedate >= ? AND pi.invoicedate <= ?) AND pi.customerid = ? GROUP BY pi.invoiceno, pi.invoicedate UNION SELECT 'creditnote' AS type,pcn.customername, pcn.customerid, pcn.creditnoteamount AS amount, pcn.creditnoteno AS noteno, pcn.creditnotedate AS notedate, pcn.duedate,COALESCE(SUM(pcnph.amount), 0) AS paidamount,(pcn.creditnoteamount - COALESCE(SUM(pcnph.amount), 0)) AS balanceamount FROM paircreditnotes pcn LEFT JOIN paircreditnotepayhistory pcnph ON pcn.creditnoteno = pcnph.creditnoteno AND pcn.creditnotedate = pcnph.creditnotedate WHERE pcn.franchisesession = ? AND pcn.createdid = ? AND (pcn.creditnotedate >= ? AND pcn.creditnotedate <= ?) AND pcn.customerid = ? GROUP BY pcn.creditnoteno, pcn.creditnotedate ORDER BY notedate ASC");
																							$sqltotlistpages->bind_param("ssssssssss", $_SESSION['franchisesession'], $companymainid, $datefrom, $dateto, $_GET['customerid'], $_SESSION['franchisesession'], $companymainid, $datefrom, $dateto, $_GET['customerid']);
										...
										<table id="print-are3" style="border: 1px solid #eee;display: none;" width="100%">
											<thead>
												<tr>
													<td class="text-uppercase" style="width:23%;border:1px solid #eee;padding-left: 10px;"><span style="font-size:13px;color:black;">Customer Name</span></td>
													...
													<td class="text-uppercase" style="text-align: left !important;width: 14%;border:1px solid #eee;padding-left: 10px;"><span style="font-size:13px;color:black;">Invoice Date</span></td>
													<td class="text-uppercase" style="text-align: right !important;width: 16%;border:1px solid #eee;padding-right: 10px;"><span style="font-size:13px;color:black;white-space: nowrap;overflow: hidden;">Invoice Amount</span></td>
													<td class="text-uppercase" style="text-align: right !important;width: 15%;border:1px solid #eee;padding-right: 10px;"><span style="font-size:13px;color:black;white-space: nowrap;overflow: hidden;">Balance</span></td>
													<td class="text-uppercase" style="width:15%;border:1px solid #eee;padding-left: 10px;"><span style="font-size:13px;color:black;">Over Due</span></td>
												</tr>
											</thead>
											<tbody id="myTables">
											</tbody>
...
const pagechanges = (page, pageSize) => {
	...
	ajaxRequestview = $.ajax({
		type: "GET",
		url: 'reportviewsearch.php?term='+perpages+'&limitings='+($('#limitforpagenum').val())+'&datefrom=<?=$datefrom?>&dateto=<?=$dateto?>&dif=custdetails&lastornone='+lastornone+'&customerid=<?=$_GET['customerid']?>',
...
$(function() {
	...
	function(start, end, label) {
		...
		window.open("reportcustdetails.php?datefrom="+datefrom+"&dateto="+dateto+"&customerid=<?=$_GET['customerid']?>","_self");
	});
});
...
function converHTMLFileToPDF() {
	...
	var finaltimer = setTimeout(function(){
		...
		doc.autoTable({
			...
			didDrawPage: header, 
			columnStyles: { 
				0: { halign: 'left', cellWidth: 123},
				1: { halign: 'left', cellWidth: 70},
				2: { halign: 'left', cellWidth: 70},
				3: { halign: 'right', cellWidth: 87},
				4: { halign: 'right', cellWidth: 87},
				5: { halign: 'left', cellWidth: 77}
			}
		});
		obj = '<object id="pdfObjObj" data="'+doc.output('bloburi')+'" type="application/pdf" width="90%" height="550"></object>';
		$.ajax({
			type: "GET",
			url: 'reporthistory.php?types=custdetails&titles=Balance Details&printdownpdfcsv=Printed PDF&fromto='+fromtopdf+'',
			success: function (result) {
				console.log(result);
			},
			error: function (error) {
...
function downloadaspdf() {
	...
	var finaltimer = setTimeout(function(){
		...
		$.ajax({
			type: "GET",
			url: 'reporthistory.php?types=custdetails&titles=Balance Details&printdownpdfcsv=Downloaded PDF&fromto='+fromtopdf+'',
			success: function (result) {
				console.log(result);
			},
			error: function (error) {
		...
		doc.save('Balance-Details-<?=$datefrom?>-To-<?=$dateto?>');
...
function downloadascsv() {
	...
	var finaltimer = setTimeout(function(){
		...
		link.setAttribute("download", "Balance-Details-<?=$datefrom?>-To-<?=$dateto?>");
		document.body.appendChild(link);
		link.click();
		$.ajax({
			type: "GET",
			url: 'reporthistory.php?types=custdetails&titles=Balance Details&printdownpdfcsv=Downloaded CSV&fromto='+fromtopdf+'',
			success: function (result) {
				console.log(result);
			},
			error: function (error) {
...
document.addEventListener('DOMContentLoaded', function() {
	...
	function makeAjaxRequest() {
		ajaxRequest = $.ajax({
			type: "GET",
			url: 'check.php?term=custdetails&datefrom=<?=$datefrom?>&dateto=<?=$dateto?>&limitfrom=0&limitto=10000&customerid=<?=$_GET['customerid']?>',
			success: function (result) {
				if (result != mytabprev) {
