...
?>
 ...
 <?php
$sqlismainaccessuser=mysqli_query($con, "select moduleno, moduleprefix, modulesuffix, modulename from pairmainaccess where franchiseid='".$_SESSION['franchisesession']."' and moduletype='Credit Notes' order by id  asc");
$infomainaccessuser=mysqli_fetch_array($sqlismainaccessuser);
...
if($customerid!='')
{
    ...
    while($infos=mysqli_fetch_array($sqls))
    {
        ...
        echo '<td data-label="'. strtoupper($infomainaccesscreditnote['modulename']) .' DATE" style="vertical-align:middle !important;text-align:left;">'.date('d/m/Y',strtotime($infos['creditnotedate'])).'</td>';
        echo '<td data-label="'. strtoupper($infomainaccesscreditnote['modulename']) .' NUMBER" style="vertical-align:middle !important;text-align:left;">'.$infos['creditnoteno'].'</td>';
...
<!-- <button name="submit"
                                                          ...
                                                          <a class="btn btn-primary btn-sm btn-custom-grey" href="customerrefunds.php" style="font-family:'Myriad Set Pro','Helvetica Neue',Helvetica,Arial,sans-serif;">Cancel</a>
    </div>
</div>
            </div>
          </div>
