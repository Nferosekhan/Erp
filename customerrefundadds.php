...
if(isset($_POST['edit']))
{
$sqlismainaccesspublicnamecheck=mysqli_query($con, "select * from pairmainaccess where createdid='$companymainid' and moduletype='Customer Refunds' and franchiseid='".$_SESSION['franchisesession']."' order by id  asc");
$infomainaccesspublicnamecheck=mysqli_fetch_array($sqlismainaccesspublicnamecheck);
$privatesqlcheck=mysqli_query($con,"select count(privateid) from paircreditnotepayments where createdid='$companymainid' and franchisesession='".$_SESSION['franchisesession']."'");
...
$sqldelacc=mysqli_query($con,"delete from pairledgers where createdid='$companymainid' AND franchisesession='".$_SESSION['franchisesession']."' AND ledgerno='$nosss' AND ledgerdate='$datesss' AND type='creditnote payment'");

$selforsel = mysqli_query($con,"select id from paircreditnotepayhistory where customerid='$customerid' and creditnoteno='$nosss' and creditnotedate='$datesss'");
...
$sqlhis=mysqli_query($con,"select * from paircreditnotepayments where id='$id'");
...
$checkforhis = 0;
$sqle=mysqli_query($con,"delete from paircreditnotepayhistory where paymentid='$id'");	
for($i=0; $i<count($_POST['nos']); $i++)
{
	$paymentid=$id;
	$nos=$_POST['nos'][$i];
	...
	if(isset($_POST['payments'.$i]))
	{
		...
		if($amount==$payment)
		{
			$status='1';
					...
					if($dates!='')
				{
					if($ch!='')
					{
						$ch.='<br> '.(($checkforhis==0)?'<span style="color:royalblue;">'.$infomainaccesscreditnote['modulename'].' Information</span> <br> ':'').' (Selected) <br> '.$infomainaccesscreditnote['modulename'].' Date <span style="color:green;" id="prohisfromtospan">( '.date($datefor,strtotime($dates)).' ) </span>';
					}
					else
					{
						$ch.=''.(($checkforhis==0)?'<span style="color:royalblue;">'.$infomainaccesscreditnote['modulename'].' Information</span> <br> ':'').' (Selected) <br> '.$infomainaccesscreditnote['modulename'].' Date <span style="color:green;" id="prohisfromtospan">( '.date($datefor,strtotime($dates)).' ) </span>';
					}					
				}
				$checkforhis++;
                if($nos!='')
				{
					if($ch!='')
					{
						$ch.='<br> '.$infomainaccesscreditnote['modulename'].' Number <span style="color:green;" id="prohisfromtospan">( '.$nos.' ) </span>';
					}
					else
					{
						$ch.=''.$infomainaccesscreditnote['modulename'].' Number <span style="color:green;" id="prohisfromtospan">( '.$nos.' ) </span>';
					}					
				}
                if($invamount!='')
				{
					if($ch!='')
					{
						$ch.='<br> '.$infomainaccesscreditnote['modulename'].' Amount <span style="color:green;" id="prohisfromtospan">( '.$invamount.' ) </span>';
					}
					else
					{
						$ch.=''.$infomainaccesscreditnote['modulename'].' Amount <span style="color:green;" id="prohisfromtospan">( '.$invamount.' ) </span>';
					}					
				}
                if($balamount!='')
				{
		...
		$sqle=mysqli_query($con,"insert into paircreditnotepayhistory set createdid='$companymainid', createdby='".$_SESSION["unqwerty"]."', franchisesession='".$_SESSION["franchisesession"]."', createdon='$createdon', paymentid='$paymentid', customerid='$customerid', creditnoteno='$nos', creditnotedate='$dates', amount='$amount',type='payment'");
						...
						$sqlismodulespublicnamemanuals->close();
							...
							$selectinvdetail=$con->prepare("SELECT totalitems FROM paircreditnotes WHERE creditnoteno=? AND creditnotedate=? AND customerid=? AND franchisesession=? AND createdid=?");
...
if($sqler)
{
$sqlicreditnote=mysqli_query($con, "select creditnoteamount, creditnotedate, creditnoteno from paircreditnotes where franchisesession='".$_SESSION['franchisesession']."' and createdid='$companymainid' and customerid='$customerid' GROUP BY creditnotedate, creditnoteno order by creditnotedate desc, creditnoteno desc");
$creditnoteamount=0;
$balanceamount=0;
$currentamount=0;
$overdueamount=0;
while($infocreditnote=mysqli_fetch_array($sqlicreditnote))
{
$creditnoteamount+=(float)$infocreditnote['creditnoteamount'];
$paidamount=0;
$sqlcustomerpay=mysqli_query($con,"select amount from paircreditnotepayhistory where franchisesession='".$_SESSION['franchisesession']."' and createdid='$companymainid' and creditnoteno='".$infocreditnote['creditnoteno']."' and creditnotedate='".$infocreditnote['creditnotedate']."' and customerid='$customerid' order by id desc");
while($infocustomerpay=mysqli_fetch_array($sqlcustomerpay))
{
$paidamount+=(float)$infocustomerpay['amount'];
}
...
$currentamount+=((float)$infocreditnote['creditnoteamount']-$paidamount);
		...
		$sqliinvoice->close();
		$sqliinvoices->close();
$cussqlup = mysqli_query($con,"update paircustomers set invoiceamount='$creditnoteamount',balanceamount='$balanceamount',currentamount='$currentamount',overdueamount='$overdueamount' where id='$customerid'");
}
				...
				if($ch!='')
				{
				$sqluse=mysqli_query($con, "insert into pairusehistory set usetype='CUSREFUND', createdon='$times',  createdby='".$_SESSION["unqwerty"]."', useid='$id', useremarks='".$ch." '");
				}
header("Location: customerrefundview.php?receiptno=".$receiptno."&receiptdate=".$receiptdate."&id=".$id."&remarks=Update Successfully");
}
else
{
header("Location: customerrefundview.php?receiptno=".$receiptno."&receiptdate=".$receiptdate."&id=".$id."&remarks=Error");
}
// }
// else{
// header("Location: customerrefunds.php?error=Already Added");
// }
}
else
{
		...
		if($amount==$payment)
		{
			$status='1';
                ...
                if($dates!='')
				{
					if($chadd!='')
					{
						$chadd.='<br> '.(($checkforhis==0)?'<span style="color:royalblue;">'.$infomainaccesscreditnote['modulename'].' Information</span> <br> ':'').' (Selected) <br> '.$infomainaccesscreditnote['modulename'].' Date <span style="color:green;" id="prohisfromtospan">( '.date($datefor,strtotime($dates)).' ) </span>';
					}
					else
					{
						$chadd.=''.(($checkforhis==0)?'<span style="color:royalblue;">'.$infomainaccesscreditnote['modulename'].' Information</span> <br> ':'').' (Selected) <br> '.$infomainaccesscreditnote['modulename'].' Date <span style="color:green;" id="prohisfromtospan">( '.date($datefor,strtotime($dates)).' ) </span>';
					}					
				}
				$checkforhis++;
                if($nos!='')
				{
					if($chadd!='')
					{
						$chadd.='<br> '.$infomainaccesscreditnote['modulename'].' Number <span style="color:green;" id="prohisfromtospan">( '.$nos.' ) </span>';
					}
					else
					{
						$chadd.=''.$infomainaccesscreditnote['modulename'].' Number <span style="color:green;" id="prohisfromtospan">( '.$nos.' ) </span>';
					}					
				}
                if($invamount!='')
				{
					if($chadd!='')
					{
						$chadd.='<br> '.$infomainaccesscreditnote['modulename'].' Amount <span style="color:green;" id="prohisfromtospan">( '.$invamount.' ) </span>';
					}
					else
					{
						$chadd.=''.$infomainaccesscreditnote['modulename'].' Amount <span style="color:green;" id="prohisfromtospan">( '.$invamount.' ) </span>';
					}					
				}
                if($balamount!='')
				{
...
$cussqlup = mysqli_query($con,"update paircustomers set invoiceamount='$creditnoteamount',balanceamount='$balanceamount',currentamount='$currentamount',overdueamount='$overdueamount' where id='$customerid'");
	...
	$sql3=mysqli_query($con, "update pairmainaccess set modulesuffix=modulesuffix+1 where franchiseid='".$_SESSION['franchisesession']."' and moduletype='Customer Refunds'");
	$sql333=mysqli_query($con, "update pairmainaccess set modulesuffix=modulesuffix+1 where franchiseid='".$_SESSION['franchisesession']."' and moduletype='Manual Journals'");
				if($chadd!='')
				{
				$sqluse=mysqli_query($con, "insert into pairusehistory set usetype='CUSREFUND', createdon='$times',  createdby='".$_SESSION["unqwerty"]."', useid='$id', useremarks='".$chadd." '");
				}
   

if(isset($_POST['open']))
{
header("Location: customerrefundadd.php?remarks=Added Successfully");	
}
else
{
header("Location: customerrefundview.php?receiptno=".$receiptno."&receiptdate=".$receiptdate."&id=".$id."&remarks=Added Successfully");	
}
