...
if($customerid!='')
{
	...
	echo '<thead>';
	echo '<tr class="info">';
	echo '<th style="vertical-align:middle !important;border:1px solid #ddd !important;"><span style="display:inline-block;width:77px;">'. strtoupper($infomainaccesssalereturn['modulename']) .' NO</span></th>';
	echo '<th style="vertical-align:middle !important;border:1px solid #ddd !important;"><span style="display:inline-block;width:77px;">'. strtoupper($infomainaccesssalereturn['modulename']) .' DATE</span></th>';
	...
	echo '<th style="vertical-align:middle !important;border:1px solid #ddd !important;"><span style="display:inline-block;width:77px;">'. strtoupper($infomainaccesssalereturn['modulename']) .' AMOUNT</span></th>';
	...
	$i=0;
	$totalbalance=0;
	$sqls=mysqli_query($con,"select $nocolumn, $datecolumn, grandtotal, customerid, customername from $tabname where franchisesession='".$_SESSION['franchisesession']."' and createdid='$companymainid' and customerid='$customerid' and paidstatus!='1' GROUP BY $datecolumn, $nocolumn  order by $datecolumn asc, $nocolumn asc");
	while($infos=mysqli_fetch_array($sqls))
	{
		$am=0;
		$sqlse=mysqli_query($con,"select sum(amount) as amount from pairsalesreturnpayhistory where franchisesession='".$_SESSION['franchisesession']."' and createdid='$companymainid' and  salesreturnno='".$infos[$nocolumn]."' and salesreturndate='".$infos[$datecolumn]."' and customerid='$customerid' ");
		...
		echo '<td data-label="'. strtoupper($infomainaccesssalereturn['modulename']) .' NO" style="vertical-align:middle !important;">'.$infos[$nocolumn].'</td>';
		echo '<td data-label="'. strtoupper($infomainaccesssalereturn['modulename']) .' DATE" style="vertical-align:middle !important;">'.date('d/m/Y',strtotime($infos[$datecolumn])).'</td>';
