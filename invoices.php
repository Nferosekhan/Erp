				...
				<div class="col-12">
					<div class="card mb-4 mt-5">
						  ...
						  <div class="app-utility-item app-user-dropdown dropdown">
							 ...
							 <div class="dropdown-menu  dropdown-menu-end customdropdown  me-sm-2" aria-labelledby="dropdownMenuButton111" style="max-height: 0px;max-width: 0px;">
								...
								<div class="col-lg-3" style="margin-bottom: 57px !important;"> 
									<input class="form-control" id="invsearch" type="text" placeholder="Search.." style="height:24px;">
								</div>
							</div>
						<?php 
							$sqlismainaccessusers=$con->prepare("SELECT * FROM pairmainaccess WHERE userid=? AND moduletype='Invoices' ORDER BY id ASC");
						...
						?>
							<div align="right" class=" p-2 addandmenuonlyforimports" style="margin-bottom: -30px;">
								<div class="row" style="width:250px;">           
									<div class="col-8">  
										<a href="invoiceadd.php" class="btn btn-custom btn-sm p-2 add" style="font-size: 13px;height: 24px;margin-bottom:1rem;margin-top: -151px;margin-right:-39px;padding-right: 5px;">
									...
									<div class="col-4">
										...
										?>
											<ul class="dropdown-menu dropdown-menu-end" aria-labelledby="dropdownMenuButton">
												<li>
													<a class="dropdown-item" href="invoiceimport.php">
						...
						<?php
							...
							$sqlismainaccessusers=$con->prepare("SELECT * FROM pairmainaccess WHERE franchiseid=? AND moduletype='Invoices' ORDER BY id ASC");
...
<div style="visibility: visible;" id="arrowsalltabs">
	...
	<div class="tab-content" id="nav-tabContent" style="position:relative;top: -88px;">
		<div class="tab-pane fade show active p-3" id="nav-general" role="tabpanel" aria-labelledby="nav-general-tab">
							<div class="table-responsive p-0 min-height-480">
								<table id="someTable" class="table table-bordered align-items-center mb-0" style="table-layout: fixed;">
									...
									<?php
										...
										$sqls=$con->prepare("SELECT MAX(id) AS id,invoicedate, invoiceno, customername, invoiceterm, duedate, invoiceamount, cancelstatus, customerid FROM pairinvoices WHERE franchisesession=? AND createdid=? ".$typesforlisting." GROUP BY invoicedate, invoiceno ORDER BY invoicedate DESC, invoicetime DESC,ordering DESC LIMIT ".(($access['invpageload']=='pagenum')?'10':'15')."");
									...
									?>
										<?php
											if ((in_array('Date', $modulecolumns))) {
										?>
											<td onclick="window.open('invoiceview.php?id=<?=$info['id']?>&invoiceno=<?=$info['invoiceno']?>&invoicedate=<?=$info['invoicedate']?>', '_self')"  data-label="Date">
												<?=(($info['invoicedate']!='')?(date($date,strtotime($info['invoicedate']))):'&nbsp;')?>
											</td>
										<?php
											}
											if ((in_array('No', $modulecolumns))) {
										?>
											<td onclick="window.open('invoiceview.php?id=<?=$info['id']?>&invoiceno=<?=$info['invoiceno']?>&invoicedate=<?=$info['invoicedate']?>', '_self')"  data-label="Number">
												<?=(($info['invoiceno']=='')?'&nbsp;':'')?><?=$info['invoiceno']?>
											</td>
										<?php
											}
											// if ((in_array('Customer Name', $modulecolumns))) {
										?>
											<td onclick="window.open('invoiceview.php?id=<?=$info['id']?>&invoiceno=<?=$info['invoiceno']?>&invoicedate=<?=$info['invoicedate']?>', '_self')"  data-label="Name">
										...
										?>
											...
											<td onclick="window.open('invoiceview.php?id=<?=$info['id']?>&invoiceno=<?=$info['invoiceno']?>&invoicedate=<?=$info['invoicedate']?>', '_self')"  data-label="Amount">
												<i class="fa fa-rupee"></i> <?=number_format((float)$info['invoiceamount'],2,'.','')?>
											</td>
										<?php
											}
											if ((in_array('Status', $modulecolumns))) {
										...
										<?php
											...
											if ((in_array('Balance', $modulecolumns))) {
										?>
											<td onclick="window.open('invoiceview.php?id=<?=$info['id']?>&invoiceno=<?=$info['invoiceno']?>&invoicedate=<?=$info['invoicedate']?>', '_self')"  data-label="Balance">
										...
										?>
											<td data-label="Edit">
												<a href="invoiceedit.php?invoiceno=<?=$info['invoiceno']?>&invoicedate=<?=$info['invoicedate']?>" class="text-secondary font-weight-bold text-xs">
									...
									<?php
										...
										$sqltotinvs=$con->prepare("SELECT COUNT(DISTINCT invoiceno) FROM pairinvoices WHERE franchisesession=? AND createdid=? ".$typesforlisting."");
									$sqltotinvs->bind_param("ss", $_SESSION['franchisesession'], $companymainid);
									$sqltotinvs->execute();
									$sqltotinv = $sqltotinvs->get_result();
										$sqlfetlist=$sqltotinv->fetch_array();
										if ($sqlfetlist[0]==0) {
											$pageinitnum = 0;
										}
										if ($sqlfetlist[0]!=0) {
							...
							<?php
								if ($access['invpageload']=='pageauto') {
							?>
								<script type="text/javascript">
								...
								$('.main-content').on('scroll', function() {
									...
									if (scrollTop + $(this).innerHeight() >= this.scrollHeight-50) {
										if (isPreviousEventComplete && isDataAvailable) {
											...
											$.ajax({
												type: "GET",
												url: 'invsearch.php?term=' + sIndex + '&limitings=15&typesforlisting=<?=urlencode($typesforlisting)?>',
					 ...
					 try{
								...
								const pagechanges = (page, pageSize) => {
									...
									$.ajax({
										type: "GET",
										url: 'invsearch.php?term='+perpages+'&limitings='+($('#limitforpagenum').val())+'&typesforlisting=<?=urlencode($typesforlisting)?>',
										success: function (result) {
		...
		<div class="modal fade" id="deleteconfirm-adddelete" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
			<div class="modal-dialog">
				<div class="modal-content">
					...
					<div class="modal-body">Are you sure you want to Cancel this Invoice?</div>
...
function submitthesorting(value) {
  $.ajax({
	 type: "GET",
	 url: 'listingsortfilters.php?moduletype=Invoices&typesforlisting='+value+'',
	 success: function (result) {
		console.log(result);
		window.location = 'invoices.php?remarks='+result+'';
	 },
	 error: function (error) {
		console.log(error);
	 }
