<?php
include('lcheck.php');
$sqlismainaccessfield=mysqli_query($con, "select * from pairmainaccess where userid='$companymainid' and moduletype='Delivery Challans' order by id  asc");
...
if (($infomainaccessuser['deliverychallancustinfo']=='defone')||($infomainaccessuser['deliverychallancustinfo']=='deftwo')) {
...
$sqlismainaccessuser=mysqli_query($con, "select * from pairmainaccess where userid='$userid' and moduletype='Delivery Challans' order by id  asc");
$infomainaccessuser=mysqli_fetch_array($sqlismainaccessuser);
if ($infomainaccessuser['deliverychallancustinfo']=='one'||($scriptonetwo=='one')) {
...
$deliverychallanterm=mysqli_real_escape_string($con, $_POST['deliverychallanterm']);
$sqlismainaccessuser=mysqli_query($con, "select moduleno, moduleprefix, modulesuffix from pairmainaccess where franchiseid='".$_SESSION['franchisesession']."' and moduletype='Delivery Challans' order by id  asc");
$infomainaccessuser=mysqli_fetch_array($sqlismainaccessuser);
$deliverychallanno=mysqli_real_escape_string($con, $infomainaccessuser['moduleprefix'].(str_pad($infomainaccessuser['modulesuffix']+1, 0, "0", STR_PAD_LEFT)));
...
$deliverychallanamount=mysqli_real_escape_string($con, $_POST['grandtotal']);
...
$custsqluse=mysqli_query($con, "insert into pairusehistory set usetype='CUSTOMERS', createdon='$times',  createdby='".$_SESSION["unqwerty"]."', useid='$custid', useremarks='".$custch."' ");
               ...
               if ($infomainaccessuser['deliverychallancustinfo']=='two'||($scriptonetwo=='two')) {
$address1 = '';
$address2 = '';
$saddress1 = '';
$saddress2 = '';
...
$custsqlins = mysqli_query($con, "select count(customercode) from paircustomers where moduletype='Customers'");
$custansins = mysqli_fetch_array($custsqlins);
$custoldid = $custansins[0];
$custcustomercode = $custoldid + 1;
$custpublicsql=mysqli_query($con,"select count(publicid) from paircustomers where createdid='$companymainid' and moduletype='Customers'");
$custpublicans=mysqli_fetch_array($custpublicsql);
$custoldcodepublic=$custpublicans[0];
$custpubliccode=$infomodulespublicnamecust['publiccolumn'] . $custoldcodepublic+1;
$custprivatesql=mysqli_query($con,"select count(privateid) from paircustomers where createdid='$companymainid' and moduletype='Customers' and franchisesession='".$_SESSION['franchisesession']."'");
$custprivateans=mysqli_fetch_array($custprivatesql);
$custoldcodeprivate=$custprivateans[0];
$custprivatecode=$infomainaccesspublicnamecust['moduleprefix'] . $custoldcodeprivate+1;
$sqlup = "insert into paircustomers set createdon='$times', createdid='$companymainid', createdby='".$_SESSION["unqwerty"]."', franchisesession='".$_SESSION["franchisesession"]."', customername='$customername',billstreet='$area',billcity='$city',billstate='$state',billpincode='$pincode',billcountry='$district',shipstreet='$sarea',shipcity='$scity',shipstate='$sstate',shippincode='$spincode',shipcountry='$sdistrict',sameasbilling='$twosameasbilling',gstrtype='$gstrtype',gstin='$gstno',moduletype='Customers',placeos='$pos',workphone='$twoworkphone',mobile='$twomobilephone',customercode='$custcustomercode',publicid='$custpubliccode',privateid='$custprivatecode'";
...
$sql2=mysqli_query($con, "select id from pairdeliverychallans where franchisesession='".$_SESSION['franchisesession']."' and createdid='$companymainid' and deliverychallanno='$deliverychallanno' and deliverychallandate='$deliverychallandate'");
...
$tid=mysqli_insert_id($con);
mysqli_query($con, "INSERT INTO pairhistory (user, ipaddress, times, remarks, tid) VALUES ('{$username}', '{$ip}', '{$times}', 'Insert A Delivery Challan', '{$tid}')");

$sql3=mysqli_query($con, "update pairmainaccess set modulesuffix=modulesuffix+1 where franchiseid='".$_SESSION['franchisesession']."' and moduletype='Delivery Challans'");
if($sql3)
{
 if(isset($_POST['submit1']))
{
echo '<script> window.open("deliverychallanprint.php?deliverychallanno='.$deliverychallanno.'&deliverychallandate='.$deliverychallandate.'", "_blank");</script>';
echo '<script> window.location.href="deliverychallans.php?remarks=Added Successfully";</script>'; 
}
else
{
echo '<script> window.location.href="deliverychallanview.php?deliverychallanno='.$deliverychallanno.'&deliverychallandate='.$deliverychallandate.'&remarks=Added Successfully";</script>';
}
}
}
else
{
header("Location: deliverychallans.php?error=Error Data");
}
}
...
$sqlismainaccessuser=mysqli_query($con, "select moduleno, moduleprefix, modulesuffix,modulename from pairmainaccess where franchiseid='".$_SESSION['franchisesession']."' and moduletype='Delivery Challans' order by id  asc");
$infomainaccessuser=mysqli_fetch_array($sqlismainaccessuser);
?>
...
<!-------------------customer add info end ------------------------------->
<form id="deliverychallanform" action="" onsubmit="return checkvalidate()" method="post" enctype="multipart/form-data" class="form-horizontal" role="form">
<div class="row">
<!-- <div class="col-lg-6">
...
<?php
     if ((in_array('Delivery Challan Information', $fieldadd))) {
        ?>
<div class="col-lg-4">
<div class="accordion" id="accordionRental">
<div class="accordion-item mb-1">
...
<div class="col-lg-5">
<label for="deliverychallanno" class="custom-label"><span class="text-danger"><?= $infomainaccessuser['modulename'] ?> Number *</span></label>
</div>
<div class="col-lg-7">
<input type="text" class="form-control  form-control-sm" id="deliverychallanno" name="deliverychallanno" required value="<?=$infomainaccessuser['moduleprefix']?><?=str_pad(($infomainaccessuser['modulesuffix']+1), 0, "0", STR_PAD_LEFT)?>" readonly>
</div>
</div>
</div>
<div class="col-lg-12">
...
<label for="deliverychallandate" class="custom-label"><span class="text-danger"><?= $infomainaccessuser['modulename'] ?> Date *</span></label>
</div>
<div class="col-lg-7">
<?php
...
$dt = new DateTime();
?>
<input type="date" class="form-control  form-control-sm" id="deliverychallandate" name="deliverychallandate" value="<?= $dt->format('Y-m-d') ?>" required>
</div>
</div>
</div>
...
<div class="form-group row" style="align-items: center !important;">
<div class="col-lg-5" style="padding-right:0px">
<label for="deliverychallandate" class="custom-label"><span class="text-danger"><?= $infomainaccessuser['modulename'] ?> Term *</span></label>
</div>
<div class="col-lg-7">
<select class="select2-field form-control  form-control-sm" name="deliverychallanterm" id="deliverychallanterm" required>
...
if(duedates!='')
{
console.log(deliverychallandate);
console.log(duedates);
today=addDays(deliverychallandate, duedates);
...
<input type="radio" class="custom-control-input" name="customerinfodefault" id="onecustomerinfo" value="one" <?= ($infomainaccessuser['deliverychallancustinfo']=='defone')?'checked':'' ?> onclick="onetwo()">
<label class="custom-control-label custom-label" for="onecustomerinfo">B2B</label>
</div>
</div>
<div class="col-lg-2 col-md-2 col-3">
<div class="custom-control custom-radio mr-sm-2">
<input type="radio" class="custom-control-input" name="customerinfodefault" id="twocustomerinfo" value="two" <?= ($infomainaccessuser['deliverychallancustinfo']=='deftwo')?'checked':'' ?> onclick="onetwo()">
...
<?php
if ($access['delchbtwocnamerequired']=='Yes') {
?>
$("#twocustomername").removeAttr("required");
<?php
}
if ($access['delchbtwocwphonerequired']=='Yes') {
?>
...
$sqlismainaccessuser=mysqli_query($con, "select * from pairmainaccess where userid='$userid' and moduletype='Delivery Challans' order by id  asc");
$infomainaccessuser=mysqli_fetch_array($sqlismainaccessuser);
if ($infomainaccessuser['deliverychallancustinfo']=='one'||(($infomainaccessuser['deliverychallancustinfo']=='defone')||($infomainaccessuser['deliverychallancustinfo']=='deftwo'))) {
?>
<div id="one">
<div class="row">
<div class="col-lg-12">
...
if ($infomainaccessuser['deliverychallancustinfo']=='two'||(($infomainaccessuser['deliverychallancustinfo']=='defone')||($infomainaccessuser['deliverychallancustinfo']=='deftwo'))) {
?>
<script type="text/javascript">
function twoposans() {
...
<?= ($access['delchbtwocnamerequired']=='Yes')? '<span class="text-danger">'. $infomainaccessusercus['modulename'] . ' Name * </span>': $infomainaccessusercus['modulename'] . ' Name ' ?>
</label>
</div>
<div class="col-lg-9">
<input type="text" name="twocustomername" id="twocustomername" class="form-control form-control-sm" placeholder="<?= $infomainaccessusercus['modulename'] ?> Name" <?= ($access['delchbtwocnamerequired']=='Yes')? 'required': ' ' ?>>
</div>
</div>
<div class="row mt-1">
<div class="col-lg-12">
...
<?= ($access['delchbtwocwphonerequired']=='Yes')? '<span class="text-danger"> WORK PHONE * </span>': ' WORK PHONE ' ?>
</label>
<input type="text" class="form-control form-control-sm" id="twoworkphone" name="twoworkphone" placeholder="Work Phone" <?= ($access['delchbtwocwphonerequired']=='Yes')? 'required': ' ' ?>>
</div>
...
<input type="text" class="form-control form-control-sm" id="twogsttreatment" name="twogsttreatment" placeholder="GST TREATMENT" value="<?= $infomainaccessuser['deliverychallantwogst'] ?>" readonly>
</div>
...
<option disabled value="" <?=($infomainaccessuser['deliverychallantwopos']=='manual'||'auto')?'selected':''?>>Select The Place</option>
<option value="JAMMU AND KASHMIR (1)" <?=($infomainaccessuser['deliverychallantwopos']=="JAMMU AND KASHMIR (1)")?'selected':''?>>JAMMU AND KASHMIR (1)</option>
<option value="ANDAMAN AND NICOBAR ISLANDS (35)" <?=($infomainaccessuser['deliverychallantwopos']=="ANDAMAN AND NICOBAR ISLANDS (35)")?'selected':''?>>ANDAMAN AND NICOBAR ISLANDS (35)</option>
<option value="ANDHRA PRADESH (NEWLY ADDED) (37)" <?=($infomainaccessuser['deliverychallantwopos']=="ANDHRA PRADESH (NEWLY ADDED) (37)")?'selected':''?>>ANDHRA PRADESH (NEWLY ADDED) (37)</option>
<option value="ANDHRA PRADESH(BEFORE DIVISION) (28)" <?=($infomainaccessuser['deliverychallantwopos']=="ANDHRA PRADESH(BEFORE DIVISION) (28)")?'selected':''?>>ANDHRA PRADESH(BEFORE DIVISION) (28)</option>
<option value="ARUNACHAL PRADESH (12)" <?=($infomainaccessuser['deliverychallantwopos']=="ARUNACHAL PRADESH (12)")?'selected':''?>>ARUNACHAL PRADESH (12)</option>
<option value="ASSAM (18)" <?=($infomainaccessuser['deliverychallantwopos']=="ASSAM (18)")?'selected':''?>>ASSAM (18)</option>
<option value="BIHAR (10)" <?=($infomainaccessuser['deliverychallantwopos']=="BIHAR (10)")?'selected':''?>>BIHAR (10)</option>
<option value="CENTRE JURISDICTION (99)" <?=($infomainaccessuser['deliverychallantwopos']=="CENTRE JURISDICTION (99)")?'selected':''?>>CENTRE JURISDICTION (99)</option>
<option value="CHANDIGARH (4)" <?=($infomainaccessuser['deliverychallantwopos']=="CHANDIGARH (4)")?'selected':''?>>CHANDIGARH (4)</option>
<option value="CHATTISGARH (22)" <?=($infomainaccessuser['deliverychallantwopos']=="CHATTISGARH (22)")?'selected':''?>>CHATTISGARH (22)</option>
<option value="DADRA AND NAGAR HAVELI AND DAMAN AND DIU (NEWLY MERGED UT) (26*)" <?=($infomainaccessuser['deliverychallantwopos']=="DADRA AND NAGAR HAVELI AND DAMAN AND DIU (NEWLY MERGED UT) (26*)")?'selected':''?>>DADRA AND NAGAR HAVELI AND DAMAN AND DIU (NEWLY MERGED UT) (26*)</option>
<option value="DELHI (7)" <?=($infomainaccessuser['deliverychallantwopos']=="DELHI (7)")?'selected':''?>>DELHI (7)</option>
<option value="GOA (30)" <?=($infomainaccessuser['deliverychallantwopos']=="GOA (30)")?'selected':''?>>GOA (30)</option>
<option value="GUJARAT (24)" <?=($infomainaccessuser['deliverychallantwopos']=="GUJARAT (24)")?'selected':''?>>GUJARAT (24)</option>
<option value="HARYANA (6)" <?=($infomainaccessuser['deliverychallantwopos']=="HARYANA (6)")?'selected':''?>>HARYANA (6)</option>
<option value="HIMACHAL PRADESH (2)" <?=($infomainaccessuser['deliverychallantwopos']=="HIMACHAL PRADESH (2)")?'selected':''?>>HIMACHAL PRADESH (2)</option>
<option value="JHARKHAND (20)" <?=($infomainaccessuser['deliverychallantwopos']=="JHARKHAND (20)")?'selected':''?>>JHARKHAND (20)</option>
<option value="KARNATAKA (29)" <?=($infomainaccessuser['deliverychallantwopos']=="KARNATAKA (29)")?'selected':''?>>KARNATAKA (29)</option>
<option value="KERALA (32)" <?=($infomainaccessuser['deliverychallantwopos']=="KERALA (32)")?'selected':''?>>KERALA (32)</option>
<option value="LADAKH (NEWLY ADDED) (38)" <?=($infomainaccessuser['deliverychallantwopos']=="LADAKH (NEWLY ADDED) (38)")?'selected':''?>>LADAKH (NEWLY ADDED) (38)</option>
<option value="LAKSHADWEEP (31)" <?=($infomainaccessuser['deliverychallantwopos']=="LAKSHADWEEP (31)")?'selected':''?>>LAKSHADWEEP (31)</option>
<option value="MADHYA PRADESH (23)" <?=($infomainaccessuser['deliverychallantwopos']=="MADHYA PRADESH (23)")?'selected':''?>>MADHYA PRADESH (23)</option>
<option value="MAHARASHTRA (27)" <?=($infomainaccessuser['deliverychallantwopos']=="MAHARASHTRA (27)")?'selected':''?>>MAHARASHTRA (27)</option>
<option value="MANIPUR (14)" <?=($infomainaccessuser['deliverychallantwopos']=="MANIPUR (14)")?'selected':''?>>MANIPUR (14)</option>
<option value="MEGHALAYA (17)" <?=($infomainaccessuser['deliverychallantwopos']=="MEGHALAYA (17)")?'selected':''?>>MEGHALAYA (17)</option>
<option value="MIZORAM (15)" <?=($infomainaccessuser['deliverychallantwopos']=="MIZORAM (15)")?'selected':''?>>MIZORAM (15)</option>
<option value="NAGALAND (13)" <?=($infomainaccessuser['deliverychallantwopos']=="NAGALAND (13)")?'selected':''?>>NAGALAND (13)</option>
<option value="ODISHA (21)" <?=($infomainaccessuser['deliverychallantwopos']=="ODISHA (21)")?'selected':''?>>ODISHA (21)</option>
<option value="OTHER TERRITORY (97)" <?=($infomainaccessuser['deliverychallantwopos']=="OTHER TERRITORY (97)")?'selected':''?>>OTHER TERRITORY (97)</option>
<option value="PUDUCHERRY (34)" <?=($infomainaccessuser['deliverychallantwopos']=="PUDUCHERRY (34)")?'selected':''?>>PUDUCHERRY (34)</option>
<option value="PUNJAB (3)" <?=($infomainaccessuser['deliverychallantwopos']=="PUNJAB (3)")?'selected':''?>>PUNJAB (3)</option>
<option value="RAJASTHAN (8)" <?=($infomainaccessuser['deliverychallantwopos']=="RAJASTHAN (8)")?'selected':''?>>RAJASTHAN (8)</option>
<option value="SIKKIM (11)" <?=($infomainaccessuser['deliverychallantwopos']=="SIKKIM (11)")?'selected':''?>>SIKKIM (11)</option>
<option value="TAMIL NADU (33)"  <?=($infomainaccessuser['deliverychallantwopos']=="TAMIL NADU (33)")?'selected':''?>>TAMIL NADU (33)</option>
<option value="TELANGANA (36)" <?=($infomainaccessuser['deliverychallantwopos']=="TELANGANA (36)")?'selected':''?>>TELANGANA (36)</option>
<option value="TRIPURA (16)" <?=($infomainaccessuser['deliverychallantwopos']=="TRIPURA (16)")?'selected':''?>>TRIPURA (16)</option>
<option value="UTTAR PRADESH (9)" <?=($infomainaccessuser['deliverychallantwopos']=="UTTAR PRADESH (9)")?'selected':''?>>UTTAR PRADESH (9)</option>
<option value="UTTARAKHAND (5)" <?=($infomainaccessuser['deliverychallantwopos']=="UTTARAKHAND (5)")?'selected':''?>>UTTARAKHAND (5)</option>
<option value="WEST BENGAL (19)" <?=($infomainaccessuser['deliverychallantwopos']=="WEST BENGAL (19)")?'selected':''?>>WEST BENGAL (19)</option>
</select>
</div>
</div>
</div>
...
<div class="table-responsive mt-1" id="gsttablediv" <?=(in_array('Tax Table', $fieldadd))?'':'style="display:none;"'?>>
...
href="deliverychallans.php" style="margin-top:-3px !important;">Cancel</a>
...
$("#customer").on("select2:open", function() {
<?php
if($access['delchnewcustomerdef']=='1'){
?>
$("#configureunits").show();
...
$("#product1").on("select2:open", function() {
<?php
if($access['delchnewproductdef']=='1'){
?>
$("#configureunits").show();
...
<script type="text/javascript">
$(function() {
$("#deliverychallansuffix").autocomplete({
source: 'deliverychallansuffixsearch.php',
select: function(event, ui) {
$("#deliverychallansuffix").val(ui.item.deliverychallansuffix);
...
<!------------------------------------------- pro script end ------------------------------------------->
<!--term start-->
<div class="modal fade" id="AddNewDeliveryChallanTerm" tabindex="-1" role="dialog" style="z-index: 1051;">
<div class="modal-dialog" role="document">
<div class="modal-content">
<div class="modal-header">
<h5 class="modal-title">New <?= $infomainaccessuser['modulename'] ?> Term</h5>
<span type="button" onclick="funesdeliverychallanterm()" class="close" data-dismiss="modal"
aria-label="Close">
...
<div class="form-group row">
<div class="col-sm-6">
<label for="missingdeliverychallanterm" class="custom-label"><span class="text-danger">
<?= $infomainaccessuser['modulename'] ?> Term *</span></label>
</div>
<div class="col-sm-6">
<input type="text" name="deliverychallanterm" class="form-control form-control-sm mb-4" id="missingdeliverychallanterm" placeholder="Name" required>
</div>
</div>
</div>
</div>
...
<div class="col">
<button   onclick="funadddeliverychallanterm()" class="btn btn-primary btn-sm btn-custom arlina-button expand-left"   type="submit"  name="submitdeliverychallanterm" value="Submit">
<span class="label">Save</span> <span class="spinner"></span>
</button>
<button type="button"
class="btn btn-primary btn-sm btn-custom-grey"
onclick="funesdeliverychallanterm()">Cancel</button> </div>
</div>
</div>
</div>
</div>
<script>
$("#deliverychallanterm").on("change", function() {
var sOptionVal = $(this).val();
if (sOptionVal == '#AddNewDeliveryChallanTerm') {
$('#AddNewDeliveryChallanTerm').modal('show');
}
});
function funadddeliverychallanterm() {
var missingdeliverychallanterm = document.getElementById('missingdeliverychallanterm');
if (missingdeliverychallanterm.value == '') {
alert('Please Enter New Delivery ChallanTerm Name');
missingdeliverychallanterm.focus();
return false;
} else {
...
url: "termadds.php",
data: {
term: $("#missingdeliverychallanterm").val(),
submit: "Submit"
},
success:function(result){
const resarray = result.split("|");
...
{
$('#deliverychallanterm').append('<option value="' + missingdeliverychallanterm.value + '">' + missingdeliverychallanterm.value +
'</option>');
$('#deliverychallanterm').val(missingdeliverychallanterm.value).change();
$("#deliverychallanterm").select2();
$('#AddNewDeliveryChallanTerm').modal('hide');
return false;
}
}});
...
function funesdeliverychallanterm() {
//$('#deliverychallanterm').val('').change();
$("#deliverychallanterm").select2();
$('#AddNewDeliveryChallanTerm').modal('hide');
return false;
}
...
$("#deliverychallanterm").on("select2:open", function() {
$("#configureunits").attr("data-bs-target","#AddNewDeliveryChallanTerm");
});
$("#deliverychallanterm").on("select2:open", function() {
document.getElementById("configureunits").innerHTML = "New Delivery ChallanTerm";
});
</script>
...
<!---payment confirm---->
<script>
function triggerpayment(deliverychallanno,deliverychallandate,deliverychallanamount,cancelstatus)
{
let allAreFilled = true;
document.getElementById("deliverychallanform").querySelectorAll("[required]").forEach(function(i) {
if (!allAreFilled) return;
if (i.type === "radio") {
let radioValueCheck = false;
document.getElementById("deliverychallanform").querySelectorAll(`[name=${i.name}]`).forEach(function(r) {
...
{
$('#validdeliverychallanno').val($('#deliverychallanno').val());
$('#validdeliverychallandate').val($('#deliverychallandate').val());
$('#validdeliverychallanamount').val($('#grandtotal').val());
