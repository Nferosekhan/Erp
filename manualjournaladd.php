...
$publicsql=mysqli_query($con,"select count(publicid) from pairledgers where createdid='$companymainid' and publicid!='' and type='ledger'");
...
$fileattach=mysqli_real_escape_string($con, $_POST['fileattachs']);
...
if($sql)
{
$sql3=mysqli_query($con, "update pairmainaccess set modulesuffix=modulesuffix+1 where franchiseid='".$_SESSION['franchisesession']."' and moduletype='Manual Journals'");
header("Location: manualjournals.php?remarks=Journal Entried Successfully");
}
}
else
{
...
<div class="accordion" id="accordionRental">
	...
	<div class="form-group row">
            ...
            <div class="col-sm-8">
              <input type="date" class="form-control  form-control-sm" id="ledgerdate" name="ledgerdate"  required value="<?=date('Y-m-d')?>">
		  ...
		  <div class="form-group row">
            ...
            <div class="col-sm-8">
              <input type="text" class="form-control  form-control-sm" id="ledgerno" name="ledgerno" required value="<?=$infomainaccessuser['moduleprefix']?><?=str_pad(($infomainaccessuser['modulesuffix']+1), 0, "0", STR_PAD_LEFT)?>" readonly>	  
            </div>
			</div>
		  ...
		  <div class="form-group row">
            ...
            <div class="col-sm-8">
              <input type="number" min="0" step="0.01" class="form-control  form-control-sm" id="referenceno" name="referenceno" maxlength="10" placeholder="Reference Number">			  
            </div>
			</div>
            ...
            <div class="col-sm-8">
              <textarea class="form-control  form-control-sm" id="notes" name="notes" placeholder="Notes" required style="height: 45px;"></textarea>
...
<input type="checkbox" class="custom-control-input" name="cashbasedjournal" id="cashbasedjournal">
...
?>
<option value="<?= $infos['currencyname'] ?>"><?= $infos['currencyname'] ?></option>
<?php
}
?>
</select>
...
<td data-label="ACCOUNT"><div class="col-sm-9 col-lg-12 selectmobview" onclick="andus()" style="width:250px;display: inline-block;margin-top: 1.5px !important;"><select style=" width: 100%;" class="select4 form-control form-control-sm" name="chartaccountname[]" id="chartaccountname1" required><option selected disabled value="">Select</option></select></div></td>
<td data-label="DESCRIPTION"><div><input type="text" name="description[]" id="description1" class="form-control form-control-sm bordernoneinput inpbordes productselectwidth" style="display: inline-block;"></div></td>
<td data-label="CONTACT"><div class="col-sm-9 col-lg-12 selectmobview" onclick="andus()" style="width:250px;display: inline-block;margin-top: 1.5px !important;"><select class="form-control  form-control-sm customerinlist" name="customername[]" id="customername1" required><option value='' selected disabled>Select</option></select></div></td>
<td data-label="DEBITS"><div><input type="number" min="0" step="0.01" name="ledgerdebit[]" id="ledgerdebit1" class="form-control form-control-sm bordernoneinput inpbordes productselectwidth" onChange="chartaccountcalc(this.id)" required value="0" style="display: inline-block;"></div></td>
<td data-label="CREDITS"><div><input type="number" min="0" step="0.01" name="ledgercredit[]" required id="ledgercredit1" class="form-control form-control-sm bordernoneinput inpbordes productselectwidth" onChange="chartaccountcalc(this.id)" value="0" style="display: inline-block;"></div></td>
...
$.ajax({
type: "GET",
url: 'inlistaccsearch.php?idname=chartaccountname&lineNo=1',
success: function (result) {
console.log(result);
if (result!='') {
$("#chartaccountname1").html(result);
$("#chartaccountname1").append('<option selected disabled value="">Select</option>');
}
else{
$("#chartaccountname1").val("");
$("#chartaccountname1").append('<option selected disabled value="">Select</option>');
}
},
