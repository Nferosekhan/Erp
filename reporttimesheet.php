...
if (isset($_POST['submitcustomizes'])) {
	...
	$sqlreportmoduless=$con->prepare("SELECT * FROM pairreportmodules WHERE types='timesheet' ORDER BY id ASC");
	...
	foreach ($newmodules as $newmoduleskey) {
		...
		$modcolumncol=$coltypemod."timesheet";
	...
	$sqlupreport = $con->prepare("UPDATE pairreports SET rowcolumns=? WHERE franchiseid=? AND createdid=? AND types='timesheet'");
	...
	$sqlreportfav = $con->prepare("UPDATE pairreportfavourites SET reporturl=? WHERE franchisesession=? AND createdid=? AND reportnames='timesheet'");
	$sqlreportfav->bind_param("sss", $newReportUrl, $_SESSION['franchisesession'], $companymainid);
	$sqlreportfav->execute();
	$sqlreportfav->close();

	$sqlreport = $con->prepare("UPDATE pairreports SET reportperiod=?,filtername=?,companyname=?,dateprepare=?,timeprepare=?,gstrule=? WHERE franchiseid=? AND createdid=? AND types='timesheet'");
	...
	header("Location:reporttimesheet.php?datefrom=".date($dateformatforurlredirection,strtotime($from))."&dateto=".date($dateformatforurlredirection,strtotime($to))."");
}
								...
								<form action="" method="post" enctype="multipart/form-data" class="form-horizontal mt-0" role="form">
									<div class="row">
																			...
																			<div id="customizerowcolumns" class="accordion-collapse collapse show" aria-labelledby="customizerowcolumn">
																				<div class="accordion-body text-sm">
																					<div class="row justify-content-center">
																						...
																						<?php
																							$newans=array();
																							$sqlreportaccesss=$con->prepare("SELECT * FROM pairreports WHERE franchiseid=? AND createdid=? AND types='timesheet'");
																								...
																								<div class="col-lg-6 my-1" style="padding-left: 0px !important;">
																									<div class="custom-control custom-checkbox mr-sm-2" style="padding: 0px 12px !important;">
																										<div class="input-group input-group-sm" id="flaghead">
																											...
																											<input type="checkbox" class="custom-control-input" name="<?= $coltypemod; ?>timesheet" id="<?= $coltypemod; ?>timesheet" checked disabled>
																											<label class="custom-control-label custom-label" for="<?= $coltypemod; ?>timesheet">
											...
											<div class="tab-pane fade" id="nav-profile" role="tabpanel" aria-labelledby="nav-profile-tab">
												<div class="table-responsive m-3" id="histables">
													<table class="table table-bordered">
														...
														<tbody>
														<?php
															$sqluse=$con->prepare("SELECT * FROM pairusehistory WHERE usetype = 'REPtimesheet' AND useid = ? ORDER BY createdon DESC");
																				...
																				<table id="print-are1" style="border: 1px solid #eee;" width="100%">
																					<thead>
																						<tr>
																							...
																							<td class="text-uppercase" style="width:6%;border:1px solid #eee;padding-left: 10px;text-align: left;">
																								<span style="font-size:13px;color:black;">
																									Task
																								</span>
																							</td>
																							<td class="text-uppercase" style="text-align: left !important;width: 10%;border:1px solid #eee;padding-left: 10px;">
																					...
																					<tbody id="myTable">
																					<?php
																						$seldatas=$con->prepare("SELECT * FROM pairtimesheet WHERE franchisesession=? AND createdid=? AND (timesheetdate>=? AND timesheetdate<=?) ORDER BY timesheetdate ASC LIMIT 36");
										      									...
										      									$seldata = $seldatas->get_result();
																					...
																					<?php
																						...
																						$sqltotlistpages=$con->prepare("SELECT * FROM pairtimesheet WHERE franchisesession=? AND createdid=? AND (timesheetdate>=? AND timesheetdate<=?) ORDER BY timesheetdate ASC");
																					...
																					?>
																						<div style="text-align: center;position: relative;top: 35px;margin-bottom: -25px;">
																							There are no login during the selected date range
																						</div>
																					<?php
																						}
																					?>
										...
										<table id="print-are3" style="border: 1px solid #eee;display: none;" width="100%">
											<thead>
												<tr>
													<td class="text-uppercase" style="width:10%;border:1px solid #eee;padding-left: 10px;"><span style="font-size:13px;color:black;">Date</span>
													</td>
													<td class="text-uppercase" style="width:10%;border:1px solid #eee;padding-left: 10px;"><span style="font-size:13px;color:black;">Project</span>
													</td>
													<td class="text-uppercase" style="width:13%;border:1px solid #eee;padding-left: 10px;"><span style="font-size:13px;color:black;">Customer</span>
													</td>
													<td class="text-uppercase" style="width:6%;border:1px solid #eee;padding-left: 10px;text-align: left;"><span style="font-size:13px;color:black;">Task</span>
													</td>
													<td class="text-uppercase" style="text-align: left !important;width: 10%;border:1px solid #eee;padding-left: 10px;"><span style="font-size:13px;color:black;white-space: nowrap;overflow: hidden;">User</span>
													</td>
													<td class="text-uppercase" style="text-align: left !important;width: 10%;border:1px solid #eee;padding-left: 10px;"><span style="font-size:13px;color:black;white-space: nowrap;overflow: hidden;">Duration</span>
													</td>
													<td class="text-uppercase" style="text-align: left !important;width: 10%;border:1px solid #eee;padding-left: 10px;"><span style="font-size:13px;color:black;white-space: nowrap;overflow: hidden;">Billing Status</span>
													</td>
												</tr>
											</thead>
											<tbody id="myTables">
...
const pagechanges = (page, pageSize) => {
	...
	ajaxRequestview = $.ajax({
		type: "GET",
		url: 'reportviewsearch.php?term='+perpages+'&limitings='+($('#limitforpagenum').val())+'&datefrom=<?=$datefrom?>&dateto=<?=$dateto?>&dif=timesheet&lastornone='+lastornone+'',
...
$(function() {
	...
	function(start, end, label) {
		...
		window.open("reporttimesheet.php?datefrom="+datefrom+"&dateto="+dateto+"","_self");
	});
});
...
function converHTMLFileToPDF() {
	...
	var finaltimer = setTimeout(function(){
		...
		var header = function(data) {
			...
			var headtext1 = "Timesheet Details";
			var fontSize = doc.internal.getFontSize();
			var pageWidth = doc.internal.pageSize.width;
		...
		doc.autoTable({
			...
			columnStyles: {
				...
				2: { halign: 'left', cellWidth: 113},
				3: { halign: 'left', cellWidth: 84},
				4: { halign: 'left', cellWidth: 110},
				4: { halign: 'left', cellWidth: 33},
				4: { halign: 'left', cellWidth: 33}
			}
		});
		...
		$.ajax({
			type: "GET",
			url: 'reporthistory.php?types=timesheet&titles=Timesheet Details&printdownpdfcsv=Printed PDF&fromto='+fromtopdf+'',
			success: function (result) {
				console.log(result);
			},
			error: function (error) {
...
function downloadaspdf() {
	...
	var finaltimer = setTimeout(function(){
		...
		$.ajax({
			type: "GET",
			url: 'reporthistory.php?types=timesheet&titles=Timesheet Details&printdownpdfcsv=Downloaded PDF&fromto='+fromtopdf+'',
			success: function (result) {
				console.log(result);
			},
			error: function (error) {
		...
		doc.save('Timesheet-Details-<?=$datefrom?>-To-<?=$dateto?>');
...
function downloadascsv() {
	...
	var finaltimer = setTimeout(function(){
		...
		link.setAttribute("download", "Timesheet-Details-<?=$datefrom?>-To-<?=$dateto?>");
		document.body.appendChild(link);
		link.click();
		$.ajax({
			type: "GET",
			url: 'reporthistory.php?types=timesheet&titles=Timesheet Details&printdownpdfcsv=Downloaded CSV&fromto='+fromtopdf+'',
			success: function (result) {
				console.log(result);
			},
			error: function (error) {
...
document.addEventListener('DOMContentLoaded', function() {
	...
	function makeAjaxRequest() {
		ajaxRequest = $.ajax({
			type: "GET",
			url: 'check.php?term=timesheet&datefrom=<?=$datefrom?>&dateto=<?=$dateto?>&limitfrom=0&limitto=10000',
			success: function (result) {
				if (result != mytabprev) {
