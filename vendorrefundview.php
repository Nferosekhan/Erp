...
include('lcheck.php');
     if(isset($_GET['id']))
{
$sql=mysqli_query($con,"select id, createdon, term, receiptdate, receiptno, cancelstatus, vendorid, vendorname, paymentmode, amount, notes, type, publicid, privateid from pairdebitnotepayments where franchisesession='".$_SESSION['franchisesession']."' and createdid='$companymainid' and id='".$_GET['id']."'");
if(mysqli_num_rows($sql)>0)
{
$sqlismainaccessfield=mysqli_query($con, "select * from pairmainaccess where userid='$companymainid' and moduletype='Vendor Refunds' order by id  asc");
...
$sqlismainaccessuser=mysqli_query($con, "select * from pairmainaccess where userid='$userid' and moduletype='Vendor Refunds' order by id  asc");
...
$sql=mysqli_query($con,"select id, createdon, term, receiptdate, receiptno, cancelstatus, vendorid, vendorname, paymentmode, amount, notes, publicid, privateid from pairdebitnotepayments where franchisesession='".$_SESSION['franchisesession']."' and createdid='$companymainid' and id='".$_GET['id']."'");
...
<div class="col-lg-6">
 ...
 <?php
$sqlismainaccessuser=mysqli_query($con, "select moduleno, moduleprefix, modulesuffix, modulename from pairmainaccess where franchiseid='".$_SESSION['franchisesession']."' and moduletype='Vendor Refunds' order by id  asc");
$infomainaccessuser=mysqli_fetch_array($sqlismainaccessuser);
...
?>
<a class="btn btn-primary btn-sm btn-custom-grey" href="vendorrefundedit.php?id=<?=$id?>" id="btngopage"><i class="fa fa-pencil-alt"></i> <span class="mobreswords "> Edit</span></a>
  <?php 
}
?>
  </span>
...
$totpros++;
        ...
        else
        {
            $sta='2';
                        ...
                        $sqlijournals=$con->prepare("SELECT customerid, chartaccountname, customername, sum(ledgerdebit) AS ledgerdebit, sum(ledgercredit) AS ledgercredit, notes,ledgerdate, ledgerno, referenceno, referencedate, totalledgerdebit, totalledgercredit, id, type FROM pairledgers WHERE franchisesession=? AND createdid=? AND ledgerno=? AND ledgerdate=? AND type='bill payment' GROUP BY type,chartaccountid,ledgerdate,ledgerno ORDER BY ledgerdate,ledgerno,type ASC");
                          $sqlijournals->bind_param("ssss", $_SESSION['franchisesession'], $companymainid, $infos['debitnoteno'], $infos['debitnotedate']);
    ...
    <div class="tab-pane fade" id="nav-profile" role="tabpanel" aria-labelledby="nav-profile-tab">
      ...
      <tbody>
        <?php
      $sqluse=mysqli_query($con, "select * from pairusehistory where usetype='VENREFUND' and useid='$id' order by createdon desc");
...
<th style="border:1px solid #ddd !important;vertical-align:middle !important;padding: 0px 6px !important;height: 13px !important;"><span style="display:inline-block;width:100%;color:black !important;font-size:12px !important;"><?=strtoupper($infomainaccessdebitnote['modulename'])?> DATE</span></th>
<th style="border:1px solid #ddd !important;vertical-align:middle !important;padding: 0px 6px !important;height: 13px !important;"><span style="display:inline-block;width:100%;color:black !important;font-size:12px !important;"><?=strtoupper($infomainaccessdebitnote['modulename'])?> NUMBER</span></th>
...
<th style="border:1px solid #ddd !important;vertical-align:middle !important;padding: 0px 6px !important;height: 13px !important;"><span style="display:inline-block;width:100%;color:black !important;font-size:12px !important;text-align:right;"><?=strtoupper($infomainaccessdebitnote['modulename'])?> AMOUNT</span></th>
...
$totpros++;
    ...
    while($infos=mysqli_fetch_array($sqls))
    {
        ...
        echo '<td data-label="'. strtoupper($infomainaccessdebitnote['modulename']) .' DATE" style="font-size:12px !important;vertical-align:middle !important;padding: 1px 6px !important;text-align:left;"><input type="text" readonly value="'.date('d/m/Y',strtotime($infos['debitnotedate'])).'" style="text-align: left;padding:0px;color:black;height: 18px !important;border: none !important;outline: none !important;display: inline-flex;font-size: 12px;" class="form-control form-control-sm"></td>';
        echo '<td data-label="'. strtoupper($infomainaccessdebitnote['modulename']) .' NUMBER" style="font-size:12px !important;vertical-align:middle !important;padding: 1px 6px !important;text-align:left;"><input type="text" readonly value="'.$infos['debitnoteno'].'" style="text-align: left;padding:0px;color:black;height: 18px !important;border: none !important;outline: none !important;display: inline-flex;font-size: 12px;" class="form-control form-control-sm"></td>';
        ...
        echo '<td data-label="'. strtoupper($infomainaccessdebitnote['modulename']) .' AMOUNT" style="font-size:12px !important;vertical-align:middle !important;padding: 1px 6px !important;text-align:right;"><span >'.$rescurrency[0].'</span><input type="text" readonly value="'.(number_format((float)$infos['grandtotal'],2,'.','')).'" style="text-align: right;padding:0px;color:black;height: 18px !important;border: none !important;outline: none !important;display: inline-flex;font-size: 12px;width:90%;" class="form-control form-control-sm"></td>';
...
<p align="right" class="mt-3" style="margin-right:-25px; cursor:pointer" id="templatetext">Template: '<?=($infofranch['purchasereturnpaymenttemplate']=='0')?'Standard A5 Landscape':''?><?=($infofranch['purchasereturnpaymenttemplate']=='1')?'Standard A4 Portrait':''?> ' <a data-bs-toggle="modal" data-bs-target="#changevendorrefundModal" class="text-blue">Change</a></p>
</div>
 <!-- Modal -->
<div class="modal fade" id="changevendorrefundModal" tabindex="-1" role="dialog" aria-labelledby="changevendorrefundModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="changevendorrefundModalLabel" style="font-weight:normal;">Choose Template</h5>
        <button type="button" class="btn-close text-dark" data-bs-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
...
if (r == true) {
    window.location.href='changetemplatereceipt.php?template=purchasereturnpayment&id=<?=$id?>&publicid=<?=$info['publicid']?>&receiptno=<?=$_GET['receiptno']?>&receiptdate=<?=$info['receiptdate']?>&temptype=0&submt=submt';
} else {
...
var r = confirm('Are you sure want to Change this Template?');
if (r == true) {
    window.location.href='changetemplatereceipt.php?template=purchasereturnpayment&id=<?=$id?>&publicid=<?=$info['publicid']?>&receiptno=<?=$_GET['receiptno']?>&receiptdate=<?=$info['receiptdate']?>&temptype=1&submt=submt';
} else {
...
$.ajax({
type: "GET",
url: 'purchasepaydompdf.php?term=vendorrefund&names=<?= $infomainaccessuser['modulename'] ?>-<?=$info['privateid']?>-<?=$_GET['receiptdate']?>&receiptno=<?=$_GET['receiptno']?>&receiptdate=<?=$_GET['receiptdate']?>&sizes=a4&id=<?=$_GET['id']?>',
...
if($infofranch['purchasereturnpaymenttemplate']=='0'){
?>
<script>
function converHTMLFileToPDF() {
$('#pdfObj').html('<div class="text-center"><span id="timer">Please wait <span id="time"></span> Seconds</span></div><br><img src="loading.gif" width="100%">');
...
$.ajax({
type: "GET",
url: 'purchasepaydompdf.php?term=vendorrefund&names=<?= $infomainaccessuser['modulename'] ?>-<?=$info['privateid']?>-<?=$_GET['receiptdate']?>&receiptno=<?=$_GET['receiptno']?>&receiptdate=<?=$_GET['receiptdate']?>&sizes=a5&id=<?=$_GET['id']?>',
success: function (result) {
if (result && result.length > 0) {       
result=$.parseJSON( result );
Timer=result.timer;
