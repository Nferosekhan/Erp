...
$oldquotationno=mysqli_real_escape_string($con, $_GET['quotationno']);
$oldquotationdate=mysqli_real_escape_string($con, $_GET['quotationdate']);
...
if($productname!='')
{
$sql=mysqli_query($con, "delete from pairquotations where createdid='$companymainid' and franchisesession='".$_SESSION["franchisesession"]."' and quotationno='$oldquotationno' and quotationdate='$oldquotationdate'");
if($sql)
{
}
else
...
$sql2=mysqli_query($con, "select id from pairquotations where franchisesession='".$_SESSION['franchisesession']."' and createdid='$companymainid' and quotationno='$oldquotationno' and quotationdate='$oldquotationdate'");
...
echo '<script> window.location.href="quotations.php?remarks=Updated Successfully";</script>'; 
}
else
{
echo '<script> window.location.href="quotationview.php?quotationno='.$quotationno.'&quotationdate='.$quotationdate.'&remarks=Updated Successfully";</script>';
}
}
}
else
...
if ($sql) {
$hissql=mysqli_query($con, "insert into pairusehistory set usetype='QUOTATIONS', createdon='$times',  createdby='".$_SESSION["unqwerty"]."', useid='$quotationno', useremarks='Quotation Updated' ");
}
}
...
<input type="hidden" name="oldquotationno" value="<?=$rows[0]['quotationno']?>">
<input type="hidden" name="oldquotationdate" value="<?=$rows[0]['quotationdate']?>">
<input type="hidden" name="quotationterm" value="<?=$rows[0]['quotationterm']?>">

<div class="row">
        <?php
     if ((in_array('Quotation Information', $fieldedit))) {
        ?>
<div class="col-lg-4">
...
<div class="col-lg-7">
<input type="text" class="form-control  form-control-sm" id="quotationno" name="quotationno" required  value="<?=$rows[0]['quotationno']?>" readonly>
</div>
</div>
</div>
<div class="col-lg-12">
...
<div class="col-lg-7">
<input type="date" class="form-control  form-control-sm" id="quotationdate" name="quotationdate" required value="<?=$rows[0]['quotationdate']?>">
</div>
</div>
</div>
...
?>
<option value="<?= $info['term'] ?>" <?=($info['term']==$rows[0]['quotationterm'])?'selected':''?>><?= $info['term'] ?></option>
<?php
}
?>
</select>
...
<input type="radio" class="custom-control-input" name="customerinfodefault" id="onecustomerinfo" value="one" <?= ($rows[0]['customerinfodefault']=='one'||$infomainaccessuser['quotationcustinfo']=='defone')?'checked':'' ?> onclick="onetwo()">
<label class="custom-control-label custom-label" for="onecustomerinfo">B2B</label>
</div>
</div>
<div class="col-lg-2 col-md-2 col-3">
<div class="custom-control custom-radio mr-sm-2">
<input type="radio" class="custom-control-input" name="customerinfodefault" id="twocustomerinfo" value="two" <?= ($rows[0]['customerinfodefault']=='two'||$infomainaccessuser['quotationcustinfo']=='deftwo')?'checked':'' ?> onclick="onetwo()">
...
while ($info = mysqli_fetch_array($sqli))
{
$sqliquotation=mysqli_query($con, "select quotationamount, quotationdate, quotationno from pairquotations where franchisesession='".$_SESSION['franchisesession']."' and createdid='$companymainid' and customerid='".$info['id']."' GROUP BY quotationdate, quotationno order by quotationdate desc, quotationno desc");
$quotationamount=0;
$balanceamount=0;
$currentamount=0;
$overdueamount=0;
while($infoquotation=mysqli_fetch_array($sqliquotation))
{
$quotationamount+=(float)$infoquotation['quotationamount'];
$paidamount=0;
$sqlsalespay=mysqli_query($con,"select amount from pairsalespayhistory where franchisesession='".$_SESSION['franchisesession']."' and createdid='$companymainid' and invoiceno='".$infoquotation['quotationno']."' and invoicedate='".$infoquotation['quotationdate']."' and customerid='".$info['id']."' order by id desc");
while($infosalespay=mysqli_fetch_array($sqlsalespay))
{
$paidamount+=(float)$infosalespay['amount'];
}
$balanceamount+=((float)$infoquotation['quotationamount']-$paidamount);
$diff = abs(time() - strtotime($infoquotation['quotationdate']));
$days = floor(($diff)/ (60*60*24));
if($days>30)
{
$overdueamount+=((float)$infoquotation['quotationamount']-$paidamount);
}
else
{
$currentamount+=((float)$infoquotation['quotationamount']-$paidamount);
}
}
?>
...
<div class="col-lg-9">
<input type="text" name="twocustomername" id="twocustomername" class="form-control form-control-sm" placeholder="<?= $infomainaccessusercus['modulename'] ?> Name" value="<?= $rows[0]['customername'] ?>" <?= ($access['quotbtwocnamerequired']=='Yes')? 'required': ' ' ?>>
...
<input type="text" class="form-control form-control-sm" id="twoworkphone" name="twoworkphone" placeholder="Work Phone" value="<?=$rows[0]['workphone']?>" <?= ($access['quotbtwocwphonerequired']=='Yes')? 'required': ' ' ?>>
