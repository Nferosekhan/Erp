...
if($customerid!='')
{
	...
	$totalbalance=0;
	if($type!=''&&$no!=''&&$date!=''){
	$sqls=mysqli_query($con,"select duedate,$nocolumn, $datecolumn, grandtotal, customerid, customername from $tabname where franchisesession='".$_SESSION['franchisesession']."' and createdid='$companymainid' and customerid='$customerid' and $datecolumn='$date' and $nocolumn='$no' and cancelstatus!='1' GROUP BY $datecolumn, $nocolumn  order by $datecolumn asc, $nocolumn asc");
	}
	else{
	$sqls=mysqli_query($con,"select duedate,$nocolumn, $datecolumn, grandtotal, customerid, customername from $tabname where franchisesession='".$_SESSION['franchisesession']."' and createdid='$companymainid' and customerid='$customerid' and cancelstatus!='1' GROUP BY $datecolumn, $nocolumn  order by $datecolumn asc, $nocolumn asc");
	}
	while($infos=mysqli_fetch_array($sqls))
	{
		$am=0;
		$bal=0;
		$sqlse=mysqli_query($con,"select sum(amount) as amount from pairsalespayhistory where franchisesession='".$_SESSION['franchisesession']."' and createdid='$companymainid' and  invoiceno='".$infos[$nocolumn]."' and invoicedate='".$infos[$datecolumn]."' and customerid='$customerid' ");
		...
		echo '<td data-label="'. strtoupper($infomainaccessinvoice['modulename']) .' DATE" style="vertical-align:middle !important;text-align:left;">'.date('d/m/Y',strtotime($infos[$datecolumn])).'<br>'.checkDueDate($infos[$datecolumn], $infos['duedate']).'</td>';
		echo '<td data-label="'. strtoupper($infomainaccessinvoice['modulename']) .' NUMBER" style="vertical-align:middle !important;text-align:left;">'.$infos[$nocolumn].'</td>';
...
// if($no==''&&$date==''){
// 	$sqlsant=mysqli_query($con,"select creditnoteno, creditnotedate, grandtotal, customerid, customername from paircreditnotes where franchisesession='".$_SESSION['franchisesession']."' and createdid='$companymainid' and customerid='$customerid' and paidstatus!='1' GROUP BY creditnotedate, creditnoteno  order by creditnotedate asc, creditnoteno asc");
