...
<div class="col-lg-3" style="margin-bottom: 57px !important;">
                                 <input class="form-control" id="vensearch" type="text" placeholder="Search.." style="height:24px;">
                                  </div> 
                                 </div>
                                <?php
$sqlismainaccessuser=mysqli_query($con, "select * from pairmainaccess where userid='$userid' and moduletype='Vendors' order by id  asc");
...
<li><a class="dropdown-item" href="vendorimport.php"><i class="fa fa-download"></i> <?= $infomainaccessuser['modulename'] ?> Import</a></li>
</ul>
<?php
}
?>
...
<?php
        ...
        ?>
          ...
          ?>
                  ...
                  <?php
                  $count=1;
                  $sqli=mysqli_query($con, "select * from paircustomers where (franchisesession='".$_SESSION["franchisesession"]."' or cvisiblity='PUBLIC') and (createdid='$companymainid' and moduletype='Vendors') ".$typesforlisting." order by customername asc limit ".(($access['venpageload']=='pagenum')?'10':'15')."");
...
$sqltotlist = mysqli_query($con,"select COUNT(DISTINCT id) from paircustomers where (franchisesession='".$_SESSION["franchisesession"]."' or cvisiblity='PUBLIC') and (createdid='$companymainid' and moduletype='Vendors') ".$typesforlisting."");
...
<?php
if ($access['venpageload']=='pageauto') {
?>
<script type="text/javascript">
...
$.ajax({
type: "GET",
url: 'listvensearch.php?term=' + sIndex + '&limitings=15&typesforlisting=<?=$typesforlisting?>',
...
$.ajax({
type: "GET",
url: 'listvensearch.php?term='+perpages+'&limitings='+($('#limitforpagenum').val())+'&typesforlisting=<?=$typesforlisting?>',
success: function (result) {
$("#myTable").html(result);
},
error: function (error) {
...
function submitthesorting(value) {
  $.ajax({
    type: "GET",
    url: 'listingsortfilters.php?moduletype=Vendors&typesforlisting='+value+'',
    success: function (result) {
      console.log(result);
      window.location = 'vendors.php?remarks='+result+'';
    },
    error: function (error) {
      console.log(error);
    }
