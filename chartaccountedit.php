...
$sql=mysqli_query($con,"update pairchartaccountings set createdid='$companymainid', createdby='".$_SESSION["unqwerty"]."', franchisesession='".$_SESSION["franchisesession"]."', createdon='$times',   accountcategory='$accountcategory',  accountcode='$accountcode', accountname='$accountname', description='$description',subaccountcheck='$subaccountcheck',parentaccount='$parentaccount' where createdid='$companymainid' and franchisesession='".$_SESSION['franchisesession']."' and id='".$_GET['id']."' and parentaccount='".$_GET['parentid']."'");
...
$updoldvalold = mysqli_query($con,"update pairchartaccounttypes set optionslist='' where id='".$accountcategory."'");

if (isset($_POST['parentaccount'])) {

if ($_GET['cid']!=$accountcategory) {
$seloldvaldel = mysqli_query($con,"select optionslist from pairchartaccounttypes where id='".$_GET['cid']."'");

$fetoldvaldel = mysqli_fetch_array($seloldvaldel);
$explodeoptionsdel = explode(';', $fetoldvaldel['optionslist']);
$finishoptdel = '';

for($i=0;$i<count($explodeoptionsdel)-1;$i++){
$finalvaluesdel = explode(',',$explodeoptionsdel[$i]);
$oldvaluesdel = $finalvaluesdel[0].','.$finalvaluesdel[1].','.$finalvaluesdel[2].','.$finalvaluesdel[3].';';
$newvaluesdel = $_GET['id'] . ',' . $_GET['name'] . ',' . (($_GET['parentid']!='')?$_GET['parentid']:'0') . ',' . $_GET['padding'].';';

if($oldvaluesdel!=$newvaluesdel){
$finishoptdel .= $finalvaluesdel[0].','.$finalvaluesdel[1].','.$finalvaluesdel[2].','.$finalvaluesdel[3].';';
}

}
$updoldvaloldval = mysqli_query($con,"update pairchartaccounttypes set optionslist='$finishoptdel' where id='".$_GET['cid']."'");
}

for($i=0;$i<count($explodeoptions)-1;$i++){

$finalvalues = explode(',',$explodeoptions[$i]);
$oldvalues = $finalvalues[0].','.$finalvalues[1].','.$finalvalues[2].','.$finalvalues[3].';';
$newvalues = $_GET['id'] . ',' . $_GET['name'] . ',' . (($_GET['parentid']!='')?$_GET['parentid']:'0') . ',' . $_GET['padding'].';';

if (($finalvalues[0]==$parentaccount)&&($parentaccount!=(($_GET['parentid']!='')?$_GET['parentid']:'0'))) {
...
$finishopt .= $finalvalues[0].','.$finalvalues[1].','.$finalvalues[2].','.$finalvalues[3].';'.$_GET['id'].','.$accountname.','.$parentaccount.','.($finalvalues[3]+10).';';
}
}
else{
if ($parentaccount!=(($_GET['parentid']!='')?$_GET['parentid']:'0')) {
if($oldvalues!=$newvalues){
...
$finishopt .= $_GET['id'].','.$accountname.',0,10;';
}
}
else{
if ($fetoldval['optionslist']!='') {
...
if ($updoldval) {
header("Location:chartaccounts.php?remarks=Updated Successfully");
}
...
$seleditdata = mysqli_query($con,"select * from pairchartaccountings where createdid='$companymainid' and franchisesession='".$_SESSION['franchisesession']."' and id='".$_GET['id']."' and parentaccount='".$_GET['parentid']."'");
$feteditdata=mysqli_fetch_array($seleditdata);
?>
<!DOCTYPE html>
<html lang="en">
...
?>
<option value="<?=$fetacctypes['id']?>" <?=(($feteditdata['accountcategory']==$fetacctypes['id'])?'selected':'')?>><?=$fetacctypes['chartaccounttypelists']?></option>
<?php
}
?>
</optgroup>
...
<div class="col-sm-8">
  <input type="text" class="form-control form-control-sm" id="chartaccountname" name="chartaccountname" placeholder="Account Name" required value="<?=$feteditdata['accountname']?>">        
</div>
      </div>
...
<input type="checkbox" class="custom-control-input" name="subaccount" id="subaccount" onchange="subaccountch()" <?=(($feteditdata['subaccountcheck']=='1')?'checked':'')?>>
...
if (<?=$feteditdata['subaccountcheck']?>==1) {
...
$.ajax({
type: "GET",
url: 'parentcheck.php?chartaccountcategory='+$("#chartaccountcategory").val()+'&editcheck=<?=$_GET['id']?>&editcheckparent=<?=$_GET['parentid']?>',
...
$('select[name^="parentaccount"] option[value="<?=$feteditdata['parentaccount']?>"]').attr("selected","selected");
}
else{
...
<div class="col-sm-8">
  <input type="text" class="form-control form-control-sm" id="chartaccountcode" name="chartaccountcode" placeholder="Account Code" value="<?=$feteditdata['accountcode']?>">
...
<div class="col-sm-8">
  <textarea placeholder="Description" class="form-control form-control-sm" id="notes" name="notes" style="height: 45px !important;"><?=$feteditdata['description']?></textarea>
</div>
      </div>
