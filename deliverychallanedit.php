...
$olddeliverychallanno=mysqli_real_escape_string($con, $_GET['deliverychallanno']);
$olddeliverychallandate=mysqli_real_escape_string($con, $_GET['deliverychallandate']);
...
if($productname!='')
{
$sql=mysqli_query($con, "delete from pairdeliverychallans where createdid='$companymainid' and franchisesession='".$_SESSION["franchisesession"]."' and deliverychallanno='$olddeliverychallanno' and deliverychallandate='$olddeliverychallandate'");
if($sql)
{
}
else
...
$sql2=mysqli_query($con, "select id from pairdeliverychallans where franchisesession='".$_SESSION['franchisesession']."' and createdid='$companymainid' and deliverychallanno='$olddeliverychallanno' and deliverychallandate='$olddeliverychallandate'");
...
echo '<script> window.location.href="deliverychallans.php?remarks=Updated Successfully";</script>'; 
}
else
{
echo '<script> window.location.href="deliverychallanview.php?deliverychallanno='.$deliverychallanno.'&deliverychallandate='.$deliverychallandate.'&remarks=Updated Successfully";</script>';
}
}
}
else
...
if ($sql) {
$hissql=mysqli_query($con, "insert into pairusehistory set usetype='DELIVERYCH', createdon='$times',  createdby='".$_SESSION["unqwerty"]."', useid='$deliverychallanno', useremarks='Delivery Challan Updated' ");
}
}
...
<input type="hidden" name="olddeliverychallanno" value="<?=$rows[0]['deliverychallanno']?>">
<input type="hidden" name="olddeliverychallandate" value="<?=$rows[0]['deliverychallandate']?>">
<input type="hidden" name="deliverychallanterm" value="<?=$rows[0]['deliverychallanterm']?>">

<div class="row">
        <?php
     if ((in_array('Delivery Challan Information', $fieldedit))) {
        ?>
<div class="col-lg-4">
...
<div class="col-lg-7">
<input type="text" class="form-control  form-control-sm" id="deliverychallanno" name="deliverychallanno" required  value="<?=$rows[0]['deliverychallanno']?>" readonly>
</div>
</div>
</div>
<div class="col-lg-12">
...
<div class="col-lg-7">
<input type="date" class="form-control  form-control-sm" id="deliverychallandate" name="deliverychallandate" required value="<?=$rows[0]['deliverychallandate']?>">
</div>
</div>
</div>
...
?>
<option value="<?= $info['term'] ?>" <?=($info['term']==$rows[0]['deliverychallanterm'])?'selected':''?>><?= $info['term'] ?></option>
<?php
}
?>
</select>
...
<input type="radio" class="custom-control-input" name="customerinfodefault" id="onecustomerinfo" value="one" <?= ($rows[0]['customerinfodefault']=='one'||$infomainaccessuser['deliverychallancustinfo']=='defone')?'checked':'' ?> onclick="onetwo()">
<label class="custom-control-label custom-label" for="onecustomerinfo">B2B</label>
</div>
</div>
<div class="col-lg-2 col-md-2 col-3">
<div class="custom-control custom-radio mr-sm-2">
<input type="radio" class="custom-control-input" name="customerinfodefault" id="twocustomerinfo" value="two" <?= ($rows[0]['customerinfodefault']=='two'||$infomainaccessuser['deliverychallancustinfo']=='deftwo')?'checked':'' ?> onclick="onetwo()">
...
while ($info = mysqli_fetch_array($sqli))
{
$sqlideliverychallan=mysqli_query($con, "select deliverychallanamount, deliverychallandate, deliverychallanno from pairdeliverychallans where franchisesession='".$_SESSION['franchisesession']."' and createdid='$companymainid' and customerid='".$info['id']."' GROUP BY deliverychallandate, deliverychallanno order by deliverychallandate desc, deliverychallanno desc");
$deliverychallanamount=0;
$balanceamount=0;
$currentamount=0;
$overdueamount=0;
while($infodeliverychallan=mysqli_fetch_array($sqlideliverychallan))
{
$deliverychallanamount+=(float)$infodeliverychallan['deliverychallanamount'];
$paidamount=0;
$sqlsalespay=mysqli_query($con,"select amount from pairsalespayhistory where franchisesession='".$_SESSION['franchisesession']."' and createdid='$companymainid' and invoiceno='".$infodeliverychallan['deliverychallanno']."' and invoicedate='".$infodeliverychallan['deliverychallandate']."' and customerid='".$info['id']."' order by id desc");
while($infosalespay=mysqli_fetch_array($sqlsalespay))
{
$paidamount+=(float)$infosalespay['amount'];
}
$balanceamount+=((float)$infodeliverychallan['deliverychallanamount']-$paidamount);
$diff = abs(time() - strtotime($infodeliverychallan['deliverychallandate']));
$days = floor(($diff)/ (60*60*24));
if($days>30)
{
$overdueamount+=((float)$infodeliverychallan['deliverychallanamount']-$paidamount);
}
else
{
$currentamount+=((float)$infodeliverychallan['deliverychallanamount']-$paidamount);
}
}
?>
...
<div class="col-lg-9">
<input type="text" name="twocustomername" id="twocustomername" class="form-control form-control-sm" placeholder="<?= $infomainaccessusercus['modulename'] ?> Name" value="<?= $rows[0]['customername'] ?>" <?= ($access['delchbtwocnamerequired']=='Yes')? 'required': ' ' ?>>
...
<input type="text" class="form-control form-control-sm" id="twoworkphone" name="twoworkphone" placeholder="Work Phone" value="<?=$rows[0]['workphone']?>" <?= ($access['delchbtwocwphonerequired']=='Yes')? 'required': ' ' ?>>
