...
if (isset($_POST['submitcustomizes'])) {
	...
	$sqlreportmoduless=$con->prepare("SELECT * FROM pairreportmodules WHERE types='crnoteconsumer' ORDER BY id ASC");
	...
	foreach ($newmodules as $newmoduleskey) {
		...
		$modcolumncol=$coltypemod."crnoteconsumer";
	...
	$sqlupreport = $con->prepare("UPDATE pairreports SET rowcolumns=? WHERE franchiseid=? AND createdid=? AND types='crnoteconsumer'");
	...
	$sqlreport = $con->prepare("UPDATE pairreports SET reportperiod=?,filtername=?,companyname=?,dateprepare=?,timeprepare=? WHERE franchiseid=? AND createdid=? AND types='crnoteconsumer'");
	...
	header("Location:reportcrnoteconsumer.php?datefrom=".date($dateformatforurlredirection,strtotime($from))."&dateto=".date($dateformatforurlredirection,strtotime($to))."");
}

$sqlreportviews=$con->prepare("SELECT * FROM pairreports WHERE franchiseid=? AND createdid=? AND types='crnoteconsumer'");
									...
									<div class="row">
										<div class="col-lg-6">
											<p class="mb-3" style="color:black;font-size: 20px;margin-top: -8px;">
												...
												&gt;
												<a href="reportcrnote.php?datefrom=<?=$datefrom?>&dateto=<?=$dateto?>" style="color: #1878F1">
																			...
																			<div id="customizerowcolumns" class="accordion-collapse collapse show" aria-labelledby="customizerowcolumn">
																				<div class="accordion-body text-sm">
																					<div class="row justify-content-center">
																						...
																						<?php
																							$newans=array();
																							$sqlreportaccesss=$con->prepare("SELECT * FROM pairreports WHERE franchiseid=? AND createdid=? AND types='crnoteconsumer'");
																								...
																								<div class="col-lg-6 my-1" style="padding-left: 0px !important;">
																									<div class="custom-control custom-checkbox mr-sm-2" style="padding: 0px 12px !important;">
																										<div class="input-group input-group-sm" id="flaghead">
																											...
																											<input type="checkbox" class="custom-control-input" name="<?= $coltypemod; ?>crnoteconsumer" id="<?= $coltypemod; ?>crnoteconsumer" checked disabled>
																											<label class="custom-control-label custom-label" for="<?= $coltypemod; ?>crnoteconsumer">
																												<?= str_replace(" or ", " / ",str_replace("Credit Note",$infomainaccessusercreditnote['modulename'],str_replace("Taxable Amount",$access['txttaxablecreditnote'],$newmoduleskey))) ?>
																											</label>
																										</div>
																									</div>
																								</div>
											...
											<div class="tab-pane fade" id="nav-profile" role="tabpanel" aria-labelledby="nav-profile-tab">
												<div class="table-responsive m-3" id="histables">
													<table class="table table-bordered">
														...
														<tbody>
      												<?php
															$sqluse=$con->prepare("SELECT * FROM pairusehistory WHERE usetype = 'REPcrnoteconsumer' AND useid = ? ORDER BY createdon DESC");
																				...
																				<table id="print-are1" style="border:1px solid #eee;height: 975px;" width="100%">
																					<thead>
																						<tr>
																							...
																							<td class="text-uppercase" style="text-align: left !important;border:1px solid #eee;padding-left: 10px;">
																								<span style="font-size:12px;color:black;">
																										<?=$infomainaccessusercreditnote["modulename"]?> DATE
																								</span>
																							</td>
																					...
																					<tbody id="myTable">
																					<?php
																						$sqls=$con->prepare("SELECT * FROM paircreditnotes WHERE franchisesession=? AND createdid=? AND (creditnotedate>=? AND creditnotedate<=?) AND cancelstatus='0' AND ".(($sqlviewreport['gstrule']=='0')?'gstrtype="Consumer"':'(gstrtype="Registered Business - Regular" OR gstrtype="Consumer") AND gstno="" ')." GROUP BY creditnotedate, creditnoteno ORDER BY creditnotedate ASC, creditnoteno ASC LIMIT 15");
																					...
																					?>
																						<tr style="vertical-align: middle;height: 18px !important;" onclick="window.open('creditnoteview.php?id=<?=$info['id']?>&creditnoteno=<?=$info['creditnoteno']?>&creditnotedate=<?=$info['creditnotedate']?>','_self')">
																							<td data-label="PLACE OF SUPPLY" style="text-align: left !important;font-size:12px;color:black;border:1px solid #eee;padding-left: 10px;">
																								...
																								<br>
																								<span style="display: inline-flex;max-width: 190px;min-width: 190px;white-space: nowrap;overflow: hidden;color: grey;"><?=$infomainaccessusercreditnote["modulename"]?> Number : 
																									<span style="color: black;">
																										<?=$info['creditnoteno']?>
																									</span>
																								</span>
																							...
																							<td data-label="<?=$infomainaccessusercreditnote["modulename"]?> DATE" style="text-align: left !important;font-size:12px;color:black;border:1px solid #eee;padding-left: 10px;">
																								<span style="display: inline-flex;max-width: 70px;min-width: 70px;white-space: nowrap;overflow: hidden;">
																									<span>
																										<?=date($datemainphp,strtotime($info['creditnotedate']))?>
																									</span>
																								</span>
																							</td>
																					...
																					<?php
										      										...
										      										while($sqlfetlist=$sqltotlistpage->fetch_array()){
																						...
																						if (($checkspage>=1)&&($checkspage<=15)) {
																							...
																							$pagetotnumpage = ceil($checkspage/15);
																						}
																						if ($pagetotnumpage==1) {
																						?>
										...
										<table id="print-are3" style="border: 1px solid #eee;display: none;" width="100%">
											<thead>
												<tr>
													...
													<td class="text-uppercase" style="text-align: left !important;border:1px solid #eee;padding-left: 10px;"><span style="font-size:12px;color:black;"> <?=$infomainaccessusercreditnote["modulename"]?> DATE</span></td>
													<td class="text-uppercase" style="text-align: right !important;border:1px solid #eee;padding-right: 10px;"><span style="font-size:12px;color:black;"> <?=($access["txttaxablecreditnote"])?></span></td>
										...
										<table id="csvprint-are3" style="border: 1px solid #eee;display: none;" width="100%">
											<thead>
												<tr>
													<td class="text-uppercase" style="text-align: left !important;border:1px solid #eee;padding-left: 10px;"><span style="font-size:12px;color:black;"> <?=$infomainaccessusercust['modulename']?> Name</span></td>
													<td class="text-uppercase" style="text-align: left !important;border:1px solid #eee;padding-left: 10px;"><span style="font-size:12px;color:black;"> <?=$infomainaccessusercreditnote["modulename"]?> NUMBER</span></td>
													<td class="text-uppercase" style="text-align: left !important;border:1px solid #eee;padding-left: 10px;"><span style="font-size:12px;color:black;"> <?=$infomainaccessusercreditnote["modulename"]?> DATE</span></td>
													...
													<td class="text-uppercase" style="text-align: left !important;border:1px solid #eee;padding-left: 10px;"><span style="font-size:12px;color:black;"> Place of Supply</span></td>
													<td class="text-uppercase" style="text-align: right !important;border:1px solid #eee;padding-right: 10px;"><span style="font-size:12px;color:black;"> Total <?=($access["txttaxablecreditnote"])?></span></td>
...
const pagechanges = (page, pageSize) => {
	...
	var totalpagesnumber = <?=ceil($checkspage/15)?>;
	if ((page=='')||(page==0)) {
		var isthisval = 1;
	}
	...
	var perpages = ''+((parseInt(isthisval)-1)*15);
	...
	ajaxRequestview = $.ajax({
		type: "GET",
		url: 'reportviewsearch.php?lastornone='+lastornone+'&term='+perpages+'&limitings='+($('#limitforpagenum').val())+'&datefrom=<?=$datefrom?>&dateto=<?=$dateto?>&dif=crnoteconsumer',
...
$(function() {
	...
	function(start, end, label) {
		...
		window.open("reportcrnoteconsumer.php?datefrom="+datefrom+"&dateto="+dateto+"","_self");
	});
});
...
function converHTMLFileToPDF() {
	...
	var finaltimer = setTimeout(function(){
		...
		var header = function(data) {
			...
			var headtext1 = "Taxable outward supplies to consumer";
			var fontSize = doc.internal.getFontSize();
			var pageWidth = doc.internal.pageSize.width;
		...
		?>
			...
			x = ( pageWidth - txtWidth ) / 2;
			doc.text(printedon, x, pageHeight - 26);
		};
		var botforprint = 40;
		doc.autoTable({ 
			html: 
		...
		$.ajax({
			type: "GET",
			url: 'reporthistory.php?types=crnoteconsumer&titles=Taxable outward supplies to consumer&printdownpdfcsv=Printed PDF&fromto='+fromtopdf+'',
			success: function (result) {
				console.log(result);
			},
			error: function (error) {
...
function downloadaspdf() {
	...
	var finaltimer = setTimeout(function(){
		...
		$.ajax({
			type: "GET",
			url: 'reporthistory.php?types=crnoteconsumer&titles=Taxable outward supplies to consumer&printdownpdfcsv=Downloaded PDF&fromto='+fromtopdf+'',
			success: function (result) {
				console.log(result);
			},
			error: function (error) {
		...
		doc.save('Taxable-Outward-Supplies-To-Consumer-<?=$datefrom?>-To-<?=$dateto?>');
...
function downloadascsv() {
	...
	var finaltimer = setTimeout(function(){
		...
		data.push(["Taxable outward supplies to consumer"]);
		data.push([fromtopdf]);
		...
		link.setAttribute("download", "Taxable-Outward-Supplies-To-Consumer-Reports-<?=$datefrom?>-To-<?=$dateto?>");
		document.body.appendChild(link);
		link.click();
		$.ajax({
			type: "GET",
			url: 'reporthistory.php?types=crnoteconsumer&titles=Taxable outward supplies to consumer&printdownpdfcsv=Downloaded CSV&fromto='+fromtopdf+'',
			success: function (result) {
				console.log(result);
			},
			error: function (error) {
...
document.addEventListener('DOMContentLoaded', function() {
	...
	function makeAjaxRequest() {
		ajaxRequest = $.ajax({
			type: "GET",
			url: 'check.php?term=crnoteconsumer&datefrom=<?=$datefrom?>&dateto=<?=$dateto?>&limitfrom=0&limitto=10000',
			success: function (result) {
				if (result != mytabprev) {
		...
		ajaxCsvRequest = $.ajax({
			type: "GET",
			url: 'reportcsv.php?term=crnoteconsumer&datefrom=<?=$datefrom?>&dateto=<?=$dateto?>',
			success: function (result) {
