...
elseif($infomainaccessuser['filtertypesforlistsorting']=='Refunded'){
  $typesforlisting = "AND paidstatus='1' AND cancelstatus='0'";
  $nameshowinglisting = "Refunded";
}
elseif($infomainaccessuser['filtertypesforlistsorting']=='Not Refunded'){
  $typesforlisting = "AND paidstatus='0' AND cancelstatus='0'";
  $nameshowinglisting = "Not Refunded";
}
elseif($infomainaccessuser['filtertypesforlistsorting']=='Partially Refunded'){
  $typesforlisting = "AND paidstatus='2' AND cancelstatus='0'";
  $nameshowinglisting = "Partially Refunded";
}
				...
				<div class="col-12">
					<div class="card mb-4 mt-5">
						  ...
						  <div class="app-utility-item app-user-dropdown dropdown">
							 ...
							 <div class="dropdown-menu  dropdown-menu-end customdropdown  me-sm-2" aria-labelledby="dropdownMenuButton111" style="max-height: 0px;max-width: 0px;">
								<div style="background-color: #3c3c46;margin-top: -39px !important;position: relative !important;left: 145px !important;" id="dropdownMenuButton111check">
								  ...
								  <a class="nav-link <?=(($infomainaccessuser['filtertypesforlistsorting']=='Refunded')?'active':'')?>" href="#" style="color: #fff;padding: 6px 3px !important;" onclick="submitthesorting('Refunded')">
									 <!-- <i class="fa fa-sign-out-alt"></i> -->
									 <span class="nav-link-text ms-2"> Refunded</span>
								  </a>
								  <a class="nav-link <?=(($infomainaccessuser['filtertypesforlistsorting']=='Not Refunded')?'active':'')?>" href="#" style="color: #fff;padding: 6px 3px !important;" onclick="submitthesorting('Not Refunded')">
									 <!-- <i class="fa fa-sign-out-alt"></i> -->
									 <span class="nav-link-text ms-2"> Not Refunded</span>
								  </a>
								  <a class="nav-link <?=(($infomainaccessuser['filtertypesforlistsorting']=='Partially Refunded')?'active':'')?>" href="#" style="color: #fff;padding: 6px 3px !important;" onclick="submitthesorting('Partially Refunded')">
									 <!-- <i class="fa fa-sign-out-alt"></i> -->
									 <span class="nav-link-text ms-2"> Partially Refunded</span>
								  </a>
								</div>
							 </div>
						  </div>
								...
								<div class="col-lg-3" style="margin-bottom: 57px !important;"> 
									<input class="form-control" id="creditnotesearch" type="text" placeholder="Search.." style="height:24px;">
								</div>
							</div>
						<?php 
							$sqlismainaccessusers=$con->prepare("SELECT * FROM pairmainaccess WHERE userid=? AND moduletype='Credit Notes' ORDER BY id ASC");
						...
						?>
							<div align="right" class=" p-2 addandmenuonlyforimports" style="margin-bottom: -30px;">
								<div class="row" style="width:250px;">           
									<div class="col-8" style="<?=((in_array('New Credit Notes', $modulecolumns))?'':'visibility: hidden;')?>">  
										<a href="creditnoteadd.php" class="btn btn-custom btn-sm p-2 add" style="font-size: 13px;height: 24px;margin-bottom:1rem;margin-top: -151px;margin-right:-39px;padding-right: 5px;">
									...
									<div class="col-4">
										...
										?>
											<ul class="dropdown-menu dropdown-menu-end" aria-labelledby="dropdownMenuButton">
												<li>
													<a class="dropdown-item" href="creditnoteimport.php">
						...
						<?php
							...
							$sqlismainaccessusers=$con->prepare("SELECT * FROM pairmainaccess WHERE franchiseid=? AND moduletype='Credit Notes' ORDER BY id ASC");
							$sqlismainaccessusers->bind_param("i", $_SESSION['franchisesession']);
							$sqlismainaccessusers->execute();
							$sqlismainaccessuser = $sqlismainaccessusers->get_result();
							$infomainaccessuser=$sqlismainaccessuser->fetch_array();
							//FOR GET MODULE NAME OF CREDITNOTE
...
<div style="visibility: visible;" id="arrowsalltabs">
	...
	<div class="tab-content" id="nav-tabContent" style="position:relative;top: -88px;">
		<div class="tab-pane fade show active p-3" id="nav-general" role="tabpanel" aria-labelledby="nav-general-tab">
							<div class="table-responsive p-0 min-height-480">
								<table id="someTable" class="table table-bordered align-items-center mb-0" style="table-layout: fixed;">
									...
									<?php
										...
										$sqls=$con->prepare("SELECT MAX(id) AS id,creditnotedate, creditnoteno, customername, creditnoteterm, duedate, creditnoteamount, cancelstatus FROM paircreditnotes WHERE franchisesession=? AND createdid=? ".$typesforlisting." GROUP BY creditnotedate, creditnoteno ORDER BY creditnotedate DESC, creditnotetime DESC,ordering DESC LIMIT ".(($access['creditnotepageload']=='pagenum')?'10':'15')."");
										...
										while($info=$sql->fetch_array()){
											...
											$paidamount=0;
											$currentbalance=0;
											$sqlsalespays=$con->prepare("SELECT amount FROM paircreditnotepayhistory WHERE franchisesession=? AND createdid=? AND creditnoteno=? AND creditnotedate=? ORDER BY id DESC");
											$sqlsalespays->bind_param("ssss", $_SESSION['franchisesession'], $companymainid, $info['creditnoteno'], $info['creditnotedate']);
									...
									<?php
										...
										$sqltotcreditnotes=$con->prepare("SELECT COUNT(DISTINCT creditnoteno) FROM paircreditnotes WHERE franchisesession=? AND createdid=? ".$typesforlisting."");
									$sqltotcreditnotes->bind_param("ss", $_SESSION['franchisesession'], $companymainid);
									$sqltotcreditnotes->execute();
									$sqltotcreditnote = $sqltotcreditnotes->get_result();
										$sqlfetlist=$sqltotcreditnote->fetch_array();
										if ($sqlfetlist[0]==0) {
											$pageinitnum = 0;
										}
										if ($sqlfetlist[0]!=0) {
							...
							<?php
								if ($access['creditnotepageload']=='pageauto') {
							?>
								<script type="text/javascript">
								...
								$('.main-content').on('scroll', function() {
									...
									if (scrollTop + $(this).innerHeight() >= this.scrollHeight-50) {
										if (isPreviousEventComplete && isDataAvailable) {
											...
											$.ajax({
												type: "GET",
												url: 'listcreditnotesearch.php?term=' + sIndex + '&limitings=15&typesforlisting=<?=urlencode($typesforlisting)?>',
					 ...
					 try{
								...
								const pagechanges = (page, pageSize) => {
									...
									$.ajax({
										type: "GET",
										url: 'listcreditnotesearch.php?term='+perpages+'&limitings='+($('#limitforpagenum').val())+'&typesforlisting=<?=urlencode($typesforlisting)?>',
										success: function (result) {
		...
		<div class="modal fade" id="deleteconfirm-adddelete" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
			<div class="modal-dialog">
				<div class="modal-content">
					...
					<div class="modal-body">Are you sure you want to Cancel this Credit Note?</div>
...
function submitthesorting(value) {
  $.ajax({
	 type: "GET",
	 url: 'listingsortfilters.php?moduletype=Credit Notes&typesforlisting='+value+'',
	 success: function (result) {
		console.log(result);
		window.location = 'creditnotes.php?remarks='+result+'';
	 },
	 error: function (error) {
		console.log(error);
	 }
