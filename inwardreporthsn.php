...
if (isset($_POST['submitcustomizes'])) {
	...
	$sqlreportmoduless=$con->prepare("SELECT * FROM pairreportmodules WHERE types='inwhsn' ORDER BY id ASC");
	...
	foreach ($newmodules as $newmoduleskey) {
		...
		$modcolumncol=$coltypemod."inwhsn";
	...
	$sqlupreport = $con->prepare("UPDATE pairreports SET rowcolumns=? WHERE franchiseid=? AND createdid=? AND types='inwhsn'");
	...
	$sqlreport = $con->prepare("UPDATE pairreports SET reportperiod=?,filtername=?,companyname=?,dateprepare=?,timeprepare=? WHERE franchiseid=? AND createdid=? AND types='inwhsn'");
	...
	header("Location:inwardreporthsn.php?datefrom=".date($dateformatforurlredirection,strtotime($from))."&dateto=".date($dateformatforurlredirection,strtotime($to))."");
}

$sqlreportviews=$con->prepare("SELECT * FROM pairreports WHERE franchiseid=? AND createdid=? AND types='inwhsn'");
																			...
																			<div id="customizerowcolumns" class="accordion-collapse collapse show" aria-labelledby="customizerowcolumn">
																				<div class="accordion-body text-sm">
																					<div class="row justify-content-center">
																						...
																						<?php
																							$newans=array();
																							$sqlreportaccesss=$con->prepare("SELECT * FROM pairreports WHERE franchiseid=? AND createdid=? AND types='inwhsn'");
																								...
																								<div class="col-lg-6 my-1" style="padding-left: 0px !important;">
																									<div class="custom-control custom-checkbox mr-sm-2" style="padding: 0px 12px !important;">
																										<div class="input-group input-group-sm" id="flaghead">
																											...
																											<input type="checkbox" class="custom-control-input" name="<?= $coltypemod; ?>inwhsn" id="<?= $coltypemod; ?>inwhsn" checked disabled>
																											<label class="custom-control-label custom-label" for="<?= $coltypemod; ?>inwhsn">
											...
											<div class="tab-pane fade" id="nav-profile" role="tabpanel" aria-labelledby="nav-profile-tab">
												<div class="table-responsive m-3" id="histables">
													<table class="table table-bordered">
														...
														<tbody>
      												<?php
															$sqluse=$con->prepare("SELECT * FROM pairusehistory WHERE usetype = 'REPinwhsn' AND useid = ? ORDER BY createdon DESC");
																		...
																		<tr style="height:1px;">
																			<td width="100%" style="padding:10px;">
																				<table id="print-are1" style="border:1px solid #eee;height: 975px;" width="100%">
																					...
																					<tbody id="myTable">
																					<?php
																						$sqls=$con->prepare("SELECT * FROM pairbills WHERE franchisesession=? AND createdid=? AND (billdate>=? AND billdate<=?) AND cancelstatus='0' ORDER BY producthsn ASC, productname ASC LIMIT 36");
																					...
																					<?php
																						...
																						$sqltotlistpages=$con->prepare("SELECT * FROM pairbills WHERE franchisesession=? AND createdid=? AND (billdate>=? AND billdate<=?) AND cancelstatus='0' ORDER BY producthsn ASC, productname ASC");
...
const pagechanges = (page, pageSize) => {
	...
	ajaxRequestview = $.ajax({
		type: "GET",
		url: 'reportviewsearch.php?lastornone='+lastornone+'&term='+perpages+'&limitings='+($('#limitforpagenum').val())+'&datefrom=<?=$datefrom?>&dateto=<?=$dateto?>&dif=inwardhsn',
...
$(function() {
	...
	function(start, end, label) {
		...
		window.open("inwardreporthsn.php?datefrom="+datefrom+"&dateto="+dateto+"","_self");
	});
});
...
function converHTMLFileToPDF() {
	...
	var finaltimer = setTimeout(function(){
		...
		var header = function(data) {
			...
			var headtext1 = "HSN-wise summary of inward supplies";
			var fontSize = doc.internal.getFontSize();
			var pageWidth = doc.internal.pageSize.width;
		...
		$.ajax({
			type: "GET",
			url: 'reporthistory.php?types=inwhsn&titles=HSN-wise summary of inward supplies&printdownpdfcsv=Printed PDF&fromto='+fromtopdf+'',
			success: function (result) {
				console.log(result);
			},
			error: function (error) {
...
function downloadaspdf() {
	...
	var finaltimer = setTimeout(function(){
		...
		$.ajax({
			type: "GET",
			url: 'reporthistory.php?types=inwhsn&titles=HSN-wise summary of inward supplies&printdownpdfcsv=Downloaded PDF&fromto='+fromtopdf+'',
			success: function (result) {
				console.log(result);
			},
			error: function (error) {
		...
		doc.save('HSN-wise-summary-of-inward-supplies-<?=$datefrom?>-To-<?=$dateto?>');
...
function downloadascsv() {
	...
	var finaltimer = setTimeout(function(){
		...
		data.push(["HSN-wise summary of inward supplies"]);
		data.push([fromtopdf]);
		...
		link.setAttribute("download", "HSN-wise-summary-of-inward-supplies-Reports-<?=$datefrom?>-To-<?=$dateto?>");
		document.body.appendChild(link);
		link.click();
		$.ajax({
			type: "GET",
			url: 'reporthistory.php?types=inwhsn&titles=HSN-wise summary of inward supplies&printdownpdfcsv=Downloaded CSV&fromto='+fromtopdf+'',
			success: function (result) {
				console.log(result);
			},
			error: function (error) {
...
document.addEventListener('DOMContentLoaded', function() {
	...
	function makeAjaxRequest() {
		ajaxRequest = $.ajax({
			type: "GET",
			url: 'check.php?term=inwardhsn&datefrom=<?=$datefrom?>&dateto=<?=$dateto?>&limitfrom=0&limitto=10000',
			success: function (result) {
				if (result != mytabprev) {
