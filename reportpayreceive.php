...
if (isset($_POST['submitcustomizes'])) {
	...
	$sqlreportmoduless=$con->prepare("SELECT * FROM pairreportmodules WHERE types='payreceive' ORDER BY id ASC");
	...
	foreach ($newmodules as $newmoduleskey) {
		...
		$modcolumncol=$coltypemod."payreceive";
	...
	$sqlupreport = $con->prepare("UPDATE pairreports SET rowcolumns=? WHERE franchiseid=? AND createdid=? AND types='payreceive'");
	...
	$sqlreportfav = $con->prepare("UPDATE pairreportfavourites SET reporturl=? WHERE franchisesession=? AND createdid=? AND reportnames='payreceive'");
	$sqlreportfav->bind_param("sss", $newReportUrl, $_SESSION['franchisesession'], $companymainid);
	$sqlreportfav->execute();
	$sqlreportfav->close();

	$sqlreport = $con->prepare("UPDATE pairreports SET cusvenname=?,reportperiod=?,filtername=?,companyname=?,dateprepare=?,timeprepare=?,names=?,payterm=? WHERE franchiseid=? AND createdid=? AND types='payreceive'");
	...
	header("Location:reportpayreceive.php?names=".$names."&datefrom=".date($dateformatforurlredirection,strtotime($from))."&dateto=".date($dateformatforurlredirection,strtotime($to))."&timefrom=".$timefrom."&timeto=".$timeto."&payterm=".$payterm."");
}
								...
								<form action="" method="post" enctype="multipart/form-data" class="form-horizontal mt-0" role="form">
									<div class="row">
																			...
																			<div id="customizerowcolumns" class="accordion-collapse collapse show" aria-labelledby="customizerowcolumn">
																				<div class="accordion-body text-sm">
																					<div class="row justify-content-center">
																						...
																						<?php
																							$newans=array();
																							$sqlreportaccesss=$con->prepare("SELECT * FROM pairreports WHERE franchiseid=? AND createdid=? AND types='payreceive'");
											...
											<div class="tab-pane fade" id="nav-profile" role="tabpanel" aria-labelledby="nav-profile-tab">
												<div class="table-responsive m-3" id="histables">
													<table class="table table-bordered">
														...
														<tbody>
														<?php
															$sqluse=$con->prepare("SELECT * FROM pairusehistory WHERE usetype = 'REPpayreceive' AND useid = ? ORDER BY createdon DESC");
																				...
																				<table id="print-are1" style="border: 1px solid #eee;" width="100%">
																					<thead style="background-color: #e1e1e1;">
																						<tr>
																							...
																							<td class="text-uppercase" style="<?=(((in_array('Customer Name', $newans)))?'':'display: none;')?>border: 1px solid #eee;padding-left: 10px;max-width: 100px !important;min-width: 100px !important;white-space: nowrap;overflow: hidden;<?=((($sqlviewreport['filtername']=='1')||($sqlviewreport['filtername']=='0'))?'':'display:none;')?>">
																								<span style="font-size:13px;color:black;">
																					...
																					<tbody id="myTable">
																					<?php
																						$sqlinfos=$con->prepare("SELECT * FROM pairsalespayments WHERE franchisesession=? AND createdid=? AND (receiptdate>=? AND receiptdate<=?) ".(($sqlviewreport['names']=='all')?' ':' AND customerid="'.$sqlviewreport['names'].'"')."".(($sqlviewreport['payterm']=='all')?' ':' AND paymentmode="'.$sqlviewreport['payterm'].'"')." ORDER BY id DESC LIMIT 36");
																					...
																					?>
																						<tr style="height: 30px;">
																							...
																							<td data-label="<?=$infomainaccessusercus['modulename']?> Name" style="<?=(((in_array('Customer Name', $newans)))?'':'display: none;')?>border: 1px solid #eee;padding-left: 10px;font-size: 13px;<?=((($sqlviewreport['filtername']=='1')||($sqlviewreport['filtername']=='0'))?'':'display:none;')?>">
										...
										<table id="print-are3" style="border: 1px solid #eee;display: none;" width="100%">
											<thead>
												<tr>
													<td class="text-uppercase" style="border: 1px solid #eee;padding-left: 10px;"><span style="font-size:13px;color:black;">Date</span></td>
													<td class="text-uppercase" style="border: 1px solid #eee;padding-left: 10px;"><span style="font-size:13px;color:black;">Number</span></td>
													<td class="text-uppercase" style="<?=(((in_array('Customer Name', $newans)))?'':'display: none;')?>border: 1px solid #eee;padding-left: 10px;<?=((($sqlviewreport['filtername']=='1')||($sqlviewreport['filtername']=='0'))?'':'display:none;')?>"><span style="font-size:13px;color:black;"><?=$infomainaccessusercus['modulename']?> Name</span></td>
													<td class="text-uppercase" style="border: 1px solid #eee;padding-right: 10px;"><span style="font-size:13px;color:black;">Amount</span></td>
													<td class="text-uppercase" style="<?=(((in_array('Payment Term', $newans)))?'':'display: none;')?>border: 1px solid #eee;padding-left: 10px;"><span style="font-size:13px;color:black;">Term</span></td>
												</tr>
											</thead>
											<tbody id="myTables">
											</tbody>
...
const pagechanges = (page, pageSize) => {
	...
	ajaxRequestview = $.ajax({
		type: "GET",
		url: 'reportviewsearch.php?term='+perpages+'&limitings='+($('#limitforpagenum').val())+'&datefrom=<?=$datefrom?>&dateto=<?=$dateto?>&dif=payreceive&names=<?=$_GET['names']?>&timefrom=<?=$timefrom?>&timeto=<?=$timeto?>&payterm=<?=$_GET['payterm']?>&lastornone='+lastornone+'',
...
$(function() {
	...
	function(start, end, label) {
		...
		window.open("reportpayreceive.php?names=<?=$_GET['names']?>&datefrom="+datefrom+"&dateto="+dateto+"&timefrom="+timefromonspots+"&timeto="+timetoonspots+"&payterm=<?=$_GET['payterm']?>","_self");
	});
...
function converHTMLFileToPDF() {
	...
	var finaltimer = setTimeout(function(){
		...
		var header = function(data) {
			...
			var headtext1 = "Payments Receive";
			var fontSize = doc.internal.getFontSize();
			var pageWidth = doc.internal.pageSize.width;
		...
		doc.autoTable({ 
			...
			didDrawPage: header,
		<?php
			if ((in_array('Customer Name', $newans))&&(in_array('Payment Term', $newans))){
		?>
			columnStyles: { 
		...
		<?php
			...
			elseif ((!in_array('Customer Name', $newans))&&(in_array('Payment Term', $newans))){
		?>
			columnStyles: { 
		...
		$.ajax({
			type: "GET",
			url: 'reporthistory.php?types=payreceive&titles=Payments Receive&printdownpdfcsv=Printed PDF&fromto='+fromtopdf+'',
			success: function (result) {
				console.log(result);
			},
			error: function (error) {
...
function downloadaspdf() {
	...
	var finaltimer = setTimeout(function(){
		...
		$.ajax({
			type: "GET",
			url: 'reporthistory.php?types=payreceive&titles=Payments Receive&printdownpdfcsv=Downloaded PDF&fromto='+fromtopdf+'',
			success: function (result) {
				console.log(result);
			},
			error: function (error) {
		...
		doc.save('Payments-Receive-<?=$datefrom?>-To-<?=$dateto?>');
...
function downloadascsv() {
	...
	var finaltimer = setTimeout(function(){
		...
		link.setAttribute("download", "Payments-Receive-Reports-<?=$datefrom?>-To-<?=$dateto?>");
		document.body.appendChild(link);
		link.click();
		$.ajax({
			type: "GET",
			url: 'reporthistory.php?types=payreceive&titles=Payments Receive&printdownpdfcsv=Downloaded CSV&fromto='+fromtopdf+'',
			success: function (result) {
				console.log(result);
			},
			error: function (error) {
...
document.addEventListener('DOMContentLoaded', function() {
	...
	function makeAjaxRequest() {
		ajaxRequest = $.ajax({
			type: "GET",
			url: 'check.php?term=payreceive&datefrom=<?=$datefrom?>&dateto=<?=$dateto?>&limitfrom=0&limitto=10000&names=<?=$sqlviewreport['names']?>&timefrom=<?=$timefrom?>&timeto=<?=$timeto?>&payterm=<?=$sqlviewreport['payterm']?>',
			success: function (result) {
				if (result != mytabprev) {
