				...
				<div class="col-12">
					<div class="card mb-4 mt-5">
						  ...
						  <div class="app-utility-item app-user-dropdown dropdown">
							 ...
							 <div class="dropdown-menu  dropdown-menu-end customdropdown  me-sm-2" aria-labelledby="dropdownMenuButton111" style="max-height: 0px;max-width: 0px;">
								...
								<div class="col-lg-3" style="margin-bottom: 57px !important;"> 
									<input class="form-control" id="salereturnsearch" type="text" placeholder="Search.." style="height:24px;">
								</div>
							</div>
						<?php 
							$sqlismainaccessusers=$con->prepare("SELECT * FROM pairmainaccess WHERE userid=? AND moduletype='Sales Returns' ORDER BY id ASC");
							...
							<div align="right" class=" p-2 addandmenuonlyforimports" style="margin-bottom: -30px;">
								<div class="row" style="width:250px;">           
									<div class="col-8" style="visibility: hidden;">  
										<a href="salesreturnadd.php" class="btn btn-custom btn-sm p-2 add" style="font-size: 13px;height: 24px;margin-bottom:1rem;margin-top: -151px;margin-right:-39px;padding-right: 5px;">
									...
									<div class="col-4">
										...
										?>
											<ul class="dropdown-menu dropdown-menu-end" aria-labelledby="dropdownMenuButton">
												<li>
													<a class="dropdown-item" href="salesreturnimport.php">
						...
						<?php
							...
							$sqlismainaccessusers=$con->prepare("SELECT * FROM pairmainaccess WHERE franchiseid=? AND moduletype='Sales Returns' ORDER BY id ASC");
							$sqlismainaccessusers->bind_param("i", $_SESSION['franchisesession']);
							$sqlismainaccessusers->execute();
							$sqlismainaccessuser = $sqlismainaccessusers->get_result();
							$infomainaccessuser=$sqlismainaccessuser->fetch_array();
							//FOR GET MODULE NAME OF SALESRETURN
...
<div style="visibility: visible;" id="arrowsalltabs">
	...
	<div class="tab-content" id="nav-tabContent" style="position:relative;top: -88px;">
		<div class="tab-pane fade show active p-3" id="nav-general" role="tabpanel" aria-labelledby="nav-general-tab">
							<div class="table-responsive p-0 min-height-480">
								<table id="someTable" class="table table-bordered align-items-center mb-0" style="table-layout: fixed;">
									...
									<?php
										...
										$sqls=$con->prepare("SELECT id,salesreturndate, salesreturnno, customername, salesreturnterm, duedate, salesreturnamount, cancelstatus FROM pairsalesreturns WHERE franchisesession=? AND createdid=? ".$typesforlisting." GROUP BY salesreturndate, salesreturnno ORDER BY salesreturndate DESC, salesreturntime DESC,ordering DESC LIMIT ".(($access['salereturnpageload']=='pagenum')?'10':'15')."");
										...
										while($info=$sql->fetch_array()){
											...
											$sqlsalespays->bind_param("ssss", $_SESSION['franchisesession'], $companymainid, $info['salesreturnno'], $info['salesreturndate']);
									...
									?>
										<?php
											if ((in_array('Date', $modulecolumns))) {
										?>
											<td onclick="window.open('salesreturnview.php?id=<?=$info['id']?>&salesreturnno=<?=$info['salesreturnno']?>&salesreturndate=<?=$info['salesreturndate']?>', '_self')"  data-label="Date">
												<?=(($info['salesreturndate']!='')?(date($date,strtotime($info['salesreturndate']))):'&nbsp;')?>
											</td>
										<?php
											}
											if ((in_array('No', $modulecolumns))) {
										?>
											<td onclick="window.open('salesreturnview.php?id=<?=$info['id']?>&salesreturnno=<?=$info['salesreturnno']?>&salesreturndate=<?=$info['salesreturndate']?>', '_self')"  data-label="Number">
												<?=(($info['salesreturnno']=='')?'&nbsp;':'')?><?=$info['salesreturnno']?>
											</td>
										<?php
											}
											// if ((in_array('Customer Name', $modulecolumns))) {
										?>
											<td onclick="window.open('salesreturnview.php?id=<?=$info['id']?>&salesreturnno=<?=$info['salesreturnno']?>&salesreturndate=<?=$info['salesreturndate']?>', '_self')"  data-label="Name">
										...
										?>
											...
											<td onclick="window.open('salesreturnview.php?id=<?=$info['id']?>&salesreturnno=<?=$info['salesreturnno']?>&salesreturndate=<?=$info['salesreturndate']?>', '_self')"  data-label="Amount">
												<i class="fa fa-rupee"></i> <?=number_format((float)$info['salesreturnamount'],2,'.','')?>
											</td>
										<?php
											}
											if ((in_array('Status', $modulecolumns))) {
												if($info['cancelstatus']=='1'){
										?>
											<td onclick="window.open('salesreturnview.php?id=<?=$info['id']?>&salesreturnno=<?=$info['salesreturnno']?>&salesreturndate=<?=$info['salesreturndate']?>', '_self')"  data-label="Status" style="color:#bbbbbb;text-decoration: none;">
												Void
											</td>
										<?php
												}
										...
										?>
											<td onclick="window.open('salesreturnview.php?id=<?=$info['id']?>&salesreturnno=<?=$info['salesreturnno']?>&salesreturndate=<?=$info['salesreturndate']?>', '_self')"  data-label="Status" class="text-success" style="text-decoration: none;">
												Paid
											</td>
										<?php
													}
										...
										?>
											<td onclick="window.open('salesreturnview.php?id=<?=$info['id']?>&salesreturnno=<?=$info['salesreturnno']?>&salesreturndate=<?=$info['salesreturndate']?>', '_self')"  data-label="Status" class="text-danger" style="text-decoration: none;">
												Pending
											</td>
										<?php
														}
										...
										?>
											<td onclick="window.open('salesreturnview.php?id=<?=$info['id']?>&salesreturnno=<?=$info['salesreturnno']?>&salesreturndate=<?=$info['salesreturndate']?>', '_self')"  data-label="Status" class="text-warning" style="text-decoration: none;">
												Partially Paid
											</td>
										<?php
														}
											...
											if ((in_array('Balance', $modulecolumns))) {
										?>
											<td onclick="window.open('salesreturnview.php?id=<?=$info['id']?>&salesreturnno=<?=$info['salesreturnno']?>&salesreturndate=<?=$info['salesreturndate']?>', '_self')"  data-label="Balance">
										...
										?>
											<td data-label="Edit">
												<a href="salesreturnedit.php?salesreturnno=<?=$info['salesreturnno']?>&salesreturndate=<?=$info['salesreturndate']?>" class="text-secondary font-weight-bold text-xs">
									...
									<?php
										...
										$sqltotsalereturns=$con->prepare("SELECT COUNT(DISTINCT salesreturnno) FROM pairsalesreturns WHERE franchisesession=? AND createdid=? ".$typesforlisting."");
									$sqltotsalereturns->bind_param("ss", $_SESSION['franchisesession'], $companymainid);
									$sqltotsalereturns->execute();
									$sqltotsalereturn = $sqltotsalereturns->get_result();
										$sqlfetlist=$sqltotsalereturn->fetch_array();
										if ($sqlfetlist[0]==0) {
											$pageinitnum = 0;
										}
										if ($sqlfetlist[0]!=0) {
							...
							<?php
								if ($access['salereturnpageload']=='pageauto') {
							?>
								<script type="text/javascript">
								...
								$('.main-content').on('scroll', function() {
									...
									if (scrollTop + $(this).innerHeight() >= this.scrollHeight-50) {
										if (isPreviousEventComplete && isDataAvailable) {
											...
											$.ajax({
												type: "GET",
												url: 'listsalereturnsearch.php?term=' + sIndex + '&limitings=15&typesforlisting=<?=urlencode($typesforlisting)?>',
							...
							<?php
								...
								if ($access['salereturnpageload']=='pagenum') {
							?>
								<br>
					 ...
					 try{
								...
								const pagechanges = (page, pageSize) => {
									...
									$.ajax({
										type: "GET",
										url: 'listsalereturnsearch.php?term='+perpages+'&limitings='+($('#limitforpagenum').val())+'&typesforlisting=<?=urlencode($typesforlisting)?>',
										success: function (result) {
		...
		<div class="modal fade" id="deleteconfirm-adddelete" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
			<div class="modal-dialog">
				<div class="modal-content">
					...
					<div class="modal-body">Are you sure you want to Cancel this Sales Return?</div>
...
function submitthesorting(value) {
  $.ajax({
	 type: "GET",
	 url: 'listingsortfilters.php?moduletype=Sales Returns&typesforlisting='+value+'',
	 success: function (result) {
		console.log(result);
		window.location = 'salesreturns.php?remarks='+result+'';
	 },
	 error: function (error) {
		console.log(error);
	 }
