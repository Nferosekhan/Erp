...
if (isset($_POST['submitcustomizes'])) {
	...
	$sqlreportmoduless=$con->prepare("SELECT * FROM pairreportmodules WHERE types='stockandsalesnew' ORDER BY id ASC");
	...
	foreach ($newmodules as $newmoduleskey) {
		...
		$modcolumncol=$coltypemod."stockandsalesnew";
	...
	$sqlupreport = $con->prepare("UPDATE pairreports SET rowcolumns=? WHERE franchiseid=? AND createdid=? AND types='stockandsalesnew'");
	...
	$sqlreportfav = $con->prepare("UPDATE pairreportfavourites SET reporturl=? WHERE franchisesession=? AND createdid=? AND reportnames='stockandsalesnew'");
	$sqlreportfav->bind_param("sss", $newReportUrl, $_SESSION['franchisesession'], $companymainid);
	$sqlreportfav->execute();
	$sqlreportfav->close();

	$sqlreport = $con->prepare("UPDATE pairreports SET category=?,categoryname=?,reportperiod=?,filtername=?,companyname=?,dateprepare=?,timeprepare=?,gstrule=? WHERE franchiseid=? AND createdid=? AND types='stockandsalesnew'");
	$sqlreport->bind_param("ssssssssss", $category, $categoryname, $reportperiod, $filtername, $companyname, $dateprepare, $timeprepare, $gstrule, $_SESSION['franchisesession'], $companymainid);
	$sqlreport->execute();
	$sqlreport->close();

	header("Location:reportstockandsalesnew.php?datefrom=".date($dateformatforurlredirection,strtotime($from))."&dateto=".date($dateformatforurlredirection,strtotime($to))."&category=".$category."");
}

$sqlreportviews=$con->prepare("SELECT * FROM pairreports WHERE franchiseid=? AND createdid=? AND types='stockandsalesnew'");
								...
								<form action="" method="post" enctype="multipart/form-data" class="form-horizontal mt-0" role="form">
									<div class="row">
																			...
																			<div id="customizerowcolumns" class="accordion-collapse collapse show" aria-labelledby="customizerowcolumn">
																				<div class="accordion-body text-sm">
																					<div class="row justify-content-center">
																						...
																						<?php
																							$newans=array();
																							$sqlreportaccesss=$con->prepare("SELECT * FROM pairreports WHERE franchiseid=? AND createdid=? AND types='stockandsalesnew'");
																								...
																								<div class="col-lg-6 my-1" style="padding-left: 0px !important;">
																									<div class="custom-control custom-checkbox mr-sm-2" style="padding: 0px 12px !important;">
																										<div class="input-group input-group-sm" id="flaghead">
																											...
																											<input type="checkbox" class="custom-control-input" name="<?= $coltypemod; ?>stockandsalesnew" id="<?= $coltypemod; ?>stockandsalesnew" checked disabled>
																											<label class="custom-control-label custom-label" for="<?= $coltypemod; ?>stockandsalesnew">
											...
											<div class="tab-pane fade" id="nav-profile" role="tabpanel" aria-labelledby="nav-profile-tab">
												<div class="table-responsive m-3" id="histables">
													<table class="table table-bordered">
														...
														<tbody>
														<?php
															$sqluse=$con->prepare("SELECT * FROM pairusehistory WHERE usetype = 'REPstockandsalesnew' AND useid = ? ORDER BY createdon DESC");
																				...
																				<table id="print-are1" style="border: 1px solid #eee;" width="100%">
																					<thead>
																						<tr>
																							...
																							<td class="text-uppercase" style="width:5%;border:1px solid #eee;padding-left: 3px;">
																								<span style="font-size:13px;color:black;">
																									<?=(new DateTime($_GET['datefrom']))->modify('-2 months')->format('M')?>
																								</span>
																							</td>
																							<td class="text-uppercase" style="width:5%;border:1px solid #eee;padding-left: 3px;">
																								<span style="font-size:13px;color:black;">
																									<?=(new DateTime($_GET['datefrom']))->modify('-1 months')->format('M')?>
																								</span>
																							</td>
																							<td class="text-uppercase" style="width:5%;border:1px solid #eee;padding-left: 3px;">
																								<span style="font-size:13px;color:black;">
																					...
																					<?php
																						$count = 1;
																						$seldatas = $con->prepare("SELECT proid, proname, SUM(salequantity) AS salequantity, SUM(purchasequantity) AS purchasequantity, SUM(adjustquantity) AS adjustquantity, SUM(crnotequantity) AS crnotequantity, SUM(drnotequantity) AS drnotequantity, MAX(noofpacks) AS noofpacks, SUM(curstockval) AS curstockval, MAX(bill_noofpacks) AS bill_noofpacks, MAX(crnote_noofpacks) AS crnote_noofpacks, MAX(drnote_noofpacks) AS drnote_noofpacks, SUM(stock) AS stock FROM (SELECT pro.id AS proid, pro.productname AS proname, SUM(inv.quantity) AS salequantity,0 AS crnotequantity,0 AS drnotequantity,0 AS crnote_noofpacks,0 AS drnote_noofpacks, 0 AS purchasequantity,0 AS curstockval, 0 AS adjustquantity, inv.noofpacks, 0 AS bill_noofpacks, 0 AS stock FROM pairproducts pro JOIN pairinvoices inv ON inv.productid = pro.id WHERE inv.franchisesession = ? AND inv.createdid = ? AND (inv.invoicedate >= ? AND inv.invoicedate <= ?) AND inv.cancelstatus = '0' GROUP BY pro.id UNION SELECT pro.id AS proid, pro.productname AS proname, 0 AS salequantity,0 AS crnotequantity,0 AS drnotequantity,0 AS crnote_noofpacks,0 AS drnote_noofpacks, SUM(bill.quantity) AS purchasequantity,SUM(bill.productvalue) AS curstockval, 0 AS adjustquantity, 0 AS noofpacks, bill.noofpacks, 0 AS stock FROM pairproducts pro JOIN pairbills bill ON bill.productid = pro.id WHERE bill.franchisesession = ? AND bill.createdid = ? AND (bill.billdate >= ? AND bill.billdate <= ?)AND bill.cancelstatus = '0' GROUP BY pro.id UNION SELECT pro.id AS proid, pro.productname AS proname, 0 AS salequantity,0 AS crnotequantity,0 AS drnotequantity,0 AS crnote_noofpacks,0 AS drnote_noofpacks, 0 AS purchasequantity,0 AS curstockval, SUM(adjust.newquantity) AS adjustquantity, 0 AS noofpacks, 0 AS bill_noofpacks, 0 AS stock FROM pairproducts pro JOIN pairadjustments adjust ON adjust.productid = pro.id WHERE adjust.franchisesession = ? AND adjust.createdid = ? AND (adjust.adjustmentdate >= ? AND adjust.adjustmentdate <= ?)AND adjust.cancelstatus = '0' GROUP BY pro.id UNION SELECT pro.id AS proid, pro.productname AS proname, 0 AS salequantity,0 AS crnotequantity,0 AS drnotequantity,0 AS drnote_noofpacks,0 AS crnote_noofpacks, 0 AS purchasequantity,0 AS curstockval, 0 AS adjustquantity, 0 AS noofpacks, 0 AS bill_noofpacks, SUM(bat.quantity) AS stock FROM pairproducts pro JOIN pairbatch bat ON bat.productid = pro.id WHERE bat.franchisesession = ? AND bat.createdid = ? GROUP BY pro.id UNION SELECT pro.id AS proid, pro.productname AS proname,  0 AS salequantity, 0 AS drnotequantity, SUM(crnote.quantity) AS crnotequantity, 0 AS purchasequantity,0 AS curstockval, 0 AS adjustquantity, crnote.noofpacks, 0 AS drnote_noofpacks,0 AS noofpacks,0 AS bill_noofpacks, 0 AS stock FROM pairproducts pro JOIN paircreditnotes crnote ON crnote.productid = pro.id WHERE crnote.franchisesession = ? AND crnote.createdid = ? AND (crnote.creditnotedate >= ? AND crnote.creditnotedate <= ?) AND crnote.cancelstatus = '0' GROUP BY pro.id UNION SELECT pro.id AS proid, pro.productname AS proname,  0 AS salequantity, 0 AS drnotequantity, SUM(drnote.quantity) AS drnotequantity, 0 AS purchasequantity,0 AS curstockval, 0 AS adjustquantity, drnote.noofpacks, 0 AS crnote_noofpacks,0 AS noofpacks,0 AS bill_noofpacks, 0 AS stock FROM pairproducts pro JOIN pairdebitnotes drnote ON drnote.productid = pro.id WHERE drnote.franchisesession = ? AND drnote.createdid = ? AND (drnote.debitnotedate >= ? AND drnote.debitnotedate <= ?) AND drnote.cancelstatus = '0' GROUP BY pro.id) AS combined GROUP BY proid, proname ORDER BY proname ASC LIMIT 35");
																						$seldatas->bind_param("ssssssssssssssssssssss", $_SESSION['franchisesession'], $companymainid, $datefrom, $dateto, $_SESSION['franchisesession'], $companymainid, $datefrom, $dateto, $_SESSION['franchisesession'], $companymainid, $datefrom, $dateto, $_SESSION['franchisesession'], $companymainid, $_SESSION['franchisesession'], $companymainid, $datefrom, $dateto, $_SESSION['franchisesession'], $companymainid, $datefrom, $dateto);
																						$seldatas->execute();
																						$seldata = $seldatas->get_result();
																						while($info=$seldata->fetch_array()){
																					?>
																						<tr style="vertical-align: middle; height: 30px !important;">
																							<td style="text-align: left !important; font-size:12px; color:black; border:1px solid #eee; padding-left: 3px;">
																								<?= $count ?>
																							</td>
																							<td style="text-align: left !important; font-size:12px; color:black; border:1px solid #eee; padding-left: 3px;">
																								<?= $info['proname'] ?>
																							...
																							<td style="text-align: left !important; font-size:12px; color:black; border:1px solid #eee; padding-left: 3px;">
																								<?= number_format($info['curstockval'],2,'.',',') ?>
																							</td>
																						</tr>
																					<?php
																							$count++;
																						...
																						if(mysqli_num_rows($seldata)<35){
																							echo '<tr style="vertical-align: middle; height: '.((35-mysqli_num_rows($seldata))*30).'px !important;">  <td style="text-align: left !important; font-size:12px; color:black; border:1px solid #eee; padding-left: 10px;"></td>  <td style="text-align: left !important; font-size:12px; color:black; border:1px solid #eee; padding-left: 10px;"></td>  <td style="text-align: left !important; font-size:12px; color:black; border:1px solid #eee; padding-left: 10px;"></td>  <td style="text-align: left !important; font-size:12px; color:black; border:1px solid #eee; padding-left: 10px;"></td>  <td style="text-align: right !important; font-size:12px; color:black; border:1px solid #eee; padding-right: 10px;"></td>  <td style="text-align: right !important; font-size:12px; color:black; border:1px solid #eee; padding-right: 10px;"></td>  <td style="text-align: right !important; font-size:12px; color:black; border:1px solid #eee; padding-right: 10px;"></td>  <td style="text-align: right !important; font-size:12px; color:black; border:1px solid #eee; padding-right: 10px;"></td>  <td style="text-align: right !important; font-size:12px; color:black; border:1px solid #eee; padding-right: 10px;"></td>  <td style="text-align: right !important; font-size:12px; color:black; border:1px solid #eee; padding-right: 10px;"></td>  <td style="text-align: right !important; font-size:12px; color:black; border:1px solid #eee; padding-right: 10px;"></td>  <td style="text-align: right !important; font-size:12px; color:black; border:1px solid #eee; padding-right: 10px;"></td> </tr>';
																						}
																						$sqltotlistpages=$con->prepare("SELECT * FROM pairinvoices WHERE franchisesession=? AND createdid=? AND (invoicedate>=? AND invoicedate<=?) AND cancelstatus='0' ".(($sqlviewreport['category']=='all')?' ':' and manufacturer="'.$sqlviewreport['categoryname'].'"')." ORDER BY productname ASC");
										...
										<table id="print-are3" style="border: 1px solid #eee;display: none;" width="100%">
											<thead>
												<tr>
													<td class="text-uppercase" style="width:10%;border:1px solid #eee;padding-left: 10px;"><span style="font-size:13px;color:black;"><?=$infomainaccessuserinvoice['modulename']?> Number</span></td>
													<td class="text-uppercase" style="width:10%;border:1px solid #eee;padding-left: 10px;"><span style="font-size:13px;color:black;"><?=$infomainaccessuserinvoice['modulename']?> Date</span></td>
													<td class="text-uppercase" style="width:13%;border:1px solid #eee;padding-left: 10px;"><span style="font-size:13px;color:black;">Customer Name</span></td>
													<td class="text-uppercase" style="width:10%;border:1px solid #eee;padding-left: 10px;"><span style="font-size:13px;color:black;">Place</span></td>
													<td class="text-uppercase" style="width:6%;border:1px solid #eee;padding-left: 10px;"><span style="font-size:13px;color:black;"><?=$access['txtqtyinv']?></span></td>
													<td class="text-uppercase" style="width:6%;border:1px solid #eee;padding-left: 10px;"><span style="font-size:13px;color:black;">Free</span></td>
													<td class="text-uppercase" style="text-align: right !important;width: 10%;border:1px solid #eee;padding-right: 10px;"><span style="font-size:13px;color:black;white-space: nowrap;overflow: hidden;">Rate</span></td>
													<td class="text-uppercase" style="text-align: right !important;width: 10%;border:1px solid #eee;padding-right: 10px;"><span style="font-size:13px;color:black;white-space: nowrap;overflow: hidden;">Value</span></td>
												</tr>
											</thead>
											<tbody id="myTables">
											</tbody>
...
const pagechanges = (page, pageSize) => {
	...
	ajaxRequestview = $.ajax({
		type: "GET",
		url: 'reportviewsearch.php?term='+perpages+'&limitings='+($('#limitforpagenum').val())+'&datefrom=<?=$datefrom?>&dateto=<?=$dateto?>&dif=stockandsalesnew&category=<?=$_GET['category']?>&lastornone='+lastornone+'',
...
$(function() {
	...
	function(start, end, label) {
		...
		window.open("reportstockandsalesnew.php?datefrom="+datefrom+"&dateto="+dateto+"&category=<?=$_GET['category']?>","_self");
	});
});
...
function converHTMLFileToPDF() {
	...
	var finaltimer = setTimeout(function(){
		...
		var header = function(data) {
			...
			var headtext1 = "Stock And Sales Statement New";
			var fontSize = doc.internal.getFontSize();
			var pageWidth = doc.internal.pageSize.width;
		...
		doc.autoTable({
			...
			didDrawPage: header, 
			columnStyles: { 
				0: { halign: 'left', cellWidth: 63},
				1: { halign: 'left', cellWidth: 63},
				2: { halign: 'left', cellWidth: 63},
				3: { halign: 'left', cellWidth: 63},
				4: { halign: 'right', cellWidth: 63},
				5: { halign: 'right', cellWidth: 63},
				6: { halign: 'right', cellWidth: 63},
				7: { halign: 'right', cellWidth: 63}
			}
		});
		obj = '<object id="pdfObjObj" data="'+doc.output('bloburi')+'" type="application/pdf" width="90%" height="550"></object>';
		$.ajax({
			type: "GET",
			url: 'reporthistory.php?types=stockandsalesnew&titles=Stock And Sales Statement New&printdownpdfcsv=Printed PDF&fromto='+fromtopdf+'',
			success: function (result) {
				console.log(result);
			},
			error: function (error) {
...
function downloadaspdf() {
	...
	var finaltimer = setTimeout(function(){
		...
		$.ajax({
			type: "GET",
			url: 'reporthistory.php?types=stockandsalesnew&titles=Stock And Sales Statement New&printdownpdfcsv=Downloaded PDF&fromto='+fromtopdf+'',
			success: function (result) {
				console.log(result);
			},
			error: function (error) {
		...
		doc.save('Stock-And-Sales-Statement-New-<?=$datefrom?>-To-<?=$dateto?>');
...
function downloadascsv() {
	...
	var finaltimer = setTimeout(function(){
		...
		data.push(["Stock And Sales Statement New"]);
		...
		link.setAttribute("download", "Stock-And-Sales-Statement-New-<?=$datefrom?>-To-<?=$dateto?>");
		document.body.appendChild(link);
		link.click();
		$.ajax({
			type: "GET",
			url: 'reporthistory.php?types=stockandsalesnew&titles=Stock And Sales Statement New&printdownpdfcsv=Downloaded CSV&fromto='+fromtopdf+'',
			success: function (result) {
				console.log(result);
			},
			error: function (error) {
...
document.addEventListener('DOMContentLoaded', function() {
	...
	function makeAjaxRequest() {
		ajaxRequest = $.ajax({
			type: "GET",
			url: 'check.php?term=stockandsalesnew&datefrom=<?=$datefrom?>&dateto=<?=$dateto?>&limitfrom=0&limitto=10000&category=<?=$_GET['category']?>',
			success: function (result) {
				if (result != mytabprev) {
