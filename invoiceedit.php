...
if((isset($_GET['invoiceno']))&&(isset($_GET['invoicedate']))){
	...
	if($sql->num_rows>0){
		...
		if(isset($_POST['grandtotal'])){
			...
			$oldinvoiceno=htmlspecialchars( $_GET['invoiceno'], ENT_QUOTES, 'UTF-8');
			$oldinvoicedate=htmlspecialchars( $_GET['invoicedate'], ENT_QUOTES, 'UTF-8');
			...
			$publicsqlforupdhis=$con->prepare("SELECT id FROM pairsalespayments WHERE receiptno=? AND receiptdate=?");
			$publicsqlforupdhis->bind_param("ss", $invoiceno, $invoicedate);
			...
			for($i=0; $i<count($_POST['oldproductname']); $i++){
				...
				if($productname!=''){
					$sql = $con->prepare("DELETE FROM pairinvoices WHERE createdid=? AND franchisesession=? AND invoiceno=? AND invoicedate=?");
					$sql->bind_param("ssss", $companymainid, $_SESSION["franchisesession"], $oldinvoiceno, $oldinvoicedate);
					$sql->execute();
					$sqldelledger = $con->prepare("DELETE FROM pairledgers WHERE createdid=? AND franchisesession=? AND ledgerno=? AND ledgerdate=? AND (type='invoice' OR type='invoice payment')");
					$sqldelledger->bind_param("ssss", $companymainid, $_SESSION["franchisesession"], $oldinvoiceno, $oldinvoicedate);
					$sqldelledger->execute();
					$sqls = $con->prepare("DELETE FROM pairsalespayments WHERE receiptno=? AND receiptdate=?");
					$sqls->bind_param("ss", $oldinvoiceno, $oldinvoicedate);
					$sqls->execute();
					$sqls->close();
					$sqlsant = $con->prepare("DELETE FROM pairsalespayhistory WHERE invoiceno=? AND invoicedate=? AND type='invoice'");
					$sqlsant->bind_param("ss", $oldinvoiceno, $oldinvoicedate);
					$sqlsant->execute();
					$sqlsant->close();
					//  margins     
			...
			$sql2->bind_param("iiss", $_SESSION['franchisesession'], $companymainid, $oldinvoiceno, $oldinvoicedate);
			$sql2->execute();
			$sql2->store_result();
			...
			if($sql2->num_rows==0){
					...
					if ($validpaidamount!=0) {
						...
						if ($sqlselects->num_rows==0) {
							...
							if($sqlsalepayins){
								...
								$oldesinvoicedate = htmlspecialchars( $_POST['oldesinvoicedate'], ENT_QUOTES, 'UTF-8');
								$oldescustomername = htmlspecialchars( $_POST['oldescustomername'], ENT_QUOTES, 'UTF-8');
								$oldesinvoiceterm = htmlspecialchars( $_POST['oldesinvoiceterm'], ENT_QUOTES, 'UTF-8');
								...
								if($invoicedate!=$oldesinvoicedate){
									if($chforpayments!=''){
										$chforpayments.='<br> Date <span style="color:green;" id="prohisfromtospan">( From '.date($datemainphp,strtotime($invoicedate)).' To '.date($datemainphp,strtotime($oldinvoicedate)).' ) </span>';
									}
									else{
										$chforpayments.=' Date <span style="color:green;" id="prohisfromtospan">( From '.date($datemainphp,strtotime($invoicedate)).' To '.date($datemainphp,strtotime($oldinvoicedate)).' ) </span>';
									}					
								}
								if($invoiceno!=$oldesinvoiceno){
									if($chforpayments!=''){
										$chforpayments.='<br> Reference Number <span style="color:green;" id="prohisfromtospan">( From '.$oldinvoiceno.' To '.$invoiceno.' ) </span>';
									}
									else{
										$chforpayments.='Reference Number <span style="color:green;" id="prohisfromtospan">( From '.$oldinvoiceno.' To '.$invoiceno.' ) </span>';
									}					
								}
								if($invoiceterm!=$oldesinvoiceterm){
									if($chforpayments!=''){
										$chforpayments.='<br> Mode of Payment <span style="color:green;" id="prohisfromtospan">( From '.$oldesinvoiceterm.' To '.$invoiceterm.' ) </span>';
									}
									else{
										$chforpayments.='Mode of Payment <span style="color:green;" id="prohisfromtospan">( From '.$oldesinvoiceterm.' To '.$invoiceterm.' ) </span>';
									}					
								}
						...
						if ($invarr[$termrun]>0) {
							...
							if ($sqlselects->num_rows==0) {
								...
								if($sqlsalepayins){
									...
									if($invarr[$termrun]!=$oldesinvoiceterm){
										if($chforpayments!=''){
											$chforpayments.='<br> Mode of Payment <span style="color:green;" id="prohisfromtospan">( From '.$oldesinvoiceterm.' To '.$invarr[$termrun].' ) </span>';
										}
										else{
											$chforpayments.='Mode of Payment <span style="color:green;" id="prohisfromtospan">( From '.$oldesinvoiceterm.' To '.$invarr[$termrun].' ) </span>';
										}					
									}
									if(explode(',', $invoiceterm)[$termrun]!=$oldesvalidpaidamount){
										if($chforpayments!=''){
											$chforpayments.='<br> Amount Received <span style="color:green;" id="prohisfromtospan">( From '.$oldesvalidpaidamount.' To '.explode(',', $invoiceterm)[$termrun].' ) </span>';
										}
										else{
											$chforpayments.='Amount Received <span style="color:green;" id="prohisfromtospan">( From '.$oldesvalidpaidamount.' To '.explode(',', $invoiceterm)[$termrun].' ) </span>';
										}					
									}
									if(explode(',', $invoiceterm)[$termrun]!=$oldesvalidpaidamount){
										if($invoicedate!=''){
										...
										if($chforpayments!=''){
											$chforpayments.='<br> Payment <span style="color:green;" id="prohisfromtospan">( From '.$oldesvalidpaidamount.' To '.explode(',', $invoiceterm)[$termrun].' ) </span>';
										}
										else{
											$chforpayments.='Payment <span style="color:green;" id="prohisfromtospan">( From '.$oldesvalidpaidamount.' To '.explode(',', $invoiceterm)[$termrun].' ) </span>';
										}					
									}
									if(explode(',', $invoiceterm)[$termrun]!=$oldesvalidpaidamount){
										if($chforpayments!=''){
											$chforpayments.='<br> Total <span style="color:green;" id="prohisfromtospan">( From '.$oldesvalidpaidamount.' To '.explode(',', $invoiceterm)[$termrun].' ) </span>';
										}
										else{
											$chforpayments.='Total <span style="color:green;" id="prohisfromtospan">( From '.$oldesvalidpaidamount.' To '.explode(',', $invoiceterm)[$termrun].' ) </span>';
										}					
									}
									if($chforpayments!=''){
										$sqlusepayments = $con->prepare("INSERT INTO pairusehistory (usetype,createdon,createdby,useid,useremarks) VALUES ('SALESPAY', ?, ?, ?, ?)");
				...
				elseif($invoiceterm=='CREDIT'){
					...
					$sqlsant->execute();
					$sqlsant->close();
					$sqler = $con->prepare("UPDATE pairinvoices SET createdid=?, createdby=?, franchisesession=?, createdon=?, paidstatus='0' WHERE invoiceno=? AND invoicedate=? AND customerid=? AND franchisesession=? AND createdid=?");
	    			$sqler->bind_param("sssssssss", $companymainid, $_SESSION["unqwerty"], $_SESSION["franchisesession"], $createdon, $invoiceno, $invoicedate, $customerid, $_SESSION['franchisesession'], $companymainid);
	    			$sqler->execute();
					$sqler->close();
					//FOR UPDATE THE PAYMENT STATUS UNPAID
				...
				if($sql){
					...
					if(isset($_POST['submit1'])){
						...
						$sqlusefinal = $con->prepare("INSERT INTO pairusehistory (usetype,createdon,createdby,useid,uniqueid,useremarks) VALUES ('INVOICES', ?, ?, ?, ?, '<span style=\"color:green;\" id=\"prohisfromtospan\">Updated Successfully</span>')");
			    		$sqlusefinal->bind_param("ssss", $times, $_SESSION["unqwerty"], $invoiceno, $salesid);
			    		$sqlusefinal->execute();
			    		$sqlusefinal->close();
			    		if ($access['invautosave']=='1') {
			    			echo '<script> window.location.href="invoiceedit.php?id='.$salesid.'&invoiceno='.$invoiceno.'&invoicedate='.$invoicedate.'&remarks=Updated Successfully";</script>';
			    		}
			    		else{
			...
			if ($sql) {
				...
				if($invoiceno!=$oldesinvoiceno){
					if($invinfoch!=''){
						$invinfoch.='<br> '.$infomainaccessuser['modulename'].' Number <span style="color:green;" id="prohisfromtospan">( From '.$oldesinvoiceno.' To '.$invoiceno.' ) </span>';
					}
					else{
						$invinfoch.='<span style="color:royalblue;">'.$infomainaccessuser['modulename'].' Information</span> <br> '.$infomainaccessuser['modulename'].' Number <span style="color:green;" id="prohisfromtospan">( From '.$oldesinvoiceno.' To '.$invoiceno.' ) </span>';
					}                   
				}
				if($invoicedate!=$oldesinvoicedate){
					if($invinfoch!=''){
						$invinfoch.='<br> '.$infomainaccessuser['modulename'].' Date <span style="color:green;" id="prohisfromtospan">( From '.date($date,strtotime($oldesinvoicedate)).' To '.date($date,strtotime($invoicedate)).' ) </span>';
					}
					else{
						$invinfoch.='<span style="color:royalblue;">'.$infomainaccessuser['modulename'].' Information</span> <br> '.$infomainaccessuser['modulename'].' Date <span style="color:green;" id="prohisfromtospan">( From '.date($date,strtotime($oldesinvoicedate)).' To '.date($date,strtotime($invoicedate)).' ) </span>';
					}                   
				}
				if($reference!=$oldesreference){
					if($invinfoch!=''){
						$invinfoch.='<br> Reference <span style="color:green;" id="prohisfromtospan">( From '.$oldesreference.' To '.$reference.' ) </span>';
					}
					else{
						$invinfoch.='<span style="color:royalblue;">'.$infomainaccessuser['modulename'].' Information</span> <br> Reference <span style="color:green;" id="prohisfromtospan">( From '.$oldesreference.' To '.$reference.' ) </span>';
					}                   
				}
				if($saleperson!=$oldessaleperson){
					if($invinfoch!=''){
						$invinfoch.='<br> Sale Person <span style="color:green;" id="prohisfromtospan">( From '.$oldessaleperson.' To '.$saleperson.' ) </span>';
					}
					else{
						$invinfoch.='<span style="color:royalblue;">'.$infomainaccessuser['modulename'].' Information</span> <br> Sale Person <span style="color:green;" id="prohisfromtospan">( From '.$oldessaleperson.' To '.$saleperson.' ) </span>';
					}                   
				}
				if($preparedby!=$oldespreparedby){
					if($invinfoch!=''){
						$invinfoch.='<br> Prepared By <span style="color:green;" id="prohisfromtospan">( From '.$oldespreparedby.' To '.$preparedby.' ) </span>';
					}
					else{
						$invinfoch.='<span style="color:royalblue;">'.$infomainaccessuser['modulename'].' Information</span> <br> Prepared By <span style="color:green;" id="prohisfromtospan">( From '.$oldespreparedby.' To '.$preparedby.' ) </span>';
					}                   
				}
				if($checkedby!=$oldescheckedby){
					if($invinfoch!=''){
						$invinfoch.='<br> Checked By <span style="color:green;" id="prohisfromtospan">( From '.$oldescheckedby.' To '.$checkedby.' ) </span>';
					}
					else{
						$invinfoch.='<span style="color:royalblue;">'.$infomainaccessuser['modulename'].' Information</span> <br> Checked By <span style="color:green;" id="prohisfromtospan">( From '.$oldescheckedby.' To '.$checkedby.' ) </span>';
					}                   
				}
				...
				if($customername!=$oldescustomername){
					if($invcustinfoch!=''){
						$invcustinfoch.='<br> '.$infomainaccessusercus['modulename'].' Name <span style="color:green;" id="prohisfromtospan">( From '.$oldescustomername.' To '.$customername.' ) </span>';
					}
					else{
						$invcustinfoch.=''.(($invinfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessusercus['modulename'].' Information</span> <br> '.$infomainaccessusercus['modulename'].' Name <span style="color:green;" id="prohisfromtospan">( From '.$oldescustomername.' To '.$customername.' ) </span>';
					}                   
				}
				if($twoage!=$oldesage){
					if($invcustinfoch!=''){
						$invcustinfoch.='<br> AGE <span style="color:green;" id="prohisfromtospan">( From '.$oldesage.' To '.$twoage.' ) </span>';
					}
					else{
						$invcustinfoch.=''.(($invinfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessusercus['modulename'].' Information</span> <br> AGE <span style="color:green;" id="prohisfromtospan">( From '.$oldesage.' To '.$twoage.' ) </span>';
					}                   
				}
				if($twosex!=$oldessex){
					if($invcustinfoch!=''){
						$invcustinfoch.='<br> SEX <span style="color:green;" id="prohisfromtospan">( From '.$oldessex.' To '.$twosex.' ) </span>';
					}
					else{
						$invcustinfoch.=''.(($invinfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessusercus['modulename'].' Information</span> <br> SEX <span style="color:green;" id="prohisfromtospan">( From '.$oldessex.' To '.$twosex.' ) </span>';
					}                   
				}
				if($area!=$oldesarea){
					if($invcustinfoch!=''){
						$invcustinfoch.='<br> BILLING STREET <span style="color:green;" id="prohisfromtospan">( From '.$oldesarea.' To '.$area.' ) </span>';
					}
					else{
						$invcustinfoch.=''.(($invinfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessusercus['modulename'].' Information</span> <br> BILLING STREET <span style="color:green;" id="prohisfromtospan">( From '.$oldesarea.' To '.$area.' ) </span>';
					}                   
				}
				if($city!=$oldescity){
					if($invcustinfoch!=''){
						$invcustinfoch.='<br> BILLING CITY/TOWN <span style="color:green;" id="prohisfromtospan">( From '.$oldescity.' To '.$city.' ) </span>';
					}
					else{
						$invcustinfoch.=''.(($invinfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessusercus['modulename'].' Information</span> <br> BILLING CITY/TOWN <span style="color:green;" id="prohisfromtospan">( From '.$oldescity.' To '.$city.' ) </span>';
					}                   
				}
				if($state!=$oldesstate){
					if($invcustinfoch!=''){
						$invcustinfoch.='<br> BILLING STATE <span style="color:green;" id="prohisfromtospan">( From '.$oldesstate.' To '.$state.' ) </span>';
					}
					else{
						$invcustinfoch.=''.(($invinfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessusercus['modulename'].' Information</span> <br> BILLING STATE <span style="color:green;" id="prohisfromtospan">( From '.$oldesstate.' To '.$state.' ) </span>';
					}                   
				}
				if($district!=$oldesdistrict){
					if($invcustinfoch!=''){
						$invcustinfoch.='<br> BILLING PIN <span style="color:green;" id="prohisfromtospan">( From '.$oldesdistrict.' To '.$district.' ) </span>';
					}
					else{
						$invcustinfoch.=''.(($invinfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessusercus['modulename'].' Information</span> <br> BILLING PIN <span style="color:green;" id="prohisfromtospan">( From '.$oldesdistrict.' To '.$district.' ) </span>';
					}                   
				}
				if($pincode!=$oldespincode){
					if($invcustinfoch!=''){
						$invcustinfoch.='<br> BILLING COUNTRY/REGION <span style="color:green;" id="prohisfromtospan">( From '.$oldespincode.' To '.$pincode.' ) </span>';
					}
					else{
						$invcustinfoch.=''.(($invinfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessusercus['modulename'].' Information</span> <br> BILLING COUNTRY/REGION <span style="color:green;" id="prohisfromtospan">( From '.$oldespincode.' To '.$pincode.' ) </span>';
					}                   
				}
				if($sarea!=$oldessarea){
					if($invcustinfoch!=''){
						$invcustinfoch.='<br> SHIPPING STREET <span style="color:green;" id="prohisfromtospan">( From '.$oldessarea.' To '.$sarea.' ) </span>';
					}
					else{
						$invcustinfoch.=''.(($invinfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessusercus['modulename'].' Information</span> <br> SHIPPING STREET <span style="color:green;" id="prohisfromtospan">( From '.$oldessarea.' To '.$sarea.' ) </span>';
					}                   
				}
				if($scity!=$oldesscity){
					if($invcustinfoch!=''){
						$invcustinfoch.='<br> SHIPPING CITY/TOWN <span style="color:green;" id="prohisfromtospan">( From '.$oldesscity.' To '.$scity.' ) </span>';
					}
					else{
						$invcustinfoch.=''.(($invinfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessusercus['modulename'].' Information</span> <br> SHIPPING CITY/TOWN <span style="color:green;" id="prohisfromtospan">( From '.$oldesscity.' To '.$scity.' ) </span>';
					}                   
				}
				if($sstate!=$oldessstate){
					if($invcustinfoch!=''){
						$invcustinfoch.='<br> SHIPPING STATE <span style="color:green;" id="prohisfromtospan">( From '.$oldessstate.' To '.$sstate.' ) </span>';
					}
					else{
						$invcustinfoch.=''.(($invinfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessusercus['modulename'].' Information</span> <br> SHIPPING STATE <span style="color:green;" id="prohisfromtospan">( From '.$oldessstate.' To '.$sstate.' ) </span>';
					}                   
				}
				if($sdistrict!=$oldessdistrict){
					if($invcustinfoch!=''){
						$invcustinfoch.='<br> SHIPPING PIN <span style="color:green;" id="prohisfromtospan">( From '.$oldessdistrict.' To '.$sdistrict.' ) </span>';
					}
					else{
						$invcustinfoch.=''.(($invinfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessusercus['modulename'].' Information</span> <br> SHIPPING PIN <span style="color:green;" id="prohisfromtospan">( From '.$oldessdistrict.' To '.$sdistrict.' ) </span>';
					}                   
				}
				if($spincode!=$oldesspincode){
					if($invcustinfoch!=''){
						$invcustinfoch.='<br> SHIPPING COUNTRY/REGION <span style="color:green;" id="prohisfromtospan">( From '.$oldesspincode.' To '.$spincode.' ) </span>';
					}
					else{
						$invcustinfoch.=''.(($invinfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessusercus['modulename'].' Information</span> <br> SHIPPING COUNTRY/REGION <span style="color:green;" id="prohisfromtospan">( From '.$oldesspincode.' To '.$spincode.' ) </span>';
					}                   
				}
				if($twoworkphone!=$oldestwoworkphone){
					if($invcustinfoch!=''){
						$invcustinfoch.='<br> WORK PHONE <span style="color:green;" id="prohisfromtospan">( From '.$oldestwoworkphone.' To '.$twoworkphone.' ) </span>';
					}
					else{
						$invcustinfoch.=''.(($invinfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessusercus['modulename'].' Information</span> <br> WORK PHONE <span style="color:green;" id="prohisfromtospan">( From '.$oldestwoworkphone.' To '.$twoworkphone.' ) </span>';
					}                   
				}
				if($twomobilephone!=$oldestwomobilephone){
					if($invcustinfoch!=''){
						$invcustinfoch.='<br> MOBILE PHONE <span style="color:green;" id="prohisfromtospan">( From '.$oldestwomobilephone.' To '.$twomobilephone.' ) </span>';
					}
					else{
						$invcustinfoch.=''.(($invinfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessusercus['modulename'].' Information</span> <br> MOBILE PHONE <span style="color:green;" id="prohisfromtospan">( From '.$oldestwomobilephone.' To '.$twomobilephone.' ) </span>';
					}                   
				}
				if($gstrtype!=$oldesgstrtype){
					if($invcustinfoch!=''){
						$invcustinfoch.='<br> GST TREATMENT <span style="color:green;" id="prohisfromtospan">( From '.$oldesgstrtype.' To '.$gstrtype.' ) </span>';
					}
					else{
						$invcustinfoch.=''.(($invinfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessusercus['modulename'].' Information</span> <br> GST TREATMENT <span style="color:green;" id="prohisfromtospan">( From '.$oldesgstrtype.' To '.$gstrtype.' ) </span>';
					}                   
				}
				if($gstno!=$oldesgstno){
					if($invcustinfoch!=''){
						$invcustinfoch.='<br> GSTIN <span style="color:green;" id="prohisfromtospan">( From '.$oldesgstno.' To '.$gstno.' ) </span>';
					}
					else{
						$invcustinfoch.=''.(($invinfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessusercus['modulename'].' Information</span> <br> GSTIN <span style="color:green;" id="prohisfromtospan">( From '.$oldesgstno.' To '.$gstno.' ) </span>';
					}                   
				}
				if($pos!=$oldespos){
					if($invcustinfoch!=''){
						$invcustinfoch.='<br> PLACE OF SUPPLY <span style="color:green;" id="prohisfromtospan">( From '.$oldespos.' To '.$pos.' ) </span>';
					}
					else{
						$invcustinfoch.=''.(($invinfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessusercus['modulename'].' Information</span> <br> PLACE OF SUPPLY <span style="color:green;" id="prohisfromtospan">( From '.$oldespos.' To '.$pos.' ) </span>';
					}                   
				}
				if($dlno20!=$oldesdlno20){
					if($invcustinfoch!=''){
						$invcustinfoch.='<br> DL No 20 <span style="color:green;" id="prohisfromtospan">( From '.$oldesdlno20.' To '.$dlno20.' ) </span>';
					}
					else{
						$invcustinfoch.=''.(($invinfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessusercus['modulename'].' Information</span> <br> DL No 20 <span style="color:green;" id="prohisfromtospan">( From '.$oldesdlno20.' To '.$dlno20.' ) </span>';
					}                   
				}
				if($dlno21!=$oldesdlno21){
					if($invcustinfoch!=''){
						$invcustinfoch.='<br> DL No 21 <span style="color:green;" id="prohisfromtospan">( From '.$oldesdlno21.' To '.$dlno21.' ) </span>';
					}
					else{
						$invcustinfoch.=''.(($invinfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessusercus['modulename'].' Information</span> <br> DL No 21 <span style="color:green;" id="prohisfromtospan">( From '.$oldesdlno21.' To '.$dlno21.' ) </span>';
					}                   
				}
					...
					for($i=0; $i<count($_POST['oldesproductname']); $i++){
						...
						if($productname!=''){
							if($productname!=$oldesproductname){
								if($inviteminfoch!=''){
									$inviteminfoch.='<br> '.$infomainaccessuserpro['modulename'].' Name <span style="color:green;" id="prohisfromtospan">( From '.$oldesproductname.' To '.$productname.' ) </span>';
								}
								else{
									$inviteminfoch.=''.(($invinfoch!=''||$invcustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> '.$infomainaccessuserpro['modulename'].' Name <span style="color:green;" id="prohisfromtospan">( From '.$oldesproductname.' To '.$productname.' ) </span>';
								}                   
							}
							if($manufacturer!=$oldesmanufacturer){
								if($inviteminfoch!=''){
									$inviteminfoch.='<br> '.$access['txtnamecategory'].' <span style="color:green;" id="prohisfromtospan">( From '.$oldesmanufacturer.' To '.$manufacturer.' ) </span>';
								}
								else{
									$inviteminfoch.=''.(($invinfoch!=''||$invcustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> '.$access['txtnamecategory'].' <span style="color:green;" id="prohisfromtospan">( From '.$oldesmanufacturer.' To '.$manufacturer.' ) </span>';
								}                   
							}
							if($producthsn!=$oldesproducthsn){
								if($inviteminfoch!=''){
									$inviteminfoch.='<br> HSN Code <span style="color:green;" id="prohisfromtospan">( From '.$oldesproducthsn.' To '.$producthsn.' ) </span>';
								}
								else{
									$inviteminfoch.=''.(($invinfoch!=''||$invcustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> HSN Code <span style="color:green;" id="prohisfromtospan">( From '.$oldesproducthsn.' To '.$producthsn.' ) </span>';
								}                   
							}
							if($productdescription!=$oldesproductdescription){
								if($inviteminfoch!=''){
									$inviteminfoch.='<br> Description <span style="color:green;" id="prohisfromtospan">( From '.$oldesproductdescription.' To '.$productdescription.' ) </span>';
								}
								else{
									$inviteminfoch.=''.(($invinfoch!=''||$invcustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Description <span style="color:green;" id="prohisfromtospan">( From '.$oldesproductdescription.' To '.$productdescription.' ) </span>';
								}                   
							}
							if($batch!=$oldesbatch){
								if($inviteminfoch!=''){
									$inviteminfoch.='<br> Batch <span style="color:green;" id="prohisfromtospan">( From '.$oldesbatch.' To '.$batch.' ) </span>';
								}
								else{
									$inviteminfoch.=''.(($invinfoch!=''||$invcustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Batch <span style="color:green;" id="prohisfromtospan">( From '.$oldesbatch.' To '.$batch.' ) </span>';
								}                   
							}
							if($expdate!=$oldesexpdate){
								if($inviteminfoch!=''){
									$inviteminfoch.='<br> Expiry <span style="color:green;" id="prohisfromtospan">( From '.date($date,strtotime($oldesexpdate)).' To '.date($date,strtotime($expdate)).' ) </span>';
								}
								else{
									$inviteminfoch.=''.(($invinfoch!=''||$invcustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Expiry <span style="color:green;" id="prohisfromtospan">( From '.date($date,strtotime($oldesexpdate)).' To '.date($date,strtotime($expdate)).' ) </span>';
								}                   
							}
							if($productrate!=$oldesproductrate){
								if($inviteminfoch!=''){
									$inviteminfoch.='<br> Rate <span style="color:green;" id="prohisfromtospan">( From '.$oldesproductrate.' To '.$productrate.' ) </span>';
								}
								else{
									$inviteminfoch.=''.(($invinfoch!=''||$invcustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Rate <span style="color:green;" id="prohisfromtospan">( From '.$oldesproductrate.' To '.$productrate.' ) </span>';
								}                   
							}
							if($mrp!=$oldesmrp){
								if($inviteminfoch!=''){
									$inviteminfoch.='<br> Mrp <span style="color:green;" id="prohisfromtospan">( From '.$oldesmrp.' To '.$mrp.' ) </span>';
								}
								else{
									$inviteminfoch.=''.(($invinfoch!=''||$invcustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Mrp <span style="color:green;" id="prohisfromtospan">( From '.$oldesmrp.' To '.$mrp.' ) </span>';
								}                   
							}
							if($quantity!=$oldesquantity){
								if($inviteminfoch!=''){
									$inviteminfoch.='<br> Quantity <span style="color:green;" id="prohisfromtospan">( From '.$oldesquantity.' To '.$quantity.' ) </span>';
								}
								else{
									$inviteminfoch.=''.(($invinfoch!=''||$invcustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Quantity <span style="color:green;" id="prohisfromtospan">( From '.$oldesquantity.' To '.$quantity.' ) </span>';
								}                   
							}
							if($unit!=$oldesunit){
								if($inviteminfoch!=''){
									$inviteminfoch.='<br> Unit <span style="color:green;" id="prohisfromtospan">( From '.$oldesunit.' To '.$unit.' ) </span>';
								}
								else{
									$inviteminfoch.=''.(($invinfoch!=''||$invcustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Unit <span style="color:green;" id="prohisfromtospan">( From '.$oldesunit.' To '.$unit.' ) </span>';
								}                   
							}
							if($noofpacks!=$oldesnoofpacks){
								if($inviteminfoch!=''){
									$inviteminfoch.='<br> Pack <span style="color:green;" id="prohisfromtospan">( From '.$oldesnoofpacks.' To '.$noofpacks.' ) </span>';
								}
								else{
									$inviteminfoch.=''.(($invinfoch!=''||$invcustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Pack <span style="color:green;" id="prohisfromtospan">( From '.$oldesnoofpacks.' To '.$noofpacks.' ) </span>';
								}                   
							}
							if($productvalue!=$oldesproductvalue){
								if($inviteminfoch!=''){
									$inviteminfoch.='<br> Taxable Value <span style="color:green;" id="prohisfromtospan">( From '.$oldesproductvalue.' To '.$productvalue.' ) </span>';
								}
								else{
									$inviteminfoch.=''.(($invinfoch!=''||$invcustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Taxable Value <span style="color:green;" id="prohisfromtospan">( From '.$oldesproductvalue.' To '.$productvalue.' ) </span>';
								}                   
							}
							if($prodiscount!=$oldesprodiscount){
								if($inviteminfoch!=''){
									$inviteminfoch.='<br> '.$access['txtprodisinv'].' <span style="color:green;" id="prohisfromtospan">( From '.$oldesprodiscount.' To '.$prodiscount.' ) </span>';
								}
								else{
									$inviteminfoch.=''.(($invinfoch!=''||$invcustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> '.$access['txtprodisinv'].' <span style="color:green;" id="prohisfromtospan">( From '.$oldesprodiscount.' To '.$prodiscount.' ) </span>';
								}                   
							}
							if($prodiscounttype!=$oldesprodiscounttype){
								if($inviteminfoch!=''){
									$inviteminfoch.='<br> Discounted By <span style="color:green;" id="prohisfromtospan">( From '.(($oldesprodiscounttype==0)?'%':'?').' To '.(($prodiscounttype==0)?'%':'?').' ) </span>';
								}
								else{
									$inviteminfoch.=''.(($invinfoch!=''||$invcustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Discounted By <span style="color:green;" id="prohisfromtospan">( From '.(($oldesprodiscounttype==0)?'%':'?').' To '.(($prodiscounttype==0)?'%':'?').' ) </span>';
								}                   
							}
							if($taxvalue!=$oldestaxvalue){
								if($inviteminfoch!=''){
									$inviteminfoch.='<br> Tax Value <span style="color:green;" id="prohisfromtospan">( From '.$oldestaxvalue.' To '.$taxvalue.' ) </span>';
								}
								else{
									$inviteminfoch.=''.(($invinfoch!=''||$invcustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Tax Value <span style="color:green;" id="prohisfromtospan">( From '.$oldestaxvalue.' To '.$taxvalue.' ) </span>';
								}                   
							}
							if($vat!=$oldesvat){
								if($inviteminfoch!=''){
									$inviteminfoch.='<br> Gst <span style="color:green;" id="prohisfromtospan">( From '.$oldesvat.' % ('.$oldesicsgsthis.') To '.$vat.' % ('.$ansforsepgstval.') ) </span>';
								}
								else{
									$inviteminfoch.=''.(($invinfoch!=''||$invcustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Gst <span style="color:green;" id="prohisfromtospan">( From '.$oldesvat.' % ('.$oldesicsgsthis.') To '.$vat.' % ('.$ansforsepgstval.') ) </span>';
								}                   
							}
							if($productnetvalue!=$oldesproductnetvalue){
								if($inviteminfoch!=''){
									$inviteminfoch.='<br> Amount <span style="color:green;" id="prohisfromtospan">( From '.$oldesproductnetvalue.' To '.$productnetvalue.' ) </span>';
								}
								else{
									$inviteminfoch.=''.(($invinfoch!=''||$invcustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Amount <span style="color:green;" id="prohisfromtospan">( From '.$oldesproductnetvalue.' To '.$productnetvalue.' ) </span>';
								}                   
							}
						}
					}
					...
					for($i=0; $i<count($_POST['oldesproductname']); $i++){
						...
						if($productname!=''){
							...
							if($oldesproductname!=''){                    
								if($oldesproductname!=''){
									if($inviteminfoch!=''){
										$inviteminfoch.='<br> '.$infomainaccessuserpro['modulename'].' Name <span style="color:red;" id="prohisfromtospan">( From '.$oldesproductname.' To &nbsp; ) </span>';
									}
									else{
										$inviteminfoch.=''.(($invinfoch!=''||$invcustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> '.$infomainaccessuserpro['modulename'].' Name <span style="color:red;" id="prohisfromtospan">( From '.$oldesproductname.' To &nbsp; ) </span>';
									}
								}
								if($oldesmanufacturer!=''){
									if($inviteminfoch!=''){
										$inviteminfoch.='<br> '.$access['txtnamecategory'].' <span style="color:red;" id="prohisfromtospan">( From '.$oldesmanufacturer.' To &nbsp; ) </span>';
									}
									else{
										$inviteminfoch.=''.(($invinfoch!=''||$invcustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> '.$access['txtnamecategory'].' <span style="color:red;" id="prohisfromtospan">( From '.$oldesmanufacturer.' To &nbsp; ) </span>';
									}
								}
								if($oldesproducthsn!=''){
									if($inviteminfoch!=''){
										$inviteminfoch.='<br> HSN Code <span style="color:red;" id="prohisfromtospan">( From '.$oldesproducthsn.' To &nbsp; ) </span>';
									}
									else{
										$inviteminfoch.=''.(($invinfoch!=''||$invcustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> HSN Code <span style="color:red;" id="prohisfromtospan">( From '.$oldesproducthsn.' To &nbsp; ) </span>';
									}
								}
								if($oldesproductdescription!=''){
									if($inviteminfoch!=''){
										$inviteminfoch.='<br> Description <span style="color:red;" id="prohisfromtospan">( From '.$oldesproductdescription.' To &nbsp; ) </span>';
									}
									else{
										$inviteminfoch.=''.(($invinfoch!=''||$invcustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Description <span style="color:red;" id="prohisfromtospan">( From '.$oldesproductdescription.' To &nbsp; ) </span>';
									}
								}
								if($oldesbatch!=''){
									if($inviteminfoch!=''){
										$inviteminfoch.='<br> Batch <span style="color:red;" id="prohisfromtospan">( From '.$oldesbatch.' To &nbsp; ) </span>';
									}
									else{
										$inviteminfoch.=''.(($invinfoch!=''||$invcustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Batch <span style="color:red;" id="prohisfromtospan">( From '.$oldesbatch.' To &nbsp; ) </span>';
									}
								}
								if($oldesexpdate!=''){
									if($inviteminfoch!=''){
										$inviteminfoch.='<br> Expiry <span style="color:red;" id="prohisfromtospan">( From '.date($date,strtotime($oldesexpdate)).' To &nbsp; ) </span>';
									}
									else{
										$inviteminfoch.=''.(($invinfoch!=''||$invcustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Expiry <span style="color:red;" id="prohisfromtospan">( From '.date($date,strtotime($oldesexpdate)).' To &nbsp; ) </span>';
									}
								}
								if($oldesproductrate!=''){
									if($inviteminfoch!=''){
										$inviteminfoch.='<br> Rate <span style="color:red;" id="prohisfromtospan">( From '.$oldesproductrate.' To &nbsp; ) </span>';
									}
									else{
										$inviteminfoch.=''.(($invinfoch!=''||$invcustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Rate <span style="color:red;" id="prohisfromtospan">( From '.$oldesproductrate.' To &nbsp; ) </span>';
									}
								}
								if($oldesmrp!=''){
									if($inviteminfoch!=''){
										$inviteminfoch.='<br> Mrp <span style="color:red;" id="prohisfromtospan">( From '.$oldesmrp.' To &nbsp; ) </span>';
									}
									else{
										$inviteminfoch.=''.(($invinfoch!=''||$invcustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Mrp <span style="color:red;" id="prohisfromtospan">( From '.$oldesmrp.' To &nbsp; ) </span>';
									}
								}
								if($oldesquantity!=''){
									if($inviteminfoch!=''){
										$inviteminfoch.='<br> Quantity <span style="color:red;" id="prohisfromtospan">( From '.$oldesquantity.' To &nbsp; ) </span>';
									}
									else{
										$inviteminfoch.=''.(($invinfoch!=''||$invcustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Quantity <span style="color:red;" id="prohisfromtospan">( From '.$oldesquantity.' To &nbsp; ) </span>';
									}
								}
								if($oldesunit!=''){
									if($inviteminfoch!=''){
										$inviteminfoch.='<br> Unit <span style="color:red;" id="prohisfromtospan">( From '.$oldesunit.' To &nbsp; ) </span>';
									}
									else{
										$inviteminfoch.=''.(($invinfoch!=''||$invcustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Unit <span style="color:red;" id="prohisfromtospan">( From '.$oldesunit.' To &nbsp; ) </span>';
									}
								}
								if($oldesnoofpacks!=''){
									if($inviteminfoch!=''){
										$inviteminfoch.='<br> Pack <span style="color:red;" id="prohisfromtospan">( From '.$oldesnoofpacks.' To &nbsp; ) </span>';
									}
									else{
										$inviteminfoch.=''.(($invinfoch!=''||$invcustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Pack <span style="color:red;" id="prohisfromtospan">( From '.$oldesnoofpacks.' To &nbsp; ) </span>';
									}
								}
								if($oldesproductvalue!=''){
									if($inviteminfoch!=''){
										$inviteminfoch.='<br> Taxable Value <span style="color:red;" id="prohisfromtospan">( From '.$oldesproductvalue.' To &nbsp; ) </span>';
									}
									else{
										$inviteminfoch.=''.(($invinfoch!=''||$invcustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Taxable Value <span style="color:red;" id="prohisfromtospan">( From '.$oldesproductvalue.' To &nbsp; ) </span>';
									}
								}
								if($oldesprodiscount!=''){
									if($inviteminfoch!=''){
										$inviteminfoch.='<br> Discount <span style="color:red;" id="prohisfromtospan">( From '.$oldesprodiscount.' To &nbsp; ) </span>';
									}
									else{
										$inviteminfoch.=''.(($invinfoch!=''||$invcustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Discount <span style="color:red;" id="prohisfromtospan">( From '.$oldesprodiscount.' To &nbsp; ) </span>';
									}
								}
								if($oldestaxvalue!=''){
									if($inviteminfoch!=''){
										$inviteminfoch.='<br> Tax Value <span style="color:red;" id="prohisfromtospan">( From '.$oldestaxvalue.' To &nbsp; ) </span>';
									}
									else{
										$inviteminfoch.=''.(($invinfoch!=''||$invcustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Tax Value <span style="color:red;" id="prohisfromtospan">( From '.$oldestaxvalue.' To &nbsp; ) </span>';
									}
								}
								if($oldesvat!=''){
									if($inviteminfoch!=''){
										$inviteminfoch.='<br> Gst <span style="color:red;" id="prohisfromtospan">( From '.$oldesvat.' % ('.$oldesicsgsthis.') To &nbsp; ) </span>';
									}
									else{
										$inviteminfoch.=''.(($invinfoch!=''||$invcustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Gst <span style="color:red;" id="prohisfromtospan">( From '.$oldesvat.' % ('.$oldesicsgsthis.') To &nbsp; ) </span>';
									}
								}
								if($oldesproductnetvalue!=''){
									if($inviteminfoch!=''){
										$inviteminfoch.='<br> Amount <span style="color:red;" id="prohisfromtospan">( From '.$oldesproductnetvalue.' To &nbsp; ) </span>';
									}
									else{
										$inviteminfoch.=''.(($invinfoch!=''||$invcustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Amount <span style="color:red;" id="prohisfromtospan">( From '.$oldesproductnetvalue.' To &nbsp; ) </span>';
									}
								}
							}
						}
				...
				elseif ($curans>$oldans) {
					for($i=0; $i<count($_POST['productname']); $i++){
						...
						if($oldesproductname!=''){
							...
							if($expdate!=''){
								if($inviteminfoch!=''){
									$inviteminfoch.='<br> Expiry <span style="color:green;" id="prohisfromtospan">( '.date($date,strtotime($expdate)).' ) </span>';
								}
								else{
									$inviteminfoch.=''.(($invinfoch!=''||$invcustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Expiry <span style="color:green;" id="prohisfromtospan">( '.date($date,strtotime($expdate)).' ) </span>';        
								}
							}
							if($productrate!=''){
								if($inviteminfoch!=''){
				...
				if($totalitems!=$oldestotalitems){
					if($inviteminfoch!=''){
						$inviteminfoch.='<br> Total Items <span style="color:green;" id="prohisfromtospan">( From '.$oldestotalitems.' To '.$totalitems.' ) </span>';
					}
					else{
						$inviteminfoch.=''.(($invinfoch!=''||$invcustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Total Items <span style="color:green;" id="prohisfromtospan">( From '.$oldestotalitems.' To '.$totalitems.' ) </span>';
					}
				}
				if($totalquantity!=$oldestotalquantity){
					if($inviteminfoch!=''){
						$inviteminfoch.='<br> Total Qty <span style="color:green;" id="prohisfromtospan">( From '.$oldestotalquantity.' To '.$totalquantity.' ) </span>';
					}
					else{
						$inviteminfoch.=''.(($invinfoch!=''||$invcustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Total Qty <span style="color:green;" id="prohisfromtospan">( From '.$oldestotalquantity.' To '.$totalquantity.' ) </span>';
					}
				}
				if($totalamount!=$oldestotalamount){
					if($inviteminfoch!=''){
						$inviteminfoch.='<br> Sub Total <span style="color:green;" id="prohisfromtospan">( From '.$oldestotalamount.' To '.$totalamount.' ) </span>';
					}
					else{
						$inviteminfoch.=''.(($invinfoch!=''||$invcustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Sub Total <span style="color:green;" id="prohisfromtospan">( From '.$oldestotalamount.' To '.$totalamount.' ) </span>';
					}
				}
				if($discountamount!=$oldesdiscountamount){
					if($inviteminfoch!=''){
						$inviteminfoch.='<br> Discount <span style="color:green;" id="prohisfromtospan">( From '.$oldesdiscountamount.' To '.$discountamount.' ) </span>';
					}
					else{
						$inviteminfoch.=''.(($invinfoch!=''||$invcustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Discount <span style="color:green;" id="prohisfromtospan">( From '.$oldesdiscountamount.' To '.$discountamount.' ) </span>';
					}
				}
				if($totalvatamount!=$oldestotalvatamount){
					if($inviteminfoch!=''){
						$inviteminfoch.='<br> Total Tax <span style="color:green;" id="prohisfromtospan">( From '.$oldestotalvatamount.' To '.$totalvatamount.' ) </span>';
					}
					else{
						$inviteminfoch.=''.(($invinfoch!=''||$invcustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Total Tax <span style="color:green;" id="prohisfromtospan">( From '.$oldestotalvatamount.' To '.$totalvatamount.' ) </span>';
					}
				}
				if($roundoff!=$oldesroundoff){
					if($inviteminfoch!=''){
						$inviteminfoch.='<br> Round Off <span style="color:green;" id="prohisfromtospan">( From '.$oldesroundoff.' To '.$roundoff.' ) </span>';
					}
					else{
						$inviteminfoch.=''.(($invinfoch!=''||$invcustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Round Off <span style="color:green;" id="prohisfromtospan">( From '.$oldesroundoff.' To '.$roundoff.' ) </span>';
					}
				}
				if($grandtotal!=$oldesgrandtotal){
					if($inviteminfoch!=''){
						$inviteminfoch.='<br> Grand Total <span style="color:green;" id="prohisfromtospan">( From '.$oldesgrandtotal.' To '.$grandtotal.' ) </span>';
					}
					else{
						$inviteminfoch.=''.(($invinfoch!=''||$invcustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Grand Total <span style="color:green;" id="prohisfromtospan">( From '.$oldesgrandtotal.' To '.$grandtotal.' ) </span>';
					}
				}
				if($description!=$oldesdescription){
					if($inviteminfoch!=''){
						$inviteminfoch.='<br> Description <span style="color:green;" id="prohisfromtospan">( From '.$oldesdescription.' To '.$description.' ) </span>';
					}
					else{
						$inviteminfoch.=''.(($invinfoch!=''||$invcustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Description <span style="color:green;" id="prohisfromtospan">( From '.$oldesdescription.' To '.$description.' ) </span>';
					}
				}
				if($notes!=$oldesnotes){
					if($inviteminfoch!=''){
						$inviteminfoch.='<br> Notes <span style="color:green;" id="prohisfromtospan">( From '.$oldesnotes.' To '.$notes.' ) </span>';
					}
					else{
						$inviteminfoch.=''.(($invinfoch!=''||$invcustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Notes <span style="color:green;" id="prohisfromtospan">( From '.$oldesnotes.' To '.$notes.' ) </span>';
					}
				}
				if($terms!=$oldesterms){
					if($inviteminfoch!=''){
						$inviteminfoch.='<br> Terms And Conditions <span style="color:green;" id="prohisfromtospan">( From '.$oldesterms.' To '.$terms.' ) </span>';
					}
					else{
						$inviteminfoch.=''.(($invinfoch!=''||$invcustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Terms And Conditions <span style="color:green;" id="prohisfromtospan">( From '.$oldesterms.' To '.$terms.' ) </span>';
					}
				}
				if($fileattach!=$oldesfileattach){
					if($inviteminfoch!=''){
						$inviteminfoch.='<br> Attach <span style="color:green;" id="prohisfromtospan">( Changed ) </span>';
					}
					else{
						$inviteminfoch.=''.(($invinfoch!=''||$invcustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Attach <span style="color:green;" id="prohisfromtospan">( Changed ) </span>';
					}
				}
				if($invoiceterm!=$oldesinvoiceterm){
					if($inviteminfoch!=''){
						$inviteminfoch.='<br> Payment Method <span style="color:green;" id="prohisfromtospan">( From '.$oldesinvoiceterm.' To '.$invoiceterm.' ) </span>';
					}
					else{
						$inviteminfoch.=''.(($invinfoch!=''||$invcustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Payment Method <span style="color:green;" id="prohisfromtospan">( From '.$oldesinvoiceterm.' To '.$invoiceterm.' ) </span>';
					}
				}
				if($validpaidamount!=$oldesvalidpaidamount&&$invoiceterm=='CASH,CARD,GPAY'){
					if($inviteminfoch!=''){
						$inviteminfoch.='<br> '.(($access['definvpay']=='split')?'Amount Received':'Cash Received').' <span style="color:green;" id="prohisfromtospan">( From '.$oldesvalidpaidamount.' To '.$validpaidamount.' ) </span>';
					}
					else{
						$inviteminfoch.=''.(($invinfoch!=''||$invcustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> '.(($access['definvpay']=='split')?'Amount Received':'Cash Received').' <span style="color:green;" id="prohisfromtospan">( From '.$oldesvalidpaidamount.' To '.$validpaidamount.' ) </span>';
					}
				}
				if($validpaidamount!=$oldesvalidpaidamount&&$invoiceterm=='CASH'){
					if($inviteminfoch!=''){
						$inviteminfoch.='<br> '.(($access['definvpay']=='split')?'Amount Received':'Cash Received').' <span style="color:green;" id="prohisfromtospan">( From '.$oldesvalidpaidamount.' To '.$validpaidamount.' ) </span>';
					}
					else{
				...
				if($validbalance!=$oldesvalidbalance&&$invoiceterm=='CASH'){
					if($inviteminfoch!=''){
						$inviteminfoch.='<br> Change Due <span style="color:green;" id="prohisfromtospan">( From '.$oldesvalidbalance.' To '.$validbalance.' ) </span>';
					}
					else{
						$inviteminfoch.=''.(($invinfoch!=''||$invcustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Change Due <span style="color:green;" id="prohisfromtospan">( From '.$oldesvalidbalance.' To '.$validbalance.' ) </span>';
					}
				}
				if($validbalance!=$oldesvalidbalance&&$invoiceterm=='CASH,CARD,GPAY'){
					if($inviteminfoch!=''){
						$inviteminfoch.='<br> Change Due <span style="color:green;" id="prohisfromtospan">( From '.$oldesvalidbalance.' To '.$validbalance.' ) </span>';
					}
					else{
				...
				if($duedates!=$oldesduedates&&$invoiceterm=='CREDIT'){
					if($inviteminfoch!=''){
						$inviteminfoch.='<br> Due Term <span style="color:green;" id="prohisfromtospan">( From '.$oldesduedates.' To '.$duedates.' ) </span>';
					}
					else{
						$inviteminfoch.=''.(($invinfoch!=''||$invcustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Due Term <span style="color:green;" id="prohisfromtospan">( From '.$oldesduedates.' To '.$duedates.' ) </span>';
					}
				}
				if($duedate!=$oldesduedate&&$invoiceterm=='CREDIT'){
					if($inviteminfoch!=''){
						$inviteminfoch.='<br> Due Date <span style="color:green;" id="prohisfromtospan">( From '.date($date,strtotime($oldesduedate)).' To '.date($date,strtotime($duedate)).' ) </span>';
					}
					else{
						$inviteminfoch.=''.(($invinfoch!=''||$invcustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Due Date <span style="color:green;" id="prohisfromtospan">( From '.date($date,strtotime($oldesduedate)).' To '.date($date,strtotime($duedate)).' ) </span>';
					}
				}
				//FOR INVOICE OTHER INFORMATIONS LIKE PAYMENT AND TERMS AND DESCRIPTION......
				$invtotinfoch = $invinfoch.$invcustinfoch.$inviteminfoch;
				if($invtotinfoch!=''){
				...
				$sqlusehisupdqry->bind_param("sss", $salesid, $invoiceno, $_GET['id']);
...
<main class="main-content position-relative max-height-vh-100 h-100 mt-0 " style="overflow-y: scroll !important;">
	...
	<?php
		...
		if((isset($_GET['invoiceno']))&&(isset($_GET['invoicedate']))){
			$invoiceno=htmlspecialchars( $_GET['invoiceno'], ENT_QUOTES, 'UTF-8');
			$invoicedate=htmlspecialchars( $_GET['invoicedate'], ENT_QUOTES, 'UTF-8');
			$sql=$con->prepare("SELECT * FROM pairinvoices WHERE franchisesession=? AND createdid=? AND invoiceno=? AND invoicedate=? ORDER BY id ASC LIMIT 5");
			$sql->bind_param("ssss", $_SESSION['franchisesession'], $companymainid, $invoiceno, $invoicedate);
			$sql->execute();
			if ($sql->errno) {
					...
					<div class="card mb-4 mt-5">
						...
						?>
							...
							?>
								...
								<input type="hidden" name="invoiceterm" value="<?=$rows[0]['invoiceterm']?>">
								<div class="row">
								<?php
									// if ((in_array('Invoice Information', $fieldedit))) {
								?>
									<div class="col-lg-4">
														...
														<div class="row">
															<div class="col-lg-12">
																<div class="form-group row" style="align-items: center !important;">
																	...
																	<div class="col-lg-7">
																		<input type="text" class="form-control  form-control-sm" id="invoiceno" name="invoiceno" required  value="<?=$rows[0]['invoiceno']?>" readonly>
															...
															<div class="col-lg-12">
																<div class="form-group row" style="align-items: center !important;">
																	...
																	<?php
																		...
																		$dateformats->close();
																	?>
																		<input type="date" class="form-control  form-control-sm" id="invoicedate" name="invoicedate" required value="<?=$rows[0]['invoicedate']?>">
																	</div>
																</div>
															</div>
															<div class="col-lg-12" <?=((in_array('Invoice Time', $fieldedit))?'':'style="display:none;"')?>>
																<div class="form-group row" style="align-items: center !important;">
																	<div class="col-lg-5">
																	...
																	<div class="col-lg-7">
																		...
																		?>
																		<input type="time" class="form-control form-control-sm" id="invoicetime" name="invoicetime" required value="<?=$rows[0]['invoicetime']?>">
																	</div>
																</div>
															</div>
														<?php
														...
														<div id="two">
															<div class="row">
																...
																<div class="<?=((in_array('Age', $fieldedit))&&(in_array('Sex', $fieldedit)))?'col-lg-4':(((in_array('Age', $fieldedit))||(in_array('Sex', $fieldedit)))?'col-lg-7':'col-lg-9')?>">
																	<input type="text" name="twocustomername" id="twocustomername" class="form-control form-control-sm" placeholder="<?= $infomainaccessusercus['modulename'] ?> Name" value="<?= $rows[0]['customername'] ?>" <?= ($access['invbtwocnamerequired']=='Yes')? 'required': ' ' ?>>
																	...
																	<div class="row mb-0" id="custaddressdiv" style="background-color:#fbfafa; color:#777777; display:block;margin: 0px !important;padding: 9px 9px 0px 9px !important;">
																		...
																		<div class="row" style="padding:0px !important;margin: 0px !important;">
																			<div class="col-lg-6" <?=(in_array('Work Phone', $fieldedit))?'':'style="display:none;"'?>>
																				...
																				<input type="text" class="form-control form-control-sm" id="twoworkphone" name="twoworkphone" placeholder="Work Phone" value="<?=$rows[0]['workphone']?>" <?= ($access['invbtwocwphonerequired']=='Yes')? 'required': ' ' ?> maxlength="25">
														...
														<table class="table table-bordered" id="purchasetable">
															<thead>
																<tr>
																	...
																	<th <?=((in_array('Barcode', $fieldedit))||(in_array('Item Details', $fieldedit))||($access['batchexpiryval']==1)||(in_array('Rate', $fieldedit))||(in_array('Quantity', $fieldedit))||(in_array('Taxable Value', $fieldedit))||(in_array('Tax Value', $fieldedit))||(in_array('Amount', $fieldedit)))?'':'style="display:none !important;"'?>>
																	</th>
																<?php
															...
															?>
																...
																setTimeout(function() {
																	...
																	function getoldmargins(lineidno) {
																		$("#totalmarginvalue"+lineidno).html($("#oldmargintotalvalue"+lineidno).val());
																	}
																},9000);
																setTimeout(function() {
...
$.ajax({
type: "GET",
url: 'editfetch.php?term=' + sIndexEdit + '&limitings=5&invoiceno=<?=$_GET['invoiceno']?>&invoicedate=<?=$_GET['invoicedate']?>&type=invoice&i='+ivalue+'',
success: function (result) {
