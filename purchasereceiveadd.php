<?php
include('lcheck.php');
$sqlismainaccessfield=mysqli_query($con, "select * from pairmainaccess where userid='$companymainid' and moduletype='Purchase Receives' order by id  asc");
...
if (($infomainaccessuser['purreceiveveninfo']=='defone')||($infomainaccessuser['purreceiveveninfo']=='deftwo')) {
...
$sqlismainaccessuser=mysqli_query($con, "select * from pairmainaccess where userid='$userid' and moduletype='Purchase Receives' order by id  asc");
$infomainaccessuser=mysqli_fetch_array($sqlismainaccessuser);
if ($infomainaccessuser['purreceiveveninfo']=='one'||($scriptonetwo=='one')) {
...
$purchasereceiveterm=mysqli_real_escape_string($con, $_POST['purchasereceiveterm']);
}
else{
$purchasereceiveterm="";
}

$sqlismainaccessuser=mysqli_query($con, "select moduleno, moduleprefix, modulesuffix from pairmainaccess where franchiseid='".$_SESSION['franchisesession']."' and moduletype='Purchase Receives' order by id  asc");
$infomainaccessuser=mysqli_fetch_array($sqlismainaccessuser);
$purchasereceiveno=mysqli_real_escape_string($con, $infomainaccessuser['moduleprefix'].(str_pad($infomainaccessuser['modulesuffix'], 0, "0", STR_PAD_LEFT)));
...
$purchasereceiveamount=mysqli_real_escape_string($con, $_POST['grandtotal']);
...
$vensqlup = "update paircustomers set createdon='$times', createdid='$companymainid', createdby='".$_SESSION["unqwerty"]."', franchisesession='".$_SESSION["franchisesession"]."',billstreet='$area',billcity='$city',billstate='$state',billpincode='$pincode',billcountry='$district',shipstreet='$sarea',shipcity='$scity',shipstate='$sstate',shippincode='$spincode',shipcountry='$sdistrict',gstrtype='$gstrtype',gstin='$vengstin',workphone='$venworkphone',mobile='$venmobilephone' where (franchisesession='".$_SESSION["franchisesession"]."' or cvisiblity='PUBLIC') and (createdid='$companymainid' and moduletype='Vendors') and id='$venid'";
            ...
            $vench='';
               ...
               if ($infomainaccessuser['purreceiveveninfo']=='two'||($scriptonetwo=='two')) {
$address1 = '';
$address2 = '';
$saddress1 = '';
$saddress2 = '';
...
$sql2=mysqli_query($con, "select id from pairpurchasereceives where franchisesession='".$_SESSION['franchisesession']."' and createdid='$companymainid' and purchasereceiveno='$purchasereceiveno' and purchasereceivedate='$purchasereceivedate'");
...
$tid=mysqli_insert_id($con);
mysqli_query($con, "INSERT INTO pairhistory (user, ipaddress, times, remarks, tid) VALUES ('{$username}', '{$ip}', '{$times}', 'Insert A Purchase Receive', '{$tid}')");

$sql3=mysqli_query($con, "update pairfranchises set purchasereceivesuffix=purchasereceivesuffix+1 where tdelete='0' and id='".$_SESSION['franchisesession']."'");
if($sql3)
{
 if(isset($_POST['submit1']))
{
echo '<script> window.open("purchasereceiveprint.php?purchasereceiveno='.$purchasereceiveno.'&purchasereceivedate='.$purchasereceivedate.'", "_blank");</script>';
echo '<script> window.location.href="purchasereceives.php?remarks=Added Successfully";</script>'; 
}
else
{
echo '<script> window.location.href="purchasereceiveview.php?purchasereceiveno='.$purchasereceiveno.'&purchasereceivedate='.$purchasereceivedate.'&remarks=Added Successfully";</script>';
}
}
}
else
{
header("Location: purchasereceives.php?error=Error Data");
}
}
...
$sqlismainaccessuser=mysqli_query($con, "select moduleno, moduleprefix, modulesuffix,modulename from pairmainaccess where franchiseid='".$_SESSION['franchisesession']."' and moduletype='Purchase Receives' order by id  asc");
$infomainaccessuser=mysqli_fetch_array($sqlismainaccessuser);
?>
...
<!-------------------vendor add info end ------------------------------->
<form id="purchasereceiveform" action="" onsubmit="return checkvalidate()" method="post" enctype="multipart/form-data" class="form-horizontal" role="form">
<div class="row">
<!-- <div class="col-lg-6">
...
<?php
     if ((in_array('Purchase Receive Information', $fieldadd))) {
        ?>
<div class="col-lg-4">
<div class="accordion" id="accordionRental">
<div class="accordion-item mb-1">
...
<div class="col-lg-5">
<label for="purchasereceiveno" class="custom-label"><span class="text-danger"><?= $infomainaccessuser['modulename'] ?> Number *</span></label>
</div>
<div class="col-lg-7">
<input type="text" class="form-control  form-control-sm" id="purchasereceiveno" name="purchasereceiveno" required value="<?=$infomainaccessuser['moduleprefix']?><?=str_pad(($infomainaccessuser['modulesuffix']+1), 0, "0", STR_PAD_LEFT)?>" readonly>
</div>
</div>
</div>
<div class="col-lg-12">
...
<label for="purchasereceivedate" class="custom-label"><span class="text-danger"><?= $infomainaccessuser['modulename'] ?> Date *</span></label>
</div>
<div class="col-lg-7">
    <?php
...
$dt = new DateTime();
    ?>
<input type="date" class="form-control  form-control-sm" id="purchasereceivedate" name="purchasereceivedate" value="<?= $dt->format('Y-m-d') ?>" required>
</div>
</div>
</div>
...
<div class="form-group row" style="align-items: center !important;">
<div class="col-lg-5" style="padding-right:0px">
<label for="purchasereceivedate" class="custom-label"><span class="text-danger"><?= $infomainaccessuser['modulename'] ?> Term *</span></label>
</div>
<div class="col-lg-7">
<select class="select2-field form-control  form-control-sm" name="purchasereceiveterm" id="purchasereceiveterm" required>
...
var purchasereceivedate=$("#purchasereceivedate").val();
var today="";
if(duedates!='')
{
console.log(purchasereceivedate);
console.log(duedates);
today=addDays(purchasereceivedate, duedates);
    ...
    if (($infomainaccessuser['purreceiveveninfo']=='defone')||($infomainaccessuser['purreceiveveninfo']=='deftwo')) {
     ...
     <div class="row">
                      ...
                      <div class="custom-control custom-radio mr-sm-2">
                        <input type="radio" class="custom-control-input" name="vendorinfodefault" id="onevendorinfo" value="one" <?= ($infomainaccessuser['purreceiveveninfo']=='defone')?'checked':'' ?> onclick="onetwo()">
                      ...
                      <div class="col-lg-2 col-md-2 col-3">
                      <div class="custom-control custom-radio mr-sm-2">
                        <input type="radio" class="custom-control-input" name="vendorinfodefault" id="twovendorinfo" value="two" <?= ($infomainaccessuser['purreceiveveninfo']=='deftwo')?'checked':'' ?> onclick="onetwo()">
...
<?php
if ($access['purreceivebtwocnamerequired']=='Yes') {
?>
$("#twovendorname").removeAttr("required");
<?php
}
if ($access['purreceivebtwocwphonerequired']=='Yes') {
?>
...
<div class="accordion-body text-sm" style="padding-bottom: 0px;padding-top: 3px">
    ...
    $sqlismainaccessuser=mysqli_query($con, "select * from pairmainaccess where userid='$userid' and moduletype='Purchase Receives' order by id  asc");
$infomainaccessuser=mysqli_fetch_array($sqlismainaccessuser);
    if ($infomainaccessuser['purreceiveveninfo']=='one'||(($infomainaccessuser['purreceiveveninfo']=='defone')||($infomainaccessuser['purreceiveveninfo']=='deftwo'))) {
    ?>
    <div id="one">
<div class="row">
<div class="col-lg-12">
...
if ($infomainaccessuser['purreceiveveninfo']=='two'||(($infomainaccessuser['purreceiveveninfo']=='defone')||($infomainaccessuser['purreceiveveninfo']=='deftwo'))) {
?>
<script type="text/javascript">
function twoposans() {
...
<?= ($access['purreceivebtwocnamerequired']=='Yes')? '<span class="text-danger">'. $infomainaccessuserven['modulename'] . ' Name * </span>': $infomainaccessuserven['modulename'] . ' Name ' ?>
</label>
</div>
<div class="col-lg-9">
<input type="text" name="twovendorname" id="twovendorname" class="form-control form-control-sm" placeholder="<?= $infomainaccessuserven['modulename'] ?> Name" <?= ($access['purreceivebtwocnamerequired']=='Yes')? 'required': ' ' ?>>
</div>
</div>
<div class="row mt-1">
<div class="col-lg-12">
...
<?= ($access['purreceivebtwocwphonerequired']=='Yes')? '<span class="text-danger"> WORK PHONE * </span>': ' WORK PHONE ' ?>
</label>
<input type="text" class="form-control form-control-sm" id="twoworkphone" name="twoworkphone" placeholder="Work Phone" <?= ($access['purreceivebtwocwphonerequired']=='Yes')? 'required': ' ' ?>>
</div>
...
<option disabled value="Select Place of Supply" <?=($infomainaccessuser['purreceivetwopos']=='Select Place of Supply')?'selected':''?>>Select The Place</option> 
<option value="JAMMU AND KASHMIR (1)" <?=($infomainaccessuser['purreceivetwopos']=="JAMMU AND KASHMIR (1)")?'selected':''?>>JAMMU AND KASHMIR (1)</option>
<option value="ANDAMAN AND NICOBAR ISLANDS (35)" <?=($infomainaccessuser['purreceivetwopos']=="ANDAMAN AND NICOBAR ISLANDS (35)")?'selected':''?>>ANDAMAN AND NICOBAR ISLANDS (35)</option>
<option value="ANDHRA PRADESH (NEWLY ADDED) (37)" <?=($infomainaccessuser['purreceivetwopos']=="ANDHRA PRADESH (NEWLY ADDED) (37)")?'selected':''?>>ANDHRA PRADESH (NEWLY ADDED) (37)</option>
<option value="ANDHRA PRADESH(BEFORE DIVISION) (28)" <?=($infomainaccessuser['purreceivetwopos']=="ANDHRA PRADESH(BEFORE DIVISION) (28)")?'selected':''?>>ANDHRA PRADESH(BEFORE DIVISION) (28)</option>
<option value="ARUNACHAL PRADESH (12)" <?=($infomainaccessuser['purreceivetwopos']=="ARUNACHAL PRADESH (12)")?'selected':''?>>ARUNACHAL PRADESH (12)</option>
<option value="ASSAM (18)" <?=($infomainaccessuser['purreceivetwopos']=="ASSAM (18)")?'selected':''?>>ASSAM (18)</option>
<option value="BIHAR (10)" <?=($infomainaccessuser['purreceivetwopos']=="BIHAR (10)")?'selected':''?>>BIHAR (10)</option>
<option value="CENTRE JURISDICTION (99)" <?=($infomainaccessuser['purreceivetwopos']=="CENTRE JURISDICTION (99)")?'selected':''?>>CENTRE JURISDICTION (99)</option>
<option value="CHANDIGARH (4)" <?=($infomainaccessuser['purreceivetwopos']=="CHANDIGARH (4)")?'selected':''?>>CHANDIGARH (4)</option>
<option value="CHATTISGARH (22)" <?=($infomainaccessuser['purreceivetwopos']=="CHATTISGARH (22)")?'selected':''?>>CHATTISGARH (22)</option>
<option value="DADRA AND NAGAR HAVELI AND DAMAN AND DIU (NEWLY MERGED UT) (26*)" <?=($infomainaccessuser['purreceivetwopos']=="DADRA AND NAGAR HAVELI AND DAMAN AND DIU (NEWLY MERGED UT) (26*)")?'selected':''?>>DADRA AND NAGAR HAVELI AND DAMAN AND DIU (NEWLY MERGED UT) (26*)</option>
<option value="DELHI (7)" <?=($infomainaccessuser['purreceivetwopos']=="DELHI (7)")?'selected':''?>>DELHI (7)</option>
<option value="GOA (30)" <?=($infomainaccessuser['purreceivetwopos']=="GOA (30)")?'selected':''?>>GOA (30)</option>
<option value="GUJARAT (24)" <?=($infomainaccessuser['purreceivetwopos']=="GUJARAT (24)")?'selected':''?>>GUJARAT (24)</option>
<option value="HARYANA (6)" <?=($infomainaccessuser['purreceivetwopos']=="HARYANA (6)")?'selected':''?>>HARYANA (6)</option>
<option value="HIMACHAL PRADESH (2)" <?=($infomainaccessuser['purreceivetwopos']=="HIMACHAL PRADESH (2)")?'selected':''?>>HIMACHAL PRADESH (2)</option>
<option value="JHARKHAND (20)" <?=($infomainaccessuser['purreceivetwopos']=="JHARKHAND (20)")?'selected':''?>>JHARKHAND (20)</option>
<option value="KARNATAKA (29)" <?=($infomainaccessuser['purreceivetwopos']=="KARNATAKA (29)")?'selected':''?>>KARNATAKA (29)</option>
<option value="KERALA (32)" <?=($infomainaccessuser['purreceivetwopos']=="KERALA (32)")?'selected':''?>>KERALA (32)</option>
<option value="LADAKH (NEWLY ADDED) (38)" <?=($infomainaccessuser['purreceivetwopos']=="LADAKH (NEWLY ADDED) (38)")?'selected':''?>>LADAKH (NEWLY ADDED) (38)</option>
<option value="LAKSHADWEEP (31)" <?=($infomainaccessuser['purreceivetwopos']=="LAKSHADWEEP (31)")?'selected':''?>>LAKSHADWEEP (31)</option>
<option value="MADHYA PRADESH (23)" <?=($infomainaccessuser['purreceivetwopos']=="MADHYA PRADESH (23)")?'selected':''?>>MADHYA PRADESH (23)</option>
<option value="MAHARASHTRA (27)" <?=($infomainaccessuser['purreceivetwopos']=="MAHARASHTRA (27)")?'selected':''?>>MAHARASHTRA (27)</option>
<option value="MANIPUR (14)" <?=($infomainaccessuser['purreceivetwopos']=="MANIPUR (14)")?'selected':''?>>MANIPUR (14)</option>
<option value="MEGHALAYA (17)" <?=($infomainaccessuser['purreceivetwopos']=="MEGHALAYA (17)")?'selected':''?>>MEGHALAYA (17)</option>
<option value="MIZORAM (15)" <?=($infomainaccessuser['purreceivetwopos']=="MIZORAM (15)")?'selected':''?>>MIZORAM (15)</option>
<option value="NAGALAND (13)" <?=($infomainaccessuser['purreceivetwopos']=="NAGALAND (13)")?'selected':''?>>NAGALAND (13)</option>
<option value="ODISHA (21)" <?=($infomainaccessuser['purreceivetwopos']=="ODISHA (21)")?'selected':''?>>ODISHA (21)</option>
<option value="OTHER TERRITORY (97)" <?=($infomainaccessuser['purreceivetwopos']=="OTHER TERRITORY (97)")?'selected':''?>>OTHER TERRITORY (97)</option>
<option value="PUDUCHERRY (34)" <?=($infomainaccessuser['purreceivetwopos']=="PUDUCHERRY (34)")?'selected':''?>>PUDUCHERRY (34)</option>
<option value="PUNJAB (3)" <?=($infomainaccessuser['purreceivetwopos']=="PUNJAB (3)")?'selected':''?>>PUNJAB (3)</option>
<option value="RAJASTHAN (8)" <?=($infomainaccessuser['purreceivetwopos']=="RAJASTHAN (8)")?'selected':''?>>RAJASTHAN (8)</option>
<option value="SIKKIM (11)" <?=($infomainaccessuser['purreceivetwopos']=="SIKKIM (11)")?'selected':''?>>SIKKIM (11)</option>
<option value="TAMIL NADU (33)"  <?=($infomainaccessuser['purreceivetwopos']=="TAMIL NADU (33)")?'selected':''?>>TAMIL NADU (33)</option>
<option value="TELANGANA (36)" <?=($infomainaccessuser['purreceivetwopos']=="TELANGANA (36)")?'selected':''?>>TELANGANA (36)</option>
<option value="TRIPURA (16)" <?=($infomainaccessuser['purreceivetwopos']=="TRIPURA (16)")?'selected':''?>>TRIPURA (16)</option>
<option value="UTTAR PRADESH (9)" <?=($infomainaccessuser['purreceivetwopos']=="UTTAR PRADESH (9)")?'selected':''?>>UTTAR PRADESH (9)</option>
<option value="UTTARAKHAND (5)" <?=($infomainaccessuser['purreceivetwopos']=="UTTARAKHAND (5)")?'selected':''?>>UTTARAKHAND (5)</option>
<option value="WEST BENGAL (19)" <?=($infomainaccessuser['purreceivetwopos']=="WEST BENGAL (19)")?'selected':''?>>WEST BENGAL (19)</option>
</select>
</div>
</div>
</div>
...
href="purchasereceives.php" style="margin-top:-3px !important;">Cancel</a>
...
$("#vendor").on("select2:open", function() {
<?php
if($access['purreceivenewvendordef']=='1'){
?>
$("#configureunits").show();
...
$("#product1").on("select2:open", function() {
<?php
if($access['purreceivenewproductdef']=='1'){
?>
$("#configureunits").show();
...
<script type="text/javascript">
$(function() {
$("#purchasereceivesuffix").autocomplete({
source: 'purchasereceivesuffixsearch.php',
select: function(event, ui) {
$("#purchasereceivesuffix").val(ui.item.purchasereceivesuffix);
...
<!------------------------------------------- pro script end ------------------------------------------->
<!--term start-->
<div class="modal fade" id="AddNewPurchaseReceiveTerm" tabindex="-1" role="dialog" style="z-index: 1051;">
<div class="modal-dialog" role="document">
<div class="modal-content">
<div class="modal-header">
<h5 class="modal-title">New <?= $infomainaccessuser['modulename'] ?> Term</h5>
<span type="button" onclick="funespurchasereceiveterm()" class="close" data-dismiss="modal"
aria-label="Close">
...
<div class="form-group row">
<div class="col-sm-6">
<label for="missingpurchasereceiveterm" class="custom-label"><span class="text-danger">
<?= $infomainaccessuser['modulename'] ?> Term *</span></label>
</div>
<div class="col-sm-6">
<input type="text" name="purchasereceiveterm" class="form-control form-control-sm mb-4" id="missingpurchasereceiveterm" placeholder="Name" required>
</div>
</div>
</div>
</div>
...
<div class="col">
<button   onclick="funaddpurchasereceiveterm()" class="btn btn-primary btn-sm btn-custom arlina-button expand-left"   type="submit"  name="submitpurchasereceiveterm" value="Submit">
<span class="label">Save</span> <span class="spinner"></span>
</button>
<button type="button"
class="btn btn-primary btn-sm btn-custom-grey"
onclick="funespurchasereceiveterm()">Cancel</button> </div>
</div>
</div>
</div>
</div>
<script>
$("#purchasereceiveterm").on("change", function() {
var sOptionVal = $(this).val();
if (sOptionVal == '#AddNewPurchaseReceiveTerm') {
$('#AddNewPurchaseReceiveTerm').modal('show');
}
});
function funaddpurchasereceiveterm() {
var missingpurchasereceiveterm = document.getElementById('missingpurchasereceiveterm');
if (missingpurchasereceiveterm.value == '') {
alert('Please Enter New Purchase ReceiveTerm Name');
missingpurchasereceiveterm.focus();
return false;
} else {
...
url: "termadds.php",
data: {
term: $("#missingpurchasereceiveterm").val(),
submit: "Submit"
},
success:function(result){
const resarray = result.split("|");
...
{
$('#purchasereceiveterm').append('<option value="' + missingpurchasereceiveterm.value + '">' + missingpurchasereceiveterm.value +
'</option>');
$('#purchasereceiveterm').val(missingpurchasereceiveterm.value).change();
$("#purchasereceiveterm").select2();
$('#AddNewPurchaseReceiveTerm').modal('hide');
return false;
}
}});
...
function funespurchasereceiveterm() {
//$('#purchasereceiveterm').val('').change();
$("#purchasereceiveterm").select2();
$('#AddNewPurchaseReceiveTerm').modal('hide');
return false;
}
...
$("#purchasereceiveterm").on("select2:open", function() {
$("#configureunits").attr("data-bs-target","#AddNewPurchaseReceiveTerm");
});
$("#purchasereceiveterm").on("select2:open", function() {
document.getElementById("configureunits").innerHTML = "New Purchase ReceiveTerm";
});
</script>
...
<!---payment confirm---->
<script>
function triggerpayment(purchasereceiveno,purchasereceivedate,purchasereceiveamount,cancelstatus)
{
let allAreFilled = true;
document.getElementById("purchasereceiveform").querySelectorAll("[required]").forEach(function(i) {
if (!allAreFilled) return;
if (i.type === "radio") {
let radioValueCheck = false;
document.getElementById("purchasereceiveform").querySelectorAll(`[name=${i.name}]`).forEach(function(r) {
...
{
$('#validpurchasereceiveno').val($('#purchasereceiveno').val());
$('#validpurchasereceivedate').val($('#purchasereceivedate').val());
$('#validpurchasereceiveamount').val($('#grandtotal').val());
