...
if (isset($_POST['submitcustomizes'])) {
	...
	$sqlreportmoduless=$con->prepare("SELECT * FROM pairreportmodules WHERE types='inwcon' ORDER BY id ASC");
	...
	foreach ($newmodules as $newmoduleskey) {
		...
		$modcolumncol=$coltypemod."inwcon";
	...
	$sqlupreport = $con->prepare("UPDATE pairreports SET rowcolumns=? WHERE franchiseid=? AND createdid=? AND types='inwcon'");
	...
	$sqlreport = $con->prepare("UPDATE pairreports SET reportperiod=?,filtername=?,companyname=?,dateprepare=?,timeprepare=? WHERE franchiseid=? AND createdid=? AND types='inwcon'");
	...
	header("Location:inwardreportconsumer.php?datefrom=".date($dateformatforurlredirection,strtotime($from))."&dateto=".date($dateformatforurlredirection,strtotime($to))."");
}

$sqlreportviews=$con->prepare("SELECT * FROM pairreports WHERE franchiseid=? AND createdid=? AND types='inwcon'");
																			...
																			<div id="customizerowcolumns" class="accordion-collapse collapse show" aria-labelledby="customizerowcolumn">
																				<div class="accordion-body text-sm">
																					<div class="row justify-content-center">
																						...
																						<?php
																							$newans=array();
																							$sqlreportaccesss=$con->prepare("SELECT * FROM pairreports WHERE franchiseid=? AND createdid=? AND types='inwcon'");
																								...
																								<div class="col-lg-6 my-1" style="padding-left: 0px !important;">
																									<div class="custom-control custom-checkbox mr-sm-2" style="padding: 0px 12px !important;">
																										<div class="input-group input-group-sm" id="flaghead">
																											...
																											<input type="checkbox" class="custom-control-input" name="<?= $coltypemod; ?>inwcon" id="<?= $coltypemod; ?>inwcon" checked disabled>
																											<label class="custom-control-label custom-label" for="<?= $coltypemod; ?>inwcon">
											...
											<div class="tab-pane fade" id="nav-profile" role="tabpanel" aria-labelledby="nav-profile-tab">
												<div class="table-responsive m-3" id="histables">
													<table class="table table-bordered">
														...
														<tbody>
      												<?php
															$sqluse=$con->prepare("SELECT * FROM pairusehistory WHERE usetype = 'REPinwcon' AND useid = ? ORDER BY createdon DESC");
																		...
																		<tr style="height:1px;">
																			<td width="100%" style="padding:10px;">
																				<table id="print-are1" style="border:1px solid #eee;height: 975px;" width="100%">
																					...
																					<tbody id="myTable">
																					<?php
																						$sqls=$con->prepare("SELECT * FROM pairbills WHERE franchisesession=? AND createdid=? AND (billdate>=? AND billdate<=?) AND cancelstatus='0' AND ".(($sqlviewreport['gstrule']=='0')?'gstrtype="Consumer"':'(gstrtype="Registered Business - Regular" OR gstrtype="Consumer") AND gstno="" ')." GROUP BY billdate, billno ORDER BY billdate ASC, billno ASC LIMIT 15");
																					...
																					<?php
																						...
																						$sqltotlistpages=$con->prepare("SELECT * FROM pairbills WHERE franchisesession=? AND createdid=? AND (billdate>=? AND billdate<=?) AND cancelstatus='0' AND ".(($sqlviewreport['gstrule']=='0')?'gstrtype="Consumer"':'(gstrtype="Registered Business - Regular" OR gstrtype="Consumer") AND gstno="" ')." GROUP BY billdate, billno");
...
const pagechanges = (page, pageSize) => {
	...
	ajaxRequestview = $.ajax({
		type: "GET",
		url: 'reportviewsearch.php?lastornone='+lastornone+'&term='+perpages+'&limitings='+($('#limitforpagenum').val())+'&datefrom=<?=$datefrom?>&dateto=<?=$dateto?>&dif=inwardcon',
...
$(function() {
	...
	function(start, end, label) {
		...
		window.open("inwardreportconsumer.php?datefrom="+datefrom+"&dateto="+dateto+"","_self");
	});
});
...
function converHTMLFileToPDF() {
	...
	var finaltimer = setTimeout(function(){
		...
		$.ajax({
			type: "GET",
			url: 'reporthistory.php?types=inwcon&titles=Purchases Received From Unregistered (Consumer) Taxpayers&printdownpdfcsv=Printed PDF&fromto='+fromtopdf+'',
			success: function (result) {
				console.log(result);
			},
			error: function (error) {
...
function downloadaspdf() {
	...
	var finaltimer = setTimeout(function(){
		...
		$.ajax({
			type: "GET",
			url: 'reporthistory.php?types=inwcon&titles=Purchases Received From Unregistered (Consumer) Taxpayers&printdownpdfcsv=Downloaded PDF&fromto='+fromtopdf+'',
			success: function (result) {
				console.log(result);
			},
			error: function (error) {
...
function downloadascsv() {
	...
	var finaltimer = setTimeout(function(){
		...
		$.ajax({
			type: "GET",
			url: 'reporthistory.php?types=inwcon&titles=Purchases Received From Unregistered (Consumer) Taxpayers&printdownpdfcsv=Downloaded CSV&fromto='+fromtopdf+'',
			success: function (result) {
				console.log(result);
			},
			error: function (error) {
...
document.addEventListener('DOMContentLoaded', function() {
	...
	function makeAjaxRequest() {
		ajaxRequest = $.ajax({
			type: "GET",
			url: 'check.php?term=inwardcon&datefrom=<?=$datefrom?>&dateto=<?=$dateto?>&limitfrom=0&limitto=10000',
			success: function (result) {
				if (result != mytabprev) {
		...
		ajaxCsvRequest = $.ajax({
			type: "GET",
			url: 'reportcsv.php?term=inwardcon&datefrom=<?=$datefrom?>&dateto=<?=$dateto?>',
			success: function (result) {
