...
if (isset($_POST['submitcustomizes'])) {
	...
	$sqlreportmoduless=$con->prepare("SELECT * FROM pairreportmodules WHERE types='stockinhandwithvalue' ORDER BY id ASC");
	...
	foreach ($newmodules as $newmoduleskey) {
		...
		$modcolumncol=$coltypemod."stockinhandwithvalue";
	...
	$sqlupreport = $con->prepare("UPDATE pairreports SET rowcolumns=? WHERE franchiseid=? AND createdid=? AND types='stockinhandwithvalue'");
	...
	$sqlreportfav = $con->prepare("UPDATE pairreportfavourites SET reporturl=? WHERE franchisesession=? AND createdid=? AND reportnames='stockinhandwithvalue'");
	$sqlreportfav->bind_param("sss", $newReportUrl, $_SESSION['franchisesession'], $companymainid);
	$sqlreportfav->execute();
	$sqlreportfav->close();

	$sqlreport = $con->prepare("UPDATE pairreports SET proid=?,proname=?,reportperiod=?,filtername=?,companyname=?,dateprepare=?,timeprepare=?,gstrule=? WHERE franchiseid=? AND createdid=? AND types='stockinhandwithvalue'");
	...
	header("Location:reportstockinhandwithvalue.php?datefrom=".date($dateformatforurlredirection,strtotime($from))."&dateto=".date($dateformatforurlredirection,strtotime($to))."&proid=".$proid."");
}

$sqlreportviews=$con->prepare("SELECT * FROM pairreports WHERE franchiseid=? AND createdid=? AND types='stockinhandwithvalue'");
								...
								<form action="" method="post" enctype="multipart/form-data" class="form-horizontal mt-0" role="form">
									<div class="row">
																			...
																			<div id="customizerowcolumns" class="accordion-collapse collapse show" aria-labelledby="customizerowcolumn">
																				<div class="accordion-body text-sm">
																					<div class="row justify-content-center">
																						...
																						<?php
																							$newans=array();
																							$sqlreportaccesss=$con->prepare("SELECT * FROM pairreports WHERE franchiseid=? AND createdid=? AND types='stockinhandwithvalue'");
																								...
																								<div class="col-lg-6 my-1" style="padding-left: 0px !important;">
																									<div class="custom-control custom-checkbox mr-sm-2" style="padding: 0px 12px !important;">
																										<div class="input-group input-group-sm" id="flaghead">
																											...
																											<input type="checkbox" class="custom-control-input" name="<?= $coltypemod; ?>stockinhandwithvalue" id="<?= $coltypemod; ?>stockinhandwithvalue" checked disabled>
																											<label class="custom-control-label custom-label" for="<?= $coltypemod; ?>stockinhandwithvalue">
											...
											<div class="tab-pane fade" id="nav-profile" role="tabpanel" aria-labelledby="nav-profile-tab">
												<div class="table-responsive m-3" id="histables">
													<table class="table table-bordered">
														...
														<tbody>
														<?php
															$sqluse=$con->prepare("SELECT * FROM pairusehistory WHERE usetype = 'REPstockinhandwithvalue' AND useid = ? ORDER BY createdon DESC");
																		...
																		<tr style="height:1px;">
																			<td width="100%" style="padding:10px;">
																				<table id="print-are1" style="border: 1px solid #eee;" width="100%">
																					<thead>
																						<tr>
																							<td class="text-uppercase" style="border:1px solid #eee;padding-left: 10px;">
																							...
																							<td class="text-uppercase" style="border:1px solid #eee;padding-right: 10px;text-align: right;">
																								<span style="font-size:13px;color:black;">
																									<?=$access['txtqtybill']?>
																								</span>
																							</td>
																					...
																					<?php
																						...
																						$allTax = 0;
																						$allAmount = 0;
																						$selbatdata=$con->prepare("SELECT batch, expdate, quantity FROM pairbatch WHERE createdid=? and franchisesession=? ".(($sqlviewreport['proid']=='all')?' ':' AND productid="'.$sqlviewreport['proid'].'"')." ORDER BY productname ASC LIMIT 35");
										      									$selbatdata->bind_param("ss", $companymainid, $_SESSION['franchisesession']);
										      									$selbatdata->execute();
										      									$selbat = $selbatdata->get_result();
										      									while($infobat=$selbat->fetch_array()){
																							$seldatas=$con->prepare("SELECT * FROM pairbills WHERE franchisesession=? AND createdid=? AND (billdate>=? AND billdate<=?) AND cancelstatus='0' ".(($sqlviewreport['proid']=='all')?' ':' AND productid="'.$sqlviewreport['proid'].'"')." ORDER BY productname ASC");
																					...
																					?>
																						<tr style="vertical-align: middle; height: 30px !important;">
																							...
																							<td style="text-align: left !important; font-size:12px; color:black; border:1px solid #eee; padding-left: 10px;">
																								<?= (($infobat['expdate']!='')?date($datemainphp,strtotime($infobat['expdate'])):'') ?>
																							</td>
																							<td style="text-align: left !important; font-size:12px; color:black; border:1px solid #eee; padding-left: 10px;">
																								<?= $info['productrate'] ?>
																							</td>
																					...
																					<?php
																						...
																						$sqltotlistpages=$con->prepare("SELECT id FROM pairbatch WHERE createdid=? and franchisesession=? ".(($sqlviewreport['proid']=='all')?' ':' AND productid="'.$sqlviewreport['proid'].'"')." ORDER BY productname ASC");
																						$sqltotlistpages->bind_param("ss", $_SESSION['franchisesession'], $companymainid);
																					...
																					?>
																						<tr style="height: 30px;font-weight: bold;">
																							<td style="border: 1px solid #eee;padding-left: 10px;" colspan="7">
																								Total
																							</td>
										...
										<table id="print-are3" style="border: 1px solid #eee;display: none;" width="100%">
											<thead>
												<tr>
													<td class="text-uppercase" style="border:1px solid #eee;padding-left: 10px;"><span style="font-size:13px;color:black;"><?=$infomainaccessuserbill['modulename']?> Number</span>
													</td>
													<td class="text-uppercase" style="border:1px solid #eee;padding-left: 10px;"><span style="font-size:13px;color:black;"><?=$infomainaccessuserbill['modulename']?> Date</span>
													</td>
													<td class="text-uppercase" style="border:1px solid #eee;padding-left: 10px;"><span style="font-size:13px;color:black;">Vendor Name</span>
													</td>
													<td class="text-uppercase" style="border:1px solid #eee;padding-left: 10px;">
														<span style="font-size:13px;color:black;">
															Batch
													...
													<td class="text-uppercase" style="border:1px solid #eee;padding-right: 10px;text-align: right;"><span style="font-size:13px;color:black;"><?=$access['txtqtybill']?></span>
													</td>
													<td class="text-uppercase" style="text-align: right !important;border:1px solid #eee;padding-right: 10px;"><span style="font-size:13px;color:black;white-space: nowrap;overflow: hidden;">Value</span>
													</td>
													<td class="text-uppercase" style="text-align: right !important;border:1px solid #eee;padding-right: 10px;"><span style="font-size:13px;color:black;white-space: nowrap;overflow: hidden;">Tax</span>
													</td>
													<td class="text-uppercase" style="text-align: right !important;border:1px solid #eee;padding-right: 10px;"><span style="font-size:13px;color:black;white-space: nowrap;overflow: hidden;">Amount</span>
													</td>
												</tr>
											</thead>
											<tbody id="myTables">
...
const pagechanges = (page, pageSize) => {
	...
	ajaxRequestview = $.ajax({
		type: "GET",
		url: 'reportviewsearch.php?term='+perpages+'&limitings='+($('#limitforpagenum').val())+'&datefrom=<?=$datefrom?>&dateto=<?=$dateto?>&dif=stockinhandwithvalue&proid=<?=$_GET['proid']?>&lastornone='+lastornone+'',
...
$(function() {
	...
	function(start, end, label) {
		...
		window.open("reportstockinhandwithvalue.php?datefrom="+datefrom+"&dateto="+dateto+"&proid=<?=$_GET['proid']?>","_self");
	});
});
...
function converHTMLFileToPDF() {
	...
	var finaltimer = setTimeout(function(){
		...
		var header = function(data) {
			...
			var headtext1 = "Stock in Hand with Value";
			var fontSize = doc.internal.getFontSize();
			var pageWidth = doc.internal.pageSize.width;
		...
		doc.autoTable({
			...
			didDrawPage: header, 
			columnStyles: {
				0: { halign: 'left', cellWidth: 51},
				1: { halign: 'left', cellWidth: 51},
				2: { halign: 'left', cellWidth: 51},
				3: { halign: 'left', cellWidth: 51},
				4: { halign: 'left', cellWidth: 51},
		...
		$.ajax({
			type: "GET",
			url: 'reporthistory.php?types=stockinhandwithvalue&titles=Stock in Hand with Value&printdownpdfcsv=Printed PDF&fromto='+fromtopdf+'',
			success: function (result) {
				console.log(result);
			},
			error: function (error) {
...
function downloadaspdf() {
	...
	var finaltimer = setTimeout(function(){
		...
		$.ajax({
			type: "GET",
			url: 'reporthistory.php?types=stockinhandwithvalue&titles=Stock in Hand with Value&printdownpdfcsv=Downloaded PDF&fromto='+fromtopdf+'',
			success: function (result) {
				console.log(result);
			},
			error: function (error) {
		...
		doc.save('Stock-in-Hand-with-Value-<?=$datefrom?>-To-<?=$dateto?>');
...
function downloadascsv() {
	...
	var finaltimer = setTimeout(function(){
		...
		data.push(["Stock in Hand with Value"]);
		...
		link.setAttribute("download", "Stock-in-Hand-with-Value-<?=$datefrom?>-To-<?=$dateto?>");
		document.body.appendChild(link);
		link.click();
		$.ajax({
			type: "GET",
			url: 'reporthistory.php?types=stockinhandwithvalue&titles=Stock in Hand with Value&printdownpdfcsv=Downloaded CSV&fromto='+fromtopdf+'',
			success: function (result) {
				console.log(result);
			},
			error: function (error) {
...
document.addEventListener('DOMContentLoaded', function() {
	...
	function makeAjaxRequest() {
		ajaxRequest = $.ajax({
			type: "GET",
			url: 'check.php?term=stockinhandwithvalue&datefrom=<?=$datefrom?>&dateto=<?=$dateto?>&limitfrom=0&limitto=10000&proid=<?=$_GET['proid']?>',
			success: function (result) {
				if (result != mytabprev) {
