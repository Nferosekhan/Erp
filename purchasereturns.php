		  ...
		  <div class="col-12">
			 <div class="card mb-4 mt-5">
				<div class="card-body p-3" style="color:black;font-family:'Myriad Set Pro','Helvetica Neue',Helvetica,Arial,sans-serif;">
				  <div class="row">
						  ...
						  <div class="app-utility-item app-user-dropdown dropdown">
							 ...
							 <div class="dropdown-menu  dropdown-menu-end customdropdown  me-sm-2" aria-labelledby="dropdownMenuButton111" style="max-height: 0px;max-width: 0px;">
								<div style="background-color: #3c3c46;margin-top: -39px !important;position: relative !important;left: 145px !important;" id="dropdownMenuButton111check">
								  <a class="nav-link <?=(($infomainaccessuser['filtertypesforlistsorting']=='All')?'active':'')?>" href="#" style="color: #fff;" onclick="submitthesorting('All')">
									 <!-- <i class="fa fa-sign-out-alt"></i> -->
									 <span class="nav-link-text ms-2"> All</span>
								  </a>
								  <a class="nav-link <?=(($infomainaccessuser['filtertypesforlistsorting']=='Paid')?'active':'')?>" href="#" style="color: #fff;" onclick="submitthesorting('Paid')">
									 <!-- <i class="fa fa-sign-out-alt"></i> -->
									 <span class="nav-link-text ms-2"> Paid</span>
								  </a>
								  <a class="nav-link <?=(($infomainaccessuser['filtertypesforlistsorting']=='Pending')?'active':'')?>" href="#" style="color: #fff;" onclick="submitthesorting('Pending')">
									 <!-- <i class="fa fa-sign-out-alt"></i> -->
									 <span class="nav-link-text ms-2"> Pending</span>
								  </a>
								  <a class="nav-link <?=(($infomainaccessuser['filtertypesforlistsorting']=='Partially Paid')?'active':'')?>" href="#" style="color: #fff;" onclick="submitthesorting('Partially Paid')">
									 <!-- <i class="fa fa-sign-out-alt"></i> -->
									 <span class="nav-link-text ms-2"> Partially Paid</span>
								  </a>
								  <a class="nav-link <?=(($infomainaccessuser['filtertypesforlistsorting']=='Void')?'active':'')?>" href="#" style="color: #fff;" onclick="submitthesorting('Void')">
									 <!-- <i class="fa fa-sign-out-alt"></i> -->
					 ...
					 <div class="col-lg-3" style="margin-bottom: 57px !important;"> 
						<input class="form-control" id="purchasereturnsearch" type="text" placeholder="Search.." style="height:24px;">
					 </div> 
				  </div>
				<?php
				  $sqlismainaccessusers=$con->prepare("SELECT * FROM pairmainaccess WHERE userid=? AND moduletype='Purchase Returns' ORDER BY id ASC");
				  ...
				  <div align="right" class=" p-2 addandmenuonlyforimports" style="margin-bottom: -30px;">
					 <div class="row" style="width:250px;">           
						<div class="col-8" style="visibility: hidden;">  
						  <a href="purchasereturnadd.php" class="btn btn-custom btn-sm p-2 add" style="font-size: 13px;height: 24px;margin-bottom:1rem;margin-top: -151px;margin-right:-39px;padding-right: 5px;">
						...
						<div class="col-4">
						  ...
						  ?>
							 <ul class="dropdown-menu dropdown-menu-end" aria-labelledby="dropdownMenuButton">
								<li>
								  <a class="dropdown-item" href="purchasereturnimport.php">
				...
				<?php
				  ...
				  $sqlismainaccessusers=$con->prepare("SELECT * FROM pairmainaccess WHERE franchiseid=? AND moduletype='Purchase Returns' ORDER BY id ASC");
					 $sqlismainaccessusers->bind_param("i", $_SESSION['franchisesession']);
					 $sqlismainaccessusers->execute();
					 $sqlismainaccessuser = $sqlismainaccessusers->get_result();
					 $infomainaccessuser=$sqlismainaccessuser->fetch_array();
					 //FOR GET MODULE NAME OF PURCHASERETURN
		...
		<div class="tab-pane fade show active p-3" id="nav-general" role="tabpanel" aria-labelledby="nav-general-tab">
				  <div class="table-responsive p-0 min-height-480">
					 <table id="someTable" class="table table-bordered align-items-center mb-0" style="table-layout: fixed;">
						...
						<tbody id="myTable">
						<?php
									 //FOR PURCHASERETURN DATA
						  $totalcancel=array();
						  $totalpurchasereturnno=array();
						  $totalpurchasereturndate=array();
									$sqls=$con->prepare("SELECT id,purchasereturndate, purchasereturnno, vendorname, purchasereturnterm, duedate, purchasereturnamount, cancelstatus FROM pairpurchasereturns WHERE franchisesession=? AND createdid=? ".$typesforlisting." GROUP BY purchasereturndate, purchasereturnno ORDER BY purchasereturndate DESC,ordering DESC LIMIT ".(($access['purreturnpaypageload']=='pagenum')?'10':'15')."");
						  ...
						  while($info=$sql->fetch_array()){
							 $purchasereturnamount+=(float)$info['purchasereturnamount'];
							 $currentamount=(float)$info['purchasereturnamount'];
							 $paidamount=0;
							 $currentbalance=0;
								...
								$sqlpurchasepays->bind_param("ssss", $_SESSION['franchisesession'], $companymainid, $info['purchasereturnno'], $info['purchasereturndate']);
							 ...
							 $currentbalance=((float)$info['purchasereturnamount']-$paidamount);
							 $balanceamount+=((float)$info['purchasereturnamount']-$paidamount);
							 $totalcancel[]=$info['cancelstatus'];
							 $totalpurchasereturnno[]=$info['purchasereturnno'];
							 $totalpurchasereturndate[]=$info['purchasereturndate'];
						...
						?>
						  ...
						  <?php
							 ...
							 if ((in_array('Date', $modulecolumns))) {
						  ?>
							 <td onclick="window.open('purchasereturnview.php?id=<?=$info['id']?>&purchasereturnno=<?=$info['purchasereturnno']?>&purchasereturndate=<?=$info['purchasereturndate']?>', '_self')"  data-label="Date">
								<?=(($info['purchasereturndate']!='')?(date($date,strtotime($info['purchasereturndate']))):'&nbsp;')?>
							 </td>
						  <?php
							 }
							 if ((in_array('No', $modulecolumns))) {
						  ?>
							 <td onclick="window.open('purchasereturnview.php?id=<?=$info['id']?>&purchasereturnno=<?=$info['purchasereturnno']?>&purchasereturndate=<?=$info['purchasereturndate']?>', '_self')"  data-label="Number">
								<?=(($info['purchasereturnno']=='')?'&nbsp;':'')?><?=$info['purchasereturnno']?>
							 </td>
						  <?php
							 }
							 // if ((in_array('Vendor Name', $modulecolumns))) {
						  ?>
							 <td onclick="window.open('purchasereturnview.php?id=<?=$info['id']?>&purchasereturnno=<?=$info['purchasereturnno']?>&purchasereturndate=<?=$info['purchasereturndate']?>', '_self')"  data-label="Name">
						  ...
						  <?php
							 ...
							 if ((in_array('Term', $modulecolumns))) {
						  ?>
							 <td onclick="window.open('purchasereturnview.php?id=<?=$info['id']?>&purchasereturnno=<?=$info['purchasereturnno']?>&purchasereturndate=<?=$info['purchasereturndate']?>', '_self')"  data-label="Term">
								<?=$info['purchasereturnterm']?>
							 </td>
						  <?php
							 }
							 if ((in_array('Amount', $modulecolumns))) {
						  ?>
							 <td onclick="window.open('purchasereturnview.php?id=<?=$info['id']?>&purchasereturnno=<?=$info['purchasereturnno']?>&purchasereturndate=<?=$info['purchasereturndate']?>', '_self')"  data-label="Amount">
								<i class="fa fa-rupee"></i>
								<?=number_format((float)$info['purchasereturnamount'],2,'.','')?>
							 </td>
						  <?php
							 }
							 if ((in_array('Status', $modulecolumns))) {
								if($info['cancelstatus']=='1'){
						  ?>
							 <td onclick="window.open('purchasereturnview.php?id=<?=$info['id']?>&purchasereturnno=<?=$info['purchasereturnno']?>&purchasereturndate=<?=$info['purchasereturndate']?>', '_self')"  data-label="Status" style="color:#bbbbbb;text-decoration: none;">
								Void
							 </td>
							 <?php
								}
							 ...
							 ?>
							 <td onclick="window.open('purchasereturnview.php?id=<?=$info['id']?>&purchasereturnno=<?=$info['purchasereturnno']?>&purchasereturndate=<?=$info['purchasereturndate']?>', '_self')"  data-label="Status" class="text-success">
								Paid
							 </td>
								<?php
								  }
								...
								?>
							 <td onclick="window.open('purchasereturnview.php?id=<?=$info['id']?>&purchasereturnno=<?=$info['purchasereturnno']?>&purchasereturndate=<?=$info['purchasereturndate']?>', '_self')"  data-label="Status" class="text-danger">
								Pending
							 </td>
								  <?php
									 }
								  ...
								  ?>
							 <td onclick="window.open('purchasereturnview.php?id=<?=$info['id']?>&purchasereturnno=<?=$info['purchasereturnno']?>&purchasereturndate=<?=$info['purchasereturndate']?>', '_self')"  data-label="Status" class="text-warning">
								Partially Paid
							 </td>
						  <?php
									 }
							 ...
							 if ((in_array('Balance', $modulecolumns))) {
						  ?>
							 <td onclick="window.open('purchasereturnview.php?id=<?=$info['id']?>&purchasereturnno=<?=$info['purchasereturnno']?>&purchasereturndate=<?=$info['purchasereturndate']?>', '_self')"  data-label="Balance">
								<i class="fa fa-rupee"></i>
						  ...
						  ?>
							 <td data-label="Print" class="">&nbsp;
								<a target="_blank" href="purchasereturnprint.php?purchasereturnno=<?=$info['purchasereturnno']?>&purchasereturndate=<?=$info['purchasereturndate']?>" class="text-secondary font-weight-bold text-xs" data-toggle="tooltip" data-original-title="Edit user">
								  Print
								</a>
							 </td>
						  <?php
						  ...
						  ?>
							 <td data-label="Edit">
								<a href="purchasereturnedit.php?purchasereturnno=<?=$info['purchasereturnno']?>&purchasereturndate=<?=$info['purchasereturndate']?>" class="text-secondary font-weight-bold text-xs">
						...
						<?php
						  ...
						  $sqltotlists=$con->prepare("SELECT COUNT(DISTINCT purchasereturnno) FROM pairpurchasereturns WHERE franchisesession=? AND createdid=? ".$typesforlisting."");
				  ...
				  <?php
					 if ($access['purreturnpaypageload']=='pageauto') {
				  ?>
					 <script type="text/javascript">
					 ...
					 $('.main-content').on('scroll', function() {
						...
						if (scrollTop + $(this).innerHeight() >= this.scrollHeight-50) {
						  if (isPreviousEventComplete && isDataAvailable) {
							 ...
							 $.ajax({
								type: "GET",
								url: 'listpurchasereturnsearch.php?term=' + sIndex + '&limitings=15&typesforlisting=<?=urlencode($typesforlisting)?>',
				  ...
				  ?>
					 ...
					 try{
						...
						const pagechanges = (page, pageSize) => {
						  ...
						  $.ajax({
							 type: "GET",
							 url: 'listpurchasereturnsearch.php?term='+perpages+'&limitings='+($('#limitforpagenum').val())+'&typesforlisting=<?=urlencode($typesforlisting)?>',
							 success: function (result) {
								$("#myTable").html(result);
							 },
							 error: function (error) {
...
function submitthesorting(value) {
  $.ajax({
	 type: "GET",
	 url: 'listingsortfilters.php?moduletype=Purchase Returns&typesforlisting='+value+'',
	 success: function (result) {
		console.log(result);
		window.location = 'purchasereturns.php?remarks='+result+'';
	 },
	 error: function (error) {
		console.log(error);
	 }
...
<script>
function deleteitem(purchasereturnno,purchasereturndate,cancelstatus){
  $('#deleteconfirm-adddelete').modal('show');
  $("#deleteconfirm-adddelete #deleteitem").attr("href","purchasereturncancel.php?purchasereturnno="+purchasereturnno+"&purchasereturndate="+purchasereturndate+"&cancelstatus="+cancelstatus);
}
//FOR DELETE ITEM
</script>
</body>
