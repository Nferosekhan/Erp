<?php
include('lcheck.php');
 if((isset($_GET['purchasereceiveno']))&&(isset($_GET['purchasereceivedate'])))
 {
     $purchasereceiveno=mysqli_real_escape_string($con, $_GET['purchasereceiveno']);
     $purchasereceivedate=mysqli_real_escape_string($con, $_GET['purchasereceivedate']);
 $sql=mysqli_query($con, "select * from pairpurchasereceives where franchisesession='".$_SESSION['franchisesession']."' and createdid='$companymainid' and purchasereceiveno='$purchasereceiveno' and purchasereceivedate='$purchasereceivedate' order by id asc");
...
if ((in_array('Term', $fieldedit))) {
$purchasereceiveterm=mysqli_real_escape_string($con, $_POST['purchasereceiveterm']);
}
else{
$purchasereceiveterm="";
}
$oldpurchasereceiveno=mysqli_real_escape_string($con, $_GET['purchasereceiveno']);
$oldpurchasereceivedate=mysqli_real_escape_string($con, $_GET['purchasereceivedate']);
...
if ((in_array('Due Date', $fieldedit))) {
...
$vendorid = mysqli_real_escape_string($con, $_POST['oldvendorname']);
$sqlup = "update paircustomers set createdon='$times', createdid='$companymainid', createdby='".$_SESSION["unqwerty"]."', franchisesession='".$_SESSION["franchisesession"]."', customername='$vendorname',billstreet='$area',billcity='$city',billstate='$state',billpincode='$pincode',billcountry='$district',shipstreet='$sarea',shipcity='$scity',shipstate='$sstate',shippincode='$spincode',shipcountry='$sdistrict',sameasbilling='$twosameasbilling',gstrtype='$gstrtype',gstin='$gstno',moduletype='Vendors',placeos='$pos',workphone='$twoworkphone',mobile='$twomobilephone' where id='$vendorid'";
...
if($productname!='')
{
$sql=mysqli_query($con, "delete from pairpurchasereceives where createdid='$companymainid' and franchisesession='".$_SESSION["franchisesession"]."' and purchasereceiveno='$oldpurchasereceiveno' and purchasereceivedate='$oldpurchasereceivedate'");
if($sql)
{
}
else
...
$sql2=mysqli_query($con, "select id from pairpurchasereceives where franchisesession='".$_SESSION['franchisesession']."' and createdid='$companymainid' and purchasereceiveno='$oldpurchasereceiveno' and purchasereceivedate='$oldpurchasereceivedate'");
...
echo '<script> window.location.href="purchasereceives.php?remarks=Updated Successfully";</script>'; 
}
else
{
echo '<script> window.location.href="purchasereceiveview.php?purchasereceiveno='.$purchasereceiveno.'&purchasereceivedate='.$purchasereceivedate.'&remarks=Updated Successfully";</script>';
}
}
}
else
...
if ($sql) {
$hissql=mysqli_query($con, "insert into pairusehistory set usetype='PURCHASERECEIVE', createdon='$times',  createdby='".$_SESSION["unqwerty"]."', useid='$purchasereceiveno', useremarks='Purchase Receive Updated' ");
}
}
...
<input type="hidden" name="oldpurchasereceiveno" value="<?=$rows[0]['purchasereceiveno']?>">
<input type="hidden" name="oldpurchasereceivedate" value="<?=$rows[0]['purchasereceivedate']?>">
<input type="hidden" name="purchasereceiveterm" value="<?=$rows[0]['purchasereceiveterm']?>">

<div class="row">
<?php
     if ((in_array('Purchase Receive Information', $fieldedit))) {
        ?>
<div class="col-lg-4">
...
<div class="col-lg-7">
<input type="text" class="form-control  form-control-sm" id="purchasereceiveno" name="purchasereceiveno" required  value="<?=$rows[0]['purchasereceiveno']?>" readonly>
</div>
</div>
</div>
<div class="col-lg-12">
...
<div class="col-lg-7">
<input type="date" class="form-control  form-control-sm" id="purchasereceivedate" name="purchasereceivedate" required value="<?=$rows[0]['purchasereceivedate']?>">
</div>
</div>
</div>
...
?>
<option value="<?= $info['term'] ?>" <?=($info['term']==$rows[0]['purchasereceiveterm'])?'selected':''?>><?= $info['term'] ?></option>
<?php
}
?>
</select>
...
<input type="date" class="form-control  form-control-sm" id="duedate" name="duedate" style="padding: 3px !important;" value="<?=$rows[0]['duedate']?>">
</div>
<script>
...
<?php
$sqli = mysqli_query($con, "SELECT id, customername, city, mobile, workphone From paircustomers WHERE franchisesession='".$_SESSION["franchisesession"]."' and (createdid='$companymainid' and moduletype='Vendors') order by customername asc");
while ($info = mysqli_fetch_array($sqli))
{
$sqlibill=mysqli_query($con, "select purchasereceiveamount, purchasereceivedate, purchasereceiveno from pairpurchasereceives where franchisesession='".$_SESSION['franchisesession']."' and createdid='$companymainid' and vendorid='".$info['id']."' GROUP BY purchasereceivedate, purchasereceiveno order by purchasereceivedate desc, purchasereceiveno desc");
$billamount=0;
$balanceamount=0;
$currentamount=0;
$overdueamount=0;
...
{
$billamount+=(float)$infobill['purchasereceiveamount'];
$paidamount=0;
$sqlpurchasepay=mysqli_query($con,"select amount from pairpurchasepayhistory where franchisesession='".$_SESSION['franchisesession']."' and createdid='$companymainid' and billno='".$infobill['purchasereceiveno']."' and billdate='".$infobill['purchasereceivedate']."' and vendorid='".$info['id']."' order by id desc");
while($infopurchasepay=mysqli_fetch_array($sqlpurchasepay))
{
$paidamount+=(float)$infopurchasepay['amount'];
}
$balanceamount+=((float)$infobill['purchasereceiveamount']-$paidamount);
$diff = abs(time() - strtotime($infobill['purchasereceivedate']));
$days = floor(($diff)/ (60*60*24));
if($days>30)
{
$overdueamount+=((float)$infobill['purchasereceiveamount']-$paidamount);
}
else
{
$currentamount+=((float)$infobill['purchasereceiveamount']-$paidamount);
}
}
?>
<option value="<?=$info['id']?>" data-foo="<?=$info['workphone']?>" data-receivable="<?php echo $rescurrency[0]; ?> <?=number_format((float)$balanceamount, 2, ".", "")?>" <?=($rows[0]['vendorid']==$info['id'])?'selected':''?> ><?=$info['customername']?></option>
<?php
}
?>
...
<div class="col-lg-9">
<input type="text" name="twovendorname" id="twovendorname" class="form-control form-control-sm" placeholder="<?= $infomainaccessuserven['modulename'] ?> Name" <?= ($access['purreceivebtwocnamerequired']=='Yes')? 'required': ' ' ?> value="<?= $rows[0]['vendorname'] ?>">
...
<input type="text" class="form-control form-control-sm" id="twoworkphone" name="twoworkphone" placeholder="WORK PHONE" value="<?=$rows[0]['workphone']?>" <?= ($access['purreceivebtwocwphonerequired']=='Yes')? 'required': ' ' ?>>
...
<label for="twogstin" class="custom-label" style="margin:0px !important;font-size: 12px !important;color: #777777;">GST IN</label>
<input type="text" class="form-control form-control-sm" id="twogstin" name="twogstin" placeholder="GST IN" value="<?=$rows[0]['gstno']?>">
</div>
</div>
</div>
...
header("Location:purchasereceives.php?error=No Information Found");
}
}
else{
header("Location:purchasereceives.php?error=No Information Found");  
