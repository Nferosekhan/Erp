...
if (isset($_POST['tonicsubmit'])) {
	...
	$tonicpairmodules = mysqli_query($toniccon,"UPDATE pairmodules SET modulecolumns = 'Business Overview,Horizontal Balance Sheet,Horizontal Profit and Loss,Account Transactions,Receivables or Who owes you,Invoice List or Details,Payments Received,Payables or What you owe,Bills List or Details,Payments Made,Taxes,Summary of Inward Supplies opparen GSTR hypens 2 closparen,Summary of Outward Supplies opparen GSTR hypens 1 closparen,Summary of Credit Note,Summary of Debit Note,Customer Balance,Balance,Sales Details,Sales,Product Customer Wise Sales,Product Movement,Sales Person,Profit And Loss,Stock And Sales Statement,Stock And Sales Statement New,Purchase Details,Stock in Hand with Value,Accounts,Journal,Accounts Transactions' WHERE id = 33;");
	...
	while($tonicrow = mysqli_fetch_array($tonicresult)){
		...
		if ($tonicrow['franchises']!='') {
			...
			if (!empty($tonicfranchiseValues)) {
				foreach ($tonicfranchiseValues as $tonicfranchiseValue) {
					if (!empty($tonicfranchiseValue)) {
						...
						$tonicreportone = mysqli_query($toniccon,"SELECT * FROM pairreports WHERE createdid='$toniccompanymainid' AND franchiseid='".$tonicfranchiseValue."' AND types='stockinhandwithvalue'");

						if (mysqli_num_rows($tonicreportone)==0) {

							$tonicpairreports = mysqli_query($toniccon,"INSERT INTO pairreports SET createdid='$toniccompanymainid', createdby='".$tonicrow['username']."', username='".$tonicrow['username']."', franchiseid='".$tonicfranchiseValue."', reportperiod='thismonth', filtername='1', names='all',proid='all',category='all',categoryname='', gstrule='0', cusvenname='', companyname='1', dateprepare='1', timeprepare='1', types='stockinhandwithvalue', rowcolumns='', rowcolumnsorder=''");

							$tonicpairreportfavourites = mysqli_query($toniccon,"INSERT INTO pairreportfavourites SET createdid='$toniccompanymainid', franchisesession='".$tonicfranchiseValue."', reportnames='stockinhandwithvalue', reporturlname='reportstockinhandwithvalue', reportstatus='0', reportoriginals='Stock in Hand with Value', reportfunctions='favouritestockinhandwithvalue', reporturl='reportstockinhandwithvalue.php?period=thismonth&proid=all', reporthref=''");
			...
			$tonicreportone = mysqli_query($toniccon,"SELECT * FROM pairreports WHERE createdid='$toniccompanymainid' AND types='stockinhandwithvalue'");

			if (mysqli_num_rows($tonicreportone)==0) {

				$tonicpairreports = mysqli_query($toniccon,"INSERT INTO pairreports SET createdid='$toniccompanymainid', createdby='".$tonicrow['username']."', username='".$tonicrow['username']."', reportperiod='thismonth', filtername='1', names='all',proid='all',category='all',categoryname='', gstrule='0', cusvenname='', companyname='1', dateprepare='1', timeprepare='1', types='stockinhandwithvalue', rowcolumns='', rowcolumnsorder='Description,IGST Amount,CGST Amount,SGST Amount,Credit Note Total'");

				$tonicpairreportfavourites = mysqli_query($toniccon,"INSERT INTO pairreportfavourites SET createdid='$toniccompanymainid',  reportnames='stockinhandwithvalue', reporturlname='reportstockinhandwithvalue', reportstatus='0', reportoriginals='Stock in Hand with Value', reportfunctions='favouritestockinhandwithvalue', reporturl='reportstockinhandwithvalue.php?period=thismonth&proid=all', reporthref=''");
...
if (isset($_POST['taskysubmit'])) {
	...
	while($taskyrow = mysqli_fetch_array($taskyresult)){
		...
		if ($taskyrow['franchises']!='') {
			...
			if (!empty($taskyfranchiseValues)) {
				foreach ($taskyfranchiseValues as $taskyfranchiseValue) {
					if (!empty($taskyfranchiseValue)) {
						...
						$taskyreportone = mysqli_query($taskycon,"SELECT * FROM pairreports WHERE createdid='$taskycompanymainid' AND franchiseid='".$taskyfranchiseValue."' AND types='stockinhandwithvalue'");
						...
						if (mysqli_num_rows($taskyreportone)==0) {
							...
							$taskypairreportfavourites = mysqli_query($taskycon,"INSERT INTO pairreportfavourites SET createdid='$taskycompanymainid', franchisesession='".$taskyfranchiseValue."', reportnames='stockinhandwithvalue', reporturlname='reportstockinhandwithvalue', reportstatus='0', reportoriginals='Stock in Hand with Value', reportfunctions='favouritestockinhandwithvalue', reporturl='reportstockinhandwithvalue.php?period=thismonth&proid=all', reporthref=''");
			...
			$taskyreportone = mysqli_query($taskycon,"SELECT * FROM pairreports WHERE createdid='$taskycompanymainid' AND types='stockinhandwithvalue'");
			...
			if (mysqli_num_rows($taskyreportone)==0) {
				...
				$taskypairreportfavourites = mysqli_query($taskycon,"INSERT INTO pairreportfavourites SET createdid='$taskycompanymainid',  reportnames='stockinhandwithvalue', reporturlname='reportstockinhandwithvalue', reportstatus='0', reportoriginals='Stock in Hand with Value', reportfunctions='favouritestockinhandwithvalue', reporturl='reportstockinhandwithvalue.php?period=thismonth&proid=all', reporthref=''");
...
if (isset($_POST['betatonicsubmit'])) {
	...
	while($betatonicrow = mysqli_fetch_array($betatonicresult)){
		...
		if ($betatonicrow['franchises']!='') {
			...
			if (!empty($betatonicfranchiseValues)) {
				foreach ($betatonicfranchiseValues as $betatonicfranchiseValue) {
					if (!empty($betatonicfranchiseValue)) {
						...
						$betatonicreportone = mysqli_query($betatoniccon,"SELECT * FROM pairreports WHERE createdid='$betatoniccompanymainid' AND franchiseid='".$betatonicfranchiseValue."' AND types='stockinhandwithvalue'");
						...
						if (mysqli_num_rows($betatonicreportone)==0) {
							...
							$betatonicpairreportfavourites = mysqli_query($betatoniccon,"INSERT INTO pairreportfavourites SET createdid='$betatoniccompanymainid', franchisesession='".$betatonicfranchiseValue."', reportnames='stockinhandwithvalue', reporturlname='reportstockinhandwithvalue', reportstatus='0', reportoriginals='Stock in Hand with Value', reportfunctions='favouritestockinhandwithvalue', reporturl='reportstockinhandwithvalue.php?period=thismonth&proid=all', reporthref=''");
			...
			$betatonicreportone = mysqli_query($betatoniccon,"SELECT * FROM pairreports WHERE createdid='$betatoniccompanymainid' AND types='stockinhandwithvalue'");
			...
			if (mysqli_num_rows($betatonicreportone)==0) {
				...
				$betatonicpairreportfavourites = mysqli_query($betatoniccon,"INSERT INTO pairreportfavourites SET createdid='$betatoniccompanymainid',  reportnames='stockinhandwithvalue', reporturlname='reportstockinhandwithvalue', reportstatus='0', reportoriginals='Stock in Hand with Value', reportfunctions='favouritestockinhandwithvalue', reporturl='reportstockinhandwithvalue.php?period=thismonth&proid=all', reporthref=''");
...
if (isset($_POST['betataskysubmit'])) {
	...
	while($betataskyrow = mysqli_fetch_array($betataskyresult)){
		...
		if ($betataskyrow['franchises']!='') {
			...
			if (!empty($betataskyfranchiseValues)) {
				foreach ($betataskyfranchiseValues as $betataskyfranchiseValue) {
					if (!empty($betataskyfranchiseValue)) {
						...
						$betataskyreportone = mysqli_query($betataskycon,"SELECT * FROM pairreports WHERE createdid='$betataskycompanymainid' AND franchiseid='".$betataskyfranchiseValue."' AND types='stockinhandwithvalue'");
			...
			$betataskyreportone = mysqli_query($betataskycon,"SELECT * FROM pairreports WHERE createdid='$betataskycompanymainid' AND types='stockinhandwithvalue'");
