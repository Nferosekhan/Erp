...
if(isset($_POST['grandtotal'])){
	...
	$sqlismainaccessusers=$con->prepare("SELECT moduleno, moduleprefix, modulesuffix FROM pairmainaccess WHERE franchiseid=? AND moduletype='Bills' ORDER BY id ASC");
	...
	$sqlismainaccessuser->close();
	$sqlismainaccessusers->close();
	$billno=htmlspecialchars($infomainaccessuser['moduleprefix'].(str_pad($infomainaccessuser['modulesuffix']+1, 0, "0", STR_PAD_LEFT)), ENT_QUOTES, 'UTF-8');
	//FOR GET THE CURRENT BILL NO 
   $ordering = (str_pad($infomainaccessuser['modulesuffix']+1, 0, "0", STR_PAD_LEFT));
	...
	$sqlismainaccessuser->close();
	$sqlismainaccessusers->close();
	$sql3 = $con->prepare("UPDATE pairmainaccess SET modulesuffix=modulesuffix + 1 WHERE franchiseid=? AND moduletype='Bills'");
   ...
   $sql3->execute();
	...
	$sql2->bind_param("iiss", $_SESSION['franchisesession'], $companymainid, $billno, $billdate);
	$sql2->execute();
	$sql2->store_result();
	//FOR CHECK THE BILL IS IN OR NOT
	if($sql2->num_rows==0){
		...
		if($billterm=='CASH'){
			// if ($validpaidamount!=0) {
				...
				$sqlpurpayins = $con->prepare("INSERT INTO pairpurchasepayments (createdid,createdby,franchisesession,createdon,term,type,vendorname,vendorid,receiptno,receiptdate,amount,paymentmode,notes,publicid,privateid) VALUES (?, ?, ?, ?, 'RECEIPT', 'bill', ?, ?, ?, ?, ?, ?, '-', ?, ?)");
    			$sqlpurpayins->bind_param("ssssssssssss", $companymainid, $_SESSION["unqwerty"], $_SESSION["franchisesession"], $createdon, $vendorname, $vendorid, $billno, $billdate, $validpaidamount, $billterm, $publiccode, $privatecode);
			...
			if($sqlpurpayins){
				...
				// }
					...
					if($billdate!=''){
						if($chforpayments!=''){
							$chforpayments.='<br> Date <span style="color:green;" id="prohisfromtospan">( '.date($datemainphp,strtotime($billdate)).' ) </span>';
						}
						else{
							$chforpayments.=' Date <span style="color:green;" id="prohisfromtospan">( '.date($datemainphp,strtotime($billdate)).' ) </span>';
						}					
					}
					if($billno!=''){
						if($chforpayments!=''){
							$chforpayments.='<br> Reference Number <span style="color:green;" id="prohisfromtospan">( '.$billno.' ) </span>';
						}
						else{
							$chforpayments.='Reference Number <span style="color:green;" id="prohisfromtospan">( '.$billno.' ) </span>';
						}					
					}
					if($billterm!=''){
						if($chforpayments!=''){
							$chforpayments.='<br> Mode of Payment <span style="color:green;" id="prohisfromtospan">( '.$billterm.' ) </span>';
						}
						else{
							$chforpayments.='Mode of Payment <span style="color:green;" id="prohisfromtospan">( '.$billterm.' ) </span>';
						}					
					}
					if($validpaidamount!=''){
						if($chforpayments!=''){
		...
		if($billno!=''){
			if($billinfoch!=''){
				$billinfoch.='<br> '.$infomainaccessuser['modulename'].' Number <span style="color:green;" id="prohisfromtospan">( '.$billno.' ) </span>';
			}
			else{
				$billinfoch.='<span style="color:royalblue;">'.$infomainaccessuser['modulename'].' Information</span> <br> '.$infomainaccessuser['modulename'].' Number <span style="color:green;" id="prohisfromtospan">( '.$billno.' ) </span>';
			}                   
		}
		if($invnumber!=''){
			if($billinfoch!=''){
				$billinfoch.='<br> Invoice Number <span style="color:green;" id="prohisfromtospan">( '.$invnumber.' ) </span>';
			}
			else{
				$billinfoch.='<span style="color:royalblue;">'.$infomainaccessuser['modulename'].' Information</span> <br> Invoice Number <span style="color:green;" id="prohisfromtospan">( '.$invnumber.' ) </span>';
			}                   
		}
		if($billdate!=''){
			if($billinfoch!=''){
				$billinfoch.='<br> '.$infomainaccessuser['modulename'].' Date <span style="color:green;" id="prohisfromtospan">( '.date($date,strtotime($billdate)).' ) </span>';
			}
			else{
				$billinfoch.='<span style="color:royalblue;">'.$infomainaccessuser['modulename'].' Information</span> <br> '.$infomainaccessuser['modulename'].' Date <span style="color:green;" id="prohisfromtospan">( '.date($date,strtotime($billdate)).' ) </span>';
			}                   
		}
		if($reference!=''){
			if($billinfoch!=''){
				$billinfoch.='<br> Reference <span style="color:green;" id="prohisfromtospan">( '.$reference.' ) </span>';
			}
			else{
				$billinfoch.='<span style="color:royalblue;">'.$infomainaccessuser['modulename'].' Information</span> <br> Reference <span style="color:green;" id="prohisfromtospan">( '.$reference.' ) </span>';
			}                   
		}
		if($saleperson!=''){
			if($billinfoch!=''){
				$billinfoch.='<br> Sale Person <span style="color:green;" id="prohisfromtospan">( '.$saleperson.' ) </span>';
			}
			else{
				$billinfoch.='<span style="color:royalblue;">'.$infomainaccessuser['modulename'].' Information</span> <br> Sale Person <span style="color:green;" id="prohisfromtospan">( '.$saleperson.' ) </span>';
			}                   
		}
		if($preparedby!=''){
			if($billinfoch!=''){
				$billinfoch.='<br> Prepared By <span style="color:green;" id="prohisfromtospan">( '.$preparedby.' ) </span>';
			}
			else{
				$billinfoch.='<span style="color:royalblue;">'.$infomainaccessuser['modulename'].' Information</span> <br> Prepared By <span style="color:green;" id="prohisfromtospan">( '.$preparedby.' ) </span>';
			}                   
		}
		if($checkedby!=''){
			if($billinfoch!=''){
				$billinfoch.='<br> Checked By <span style="color:green;" id="prohisfromtospan">( '.$checkedby.' ) </span>';
			}
			else{
				$billinfoch.='<span style="color:royalblue;">'.$infomainaccessuser['modulename'].' Information</span> <br> Checked By <span style="color:green;" id="prohisfromtospan">( '.$checkedby.' ) </span>';
			}                   
		}
		if($invgrandtotal!=''){
			if($billinfoch!=''){
				$billinfoch.='<br> Grand Total <span style="color:green;" id="prohisfromtospan">( '.$invgrandtotal.' ) </span>';
			}
			else{
				$billinfoch.='<span style="color:royalblue;">'.$infomainaccessuser['modulename'].' Information</span> <br> Grand Total <span style="color:green;" id="prohisfromtospan">( '.$invgrandtotal.' ) </span>';
			}                   
		}
		...
		if($vendorname!=''){
			if($billveninfoch!=''){
				$billveninfoch.='<br> '.$infomainaccessuserven['modulename'].' Name <span style="color:green;" id="prohisfromtospan">( '.$vendorname.' ) </span>';
			}
			else{
				$billveninfoch.=''.(($billinfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessuserven['modulename'].' Information</span> <br> '.$infomainaccessuserven['modulename'].' Name <span style="color:green;" id="prohisfromtospan">( '.$vendorname.' ) </span>';
			}                   
		}
		if($area!=''){
			if($billveninfoch!=''){
				$billveninfoch.='<br> BILLING STREET <span style="color:green;" id="prohisfromtospan">( '.$area.' ) </span>';
			}
			else{
				$billveninfoch.=''.(($billinfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessuserven['modulename'].' Information</span> <br> BILLING STREET <span style="color:green;" id="prohisfromtospan">( '.$area.' ) </span>';
			}                   
		}
		if($city!=''){
			if($billveninfoch!=''){
				$billveninfoch.='<br> BILLING CITY/TOWN <span style="color:green;" id="prohisfromtospan">( '.$city.' ) </span>';
			}
			else{
				$billveninfoch.=''.(($billinfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessuserven['modulename'].' Information</span> <br> BILLING CITY/TOWN <span style="color:green;" id="prohisfromtospan">( '.$city.' ) </span>';
			}                   
		}
		if($state!=''){
			if($billveninfoch!=''){
				$billveninfoch.='<br> BILLING STATE <span style="color:green;" id="prohisfromtospan">( '.$state.' ) </span>';
			}
			else{
				$billveninfoch.=''.(($billinfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessuserven['modulename'].' Information</span> <br> BILLING STATE <span style="color:green;" id="prohisfromtospan">( '.$state.' ) </span>';
			}                   
		}
		if($district!=''){
			if($billveninfoch!=''){
				$billveninfoch.='<br> BILLING PIN <span style="color:green;" id="prohisfromtospan">( '.$district.' ) </span>';
			}
			else{
				$billveninfoch.=''.(($billinfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessuserven['modulename'].' Information</span> <br> BILLING PIN <span style="color:green;" id="prohisfromtospan">( '.$district.' ) </span>';
			}                   
		}
		if($pincode!=''){
			if($billveninfoch!=''){
				$billveninfoch.='<br> BILLING COUNTRY/REGION <span style="color:green;" id="prohisfromtospan">( '.$pincode.' ) </span>';
			}
			else{
				$billveninfoch.=''.(($billinfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessuserven['modulename'].' Information</span> <br> BILLING COUNTRY/REGION <span style="color:green;" id="prohisfromtospan">( '.$pincode.' ) </span>';
			}                   
		}
		if($sarea!=''){
			if($billveninfoch!=''){
				$billveninfoch.='<br> SHIPPING STREET <span style="color:green;" id="prohisfromtospan">( '.$sarea.' ) </span>';
			}
			else{
				$billveninfoch.=''.(($billinfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessuserven['modulename'].' Information</span> <br> SHIPPING STREET <span style="color:green;" id="prohisfromtospan">( '.$sarea.' ) </span>';
			}                   
		}
		if($scity!=''){
			if($billveninfoch!=''){
				$billveninfoch.='<br> SHIPPING CITY/TOWN <span style="color:green;" id="prohisfromtospan">( '.$scity.' ) </span>';
			}
			else{
				$billveninfoch.=''.(($billinfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessuserven['modulename'].' Information</span> <br> SHIPPING CITY/TOWN <span style="color:green;" id="prohisfromtospan">( '.$scity.' ) </span>';
			}                   
		}
		if($sstate!=''){
			if($billveninfoch!=''){
				$billveninfoch.='<br> SHIPPING STATE <span style="color:green;" id="prohisfromtospan">( '.$sstate.' ) </span>';
			}
			else{
				$billveninfoch.=''.(($billinfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessuserven['modulename'].' Information</span> <br> SHIPPING STATE <span style="color:green;" id="prohisfromtospan">( '.$sstate.' ) </span>';
			}                   
		}
		if($sdistrict!=''){
			if($billveninfoch!=''){
				$billveninfoch.='<br> SHIPPING PIN <span style="color:green;" id="prohisfromtospan">( '.$sdistrict.' ) </span>';
			}
			else{
				$billveninfoch.=''.(($billinfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessuserven['modulename'].' Information</span> <br> SHIPPING PIN <span style="color:green;" id="prohisfromtospan">( '.$sdistrict.' ) </span>';
			}                   
		}
		if($spincode!=''){
			if($billveninfoch!=''){
				$billveninfoch.='<br> SHIPPING COUNTRY/REGION <span style="color:green;" id="prohisfromtospan">( '.$spincode.' ) </span>';
			}
			else{
				$billveninfoch.=''.(($billinfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessuserven['modulename'].' Information</span> <br> SHIPPING COUNTRY/REGION <span style="color:green;" id="prohisfromtospan">( '.$spincode.' ) </span>';
			}                   
		}
		if($twoworkphone!=''){
			if($billveninfoch!=''){
				$billveninfoch.='<br> WORK PHONE <span style="color:green;" id="prohisfromtospan">( '.$twoworkphone.' ) </span>';
			}
			else{
				$billveninfoch.=''.(($billinfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessuserven['modulename'].' Information</span> <br> WORK PHONE <span style="color:green;" id="prohisfromtospan">( '.$twoworkphone.' ) </span>';
			}                   
		}
		if($twomobilephone!=''){
			if($billveninfoch!=''){
				$billveninfoch.='<br> MOBILE PHONE <span style="color:green;" id="prohisfromtospan">( '.$twomobilephone.' ) </span>';
			}
			else{
				$billveninfoch.=''.(($billinfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessuserven['modulename'].' Information</span> <br> MOBILE PHONE <span style="color:green;" id="prohisfromtospan">( '.$twomobilephone.' ) </span>';
			}                   
		}
		if($gstrtype!=''){
			if($billveninfoch!=''){
				$billveninfoch.='<br> GST TREATMENT <span style="color:green;" id="prohisfromtospan">( '.$gstrtype.' ) </span>';
			}
			else{
				$billveninfoch.=''.(($billinfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessuserven['modulename'].' Information</span> <br> GST TREATMENT <span style="color:green;" id="prohisfromtospan">( '.$gstrtype.' ) </span>';
			}                   
		}
		if($gstno!=''){
			if($billveninfoch!=''){
				$billveninfoch.='<br> GSTIN <span style="color:green;" id="prohisfromtospan">( '.$gstno.' ) </span>';
			}
			else{
				$billveninfoch.=''.(($billinfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessuserven['modulename'].' Information</span> <br> GSTIN <span style="color:green;" id="prohisfromtospan">( '.$gstno.' ) </span>';
			}                   
		}
		if($pos!=''){
			if($billveninfoch!=''){
				$billveninfoch.='<br> PLACE OF SUPPLY <span style="color:green;" id="prohisfromtospan">( '.$pos.' ) </span>';
			}
			else{
				$billveninfoch.=''.(($billinfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessuserven['modulename'].' Information</span> <br> PLACE OF SUPPLY <span style="color:green;" id="prohisfromtospan">( '.$pos.' ) </span>';
			}                   
		}
		if($dlno20!=''){
			if($billveninfoch!=''){
				$billveninfoch.='<br> DL No 20 <span style="color:green;" id="prohisfromtospan">( '.$dlno20.' ) </span>';
			}
			else{
				$billveninfoch.=''.(($billinfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessuserven['modulename'].' Information</span> <br> DL No 20 <span style="color:green;" id="prohisfromtospan">( '.$dlno20.' ) </span>';
			}                   
		}
		if($dlno21!=''){
			if($billveninfoch!=''){
				$billveninfoch.='<br> DL No 21 <span style="color:green;" id="prohisfromtospan">( '.$dlno21.' ) </span>';
			}
			else{
				$billveninfoch.=''.(($billinfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessuserven['modulename'].' Information</span> <br> DL No 21 <span style="color:green;" id="prohisfromtospan">( '.$dlno21.' ) </span>';
			}                   
		}
		...
		for($i=0; $i<count($_POST['productname']); $i++){
			...
			if($productname!=''){
				...
				if($expdate!=''){
					if($billiteminfoch!=''){
						$billiteminfoch.='<br> Expiry <span style="color:green;" id="prohisfromtospan">( '.$expdate.' ) </span>';
					}
					else{
						$billiteminfoch.=''.(($billinfoch!=''||$billveninfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Expiry <span style="color:green;" id="prohisfromtospan">( '.$expdate.' ) </span>';
					}                   
				}
				if($productrate!=''){
					if($billiteminfoch!=''){
		...
		if($totalitems!=''){
			if($billiteminfoch!=''){
				$billiteminfoch.='<br> Total Items <span style="color:green;" id="prohisfromtospan">( '.$totalitems.' ) </span>';
			}
			else{
				$billiteminfoch.=''.(($billinfoch!=''||$billveninfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Total Items <span style="color:green;" id="prohisfromtospan">( '.$totalitems.' ) </span>';
			}                   
		}
		if($totalquantity!=''){
			if($billiteminfoch!=''){
				$billiteminfoch.='<br> Total Qty <span style="color:green;" id="prohisfromtospan">( '.$totalquantity.' ) </span>';
			}
			else{
				$billiteminfoch.=''.(($billinfoch!=''||$billveninfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Total Qty <span style="color:green;" id="prohisfromtospan">( '.$totalquantity.' ) </span>';
			}                   
		}
		if($totalamount!=''){
			if($billiteminfoch!=''){
				$billiteminfoch.='<br> Sub Total <span style="color:green;" id="prohisfromtospan">( '.$totalamount.' ) </span>';
			}
			else{
				$billiteminfoch.=''.(($billinfoch!=''||$billveninfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Sub Total <span style="color:green;" id="prohisfromtospan">( '.$totalamount.' ) </span>';
			}                   
		}
		if($discountamount!=''){
			if($billiteminfoch!=''){
				$billiteminfoch.='<br> Discount <span style="color:green;" id="prohisfromtospan">( '.$discountamount.' ) </span>';
			}
			else{
				$billiteminfoch.=''.(($billinfoch!=''||$billveninfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Discount <span style="color:green;" id="prohisfromtospan">( '.$discountamount.' ) </span>';
			}                   
		}
		if($totalvatamount!=''){
			if($billiteminfoch!=''){
				$billiteminfoch.='<br> Total Tax <span style="color:green;" id="prohisfromtospan">( '.$totalvatamount.' ) </span>';
			}
			else{
				$billiteminfoch.=''.(($billinfoch!=''||$billveninfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Total Tax <span style="color:green;" id="prohisfromtospan">( '.$totalvatamount.' ) </span>';
			}                   
		}
		if($roundoff!=''){
			if($billiteminfoch!=''){
				$billiteminfoch.='<br> Round Off <span style="color:green;" id="prohisfromtospan">( '.$roundoff.' ) </span>';
			}
			else{
				$billiteminfoch.=''.(($billinfoch!=''||$billveninfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Round Off <span style="color:green;" id="prohisfromtospan">( '.$roundoff.' ) </span>';
			}                   
		}
		if($grandtotal!=''){
			if($billiteminfoch!=''){
				$billiteminfoch.='<br> Grand Total <span style="color:green;" id="prohisfromtospan">( '.$grandtotal.' ) </span>';
			}
			else{
				$billiteminfoch.=''.(($billinfoch!=''||$billveninfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Grand Total <span style="color:green;" id="prohisfromtospan">( '.$grandtotal.' ) </span>';
			}                   
		}
		if($description!=''){
			if($billiteminfoch!=''){
				$billiteminfoch.='<br> Description <span style="color:green;" id="prohisfromtospan">( '.$description.' ) </span>';
			}
			else{
				$billiteminfoch.=''.(($billinfoch!=''||$billveninfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Description <span style="color:green;" id="prohisfromtospan">( '.$description.' ) </span>';
			}                   
		}
		if($notes!=''){
			if($billiteminfoch!=''){
				$billiteminfoch.='<br> Notes <span style="color:green;" id="prohisfromtospan">( '.$notes.' ) </span>';
			}
			else{
				$billiteminfoch.=''.(($billinfoch!=''||$billveninfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Notes <span style="color:green;" id="prohisfromtospan">( '.$notes.' ) </span>';
			}                   
		}
		if($terms!=''){
			if($billiteminfoch!=''){
				$billiteminfoch.='<br> Terms And Conditions <span style="color:green;" id="prohisfromtospan">( '.$terms.' ) </span>';
			}
			else{
				$billiteminfoch.=''.(($billinfoch!=''||$billveninfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Terms And Conditions <span style="color:green;" id="prohisfromtospan">( '.$terms.' ) </span>';
			}                   
		}
		if($fileattach!=''){
			if($billiteminfoch!=''){
				$billiteminfoch.='<br> Attach <span style="color:green;" id="prohisfromtospan">( Added ) </span>';
			}
			else{
				$billiteminfoch.=''.(($billinfoch!=''||$billveninfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Attach <span style="color:green;" id="prohisfromtospan">( Added ) </span>';
			}                   
		}
		if($billterm!=''){
			if($billiteminfoch!=''){
				$billiteminfoch.='<br> Payment Method <span style="color:green;" id="prohisfromtospan">( '.$billterm.' ) </span>';
			}
			else{
				$billiteminfoch.=''.(($billinfoch!=''||$billveninfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Payment Method <span style="color:green;" id="prohisfromtospan">( '.$billterm.' ) </span>';
			}                   
		}
		if($validpaidamount!=''&&$billterm=='CASH'){
			if($billiteminfoch!=''){
				$billiteminfoch.='<br> Cash Received <span style="color:green;" id="prohisfromtospan">( '.$validpaidamount.' ) </span>';
			}
			else{
				$billiteminfoch.=''.(($billinfoch!=''||$billveninfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Cash Received <span style="color:green;" id="prohisfromtospan">( '.$validpaidamount.' ) </span>';
			}                   
		}
		if($validbalance!=''&&$billterm=='CASH'){
			if($billiteminfoch!=''){
				$billiteminfoch.='<br> Change Due <span style="color:green;" id="prohisfromtospan">( '.$validbalance.' ) </span>';
			}
			else{
				$billiteminfoch.=''.(($billinfoch!=''||$billveninfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Change Due <span style="color:green;" id="prohisfromtospan">( '.$validbalance.' ) </span>';
			}                   
		}
		if($duedates!=''&&$billterm=='CREDIT'){
			if($billiteminfoch!=''){
				$billiteminfoch.='<br> Due Term <span style="color:green;" id="prohisfromtospan">( '.$duedates.' ) </span>';
			}
			else{
				$billiteminfoch.=''.(($billinfoch!=''||$billveninfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Due Term <span style="color:green;" id="prohisfromtospan">( '.$duedates.' ) </span>';
			}                   
		}
		if($duedate!=''&&$billterm=='CREDIT'){
			if($billiteminfoch!=''){
				$billiteminfoch.='<br> Due Date <span style="color:green;" id="prohisfromtospan">( '.date($date,strtotime($duedate)).' ) </span>';
			}
			else{
				$billiteminfoch.=''.(($billinfoch!=''||$billveninfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Due Date <span style="color:green;" id="prohisfromtospan">( '.date($date,strtotime($duedate)).' ) </span>';
			}                   
		}
		//FOR BILL OTHER INFORMATIONS LIKE PAYMENT AND TERMS AND DESCRIPTION......
		$billtotinfoch = "BILL CREATED<br>".$billinfoch.$billveninfoch.$billiteminfoch;
		if($billtotinfoch!=''){
    	...
    	$cussqlup->bind_param("ssssi", $billamount, $balanceamount, $currentamount, $overdueamount, $vendorid);
    	$cussqlup->execute();
		$cussqlup->close();
		//FOR UPDATE THE VENDOR BALANCE AND OTHER PAYMENT INFORMATIONS
 		if(isset($_POST['submit1'])){
			...
			echo '<script> window.location.href="bills.php?remarks=Added Successfully";</script>'; 
		}
		else{
			$sqluse = $con->prepare("INSERT INTO pairusehistory (usetype,createdon,createdby,useid,uniqueid,useremarks) VALUES ('BILLS', ?, ?, ?, ?, '<span style=\"color:green;\" id=\"prohisfromtospan\">Added Successfully</span>')");
    		$sqluse->bind_param("ssss", $times, $_SESSION["unqwerty"], $billno, $purchaseid);
    		$sqluse->execute();
    		$sqluse->close();
			echo '<script> window.location.href="billview.php?id='.$purchaseid.'&billno='.$billno.'&billdate='.$billdate.'&remarks=Added Successfully";</script>';
		}
	}
	else{
		header("Location: bills.php?error=Error Data");
		//FOR IF THE BILL IS ALREADY EXISTS IN THIS NUMBER AND DATE AND FRANCHISE AND COMPANY
	}
}
$sqlismainaccessuservens=$con->prepare("SELECT * FROM pairmainaccess WHERE userid=? AND moduletype='Vendors' ORDER BY id ASC");
$sqlismainaccessuservens->bind_param("s", $userid);
														...
														<div class="row">
															<div class="col-lg-12">
																<div class="form-group row" style="align-items: center !important;">
																	...
																	<div class="col-lg-7">
																		<input type="text" class="form-control  form-control-sm" id="billno" name="billno" required value="<?=$infomainaccessuser['moduleprefix']?><?=str_pad(($infomainaccessuser['modulesuffix']+1), 0, "0", STR_PAD_LEFT)?>" readonly>
																	</div>
																</div>
															</div>
															<div class="col-lg-12">
															...
															<div class="col-lg-12">
																<div class="form-group row" style="align-items: center !important;">
																	...
																	<?php
																		...
																		$dt = new DateTime();
																	?>
																		<input type="date" class="form-control  form-control-sm" id="billdate" name="billdate" value="<?= $dt->format('Y-m-d') ?>" required>
																	</div>
																</div>
															</div>
														<?php
																...
																<div class="row">
																	<div class="col-lg-2 col-md-2 col-3">
																		<div class="custom-control custom-radio mr-sm-2">
																			<input type="radio" class="custom-control-input" name="vendorinfodefault" id="onevendorinfo" value="one" <?= ($infomainaccessuser['billveninfo']=='defone')?'checked':'' ?> onclick="onetwo()">
																	...
																	<div class="col-lg-2 col-md-2 col-3">
																		<div class="custom-control custom-radio mr-sm-2">
																			<input type="radio" class="custom-control-input" name="vendorinfodefault" id="twovendorinfo" value="two" <?= ($infomainaccessuser['billveninfo']=='deftwo')?'checked':'' ?> onclick="onetwo()">
													...
													?>
														...
														<div id="two">
															<div class="row">
																...
																<div class="col-lg-9">
																	<input type="text" name="twovendorname" id="twovendorname" class="form-control form-control-sm" placeholder="<?= $infomainaccessuserven['modulename'] ?> Name" <?= ($access['billbtwocnamerequired']=='Yes')? 'required': ' ' ?>>
																</div>
															</div>
															<div class="row mt-1">
																<div class="col-lg-12">
																	<div class="row mb-0" id="custaddressdiv" style="background-color:#fbfafa; color:#777777; display:block;margin: 0px !important;padding: 9px 9px 0px 9px !important;">
																		...
																		<div class="row" style="padding:0px !important;margin: 0px !important;">
																			<div class="col-lg-6" <?=(in_array('Work Phone', $fieldadd))?'':'style="display:none;"'?>>
																				...
																				<input type="text" class="form-control form-control-sm" id="twoworkphone" name="twoworkphone" placeholder="Work Phone" <?= ($access['billbtwocwphonerequired']=='Yes')? 'required': ' ' ?> maxlength="25">
																			</div>
																			...
																			<div class="col-lg-8" <?=(in_array('GSTIN', $fieldadd))?'':'style="display:none;"'?>>
																				<div class="row">
																					<div class="col-lg-6">
																						...
																						<input type="text" class="form-control form-control-sm" id="twogsttreatment" name="twogsttreatment" placeholder="GST TREATMENT" value="<?=$infomainaccessuser['billtwogst']?>" readonly>
																					</div>
																				...
																				<div class="row">
																					<div class="col-lg-12" id="twoposblock">
																						...
																						<select name="twopos" id="twopos" class="select5 form-control form-control-sm" required onchange="twoposans()">
																							<option disabled value="Select Place of Supply" <?=($infomainaccessuser['billtwopos']=='Select Place of Supply')?'selected':''?>>
																								Select The Place
																							</option> 
																							<option value="JAMMU AND KASHMIR (1)" <?=($infomainaccessuser['billtwopos']=="JAMMU AND KASHMIR (1)")?'selected':''?>>
																								JAMMU AND KASHMIR (1)
																							</option>
																							<option value="ANDAMAN AND NICOBAR ISLANDS (35)" <?=($infomainaccessuser['billtwopos']=="ANDAMAN AND NICOBAR ISLANDS (35)")?'selected':''?>>
																								ANDAMAN AND NICOBAR ISLANDS (35)
																							</option>
																							<option value="ANDHRA PRADESH (NEWLY ADDED) (37)" <?=($infomainaccessuser['billtwopos']=="ANDHRA PRADESH (NEWLY ADDED) (37)")?'selected':''?>>
																								ANDHRA PRADESH (NEWLY ADDED) (37)
																							</option>
																							<option value="ANDHRA PRADESH(BEFORE DIVISION) (28)" <?=($infomainaccessuser['billtwopos']=="ANDHRA PRADESH(BEFORE DIVISION) (28)")?'selected':''?>>
																								ANDHRA PRADESH(BEFORE DIVISION) (28)
																							</option>
																							<option value="ARUNACHAL PRADESH (12)" <?=($infomainaccessuser['billtwopos']=="ARUNACHAL PRADESH (12)")?'selected':''?>>
																								ARUNACHAL PRADESH (12)
																							</option>
																							<option value="ASSAM (18)" <?=($infomainaccessuser['billtwopos']=="ASSAM (18)")?'selected':''?>>
																								ASSAM (18)
																							</option>
																							<option value="BIHAR (10)" <?=($infomainaccessuser['billtwopos']=="BIHAR (10)")?'selected':''?>>
																								BIHAR (10)
																							</option>
																							<option value="CENTRE JURISDICTION (99)" <?=($infomainaccessuser['billtwopos']=="CENTRE JURISDICTION (99)")?'selected':''?>>
																								CENTRE JURISDICTION (99)
																							</option>
																							<option value="CHANDIGARH (4)" <?=($infomainaccessuser['billtwopos']=="CHANDIGARH (4)")?'selected':''?>>
																								CHANDIGARH (4)
																							</option>
																							<option value="CHATTISGARH (22)" <?=($infomainaccessuser['billtwopos']=="CHATTISGARH (22)")?'selected':''?>>
																								CHATTISGARH (22)
																							</option>
																							<option value="DADRA AND NAGAR HAVELI AND DAMAN AND DIU (NEWLY MERGED UT) (26*)" <?=($infomainaccessuser['billtwopos']=="DADRA AND NAGAR HAVELI AND DAMAN AND DIU (NEWLY MERGED UT) (26*)")?'selected':''?>>
																								DADRA AND NAGAR HAVELI AND DAMAN AND DIU (NEWLY MERGED UT) (26*)
																							</option>
																							<option value="DELHI (7)" <?=($infomainaccessuser['billtwopos']=="DELHI (7)")?'selected':''?>>
																								DELHI (7)
																							</option>
																							<option value="GOA (30)" <?=($infomainaccessuser['billtwopos']=="GOA (30)")?'selected':''?>>
																								GOA (30)
																							</option>
																							<option value="GUJARAT (24)" <?=($infomainaccessuser['billtwopos']=="GUJARAT (24)")?'selected':''?>>
																								GUJARAT (24)
																							</option>
																							<option value="HARYANA (6)" <?=($infomainaccessuser['billtwopos']=="HARYANA (6)")?'selected':''?>>
																								HARYANA (6)
																							</option>
																							<option value="HIMACHAL PRADESH (2)" <?=($infomainaccessuser['billtwopos']=="HIMACHAL PRADESH (2)")?'selected':''?>>
																								HIMACHAL PRADESH (2)
																							</option>
																							<option value="JHARKHAND (20)" <?=($infomainaccessuser['billtwopos']=="JHARKHAND (20)")?'selected':''?>>
																								JHARKHAND (20)
																							</option>
																							<option value="KARNATAKA (29)" <?=($infomainaccessuser['billtwopos']=="KARNATAKA (29)")?'selected':''?>>
																								KARNATAKA (29)
																							</option>
																							<option value="KERALA (32)" <?=($infomainaccessuser['billtwopos']=="KERALA (32)")?'selected':''?>>
																								KERALA (32)
																							</option>
																							<option value="LADAKH (NEWLY ADDED) (38)" <?=($infomainaccessuser['billtwopos']=="LADAKH (NEWLY ADDED) (38)")?'selected':''?>>
																								LADAKH (NEWLY ADDED) (38)
																							</option>
																							<option value="LAKSHADWEEP (31)" <?=($infomainaccessuser['billtwopos']=="LAKSHADWEEP (31)")?'selected':''?>>
																								LAKSHADWEEP (31)
																							</option>
																							<option value="MADHYA PRADESH (23)" <?=($infomainaccessuser['billtwopos']=="MADHYA PRADESH (23)")?'selected':''?>>
																								MADHYA PRADESH (23)
																							</option>
																							<option value="MAHARASHTRA (27)" <?=($infomainaccessuser['billtwopos']=="MAHARASHTRA (27)")?'selected':''?>>
																								MAHARASHTRA (27)
																							</option>
																							<option value="MANIPUR (14)" <?=($infomainaccessuser['billtwopos']=="MANIPUR (14)")?'selected':''?>>
																								MANIPUR (14)
																							</option>
																							<option value="MEGHALAYA (17)" <?=($infomainaccessuser['billtwopos']=="MEGHALAYA (17)")?'selected':''?>>
																								MEGHALAYA (17)
																							</option>
																							<option value="MIZORAM (15)" <?=($infomainaccessuser['billtwopos']=="MIZORAM (15)")?'selected':''?>>
																								MIZORAM (15)
																							</option>
																							<option value="NAGALAND (13)" <?=($infomainaccessuser['billtwopos']=="NAGALAND (13)")?'selected':''?>>
																								NAGALAND (13)
																							</option>
																							<option value="ODISHA (21)" <?=($infomainaccessuser['billtwopos']=="ODISHA (21)")?'selected':''?>>
																								ODISHA (21)
																							</option>
																							<option value="OTHER TERRITORY (97)" <?=($infomainaccessuser['billtwopos']=="OTHER TERRITORY (97)")?'selected':''?>>
																								OTHER TERRITORY (97)
																							</option>
																							<option value="PUDUCHERRY (34)" <?=($infomainaccessuser['billtwopos']=="PUDUCHERRY (34)")?'selected':''?>>
																								PUDUCHERRY (34)
																							</option>
																							<option value="PUNJAB (3)" <?=($infomainaccessuser['billtwopos']=="PUNJAB (3)")?'selected':''?>>
																								PUNJAB (3)
																							</option>
																							<option value="RAJASTHAN (8)" <?=($infomainaccessuser['billtwopos']=="RAJASTHAN (8)")?'selected':''?>>
																								RAJASTHAN (8)
																							</option>
																							<option value="SIKKIM (11)" <?=($infomainaccessuser['billtwopos']=="SIKKIM (11)")?'selected':''?>>
																								SIKKIM (11)
																							</option>
																							<option value="TAMIL NADU (33)"  <?=($infomainaccessuser['billtwopos']=="TAMIL NADU (33)")?'selected':''?>>
																								TAMIL NADU (33)
																							</option>
																							<option value="TELANGANA (36)" <?=($infomainaccessuser['billtwopos']=="TELANGANA (36)")?'selected':''?>>
																								TELANGANA (36)
																							</option>
																							<option value="TRIPURA (16)" <?=($infomainaccessuser['billtwopos']=="TRIPURA (16)")?'selected':''?>>
																								TRIPURA (16)
																							</option>
																							<option value="UTTAR PRADESH (9)" <?=($infomainaccessuser['billtwopos']=="UTTAR PRADESH (9)")?'selected':''?>>
																								UTTAR PRADESH (9)
																							</option>
																							<option value="UTTARAKHAND (5)" <?=($infomainaccessuser['billtwopos']=="UTTARAKHAND (5)")?'selected':''?>>
																								UTTARAKHAND (5)
																							</option>
																							<option value="WEST BENGAL (19)" <?=($infomainaccessuser['billtwopos']=="WEST BENGAL (19)")?'selected':''?>>
																								WEST BENGAL (19)
																							</option>
																						</select>
																					</div>
															...
															<table class="table table-bordered" id="purchasetable">
																<thead>
																	<tr>
																		...
																		<th <?=((in_array('Barcode', $fieldadd))||(in_array('Item Details', $fieldadd))||($access['batchexpiryval']==1)||(in_array('Rate', $fieldadd))||(in_array('Quantity', $fieldadd))||(in_array('Taxable Value', $fieldadd))||(in_array('Tax Value', $fieldadd))||(in_array('Amount', $fieldadd))||(in_array('Sale Quantity', $fieldadd)))?'':'style="display:none !important;"'?>>
																		</th>
																	<?php
																...
																<tbody>
																	<tr>
																		...
																		<td class="tdmove" <?=((in_array('Barcode', $fieldadd))||(in_array('Item Details', $fieldadd))||($access['batchexpiryval']==1)||(in_array('Rate', $fieldadd))||(in_array('Quantity', $fieldadd))||(in_array('Taxable Value', $fieldadd))||(in_array('Tax Value', $fieldadd))||(in_array('Amount', $fieldadd))||(in_array('Sale Quantity', $fieldadd)))?'':'style="display:none !important;"'?>>
																					...
																					<table width="100%">
																						<tr style="border-bottom: 1px solid #cccccc;margin-bottom: 0px;">
																							...
																							<td align="center" style="width: 50% !important;display: inline-block !important;text-align: center;">
																								<span onclick="othersgetfun(1)" id="billonoff1">
																									OTHERS
																								</span>
																							</td>
																		...
																		<td data-label="<?=($access['txtqtybill'])?>" <?=(in_array('Quantity', $fieldadd))?'':'style="display:none !important;"'?>>
																			<div>
																		...
																		<td data-label="<?=($access['txttaxablebill'])?>" <?=(in_array('Taxable Value', $fieldadd))?'':'style="display:none;"'?>>
																		...
																		<td data-label="<?=$access['txtsqty']?>" <?=(in_array('Sale Quantity', $fieldadd))?'':'style="display:none;"'?>>
																			<div>
	...
	setTimeout(function() {
		...
		if (!allAreFilled) {
			...
			if(accessallow){
				$.ajax({
					type: "GET",
					url: 'financialyear.php?types=bill&finyear='+document.getElementById("billdate").value.split('-')[0]+'',
					success: function (result) {
						// console.log(result);
						if (result=='exist') {
...
// FOR GET OLD RATE OF THE VENDOR AND PRODUCT IN BILL FOR SELF BRANCH
...
// FOR GET OLD RATE OF THE VENDOR AND PRODUCT IN BILL FOR OTHER BRANCHES
