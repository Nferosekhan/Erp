<?php
include('lcheck.php');
 if((isset($_GET['purchaseorderno']))&&(isset($_GET['purchaseorderdate'])))
 {
     $purchaseorderno=mysqli_real_escape_string($con, $_GET['purchaseorderno']);
     $purchaseorderdate=mysqli_real_escape_string($con, $_GET['purchaseorderdate']);
 $sql=mysqli_query($con, "select * from pairpurchaseorders where franchisesession='".$_SESSION['franchisesession']."' and createdid='$companymainid' and purchaseorderno='$purchaseorderno' and purchaseorderdate='$purchaseorderdate' order by id asc");
...
$oldpurchaseorderno=mysqli_real_escape_string($con, $_GET['purchaseorderno']);
$oldpurchaseorderdate=mysqli_real_escape_string($con, $_GET['purchaseorderdate']);
...
if($productname!='')
{
$sql=mysqli_query($con, "delete from pairpurchaseorders where createdid='$companymainid' and franchisesession='".$_SESSION["franchisesession"]."' and purchaseorderno='$oldpurchaseorderno' and purchaseorderdate='$oldpurchaseorderdate'");
if($sql)
{
}
else
...
$sql2=mysqli_query($con, "select id from pairpurchaseorders where franchisesession='".$_SESSION['franchisesession']."' and createdid='$companymainid' and purchaseorderno='$oldpurchaseorderno' and purchaseorderdate='$oldpurchaseorderdate'");
...
echo '<script> window.location.href="purchaseorders.php?remarks=Updated Successfully";</script>'; 
}
else
{
echo '<script> window.location.href="purchaseorderview.php?purchaseorderno='.$purchaseorderno.'&purchaseorderdate='.$purchaseorderdate.'&remarks=Updated Successfully";</script>';
}
}
}
else
...
if ($sql) {
$hissql=mysqli_query($con, "insert into pairusehistory set usetype='PURCHASEORDER', createdon='$times',  createdby='".$_SESSION["unqwerty"]."', useid='$purchaseorderno', useremarks='Purchase Order Updated' ");
}
}
...
<input type="hidden" name="oldpurchaseorderno" value="<?=$rows[0]['purchaseorderno']?>">
<input type="hidden" name="oldpurchaseorderdate" value="<?=$rows[0]['purchaseorderdate']?>">
<input type="hidden" name="purchaseorderterm" value="<?=$rows[0]['purchaseorderterm']?>">

<div class="row">
<?php
     if ((in_array('Purchase Order Information', $fieldedit))) {
        ?>
<div class="col-lg-4">
...
<div class="col-lg-7">
<input type="text" class="form-control  form-control-sm" id="purchaseorderno" name="purchaseorderno" required  value="<?=$rows[0]['purchaseorderno']?>" readonly>
</div>
</div>
</div>
<div class="col-lg-12">
...
<div class="col-lg-7">
<input type="date" class="form-control  form-control-sm" id="purchaseorderdate" name="purchaseorderdate" required value="<?=$rows[0]['purchaseorderdate']?>">
</div>
</div>
</div>
...
?>
<option value="<?= $info['term'] ?>" <?=($info['term']==$rows[0]['purchaseorderterm'])?'selected':''?>><?= $info['term'] ?></option>
<?php
}
?>
</select>
...
while ($info = mysqli_fetch_array($sqli))
{
$sqlibill=mysqli_query($con, "select purchaseorderamount, purchaseorderdate, purchaseorderno from pairpurchaseorders where franchisesession='".$_SESSION['franchisesession']."' and createdid='$companymainid' and vendorid='".$info['id']."' GROUP BY purchaseorderdate, purchaseorderno order by purchaseorderdate desc, purchaseorderno desc");
$billamount=0;
$balanceamount=0;
$currentamount=0;
$overdueamount=0;
...
{
$billamount+=(float)$infobill['purchaseorderamount'];
$paidamount=0;
$sqlpurchasepay=mysqli_query($con,"select amount from pairpurchasepayhistory where franchisesession='".$_SESSION['franchisesession']."' and createdid='$companymainid' and billno='".$infobill['purchaseorderno']."' and billdate='".$infobill['purchaseorderdate']."' and vendorid='".$info['id']."' order by id desc");
while($infopurchasepay=mysqli_fetch_array($sqlpurchasepay))
{
$paidamount+=(float)$infopurchasepay['amount'];
}
$balanceamount+=((float)$infobill['purchaseorderamount']-$paidamount);
$diff = abs(time() - strtotime($infobill['purchaseorderdate']));
$days = floor(($diff)/ (60*60*24));
if($days>30)
{
$overdueamount+=((float)$infobill['purchaseorderamount']-$paidamount);
}
else
{
$currentamount+=((float)$infobill['purchaseorderamount']-$paidamount);
}
}
?>
...
<div class="col-lg-9">
<input type="text" name="twovendorname" id="twovendorname" class="form-control form-control-sm" placeholder="<?= $infomainaccessuserven['modulename'] ?> Name" <?= ($access['purorderbtwocnamerequired']=='Yes')? 'required': ' ' ?> value="<?= $rows[0]['vendorname'] ?>">
...
<input type="text" class="form-control form-control-sm" id="twoworkphone" name="twoworkphone" placeholder="WORK PHONE" value="<?=$rows[0]['workphone']?>" <?= ($access['purorderbtwocwphonerequired']=='Yes')? 'required': ' ' ?>>
...
header("Location:purchaseorders.php?error=No Information Found");
}
}
else{
header("Location:purchaseorders.php?error=No Information Found");  
