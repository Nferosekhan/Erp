<?php
include('lcheck.php');
$sqlismainaccessfield=mysqli_query($con, "select * from pairmainaccess where userid='$companymainid' and moduletype='Purchase Orders' order by id  asc");
...
$sqlismainaccessuser=mysqli_query($con, "select * from pairmainaccess where userid='$userid' and moduletype='Purchase Orders' order by id  asc");
...
if (($infomainaccessuser['purorderveninfo']=='defone')||($infomainaccessuser['purorderveninfo']=='deftwo')) {
    $scriptonetwo = mysqli_real_escape_string($con, $_POST['vendorinfodefault']);
}
else{
    $scriptonetwo = 'three';
}
...
$sqlismainaccessuser=mysqli_query($con, "select * from pairmainaccess where userid='$userid' and moduletype='Purchase Orders' order by id  asc");
$infomainaccessuser=mysqli_fetch_array($sqlismainaccessuser);
if ($infomainaccessuser['purorderveninfo']=='one'||($scriptonetwo=='one')) {
...
if ((in_array('Term', $fieldadd))) {
$purchaseorderterm=mysqli_real_escape_string($con, $_POST['purchaseorderterm']);
}
else{
$purchaseorderterm="";
}

$sqlismainaccessuser=mysqli_query($con, "select moduleno, moduleprefix, modulesuffix from pairmainaccess where franchiseid='".$_SESSION['franchisesession']."' and moduletype='Purchase Orders' order by id  asc");
$infomainaccessuser=mysqli_fetch_array($sqlismainaccessuser);
$purchaseorderno=mysqli_real_escape_string($con, $infomainaccessuser['moduleprefix'].(str_pad($infomainaccessuser['modulesuffix'], 0, "0", STR_PAD_LEFT)));

$purchaseorderdate=mysqli_real_escape_string($con, $_POST['purchaseorderdate']);
if ((in_array('Due Date', $fieldadd))) {
...
$purchaseorderamount=mysqli_real_escape_string($con, $_POST['grandtotal']);
...
if ((in_array('Tax Table', $fieldadd))) {
...
$infomainaccessuser=mysqli_fetch_array($sqlismainaccessuser);
if ($infomainaccessuser['purorderveninfo']=='one'||($scriptonetwo=='one')) {
$venid=mysqli_real_escape_string($con, $_POST['vendor']);
$vensqlvenhis=mysqli_query($con,"select * from paircustomers where (franchisesession='".$_SESSION["franchisesession"]."' or cvisiblity='PUBLIC') and (createdid='$companymainid' and moduletype='Vendors') and id='".$venid."'");
$vensqlanscus = mysqli_fetch_array($vensqlvenhis);
...
$vengstin = mysqli_real_escape_string($con, $_POST['gstgstin']);
$venworkphone = mysqli_real_escape_string($con, $_POST['workworkphone']);
$venmobilephone = mysqli_real_escape_string($con, $_POST['workmobile']);
...
$vensqlup = "update paircustomers set createdon='$times', createdid='$companymainid', createdby='".$_SESSION["unqwerty"]."', franchisesession='".$_SESSION["franchisesession"]."',billstreet='$area',billcity='$city',billstate='$state',billpincode='$pincode',billcountry='$district',shipstreet='$sarea',shipcity='$scity',shipstate='$sstate',shippincode='$spincode',shipcountry='$sdistrict',gstrtype='$gstrtype',gstin='$vengstin',workphone='$venworkphone',mobile='$venmobilephone' where (franchisesession='".$_SESSION["franchisesession"]."' or cvisiblity='PUBLIC') and (createdid='$companymainid' and moduletype='Vendors') and id='$venid'";
            $venqueryup = mysqli_query($con, $vensqlup);
            $vench='';
                ...
                if($vench!='')
                {
                $vensqluse=mysqli_query($con, "insert into pairusehistory set usetype='VENDORS', createdon='$times',  createdby='".$_SESSION["unqwerty"]."', useid='$venid', useremarks='".$vench."' ");
                }
            }
               if ($infomainaccessuser['purorderveninfo']=='two'||($scriptonetwo=='two')) {
$address1 = '';
$address2 = '';
$saddress1 = '';
$saddress2 = '';
$vendorname = mysqli_real_escape_string($con, $_POST['twovendorname']);
...
$sqlismodulespublicnameven=mysqli_query($con, "select * from pairmodules where moduletype='Vendors' order by id  asc");
$infomodulespublicnameven=mysqli_fetch_array($sqlismodulespublicnameven);
$sqlismainaccesspublicnameven=mysqli_query($con, "select * from pairmainaccess where createdid='$companymainid' and moduletype='Vendors' and franchiseid='".$_SESSION['franchisesession']."' order by id  asc");
$infomainaccesspublicnameven=mysqli_fetch_array($sqlismainaccesspublicnameven);
$vensqlins = mysqli_query($con, "select count(customercode) from paircustomers where moduletype='Vendors'");
$venansins = mysqli_fetch_array($vensqlins);
$venoldid = $venansins[0];
$venvendorcode = $venoldid + 1;
$venpublicsql=mysqli_query($con,"select count(publicid) from paircustomers where createdid='$companymainid' and moduletype='Vendors'");
$venpublicans=mysqli_fetch_array($venpublicsql);
$venoldcodepublic=$venpublicans[0];
$venpubliccode=$infomodulespublicnameven['publiccolumn'] . $venoldcodepublic+1;
$venprivatesql=mysqli_query($con,"select count(privateid) from paircustomers where createdid='$companymainid' and moduletype='Vendors' and franchisesession='".$_SESSION['franchisesession']."'");
$venprivateans=mysqli_fetch_array($venprivatesql);
$venoldcodeprivate=$venprivateans[0];
$venprivatecode=$infomainaccesspublicnameven['moduleprefix'] . $venoldcodeprivate+1;
$sqlup = "insert into paircustomers set createdon='$times', createdid='$companymainid', createdby='".$_SESSION["unqwerty"]."', franchisesession='".$_SESSION["franchisesession"]."', customername='$vendorname',billstreet='$area',billcity='$city',billstate='$state',billpincode='$pincode',billcountry='$district',shipstreet='$sarea',shipcity='$scity',shipstate='$sstate',shippincode='$spincode',shipcountry='$sdistrict',sameasbilling='$twosameasbilling',gstrtype='$gstrtype',gstin='$gstno',moduletype='Vendors',placeos='$pos',workphone='$twoworkphone',mobile='$twomobilephone',customercode='$venvendorcode',publicid='$venpubliccode',privateid='$venprivatecode'";
...
// if($vendorid=='')
// {
// $sqlcon = "SELECT id From pairvendors WHERE franchisesession='".$_SESSION["franchisesession"]."' and vendorname = '{$vendorname}'";
...
// if($rowCountcon == 0)
// {
// $sqlup = "insert into pairvendors set createdon='$times', createdid='$companymainid', createdby='".$_SESSION["unqwerty"]."', franchisesession='".$_SESSION["franchisesession"]."', vendorname='$vendorname', city='$city', address='$area', pin='$pincode', state='$state', country='$district'";
// $queryup = mysqli_query($con, $sqlup);
// if(!$queryup){
// die("SQL query failed: " . mysqli_error($con));
// }
...
// $vendorid=$tid;
// mysqli_query($con, "INSERT INTO pairhistory (user, ipaddress, times, remarks, tid) VALUES ('{$username}', '{$ip}', '{$times}', 'Insert A Vendor', '{$tid}')");
// }
// }
// }
$sql2=mysqli_query($con, "SET sql_mode = ''");
$sql2=mysqli_query($con, "select id from pairpurchaseorders where franchisesession='".$_SESSION['franchisesession']."' and createdid='$companymainid' and purchaseorderno='$purchaseorderno' and purchaseorderdate='$purchaseorderdate'");
...
if($productname!='')
{
$sql=mysqli_query($con, "insert into pairpurchaseorders set createdon='$times', createdid='$companymainid', createdby='".$_SESSION["unqwerty"]."', franchisesession='".$_SESSION["franchisesession"]."', vendorname='$vendorname', vendorid='$vendorid', address1='$address1', address2='$address2', area='$area', city='$city', state='$state', district='$district', pincode='$pincode', saddress1='$saddress1', saddress2='$saddress2', sarea='$sarea', scity='$scity', sstate='$sstate', sdistrict='$sdistrict', spincode='$spincode', gstno='$gstno', purchaseorderterm='$purchaseorderterm', purchaseorderno='$purchaseorderno', purchaseorderdate='$purchaseorderdate', purchaseorderamount='$purchaseorderamount', duedate='$duedate', duedates='$duedates', productid='$productid', productname='$productname', manufacturer='$manufacturer', producthsn='$producthsn', productnotes='$productnotes', productdescription='$productdescription', itemmodule='$itemmodule', batch='$batch', expdate='$expdate', mrp='$mrp', vat='$vat', quantity='$quantity',unit='$unit', productrate='$productrate', noofpacks='$noofpacks', prodiscount='$prodiscount', productvalue='$productvalue',  taxvalue='$taxvalue',  productnetvalue='$productnetvalue', totalitems='$totalitems', orderdcno='$orderdcno', reference='$reference', saleperson='$saleperson', totalvatamount='$totalvatamount', totalamount='$totalamount', totalquantity='$totalquantity', discounttype='$discounttype', discount='$discount', discountamount='$discountamount', freightamount='$freightamount', roundoff='$roundoff', grandtotal='$grandtotal', preparedby='$preparedby',checkedby='$checkedby', taxtype='$taxtype', cgst25='$cgst25', sgst25='$sgst25', gst25='$gst25', cgst6='$cgst6', sgst6='$sgst6', gst6='$gst6', cgst9='$cgst9', sgst9='$sgst9', gst9='$gst9', cgst14='$cgst14', sgst14='$sgst14', gst14='$gst14', tax25='$tax25', tax6='$tax6', tax9='$tax9', tax14='$tax14', terms='$terms', notes='$notes', description='$description', fileattach='$fileattach',pos='$pos',workphone='$twoworkphone',mobile='$twomobilephone',sameasbilling='$twosameasbilling',vendorinfodefault='$scriptonetwo',gstrtype='$gstrtype'");
if($sql)
{
$salesid=mysqli_insert_id($con);
if(isset($_POST['procount']))
{
	$sql4=mysqli_query($con, "update pairproducts set openingstock=openingstock+$quantity where id='$productid'");
if($sql4)
{
$sqlibatch=mysqli_query($con, "select id from pairbatch where createdid='$companymainid' and franchisesession='".$_SESSION["franchisesession"]."' and productid='$productid' and batch='$batch'");
if(mysqli_num_rows($sqlibatch)>0)
{
$sql=mysqli_query($con, "update pairbatch set createdon='$times', createdid='$companymainid', createdby='".$_SESSION["unqwerty"]."', franchisesession='".$_SESSION["franchisesession"]."', productid='$productid', productname='$productname', manufacturer='$manufacturer', producthsn='$producthsn', productnotes='$productnotes', productdescription='$productdescription', batch='$batch', expdate='$expdate', mrp='$mrp', vat='$vat', quantity=quantity+$quantity, productrate='$productrate', noofpacks='$noofpacks', prodiscount='$prodiscount' where createdid='$companymainid' and franchisesession='".$_SESSION["franchisesession"]."' and productid='$productid' and batch='$batch'");
}
else
{
$sql=mysqli_query($con, "insert into pairbatch set createdon='$times', createdid='$companymainid', createdby='".$_SESSION["unqwerty"]."', franchisesession='".$_SESSION["franchisesession"]."', productid='$productid', productname='$productname', manufacturer='$manufacturer', producthsn='$producthsn', productnotes='$productnotes', productdescription='$productdescription', batch='$batch', expdate='$expdate', mrp='$mrp', vat='$vat', quantity=0+$quantity, productrate='$productrate', noofpacks='$noofpacks', prodiscount='$prodiscount'");
if(!$sql)
{
...
$tid=mysqli_insert_id($con);
mysqli_query($con, "INSERT INTO pairhistory (user, ipaddress, times, remarks, tid) VALUES ('{$username}', '{$ip}', '{$times}', 'Insert A Purchase Order', '{$tid}')");

$sql3=mysqli_query($con, "update pairfranchises set purchaseordersuffix=purchaseordersuffix+1 where tdelete='0' and id='".$_SESSION['franchisesession']."'");
if($sql3)
{
 if(isset($_POST['submit1']))
{
echo '<script> window.open("purchaseorderprint.php?purchaseorderno='.$purchaseorderno.'&purchaseorderdate='.$purchaseorderdate.'", "_blank");</script>';
echo '<script> window.location.href="purchaseorders.php?remarks=Added Successfully";</script>'; 
}
else
{
echo '<script> window.location.href="purchaseorderview.php?purchaseorderno='.$purchaseorderno.'&purchaseorderdate='.$purchaseorderdate.'&remarks=Added Successfully";</script>';
}
}
}
else
{
header("Location: purchaseorders.php?error=Error Data");
}
}
...
$sqlismainaccessuser=mysqli_query($con, "select moduleno, moduleprefix, modulesuffix,modulename from pairmainaccess where franchiseid='".$_SESSION['franchisesession']."' and moduletype='Purchase Orders' order by id  asc");
$infomainaccessuser=mysqli_fetch_array($sqlismainaccessuser);
?>
...
<span type="button" onclick="funesvendorview()" class="close" data-dismiss="modal"
aria-label="Close">
<span aria-hidden="true" id="procloseicon">&times;</span>
</span>
</div>
...
<h5 class="modal-title" style="color:#212529;">New <?= $infomainaccessuserven['modulename'] ?></h5>
<span type="button" onclick="funesvendor()" class="close" data-dismiss="modal"
aria-label="Close">
...
onclick="funesvendor()">Cancel</button> </div>
</div>
</form>
</div>
</div>
...
<!-- </form> -->
<!-- End AddNewVendor modal -->
<!-------------------vendor add info end ------------------------------->
<form id="purchaseorderform" action="" onsubmit="return checkvalidate()" method="post" enctype="multipart/form-data" class="form-horizontal" role="form">
<div class="row">
<!-- <div class="col-lg-6">
...
<a class="customcont-heading" style="padding: 7px 0 7px;">Vendor Information</a>
</div>
</button>
</h5>
...
<div class="col-sm-4">
<label for="vendorname" class="custom-label"><span class="text-danger">Vendor Name *</span></label>
</div>
<div class="col-sm-8">
<input type="text" class="form-control  form-control-sm" id="vendorname" name="vendorname" required placeholder="Vendor Name">
</div>
</div>
</div>
</div>
...
<div class="form-group row">
<div class="col-sm-4">
<label for="vendorname" class="custom-label">Billing Address</label>
</div>
<div class="col-sm-5">
...
<?php
     if ((in_array('Purchase Order Information', $fieldadd))) {
        ?>
<div class="col-lg-4">
<div class="accordion" id="accordionRental">
<div class="accordion-item mb-1">
...
<div class="col-lg-5">
<label for="purchaseorderno" class="custom-label"><span class="text-danger"><?= $infomainaccessuser['modulename'] ?> Number *</span></label>
</div>
<div class="col-lg-7">
<input type="text" class="form-control  form-control-sm" id="purchaseorderno" name="purchaseorderno" required value="<?=$infomainaccessuser['moduleprefix']?><?=str_pad(($infomainaccessuser['modulesuffix']+1), 0, "0", STR_PAD_LEFT)?>" readonly>
</div>
</div>
</div>
<div class="col-lg-12">
...
<label for="purchaseorderdate" class="custom-label"><span class="text-danger"><?= $infomainaccessuser['modulename'] ?> Date *</span></label>
</div>
<div class="col-lg-7">
    <?php
...
$dt = new DateTime();
    ?>
<input type="date" class="form-control  form-control-sm" id="purchaseorderdate" name="purchaseorderdate" value="<?= $dt->format('Y-m-d') ?>" required>
</div>
</div>
</div>
...
<div class="form-group row" style="align-items: center !important;">
<div class="col-lg-5" style="padding-right:0px">
<label for="purchaseorderdate" class="custom-label"><span class="text-danger"><?= $infomainaccessuser['modulename'] ?> Term *</span></label>
</div>
<div class="col-lg-7">
<select class="select2-field form-control  form-control-sm" name="purchaseorderterm" id="purchaseorderterm" required>
...
<div class="col-lg-4 duedatepicker">
<input type="date" class="form-control  form-control-sm" id="duedate" name="duedate" style="padding: 3px !important;">
</div>
<script>
...
today=addDays(purchaseorderdate, duedates);
...
<div class="customcont-header ml-0 mb-1" style="height: 30px;">
<a class="customcont-heading" style="padding: 7px 0 7px;"><?= $infomainaccessuserven['modulename'] ?> Information</a>
    ...
    if (($infomainaccessuser['purorderveninfo']=='defone')||($infomainaccessuser['purorderveninfo']=='deftwo')) {
     ...
     <div class="row">
                      ...
                      <div class="custom-control custom-radio mr-sm-2">
                        <input type="radio" class="custom-control-input" name="vendorinfodefault" id="onevendorinfo" value="one" <?= ($infomainaccessuser['purorderveninfo']=='defone')?'checked':'' ?> onclick="onetwo()">
                        <label class="custom-control-label custom-label" for="onevendorinfo">B2B</label>
                      </div>
                      
                      </div>
                      <div class="col-lg-2 col-md-2 col-3">
                      <div class="custom-control custom-radio mr-sm-2">
                        <input type="radio" class="custom-control-input" name="vendorinfodefault" id="twovendorinfo" value="two" <?= ($infomainaccessuser['purorderveninfo']=='deftwo')?'checked':'' ?> onclick="onetwo()">
                        <label class="custom-control-label custom-label" for="twovendorinfo">B2C</label>
                      </div>
...
<?php
if ($access['purorderbtwocnamerequired']=='Yes') {
?>
$("#twovendorname").removeAttr("required");
<?php
}
if ($access['purorderbtwocwphonerequired']=='Yes') {
?>
...
<div class="accordion-body text-sm" style="padding-bottom: 0px;padding-top: 3px">
    ...
    $sqlismainaccessuser=mysqli_query($con, "select * from pairmainaccess where userid='$userid' and moduletype='Purchase Orders' order by id  asc");
$infomainaccessuser=mysqli_fetch_array($sqlismainaccessuser);
    if ($infomainaccessuser['purorderveninfo']=='one'||(($infomainaccessuser['purorderveninfo']=='defone')||($infomainaccessuser['purorderveninfo']=='deftwo'))) {
    ?>
    <div id="one">
<div class="row">
<div class="col-lg-12">
...
<input type="hidden" name="vendorid" id="vendorid">
<div class="col-sm-3">
<label for="vendorname" class="custom-label"><span class="text-danger"><?= $infomainaccessuserven['modulename'] ?> Name *</span></label>
</div>
...
</svg>&nbsp; View <?= $infomainaccessuserven['modulename'] ?> Details </span>
<!---->
...
if ($infomainaccessuser['purorderveninfo']=='two'||(($infomainaccessuser['purorderveninfo']=='defone')||($infomainaccessuser['purorderveninfo']=='deftwo'))) {
?>
<script type="text/javascript">
function twoposans() {
...
<?= ($access['purorderbtwocnamerequired']=='Yes')? '<span class="text-danger">'. $infomainaccessuserven['modulename'] . ' Name * </span>': $infomainaccessuserven['modulename'] . ' Name ' ?>
</label>
</div>
<div class="col-lg-9">
<input type="text" name="twovendorname" id="twovendorname" class="form-control form-control-sm" placeholder="<?= $infomainaccessuserven['modulename'] ?> Name" <?= ($access['purorderbtwocnamerequired']=='Yes')? 'required': ' ' ?>>
</div>
</div>
<div class="row mt-1">
<div class="col-lg-12">
...
<?= ($access['purorderbtwocwphonerequired']=='Yes')? '<span class="text-danger"> WORK PHONE * </span>': ' WORK PHONE ' ?>
</label>
<input type="text" class="form-control form-control-sm" id="twoworkphone" name="twoworkphone" placeholder="Work Phone" <?= ($access['purorderbtwocwphonerequired']=='Yes')? 'required': ' ' ?>>
</div>
...
<input type="text" class="form-control form-control-sm" id="twogsttreatment" name="twogsttreatment" placeholder="GST TREATMENT" value="Consumer" readonly>
</div>
...
<input type="text" class="form-control form-control-sm" id="twogstin" name="twogstin" placeholder="GSTIN">
</div>
</div>
</div>
...
<option disabled value="Select Place of Supply" <?=($infomainaccessuser['purordertwopos']=='Select Place of Supply')?'selected':''?>>Select The Place</option> 
<option value="JAMMU AND KASHMIR (1)" <?=($infomainaccessuser['purordertwopos']=="JAMMU AND KASHMIR (1)")?'selected':''?>>JAMMU AND KASHMIR (1)</option>
<option value="ANDAMAN AND NICOBAR ISLANDS (35)" <?=($infomainaccessuser['purordertwopos']=="ANDAMAN AND NICOBAR ISLANDS (35)")?'selected':''?>>ANDAMAN AND NICOBAR ISLANDS (35)</option>
<option value="ANDHRA PRADESH (NEWLY ADDED) (37)" <?=($infomainaccessuser['purordertwopos']=="ANDHRA PRADESH (NEWLY ADDED) (37)")?'selected':''?>>ANDHRA PRADESH (NEWLY ADDED) (37)</option>
<option value="ANDHRA PRADESH(BEFORE DIVISION) (28)" <?=($infomainaccessuser['purordertwopos']=="ANDHRA PRADESH(BEFORE DIVISION) (28)")?'selected':''?>>ANDHRA PRADESH(BEFORE DIVISION) (28)</option>
<option value="ARUNACHAL PRADESH (12)" <?=($infomainaccessuser['purordertwopos']=="ARUNACHAL PRADESH (12)")?'selected':''?>>ARUNACHAL PRADESH (12)</option>
<option value="ASSAM (18)" <?=($infomainaccessuser['purordertwopos']=="ASSAM (18)")?'selected':''?>>ASSAM (18)</option>
<option value="BIHAR (10)" <?=($infomainaccessuser['purordertwopos']=="BIHAR (10)")?'selected':''?>>BIHAR (10)</option>
<option value="CENTRE JURISDICTION (99)" <?=($infomainaccessuser['purordertwopos']=="CENTRE JURISDICTION (99)")?'selected':''?>>CENTRE JURISDICTION (99)</option>
<option value="CHANDIGARH (4)" <?=($infomainaccessuser['purordertwopos']=="CHANDIGARH (4)")?'selected':''?>>CHANDIGARH (4)</option>
<option value="CHATTISGARH (22)" <?=($infomainaccessuser['purordertwopos']=="CHATTISGARH (22)")?'selected':''?>>CHATTISGARH (22)</option>
<option value="DADRA AND NAGAR HAVELI AND DAMAN AND DIU (NEWLY MERGED UT) (26*)" <?=($infomainaccessuser['purordertwopos']=="DADRA AND NAGAR HAVELI AND DAMAN AND DIU (NEWLY MERGED UT) (26*)")?'selected':''?>>DADRA AND NAGAR HAVELI AND DAMAN AND DIU (NEWLY MERGED UT) (26*)</option>
<option value="DELHI (7)" <?=($infomainaccessuser['purordertwopos']=="DELHI (7)")?'selected':''?>>DELHI (7)</option>
<option value="GOA (30)" <?=($infomainaccessuser['purordertwopos']=="GOA (30)")?'selected':''?>>GOA (30)</option>
<option value="GUJARAT (24)" <?=($infomainaccessuser['purordertwopos']=="GUJARAT (24)")?'selected':''?>>GUJARAT (24)</option>
<option value="HARYANA (6)" <?=($infomainaccessuser['purordertwopos']=="HARYANA (6)")?'selected':''?>>HARYANA (6)</option>
<option value="HIMACHAL PRADESH (2)" <?=($infomainaccessuser['purordertwopos']=="HIMACHAL PRADESH (2)")?'selected':''?>>HIMACHAL PRADESH (2)</option>
<option value="JHARKHAND (20)" <?=($infomainaccessuser['purordertwopos']=="JHARKHAND (20)")?'selected':''?>>JHARKHAND (20)</option>
<option value="KARNATAKA (29)" <?=($infomainaccessuser['purordertwopos']=="KARNATAKA (29)")?'selected':''?>>KARNATAKA (29)</option>
<option value="KERALA (32)" <?=($infomainaccessuser['purordertwopos']=="KERALA (32)")?'selected':''?>>KERALA (32)</option>
<option value="LADAKH (NEWLY ADDED) (38)" <?=($infomainaccessuser['purordertwopos']=="LADAKH (NEWLY ADDED) (38)")?'selected':''?>>LADAKH (NEWLY ADDED) (38)</option>
<option value="LAKSHADWEEP (31)" <?=($infomainaccessuser['purordertwopos']=="LAKSHADWEEP (31)")?'selected':''?>>LAKSHADWEEP (31)</option>
<option value="MADHYA PRADESH (23)" <?=($infomainaccessuser['purordertwopos']=="MADHYA PRADESH (23)")?'selected':''?>>MADHYA PRADESH (23)</option>
<option value="MAHARASHTRA (27)" <?=($infomainaccessuser['purordertwopos']=="MAHARASHTRA (27)")?'selected':''?>>MAHARASHTRA (27)</option>
<option value="MANIPUR (14)" <?=($infomainaccessuser['purordertwopos']=="MANIPUR (14)")?'selected':''?>>MANIPUR (14)</option>
<option value="MEGHALAYA (17)" <?=($infomainaccessuser['purordertwopos']=="MEGHALAYA (17)")?'selected':''?>>MEGHALAYA (17)</option>
<option value="MIZORAM (15)" <?=($infomainaccessuser['purordertwopos']=="MIZORAM (15)")?'selected':''?>>MIZORAM (15)</option>
<option value="NAGALAND (13)" <?=($infomainaccessuser['purordertwopos']=="NAGALAND (13)")?'selected':''?>>NAGALAND (13)</option>
<option value="ODISHA (21)" <?=($infomainaccessuser['purordertwopos']=="ODISHA (21)")?'selected':''?>>ODISHA (21)</option>
<option value="OTHER TERRITORY (97)" <?=($infomainaccessuser['purordertwopos']=="OTHER TERRITORY (97)")?'selected':''?>>OTHER TERRITORY (97)</option>
<option value="PUDUCHERRY (34)" <?=($infomainaccessuser['purordertwopos']=="PUDUCHERRY (34)")?'selected':''?>>PUDUCHERRY (34)</option>
<option value="PUNJAB (3)" <?=($infomainaccessuser['purordertwopos']=="PUNJAB (3)")?'selected':''?>>PUNJAB (3)</option>
<option value="RAJASTHAN (8)" <?=($infomainaccessuser['purordertwopos']=="RAJASTHAN (8)")?'selected':''?>>RAJASTHAN (8)</option>
<option value="SIKKIM (11)" <?=($infomainaccessuser['purordertwopos']=="SIKKIM (11)")?'selected':''?>>SIKKIM (11)</option>
<option value="TAMIL NADU (33)"  <?=($infomainaccessuser['purordertwopos']=="TAMIL NADU (33)")?'selected':''?>>TAMIL NADU (33)</option>
<option value="TELANGANA (36)" <?=($infomainaccessuser['purordertwopos']=="TELANGANA (36)")?'selected':''?>>TELANGANA (36)</option>
<option value="TRIPURA (16)" <?=($infomainaccessuser['purordertwopos']=="TRIPURA (16)")?'selected':''?>>TRIPURA (16)</option>
<option value="UTTAR PRADESH (9)" <?=($infomainaccessuser['purordertwopos']=="UTTAR PRADESH (9)")?'selected':''?>>UTTAR PRADESH (9)</option>
<option value="UTTARAKHAND (5)" <?=($infomainaccessuser['purordertwopos']=="UTTARAKHAND (5)")?'selected':''?>>UTTARAKHAND (5)</option>
<option value="WEST BENGAL (19)" <?=($infomainaccessuser['purordertwopos']=="WEST BENGAL (19)")?'selected':''?>>WEST BENGAL (19)</option>
</select>
</div>
</div>
</div>
...
<td data-label="ITEM DETAILS" style="padding-top: 1px !important;"><input type="hidden" name="productid[]" id="productid1">
...
<span class="badge" style="display:none; width:75px; padding:3px; margin:5px 0; background-color: #57b729; font-size:75%;border-radius: 0px !important;" id="itemmodulespan1"></span><br><input type="hidden" name="itemmodule[]" id="itemmodule1">
<div <?=(in_array('Product Category', $fieldadd))?'':'style="display:none !important;"'?>>
<span id="productmanufacturerspan1" style="display:none; font-size:11px;"><?=$access['txtnamecategory']?>:<input type="text" name="manufacturer[]" id="manufacturer1" class="form-control form-control-sm proitemselect" style="display:none;width: 63px !important;padding: 0px !important;height: 18px !important;border: 1px solid #eee !important;" readonly> <span id="productmrpval1" style=" font-size:11px;" class="text-primary"></span> <span id="productmanufacturerval1" style=" font-size:11px;" class="text-primary"></span>
...
<span id="producthsncodespan1" style="display:none; font-size:11px;">HSN Code:</span><input type="text" name="producthsn[]" id="producthsn1" class="form-control form-control-sm proitemselect" style="display:none;width: 27px !important;padding: 0px !important;height: 18px !important;border: 1px solid #eee !important;"> <span id="producthsncodeval1" style=" font-size:11px;" class="text-primary"></span>
...
<span <?=(in_array('Product Description', $fieldadd))?'':'style="display:none !important;"'?>><span id="productdescriptionspan1" style="display:none; font-size:11px;">Description:</span><textarea class="form-control form-control-sm" name="productdescription[]" id="productdescription1" style="height:50px; display:none; border:1px solid #eee !important;width: 146px !important;"></textarea></span></td>
<td style="display:none"><input type="text" name="productnotes[]" id="productnotes1" class="form-control form-control-sm proitemselect bordernoneinput"></td>

<td data-label="BATCH" <?=(in_array('Batch', $fieldadd))?'':'style="display:none;"'?>><div><input type="text" name="batch[]" id="batch1" onClick="batchget(1);" onFocus="batchget(1);"  class="form-control form-control-sm proitemselect" style="margin-bottom: 3px !important;border: 1px solid #eee !important;text-align: right !important;padding: 0px !important;" list="" autocomplete="off"></div>
<div>
<span id="productexpdatespan1" style="display:none; font-size:11px;">EXPIRY:</span>
<input type="date" name="expdate[]" id="expdate1" class="form-control form-control-sm proitemselect" style="display:none;width: 94px !important;padding: 0px !important;height: 18px !important;"> <span id="productexpdateval1" style=" font-size:11px;" class="text-primary"></span>
...
<div>
<div id="outfordone1" class="dvi" style="display:none;"></div>
...
<td data-label="RATE">
<div><span style="font-size:15px !important;"><?php echo $rescurrency[0]; ?></span>
<input type="number" min="0" step="0.01" name="productrate[]"   required id="productrate1" class="form-control form-control-sm proitemselect productselectwidth" onChange="productcalc(1)" onClick="rateget(1);" onFocus="rateget(1);" style="border: 1px solid #eee !important;margin-bottom: 3px !important;text-align: right !important;padding: 0px !important;"></div>
<div <?=(in_array('Product Mrp', $fieldadd))?'':'style="display:none !important;"'?>>
<span id="productmrpspan1" style="display:none; font-size:11px;">MRP:<input type="number" name="mrp[]" id="mrp1" class="form-control form-control-sm proitemselect" style="display:none;width: 63px !important;padding: 0px !important;height: 18px !important;border: 1px solid #eee !important;" min="0" step="0.01"> <span id="productmrpval1" style=" font-size:11px;" class="text-primary"></span>
...
<div>
<div id="ratelist1" class="dvi" style="display:none;"></div>
<input type="text" id="errrate1" style="display:none;">
</div>
</td>
<td data-label="QUANTITY"><div><input type="number" min="0" step="0.01" name="quantity[]" required id="quantity1" class="form-control form-control-sm proitemselect productselectwidth" onChange="productcalc(1)" style="margin-bottom: 3px !important;border: 1px solid #eee !important;text-align: right !important;padding: 0px !important;"></div>
<div>
<span id="productunitspan1" style="display:none; font-size:11px;">UNIT:</span><input type="text" name="productunit[]" id="productunit1" class="form-control form-control-sm proitemselect" style="display:none;width: 33px !important;padding: 0px !important;height: 18px !important;" readonly>
<span id="productunitval1" style=" font-size:11px;" class="text-primary"></span>
<span id="productunitedit1" style="display:none; font-size:11px; cursor:pointer" class="text-blue" onclick="editunit(1)"><i class="fa fa-edit"></i></span>
<span id="productunitupdate1" style="display:none; font-size:11px; cursor:pointer" class="text-success" onclick="changeunit(1)"><i class="fa fa-save"></i></span>
</div>
<span id="productnoofpacksspan1" style="display:none; font-size:11px;">PACK:</span><input type="text" name="noofpacks[]" id="noofpacks1" class="form-control form-control-sm proitemselect" style="display:none;width: 33px !important;padding: 0px !important;height: 18px !important;"> <span id="productnoofpacksval1" style=" font-size:11px;" class="text-primary"></span>
...
<td data-label="TAXABLE VALUE" <?=(in_array('Taxable Value', $fieldadd))?'':'style="display:none;"'?>><!---<div class="input-group" id="ruppeitemtable"><div class="input-group-prepend"><div class="input-group-text" style="color: #495057;padding: 13px 3.75px;height:21px;font-size: 16px !important;background-color: transparent;"><?php echo $rescurrency[0]; ?></div></div><input type="number" min="0" step="0.01" name="productvalue[]" id="productvalue1" class="form-control form-control-sm proitemselect"style="background:none;margin-bottom: 3px !important;border: 1px solid #eee !important;" readonly ></div>-->
<div id="ruppeitemtablemobdfs"><span style="font-size:15px !important;"><?php echo $rescurrency[0]; ?></span>
<input type="number" min="0" step="0.01" name="productvalue[]" id="productvalue1" class="form-control form-control-sm proitemselect"style="background:none;margin-bottom: 3px !important;border: 1px solid #eee !important;text-align: right !important;padding: 0px !important;" readonly ></div>
<span id="productprodiscountspan1" style="display:none; font-size:11px;">DISCOUNT:</span><div class="input-group input-group-sm" style="width: max-content !important;display: none;" id="discountselect1">
<div class="input-group-prepend">
<input type="number" min="0" step="0.01" name="prodiscount[]" id="prodiscount1" class="form-control form-control-sm proitemselect" style="display:none;width: 27px !important;padding: 0px !important;height: 18px !important;" onChange="productcalc(1)">
</div>
...
</div> <span id="productprodiscountval1" style=" font-size:11px;" class="text-primary"></span>
...
<td data-label="TAX VALUE" <?=(in_array('Tax Value', $fieldadd))?'':'style="display:none;"'?>><!--<div class="input-group" id="ruppeitemtable"><div class="input-group-prepend"><div class="input-group-text" style="color: #495057;padding: 13px 3.75px;height:21px;font-size: 16px !important;background-color: transparent;"><?php echo $rescurrency[0]; ?></div></div><input type="number" min="0" step="0.01" name="taxvalue[]" id="taxvalue1" class="form-control form-control-sm proitemselect"style="background:none;margin-bottom: 3px !important;border: 1px solid #eee !important;" readonly ></div>-->
<div id="ruppeitemtablemobasdf"><span style="font-size:15px !important;"><?php echo $rescurrency[0]; ?></span>
<input type="number" min="0" step="0.01" name="taxvalue[]" id="taxvalue1" class="form-control form-control-sm proitemselect"style="background:none;margin-bottom: 3px !important;border: 1px solid #eee !important;text-align: right !important;padding: 0px !important;" readonly ></div>
<span id="productvatspan1" style="display:none; font-size:11px;">GST:<input type="number" min="0" step="0.01" name="vat[]" id="vat1" class="form-control form-control-sm proitemselect" style="display:none;width: 27px !important;padding: 0px !important;height: 18px !important;" onChange="productcalc(1)"> <span id="productvatval1" style=" font-size:11px;" class="text-primary"></span>
...
<td data-label="AMOUNT"><!--<div class="input-group" id="ruppeitemtable"><div class="input-group-prepend"><div class="input-group-text" style="color: #495057;padding: 13px 3.75px;height:21px;font-size: 16px !important;background-color: transparent;"><?php echo $rescurrency[0]; ?></div></div><input type="number" min="0" step="0.01" name="productnetvalue[]" id="productnetvalue1" class="form-control form-control-sm proitemselect"style="background:none;margin-bottom: 3px !important;border: 1px solid #eee !important;" readonly ></div>-->
<div id="ruppeitemtablemobasdf"><span style="font-size:15px !important;"><?php echo $rescurrency[0]; ?></span>
<input type="number" min="0" step="0.01" name="productnetvalue[]" id="productnetvalue1" class="form-control form-control-sm proitemselect"style="background:none;margin-bottom: 3px !important;border: 1px solid #eee !important;text-align: right !important;padding: 0px !important;" readonly ></div></td>
...
<tbody>
<tr>
<td style="text-align: right;justify-content: right;justify-items: right;margin-right: 0px !important;padding-right: 0px !important;background-color:#E9ECEF !important;border: 1px solid #999;" data-label="TAXABLE VALUE"><input type="text" name="tax25" id="tax25" class="form-control form-control-sm bordernoneinput taxabledesign"  readonly style="background-color:#E9ECEF !important;text-align: right !important;"></td>
<td style="text-align: right;justify-content: right;justify-items: right;margin-right: 0px !important;padding-right: 0px !important;background-color:#E9ECEF !important;border: 1px solid #999;border-left: 1px dashed #999;border-right: 1px dashed #999;" data-label="CGST %">2.5%</td>
<td style="text-align: right;justify-content: right;justify-items: right;margin-right: 0px !important;padding-right: 0px !important;background-color:#E9ECEF !important;border: 1px solid #999;border-left: 1px dashed #999;" data-label="CGST"><div><?php echo $rescurrency[0]; ?> <input type="text" name="cgst25" id="cgst25" class="form-control form-control-sm amountdesign"  readonly style="background-color:#E9ECEF !important;text-align: right !important;"></div></td>
...
<tr>
<td style="text-align: right;justify-content: right;justify-items: right;margin-right: 0px !important;padding-right: 0px !important;background-color:#E9ECEF !important;border: 1px solid #999;" data-label="TAXABLE VALUE"><input type="text" name="tax6" id="tax6"  class="form-control form-control-sm taxabledesign"  readonly style="background-color:#E9ECEF !important;text-align: right !important;" ></td><td data-label="CGST %" style="border-left: 1px dashed #999;border-right: 1px dashed #999 !important;text-align: right;justify-content: right;justify-items: right;margin-right: 0px !important;padding-right: 0px !important;background-color:#E9ECEF !important;border: 1px solid #999;" >6%</td><td style="text-align: right;justify-content: right;justify-items: right;margin-right: 0px !important;padding-right: 0px !important;background-color:#E9ECEF !important;border: 1px solid #999;border-left: 1px dashed #999;" data-label="CGST"><div><?php echo $rescurrency[0]; ?><input type="text" name="cgst6" id="cgst6"  class="form-control form-control-sm amountdesign"  readonly style="background-color:#E9ECEF !important;text-align: right !important;" ></div></td>
...
<tr>
<td style="text-align: right;justify-content: right;justify-items: right;margin-right: 0px !important;padding-right: 0px !important;background-color:#E9ECEF !important;border: 1px solid #999;" data-label="TAXABLE VALUE"><input type="text" name="tax9" id="tax9"  class="form-control form-control-sm taxabledesign"  readonly style="background-color:#E9ECEF !important;text-align: right !important;" ></td><td data-label="CGST %" style="border-left: 1px dashed #999;border-right: 1px dashed #999 !important;text-align: right;justify-content: right;justify-items: right;margin-right: 0px !important;padding-right: 0px !important;background-color:#E9ECEF !important;border: 1px solid #999;" >9%</td><td style="text-align: right;justify-content: right;justify-items: right;margin-right: 0px !important;padding-right: 0px !important;background-color:#E9ECEF !important;border: 1px solid #999;border-left: 1px dashed #999;" data-label="CGST"><div><?php echo $rescurrency[0]; ?><input type="text" name="cgst9" id="cgst9"  class="form-control form-control-sm amountdesign"  readonly style="background-color:#E9ECEF !important;text-align: right !important;" ></div></td>
...
<tr>
<td style="text-align: right;justify-content: right;justify-items: right;margin-right: 0px !important;padding-right: 0px !important;background-color:#E9ECEF !important;border: 1px solid #999;" data-label="TAXABLE VALUE"><input type="text" name="tax14" id="tax14"  class="form-control form-control-sm taxabledesign"  readonly style="background-color:#E9ECEF !important;text-align: right !important;" ></td><td data-label="CGST %" style="border-left: 1px dashed #999;border-right: 1px dashed #999 !important;text-align: right;justify-content: right;justify-items: right;margin-right: 0px !important;padding-right: 0px !important;background-color:#E9ECEF !important;border: 1px solid #999;" >14%</td> <td style="text-align: right;justify-content: right;justify-items: right;margin-right: 0px !important;padding-right: 0px !important;background-color:#E9ECEF !important;border: 1px solid #999;border-left: 1px dashed #999;" data-label="CGST"><div><?php echo $rescurrency[0]; ?><input type="text" name="cgst14" id="cgst14"  class="form-control form-control-sm amountdesign"  readonly style="background-color:#E9ECEF !important;text-align: right !important;" ></div></td>
...
<select name="discounttype" id="discounttype" class="form-control form-control-sm" style="border: 1px solid #e0e3e6 !important;background-color: #f5f5f5;" onChange="discount1()">
<option value="0">%</option>
<option value="1"><?php echo $rescurrency[0]; ?></option>
</select>
</div> -->

<!-- <div class="input-group"><div class="input-group-prepend"><div class="input-group-text" style="color: #495057;padding: 13px 3.75px;height:21px;font-size: 16px !important;background-color: transparent;"><span style="float:left;font-size: 13.6px !important;margin-left: -3px !important;color: #212529 !important">Discount &nbsp; &nbsp; </span><input type="text" name="discount" class="form-control form-control-sm"  id="discount" value="0" onChange="discount1()" style="width:50px;border: 1px solid #e0e3e6 !important;"><select name="discounttype" id="discounttype" class="form-control form-control-sm" style="border: 1px solid #e0e3e6 !important;background-color: #f5f5f5;" onChange="discount1()">
<option value="0">%</option>
<option value="1"><?php echo $rescurrency[0]; ?></option>
</select></div></div>
</div> -->
...
<input required type="text" name="discountamount" id="discountamount" class="form-control form-control-sm " style="background:none; text-align:right;border:1px solid #eee;" value="0" onChange="productcalc1()" >
</div>
</div>
</div>
<div class="row mb-1" >
...
<div class="col-6">
<div class="input-group"><div class="input-group-prepend"><div class="input-group-text" style="color: #495057;padding: 13px 3.75px;height:21px;font-size: 16px !important;background-color: transparent;"><?php echo $rescurrency[0]; ?></div></div>
<input required type="text" name="totalvatamount" id="totalvatamount" class="form-control form-control-sm " style="background:none; text-align:right;border: 1px solid #eee;"  readonly  value="0" ></div>
</div>
</div>
...
<div class="col-6">
<div class="input-group"><div class="input-group-prepend"><div class="input-group-text" style="color: #495057;padding: 13px 3.75px;height:21px;font-size: 16px !important;background-color: transparent;"><?php echo $rescurrency[0]; ?></div></div>
<input required type="text" name="roundoff" id="roundoff" class="form-control form-control-sm " style="background:none; text-align:right;border:1px solid #eee;" value="0" onChange="productcalcround()" ></div>
</div>
</div>
...
<div class="col-6">
<div class="input-group"><div class="input-group-prepend"><div class="input-group-text" style="color: #495057;padding: 13px 3.75px;height:21px;font-size: 16px !important;background-color: transparent;"><?php echo $rescurrency[0]; ?></div></div>
<input required type="text" name="grandtotal" id="grandtotal" class="form-control form-control-sm " style="background:none; text-align:right; font-size:14.6px;border: 1px solid #eee;font-weight: 700 !important;" value="0" onChange="productcalc1()" readonly ></div>
</div>
</div>
<div class="row mb-1">
<div class="col-12">
...
<label style="margin-bottom:5px; padding-bottom:5px;" class="custom-label"><input type="checkbox" name="procount" id="procount" value="1"> Pro</label>
...
href="purchaseorders.php" style="margin-top:-3px !important;">Cancel</a>
...
$("#vendor").on("select2:open", function() {
<?php
if($access['purordernewvendordef']=='1'){
?>
$("#configureunits").show();
...
$("#product1").on("select2:open", function() {
<?php
if($access['purordernewproductdef']=='1'){
?>
$("#configureunits").show();
...
$('#custvendor').append('<option value="' + custcustomerdname.value + '">' + custcustomerdname.value +
'</option>');
...
{
if (custvendordname == '') {alert("Please Enter the <?= $infomainaccessuserven['modulename'] ?> Name");}
...
$('#vendor').append('<option value="' + resarray[1] + '">' + custvendordname + ' - ' + billcity+ ' - ' + mobilephone + '</option>');
...
$.get("ratesearchbill.php", {term: productid,custid: customerid} , function(datas){
console.log("normal"+datas);
...
for (var key in objbatch) {
  console.log("key"+key);
  check+="<div id='option"+lineNo+objbatch[key].billno+"inv' style='border:1px solid #cccccc;'><table width='100%'><tr style='border-bottom:none;'><td align='left' id='invno"+objbatch[key].billno+"inv' style='border:none;'>Bill No : "+objbatch[key].billno+" </td><td align='right' id='invdt"+objbatch[key].billno+"inv' style='border:none;'>Bill Date : "+objbatch[key].billdate+" </td></tr><tr style='border-bottom:none;'><td align='left' id='rate"+objbatch[key].productrate+"inv' style='border:none;'>Product Rate : "+objbatch[key].productrate+" </td></tr></table></div>";
}
console.log(check);
...
source: 'vendorsearch.php', select: function (event, ui) { $("#area").val(ui.item.address); $("#city").val(ui.item.city); $("#district").val(ui.item.country); $("#state").val(ui.item.state); $("#pincode").val(ui.item.pin); $( "#productname1" ).focus();}, minLength: 1
}); */
$( "#area" ).autocomplete({
...
console.log(obj[0]);
<?php
$sqlismainaccessfieldcusview=mysqli_query($con, "select * from pairmainaccess where userid='$companymainid' and moduletype='Vendors' order by id  asc");
...
<script type="text/javascript">
$(function() {
$("#purchaseordersuffix").autocomplete({
source: 'purchaseordersuffixsearch.php',
select: function(event, ui) {
$("#purchaseordersuffix").val(ui.item.purchaseordersuffix);
...
<!------------------------------------------- pro script end ------------------------------------------->
<!--term start-->
<div class="modal fade" id="AddNewPurchaseOrderTerm" tabindex="-1" role="dialog" style="z-index: 1051;">
<div class="modal-dialog" role="document">
<div class="modal-content">
<div class="modal-header">
<h5 class="modal-title">New <?= $infomainaccessuser['modulename'] ?> Term</h5>
<span type="button" onclick="funespurchaseorderterm()" class="close" data-dismiss="modal"
aria-label="Close">
...
<div class="form-group row">
<div class="col-sm-6">
<label for="missingpurchaseorderterm" class="custom-label"><span class="text-danger">
<?= $infomainaccessuser['modulename'] ?> Term *</span></label>
</div>
<div class="col-sm-6">
<input type="text" name="purchaseorderterm" class="form-control form-control-sm mb-4" id="missingpurchaseorderterm" placeholder="Name" required>
</div>
</div>
</div>
</div>
...
<div class="col">
<button   onclick="funaddpurchaseorderterm()" class="btn btn-primary btn-sm btn-custom arlina-button expand-left"   type="submit"  name="submitpurchaseorderterm" value="Submit">
<span class="label">Save</span> <span class="spinner"></span>
</button>
<button type="button"
class="btn btn-primary btn-sm btn-custom-grey"
onclick="funespurchaseorderterm()">Cancel</button> </div>
</div>
</div>
</div>
</div>
<script>
$("#purchaseorderterm").on("change", function() {
var sOptionVal = $(this).val();
if (sOptionVal == '#AddNewPurchaseOrderTerm') {
$('#AddNewPurchaseOrderTerm').modal('show');
}
});
function funaddpurchaseorderterm() {
var missingpurchaseorderterm = document.getElementById('missingpurchaseorderterm');
if (missingpurchaseorderterm.value == '') {
alert('Please Enter New Purchase OrderTerm Name');
missingpurchaseorderterm.focus();
return false;
} else {
...
url: "termadds.php",
data: {
term: $("#missingpurchaseorderterm").val(),
submit: "Submit"
},
success:function(result){
const resarray = result.split("|");
...
{
$('#purchaseorderterm').append('<option value="' + missingpurchaseorderterm.value + '">' + missingpurchaseorderterm.value +
'</option>');
$('#purchaseorderterm').val(missingpurchaseorderterm.value).change();
$("#purchaseorderterm").select2();
$('#AddNewPurchaseOrderTerm').modal('hide');
return false;
}
}});
...
function funespurchaseorderterm() {
//$('#purchaseorderterm').val('').change();
$("#purchaseorderterm").select2();
$('#AddNewPurchaseOrderTerm').modal('hide');
return false;
}
...
$("#purchaseorderterm").on("select2:open", function() {
$("#configureunits").attr("data-bs-target","#AddNewPurchaseOrderTerm");
});
$("#purchaseorderterm").on("select2:open", function() {
document.getElementById("configureunits").innerHTML = "New Purchase OrderTerm";
});
</script>
...
<!---payment confirm---->
<script>
function triggerpayment(purchaseorderno,purchaseorderdate,purchaseorderamount,cancelstatus)
{
let allAreFilled = true;
document.getElementById("purchaseorderform").querySelectorAll("[required]").forEach(function(i) {
if (!allAreFilled) return;
if (i.type === "radio") {
let radioValueCheck = false;
document.getElementById("purchaseorderform").querySelectorAll(`[name=${i.name}]`).forEach(function(r) {
...
{
$('#validpurchaseorderno').val($('#purchaseorderno').val());
$('#validpurchaseorderdate').val($('#purchaseorderdate').val());
$('#validpurchaseorderamount').val($('#grandtotal').val());
