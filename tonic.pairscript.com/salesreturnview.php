...
if((isset($_GET['salesreturnno']))&&(isset($_GET['salesreturndate']))){
	$salesreturnno=htmlspecialchars($_GET['salesreturnno'], ENT_QUOTES, 'UTF-8');
	$salesreturndate=htmlspecialchars($_GET['salesreturndate'], ENT_QUOTES, 'UTF-8');
	$sql=$con->prepare("SELECT * FROM pairsalesreturns WHERE franchisesession=? AND createdid=? AND salesreturnno=? AND salesreturndate=? ORDER BY id ASC");
	$sql->bind_param("ssss", $_SESSION['franchisesession'], $companymainid, $salesreturnno, $salesreturndate);
	$sql->execute();
   $sql->store_result();
	if($sql->num_rows>0){
		$sql->close();
		$sqlismainaccessfields=$con->prepare("SELECT * FROM pairmainaccess WHERE userid=? AND moduletype='Sales Returns' ORDER BY id ASC");
...
<?php
	...
	$datefetch=$dateformat->fetch_array();
	if ($datefetch['date']=='DD/MM/YYYY') {
		if (($access['salereturnprintexpdate']=='0')&&($access['salereturnprintexpmonth']=='0')&&($access['salereturnprintexpyear']=='0')) {
			$dateforexp = '';
		}
		if (($access['salereturnprintexpdate']=='1')&&($access['salereturnprintexpmonth']=='1')&&($access['salereturnprintexpyear']=='1')) {
			$dateforexp = 'd/m/Y';
		}
		if (($access['salereturnprintexpdate']=='0')&&($access['salereturnprintexpmonth']=='1')&&($access['salereturnprintexpyear']=='1')) {
			$dateforexp = 'm/Y';
		}
		if (($access['salereturnprintexpdate']=='1')&&($access['salereturnprintexpmonth']=='0')&&($access['salereturnprintexpyear']=='1')) {
			$dateforexp = 'd/Y';
		}
		if (($access['salereturnprintexpdate']=='1')&&($access['salereturnprintexpmonth']=='1')&&($access['salereturnprintexpyear']=='0')) {
			$dateforexp = 'd/m';
		}
		if (($access['salereturnprintexpdate']=='1')&&($access['salereturnprintexpmonth']=='0')&&($access['salereturnprintexpyear']=='0')) {
			$dateforexp = 'd';
		}
		if (($access['salereturnprintexpdate']=='0')&&($access['salereturnprintexpmonth']=='1')&&($access['salereturnprintexpyear']=='0')) {
			$dateforexp = 'm';
		}
		if (($access['salereturnprintexpdate']=='0')&&($access['salereturnprintexpmonth']=='0')&&($access['salereturnprintexpyear']=='1')) {
			$dateforexp = 'Y';
		}
	}
	//FOR DATE FORMAT
	...
	<?php
      ...
      //FOR MODULE NAME
		if((isset($_GET['salesreturnno']))&&(isset($_GET['salesreturndate']))){
			$salesreturnno=htmlspecialchars($_GET['salesreturnno'], ENT_QUOTES, 'UTF-8');
			$salesreturndate=htmlspecialchars($_GET['salesreturndate'], ENT_QUOTES, 'UTF-8');
			$sqls=$con->prepare("SELECT * FROM pairsalesreturns WHERE franchisesession=? AND createdid=? AND salesreturnno=? AND salesreturndate=? ORDER BY id ASC");
			$sqls->bind_param("ssss", $_SESSION['franchisesession'], $companymainid, $salesreturnno, $salesreturndate);
			$sqls->execute();
      	$sql = $sqls->get_result();
			$count=1;
			if($sql->num_rows>0){
					...
					<div class="card mb-4 mt-5">
						<div class="card-body p-3" style="font-family:'Myriad Set Pro','Helvetica Neue',Helvetica,Arial,sans-serif;">
							<div class="row">
								...
								<div class="col-lg-6">
									<span style="float:right">
										...
										<a style="margin:4.5px 4.5px !important;" class="btn btn-primary btn-sm btn-custom-grey" href="salesreturnpaymentadd.php?no=<?=$salesreturnno?>&date=<?=$salesreturndate?>&type=salesreturn" id="recordpaymentlink">
										...
										<div class="modal fade" id="paymentconfirmmodal" tabindex="-1" role="dialog" aria-labelledby="paymentconfirmmodalLabel" aria-hidden="true">
											<div class="modal-dialog">
												<div class="modal-content">
													<form action="salesreturnviewedit.php" method="post" enctype="multipart/form-data" class="form-horizontal" role="form">
														...
														<div class="modal-body" style="height: max-content;">
															...
															<table class="table notforfinaloutputsave showhideload">
																<tr>
																	...
																	<td>
																		...
																		<input type="text" name="validsalesreturnno" id="validsalesreturnno" class="form-control form-control-sm finalsubmitrequired" readonly required value="<?=$rows[0]['salesreturnno']?>" style="background-color: #e9ecef !important;box-shadow: none !important;border: 1px solid #e7e9ed !important;">
																	</td>
																</tr>
																<tr>
																	<th style="text-align:left !important">
																	...
																	<td>
																		<input type="date" name="validsalesreturndate" id="validsalesreturndate" class="form-control form-control-sm finalsubmitrequired" readonly required value="<?=$rows[0]['salesreturndate']?>" style="background-color: #e9ecef !important;box-shadow: none !important;border: 1px solid #e7e9ed !important;">
																	</td>
																</tr>
																<tr>
																	<th style="text-align:left !important">
																	...
																	<td>
																		<input type="number" name="validsalesreturnamount" id="validsalesreturnamount" class="form-control form-control-sm finalsubmitrequired" readonly required value="<?=$rows[0]['grandtotal']?>" style="background-color: #e9ecef !important;box-shadow: none !important;border: 1px solid #e7e9ed !important;">
																	</td>
																</tr>
																<tr style="<?=(($access['defsalereturnpay']=='split')?'':'display: none;')?>">
																...
																<tr style="<?=(($access['defsalereturnpay']=='split')?'display: none;':'')?>">
																	...
																	<td>
																		<div class="col-lg-12">
																			<select class="select2-field form-control  form-control-sm finalsubmitrequired" name="salesreturnterm" id="salesreturnterm" required onchange="ihaveblock()">
																			<?php
																			...
																			?>
																				<option value="<?= $infoterm['term'] ?>" <?=($infoterm['term']==$rows[0]['salesreturnterm'])?'selected':''?>>
																					<?= $infoterm['term'] ?>
																				</option>
																			<?php
																					}
																...
																<tr id="validpaidblock">
																	...
																	<td>
																		...
																		function paidamountcalc(){
																			...
																			if((validsalesreturnamount!='')&&(validpaidamount!='')){
																				var balance=parseFloat(validsalesreturnamount)-parseFloat(validpaidamount);
																...
																<tr id="duedateblock">
																	...
																	<td>
																		...
																		function splitchecking() {
																			...
																			if (($("#validsalesreturnamount").val()) == (parseFloat($("#cashreceived").val()) + parseFloat($("#cardreceived").val()) + parseFloat($("#gpayreceived").val()))) {
																		...
																		function funduedates(){
																			...
																			var salesreturndate=$("#validsalesreturndate").val();
																			var today="";
																			if(duedates!=''){
																				// console.log(salesreturndate);
																				// console.log(duedates);
																				today=addDays(salesreturndate, duedates);
															...
															var ihaveterm = document.getElementById("salesreturnterm").value;
									...
									?>
										<a class="btn btn-primary btn-sm btn-custom-grey" href="salesreturnedit.php?id=<?=$_GET['id']?>&salesreturnno=<?=$salesreturnno?>&salesreturndate=<?=$salesreturndate?>" style="margin:4.5px 4.5px !important;">
									...
									<?php
										$paidamount=0;
										$currentbalance=0;
										$currentamount=(float)$rows[0]['salesreturnamount'];
										$sqlsalespays=$con->prepare("SELECT amount FROM pairsalesreturnpayhistory WHERE franchisesession=? AND createdid=? AND salesreturnno=? AND salesreturndate=? ORDER BY id DESC");
      								$sqlsalespays->bind_param("ssss", $_SESSION['franchisesession'], $companymainid, $salesreturnno, $salesreturndate);
      								...
      								$sqlsalespay = $sqlsalespays->get_result();
										...
										$currentbalance=((float)$rows[0]['salesreturnamount']-$paidamount);
										if(($rows[0]['cancelstatus']!="1")&&($currentbalance==$currentamount)){
									?>
										<ul class="dropdown-menu  dropdown-menu-end customdropdown me-sm-n4 mt-1" aria-labelledby="dropdownMenuButton">   
											<div style="background-color: #3c3c46;margin-top: 0px !important;cursor: pointer;"> 
												...
												<li class="nav-item" style="margin-top:-22px !important;">
													<a class="nav-link" href="salesreturncancel.php?salesreturnno=<?=$salesreturnno?>&salesreturndate=<?=$salesreturndate?>&cancelstatus=<?=($rows[0]['cancelstatus']=='0')?'1':'0'?>" onclick="return confirm('Are you sure want to <?=($rows[0]['cancelstatus']=='0')?'Cancel':'Enable'?> this Sales Return?')">
							...
							<div class="tab-content" id="nav-tabContent">
								<div class="tab-pane fade" id="nav-journal" role="tabpanel" aria-labelledby="nav-journal-tab">
									<div class="table-responsive m-3" id="journaltables">
										<table class="table table-bordered">
											<tbody>
											<?php
												$sqlijournals=$con->prepare("SELECT customerid, chartaccountname, customername, sum(ledgerdebit) AS ledgerdebit, sum(ledgercredit) AS ledgercredit, notes,ledgerdate, ledgerno, referenceno, referencedate, totalledgerdebit, totalledgercredit, id, type FROM pairledgers WHERE franchisesession=? AND createdid=? AND ledgerno=? AND ledgerdate=? AND (type='salesreturn' OR type='salesreturn payment') GROUP BY type,chartaccountid,ledgerdate,ledgerno ORDER BY ledgerdate,ledgerno,type ASC");
      										$sqlijournals->bind_param("ssss", $_SESSION['franchisesession'], $companymainid, $salesreturnno, $salesreturndate);
								...
								<div class="tab-pane fade" id="nav-profile" role="tabpanel" aria-labelledby="nav-profile-tab">
									<div class="table-responsive m-3" id="histables">
										<table class="table table-bordered">
											...
											<tbody>
											<?php
												$sqluse=$con->prepare("SELECT * FROM pairusehistory WHERE (usetype = 'SALESRETURNS' AND (useid = ? AND (uniqueid = ? OR uniqueid = ?))) ORDER BY createdon DESC");
      										$sqluse->bind_param("sss", $salesreturnno, $_GET['id'], $salesreturnno);
      										$sqluse->execute();
											...
											?>
												<tr>
													...
													<td data-label="DETAILS">
														...
														<br>
														<span>
															<?=((str_contains($infouse['useremarks'],'SALESRETURN CREATED'))?'Created By':'Changed By')?>
														</span>
														<span  id="chhis">
															<?=$infouse['createdby']?>
														</span>
								...
								<?php
									$sqlismainaccessusers=$con->prepare("SELECT moduleno, moduleprefix, modulesuffix,salereturnprintgsttreatment,salereturnprintgstin,modulename,salereturnprintpos,viewprintdlno20,viewprintdlno21 FROM pairmainaccess WHERE franchiseid = ? AND moduletype = 'Sales Returns' ORDER BY id ASC");
											...
											<div class="row d-flex justify-content-center">
												<div class="col-lg-8 col-md-12 justify-content-center">
																					...
																					<td style="text-align:center;height: 80px !important;">
																						<table width="100%">
																							<tr>
																								...
																								<td width="51.5%" style="text-align:left;vertical-align: top;padding-left: 10px !important;border-right: 0px solid #cccccc;">
																									<table width="100%">
																										...
																										<tr>
																											<td>
																												<table width="100%">
																													<tr style="<?=($access['salereturnbranchphone']=='0')?'display:none;':''?>">
																													...
																													<tr>
																														<td width="10%" style="font-size:12px;padding: 0px !important;line-height: 15px !important;<?=($access['salereturnbranchemail']=='0')?'visibility:hidden;':''?>">
																															E-mail
																														</td>
																														<td width="90%" style="font-size:12px;padding: 0px !important;line-height: 15px !important;">
																															<input type="text" readonly value=": <?=$info['email']?>" style="padding:0px;color:black;width: 206px !important;height: 18px !important;border: none !important;outline: none !important;display: inline-flex;font-size: 12px;<?=($access['salereturnbranchemail']=='0')?'visibility:hidden;':''?>" class="form-control form-control-sm">
																															<span style="<?=(($access['salereturnprintdlno20']=='1')?'display:inline-flex;':'display:none;')?>white-space: nowrap;width: 169px;overflow: hidden;float: right;margin-right: 9px;">
																																DL No 20 &nbsp; : <?=$info['dlno20']?>
																															</span>
																														</td>
																													</tr>
																													<tr>
																														<td width="10%" style="font-size:12px;padding: 0px !important;line-height: 15px !important;<?=($access['salereturnbranchgstin']=='0')?'visibility:hidden;':''?>">
																															GSTIN
																														</td>
																														<td width="90%" style="font-size:12px;padding: 0px !important;line-height: 15px !important;">
																															<span style="display: inline-flex;white-space: nowrap;width: 134px;overflow: hidden;<?=($access['salereturnbranchgstin']=='0')?'visibility:hidden;':''?>">
																																: <?=$info['gstno']?>
																															</span>
																															<span style="<?=(($access['salereturnprintdlno21']=='1')?'display:inline-flex;':'display:none;')?>white-space: nowrap;width: 169px;overflow: hidden;float: right;margin-right: 9px;">
																								...
																								<td width="34.5%">
																									<table width="100%">
																										<tr <?=(($access['salereturnbank']=='1')?'':'style="display:none;"')?>>
																										...
																										<tr <?=(($access['salereturnname']=='1')?'':'style="display:none;"')?>>
																											<td style="padding: 0px 6px !important;font-size: 11px !important;line-height: 15px !important;text-align: left;" width="35%">
																										...
																										<tr <?=(($access['salereturnaccnumber']=='1')?'':'style="display:none;"')?>>
																											<td style="padding: 0px 6px !important;font-size: 11px !important;line-height: 15px !important;text-align: left;" width="35%">
																										...
																										<tr <?=(($access['salereturnifsccode']=='1')?'':'style="display:none;"')?>>
																											<td style="padding: 0px 6px !important;font-size: 11px !important;line-height: 15px !important;text-align: left;" width="35%">
																										...
																										<tr <?=(($access['salereturnbranchandcity']=='1')?'':'style="display:none;"')?>>
																											<td style="padding: 0px 6px !important;font-size: 11px !important;line-height: 15px !important;text-align: left;" width="35%">
																			...
																			<table width="100%">
																				<tr>
																								...
																								<td style="padding: 0px !important;">
																									<table width="100%">
																										...
																										<tr <?=((in_array('GSTIN', $fieldview))?'':'style="display:none;"')?>>
																											<td width="38%" style="<?=((($infomainaccessuser['salereturnprintgstin']=='show')||($rows[0]['gstno']!='')&&($infomainaccessuser['salereturnprintgstin']!='hide'))?'':'display:none;')?>font-size:12px;padding:0px 6px !important;line-height: 15px !important;">
																												GSTIN
																											</td>
																											<td width="62%" style="<?=((($infomainaccessuser['salereturnprintgstin']=='show')||($rows[0]['gstno']!='')&&($infomainaccessuser['salereturnprintgstin']!='hide'))?'':'display:none;')?>font-size:12px;padding: 0px !important;line-height: 15px !important;">
																												: 
																											<?php
																												if (($infomainaccessuser['salereturnprintgstin']=='show')||($rows[0]['gstno']!='')&&($infomainaccessuser['salereturnprintgstin']!='hide')) {
																											?>
																								...
																								<td style="padding: 0px !important;">
																									<table width="100%">
																										...
																										<tr <?=(in_array('Place of Supply', $fieldview))?'':'style="display:none;"'?>>
																											<td width="38%" style="<?=(($infomainaccessuser['salereturnprintpos']=='show')||($rows[0]['pos']!='')&&($infomainaccessuser['salereturnprintpos']!='hide'))?'':'display:none;'?>font-size:12px;padding:0px 6px !important;line-height: 15px !important;white-space: nowrap !important;">
																												Place Of Supply
																											</td>
																											<td width="62%" style="<?=(($infomainaccessuser['salereturnprintpos']=='show')||($rows[0]['pos']!='')&&($infomainaccessuser['salereturnprintpos']!='hide'))?'':'display:none;'?>font-size:12px;padding: 0px !important;line-height: 15px !important;">: 
																											<?php
																												if (($infomainaccessuser['salereturnprintpos']=='show')||($rows[0]['pos']!='')&&($infomainaccessuser['salereturnprintpos']!='hide')) {
																											?>
																					...
																					<td width="33%" style="white-space: normal !important;vertical-align: middle;padding: 0px !important;">
																						...
																						<?php
																							// if ((in_array('Sales Return Information', $fieldview))) {
																						?>
																							<tr>
																							...
																							<?php
	 																						...
	 																						?>
																								<td style="padding: 0px !important;font-size: 14px !important;line-height: 15px !important;text-align: left !important;" width="45%">
																									: 	<b>
																											<?=date($date,strtotime($rows[0]['salesreturndate']))?>
																										</b>
																								</td>
																							</tr>
																							<tr>
																								...
																								<td style="padding: 0px !important;">
																									<table id="gsttable" width="100%" style="line-height: 13px !important;padding: 0px !important;border: 1px solid #cccccc;border-collapse: collapse;">
																										...
																										<?php
																											...
																											$sqlitaxess=$con->prepare("SELECT * FROM pairsalesreturns WHERE franchisesession = ? AND createdid = ? AND salesreturnno=? and salesreturndate=? ORDER BY id ASC");
      																									$sqlitaxess->bind_param("ssss", $_SESSION['franchisesession'], $companymainid, $salesreturnno, $salesreturndate);
      																									$sqlitaxess->execute();
													...
													<?php
														...
														elseif($info['salesreturntemplate']=='4'){
															//FOR 3 Inch PAPER
													?>
														<div id="printalble">
															<div class="table-responsive" style="width: max-content !important;height: max-content !important;max-width:max-content !important; max-height:max-content !important;min-width:max-content !important; min-height:max-content !important;">
																<table id="printarea" style="border:1px solid #cccccc;margin-bottom: -13px !important;width: 7.62cm !important;max-width:7.62cm !important;min-width:7.62cm !important;">
																	...
																	<tr style="height:10px !important;<?=($access['salereturnbranchphone']=='0')?'display:none;':''?>">
																		<td style="border-bottom: 1px solid #cccccc;">
																			<table width="100%">
																				<tr>
																					<td style="text-align:center;line-height: 9px !important;">
																				...
																				<tr>
																					...
																					<td style="text-align:right;line-height: 13px !important;" width="50%">
																						<b style="font-size:10px !important;display: inline-flex;white-space: nowrap;width: max-content;overflow: hidden;text-align: right;">
																							Date : <?=date($date,strtotime($rows[0]['salesreturndate']))?>
																						</b>
																					</td>
																				</tr>
																			</table>
													...
													<?php
														...
														elseif($info['salesreturntemplate']=='0'){
																											...
																											<td>
																												<table width="100%">
																													...
																													<tr>
																														...
																														<td width="90%" style="font-size:12px;padding: 0px !important;line-height: 15px !important;">
																															<input type="text" readonly value=": <?=$info['email']?>" style="padding:0px;color:black;width: 150px !important;height: 18px !important;border: none !important;outline: none !important;display: inline-flex;font-size: 12px;<?=($access['salereturnbranchemail']=='0')?'visibility:hidden;':''?>" class="form-control form-control-sm">
																															<span style="<?=(($access['salereturnprintdlno20']=='1')?'display:inline-flex;':'display:none;')?>white-space: nowrap;width: 174px;overflow: hidden;float: right;margin-right: 9px;">
																																DL No 20 &nbsp; : <?=$info['dlno20']?>
																															</span>
																														</td>
																													</tr>
																													<tr>
																														...
																														<td width="90%" style="font-size:12px;padding: 0px !important;line-height: 15px !important;">
																															<span style="display: inline-flex;white-space: nowrap;width: 113px;overflow: hidden;<?=($access['salereturnbranchgstin']=='0')?'visibility:hidden;':''?>">
																																: <?=$info['gstno']?>
																															</span>
																															<span style="<?=(($access['salereturnprintdlno21']=='1')?'display:inline-flex;':'display:none;')?>white-space: nowrap;width: 174px;overflow: hidden;float: right;margin-right: 9px;">
																																DL No 21 &nbsp; : <?=$info['dlno21']?>
																															</span>
																														</td>
																													</tr>
																								...
																								<td width="34.5%">
																									<table width="100%">
																										...
																										<tr <?=(($access['salereturnname']=='1')?'':'style="display:none;"')?>>
																											...
																											<td style="padding: 0px !important;font-size: 11px !important;line-height: 15px !important;text-align: left !important;white-space: nowrap !important;overflow: hidden !important;" width="65%">
																												: 	<b style="display:inline-flex;max-width: 163px !important;<?=(($access['salereturnbank']=='1')?'overflow: hidden;':'white-space: pre-line;')?>"><?=$info['names']?></b>
																											</td>
																										</tr>
																										<tr <?=(($access['salereturnaccnumber']=='1')?'':'style="display:none;"')?>>
																											<td style="padding: 0px 6px !important;font-size: 11px !important;line-height: 15px !important;text-align: left;" width="35%">
																							...
																							<table width="100%" style="border:1px solid #cccccc 0px 0px 0px !important;line-height: 13px !important;height: 723px;">
																								<thead style="background-color: #eee;">
																									...
																									<?php
																										if ((in_array('Taxable Value', $fieldview))) {
																									?>
																										<th width="5%" style="white-space: nowrap;overflow: hidden;border: 1px solid #cccccc;font-size: 12px !important;padding:0px 3px !important;text-align: right !important;max-width: 104px;min-width: 104px;"><?=($access['txttaxablesalereturn'])?></th>
																									<?php
																										}
																									?>
																									<?php
													...
													<?php
														...
														elseif($info['salesreturntemplate']=='2'){
																											...
																											<td>
																												<table width="100%">
																													...
																													<tr>
																														...
																														<td width="90%" style="font-size:12px;padding: 0px !important;line-height: 15px !important;">
																															<input type="text" readonly value=": <?=$info['email']?>" style="padding:0px;color:black;width: 186px !important;height: 18px !important;border: none !important;outline: none !important;display: inline-flex;font-size: 12px;<?=($access['salereturnbranchemail']=='0')?'visibility:hidden;':''?>" class="form-control form-control-sm">
																															<span style="<?=(($access['salereturnprintdlno20']=='1')?'display:inline-flex;':'display:none;')?>white-space: nowrap;width: 139px;overflow: hidden;float: right;margin-right: 9px;">
																																DL No 20 &nbsp; : <?=$info['dlno20']?>
																															</span>
																														</td>
																													</tr>
																													<tr>
																														...
																														<td width="90%" style="font-size:12px;padding: 0px !important;line-height: 15px !important;">
																															...
																															<span style="<?=(($access['salereturnprintdlno21']=='1')?'display:inline-flex;':'display:none;')?>white-space: nowrap;width: 139px;overflow: hidden;float: right;margin-right: 9px;">
																																DL No 21 &nbsp; : <?=$info['dlno21']?>
																															</span>
																														</td>
																													</tr>
																							...
																							<table width="100%" style="border:1px solid #cccccc 0px 0px 0px !important;line-height: 13px !important;height: 176px;">
																								...
																								?>
																									...
																									<?php
																										...
																										$salereturn = '<tr><td style="padding: 0px !important;font-size: 14px !important;line-height: 15px !important;" width="55%"><input type="text" readonly value="'. $infomainaccessuser['modulename'] .'" style="padding:0px;color:black;width: 56px !important;height: 18px !important;border: none !important;outline: none !important;display: inline-flex;font-size: 14px;" class="form-control form-control-sm"> <input type="text" readonly value="Number" style="padding:0px;color:black;width: 54px !important;height: 18px !important;border: none !important;outline: none !important;display: inline-flex;font-size: 14px;" class="form-control form-control-sm"> </td><td style="padding: 0px !important;font-size: 14px !important;line-height: 15px !important;text-align: left !important;" width="45%">: <b>'.$rows[0]['salesreturnno'].'</b></td></tr><tr><td style="padding: 0px !important;font-size: 14px !important;line-height: 15px !important;" width="55%"><input type="text" readonly value="'. $infomainaccessuser['modulename'] .'" style="padding:0px;color:black;width: 56px !important;height: 18px !important;border: none !important;outline: none !important;display: inline-flex;font-size: 14px;" class="form-control form-control-sm"> <input type="text" readonly value="Date" style="padding:0px;color:black;width: 54px !important;height: 18px !important;border: none !important;outline: none !important;display: inline-flex;font-size: 14px;" class="form-control form-control-sm"> </td> </td><td style="padding: 0px !important;font-size: 14px !important;line-height: 15px !important;text-align: left !important;" width="45%">: <b>'.date($date,strtotime($rows[0]['salesreturndate'])).'</b></td></tr><tr><td style="padding: 0px !important;font-size: 14px !important;line-height: 15px !important;" width="55%"><input type="text" readonly value="Payment" style="padding:0px;color:black;width: 56px !important;height: 18px !important;border: none !important;outline: none !important;display: inline-flex;font-size: 14px;" class="form-control form-control-sm"> <input type="text" readonly value="Term" style="padding:0px;color:black;width: 54px !important;height: 18px !important;border: none !important;outline: none !important;display: inline-flex;font-size: 14px;" class="form-control form-control-sm"> </td><td style="padding: 0px !important;font-size: 14px !important;line-height: 15px !important;text-align: left !important;" width="45%">: <b>'.$rows[0]['salesreturnterm'].'</b></td></tr>';
																										// }
																										$merge = '</tbody></table></div></td></tr></table></td></tr><tr><td><div style="min-height:92px !important;max-height:92px !important;"><div></td></tr><tr><td><div style="min-height:25px !important;max-height:25px !important;padding: 0px !important;"><div></td></tr><tr style="height:40px !important;"><td style="padding:0px !important;border-bottom: none;"><table width="100%"><tr><td width="30%" style="padding: 0px !important;border-right: 1px solid #cccccc;"><table width="100%"><tr><td style="'.(((in_array('Printed On', $fieldview)))?'':'visibility:hidden;').'vertical-align:middle !important;text-align: center !important;padding-top: 7px !important;"><div style="text-align:center;margin-top: -10px !important;font-size: 12px !important;"><span>Printed On :'.date($dateforfooter,strtotime($dates)).'</span></div><div style="text-align:center;line-height: 7px !important;font-size: 12px !important;"><b>(Page '.$begin.'/'.$finish.')</b></div></td></tr></table></td></tr></table></td></tr></table><span><span style="background-color:#fff !important;font-size: 8px !important;color: #ccc !important;">PAIRSCRIPT</span></span></div><div class="table-responsive" style="margin-top:0.75rem;width: max-content !important;height: max-content !important;max-width:max-content !important; max-height:max-content !important;min-width:max-content !important; min-height:max-content !important;"><table id="printarea" style="border:1px solid #cccccc;margin-bottom: -13px !important;width: 21cm !important;height: 14.8cm !important;max-width:21cm !important; max-height:14.8cm !important;min-width:21cm !important; min-height:14.8cm !important;"><tr style="height:10px !important;"><td style="border-bottom: 1px solid #cccccc;"><table width="100%"><tr><td style="text-align:center;line-height: 13px !important;"><b style="font-size:15px !important;display: inline-flex;white-space: nowrap;width: max-content;overflow: hidden;text-align: center;max-width: 166px;">'.$infomainaccessuser['modulename'].'</b></td></tr></table></td></tr><tr style="height:10px !important;"><td><table width="100%"><tr><td style="text-align:center;height: 80px !important;"><table width="100%"><tr><td width="16%" style="text-align:right;">'.$imgans.'</td><td width="51.5%" style="text-align:left;vertical-align: top;padding-left: 30px !important;"><table width="100%"><tr style="padding:0px !important;"><td><table width="100%" style="padding:0px !important;"><tr style="padding:0px !important;"><td style="padding:0px !important;line-height: 15px !important;"><strong style="font-size:14px;vertical-align: top;display: inline-flex;white-space: nowrap;width: 361px;overflow: hidden;">'.$info['franchisename'].'</strong></td></tr><tr style="padding:0px !important;"><td style="padding:0px !important;line-height: 15px !important;margin-bottom: -3px;"><span style="font-size:12px;margin: 0px !important;display: inline-flex;width: 361px;overflow: hidden;white-space: nowrap;">'.$info['street'].' '.$info['city'].' '.$info['pincode'].' '.$info['state'].' '.$info['country'].'</span></td></tr></table></td></tr><tr><td><table width="100%"><tr style="'.(($access['salereturnbranchphone']=='0')?'display:none;':'').'"><td width="10%" style="font-size:12px;padding: 0px !important;line-height: 13px !important;">Phone </td><td width="90%" style="font-size:12px;padding: 0px !important;line-height: 13px !important;">: <span style="display: inline-flex;white-space: nowrap;width: 253px;overflow: hidden;">'.$info['mobile'].'</span></td></tr><tr><td width="10%" style="font-size:12px;padding: 0px !important;line-height: 15px !important;'.(($access['salereturnbranchemail']=='0')?'visibility:hidden;':'').'">E-mail </td><td width="90%" style="font-size:12px;padding: 0px !important;line-height: 15px !important;"><input type="text" readonly value=": '.$info['email'].'" style="padding:0px;color:black;width: 186px !important;height: 18px !important;border: none !important;outline: none !important;display: inline-flex;font-size: 12px;'.(($access['salereturnbranchemail']=='0')?'visibility:hidden;':'').'" class="form-control form-control-sm"><span style="'.(($access['salereturnprintdlno20']=='1')?'display:inline-flex;':'display:none;').'white-space: nowrap;width: 139px;overflow: hidden;float: right;margin-right: 9px;">DL No 20 &nbsp; : '.$info['dlno20'].'</span></td></tr><tr><td width="10%" style="font-size:12px;padding: 0px !important;line-height: 15px !important;'.(($access['salereturnbranchgstin']=='0')?'visibility:hidden;':'').'">GSTIN </td><td width="90%" style="font-size:12px;padding: 0px !important;line-height: 15px !important;"><span style="display: inline-flex;white-space: nowrap;width: 134px;overflow: hidden;'.(($access['salereturnbranchgstin']=='0')?'visibility:hidden;':'').'">: '.$info['gstno'].'</span><span style="'.(($access['salereturnprintdlno21']=='1')?'display:inline-flex;':'display:none;').'white-space: nowrap;width: 139px;overflow: hidden;float: right;margin-right: 9px;">DL No 21 &nbsp; : '.$info['dlno21'].'</span></td></tr></table></td></tr></table></td><td width="34.5%"><table width="100%"><tr '.(($access['billbank']=='1')?'':'style="display:none;"').'><td style="padding: 0px 6px !important;font-size: 11px !important;line-height: 15px !important;text-align: left;" width="35%"><input type="text" readonly value="Bank" style="padding:0px;color:black;width: 113px !important;height: 15px !important;border: none !important;outline: none !important;display: inline-flex;font-size: 11px;" class="form-control form-control-sm"></td><td style="padding: 0px !important;font-size: 11px !important;line-height: 15px !important;text-align: left !important;white-space: nowrap !important;overflow: hidden !important;" width="65%">: <b style="display:inline-flex;max-width: 163px !important;overflow: hidden;">'.$info['bank'].'</b></td></tr><tr '.(($access['billname']=='1')?'':'style="display:none;"').'><td style="padding: 0px 6px !important;font-size: 11px !important;line-height: 15px !important;text-align: left;" width="35%"><input type="text" readonly value="Name" style="padding:0px;color:black;width: 113px !important;height: 15px !important;border: none !important;outline: none !important;display: inline-flex;font-size: 11px;" class="form-control form-control-sm"></td><td style="padding: 0px !important;font-size: 11px !important;line-height: 15px !important;text-align: left !important;white-space: nowrap !important;overflow: hidden !important;" width="65%">: <b style="display:inline-flex;max-width: 163px !important;overflow: hidden;">'.$info['names'].'</b></td></tr><tr '.(($access['billaccnumber']=='1')?'':'style="display:none;"').'><td style="padding: 0px 6px !important;font-size: 11px !important;line-height: 15px !important;text-align: left;" width="35%"><input type="text" readonly value="Account Number" style="padding:0px;color:black;width: 113px !important;height: 15px !important;border: none !important;outline: none !important;display: inline-flex;font-size: 11px;" class="form-control form-control-sm"></td><td style="padding: 0px !important;font-size: 11px !important;line-height: 15px !important;text-align: left !important;white-space: nowrap !important;overflow: hidden !important;" width="65%">: <b style="display:inline-flex;max-width: 163px !important;overflow: hidden;">'.$info['accountnumber'].'</b></td></tr><tr '.(($access['billifsccode']=='1')?'':'style="display:none;"').'><td style="padding: 0px 6px !important;font-size: 11px !important;line-height: 15px !important;text-align: left;" width="35%"><input type="text" readonly value="IFSC Code" style="padding:0px;color:black;width: 113px !important;height: 15px !important;border: none !important;outline: none !important;display: inline-flex;font-size: 11px;" class="form-control form-control-sm"></td><td style="padding: 0px !important;font-size: 11px !important;line-height: 15px !important;text-align: left !important;white-space: nowrap !important;overflow: hidden !important;" width="65%">: <b style="display:inline-flex;max-width: 163px !important;overflow: hidden;">'.$info['ifsccode'].'</b></td></tr><tr '.(($access['billbranchandcity']=='1')?'':'style="display:none;"').'><td style="padding: 0px 6px !important;font-size: 11px !important;line-height: 15px !important;text-align: left;" width="35%"><input type="text" readonly value="Branch & City" style="padding:0px;color:black;width: 100px !important;height: 16px !important;border: none !important;outline: none !important;display: inline-flex;font-size: 11px;" class="form-control form-control-sm"></td><td style="padding: 0px !important;font-size: 11px !important;line-height: 15px !important;text-align: left !important;white-space: nowrap !important;overflow: hidden !important;" width="65%">: <b style="display:inline-flex;max-width: 163px !important;overflow: hidden;">'.$info['branchandcity'].'</b></td></tr></table></td></tr></table></td></tr></table></td></tr><tr style="height:1px !important;border: 1px solid #cccccc;"><td style="padding: 0px !important;"><table width="100%"><tr><td width="33%" style="white-space: normal !important;vertical-align: top;border-right: 1px solid #cccccc;padding: 0px !important;height: 98px !important;"><table width="100%"><tr style="background-color: #eee;"><td style="padding: 0px !important;"><strong style="padding:0px 6px !important;">Billing Address</strong></td></tr><tr>'.$custname.'</tr><tr><td style="padding: 0px 6px !important;line-height: 15px !important;white-space: nowrap !important;">'.$bill1.$bill2.'</td></tr><tr><td style="padding: 0px !important;"><table width="100%"><tr '.(((in_array('Work Phone', $fieldview))?'':'style="display:none;"')).'><td width="38%" style="font-size:12px;padding:0px 6px !important;line-height: 15px !important;">Work Phone </td><td width="62%" style="font-size:12px;padding: 0px !important;line-height: 15px !important;">: <b style="display: inline-flex;white-space: nowrap;width: 100px;overflow: hidden;">'.$rows[0]['workphone'].'</b></td></tr><tr '.(((in_array('GSTIN', $fieldview))?'':'style="display:none;"')).'><td width="38%" style="'.((($infomainaccessuser['salereturnprintgstin']=='show')||($rows[0]['gstno']!='')&&($infomainaccessuser['salereturnprintgstin']!='hide'))?'':'display:none;').'font-size:12px;padding:0px 6px !important;line-height: 15px !important;">GSTIN </td><td width="62%" style="'.((($infomainaccessuser['salereturnprintgstin']=='show')||($rows[0]['gstno']!='')&&($infomainaccessuser['salereturnprintgstin']!='hide'))?'':'display:none;').'font-size:12px;padding: 0px !important;line-height: 15px !important;">:'.$gstin.'</td></tr></table></td></tr></table></td><td width="33%" style="white-space: normal !important;vertical-align: top;border-right: 1px solid #cccccc;padding: 0px !important;"><table width="100%"><tr style="background-color: #eee;"><td style="padding: 0px !important;"><strong style="padding:0px 6px !important;">Shipping Address</strong></td></tr><tr>'.$custname.'</tr><tr><td style="padding: 0px 6px !important;line-height: 15px !important;white-space: nowrap !important;">'.$ship1.$ship2.'</td></tr><tr><td style="padding: 0px !important;"><table width="100%"><tr '.((in_array('Mobile Phone', $fieldview))?'':'style="display:none;"').'><td width="38%" style="font-size:12px;padding:0px 6px !important;line-height: 15px !important;">Mobile Phone </td><td width="62%" style="font-size:12px;padding: 0px !important;line-height: 15px !important;">: <b style="display: inline-flex;white-space: nowrap;width: 100px;overflow: hidden;">'.$rows[0]['mobile'].'</b></td></tr><tr '.((in_array('Place of Supply', $fieldview))?'':'style="display:none;"').'><td width="38%" style="'.((($infomainaccessuser['salereturnprintpos']=='show')||($rows[0]['pos']!='')&&($infomainaccessuser['salereturnprintpos']!='hide'))?'':'display:none;').'font-size:12px;padding:0px 6px !important;line-height: 15px !important;white-space: nowrap !important;">Place Of Supply </td><td width="62%" style="'.((($infomainaccessuser['salereturnprintpos']=='show')||($rows[0]['pos']!='')&&($infomainaccessuser['salereturnprintpos']!='hide'))?'':'display:none;').'font-size:12px;padding: 0px !important;line-height: 15px !important;">: '.$pos.'</td></tr></table></td></tr></table></td><td width="33%" style="white-space: normal !important;vertical-align: middle;padding: 0px !important;"><table width="100%" style="text-align: center !important;">'.$salereturn.'</table></td></tr></table></td></tr><tr style="height:1px !important;"><td style="padding: 0px !important;"><table width="100%"><tr style="vertical-align: top !important;"><td style="padding: 0px !important;height:176px !important;"><div style="max-height: 176px !important;min-height: 176px !important;overflow: hidden;"><table width="100%" style="border:1px solid #cccccc 0px 0px 0px !important;line-height: 13px !important;height: 176px;"><thead style="background-color: #eee;"><tr style="height:1px !important;"><th width="20%" style="white-space: nowrap;overflow: hidden;border: 1px solid #cccccc;font-size: 12px !important;padding:0px 3px !important;'.((in_array('Item Details', $fieldview))?'':'display:none !important;').'">ITEM DETAILS</th><th width="7%" style="white-space: nowrap;overflow: hidden;border: 1px solid #cccccc;font-size: 12px !important;padding:0px 3px !important;'.((in_array('Category', $fieldview))?'':'display:none !important;').'max-width: 40px;min-width: 40px;">'.($access['txtnamecategory']).'</th><th width="7%" style="white-space: nowrap;overflow: hidden;border: 1px solid #cccccc;font-size: 12px !important;padding:0px 3px !important;'.((in_array('Hsn or Sac', $fieldview))?'':'display:none !important;').'max-width: 56px;min-width: 56px;">HSN/SAC</th><th width="7%" style="white-space: nowrap;overflow: hidden;border: 1px solid #cccccc;font-size: 12px !important;padding:0px 3px !important;'.(($access['batchexpiryval']==1)?'':'display:none !important;').'min-width:59px !important;max-width:59px !important;">BATCH</th><th width="7%" style="white-space: nowrap;overflow: hidden;border: 1px solid #cccccc;font-size: 12px !important;padding:0px 3px !important;'.(($access['batchexpiryval']==1)?'':'display:none !important;').'">EXPIRY</th><th width="9%" style="white-space: nowrap;overflow: hidden;border: 1px solid #cccccc;font-size: 12px !important;padding:0px 3px !important;text-align: right !important; '.((in_array('Mrp', $fieldview))?'':'display:none !important;').'">MRP</th><th width="9%" style="white-space: nowrap;overflow: hidden;border: 1px solid #cccccc;font-size: 12px !important;padding:0px 3px !important;text-align: right !important; '.((in_array('Rate', $fieldview))?'':'display:none !important;').'">RATE</th><th width="7%" style="white-space: nowrap;overflow: hidden;border: 1px solid #cccccc;font-size: 12px !important;padding:0px 3px !important;text-align: left !important;min-width:25px;max-width:25px;'.((in_array('Discount', $fieldview))?'':'display:none !important;').'">
																											'.(($access['txtprodissalereturn'])).'
																										</th><th width="7%" style="white-space: nowrap;overflow: hidden;border: 1px solid #cccccc;font-size: 12px !important;padding:0px 3px !important;text-align: right !important;min-width:63px;max-width:63px;'.((in_array('Quantity', $fieldview))?'':'display:none !important;').'">'.($access['txtqtysalereturn']).'</th><th width="7%" style="white-space: nowrap;overflow: hidden;border: 1px solid #cccccc;font-size: 12px !important;padding:0px 3px !important;text-align: right !important;'.((in_array('GST Percentage', $fieldview))?'':'display:none !important;').'">GST %</th><th width="5%" style="white-space: nowrap;overflow: hidden;border: 1px solid #cccccc;font-size: 12px !important;padding:0px 3px !important;text-align: right !important;max-width: 104px;min-width: 104px;'.((in_array('Taxable Value', $fieldview))?'':'display:none !important;').'">'.(($access['txttaxablesalereturn'])).'</th></tr></thead><tbody>';
																										$finalrow=3;
																										if ((($item-1)%$finalrow)==0) {
													...
													<?php
														...
														elseif($info['salesreturntemplate']=='3'){
																					...
																					<td style="text-align:center;height: 80px !important;">
																						<table width="100%">
																							<tr>
																								...
																								<td width="<?=(($access['salereturnbank']=='0' && $access['salereturnname']=='0' && $access['salereturnaccnumber']=='0' && $access['salereturnifsccode']=='0' && $access['salereturnbranchandcity']=='0')?'86%':'56%')?>" style="text-align:left;vertical-align: top;padding-left: 30px !important;">
																									<table width="100%">
																										<tr style="padding:0px !important;">
																											<td>
																												<table width="100%" style="padding:0px !important;">
																													...
																													<tr style="padding:0px !important;">
																														<td style="padding:0px !important;line-height: 15px !important;margin-bottom: -3px;">
																															<span style="font-size:12px;margin: 0px !important;display: inline-flex;width: <?=(($access['salereturnbank']=='0' && $access['salereturnname']=='0' && $access['salereturnaccnumber']=='0' && $access['salereturnifsccode']=='0' && $access['salereturnbranchandcity']=='0')?'618px':'361px')?>;overflow: hidden;white-space: nowrap;">
																																<?=$info['street']?> <?=$info['city']?> <?=$info['pincode']?> <?=$info['state']?> <?=$info['country']?>
																															</span>
																														</td>
																													</tr>
																											...
																											<td>
																												<table width="100%">
																													...
																													<tr>
																														...
																														<td width="90%" style="font-size:12px;padding: 0px !important;line-height: 15px !important;">
																															<input type="text" readonly value=": <?=$info['email']?>" style="padding:0px;color:black;width: 186px !important;height: 18px !important;border: none !important;outline: none !important;display: inline-flex;font-size: 12px;<?=($access['salereturnbranchemail']=='0')?'visibility:hidden;':''?>" class="form-control form-control-sm">
																															<span style="<?=(($access['salereturnprintdlno20']=='1')?'display:inline-flex;':'display:none;')?>white-space: nowrap;width: 163px;overflow: hidden;float: right;margin-right: 9px;">
																																DL No 20 &nbsp; : <?=$info['dlno20']?>
																															</span>
																														</td>
																													</tr>
																													<tr>
																														...
																														<td width="90%" style="font-size:12px;padding: 0px !important;line-height: 15px !important;">
																															...
																															<span style="<?=(($access['salereturnprintdlno21']=='1')?'display:inline-flex;':'display:none;')?>white-space: nowrap;width: 163px;overflow: hidden;float: right;margin-right: 9px;">
																																DL No 21 &nbsp; : <?=$info['dlno21']?>
																															</span>
																														</td>
																													</tr>
																								...
																								<td width="30%" style="<?=(($access['salereturnbank']=='0' && $access['salereturnname']=='0' && $access['salereturnaccnumber']=='0' && $access['salereturnifsccode']=='0' && $access['salereturnbranchandcity']=='0')?'display: none;':'')?>">
																									<table width="100%">
																										<tr <?=(($access['salereturnbank']=='1')?'':'style="display:none;"')?>>
																											<td style="padding: 0px 6px !important;font-size: 11px !important;line-height: 15px !important;text-align: left;" width="35%">
																												<input type="text" readonly value="Bank" style="padding:0px;color:black;width: 100px !important;height: 15px !important;border: none !important;outline: none !important;display: inline-flex;font-size: 11px;" class="form-control form-control-sm">
																				...
																				<tr>
																							...
																							<tr>
																								<td style="padding: 0px !important;">
																									<table width="100%">
																										...
																										<tr>
																											<td width="38%" style="<?=(((($infomainaccessuser['salereturnprintgstin']=='show')||($rows[0]['gstno']!='')&&($infomainaccessuser['salereturnprintgstin']!='hide'))||(in_array('Mobile Phone', $fieldview)))?'':'display:none;')?>font-size:12px;padding:0px 6px !important;line-height: 15px !important;">
																												<?=((($infomainaccessuser['salereturnprintgstin']=='show')||($rows[0]['gstno']!='')&&($infomainaccessuser['salereturnprintgstin']!='hide'))?'GSTIN':'')?>
																											</td>
																											<td width="62%" style="<?=(((($infomainaccessuser['salereturnprintgstin']=='show')||($rows[0]['gstno']!='')&&($infomainaccessuser['salereturnprintgstin']!='hide'))||(in_array('Mobile Phone', $fieldview)))?'':'display:none;')?>font-size:12px;padding: 0px !important;line-height: 15px !important;">
																											<?php
																												if (($infomainaccessuser['salereturnprintgstin']=='show')||($rows[0]['gstno']!='')&&($infomainaccessuser['salereturnprintgstin']!='hide')) {
																											?>
																												: 
																					...
																					<td width="33%" style="white-space: normal !important;vertical-align: middle;padding: 0px !important;">
																						...
																						?>
																							...
																							<tr style="<?=((in_array('Sales Return Time', $fieldview))?'':'display:none;')?>">
																								<td style="padding: 0px !important;font-size: 14px !important;line-height: 15px !important;" width="55%">
																									<input type="text" readonly value="<?= $infomainaccessuser['modulename'] ?>" style="padding:0px;color:black;width: 56px !important;height: 18px !important;border: none !important;outline: none !important;display: inline-flex;font-size: 14px;" class="form-control form-control-sm">
																							...
																							?>
																								<td style="padding: 0px !important;font-size: 14px !important;line-height: 15px !important;text-align: left !important;" width="45%">
																									: 	<b>
																											<?=date('h:i:s A',strtotime($rows[0]['salesreturntime']))?>
																										</b>
																								</td>
																							</tr>
																							<tr>
																							...
																							<table width="100%" style="border:1px solid #cccccc 0px 0px 0px !important;line-height: 13px !important;height: 176px;">
																								...
																								?>
																									...
																									<?php
																										...
																										//FOR DATE FORMATS
																										$salereturn = '<tr><td style="padding: 0px !important;font-size: 14px !important;line-height: 15px !important;" width="55%"><input type="text" readonly value="'. $infomainaccessuser['modulename'] .'" style="padding:0px;color:black;width: 56px !important;height: 18px !important;border: none !important;outline: none !important;display: inline-flex;font-size: 14px;" class="form-control form-control-sm"> <input type="text" readonly value="Number" style="padding:0px;color:black;width: 54px !important;height: 18px !important;border: none !important;outline: none !important;display: inline-flex;font-size: 14px;" class="form-control form-control-sm"> </td><td style="padding: 0px !important;font-size: 14px !important;line-height: 15px !important;text-align: left !important;" width="45%">: <b>'.$rows[0]['salesreturnno'].'</b></td></tr><tr><td style="padding: 0px !important;font-size: 14px !important;line-height: 15px !important;" width="55%"><input type="text" readonly value="'. $infomainaccessuser['modulename'] .'" style="padding:0px;color:black;width: 56px !important;height: 18px !important;border: none !important;outline: none !important;display: inline-flex;font-size: 14px;" class="form-control form-control-sm"> <input type="text" readonly value="Date" style="padding:0px;color:black;width: 54px !important;height: 18px !important;border: none !important;outline: none !important;display: inline-flex;font-size: 14px;" class="form-control form-control-sm"> </td> </td><td style="padding: 0px !important;font-size: 14px !important;line-height: 15px !important;text-align: left !important;" width="45%">: <b>'.date($date,strtotime($rows[0]['salesreturndate'])).'</b></td></tr><tr style="'.(((in_array('Sales Return Time', $fieldview))?'':'display:none;')).'"><td style="padding: 0px !important;font-size: 14px !important;line-height: 15px !important;" width="55%"><input type="text" readonly value="'. $infomainaccessuser['modulename'] .'" style="padding:0px;color:black;width: 56px !important;height: 18px !important;border: none !important;outline: none !important;display: inline-flex;font-size: 14px;" class="form-control form-control-sm"> <input type="text" readonly value="Time" style="padding:0px;color:black;width: 54px !important;height: 18px !important;border: none !important;outline: none !important;display: inline-flex;font-size: 14px;" class="form-control form-control-sm"> </td> </td><td style="padding: 0px !important;font-size: 14px !important;line-height: 15px !important;text-align: left !important;" width="45%">: <b>'.date('h:i:s A',strtotime($rows[0]['salesreturntime'])).'</b></td></tr><tr><td style="padding: 0px !important;font-size: 14px !important;line-height: 15px !important;" width="55%"><input type="text" readonly value="Payment" style="padding:0px;color:black;width: 56px !important;height: 18px !important;border: none !important;outline: none !important;display: inline-flex;font-size: 14px;" class="form-control form-control-sm"> <input type="text" readonly value="Term" style="padding:0px;color:black;width: 54px !important;height: 18px !important;border: none !important;outline: none !important;display: inline-flex;font-size: 14px;" class="form-control form-control-sm"> </td><td style="padding: 0px !important;font-size: 14px !important;line-height: 15px !important;text-align: left !important;" width="45%">: <b>'.$rows[0]['salesreturnterm'].'</b></td></tr>';
																										// }
																										$merge = '</tbody></table></div></td></tr></table></td></tr><tr><td><div style="min-height:92px !important;max-height:92px !important;"><div></td></tr><tr><td><div style="min-height:25px !important;max-height:25px !important;padding: 0px !important;"><div></td></tr><tr style="height:40px !important;"><td style="padding:0px !important;border-bottom: none;"><table width="100%"><tr><td width="30%" style="padding: 0px !important;border-right: 1px solid #cccccc;"><table width="100%"><tr><td style="'.(((in_array('Printed On', $fieldview)))?'':'visibility:hidden;').'vertical-align:middle !important;text-align: center !important;padding-top: 7px !important;"><div style="text-align:center;margin-top: -10px !important;font-size: 12px !important;"><span>Printed On :'.date($dateforfooter,strtotime($dates)).'</span></div><div style="text-align:center;line-height: 7px !important;font-size: 12px !important;"><b>(Page '.$begin.'/'.$finish.')</b></div></td></tr></table></td></tr></table></td></tr></table><span><span style="background-color:#fff !important;font-size: 8px !important;color: #ccc !important;">PAIRSCRIPT</span></span></div><div class="table-responsive" style="margin-top:0.75rem;width: max-content !important;height: max-content !important;max-width:max-content !important; max-height:max-content !important;min-width:max-content !important; min-height:max-content !important;"><table id="printarea" style="border:1px solid #cccccc;margin-bottom: -13px !important;width: 21cm !important;height: 14.8cm !important;max-width:21cm !important; max-height:14.8cm !important;min-width:21cm !important; min-height:14.8cm !important;"><tr style="height:10px !important;"><td style="border-bottom: 1px solid #cccccc;"><table width="100%"><tr><td style="text-align:center;line-height: 13px !important;"><b style="font-size:15px !important;display: inline-flex;white-space: nowrap;width: max-content;overflow: hidden;text-align: center;max-width: 166px;">'.$infomainaccessuser['modulename'].'</b></td></tr></table></td></tr><tr style="height:10px !important;"><td><table width="100%"><tr><td style="text-align:center;height: 80px !important;"><table width="100%"><tr><td width="16%" style="text-align:right;">'.$imgans.'</td><td width="'.((($access['salereturnbank']=='0' && $access['salereturnname']=='0' && $access['salereturnaccnumber']=='0' && $access['salereturnifsccode']=='0' && $access['salereturnbranchandcity']=='0')?'86%':'56%')).'" style="text-align:left;vertical-align: top;padding-left: 30px !important;"><table width="100%"><tr style="padding:0px !important;"><td><table width="100%" style="padding:0px !important;"><tr style="padding:0px !important;"><td style="padding:0px !important;line-height: 15px !important;"><strong style="font-size:14px;vertical-align: top;display: inline-flex;white-space: nowrap;width: 361px;overflow: hidden;">'.$info['franchisename'].'</strong></td></tr><tr style="padding:0px !important;"><td style="padding:0px !important;line-height: 15px !important;margin-bottom: -3px;"><span style="font-size:12px;margin: 0px !important;display: inline-flex;width: '.((($access['salereturnbank']=='0' && $access['salereturnname']=='0' && $access['salereturnaccnumber']=='0' && $access['salereturnifsccode']=='0' && $access['salereturnbranchandcity']=='0')?'618px':'361px')).';overflow: hidden;white-space: nowrap;">'.$info['street'].' '.$info['city'].' '.$info['pincode'].' '.$info['state'].' '.$info['country'].'</span></td></tr></table></td></tr><tr><td><table width="100%"><tr style="'.(($access['salereturnbranchphone']=='0')?'display:none;':'').'"><td width="10%" style="font-size:12px;padding: 0px !important;line-height: 13px !important;">Phone </td><td width="90%" style="font-size:12px;padding: 0px !important;line-height: 13px !important;">: <span style="display: inline-flex;white-space: nowrap;width: 253px;overflow: hidden;">'.$info['mobile'].'</span></td></tr><tr><td width="10%" style="font-size:12px;padding: 0px !important;line-height: 15px !important;'.(($access['salereturnbranchemail']=='0')?'visibility:hidden;':'').'">E-mail </td><td width="90%" style="font-size:12px;padding: 0px !important;line-height: 15px !important;"><input type="text" readonly value=": '.$info['email'].'" style="padding:0px;color:black;width: 186px !important;height: 18px !important;border: none !important;outline: none !important;display: inline-flex;font-size: 12px;'.(($access['salereturnbranchemail']=='0')?'visibility:hidden;':'').'" class="form-control form-control-sm"><span style="'.(($access['salereturnprintdlno20']=='1')?'display:inline-flex;':'display:none;').'white-space: nowrap;width: 163px;overflow: hidden;float: right;margin-right: 9px;">DL No 20 &nbsp; : '.$info['dlno20'].'</span></td></tr><tr><td width="10%" style="font-size:12px;padding: 0px !important;line-height: 15px !important;'.(($access['salereturnbranchgstin']=='0')?'visibility:hidden;':'').'">GSTIN </td><td width="90%" style="font-size:12px;padding: 0px !important;line-height: 15px !important;"><span style="display: inline-flex;white-space: nowrap;width: 134px;overflow: hidden;'.(($access['salereturnbranchgstin']=='0')?'visibility:hidden;':'').'">: '.$info['gstno'].'</span><span style="'.(($access['salereturnprintdlno21']=='1')?'display:inline-flex;':'display:none;').'white-space: nowrap;width: 163px;overflow: hidden;float: right;margin-right: 9px;">DL No 21 &nbsp; : '.$info['dlno21'].'</span></td></tr></table></td></tr></table></td><td width="30%" style="'.((($access['salereturnbank']=='0' && $access['salereturnname']=='0' && $access['salereturnaccnumber']=='0' && $access['salereturnifsccode']=='0' && $access['salereturnbranchandcity']=='0')?'display: none;':'')).'"><table width="100%"><tr '.(($access['billbank']=='1')?'':'style="display:none;"').'><td style="padding: 0px 6px !important;font-size: 11px !important;line-height: 15px !important;text-align: left;" width="35%"><input type="text" readonly value="Bank" style="padding:0px;color:black;width: 113px !important;height: 15px !important;border: none !important;outline: none !important;display: inline-flex;font-size: 11px;" class="form-control form-control-sm"></td><td style="padding: 0px !important;font-size: 11px !important;line-height: 15px !important;text-align: left !important;white-space: nowrap !important;overflow: hidden !important;" width="65%">: <b style="display:inline-flex;max-width: 163px !important;overflow: hidden;">'.$info['bank'].'</b></td></tr><tr '.(($access['billname']=='1')?'':'style="display:none;"').'><td style="padding: 0px 6px !important;font-size: 11px !important;line-height: 15px !important;text-align: left;" width="35%"><input type="text" readonly value="Name" style="padding:0px;color:black;width: 113px !important;height: 15px !important;border: none !important;outline: none !important;display: inline-flex;font-size: 11px;" class="form-control form-control-sm"></td><td style="padding: 0px !important;font-size: 11px !important;line-height: 15px !important;text-align: left !important;white-space: nowrap !important;overflow: hidden !important;" width="65%">: <b style="display:inline-flex;max-width: 163px !important;overflow: hidden;">'.$info['names'].'</b></td></tr><tr '.(($access['billaccnumber']=='1')?'':'style="display:none;"').'><td style="padding: 0px 6px !important;font-size: 11px !important;line-height: 15px !important;text-align: left;" width="35%"><input type="text" readonly value="Account Number" style="padding:0px;color:black;width: 113px !important;height: 15px !important;border: none !important;outline: none !important;display: inline-flex;font-size: 11px;" class="form-control form-control-sm"></td><td style="padding: 0px !important;font-size: 11px !important;line-height: 15px !important;text-align: left !important;white-space: nowrap !important;overflow: hidden !important;" width="65%">: <b style="display:inline-flex;max-width: 163px !important;overflow: hidden;">'.$info['accountnumber'].'</b></td></tr><tr '.(($access['billifsccode']=='1')?'':'style="display:none;"').'><td style="padding: 0px 6px !important;font-size: 11px !important;line-height: 15px !important;text-align: left;" width="35%"><input type="text" readonly value="IFSC Code" style="padding:0px;color:black;width: 113px !important;height: 15px !important;border: none !important;outline: none !important;display: inline-flex;font-size: 11px;" class="form-control form-control-sm"></td><td style="padding: 0px !important;font-size: 11px !important;line-height: 15px !important;text-align: left !important;white-space: nowrap !important;overflow: hidden !important;" width="65%">: <b style="display:inline-flex;max-width: 163px !important;overflow: hidden;">'.$info['ifsccode'].'</b></td></tr><tr '.(($access['billbranchandcity']=='1')?'':'style="display:none;"').'><td style="padding: 0px 6px !important;font-size: 11px !important;line-height: 15px !important;text-align: left;" width="35%"><input type="text" readonly value="Branch & City" style="padding:0px;color:black;width: 100px !important;height: 16px !important;border: none !important;outline: none !important;display: inline-flex;font-size: 11px;" class="form-control form-control-sm"></td><td style="padding: 0px !important;font-size: 11px !important;line-height: 15px !important;text-align: left !important;white-space: nowrap !important;overflow: hidden !important;" width="65%">: <b style="display:inline-flex;max-width: 163px !important;overflow: hidden;">'.$info['branchandcity'].'</b></td></tr></table></td></tr></table></td></tr></table></td></tr><tr style="height:1px !important;border: 1px solid #cccccc;"><td style="padding: 0px !important;"><table width="100%"><tr><td width="66%" style="white-space: normal !important;vertical-align: top;border-right: 1px solid #cccccc;padding: 0px !important;height: 98px !important;"><table width="100%"><tr style="background-color: #eee;"><td style="padding: 0px !important;"><strong style="padding:0px 6px !important;">Billing Address</strong></td></tr><tr>'.$custname.'</tr><tr><td style="padding: 0px 6px !important;line-height: 15px !important;white-space: nowrap !important;">'.$bill1.$bill2.'</td></tr><tr><td style="padding: 0px !important;"><table width="100%"><tr '.(((in_array('Work Phone', $fieldview))?'':'style="display:none;"')).'><td width="38%" style="font-size:12px;padding:0px 6px !important;line-height: 15px !important;">Work Phone </td><td width="62%" style="font-size:12px;padding: 0px !important;line-height: 15px !important;">: <b style="display: inline-flex;white-space: nowrap;width: 100px;overflow: hidden;">'.$rows[0]['workphone'].'</b></td></tr><tr><td width="38%" style="'.(((($infomainaccessuser['salereturnprintgstin']=='show')||($rows[0]['gstno']!='')&&($infomainaccessuser['salereturnprintgstin']!='hide'))||(in_array('Mobile Phone', $fieldview)))?'':'display:none;').'font-size:12px;padding:0px 6px !important;line-height: 15px !important;">'.(((($infomainaccessuser['salereturnprintgstin']=='show')||($rows[0]['gstno']!='')&&($infomainaccessuser['salereturnprintgstin']!='hide'))?'GSTIN':'')).' </td><td width="62%" style="'.(((($infomainaccessuser['salereturnprintgstin']=='show')||($rows[0]['gstno']!='')&&($infomainaccessuser['salereturnprintgstin']!='hide'))||(in_array('Mobile Phone', $fieldview)))?'':'display:none;').'font-size:12px;padding: 0px !important;line-height: 15px !important;"> '.(((($infomainaccessuser['salereturnprintgstin']=='show')||($rows[0]['gstno']!='')&&($infomainaccessuser['salereturnprintgstin']!='hide'))?' :'.$gstin:'')).' <span style=" '.((in_array('Mobile Phone', $fieldview))?'display: inline-flex;':'display:none;').'white-space: nowrap;width: 175px;overflow: hidden;float:right;"><span style="display: block;width: 100%;"><span style="display: inline-flex;width: 50%;"> Mobile Phone</span><b style="display: inline-flex;width: 50%;"> : '.($rows[0]['mobile']).'</b></span></span></td></tr></table></td></tr></table></td><td width="33%" style="white-space: normal !important;vertical-align: middle;padding: 0px !important;"><table width="100%" style="text-align: center !important;">'.$salereturn.'</table></td></tr></table></td></tr><tr style="height:1px !important;"><td style="padding: 0px !important;"><table width="100%"><tr style="vertical-align: top !important;"><td style="padding: 0px !important;height:176px !important;"><div style="max-height: 176px !important;min-height: 176px !important;overflow: hidden;"><table width="100%" style="border:1px solid #cccccc 0px 0px 0px !important;line-height: 13px !important;height: 176px;"><thead style="background-color: #eee;"><tr style="height:1px !important;"><th width="20%" style="white-space: nowrap;overflow: hidden;border: 1px solid #cccccc;font-size: 12px !important;padding:0px 6px !important;'.((in_array('Item Details', $fieldview))?'':'display:none !important;').'">ITEM DETAILS</th><th width="7%" style="white-space: nowrap;overflow: hidden;border: 1px solid #cccccc;font-size: 12px !important;padding:0px 6px !important;'.((in_array('Category', $fieldview))?'':'display:none !important;').'max-width: 40px;min-width: 40px;">'.($access['txtnamecategory']).'</th><th width="7%" style="white-space: nowrap;overflow: hidden;border: 1px solid #cccccc;font-size: 12px !important;padding:0px 6px !important;'.((in_array('Hsn or Sac', $fieldview))?'':'display:none !important;').'max-width: 56px;min-width: 56px;">HSN/SAC</th><th width="7%" style="white-space: nowrap;overflow: hidden;border: 1px solid #cccccc;font-size: 12px !important;padding:0px 6px !important;'.(($access['batchexpiryval']==1)?'':'display:none !important;').'min-width:59px !important;max-width:59px !important;">BATCH</th><th width="7%" style="white-space: nowrap;overflow: hidden;border: 1px solid #cccccc;font-size: 12px !important;padding:0px 6px !important;'.(($access['batchexpiryval']==1)?'':'display:none !important;').'">EXPIRY</th><th width="9%" style="white-space: nowrap;overflow: hidden;border: 1px solid #cccccc;font-size: 12px !important;padding:0px 6px !important;text-align: right !important; '.((in_array('Mrp', $fieldview))?'':'display:none !important;').'">MRP</th><th width="9%" style="white-space: nowrap;overflow: hidden;border: 1px solid #cccccc;font-size: 12px !important;padding:0px 6px !important;text-align: right !important; '.((in_array('Rate', $fieldview))?'':'display:none !important;').'">RATE</th><th width="7%" style="white-space: nowrap;overflow: hidden;border: 1px solid #cccccc;font-size: 12px !important;padding:0px 6px !important;text-align: right !important;min-width:63px;max-width:63px;'.((in_array('Quantity', $fieldview))?'':'display:none !important;').'">'.($access['txtqtysalereturn']).'</th><th width="7%" style="white-space: nowrap;overflow: hidden;border: 1px solid #cccccc;font-size: 12px !important;padding:0px 6px !important;text-align: right !important;'.((in_array('GST Percentage', $fieldview))?'':'display:none !important;').'">GST %</th><th width="5%" style="white-space: nowrap;overflow: hidden;border: 1px solid #cccccc;font-size: 12px !important;padding:0px 6px !important;text-align: right !important;max-width: 104px;min-width: 104px;'.((in_array('Taxable Value', $fieldview))?'':'display:none !important;').'">'.(($access['txttaxablesalereturn'])).'</th></tr></thead><tbody>';
																										$finalrow=3;
																										if ((($item-1)%$finalrow)==0) {
																											if ((($countt-1)-$totpros)!=0) {
																												if (($item-1)!=0) {
													...
													?>
			 										...
			 										<p align="right" class="mt-3" style="margin-right:-25px; cursor:pointer" id="templatetext">
			 											Template: '<?=($info['salesreturntemplate']=='1')?'24.13cm X 15.75cm':''?><?=($info['salesreturntemplate']=='0')?'Standard A4 Portrait':''?><?=($info['salesreturntemplate']=='2')?'Standard A5 Landscape':''?><?=($info['salesreturntemplate']=='3')?'A5 Landscape ()':''?><?=($info['salesreturntemplate']=='4')?'3 Inch Paper':''?> ' 
			 											<a data-bs-toggle="modal" data-bs-target="#changesalesreturnModal" class="text-blue">
			 												Change
			 											</a>
			 										</p>
			 									</div>
												<div class="modal fade" id="changesalesreturnModal" tabindex="-1" role="dialog" aria-labelledby="changesalesreturnModalLabel" aria-hidden="true">
  													<div class="modal-dialog modal-dialog-centered" role="document">
	 													<div class="modal-content">
															<div class="modal-header">
		  														<h5 class="modal-title" id="changesalesreturnModalLabel">
		  															Choose Template
		  														</h5>
		  														<button type="button" class="btn-close text-dark" data-bs-dismiss="modal" aria-label="Close">
			 														<span aria-hidden="true">&times;</span>
															...
															<div class="modal-body">
																...
																function standandclick(){
																	...
																	setTimeout(function() {
																		...
																		if (r == true) {
	 																		window.location.href='changetemplate.php?template=salesreturn&id=<?=$_GET['id']?>&no=<?=$salesreturnno?>&date=<?=$salesreturndate?>&temptype=0&submt=submt';
																		}
																	}, 100);
																}
																function standardafiveclick(){
																	...
																	setTimeout(function() {
																		var r = confirm('Are you sure want to Change this Template?');
																		if (r == true) {
	 																		window.location.href='changetemplate.php?template=salesreturn&id=<?=$_GET['id']?>&no=<?=$salesreturnno?>&date=<?=$salesreturndate?>&temptype=2&submt=submt';
																		}
																	}, 100);
																}
																function standardafive2click(){
																	...
																	setTimeout(function() {
																		var r = confirm('Are you sure want to Change this Template?');
																		if (r == true) {
	 																		window.location.href='changetemplate.php?template=salesreturn&id=<?=$_GET['id']?>&no=<?=$salesreturnno?>&date=<?=$salesreturndate?>&temptype=3&submt=submt';
																		}
																	}, 100);
																}
																function advancedclick(){
																	...
																	setTimeout(function() {
																		var r = confirm('Are you sure want to Change this Template?');
																		if (r == true) {
	 																		window.location.href='changetemplate.php?template=salesreturn&id=<?=$_GET['id']?>&no=<?=$salesreturnno?>&date=<?=$salesreturndate?>&temptype=1&submt=submt';
																		}
																	}, 100);
																}
																function threeinchclick(){
																	...
																	setTimeout(function() {
																		var r = confirm('Are you sure want to Change this Template?');
																		if (r == true) {
	 																		window.location.href='changetemplate.php?template=salesreturn&id=<?=$_GET['id']?>&no=<?=$salesreturnno?>&date=<?=$salesreturndate?>&temptype=4&submt=submt';
																		}
																	}, 100);
																}
															<?php
...
<div class="modal fade" id="AddNewSales ReturnTerm" tabindex="-1" role="dialog" style="z-index: 1051;">
		...
		$.ajax({
			...
			success:function(result){
				...
				if(resarray[1]=='0'){}
					...
					$('#AddNewSales ReturnTerm').modal('hide');
					return false;
				}
			}
		});
...
$("#salesreturnterm").on("select2:open", function() {
	$("#configureunits").attr("data-bs-target","#AddNewSales ReturnTerm");
});
$("#salesreturnterm").on("select2:open", function() {
...
function converHTMLFileToPDF() {
	...
	$.ajax({
		type: "GET",
		url: 'salesreturndompdf.php?term=salesreturn&icsgst='+finalforprint+'&names=Sales Returns-<?=$_GET['salesreturnno']?>-<?=$_GET['salesreturndate']?>&salesreturnno=<?=$_GET['salesreturnno']?>&salesreturndate=<?=$_GET['salesreturndate']?>&sizes=a4',
		success: function (result) {
			...
			$("#time").html(Timer);
 			var req = new XMLHttpRequest();
  			req.open("GET", "dompdf/Sales Returns-<?=$_GET['salesreturnno']?>-<?=$_GET['salesreturndate']?>.pdf", true);
			...
			$("#timedownload").html(Timer);
 			var req = new XMLHttpRequest();
  			...
  			req.onload = function (event) {
	 			...
	 			link.download="Sales Returns-<?=$_GET['salesreturnno']?>-<?=$_GET['salesreturndate']?>.pdf";
	 			link.click();
  			};
  			req.send();
...
function converHTMLFileToPDF() {
	...
	$.ajax({
		type: "GET",
		url: 'salesreturndompdf.php?term=salesreturn&icsgst='+finalforprint+'&names=Sales Returns-<?=$_GET['salesreturnno']?>-<?=$_GET['salesreturndate']?>&salesreturnno=<?=$_GET['salesreturnno']?>&salesreturndate=<?=$_GET['salesreturndate']?>&sizes=a5',
		success: function (result) {
			if (result && result.length > 0) {       
				result=$.parseJSON( result );
				Timer=result.timer;
...
function converHTMLFileToPDF() {
	...
	$.ajax({
		type: "GET",
		url: 'salesreturndompdf.php?term=salesreturn&icsgst='+finalforprint+'&names=Sales Returns-<?=$_GET['salesreturnno']?>-<?=$_GET['salesreturndate']?>&salesreturnno=<?=$_GET['salesreturnno']?>&salesreturndate=<?=$_GET['salesreturndate']?>&sizes=dt',
		success: function (result) {
			if (result && result.length > 0) {       
				result=$.parseJSON( result );
				Timer=result.timer;
...
function converHTMLFileToPDF() {
	...
	$.ajax({
		type: "GET",
		url: 'salesreturndompdf.php?term=salesreturn&icsgst='+finalforprint+'&names=Sales Returns-<?=$_GET['salesreturnno']?>-<?=$_GET['salesreturndate']?>&salesreturnno=<?=$_GET['salesreturnno']?>&salesreturndate=<?=$_GET['salesreturndate']?>&sizes=a52',
		success: function (result) {
			if (result && result.length > 0) {       
				result=$.parseJSON( result );
				Timer=result.timer;
...
function converHTMLFileToPDF() {
	...
	$.ajax({
		type: "GET",
		url: 'salesreturndompdf.php?term=salesreturn&icsgst='+finalforprint+'&names=Sales Returns-<?=$_GET['salesreturnno']?>-<?=$_GET['salesreturndate']?>&salesreturnno=<?=$_GET['salesreturnno']?>&salesreturndate=<?=$_GET['salesreturndate']?>&sizes=threeinch',
		success: function (result) {
			if (result && result.length > 0) {       
				result=$.parseJSON( result );
				Timer=result.timer;
...
<?php
		...
		header("Location:salesreturns.php?error=No Information Found");
	}
}
else{
	header("Location:salesreturns.php?error=No Information Found");  
