...
if((!isset($_POST['searchTerm']))&&(!isset($_POST['customername']))){
	$fetchData = mysqli_query($con,"select * from paircustomers where customername!='' and moduletype='Vendors' and (franchisesession='".$_SESSION["franchisesession"]."' or cvisiblity='PUBLIC') and (createdid='$companymainid') order by customername limit 50");
}else if(isset($_POST['customername'])){
	$search = mysqli_real_escape_string($con,$_POST['customername']); 
	$fetchData = mysqli_query($con,"select * from paircustomers where id='".$search."' and moduletype='Vendors' and (franchisesession='".$_SESSION["franchisesession"]."' or cvisiblity='PUBLIC') and (createdid='$companymainid') order by customername limit 50");
}else{
	$search = mysqli_real_escape_string($con,$_POST['searchTerm']); 
	$fetchData = mysqli_query($con,"select * from paircustomers where customername like '%".$search."%' and moduletype='Vendors' and (franchisesession='".$_SESSION["franchisesession"]."' or cvisiblity='PUBLIC') and (createdid='$companymainid') limit 50");
}
