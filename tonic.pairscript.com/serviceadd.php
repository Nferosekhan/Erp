...
$sqlismodulespublicname=mysqli_query($con, "select * from pairmodules where moduletype='Services' order by id  asc");
...
$sqlismainaccesspublicname=mysqli_query($con, "select * from pairmainaccess where createdid='$companymainid' and moduletype='Services' and franchiseid='".$_SESSION['franchisesession']."' order by id  asc");
$infomainaccesspublicname=mysqli_fetch_array($sqlismainaccesspublicname);
$sqlcode=mysqli_query($con,"select count(productcode) from pairproducts where itemmodule='Services'");
$anscode=mysqli_fetch_array($sqlcode);
$oldcode=$anscode[0];
$productcode=$oldcode+1;
$publicsql=mysqli_query($con,"select count(publicid) from pairproducts where createdid='$companymainid' and itemmodule='Services'");
...
$privatesql=mysqli_query($con,"select count(privateid) from pairproducts where createdid='$companymainid' and itemmodule='Services' and franchisesession='".$_SESSION['franchisesession']."'");
...
{
    $descriptionserv=mysqli_real_escape_string($con, $_POST['description']);
}
else{
    $descriptionserv=" ";
}
...
$sqlup = "insert into pairproducts set createdon='$times',productname='$productname',createdid='$companymainid', createdby='".$_SESSION["unqwerty"]."', codetags='$codetags',itemmodule='Services',franchisesession='".$_SESSION["franchisesession"]."',  description='$descriptionserv',defaultunit='$forpro',category='$category', subcategory='$subcategory',pvisiblity='$pvisibility',delivery='$delivery',productcode='$productcode',taxpref='$taxpref',intratax='$intratax',intertax='$intertax',hsncode='$hsncode',publicid='$publiccode',privateid='$privatecode',ordering='$ordering'";
            $queryup = mysqli_query($con, $sqlup);
            if ($queryup) {
             header('Location:services.php?remarks=Added Successfully');
            }
            
$tid=mysqli_insert_id($con);  
             $sql3=mysqli_query($con, "update pairmainaccess set modulesuffix=modulesuffix+1 where franchiseid='".$_SESSION['franchisesession']."' and moduletype='Services'");
$productid=$tid;
$anspricenames="";
...
$sqlupn = "insert into pairprosale set productid='$productid',salename='$pricename',salemrp='$ansmrps',salecost='$anssellprices',saledescription='$ansdescriptions',itemmodule='Services',createdid='$companymainid'";
            $queryupn = mysqli_query($con, $sqlupn);
            $sqlupnpro = mysqli_query($con,"update pairproducts set salescost='$anssellprices' where id='$tid'");
            if(!$queryupn){
               header('Location:serviceadd.php?error=Pricename Added Unsuccessfully');
            }
}
else{
    $sqlupn = "insert into pairprosale set productid='$productid',itemmodule='Services',createdid='$companymainid'";
            $queryupn = mysqli_query($con, $sqlupn);
            if(!$queryupn){
               header('Location:serviceadd.php?error=Pricename Added Unsuccessfully');
            }
...
$sqlupn = "insert into pairpropurchase set productid='$productid',purchasename='$pricenamepur',purchasemrp='$ansmrpspur',purchasecost='$anssellpricespur',purchasedescription='$ansdescriptionspur',itemmodule='Services',createdid='$companymainid'";
            $queryupn = mysqli_query($con, $sqlupn);
            if(!$queryupn){
               header('Location:serviceadd.php?error=Pricename Added Unsuccessfully');
            }
    ...
    $sqlupn = "insert into pairpropurchase set productid='$productid',itemmodule='Services',createdid='$companymainid'";
            $queryupn = mysqli_query($con, $sqlupn);
            if(!$queryupn){
               header('Location:serviceadd.php?error=Pricename Added Unsuccessfully');
            }
                ...
                if($productname!='')
                {
                    if($ch!='')
                    {
                        $ch.='<br> '.$servhead.' Name <span style="color:green;" id="prohisfromtospan">( '.$productname.' ) </span>';
                    }
                    else
                    {
                        $ch.=''.$servhead.' Name <span style="color:green;" id="prohisfromtospan">( '.$productname.' ) </span>';
                    }                   
                } 
                if($codetags!='')
                {
                ...
                if($hsncode!='')
                {
                    if($ch!='')
                    {
                        $ch.='<br> SAC Code <span style="color:green;" id="prohisfromtospan">( '.$hsncode.' ) </span>';
                    }
                    else
                    {
                        $ch.='SAC Code <span style="color:green;" id="prohisfromtospan">( '.$hsncode.' ) </span>';
                    }                   
                }
                if($category!=' ')
                {
                ...
                if($descriptionserv!='')
                {
                    if($ch!='')
                    {
                        $ch.='<br> Description <span style="color:green;" id="prohisfromtospan">( '.$descriptionserv.' ) </span>';
                    }
                    else
                    {
                        $ch.='Description <span style="color:green;" id="prohisfromtospan">( '.$descriptionserv.' ) </span>';
                    }    
                }
                if (in_array('Service Visibility', $fieldadd)) {
                if($pvisibility!='')
                {
                    if($ch!='')
                    {
                ...
                if($ch!='')
                {
                $sqluse=mysqli_query($con, "insert into pairusehistory set usetype='SERVICES', createdon='$times',  createdby='".$_SESSION["unqwerty"]."', useid='$tid', useremarks='".$ch."' ");
                }
                    ...
                    <div class="col-12">
                        <div class="card mb-4 mt-5 p-3">
                            <div class="card-body p-0" style="font-family:'Myriad Set Pro','Helvetica Neue',Helvetica,Arial,sans-serif !important;">
                                <p class="mb-3 mt-0 ml-0" id="neweditpro" data-toggle="tooltip" title="Services you by and/or sell and that you track quantities of-Services you by and/or sell but don't need to (or can't) track quantities of, for example, nuts and bolts used in an installation" data-placement="right"><i class="fa fa-plus-square-o"></i> New
...
<?php
    ...
    ?>
                                    ...
                                    <?php
     // if ((in_array('Service Information', $fieldadd))) {
        ?>
        ...
        <?php
            $sql=mysqli_query($con,"select count(productcode) from pairproducts where itemmodule='Services'");
            $ans=mysqli_fetch_array($sql);
            ?>
                                                    <div class="row justify-content-center" <?=((in_array('Service Code', $fieldadd))?'':'style="display:none;"')?>>
                                                        <div class="col-lg-6">
                                                            <div class="form-group row">
                                                                <div class="col-sm-4">
...
<?php
            ...
            $sqlismodulespublicname=mysqli_query($con, "select * from pairmodules where moduletype='Services' order by id  asc");
                                $infomodulespublicname=mysqli_fetch_array($sqlismodulespublicname);
            ?>
                                                    <div class="row justify-content-center" <?=((in_array('Service Public Code', $fieldadd))?'':'style="display:none;"')?>>
                                                        <div class="col-lg-6">
                                                            <div class="form-group row">
                                                                <div class="col-sm-4">
...
<?php
            ...
            $sqlismainaccesspublicname=mysqli_query($con, "select * from pairmainaccess where createdid='$companymainid' and moduletype='Services' and franchiseid='".$_SESSION['franchisesession']."' order by id  asc");
                                $infomainaccesspublicname=mysqli_fetch_array($sqlismainaccesspublicname);
            ?>
                                                    <div class="row justify-content-center" <?=((in_array('Service Private Code', $fieldadd))?'':'style="display:none;"')?>>
                                                        <div class="col-lg-6">
                                                            <div class="form-group row">
                                                                <div class="col-sm-4">
                                                            ...
                                                            <div class="form-group row">
                                                                <div class="col-sm-4">
                                                                    <label for="Unit" class="custom-label"><span
                                                                            class="text-danger">Unit * <svg data-toggle="tooltip" title="The services will be measured in terms of this unit (e.g.: kg, dozen)" xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-question-circle" viewBox="0 0 16 16">
...
?>
                                                    ...
                                                    <div class="row justify-content-center" <?=((in_array('SAC Code', $fieldadd))?'':'style="display:none;"')?>>
                                                        <div class="col-lg-6">
                                                            <div class="form-group row">
                                                                <div class="col-sm-4">
                                                                ...
                                                                <div class="col-sm-8">
                                                                    <input type="text"
                                                                        ...
                                                                        placeholder="SAC Code" maxlength="100">
                                                                </div>
                                                            </div>
                                                        </div>
                                                    </div>
                                                            ...
                                                            <div class="form-group row">
                                                                ...
                                                                <div class="col-sm-8" onclick="andus()">
                                                                    <select
                                                                        ...
                                                                        <?php
                                                                    $sqli = mysqli_query($con, "select * from paircategory where (createdid='$companymainid' or createdid='0') and itemmodule='Services' and category!='' order by category asc");
                                                                    while ($info = mysqli_fetch_array($sqli)) {
                                                                    ?>
        ...
        ?>
          ...
          <div class="row justify-content-center deltophead" <?=((in_array('Delivery', $fieldadd))?'':'style="display:none;"')?>>
                                   ...
                                   <?php
                                    // }
                                    ?>
                                    <div class="accordion" id="accordionRental" <?=((in_array('Service Visibility', $fieldadd))?'':'style="display:none;"')?>>
                                        <div class="accordion-item mb-1">
  ...
  <path d="M5.255 5.786a.237.237 0 0 0 .241.247h.825c.138 0 .248-.113.266-.25.09-.656.54-1.134 1.342-1.134.686 0 1.314.343 1.314 1.168 0 .635-.374.927-.965 1.371-.673.489-1.206 1.06-1.168 1.987l.003.217a.25.25 0 0 0 .25.246h.811a.25.25 0 0 0 .25-.25v-.105c0-.718.273-.927 1.01-1.486.609-.463 1.244-.977 1.244-2.056 0-1.511-1.276-2.241-2.673-2.241-1.267 0-2.655.59-2.75 2.286zm1.557 5.763c0 .533.425.927 1.01.927.609 0 1.028-.394 1.028-.927 0-.552-.42-.94-1.029-.94-.584 0-1.009.388-1.009.94z"/>
                      ...
                      <div class="custom-control custom-radio mr-sm-2">
                        <input type="radio" class="custom-control-input" name="visibility" id="visibility" value="PUBLIC" <?=($infomainaccessuser['servicevisible']=='PUBLIC')?'checked':''?>>
                      ...
                      <div class="col-sm-6 my-1">
                      <div class="custom-control custom-radio mr-sm-2">
                        <input type="radio" class="custom-control-input" name="visibility" id="novisibility" value="PRIVATE" <?=($infomainaccessuser['servicevisible']=='PRIVATE')?'checked':''?>>
...
<td data-label="MRP" <?=((in_array('Purchase MRP', $fieldadd))?'':'style="display:none;"')?>>
    <div>
       <span><?php echo $res[0]; ?></span>
    <input type="age" min="0" name="mrppur[]" id="quantity1" class="form-control form-control-sm bordernoneinput bor totaldesign productselectwidth" onChange="productcalc(1)" placeholder="0.00">
  </div>
</td>
