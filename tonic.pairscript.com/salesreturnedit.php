...
if((isset($_GET['salesreturnno']))&&(isset($_GET['salesreturndate']))){
	...
	if($sql->num_rows>0){
		...
		if(isset($_POST['grandtotal'])){
			...
			$oldsalesreturnno=htmlspecialchars( $_GET['salesreturnno'], ENT_QUOTES, 'UTF-8');
			$oldsalesreturndate=htmlspecialchars( $_GET['salesreturndate'], ENT_QUOTES, 'UTF-8');
			$salesreturnno=htmlspecialchars( $_POST['salesreturnno'], ENT_QUOTES, 'UTF-8');
			...
			for($i=0; $i<count($_POST['oldproductname']); $i++){
				...
				if($productname!=''){
					$sql = $con->prepare("DELETE FROM pairsalesreturns WHERE createdid=? AND franchisesession=? AND salesreturnno=? AND salesreturndate=?");
					$sql->bind_param("ssss", $companymainid, $_SESSION["franchisesession"], $oldsalesreturnno, $oldsalesreturndate);
					$sql->execute();
					$sqldelledger = $con->prepare("DELETE FROM pairledgers WHERE createdid=? AND franchisesession=? AND ledgerno=? AND ledgerdate=? AND (type='salesreturn' OR type='salesreturn payment')");
					$sqldelledger->bind_param("ssss", $companymainid, $_SESSION["franchisesession"], $oldsalesreturnno, $oldsalesreturndate);
					$sqldelledger->execute();
					$sqls = $con->prepare("DELETE FROM pairsalesreturnpayments WHERE receiptno=? AND receiptdate=?");
					$sqls->bind_param("ss", $oldsalesreturnno, $oldsalesreturndate);
					$sqls->execute();
					$sqls->close();
					$sqlsant = $con->prepare("DELETE FROM pairsalesreturnpayhistory WHERE salesreturnno=? AND salesreturndate=? AND type='salesreturn'");
					$sqlsant->bind_param("ss", $oldsalesreturnno, $oldsalesreturndate);
					$sqlsant->execute();
					$sqlsant->close();
					//  margins     
			...
			$sql2->bind_param("iiss", $_SESSION['franchisesession'], $companymainid, $oldsalesreturnno, $oldsalesreturndate);
			$sql2->execute();
			$sql2->store_result();
			...
			if($sql2->num_rows==0){
				...
				if(($salesreturnterm=='CASH')){
					if ($validpaidamount!=0) {
						...
						$sqlselects=$con->prepare("SELECT id FROM pairsalesreturnpayments WHERE receiptno=? AND receiptdate=?");
						$sqlselects->bind_param("ss", $salesreturnno, $salesreturndate);
						$sqlselects->execute();
						...
						if ($sqlselects->num_rows==0) {
							...
							$sqlbatchupd = $con->prepare("UPDATE pairsalesreturnpayments SET createdid=?, createdby=?, franchisesession=?, createdon=?,  term='RECEIPT', type='salesreturn', customername=?, customerid=?, receiptno=?, receiptdate=?, amount=?, paymentmode=?, notes='-' WHERE receiptno=? AND receiptdate=?");
							$sqlbatchupd->bind_param("ssssssssssss", $companymainid, $_SESSION["unqwerty"], $_SESSION["franchisesession"], $createdon, $customername, $customerid, $salesreturnno, $salesreturndate, $validpaidamount, $salesreturnterm, $salesreturnno, $salesreturndate);
							$sqlbatchupd->execute();
							if($sqlbatchupd){
								$sqlbatchupd->close();
								$sqle = $con->prepare("UPDATE pairsalesreturnpayhistory SET amount=? WHERE salesreturnno=? AND salesreturndate=? AND type='salesreturn'");
								$sqle->bind_param("sss", $validpaidamount, $salesreturnno, $salesreturndate);
								$sqle->execute();
								$sqle->close();
							}
						...
						$sqlbatchupdant = $con->prepare("UPDATE pairsalesreturnpayments SET createdid=?, createdby=?, franchisesession=?, createdon=?,  term='RECEIPT', type='salesreturn', customername=?, customerid=?, receiptno=?, receiptdate=?, amount=?, paymentmode=?, notes='-' WHERE receiptno=? AND receiptdate=?");
						$sqlbatchupdant->bind_param("ssssssssssss", $companymainid, $_SESSION["unqwerty"], $_SESSION["franchisesession"], $createdon, $customername, $customerid, $salesreturnno, $salesreturndate, $validpaidamount, $salesreturnterm, $salesreturnno, $salesreturndate);
						$sqlbatchupdant->execute();
						if($sqlbatchupdant){
							$sqlbatchupdant->close();
							$sqleant = $con->prepare("UPDATE pairsalesreturnpayhistory SET amount=? WHERE salesreturnno=? AND salesreturndate=? AND type='salesreturn'");
							$sqleant->bind_param("sss", $validpaidamount, $salesreturnno, $salesreturndate);
							$sqleant->execute();
							$sqleant->close();
						}
				...
				if(($salesreturnterm=='CASH,CARD,GPAY')){
					...
					for($termrun = 0;$termrun<count(explode(',', $salesreturnterm));$termrun++){
						if ($salereturnarr[$termrun]>0) {
							...
							if ($sqlselects->num_rows==0) {
								...
								$sqlbatchupd = $con->prepare("UPDATE pairsalesreturnpayments SET createdid=?, createdby=?, franchisesession=?, createdon=?,  term='RECEIPT', type='salesreturn', customername=?, customerid=?, receiptno=?, receiptdate=?, amount=?, paymentmode=?, notes='-' WHERE receiptno=? AND receiptdate=?");
								$sqlbatchupd->bind_param("ssssssssssss", $companymainid, $_SESSION["unqwerty"], $_SESSION["franchisesession"], $createdon, $customername, $customerid, $salesreturnno, $salesreturndate, $salereturnarr[$termrun], explode(',', $salesreturnterm)[$termrun], $salesreturnno, $salesreturndate);
								$sqlbatchupd->execute();
								if($sqlbatchupd){
									$sqlbatchupd->close();
									$sqle = $con->prepare("UPDATE pairsalesreturnpayhistory SET amount=? WHERE salesreturnno=? AND salesreturndate=? AND type='salesreturn'");
									$sqle->bind_param("sss", $salereturnarr[$termrun], $salesreturnno, $salesreturndate);
									$sqle->execute();
									$sqle->close();
								}
								//FOR UPDATE THE PAYMENT DETAILS
							...
							$publicsqlsdel=$con->prepare("SELECT id FROM pairsalesreturnpayments WHERE receiptno=? AND receiptdate=? AND paymentmode=?");
							$publicsqlsdel->bind_param("sss", $salesreturnno, $salesreturndate, explode(',', $salesreturnterm)[$termrun]);
							...
							if($publicsqldel->num_rows > 0){
								...
								$publicsqldel->close();
								$publicsqlsdel->close();
								$sqls = $con->prepare("DELETE FROM pairsalesreturnpayments WHERE receiptno=? AND receiptdate=? AND paymentmode=?");
								$sqls->bind_param("sss", $salesreturnno, $salesreturndate, explode(',', $salesreturnterm)[$termrun]);
								$sqls->execute();
								$sqls->close();
								$sqlsant = $con->prepare("DELETE FROM pairsalesreturnpayhistory WHERE salesreturnno=? AND salesreturndate=? AND type='salesreturn' AND paymentid=?");
								$sqlsant->bind_param("sss", $salesreturnno, $salesreturndate, $paymentidfordelete);
								$sqlsant->execute();
								$sqlsant->close();
								// $sqlbatchupdant = $con->prepare("UPDATE pairsalesreturnpayments SET createdid=?, createdby=?, franchisesession=?, createdon=?,  term='RECEIPT', type='salesreturn', customername=?, customerid=?, receiptno=?, receiptdate=?, amount=?, paymentmode=?, notes='-' WHERE receiptno=? AND receiptdate=?");
								// $sqlbatchupdant->bind_param("ssssssssssss", $companymainid, $_SESSION["unqwerty"], $_SESSION["franchisesession"], $createdon, $customername, $customerid, $salesreturnno, $salesreturndate, $salereturnarr[$termrun], explode(',', $salesreturnterm)[$termrun], $salesreturnno, $salesreturndate);
								// $sqlbatchupdant->execute();
								// if($sqlbatchupdant){
								// 	$sqlbatchupdant->close();
								// 	$sqleant = $con->prepare("UPDATE pairsalesreturnpayhistory SET amount=? WHERE salesreturnno=? AND salesreturndate=? AND type='salesreturn'");
								// 	$sqleant->bind_param("sss", $salereturnarr[$termrun], $salesreturnno, $salesreturndate);
								// 	$sqleant->execute();
								// 	$sqleant->close();
								// }
								//FOR UPDATE THE PAYMENT DETAILS HISTORY
				...
				elseif($salesreturnterm=='CREDIT'){
					$sqls = $con->prepare("DELETE FROM pairsalesreturnpayments WHERE receiptno=? AND receiptdate=?");
					$sqls->bind_param("ss", $salesreturnno, $salesreturndate);
					$sqls->execute();
					$sqls->close();
					$sqlsant = $con->prepare("DELETE FROM pairsalesreturnpayhistory WHERE salesreturnno=? AND salesreturndate=? AND type='salesreturn'");
					$sqlsant->bind_param("ss", $salesreturnno, $salesreturndate);
					$sqlsant->execute();
					$sqlsant->close();
					$sqler = $con->prepare("UPDATE pairsalesreturns SET createdid=?, createdby=?, franchisesession=?, createdon=?, paidstatus='2' WHERE salesreturnno=? AND salesreturndate=? AND customerid=? AND franchisesession=? AND createdid=?");
	    			$sqler->bind_param("sssssssss", $companymainid, $_SESSION["unqwerty"], $_SESSION["franchisesession"], $createdon, $salesreturnno, $salesreturndate, $customerid, $_SESSION['franchisesession'], $companymainid);
					...
					$sqler->close();
					//FOR UPDATE THE PAYMENT STATUS UNPAID
					$sqldelledgerpay = $con->prepare("DELETE FROM pairledgers WHERE createdid=? AND franchisesession=? AND ledgerno=? AND ledgerdate=? AND type='salesreturn payment'");
					$sqldelledgerpay->bind_param("ssss", $companymainid, $_SESSION["franchisesession"], $salesreturnno, $salesreturndate);
				...
				if($sql){
					...
					while($infosalesreturn=$sqlisalesreturn->fetch_array()){
						...
						$paidamount=0;
						$sqlsalespays=$con->prepare("SELECT amount FROM pairsalesreturnpayhistory WHERE franchisesession=? AND createdid=? AND salesreturnno=? AND salesreturndate=? AND customerid=? ORDER BY id DESC");
					...
					$cussqlup->bind_param("sssss", $salesreturnamount, $balanceamount, $currentamount, $overdueamount, $customerid);
					$cussqlup->execute();
					$cussqlup->close();
					if(isset($_POST['submit1'])){
						...
						echo '<script> window.location.href="salesreturns.php?remarks=Updated Successfully";</script>'; 
					}
					else{
						$sqlusefinal = $con->prepare("INSERT INTO pairusehistory (usetype,createdon,createdby,useid,uniqueid,useremarks) VALUES ('SALESRETURNS', ?, ?, ?, ?, '<span style=\"color:green;\" id=\"prohisfromtospan\">Updated Successfully</span>')");
			    		$sqlusefinal->bind_param("ssss", $times, $_SESSION["unqwerty"], $salesreturnno, $salesid);
			    		$sqlusefinal->execute();
			    		$sqlusefinal->close();
						echo '<script> window.location.href="salesreturnview.php?id='.$salesid.'&salesreturnno='.$salesreturnno.'&salesreturndate='.$salesreturndate.'&remarks=Updated Successfully";</script>';
					}
				}
			...
			if ($sql) {
				$oldessalesreturnno = htmlspecialchars( $_POST['oldessalesreturnno'], ENT_QUOTES, 'UTF-8');
				$oldessalesreturndate = htmlspecialchars( $_POST['oldessalesreturndate'], ENT_QUOTES, 'UTF-8');
				...
				$oldessalesreturnterm = htmlspecialchars( $_POST['oldessalesreturnterm'], ENT_QUOTES, 'UTF-8');
				...
				if($salesreturnno!=$oldessalesreturnno){
					if($salereturninfoch!=''){
						$salereturninfoch.='<br> '.$infomainaccessuser['modulename'].' Number <span style="color:green;" id="prohisfromtospan">( From '.$oldessalesreturnno.' To '.$salesreturnno.' ) </span>';
					}
					else{
						$salereturninfoch.='<span style="color:royalblue;">'.$infomainaccessuser['modulename'].' Information</span> <br> '.$infomainaccessuser['modulename'].' Number <span style="color:green;" id="prohisfromtospan">( From '.$oldessalesreturnno.' To '.$salesreturnno.' ) </span>';
					}                   
				}
				if($salesreturndate!=$oldessalesreturndate){
					if($salereturninfoch!=''){
						$salereturninfoch.='<br> '.$infomainaccessuser['modulename'].' Date <span style="color:green;" id="prohisfromtospan">( From '.date($date,strtotime($oldessalesreturndate)).' To '.date($date,strtotime($salesreturndate)).' ) </span>';
					}
					else{
						$salereturninfoch.='<span style="color:royalblue;">'.$infomainaccessuser['modulename'].' Information</span> <br> '.$infomainaccessuser['modulename'].' Date <span style="color:green;" id="prohisfromtospan">( From '.date($date,strtotime($oldessalesreturndate)).' To '.date($date,strtotime($salesreturndate)).' ) </span>';
					}                   
				}
				if($reference!=$oldesreference){
					if($salereturninfoch!=''){
						$salereturninfoch.='<br> Reference <span style="color:green;" id="prohisfromtospan">( From '.$oldesreference.' To '.$reference.' ) </span>';
					}
					else{
						$salereturninfoch.='<span style="color:royalblue;">'.$infomainaccessuser['modulename'].' Information</span> <br> Reference <span style="color:green;" id="prohisfromtospan">( From '.$oldesreference.' To '.$reference.' ) </span>';
					}                   
				}
				if($saleperson!=$oldessaleperson){
					if($salereturninfoch!=''){
						$salereturninfoch.='<br> Sale Person <span style="color:green;" id="prohisfromtospan">( From '.$oldessaleperson.' To '.$saleperson.' ) </span>';
					}
					else{
						$salereturninfoch.='<span style="color:royalblue;">'.$infomainaccessuser['modulename'].' Information</span> <br> Sale Person <span style="color:green;" id="prohisfromtospan">( From '.$oldessaleperson.' To '.$saleperson.' ) </span>';
					}                   
				}
				if($preparedby!=$oldespreparedby){
					if($salereturninfoch!=''){
						$salereturninfoch.='<br> Prepared By <span style="color:green;" id="prohisfromtospan">( From '.$oldespreparedby.' To '.$preparedby.' ) </span>';
					}
					else{
						$salereturninfoch.='<span style="color:royalblue;">'.$infomainaccessuser['modulename'].' Information</span> <br> Prepared By <span style="color:green;" id="prohisfromtospan">( From '.$oldespreparedby.' To '.$preparedby.' ) </span>';
					}                   
				}
				if($checkedby!=$oldescheckedby){
					if($salereturninfoch!=''){
						$salereturninfoch.='<br> Checked By <span style="color:green;" id="prohisfromtospan">( From '.$oldescheckedby.' To '.$checkedby.' ) </span>';
					}
					else{
						$salereturninfoch.='<span style="color:royalblue;">'.$infomainaccessuser['modulename'].' Information</span> <br> Checked By <span style="color:green;" id="prohisfromtospan">( From '.$oldescheckedby.' To '.$checkedby.' ) </span>';
					}                   
				}
				...
				if($customername!=$oldescustomername){
					if($salereturncustinfoch!=''){
						$salereturncustinfoch.='<br> '.$infomainaccessusercus['modulename'].' Name <span style="color:green;" id="prohisfromtospan">( From '.$oldescustomername.' To '.$customername.' ) </span>';
					}
					else{
						$salereturncustinfoch.=''.(($salereturninfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessusercus['modulename'].' Information</span> <br> '.$infomainaccessusercus['modulename'].' Name <span style="color:green;" id="prohisfromtospan">( From '.$oldescustomername.' To '.$customername.' ) </span>';
					}                   
				}
				if($twoage!=$oldesage){
					if($salereturncustinfoch!=''){
						$salereturncustinfoch.='<br> AGE <span style="color:green;" id="prohisfromtospan">( From '.$oldesage.' To '.$twoage.' ) </span>';
					}
					else{
						$salereturncustinfoch.=''.(($salereturninfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessusercus['modulename'].' Information</span> <br> AGE <span style="color:green;" id="prohisfromtospan">( From '.$oldesage.' To '.$twoage.' ) </span>';
					}                   
				}
				if($twosex!=$oldessex){
					if($salereturncustinfoch!=''){
						$salereturncustinfoch.='<br> SEX <span style="color:green;" id="prohisfromtospan">( From '.$oldessex.' To '.$twosex.' ) </span>';
					}
					else{
						$salereturncustinfoch.=''.(($salereturninfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessusercus['modulename'].' Information</span> <br> SEX <span style="color:green;" id="prohisfromtospan">( From '.$oldessex.' To '.$twosex.' ) </span>';
					}                   
				}
				if($area!=$oldesarea){
					if($salereturncustinfoch!=''){
						$salereturncustinfoch.='<br> BILLING STREET <span style="color:green;" id="prohisfromtospan">( From '.$oldesarea.' To '.$area.' ) </span>';
					}
					else{
						$salereturncustinfoch.=''.(($salereturninfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessusercus['modulename'].' Information</span> <br> BILLING STREET <span style="color:green;" id="prohisfromtospan">( From '.$oldesarea.' To '.$area.' ) </span>';
					}                   
				}
				if($city!=$oldescity){
					if($salereturncustinfoch!=''){
						$salereturncustinfoch.='<br> BILLING CITY/TOWN <span style="color:green;" id="prohisfromtospan">( From '.$oldescity.' To '.$city.' ) </span>';
					}
					else{
						$salereturncustinfoch.=''.(($salereturninfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessusercus['modulename'].' Information</span> <br> BILLING CITY/TOWN <span style="color:green;" id="prohisfromtospan">( From '.$oldescity.' To '.$city.' ) </span>';
					}                   
				}
				if($state!=$oldesstate){
					if($salereturncustinfoch!=''){
						$salereturncustinfoch.='<br> BILLING STATE <span style="color:green;" id="prohisfromtospan">( From '.$oldesstate.' To '.$state.' ) </span>';
					}
					else{
						$salereturncustinfoch.=''.(($salereturninfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessusercus['modulename'].' Information</span> <br> BILLING STATE <span style="color:green;" id="prohisfromtospan">( From '.$oldesstate.' To '.$state.' ) </span>';
					}                   
				}
				if($district!=$oldesdistrict){
					if($salereturncustinfoch!=''){
						$salereturncustinfoch.='<br> BILLING PIN <span style="color:green;" id="prohisfromtospan">( From '.$oldesdistrict.' To '.$district.' ) </span>';
					}
					else{
						$salereturncustinfoch.=''.(($salereturninfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessusercus['modulename'].' Information</span> <br> BILLING PIN <span style="color:green;" id="prohisfromtospan">( From '.$oldesdistrict.' To '.$district.' ) </span>';
					}                   
				}
				if($pincode!=$oldespincode){
					if($salereturncustinfoch!=''){
						$salereturncustinfoch.='<br> BILLING COUNTRY/REGION <span style="color:green;" id="prohisfromtospan">( From '.$oldespincode.' To '.$pincode.' ) </span>';
					}
					else{
						$salereturncustinfoch.=''.(($salereturninfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessusercus['modulename'].' Information</span> <br> BILLING COUNTRY/REGION <span style="color:green;" id="prohisfromtospan">( From '.$oldespincode.' To '.$pincode.' ) </span>';
					}                   
				}
				if($sarea!=$oldessarea){
					if($salereturncustinfoch!=''){
						$salereturncustinfoch.='<br> SHIPPING STREET <span style="color:green;" id="prohisfromtospan">( From '.$oldessarea.' To '.$sarea.' ) </span>';
					}
					else{
						$salereturncustinfoch.=''.(($salereturninfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessusercus['modulename'].' Information</span> <br> SHIPPING STREET <span style="color:green;" id="prohisfromtospan">( From '.$oldessarea.' To '.$sarea.' ) </span>';
					}                   
				}
				if($scity!=$oldesscity){
					if($salereturncustinfoch!=''){
						$salereturncustinfoch.='<br> SHIPPING CITY/TOWN <span style="color:green;" id="prohisfromtospan">( From '.$oldesscity.' To '.$scity.' ) </span>';
					}
					else{
						$salereturncustinfoch.=''.(($salereturninfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessusercus['modulename'].' Information</span> <br> SHIPPING CITY/TOWN <span style="color:green;" id="prohisfromtospan">( From '.$oldesscity.' To '.$scity.' ) </span>';
					}                   
				}
				if($sstate!=$oldessstate){
					if($salereturncustinfoch!=''){
						$salereturncustinfoch.='<br> SHIPPING STATE <span style="color:green;" id="prohisfromtospan">( From '.$oldessstate.' To '.$sstate.' ) </span>';
					}
					else{
						$salereturncustinfoch.=''.(($salereturninfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessusercus['modulename'].' Information</span> <br> SHIPPING STATE <span style="color:green;" id="prohisfromtospan">( From '.$oldessstate.' To '.$sstate.' ) </span>';
					}                   
				}
				if($sdistrict!=$oldessdistrict){
					if($salereturncustinfoch!=''){
						$salereturncustinfoch.='<br> SHIPPING PIN <span style="color:green;" id="prohisfromtospan">( From '.$oldessdistrict.' To '.$sdistrict.' ) </span>';
					}
					else{
						$salereturncustinfoch.=''.(($salereturninfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessusercus['modulename'].' Information</span> <br> SHIPPING PIN <span style="color:green;" id="prohisfromtospan">( From '.$oldessdistrict.' To '.$sdistrict.' ) </span>';
					}                   
				}
				if($spincode!=$oldesspincode){
					if($salereturncustinfoch!=''){
						$salereturncustinfoch.='<br> SHIPPING COUNTRY/REGION <span style="color:green;" id="prohisfromtospan">( From '.$oldesspincode.' To '.$spincode.' ) </span>';
					}
					else{
						$salereturncustinfoch.=''.(($salereturninfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessusercus['modulename'].' Information</span> <br> SHIPPING COUNTRY/REGION <span style="color:green;" id="prohisfromtospan">( From '.$oldesspincode.' To '.$spincode.' ) </span>';
					}                   
				}
				if($twoworkphone!=$oldestwoworkphone){
					if($salereturncustinfoch!=''){
						$salereturncustinfoch.='<br> WORK PHONE <span style="color:green;" id="prohisfromtospan">( From '.$oldestwoworkphone.' To '.$twoworkphone.' ) </span>';
					}
					else{
						$salereturncustinfoch.=''.(($salereturninfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessusercus['modulename'].' Information</span> <br> WORK PHONE <span style="color:green;" id="prohisfromtospan">( From '.$oldestwoworkphone.' To '.$twoworkphone.' ) </span>';
					}                   
				}
				if($twomobilephone!=$oldestwomobilephone){
					if($salereturncustinfoch!=''){
						$salereturncustinfoch.='<br> MOBILE PHONE <span style="color:green;" id="prohisfromtospan">( From '.$oldestwomobilephone.' To '.$twomobilephone.' ) </span>';
					}
					else{
						$salereturncustinfoch.=''.(($salereturninfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessusercus['modulename'].' Information</span> <br> MOBILE PHONE <span style="color:green;" id="prohisfromtospan">( From '.$oldestwomobilephone.' To '.$twomobilephone.' ) </span>';
					}                   
				}
				if($gstrtype!=$oldesgstrtype){
					if($salereturncustinfoch!=''){
						$salereturncustinfoch.='<br> GST TREATMENT <span style="color:green;" id="prohisfromtospan">( From '.$oldesgstrtype.' To '.$gstrtype.' ) </span>';
					}
					else{
						$salereturncustinfoch.=''.(($salereturninfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessusercus['modulename'].' Information</span> <br> GST TREATMENT <span style="color:green;" id="prohisfromtospan">( From '.$oldesgstrtype.' To '.$gstrtype.' ) </span>';
					}                   
				}
				if($gstno!=$oldesgstno){
					if($salereturncustinfoch!=''){
						$salereturncustinfoch.='<br> GSTIN <span style="color:green;" id="prohisfromtospan">( From '.$oldesgstno.' To '.$gstno.' ) </span>';
					}
					else{
						$salereturncustinfoch.=''.(($salereturninfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessusercus['modulename'].' Information</span> <br> GSTIN <span style="color:green;" id="prohisfromtospan">( From '.$oldesgstno.' To '.$gstno.' ) </span>';
					}                   
				}
				if($pos!=$oldespos){
					if($salereturncustinfoch!=''){
						$salereturncustinfoch.='<br> PLACE OF SUPPLY <span style="color:green;" id="prohisfromtospan">( From '.$oldespos.' To '.$pos.' ) </span>';
					}
					else{
						$salereturncustinfoch.=''.(($salereturninfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessusercus['modulename'].' Information</span> <br> PLACE OF SUPPLY <span style="color:green;" id="prohisfromtospan">( From '.$oldespos.' To '.$pos.' ) </span>';
					}                   
				}
				if($dlno20!=$oldesdlno20){
					if($salereturncustinfoch!=''){
						$salereturncustinfoch.='<br> DL No 20 <span style="color:green;" id="prohisfromtospan">( From '.$oldesdlno20.' To '.$dlno20.' ) </span>';
					}
					else{
						$salereturncustinfoch.=''.(($salereturninfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessusercus['modulename'].' Information</span> <br> DL No 20 <span style="color:green;" id="prohisfromtospan">( From '.$oldesdlno20.' To '.$dlno20.' ) </span>';
					}                   
				}
				if($dlno21!=$oldesdlno21){
					if($salereturncustinfoch!=''){
						$salereturncustinfoch.='<br> DL No 21 <span style="color:green;" id="prohisfromtospan">( From '.$oldesdlno21.' To '.$dlno21.' ) </span>';
					}
					else{
						$salereturncustinfoch.=''.(($salereturninfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessusercus['modulename'].' Information</span> <br> DL No 21 <span style="color:green;" id="prohisfromtospan">( From '.$oldesdlno21.' To '.$dlno21.' ) </span>';
					}                   
				}
						...
						if($productname!=''){
							if($productname!=$oldesproductname){
								if($salereturniteminfoch!=''){
									$salereturniteminfoch.='<br> '.$infomainaccessuserpro['modulename'].' Name <span style="color:green;" id="prohisfromtospan">( From '.$oldesproductname.' To '.$productname.' ) </span>';
								}
								else{
									$salereturniteminfoch.=''.(($salereturninfoch!=''||$salereturncustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> '.$infomainaccessuserpro['modulename'].' Name <span style="color:green;" id="prohisfromtospan">( From '.$oldesproductname.' To '.$productname.' ) </span>';
								}                   
							}
							if($manufacturer!=$oldesmanufacturer){
								if($salereturniteminfoch!=''){
									$salereturniteminfoch.='<br> '.$access['txtnamecategory'].' <span style="color:green;" id="prohisfromtospan">( From '.$oldesmanufacturer.' To '.$manufacturer.' ) </span>';
								}
								else{
									$salereturniteminfoch.=''.(($salereturninfoch!=''||$salereturncustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> '.$access['txtnamecategory'].' <span style="color:green;" id="prohisfromtospan">( From '.$oldesmanufacturer.' To '.$manufacturer.' ) </span>';
								}                   
							}
							if($producthsn!=$oldesproducthsn){
								if($salereturniteminfoch!=''){
									$salereturniteminfoch.='<br> HSN Code <span style="color:green;" id="prohisfromtospan">( From '.$oldesproducthsn.' To '.$producthsn.' ) </span>';
								}
								else{
									$salereturniteminfoch.=''.(($salereturninfoch!=''||$salereturncustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> HSN Code <span style="color:green;" id="prohisfromtospan">( From '.$oldesproducthsn.' To '.$producthsn.' ) </span>';
								}                   
							}
							if($productdescription!=$oldesproductdescription){
								if($salereturniteminfoch!=''){
									$salereturniteminfoch.='<br> Description <span style="color:green;" id="prohisfromtospan">( From '.$oldesproductdescription.' To '.$productdescription.' ) </span>';
								}
								else{
									$salereturniteminfoch.=''.(($salereturninfoch!=''||$salereturncustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Description <span style="color:green;" id="prohisfromtospan">( From '.$oldesproductdescription.' To '.$productdescription.' ) </span>';
								}                   
							}
							if($batch!=$oldesbatch){
								if($salereturniteminfoch!=''){
									$salereturniteminfoch.='<br> Batch <span style="color:green;" id="prohisfromtospan">( From '.$oldesbatch.' To '.$batch.' ) </span>';
								}
								else{
									$salereturniteminfoch.=''.(($salereturninfoch!=''||$salereturncustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Batch <span style="color:green;" id="prohisfromtospan">( From '.$oldesbatch.' To '.$batch.' ) </span>';
								}                   
							}
							if($expdate!=$oldesexpdate){
								if($salereturniteminfoch!=''){
									$salereturniteminfoch.='<br> Expiry <span style="color:green;" id="prohisfromtospan">( From '.date($date,strtotime($oldesexpdate)).' To '.date($date,strtotime($expdate)).' ) </span>';
								}
								else{
									$salereturniteminfoch.=''.(($salereturninfoch!=''||$salereturncustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Expiry <span style="color:green;" id="prohisfromtospan">( From '.date($date,strtotime($oldesexpdate)).' To '.date($date,strtotime($expdate)).' ) </span>';
								}                   
							}
							if($productrate!=$oldesproductrate){
								if($salereturniteminfoch!=''){
									$salereturniteminfoch.='<br> Rate <span style="color:green;" id="prohisfromtospan">( From '.$oldesproductrate.' To '.$productrate.' ) </span>';
								}
								else{
									$salereturniteminfoch.=''.(($salereturninfoch!=''||$salereturncustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Rate <span style="color:green;" id="prohisfromtospan">( From '.$oldesproductrate.' To '.$productrate.' ) </span>';
								}                   
							}
							if($mrp!=$oldesmrp){
								if($salereturniteminfoch!=''){
									$salereturniteminfoch.='<br> Mrp <span style="color:green;" id="prohisfromtospan">( From '.$oldesmrp.' To '.$mrp.' ) </span>';
								}
								else{
									$salereturniteminfoch.=''.(($salereturninfoch!=''||$salereturncustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Mrp <span style="color:green;" id="prohisfromtospan">( From '.$oldesmrp.' To '.$mrp.' ) </span>';
								}                   
							}
							if($quantity!=$oldesquantity){
								if($salereturniteminfoch!=''){
									$salereturniteminfoch.='<br> Quantity <span style="color:green;" id="prohisfromtospan">( From '.$oldesquantity.' To '.$quantity.' ) </span>';
								}
								else{
									$salereturniteminfoch.=''.(($salereturninfoch!=''||$salereturncustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Quantity <span style="color:green;" id="prohisfromtospan">( From '.$oldesquantity.' To '.$quantity.' ) </span>';
								}                   
							}
							if($unit!=$oldesunit){
								if($salereturniteminfoch!=''){
									$salereturniteminfoch.='<br> Unit <span style="color:green;" id="prohisfromtospan">( From '.$oldesunit.' To '.$unit.' ) </span>';
								}
								else{
									$salereturniteminfoch.=''.(($salereturninfoch!=''||$salereturncustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Unit <span style="color:green;" id="prohisfromtospan">( From '.$oldesunit.' To '.$unit.' ) </span>';
								}                   
							}
							if($noofpacks!=$oldesnoofpacks){
								if($salereturniteminfoch!=''){
									$salereturniteminfoch.='<br> Pack <span style="color:green;" id="prohisfromtospan">( From '.$oldesnoofpacks.' To '.$noofpacks.' ) </span>';
								}
								else{
									$salereturniteminfoch.=''.(($salereturninfoch!=''||$salereturncustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Pack <span style="color:green;" id="prohisfromtospan">( From '.$oldesnoofpacks.' To '.$noofpacks.' ) </span>';
								}                   
							}
							if($productvalue!=$oldesproductvalue){
								if($salereturniteminfoch!=''){
									$salereturniteminfoch.='<br> Taxable Value <span style="color:green;" id="prohisfromtospan">( From '.$oldesproductvalue.' To '.$productvalue.' ) </span>';
								}
								else{
									$salereturniteminfoch.=''.(($salereturninfoch!=''||$salereturncustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Taxable Value <span style="color:green;" id="prohisfromtospan">( From '.$oldesproductvalue.' To '.$productvalue.' ) </span>';
								}                   
							}
							if($prodiscount!=$oldesprodiscount){
								if($salereturniteminfoch!=''){
									$salereturniteminfoch.='<br> '.$access['txtprodissalereturn'].' <span style="color:green;" id="prohisfromtospan">( From '.$oldesprodiscount.' To '.$prodiscount.' ) </span>';
								}
								else{
									$salereturniteminfoch.=''.(($salereturninfoch!=''||$salereturncustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> '.$access['txtprodissalereturn'].' <span style="color:green;" id="prohisfromtospan">( From '.$oldesprodiscount.' To '.$prodiscount.' ) </span>';
								}                   
							}
							if($prodiscounttype!=$oldesprodiscounttype){
								if($salereturniteminfoch!=''){
									$salereturniteminfoch.='<br> Discounted By <span style="color:green;" id="prohisfromtospan">( From '.(($oldesprodiscounttype==0)?'%':'?').' To '.(($prodiscounttype==0)?'%':'?').' ) </span>';
								}
								else{
									$salereturniteminfoch.=''.(($salereturninfoch!=''||$salereturncustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Discounted By <span style="color:green;" id="prohisfromtospan">( From '.(($oldesprodiscounttype==0)?'%':'?').' To '.(($prodiscounttype==0)?'%':'?').' ) </span>';
								}                   
							}
							if($taxvalue!=$oldestaxvalue){
								if($salereturniteminfoch!=''){
									$salereturniteminfoch.='<br> Tax Value <span style="color:green;" id="prohisfromtospan">( From '.$oldestaxvalue.' To '.$taxvalue.' ) </span>';
								}
								else{
									$salereturniteminfoch.=''.(($salereturninfoch!=''||$salereturncustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Tax Value <span style="color:green;" id="prohisfromtospan">( From '.$oldestaxvalue.' To '.$taxvalue.' ) </span>';
								}                   
							}
							if($vat!=$oldesvat){
								if($salereturniteminfoch!=''){
									$salereturniteminfoch.='<br> Gst <span style="color:green;" id="prohisfromtospan">( From '.$oldesvat.' % ('.$oldesicsgsthis.') To '.$vat.' % ('.$ansforsepgstval.') ) </span>';
								}
								else{
									$salereturniteminfoch.=''.(($salereturninfoch!=''||$salereturncustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Gst <span style="color:green;" id="prohisfromtospan">( From '.$oldesvat.' % ('.$oldesicsgsthis.') To '.$vat.' % ('.$ansforsepgstval.') ) </span>';
								}                   
							}
							if($productnetvalue!=$oldesproductnetvalue){
								if($salereturniteminfoch!=''){
									$salereturniteminfoch.='<br> Amount <span style="color:green;" id="prohisfromtospan">( From '.$oldesproductnetvalue.' To '.$productnetvalue.' ) </span>';
								}
								else{
									$salereturniteminfoch.=''.(($salereturninfoch!=''||$salereturncustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Amount <span style="color:green;" id="prohisfromtospan">( From '.$oldesproductnetvalue.' To '.$productnetvalue.' ) </span>';
								}                   
							}
						}
					}
					...
					for($i=0; $i<count($_POST['oldesproductname']); $i++){
						...
						if($productname!=''){
							...
							if($oldesproductname!=''){                    
								if($oldesproductname!=''){
									if($salereturniteminfoch!=''){
										$salereturniteminfoch.='<br> '.$infomainaccessuserpro['modulename'].' Name <span style="color:red;" id="prohisfromtospan">( From '.$oldesproductname.' To &nbsp; ) </span>';
									}
									else{
										$salereturniteminfoch.=''.(($salereturninfoch!=''||$salereturncustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> '.$infomainaccessuserpro['modulename'].' Name <span style="color:red;" id="prohisfromtospan">( From '.$oldesproductname.' To &nbsp; ) </span>';
									}
								}
								if($oldesmanufacturer!=''){
									if($salereturniteminfoch!=''){
										$salereturniteminfoch.='<br> '.$access['txtnamecategory'].' <span style="color:red;" id="prohisfromtospan">( From '.$oldesmanufacturer.' To &nbsp; ) </span>';
									}
									else{
										$salereturniteminfoch.=''.(($salereturninfoch!=''||$salereturncustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> '.$access['txtnamecategory'].' <span style="color:red;" id="prohisfromtospan">( From '.$oldesmanufacturer.' To &nbsp; ) </span>';
									}
								}
								if($oldesproducthsn!=''){
									if($salereturniteminfoch!=''){
										$salereturniteminfoch.='<br> HSN Code <span style="color:red;" id="prohisfromtospan">( From '.$oldesproducthsn.' To &nbsp; ) </span>';
									}
									else{
										$salereturniteminfoch.=''.(($salereturninfoch!=''||$salereturncustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> HSN Code <span style="color:red;" id="prohisfromtospan">( From '.$oldesproducthsn.' To &nbsp; ) </span>';
									}
								}
								if($oldesproductdescription!=''){
									if($salereturniteminfoch!=''){
										$salereturniteminfoch.='<br> Description <span style="color:red;" id="prohisfromtospan">( From '.$oldesproductdescription.' To &nbsp; ) </span>';
									}
									else{
										$salereturniteminfoch.=''.(($salereturninfoch!=''||$salereturncustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Description <span style="color:red;" id="prohisfromtospan">( From '.$oldesproductdescription.' To &nbsp; ) </span>';
									}
								}
								if($oldesbatch!=''){
									if($salereturniteminfoch!=''){
										$salereturniteminfoch.='<br> Batch <span style="color:red;" id="prohisfromtospan">( From '.$oldesbatch.' To &nbsp; ) </span>';
									}
									else{
										$salereturniteminfoch.=''.(($salereturninfoch!=''||$salereturncustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Batch <span style="color:red;" id="prohisfromtospan">( From '.$oldesbatch.' To &nbsp; ) </span>';
									}
								}
								if($oldesexpdate!=''){
									if($salereturniteminfoch!=''){
										$salereturniteminfoch.='<br> Expiry <span style="color:red;" id="prohisfromtospan">( From '.date($date,strtotime($oldesexpdate)).' To &nbsp; ) </span>';
									}
									else{
										$salereturniteminfoch.=''.(($salereturninfoch!=''||$salereturncustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Expiry <span style="color:red;" id="prohisfromtospan">( From '.date($date,strtotime($oldesexpdate)).' To &nbsp; ) </span>';
									}
								}
								if($oldesproductrate!=''){
									if($salereturniteminfoch!=''){
										$salereturniteminfoch.='<br> Rate <span style="color:red;" id="prohisfromtospan">( From '.$oldesproductrate.' To &nbsp; ) </span>';
									}
									else{
										$salereturniteminfoch.=''.(($salereturninfoch!=''||$salereturncustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Rate <span style="color:red;" id="prohisfromtospan">( From '.$oldesproductrate.' To &nbsp; ) </span>';
									}
								}
								if($oldesmrp!=''){
									if($salereturniteminfoch!=''){
										$salereturniteminfoch.='<br> Mrp <span style="color:red;" id="prohisfromtospan">( From '.$oldesmrp.' To &nbsp; ) </span>';
									}
									else{
										$salereturniteminfoch.=''.(($salereturninfoch!=''||$salereturncustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Mrp <span style="color:red;" id="prohisfromtospan">( From '.$oldesmrp.' To &nbsp; ) </span>';
									}
								}
								if($oldesquantity!=''){
									if($salereturniteminfoch!=''){
										$salereturniteminfoch.='<br> Quantity <span style="color:red;" id="prohisfromtospan">( From '.$oldesquantity.' To &nbsp; ) </span>';
									}
									else{
										$salereturniteminfoch.=''.(($salereturninfoch!=''||$salereturncustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Quantity <span style="color:red;" id="prohisfromtospan">( From '.$oldesquantity.' To &nbsp; ) </span>';
									}
								}
								if($oldesunit!=''){
									if($salereturniteminfoch!=''){
										$salereturniteminfoch.='<br> Unit <span style="color:red;" id="prohisfromtospan">( From '.$oldesunit.' To &nbsp; ) </span>';
									}
									else{
										$salereturniteminfoch.=''.(($salereturninfoch!=''||$salereturncustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Unit <span style="color:red;" id="prohisfromtospan">( From '.$oldesunit.' To &nbsp; ) </span>';
									}
								}
								if($oldesnoofpacks!=''){
									if($salereturniteminfoch!=''){
										$salereturniteminfoch.='<br> Pack <span style="color:red;" id="prohisfromtospan">( From '.$oldesnoofpacks.' To &nbsp; ) </span>';
									}
									else{
										$salereturniteminfoch.=''.(($salereturninfoch!=''||$salereturncustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Pack <span style="color:red;" id="prohisfromtospan">( From '.$oldesnoofpacks.' To &nbsp; ) </span>';
									}
								}
								if($oldesproductvalue!=''){
									if($salereturniteminfoch!=''){
										$salereturniteminfoch.='<br> Taxable Value <span style="color:red;" id="prohisfromtospan">( From '.$oldesproductvalue.' To &nbsp; ) </span>';
									}
									else{
										$salereturniteminfoch.=''.(($salereturninfoch!=''||$salereturncustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Taxable Value <span style="color:red;" id="prohisfromtospan">( From '.$oldesproductvalue.' To &nbsp; ) </span>';
									}
								}
								if($oldesprodiscount!=''){
									if($salereturniteminfoch!=''){
										$salereturniteminfoch.='<br> Discount <span style="color:red;" id="prohisfromtospan">( From '.$oldesprodiscount.' To &nbsp; ) </span>';
									}
									else{
										$salereturniteminfoch.=''.(($salereturninfoch!=''||$salereturncustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Discount <span style="color:red;" id="prohisfromtospan">( From '.$oldesprodiscount.' To &nbsp; ) </span>';
									}
								}
								if($oldestaxvalue!=''){
									if($salereturniteminfoch!=''){
										$salereturniteminfoch.='<br> Tax Value <span style="color:red;" id="prohisfromtospan">( From '.$oldestaxvalue.' To &nbsp; ) </span>';
									}
									else{
										$salereturniteminfoch.=''.(($salereturninfoch!=''||$salereturncustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Tax Value <span style="color:red;" id="prohisfromtospan">( From '.$oldestaxvalue.' To &nbsp; ) </span>';
									}
								}
								if($oldesvat!=''){
									if($salereturniteminfoch!=''){
										$salereturniteminfoch.='<br> Gst <span style="color:red;" id="prohisfromtospan">( From '.$oldesvat.' % ('.$oldesicsgsthis.') To &nbsp; ) </span>';
									}
									else{
										$salereturniteminfoch.=''.(($salereturninfoch!=''||$salereturncustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Gst <span style="color:red;" id="prohisfromtospan">( From '.$oldesvat.' % ('.$oldesicsgsthis.') To &nbsp; ) </span>';
									}
								}
								if($oldesproductnetvalue!=''){
									if($salereturniteminfoch!=''){
										$salereturniteminfoch.='<br> Amount <span style="color:red;" id="prohisfromtospan">( From '.$oldesproductnetvalue.' To &nbsp; ) </span>';
									}
									else{
										$salereturniteminfoch.=''.(($salereturninfoch!=''||$salereturncustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Amount <span style="color:red;" id="prohisfromtospan">( From '.$oldesproductnetvalue.' To &nbsp; ) </span>';
									}
								}
							}
						}
				...
				elseif ($curans>$oldans) {
					for($i=0; $i<count($_POST['productname']); $i++){
						...
						if($oldesproductname!=''){
							...
							if($expdate!=''){
								if($salereturniteminfoch!=''){
									$salereturniteminfoch.='<br> Expiry <span style="color:green;" id="prohisfromtospan">( '.date($date,strtotime($expdate)).' ) </span>';
								}
								else{
									$salereturniteminfoch.=''.(($salereturninfoch!=''||$salereturncustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Expiry <span style="color:green;" id="prohisfromtospan">( '.date($date,strtotime($expdate)).' ) </span>';        
								}
							}
							if($productrate!=''){
								if($salereturniteminfoch!=''){
				...
				if($totalitems!=$oldestotalitems){
					if($salereturniteminfoch!=''){
						$salereturniteminfoch.='<br> Total Items <span style="color:green;" id="prohisfromtospan">( From '.$oldestotalitems.' To '.$totalitems.' ) </span>';
					}
					else{
						$salereturniteminfoch.=''.(($salereturninfoch!=''||$salereturncustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Total Items <span style="color:green;" id="prohisfromtospan">( From '.$oldestotalitems.' To '.$totalitems.' ) </span>';
					}
				}
				if($totalquantity!=$oldestotalquantity){
					if($salereturniteminfoch!=''){
						$salereturniteminfoch.='<br> Total Qty <span style="color:green;" id="prohisfromtospan">( From '.$oldestotalquantity.' To '.$totalquantity.' ) </span>';
					}
					else{
						$salereturniteminfoch.=''.(($salereturninfoch!=''||$salereturncustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Total Qty <span style="color:green;" id="prohisfromtospan">( From '.$oldestotalquantity.' To '.$totalquantity.' ) </span>';
					}
				}
				if($totalamount!=$oldestotalamount){
					if($salereturniteminfoch!=''){
						$salereturniteminfoch.='<br> Sub Total <span style="color:green;" id="prohisfromtospan">( From '.$oldestotalamount.' To '.$totalamount.' ) </span>';
					}
					else{
						$salereturniteminfoch.=''.(($salereturninfoch!=''||$salereturncustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Sub Total <span style="color:green;" id="prohisfromtospan">( From '.$oldestotalamount.' To '.$totalamount.' ) </span>';
					}
				}
				if($discountamount!=$oldesdiscountamount){
					if($salereturniteminfoch!=''){
						$salereturniteminfoch.='<br> Discount <span style="color:green;" id="prohisfromtospan">( From '.$oldesdiscountamount.' To '.$discountamount.' ) </span>';
					}
					else{
						$salereturniteminfoch.=''.(($salereturninfoch!=''||$salereturncustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Discount <span style="color:green;" id="prohisfromtospan">( From '.$oldesdiscountamount.' To '.$discountamount.' ) </span>';
					}
				}
				if($totalvatamount!=$oldestotalvatamount){
					if($salereturniteminfoch!=''){
						$salereturniteminfoch.='<br> Total Tax <span style="color:green;" id="prohisfromtospan">( From '.$oldestotalvatamount.' To '.$totalvatamount.' ) </span>';
					}
					else{
						$salereturniteminfoch.=''.(($salereturninfoch!=''||$salereturncustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Total Tax <span style="color:green;" id="prohisfromtospan">( From '.$oldestotalvatamount.' To '.$totalvatamount.' ) </span>';
					}
				}
				if($roundoff!=$oldesroundoff){
					if($salereturniteminfoch!=''){
						$salereturniteminfoch.='<br> Round Off <span style="color:green;" id="prohisfromtospan">( From '.$oldesroundoff.' To '.$roundoff.' ) </span>';
					}
					else{
						$salereturniteminfoch.=''.(($salereturninfoch!=''||$salereturncustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Round Off <span style="color:green;" id="prohisfromtospan">( From '.$oldesroundoff.' To '.$roundoff.' ) </span>';
					}
				}
				if($grandtotal!=$oldesgrandtotal){
					if($salereturniteminfoch!=''){
						$salereturniteminfoch.='<br> Grand Total <span style="color:green;" id="prohisfromtospan">( From '.$oldesgrandtotal.' To '.$grandtotal.' ) </span>';
					}
					else{
						$salereturniteminfoch.=''.(($salereturninfoch!=''||$salereturncustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Grand Total <span style="color:green;" id="prohisfromtospan">( From '.$oldesgrandtotal.' To '.$grandtotal.' ) </span>';
					}
				}
				if($description!=$oldesdescription){
					if($salereturniteminfoch!=''){
						$salereturniteminfoch.='<br> Description <span style="color:green;" id="prohisfromtospan">( From '.$oldesdescription.' To '.$description.' ) </span>';
					}
					else{
						$salereturniteminfoch.=''.(($salereturninfoch!=''||$salereturncustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Description <span style="color:green;" id="prohisfromtospan">( From '.$oldesdescription.' To '.$description.' ) </span>';
					}
				}
				if($notes!=$oldesnotes){
					if($salereturniteminfoch!=''){
						$salereturniteminfoch.='<br> Notes <span style="color:green;" id="prohisfromtospan">( From '.$oldesnotes.' To '.$notes.' ) </span>';
					}
					else{
						$salereturniteminfoch.=''.(($salereturninfoch!=''||$salereturncustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Notes <span style="color:green;" id="prohisfromtospan">( From '.$oldesnotes.' To '.$notes.' ) </span>';
					}
				}
				if($terms!=$oldesterms){
					if($salereturniteminfoch!=''){
						$salereturniteminfoch.='<br> Terms And Conditions <span style="color:green;" id="prohisfromtospan">( From '.$oldesterms.' To '.$terms.' ) </span>';
					}
					else{
						$salereturniteminfoch.=''.(($salereturninfoch!=''||$salereturncustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Terms And Conditions <span style="color:green;" id="prohisfromtospan">( From '.$oldesterms.' To '.$terms.' ) </span>';
					}
				}
				if($fileattach!=$oldesfileattach){
					if($salereturniteminfoch!=''){
						$salereturniteminfoch.='<br> Attach <span style="color:green;" id="prohisfromtospan">( Changed ) </span>';
					}
					else{
						$salereturniteminfoch.=''.(($salereturninfoch!=''||$salereturncustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Attach <span style="color:green;" id="prohisfromtospan">( Changed ) </span>';
					}
				}
				if($salesreturnterm!=$oldessalesreturnterm){
					if($salereturniteminfoch!=''){
						$salereturniteminfoch.='<br> Payment Term <span style="color:green;" id="prohisfromtospan">( From '.$oldessalesreturnterm.' To '.$salesreturnterm.' ) </span>';
					}
					else{
						$salereturniteminfoch.=''.(($salereturninfoch!=''||$salereturncustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Payment Term <span style="color:green;" id="prohisfromtospan">( From '.$oldessalesreturnterm.' To '.$salesreturnterm.' ) </span>';
					}
				}
				if($validpaidamount!=$oldesvalidpaidamount&&$salesreturnterm=='CASH,CARD,GPAY'){
					if($salereturniteminfoch!=''){
						$salereturniteminfoch.='<br> Paid Amount <span style="color:green;" id="prohisfromtospan">( From '.$oldesvalidpaidamount.' To '.$validpaidamount.' ) </span>';
					}
					else{
						$salereturniteminfoch.=''.(($salereturninfoch!=''||$salereturncustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Paid Amount <span style="color:green;" id="prohisfromtospan">( From '.$oldesvalidpaidamount.' To '.$validpaidamount.' ) </span>';
					}
				}
				if($validpaidamount!=$oldesvalidpaidamount&&$salesreturnterm=='CASH'){
					if($salereturniteminfoch!=''){
						$salereturniteminfoch.='<br> Paid Amount <span style="color:green;" id="prohisfromtospan">( From '.$oldesvalidpaidamount.' To '.$validpaidamount.' ) </span>';
					}
					else{
				...
				if($validbalance!=$oldesvalidbalance&&$salesreturnterm=='CASH'){
					if($salereturniteminfoch!=''){
						$salereturniteminfoch.='<br> Balance Due <span style="color:green;" id="prohisfromtospan">( From '.$oldesvalidbalance.' To '.$validbalance.' ) </span>';
					}
					else{
						$salereturniteminfoch.=''.(($salereturninfoch!=''||$salereturncustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Balance Due <span style="color:green;" id="prohisfromtospan">( From '.$oldesvalidbalance.' To '.$validbalance.' ) </span>';
					}
				}
				if($validbalance!=$oldesvalidbalance&&$salesreturnterm=='CASH,CARD,GPAY'){
					if($salereturniteminfoch!=''){
						$salereturniteminfoch.='<br> Balance Due <span style="color:green;" id="prohisfromtospan">( From '.$oldesvalidbalance.' To '.$validbalance.' ) </span>';
					}
					else{
				...
				if($duedates!=$oldesduedates&&$salesreturnterm=='CREDIT'){
					if($salereturniteminfoch!=''){
						$salereturniteminfoch.='<br> Due Term <span style="color:green;" id="prohisfromtospan">( From '.$oldesduedates.' To '.$duedates.' ) </span>';
					}
					else{
						$salereturniteminfoch.=''.(($salereturninfoch!=''||$salereturncustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Due Term <span style="color:green;" id="prohisfromtospan">( From '.$oldesduedates.' To '.$duedates.' ) </span>';
					}
				}
				if($duedate!=$oldesduedate&&$salesreturnterm=='CREDIT'){
					if($salereturniteminfoch!=''){
						$salereturniteminfoch.='<br> Due Date <span style="color:green;" id="prohisfromtospan">( From '.date($date,strtotime($oldesduedate)).' To '.date($date,strtotime($duedate)).' ) </span>';
					}
					else{
						$salereturniteminfoch.=''.(($salereturninfoch!=''||$salereturncustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Due Date <span style="color:green;" id="prohisfromtospan">( From '.date($date,strtotime($oldesduedate)).' To '.date($date,strtotime($duedate)).' ) </span>';
					}
				}
				//FOR SALESRETURN OTHER INFORMATIONS LIKE PAYMENT AND TERMS AND DESCRIPTION......
				$salereturntotinfoch = $salereturninfoch.$salereturncustinfoch.$salereturniteminfoch;
				...
				$sqlusehisupdqry->bind_param("sss", $salesid, $salesreturnno, $_GET['id']);
...
<main class="main-content position-relative max-height-vh-100 h-100 mt-0 " style="overflow-y: scroll !important;">
	...
	<?php
		...
		if((isset($_GET['salesreturnno']))&&(isset($_GET['salesreturndate']))){
			$salesreturnno=htmlspecialchars( $_GET['salesreturnno'], ENT_QUOTES, 'UTF-8');
			$salesreturndate=htmlspecialchars( $_GET['salesreturndate'], ENT_QUOTES, 'UTF-8');
			$sql=$con->prepare("SELECT * FROM pairsalesreturns WHERE franchisesession=? AND createdid=? AND salesreturnno=? AND salesreturndate=? ORDER BY id ASC LIMIT 5");
			$sql->bind_param("ssss", $_SESSION['franchisesession'], $companymainid, $salesreturnno, $salesreturndate);
			$sql->execute();
			if ($sql->errno) {
				...
				<div class="col-12">
					<div class="card mb-4 mt-5">
						...
						?>
							...
							<!-- CUSTOMER ADD MODAL -->
							<form autocomplete="off" id="salesreturnform" action="" onsubmit="return checkvalidate()" method="post" enctype="multipart/form-data" class="form-horizontal" role="form">
							...
							<?php
								$oldarray=array( 'salesreturnno','salesreturndate','reference','saleperson','preparedby','checkedby','customername','age','sex','area','city','state','district','pincode','sarea','scity','sstate','sdistrict','spincode','workphone','mobile','gstrtype','gstno','pos','dlno20','dlno21' );
							...
							?>
								...
								<input type="hidden" value="<?=$rows[0]['salesreturnterm']?>" name="oldessalesreturnterm">
								...
								<input type="hidden" name="oldsalesreturnno" value="<?=$rows[0]['salesreturnno']?>">
								<input type="hidden" name="oldsalesreturndate" value="<?=$rows[0]['salesreturndate']?>">
								<input type="hidden" name="salesreturnterm" value="<?=$rows[0]['salesreturnterm']?>">
								<div class="row">
								<?php
									// if ((in_array('Sales Return Information', $fieldedit))) {
								?>
									<div class="col-lg-4">
														...
														<div class="row">
															<div class="col-lg-12">
																<div class="form-group row" style="align-items: center !important;">
																	<div class="col-lg-5">
																		<label for="salesreturnno" class="custom-label">
																	...
																	<div class="col-lg-7">
																		<input type="text" class="form-control  form-control-sm" id="salesreturnno" name="salesreturnno" required  value="<?=$rows[0]['salesreturnno']?>" readonly>
															...
															<div class="col-lg-12">
																<div class="form-group row" style="align-items: center !important;">
																	<div class="col-lg-5">
																		<label for="salesreturndate" class="custom-label">
																			<span class="text-danger">
																	...
																	<?php
																		...
																		$dateformats->close();
																	?>
																		<input type="date" class="form-control  form-control-sm" id="salesreturndate" name="salesreturndate" required value="<?=$rows[0]['salesreturndate']?>">
																	</div>
																</div>
															</div>
															<div class="col-lg-12" <?=((in_array('Sales Return Time', $fieldedit))?'':'style="display:none;"')?>>
																<div class="form-group row" style="align-items: center !important;">
																	<div class="col-lg-5">
																		<label for="salesreturntime" class="custom-label">
																			<span class="text-danger">
																	...
																	<div class="col-lg-7">
																		...
																		?>
																		<input type="time" class="form-control form-control-sm" id="salesreturntime" name="salesreturntime" required value="<?=$rows[0]['salesreturntime']?>">
																	</div>
																</div>
															</div>
														<?php
											...
											<div class="accordion-item mb-1">
														...
														<div class="row" style="width:100% !important;">
															...
															?>
																...
																$(document).ready(function () {
																	...
																	<?php
																		if ($access['salereturnbtwocnamerequired']=='Yes') {
																	?>
																		$("#twocustomername").removeAttr("required");
																	<?php
																		}
																		if ($access['salereturnbtwocwphonerequired']=='Yes') {
																	?>
												...
												<div id="collapseOne" class="accordion-collapse collapse show" aria-labelledby="headingOne" style="padding-left:7px !important;padding-right: 7px !important;">
													...
													<?php
														...
														if ($infomainaccessuser['salereturncustinfo']=='one'||(($infomainaccessuser['salereturncustinfo']=='defone')||($infomainaccessuser['salereturncustinfo']=='deftwo'))) {
													?>
														<div id="one">
															<div class="row">
																<div class="col-lg-12">
													...
													<?php
														...
														if ($infomainaccessuser['salereturncustinfo']=='two'||(($infomainaccessuser['salereturncustinfo']=='defone')||($infomainaccessuser['salereturncustinfo']=='deftwo'))) {
													?>
														<script type="text/javascript">
														function twoposans() {
														...
														<div id="two">
															<div class="row">
																<div class="<?=((!in_array('Age', $fieldedit))&&(!in_array('Sex', $fieldedit)))?'col-lg-3':'col-lg-2'?>" style="padding-right: 0px;">
																	<label for="customername" class="custom-label">
																		<?= ($access['salereturnbtwocnamerequired']=='Yes')? '<span class="text-danger">'. $infomainaccessusercus['modulename'] . ' Name * </span>': $infomainaccessusercus['modulename'] . ' Name ' ?>
																	</label>
																</div>
																<div class="<?=((in_array('Age', $fieldedit))&&(in_array('Sex', $fieldedit)))?'col-lg-4':(((in_array('Age', $fieldedit))||(in_array('Sex', $fieldedit)))?'col-lg-7':'col-lg-9')?>">
																	<input type="text" name="twocustomername" id="twocustomername" class="form-control form-control-sm" placeholder="<?= $infomainaccessusercus['modulename'] ?> Name" value="<?= $rows[0]['customername'] ?>" <?= ($access['salereturnbtwocnamerequired']=='Yes')? 'required': ' ' ?>>
																		...
																		<div class="row" style="padding:0px !important;margin: 0px !important;">
																			<div class="col-lg-6" <?=(in_array('Work Phone', $fieldedit))?'':'style="display:none;"'?>>
																				<label for="twoworkphone" class="custom-label" style="margin:0px !important;font-size: 12px !important;color: #777777;">
																					<?= ($access['salereturnbtwocwphonerequired']=='Yes')? '<span class="text-danger"> WORK PHONE * </span>': ' WORK PHONE ' ?>
																				</label>
																				<input type="text" class="form-control form-control-sm" id="twoworkphone" name="twoworkphone" placeholder="Work Phone" value="<?=$rows[0]['workphone']?>" <?= ($access['salereturnbtwocwphonerequired']=='Yes')? 'required': ' ' ?>>
													...
													<div class="table-responsive" style="overflow-y: hidden;">
														<table class="table table-bordered" id="purchasetable">
															<thead>
																...
																?>
																	<th width="13%">
																		BATCH <?=(($access['salereturnbatexprequired']=='Yes')?'<span class="text-danger"> *</span>':'')?>
																	</th>
																<?php
																	}
																?>
																...
																?>
																	<th style="width: 87px !important;text-align: right !important;">
																		<?=($access['txtqtysalereturn'])?><span class="text-danger"> *</span>
																	</th>
																<?php
																	}
																?>
															...
															?>
																...
																<tr>
																	...
																	<td data-label="RATE" <?=(in_array('Rate', $fieldedit))?'':'style="display:none !important;"'?>>
																		...
																		<div>
																			<div class="dvi salereturnbillgets" id="salereturnbillgets<?=$i?>" style="margin-top:-22px;display: none;width: 250px;border-radius: 0px !important;">
																				<table width="100%">
																					<tr style="border-bottom: 1px solid #cccccc;margin-bottom: 0px;">
																						<td align="center" style="border-right: 1px solid #cccccc;width: 50% !important;display: inline-block !important;text-align: center;">
																							<span onclick="salereturngetfun(<?=$i?>,0)" id="salereturnonoff<?=$i?>">
																						...
																						<input type="text" style="display: none;" value="salereturn" id="billorsalereturn<?=$i?>">
																					</tr>
																				</table>
																			</div>
																	...
																	<td data-label="<?=($access['txtqtysalereturn'])?>" <?=(in_array('Quantity', $fieldedit))?'':'style="display:none !important;"'?>>
																		<div>
																	...
																	<td data-label="<?=$access['txttaxablesalereturn']?>" <?=(in_array('Taxable Value', $fieldedit))?'':'style="display:none;"'?>>
																		<div>
																			<span style="font-size:15px !important;">
																				<?php echo $resmaincurrencyans; ?>
																			</span>
																		...
																		<div style="<?=(in_array('Profit Margin', $fieldedit))?'':'display:none;'?>">
																			<span data-bs-toggle="modal" data-bs-target="#ViewMarginDetails" style="color: green;font-size:13px;" id="margintotal<?=$i?>" onclick="getmargindetails(<?=$i?>)">
																				Profit Margin :
																				<span id="totalmargsalereturnalue<?=$i?>">
															...
															?>
																...
																setTimeout(function() {
																	...
																	function getoldmargins(lineidno) {
																		$("#totalmargsalereturnalue"+lineidno).html($("#oldmargintotalvalue"+lineidno).val());
																	}
																},9000);
																setTimeout(function() {
																	...
																	// 	}
																	// ?>
																	// 	$.get("ratesearch.php", {term: productid,custid: customerid,ratedef: salereturnratedef,differ: 'salereturn'} , function(datas){
																	...
																	// 	var productid= $('#product'+lineNo).val();
																	// <?php
																	// 	if ($access['salereturnbatchdef']=='avail') {
																	// ?>
												...
												<div class="table-responsive mt-1" id="gsttablediv" <?=(in_array('Tax Informations', $fieldedit))?'':'style="display:none;"'?>>
													<table class="table table-bordered" id="gsttable" style="font-size:12px;">
														...
														<?php
															...
															$sqlitaxes=$con->prepare("SELECT * FROM pairsalesreturns WHERE franchisesession=? AND createdid=? AND salesreturnno=? AND salesreturndate=? ORDER BY id ASC");
															$sqlitaxes->bind_param("ssss", $_SESSION["franchisesession"], $companymainid, $salesreturnno, $salesreturndate);
															$sqlitaxes->execute();
												...
												<div class="col" style="width:34px !important;margin-top: 1px !important;margin-left: -9px !important;">
													<div class="form-group row">
														<div class="col-7 col-sm-6">
															...
															<a class="btn btn-primary btn-sm btn-custom-grey" href="salesreturns.php" style="margin-top:-3px !important;">
																Cancel
															</a>
														</div>
										...
										<div class="modal-body" style="height: max-content;">
											...
											<table class="table notforfinaloutputsave showhideload">
												<tr>
													...
													<td>
														<input type="text" name="validsalesreturnno" id="validsalesreturnno" class="form-control form-control-sm finalsubmitrequired" readonly required>
													</td>
												</tr>
												<tr>
													<th style="text-align:left !important">
													...
													<td>
														<input type="date" name="validsalesreturndate" id="validsalesreturndate" class="form-control form-control-sm finalsubmitrequired" readonly required>
													</td>
												</tr>
												<tr>
													<th style="text-align:left !important">
													...
													<td>
														<input type="number" name="validsalesreturnamount" id="validsalesreturnamount" class="form-control form-control-sm finalsubmitrequired" readonly required>
													</td>
												</tr>
												...
												<tr style="<?=(($access['defsalereturnpay']=='split')?'display: none;':'')?>">
													...
													<td>
														<div class="col-lg-12">
															...
															?>
																<option value="<?= $info['term'] ?>" <?=($info['term']==$rows[0]['salesreturnterm'])?'selected':''?>>
																	<?= $info['term'] ?>
																</option>
															<?php
																	}
												...
												<tr id="validpaidblock">
													...
													<td>
														<input type="number" name="validpaidamount" id="validpaidamount" class="form-control form-control-sm finalsubmitrequired" placeholder="0.00" required onchange="paidamountcalc()" value="<?= $rows[0]['validpaidamount'] ?>" readonly>
														<script>
												...
												<tr id="duedateblock">
													...
													<td>
														...
														function funduedates(){
															...
															var salesreturndate=$("#salesreturndate").val();
															var today="";
															if(duedates!=''){
...
function rateget(id){
	...
	?>
		if (document.getElementById("billorsalereturn"+id).value=='salereturn') {
			var salereturnorbills = 'salereturn';
		}
		else{
...
$.ajax({
type: "GET",
url: 'editfetch.php?term=' + sIndexEdit + '&limitings=5&salesreturnno=<?=$_GET['salesreturnno']?>&salesreturndate=<?=$_GET['salesreturndate']?>&type=salesreturn&i='+ivalue+'',
success: function (result) {
