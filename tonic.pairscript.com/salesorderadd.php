...
$saleordermodtype = 'Sales Orders';
$sqlismainaccessfields->bind_param("is", $companymainid,$saleordermodtype);
...
$sqlismainaccessusers->bind_param("is", $userid,$saleordermodtype);
...
if(isset($_POST['grandtotal'])){
    ...
    if (($infomainaccessuser['saleordercustinfo']=='one')||($scriptonetwo=='one')) {
        ...
        $gstno = htmlspecialchars($_POST['gstno'], ENT_QUOTES, 'UTF-8');
    ...
    $sqlismainaccessusers->bind_param("is", $franchsess,$saleordermodtype);
    $sqlismainaccessusers->execute();
    $sqlismainaccessuser = $sqlismainaccessusers->get_result();
    $infomainaccessuser=$sqlismainaccessuser->fetch_assoc();
    $sqlismainaccessuser->close();
    ...
    $salesorderno = htmlspecialchars($infomainaccessuser['moduleprefix'].(str_pad($infomainaccessuser['modulesuffix']+1, 0, "0", STR_PAD_LEFT)), ENT_QUOTES, 'UTF-8');
    ...
    if ((in_array('Table Number', $fieldadd))) {
    ...
    if (($infomainaccessuser['saleordercustinfo']=='one')||($scriptonetwo=='one')) {
        ...
        $custsqlup->bind_param("ssssssssssssssssssissss", $times, $companymainid, $sesunqwerty, $franchsess, $area, $city, $state, $pincode, $district, $sarea, $scity, $sstate, $spincode, $sdistrict, $gstrtype, $custgstin, $twoworkphone, $twomobilephone, $custid, $companymainid, $cusmodtype, $franchsess, $pubcvisiblity);
        $custsqlup->execute();
        $custsqlup->close();
    ...
    $sesunqwerty = $_SESSION['unqwerty'];
    $franchsess = $_SESSION["franchisesession"];
    $sql3 = $con->prepare("UPDATE pairmainaccess SET modulesuffix=modulesuffix + 1 WHERE franchiseid=? AND moduletype='Sales Orders'");
    ...
    $sql2->bind_param("iiss", $franchsess, $companymainid, $salesorderno, $salesorderdate);
    $sql2->execute();
    $sql2->store_result();
    ...
    if($sql2->num_rows==0){
        ...
        if(($salesorderterm=='CASH')||($salesorderterm=='CASH,CARD,GPAY')){
            ...
            //         $sqler = $con->prepare("UPDATE pairsalesorders SET createdid=?, createdby=?, franchisesession=?, createdon=?, paidstatus='1' WHERE salesorderno=? AND salesorderdate=? AND customerid=? AND franchisesession=? AND createdid=?");
            //         $sqler->bind_param("sssssssss", $companymainid, $_SESSION["unqwerty"], $_SESSION["franchisesession"], $createdon, $salesorderno, $salesorderdate, $customerid, $_SESSION['franchisesession'], $companymainid);
            //         $sqler->execute();
            //         $sqler->close();
            //         //FOR UPDATE THE PAYMENT STATUS PAID
            //     }
            //     else{
            //         $sqler = $con->prepare("UPDATE pairsalesorders SET createdid=?, createdby=?, franchisesession=?, createdon=?, paidstatus='2' WHERE salesorderno=? AND salesorderdate=? AND customerid=? AND franchisesession=? AND createdid=?");
            //         $sqler->bind_param("sssssssss", $companymainid, $_SESSION["unqwerty"], $_SESSION["franchisesession"], $createdon, $salesorderno, $salesorderdate, $customerid, $_SESSION['franchisesession'], $companymainid);
            //         $sqler->execute();
            //         $sqler->close();
        ...
        if($salesorderno!=''){
            if($saleorderinfoch!=''){
                $saleorderinfoch.='<br> '.$infomainaccessuser['modulename'].' Number <span style="color:green;" id="prohisfromtospan">( '.$salesorderno.' ) </span>';
            }
            else{
                $saleorderinfoch.='<span style="color:royalblue;">'.$infomainaccessuser['modulename'].' Information</span> <br> '.$infomainaccessuser['modulename'].' Number <span style="color:green;" id="prohisfromtospan">( '.$salesorderno.' ) </span>';
            }                   
        }
        if($salesorderdate!=''){
            if($saleorderinfoch!=''){
                $saleorderinfoch.='<br> '.$infomainaccessuser['modulename'].' Date <span style="color:green;" id="prohisfromtospan">( '.date($date,strtotime($salesorderdate)).' ) </span>';
            }
            else{
                $saleorderinfoch.='<span style="color:royalblue;">'.$infomainaccessuser['modulename'].' Information</span> <br> '.$infomainaccessuser['modulename'].' Date <span style="color:green;" id="prohisfromtospan">( '.date($date,strtotime($salesorderdate)).' ) </span>';
            }                   
        }
        if($reference!=''){
            if($saleorderinfoch!=''){
                $saleorderinfoch.='<br> Reference <span style="color:green;" id="prohisfromtospan">( '.$reference.' ) </span>';
            }
            else{
                $saleorderinfoch.='<span style="color:royalblue;">'.$infomainaccessuser['modulename'].' Information</span> <br> Reference <span style="color:green;" id="prohisfromtospan">( '.$reference.' ) </span>';
            }                   
        }
        if($tablenumber!=''){
            if($saleorderinfoch!=''){
                $saleorderinfoch.='<br> '.$access['txttablenumber'].' <span style="color:green;" id="prohisfromtospan">( '.$tablenumber.' ) </span>';
            }
            else{
                $saleorderinfoch.='<span style="color:royalblue;">'.$infomainaccessuser['modulename'].' Information</span> <br> '.$access['txttablenumber'].' <span style="color:green;" id="prohisfromtospan">( '.$tablenumber.' ) </span>';
            }                   
        }
        if($saleperson!=''){
            if($saleorderinfoch!=''){
                $saleorderinfoch.='<br> '.$access['txtsaleperson'].' <span style="color:green;" id="prohisfromtospan">( '.$saleperson.' ) </span>';
            }
            else{
                $saleorderinfoch.='<span style="color:royalblue;">'.$infomainaccessuser['modulename'].' Information</span> <br> '.$access['txtsaleperson'].' <span style="color:green;" id="prohisfromtospan">( '.$saleperson.' ) </span>';
            }                   
        }
        if($preparedby!=''){
            if($saleorderinfoch!=''){
                $saleorderinfoch.='<br> Prepared By <span style="color:green;" id="prohisfromtospan">( '.$preparedby.' ) </span>';
            }
            else{
                $saleorderinfoch.='<span style="color:royalblue;">'.$infomainaccessuser['modulename'].' Information</span> <br> Prepared By <span style="color:green;" id="prohisfromtospan">( '.$preparedby.' ) </span>';
            }                   
        }
        if($checkedby!=''){
            if($saleorderinfoch!=''){
                $saleorderinfoch.='<br> Checked By <span style="color:green;" id="prohisfromtospan">( '.$checkedby.' ) </span>';
            }
            else{
                $saleorderinfoch.='<span style="color:royalblue;">'.$infomainaccessuser['modulename'].' Information</span> <br> Checked By <span style="color:green;" id="prohisfromtospan">( '.$checkedby.' ) </span>';
            }                   
        }
        ...
        if($customername!=''){
            if($saleordercustinfoch!=''){
                $saleordercustinfoch.='<br> '.$infomainaccessusercus['modulename'].' Name <span style="color:green;" id="prohisfromtospan">( '.$customername.' ) </span>';
            }
            else{
                $saleordercustinfoch.=''.(($saleorderinfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessusercus['modulename'].' Information</span> <br> '.$infomainaccessusercus['modulename'].' Name <span style="color:green;" id="prohisfromtospan">( '.$customername.' ) </span>';
            }                   
        }
        if($twoage!=''){
            if($saleordercustinfoch!=''){
                $saleordercustinfoch.='<br> AGE <span style="color:green;" id="prohisfromtospan">( '.$twoage.' ) </span>';
            }
            else{
                $saleordercustinfoch.=''.(($saleorderinfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessusercus['modulename'].' Information</span> <br> AGE <span style="color:green;" id="prohisfromtospan">( '.$twoage.' ) </span>';
            }                   
        }
        if($twosex!=''){
            if($saleordercustinfoch!=''){
                $saleordercustinfoch.='<br> SEX <span style="color:green;" id="prohisfromtospan">( '.$twosex.' ) </span>';
            }
            else{
                $saleordercustinfoch.=''.(($saleorderinfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessusercus['modulename'].' Information</span> <br> SEX <span style="color:green;" id="prohisfromtospan">( '.$twosex.' ) </span>';
            }                   
        }
        if($area!=''){
            if($saleordercustinfoch!=''){
                $saleordercustinfoch.='<br> BILLING STREET <span style="color:green;" id="prohisfromtospan">( '.$area.' ) </span>';
            }
            else{
                $saleordercustinfoch.=''.(($saleorderinfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessusercus['modulename'].' Information</span> <br> BILLING STREET <span style="color:green;" id="prohisfromtospan">( '.$area.' ) </span>';
            }                   
        }
        if($city!=''){
            if($saleordercustinfoch!=''){
                $saleordercustinfoch.='<br> BILLING CITY/TOWN <span style="color:green;" id="prohisfromtospan">( '.$city.' ) </span>';
            }
            else{
                $saleordercustinfoch.=''.(($saleorderinfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessusercus['modulename'].' Information</span> <br> BILLING CITY/TOWN <span style="color:green;" id="prohisfromtospan">( '.$city.' ) </span>';
            }                   
        }
        if($state!=''){
            if($saleordercustinfoch!=''){
                $saleordercustinfoch.='<br> BILLING STATE <span style="color:green;" id="prohisfromtospan">( '.$state.' ) </span>';
            }
            else{
                $saleordercustinfoch.=''.(($saleorderinfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessusercus['modulename'].' Information</span> <br> BILLING STATE <span style="color:green;" id="prohisfromtospan">( '.$state.' ) </span>';
            }                   
        }
        if($district!=''){
            if($saleordercustinfoch!=''){
                $saleordercustinfoch.='<br> BILLING PIN <span style="color:green;" id="prohisfromtospan">( '.$district.' ) </span>';
            }
            else{
                $saleordercustinfoch.=''.(($saleorderinfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessusercus['modulename'].' Information</span> <br> BILLING PIN <span style="color:green;" id="prohisfromtospan">( '.$district.' ) </span>';
            }                   
        }
        if($pincode!=''){
            if($saleordercustinfoch!=''){
                $saleordercustinfoch.='<br> BILLING COUNTRY/REGION <span style="color:green;" id="prohisfromtospan">( '.$pincode.' ) </span>';
            }
            else{
                $saleordercustinfoch.=''.(($saleorderinfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessusercus['modulename'].' Information</span> <br> BILLING COUNTRY/REGION <span style="color:green;" id="prohisfromtospan">( '.$pincode.' ) </span>';
            }                   
        }
        if($sarea!=''){
            if($saleordercustinfoch!=''){
                $saleordercustinfoch.='<br> SHIPPING STREET <span style="color:green;" id="prohisfromtospan">( '.$sarea.' ) </span>';
            }
            else{
                $saleordercustinfoch.=''.(($saleorderinfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessusercus['modulename'].' Information</span> <br> SHIPPING STREET <span style="color:green;" id="prohisfromtospan">( '.$sarea.' ) </span>';
            }                   
        }
        if($scity!=''){
            if($saleordercustinfoch!=''){
                $saleordercustinfoch.='<br> SHIPPING CITY/TOWN <span style="color:green;" id="prohisfromtospan">( '.$scity.' ) </span>';
            }
            else{
                $saleordercustinfoch.=''.(($saleorderinfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessusercus['modulename'].' Information</span> <br> SHIPPING CITY/TOWN <span style="color:green;" id="prohisfromtospan">( '.$scity.' ) </span>';
            }                   
        }
        if($sstate!=''){
            if($saleordercustinfoch!=''){
                $saleordercustinfoch.='<br> SHIPPING STATE <span style="color:green;" id="prohisfromtospan">( '.$sstate.' ) </span>';
            }
            else{
                $saleordercustinfoch.=''.(($saleorderinfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessusercus['modulename'].' Information</span> <br> SHIPPING STATE <span style="color:green;" id="prohisfromtospan">( '.$sstate.' ) </span>';
            }                   
        }
        if($sdistrict!=''){
            if($saleordercustinfoch!=''){
                $saleordercustinfoch.='<br> SHIPPING PIN <span style="color:green;" id="prohisfromtospan">( '.$sdistrict.' ) </span>';
            }
            else{
                $saleordercustinfoch.=''.(($saleorderinfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessusercus['modulename'].' Information</span> <br> SHIPPING PIN <span style="color:green;" id="prohisfromtospan">( '.$sdistrict.' ) </span>';
            }                   
        }
        if($spincode!=''){
            if($saleordercustinfoch!=''){
                $saleordercustinfoch.='<br> SHIPPING COUNTRY/REGION <span style="color:green;" id="prohisfromtospan">( '.$spincode.' ) </span>';
            }
            else{
                $saleordercustinfoch.=''.(($saleorderinfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessusercus['modulename'].' Information</span> <br> SHIPPING COUNTRY/REGION <span style="color:green;" id="prohisfromtospan">( '.$spincode.' ) </span>';
            }                   
        }
        if($twoworkphone!=''){
            if($saleordercustinfoch!=''){
                $saleordercustinfoch.='<br> WORK PHONE <span style="color:green;" id="prohisfromtospan">( '.$twoworkphone.' ) </span>';
            }
            else{
                $saleordercustinfoch.=''.(($saleorderinfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessusercus['modulename'].' Information</span> <br> WORK PHONE <span style="color:green;" id="prohisfromtospan">( '.$twoworkphone.' ) </span>';
            }                   
        }
        if($twomobilephone!=''){
            if($saleordercustinfoch!=''){
                $saleordercustinfoch.='<br> MOBILE PHONE <span style="color:green;" id="prohisfromtospan">( '.$twomobilephone.' ) </span>';
            }
            else{
                $saleordercustinfoch.=''.(($saleorderinfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessusercus['modulename'].' Information</span> <br> MOBILE PHONE <span style="color:green;" id="prohisfromtospan">( '.$twomobilephone.' ) </span>';
            }                   
        }
        if($gstrtype!=''){
            if($saleordercustinfoch!=''){
                $saleordercustinfoch.='<br> GST TREATMENT <span style="color:green;" id="prohisfromtospan">( '.$gstrtype.' ) </span>';
            }
            else{
                $saleordercustinfoch.=''.(($saleorderinfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessusercus['modulename'].' Information</span> <br> GST TREATMENT <span style="color:green;" id="prohisfromtospan">( '.$gstrtype.' ) </span>';
            }                   
        }
        if($gstno!=''){
            if($saleordercustinfoch!=''){
                $saleordercustinfoch.='<br> GSTIN <span style="color:green;" id="prohisfromtospan">( '.$gstno.' ) </span>';
            }
            else{
                $saleordercustinfoch.=''.(($saleorderinfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessusercus['modulename'].' Information</span> <br> GSTIN <span style="color:green;" id="prohisfromtospan">( '.$gstno.' ) </span>';
            }                   
        }
        if($pos!=''){
            if($saleordercustinfoch!=''){
                $saleordercustinfoch.='<br> PLACE OF SUPPLY <span style="color:green;" id="prohisfromtospan">( '.$pos.' ) </span>';
            }
            else{
                $saleordercustinfoch.=''.(($saleorderinfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessusercus['modulename'].' Information</span> <br> PLACE OF SUPPLY <span style="color:green;" id="prohisfromtospan">( '.$pos.' ) </span>';
            }                   
        }
        ...
        for($i=0; $i<count($_POST['productname']); $i++){
            ...
            if($productname!=''){
                ...
                if($expdate!=''){
                    if($saleorderiteminfoch!=''){
                        $saleorderiteminfoch.='<br> Expiry <span style="color:green;" id="prohisfromtospan">( '.$expdate.' ) </span>';
                    }
                    else{
                        $saleorderiteminfoch.=''.(($saleorderinfoch!=''||$saleordercustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Expiry <span style="color:green;" id="prohisfromtospan">( '.$expdate.' ) </span>';
                    }                   
                }
                if($productrate!=''){
                    if($saleorderiteminfoch!=''){
        ...
        if($totalitems!=''){
            if($saleorderiteminfoch!=''){
                $saleorderiteminfoch.='<br> Total Items <span style="color:green;" id="prohisfromtospan">( '.$totalitems.' ) </span>';
            }
            else{
                $saleorderiteminfoch.=''.(($saleorderinfoch!=''||$saleordercustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Total Items <span style="color:green;" id="prohisfromtospan">( '.$totalitems.' ) </span>';
            }                   
        }
        if($totalquantity!=''){
            if($saleorderiteminfoch!=''){
                $saleorderiteminfoch.='<br> Total Qty <span style="color:green;" id="prohisfromtospan">( '.$totalquantity.' ) </span>';
            }
            else{
                $saleorderiteminfoch.=''.(($saleorderinfoch!=''||$saleordercustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Total Qty <span style="color:green;" id="prohisfromtospan">( '.$totalquantity.' ) </span>';
            }                   
        }
        if($totalamount!=''){
            if($saleorderiteminfoch!=''){
                $saleorderiteminfoch.='<br> Sub Total <span style="color:green;" id="prohisfromtospan">( '.$totalamount.' ) </span>';
            }
            else{
                $saleorderiteminfoch.=''.(($saleorderinfoch!=''||$saleordercustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Sub Total <span style="color:green;" id="prohisfromtospan">( '.$totalamount.' ) </span>';
            }                   
        }
        if($discountamount!=''){
            if($saleorderiteminfoch!=''){
                $saleorderiteminfoch.='<br> Discount <span style="color:green;" id="prohisfromtospan">( '.$discountamount.' ) </span>';
            }
            else{
                $saleorderiteminfoch.=''.(($saleorderinfoch!=''||$saleordercustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Discount <span style="color:green;" id="prohisfromtospan">( '.$discountamount.' ) </span>';
            }                   
        }
        if($totalvatamount!=''){
            if($saleorderiteminfoch!=''){
                $saleorderiteminfoch.='<br> Total Tax <span style="color:green;" id="prohisfromtospan">( '.$totalvatamount.' ) </span>';
            }
            else{
                $saleorderiteminfoch.=''.(($saleorderinfoch!=''||$saleordercustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Total Tax <span style="color:green;" id="prohisfromtospan">( '.$totalvatamount.' ) </span>';
            }                   
        }
        if($roundoff!=''){
            if($saleorderiteminfoch!=''){
                $saleorderiteminfoch.='<br> Round Off <span style="color:green;" id="prohisfromtospan">( '.$roundoff.' ) </span>';
            }
            else{
                $saleorderiteminfoch.=''.(($saleorderinfoch!=''||$saleordercustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Round Off <span style="color:green;" id="prohisfromtospan">( '.$roundoff.' ) </span>';
            }                   
        }
        if($grandtotal!=''){
            if($saleorderiteminfoch!=''){
                $saleorderiteminfoch.='<br> Grand Total <span style="color:green;" id="prohisfromtospan">( '.$grandtotal.' ) </span>';
            }
            else{
                $saleorderiteminfoch.=''.(($saleorderinfoch!=''||$saleordercustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Grand Total <span style="color:green;" id="prohisfromtospan">( '.$grandtotal.' ) </span>';
            }                   
        }
        if($description!=''){
            if($saleorderiteminfoch!=''){
                $saleorderiteminfoch.='<br> Description <span style="color:green;" id="prohisfromtospan">( '.$description.' ) </span>';
            }
            else{
                $saleorderiteminfoch.=''.(($saleorderinfoch!=''||$saleordercustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Description <span style="color:green;" id="prohisfromtospan">( '.$description.' ) </span>';
            }                   
        }
        if($notes!=''){
            if($saleorderiteminfoch!=''){
                $saleorderiteminfoch.='<br> Notes <span style="color:green;" id="prohisfromtospan">( '.$notes.' ) </span>';
            }
            else{
                $saleorderiteminfoch.=''.(($saleorderinfoch!=''||$saleordercustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Notes <span style="color:green;" id="prohisfromtospan">( '.$notes.' ) </span>';
            }                   
        }
        if($terms!=''){
            if($saleorderiteminfoch!=''){
                $saleorderiteminfoch.='<br> Terms And Conditions <span style="color:green;" id="prohisfromtospan">( '.$terms.' ) </span>';
            }
            else{
                $saleorderiteminfoch.=''.(($saleorderinfoch!=''||$saleordercustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Terms And Conditions <span style="color:green;" id="prohisfromtospan">( '.$terms.' ) </span>';
            }                   
        }
        if($fileattach!=''){
            if($saleorderiteminfoch!=''){
                $saleorderiteminfoch.='<br> Attach <span style="color:green;" id="prohisfromtospan">( Added ) </span>';
            }
            else{
                $saleorderiteminfoch.=''.(($saleorderinfoch!=''||$saleordercustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Attach <span style="color:green;" id="prohisfromtospan">( Added ) </span>';
            }                   
        }
        if($salesorderterm!=''){
            if($saleorderiteminfoch!=''){
                $saleorderiteminfoch.='<br> Payment Method <span style="color:green;" id="prohisfromtospan">( '.$salesorderterm.' ) </span>';
            }
            else{
                $saleorderiteminfoch.=''.(($saleorderinfoch!=''||$saleordercustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Payment Method <span style="color:green;" id="prohisfromtospan">( '.$salesorderterm.' ) </span>';
            }                   
        }
        if($validpaidamount!=''&&$salesorderterm=='CASH'){
            if($saleorderiteminfoch!=''){
                $saleorderiteminfoch.='<br> '.(($access['defsaleorderpay']=='split')?'Amount Received':'Cash Received').' <span style="color:green;" id="prohisfromtospan">( '.$validpaidamount.' ) </span>';
            }
            else{
                $saleorderiteminfoch.=''.(($saleorderinfoch!=''||$saleordercustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> '.(($access['defsaleorderpay']=='split')?'Amount Received':'Cash Received').' <span style="color:green;" id="prohisfromtospan">( '.$validpaidamount.' ) </span>';
            }                   
        }
        if($validpaidamount!=''&&$salesorderterm=='CASH,CARD,GPAY'){
            if($saleorderiteminfoch!=''){
                $saleorderiteminfoch.='<br> '.(($access['defsaleorderpay']=='split')?'Amount Received':'Cash Received').' <span style="color:green;" id="prohisfromtospan">( '.$validpaidamount.' ) </span>';
            }
            else{
        ...
        if($validbalance!=''&&$salesorderterm=='CASH'){
            if($saleorderiteminfoch!=''){
                $saleorderiteminfoch.='<br> Change Due <span style="color:green;" id="prohisfromtospan">( '.$validbalance.' ) </span>';
            }
            else{
                $saleorderiteminfoch.=''.(($saleorderinfoch!=''||$saleordercustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Change Due <span style="color:green;" id="prohisfromtospan">( '.$validbalance.' ) </span>';
            }                   
        }
        if($validbalance!=''&&$salesorderterm=='CASH,CARD,GPAY'){
            if($saleorderiteminfoch!=''){
                $saleorderiteminfoch.='<br> Change Due <span style="color:green;" id="prohisfromtospan">( '.$validbalance.' ) </span>';
            }
            else{
        ...
        if($duedates!=''&&$salesorderterm=='CREDIT'){
            if($saleorderiteminfoch!=''){
                $saleorderiteminfoch.='<br> Due Term <span style="color:green;" id="prohisfromtospan">( '.$duedates.' ) </span>';
            }
            else{
                $saleorderiteminfoch.=''.(($saleorderinfoch!=''||$saleordercustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Due Term <span style="color:green;" id="prohisfromtospan">( '.$duedates.' ) </span>';
            }                   
        }
        if($duedate!=''&&$salesorderterm=='CREDIT'){
            if($saleorderiteminfoch!=''){
                $saleorderiteminfoch.='<br> Due Date <span style="color:green;" id="prohisfromtospan">( '.date($date,strtotime($duedate)).' ) </span>';
            }
            else{
                $saleorderiteminfoch.=''.(($saleorderinfoch!=''||$saleordercustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Due Date <span style="color:green;" id="prohisfromtospan">( '.date($date,strtotime($duedate)).' ) </span>';
            }                   
        }
        //FOR SALE ORDER OTHER INFORMATIONS LIKE PAYMENT AND TERMS AND DESCRIPTION......
        $saleordertotinfoch = "SALE ORDER CREATED<br>".$saleorderinfoch.$saleordercustinfoch.$saleorderiteminfoch;
        ...
        //     $sqlsalespays->close();
        // }
        // $sqlisalesorder->close();
        // $sqlisalesorders->close();
        // $cussqlup = $con->prepare("UPDATE paircustomers SET invoiceamount=?, balanceamount=?, currentamount=?, overdueamount=? WHERE id=?");
        // $cussqlup->bind_param("ssssi", $salesorderamount, $balanceamount, $currentamount, $overdueamount, $customerid);
        // $cussqlup->execute();
        // $cussqlup->close();
        //FOR UPDATE THE CUSTOMERS BALANCE AND OTHER PAYMENT INFORMATIONS
        if(isset($_POST['submit1'])){
            echo '<script> window.open("salesorderprint.php?salesorderno='.$salesorderno.'&salesorderdate='.$salesorderdate.'", "_blank");</script>';
            echo '<script> window.location.href="salesorders.php?remarks=Added Successfully";</script>';
        }
        else{
            echo '<script> window.location.href="salesorderview.php?id='.$salesid.'&salesorderno='.$salesorderno.'&salesorderdate='.$salesorderdate.'&remarks=Added Successfully";</script>';
        }
    }
}
else{
    header("Location: salesorders.php?error=Error Data");
    //FOR IF THE SALE ORDER IS ALREADY EXISTS IN THIS NUMBER AND DATE AND FRANCHISE AND COMPANY
}
}
$sqlismainaccessusercuss=$con->prepare("SELECT * FROM pairmainaccess WHERE userid=? AND moduletype='Customers' ORDER BY id ASC");
$sqlismainaccessusercuss->bind_param("s", $userid);
...
<main class="main-content position-relative max-height-vh-100 h-100 mt-0 " style="overflow-y: scroll !important;">
    ...
    <div class="container-fluid py-4 bg-body">
        ...
        <?php
            ...
            $sqlismainaccessusers->close();
        ?>
        <!-- FOR SALE ORDER MODULE INFORMATIONS AND PREFERENCES -->
                    ...
                    <div class="card mb-4 mt-5">
                        <div class="card-body p-3" style="font-family:'Myriad Set Pro','Helvetica Neue',Helvetica,Arial,sans-serif;">
                            ...
                            <form autocomplete="off" id="salesorderform" action="" onsubmit="return checkvalidate()" method="post" enctype="multipart/form-data" class="form-horizontal" role="form">
                                ...
                                <div class="row">
                                    <?php
                                        // if ((in_array('Sales Order Information', $fieldadd))) {
                                    ?>
                                    <div class="col-lg-4">
                                                        ...
                                                        <div class="row">
                                                            <div class="col-lg-12">
                                                                <div class="form-group row" style="align-items: center !important;">
                                                                    ...
                                                                    <div class="col-lg-7">
                                                                        <input type="text" class="form-control form-control-sm" id="salesorderno" name="salesorderno" required value="<?=$infomainaccessuser['moduleprefix']?><?=str_pad(($infomainaccessuser['modulesuffix']+1), 0, "0", STR_PAD_LEFT)?>" readonly>
                                                                    </div>
                                                                </div>
                                                            </div>
                                                            <div class="col-lg-12">
                                                                <div class="form-group row" style="align-items: center !important;">
                                                                    ...
                                                                    <div class="col-lg-7">
                                                                        <?php
                                                                            ...
                                                                            $dt = new DateTime();
                                                                        ?>
                                                                        <input type="date" class="form-control form-control-sm" id="salesorderdate" name="salesorderdate" value="<?= $dt->format('Y-m-d') ?>" required>
                                                                    </div>
                                                                </div>
                                                            </div>
                                                            <div class="col-lg-12" <?=((in_array('Sales Order Time', $fieldadd))?'':'style="display:none;"')?>>
                                                                <div class="form-group row" style="align-items: center !important;">
                                                                    <div class="col-lg-5">
                                                                    ...
                                                                    <div class="col-lg-7">
                                                                        ...
                                                                        ?>
                                                                        <input type="time" class="form-control form-control-sm" value="<?php echo $current_time_def; ?>" id="salesordertime" name="salesordertime" required>
                                                                    </div>
                                                                </div>
                                                            </div>
                                                            <div class="col-lg-12" <?=((in_array('Table Number', $fieldadd))?'':'style="display:none;"')?>>
                                                                <div class="form-group row" style="align-items: center !important;">
                                                                    <div class="col-lg-5">
                                                                    ...
                                                                    <div class="col-lg-7">
                                                                        <select class="select2-field form-control form-control-sm" name="tablenumber" id="tablenumber">
                                                                            ...
                                                                            ?>
                                                                                <option <?=(((isset($_GET['tabledinfo']))&&($_GET['tabledinfo']!=''))?((((base64_decode($_GET['tabledinfo'])==$info['id'])?'selected':''))):'')?> value="<?= $info['tablenumber'].'|||'.$info['id'] ?>"><?= $info['tablenumber'] ?></option>
                                                                            <?php
                                                                                }
                                                                                $sqli->close();
                                                                                $sqlis->close();
                                                        ...
                                                        ?>
                                                            <div class="col-lg-12" <?=((in_array('Prepared By', $fieldadd))?'':'style="display:none;"')?>>
                                                                <div class="form-group row" style="align-items: center !important;">
                                                                    ...
                                                                    <div class="col-lg-7">
                                                                        <input type="text" class="form-control form-control-sm" id="preparedby" name="preparedby">
                                                                    </div>
                                                                </div>
                                                            </div>
                                                            <div class="col-lg-12" <?=((in_array('Checked By', $fieldadd))?'':'style="display:none;"')?>>
                                                                <div class="form-group row" style="align-items: center !important;">
                                                                    ...
                                                                    <div class="col-lg-7">
                                                                        <input type="text" class="form-control form-control-sm" id="checkedby" name="checkedby">
                                                                    </div>
                                                                </div>
                                                            </div>
                                                                ...
                                                                <div class="row">
                                                                    <div class="col-lg-2 col-md-2 col-3">
                                                                        <div class="custom-control custom-radio mr-sm-2">
                                                                            <input type="radio" class="custom-control-input" name="customerinfodefault" id="onecustomerinfo" value="one" <?= ($infomainaccessuser['saleordercustinfo']=='defone')?'checked':'' ?> onclick="onetwo()">
                                                                    ...
                                                                    <div class="col-lg-2 col-md-2 col-3">
                                                                        <div class="custom-control custom-radio mr-sm-2">
                                                                            <input type="radio" class="custom-control-input" name="customerinfodefault" id="twocustomerinfo" value="two" <?= ($infomainaccessuser['saleordercustinfo']=='deftwo')?'checked':'' ?> onclick="onetwo()">
                                                            ...
                                                            <div id="two">
                                                                <div class="row">
                                                                    ...
                                                                    <div class="<?=((in_array('Age', $fieldadd))&&(in_array('Sex', $fieldadd)))?'col-lg-4':(((in_array('Age', $fieldadd))||(in_array('Sex', $fieldadd)))?'col-lg-7':'col-lg-9')?>">
                                                                        <input type="text" name="twocustomername" id="twocustomername" class="form-control form-control-sm" placeholder="<?= $infomainaccessusercus['modulename'] ?> Name" <?= ($access['saleorderbtwocnamerequired']=='Yes')? 'required': ' ' ?>>
                                                                    </div>
                                                                        ...
                                                                        <div class="row mb-0" id="custaddressdiv" style="background-color:#fbfafa; color:#777777; display:block;margin: 0px !important;padding: 9px 9px 0px 9px !important;">
                                                                            ...
                                                                            <div class="row" style="padding:0px !important;margin: 0px !important;">
                                                                                <div class="col-lg-6" <?=(in_array('Work Phone', $fieldadd))?'':'style="display:none;"'?>>
                                                                                    ...
                                                                                    <input type="text" class="form-control form-control-sm" id="twoworkphone" name="twoworkphone" placeholder="Work Phone" <?= ($access['saleorderbtwocwphonerequired']=='Yes')? 'required': ' ' ?>>
                                                                                </div>
                                                                                ...
                                                                                <div class="col-lg-8" <?=(in_array('GSTIN', $fieldadd))?'':'style="display:none;"'?>>
                                                                                    <div class="row">
                                                                                        <div class="col-lg-6">
                                                                                            ...
                                                                                            <input type="text" class="form-control form-control-sm" id="twogsttreatment" name="twogsttreatment" placeholder="GST TREATMENT" value="<?=$infomainaccessuser['salesordertwogst']?>" readonly>
                                                                                        </div>
                                                                                    ...
                                                                                    <div class="row">
                                                                                        <div class="col-lg-12" id="twoposblock">
                                                                                            ...
                                                                                            <select name="twopos" id="twopos" class="select5 form-control form-control-sm" required onchange="twoposans()">
                                                                                                <option disabled value="" <?=($infomainaccessuser['saleordertwopos']=='manual'||'auto')?'selected':''?>>
                                                                                                    Select The Place
                                                                                                </option>
                                                                                                <option value="JAMMU AND KASHMIR (1)" <?=($infomainaccessuser['saleordertwopos']=="JAMMU AND KASHMIR (1)")?'selected':''?>>
                                                                                                    JAMMU AND KASHMIR (1)
                                                                                                </option>
                                                                                                <option value="ANDAMAN AND NICOBAR ISLANDS (35)" <?=($infomainaccessuser['saleordertwopos']=="ANDAMAN AND NICOBAR ISLANDS (35)")?'selected':''?>>
                                                                                                    ANDAMAN AND NICOBAR ISLANDS (35)
                                                                                                </option>
                                                                                                <option value="ANDHRA PRADESH (NEWLY ADDED) (37)" <?=($infomainaccessuser['saleordertwopos']=="ANDHRA PRADESH (NEWLY ADDED) (37)")?'selected':''?>>
                                                                                                    ANDHRA PRADESH (NEWLY ADDED) (37)
                                                                                                </option>
                                                                                                <option value="ANDHRA PRADESH(BEFORE DIVISION) (28)" <?=($infomainaccessuser['saleordertwopos']=="ANDHRA PRADESH(BEFORE DIVISION) (28)")?'selected':''?>>
                                                                                                    ANDHRA PRADESH(BEFORE DIVISION) (28)
                                                                                                </option>
                                                                                                <option value="ARUNACHAL PRADESH (12)" <?=($infomainaccessuser['saleordertwopos']=="ARUNACHAL PRADESH (12)")?'selected':''?>>
                                                                                                    ARUNACHAL PRADESH (12)
                                                                                                </option>
                                                                                                <option value="ASSAM (18)" <?=($infomainaccessuser['saleordertwopos']=="ASSAM (18)")?'selected':''?>>
                                                                                                    ASSAM (18)
                                                                                                </option>
                                                                                                <option value="BIHAR (10)" <?=($infomainaccessuser['saleordertwopos']=="BIHAR (10)")?'selected':''?>>
                                                                                                    BIHAR (10)
                                                                                                </option>
                                                                                                <option value="CENTRE JURISDICTION (99)" <?=($infomainaccessuser['saleordertwopos']=="CENTRE JURISDICTION (99)")?'selected':''?>>
                                                                                                    CENTRE JURISDICTION (99)
                                                                                                </option>
                                                                                                <option value="CHANDIGARH (4)" <?=($infomainaccessuser['saleordertwopos']=="CHANDIGARH (4)")?'selected':''?>>
                                                                                                    CHANDIGARH (4)
                                                                                                </option>
                                                                                                <option value="CHATTISGARH (22)" <?=($infomainaccessuser['saleordertwopos']=="CHATTISGARH (22)")?'selected':''?>>
                                                                                                    CHATTISGARH (22)
                                                                                                </option>
                                                                                                <option value="DADRA AND NAGAR HAVELI AND DAMAN AND DIU (NEWLY MERGED UT) (26*)" <?=($infomainaccessuser['saleordertwopos']=="DADRA AND NAGAR HAVELI AND DAMAN AND DIU (NEWLY MERGED UT) (26*)")?'selected':''?>>
                                                                                                    DADRA AND NAGAR HAVELI AND DAMAN AND DIU (NEWLY MERGED UT) (26*)
                                                                                                </option>
                                                                                                <option value="DELHI (7)" <?=($infomainaccessuser['saleordertwopos']=="DELHI (7)")?'selected':''?>>
                                                                                                    DELHI (7)
                                                                                                </option>
                                                                                                <option value="GOA (30)" <?=($infomainaccessuser['saleordertwopos']=="GOA (30)")?'selected':''?>>
                                                                                                    GOA (30)
                                                                                                </option>
                                                                                                <option value="GUJARAT (24)" <?=($infomainaccessuser['saleordertwopos']=="GUJARAT (24)")?'selected':''?>>
                                                                                                    GUJARAT (24)
                                                                                                </option>
                                                                                                <option value="HARYANA (6)" <?=($infomainaccessuser['saleordertwopos']=="HARYANA (6)")?'selected':''?>>
                                                                                                    HARYANA (6)
                                                                                                </option>
                                                                                                <option value="HIMACHAL PRADESH (2)" <?=($infomainaccessuser['saleordertwopos']=="HIMACHAL PRADESH (2)")?'selected':''?>>
                                                                                                    HIMACHAL PRADESH (2)
                                                                                                </option>
                                                                                                <option value="JHARKHAND (20)" <?=($infomainaccessuser['saleordertwopos']=="JHARKHAND (20)")?'selected':''?>>
                                                                                                    JHARKHAND (20)
                                                                                                </option>
                                                                                                <option value="KARNATAKA (29)" <?=($infomainaccessuser['saleordertwopos']=="KARNATAKA (29)")?'selected':''?>>
                                                                                                    KARNATAKA (29)
                                                                                                </option>
                                                                                                <option value="KERALA (32)" <?=($infomainaccessuser['saleordertwopos']=="KERALA (32)")?'selected':''?>>
                                                                                                    KERALA (32)
                                                                                                </option>
                                                                                                <option value="LADAKH (NEWLY ADDED) (38)" <?=($infomainaccessuser['saleordertwopos']=="LADAKH (NEWLY ADDED) (38)")?'selected':''?>>
                                                                                                    LADAKH (NEWLY ADDED) (38)
                                                                                                </option>
                                                                                                <option value="LAKSHADWEEP (31)" <?=($infomainaccessuser['saleordertwopos']=="LAKSHADWEEP (31)")?'selected':''?>>
                                                                                                    LAKSHADWEEP (31)
                                                                                                </option>
                                                                                                <option value="MADHYA PRADESH (23)" <?=($infomainaccessuser['saleordertwopos']=="MADHYA PRADESH (23)")?'selected':''?>>
                                                                                                    MADHYA PRADESH (23)
                                                                                                </option>
                                                                                                <option value="MAHARASHTRA (27)" <?=($infomainaccessuser['saleordertwopos']=="MAHARASHTRA (27)")?'selected':''?>>
                                                                                                    MAHARASHTRA (27)
                                                                                                </option>
                                                                                                <option value="MANIPUR (14)" <?=($infomainaccessuser['saleordertwopos']=="MANIPUR (14)")?'selected':''?>>
                                                                                                    MANIPUR (14)
                                                                                                </option>
                                                                                                <option value="MEGHALAYA (17)" <?=($infomainaccessuser['saleordertwopos']=="MEGHALAYA (17)")?'selected':''?>>
                                                                                                    MEGHALAYA (17)
                                                                                                </option>
                                                                                                <option value="MIZORAM (15)" <?=($infomainaccessuser['saleordertwopos']=="MIZORAM (15)")?'selected':''?>>
                                                                                                    MIZORAM (15)
                                                                                                </option>
                                                                                                <option value="NAGALAND (13)" <?=($infomainaccessuser['saleordertwopos']=="NAGALAND (13)")?'selected':''?>>
                                                                                                    NAGALAND (13)
                                                                                                </option>
                                                                                                <option value="ODISHA (21)" <?=($infomainaccessuser['saleordertwopos']=="ODISHA (21)")?'selected':''?>>
                                                                                                    ODISHA (21)
                                                                                                </option>
                                                                                                <option value="OTHER TERRITORY (97)" <?=($infomainaccessuser['saleordertwopos']=="OTHER TERRITORY (97)")?'selected':''?>>
                                                                                                    OTHER TERRITORY (97)
                                                                                                </option>
                                                                                                <option value="PUDUCHERRY (34)" <?=($infomainaccessuser['saleordertwopos']=="PUDUCHERRY (34)")?'selected':''?>>
                                                                                                    PUDUCHERRY (34)
                                                                                                </option>
                                                                                                <option value="PUNJAB (3)" <?=($infomainaccessuser['saleordertwopos']=="PUNJAB (3)")?'selected':''?>>
                                                                                                    PUNJAB (3)
                                                                                                </option>
                                                                                                <option value="RAJASTHAN (8)" <?=($infomainaccessuser['saleordertwopos']=="RAJASTHAN (8)")?'selected':''?>>
                                                                                                    RAJASTHAN (8)
                                                                                                </option>
                                                                                                <option value="SIKKIM (11)" <?=($infomainaccessuser['saleordertwopos']=="SIKKIM (11)")?'selected':''?>>
                                                                                                    SIKKIM (11)
                                                                                                </option>
                                                                                                <option value="TAMIL NADU (33)"  <?=($infomainaccessuser['saleordertwopos']=="TAMIL NADU (33)")?'selected':''?>>
                                                                                                    TAMIL NADU (33)
                                                                                                </option>
                                                                                                <option value="TELANGANA (36)" <?=($infomainaccessuser['saleordertwopos']=="TELANGANA (36)")?'selected':''?>>
                                                                                                    TELANGANA (36)
                                                                                                </option>
                                                                                                <option value="TRIPURA (16)" <?=($infomainaccessuser['saleordertwopos']=="TRIPURA (16)")?'selected':''?>>
                                                                                                    TRIPURA (16)
                                                                                                </option>
                                                                                                <option value="UTTAR PRADESH (9)" <?=($infomainaccessuser['saleordertwopos']=="UTTAR PRADESH (9)")?'selected':''?>>
                                                                                                    UTTAR PRADESH (9)
                                                                                                </option>
                                                                                                <option value="UTTARAKHAND (5)" <?=($infomainaccessuser['saleordertwopos']=="UTTARAKHAND (5)")?'selected':''?>>
                                                                                                    UTTARAKHAND (5)
                                                                                                </option>
                                                                                                <option value="WEST BENGAL (19)" <?=($infomainaccessuser['saleordertwopos']=="WEST BENGAL (19)")?'selected':''?>>
                                                                                                    WEST BENGAL (19)
                                                                                                </option>
                                                                                            </select>
                                                                                        </div>
                                                                        ...
                                                                        <tr>
                                                                            ...
                                                                            <td data-label="ITEM DETAILS" style="padding-top: 1px !important;<?=(in_array('Item Details', $fieldadd))?'':'display:none !important;'?>">
                                                                                ...
                                                                                <div <?=(in_array('Hsn OR Sac', $fieldadd))?'':'style="display:none !important;"'?>>
                                                                                    ...
                                                                                    <input type="text" name="producthsn[]" id="producthsn1" class="form-control form-control-sm proitemselect" style="display:none;width: 27px !important;padding: 0px !important;height: 18px !important;">
                                                                            ...
                                                                            <td data-label="BATCH" <?=($access['batchexpiryval']==1)?'':'style="display:none;"'?>>
                                                                                <div>
                                                                                    <input type="text" name="batch[]" id="batch1" onClick="batchget(1);" onFocus="batchget(1);" class="form-control form-control-sm proitemselect productselectwidth" style="margin-bottom: 3px !important;padding: 0px !important;" list="" autocomplete="off" <?=(($access['saleorderbatexprequired']=='Yes')?'required':'')?> <?= ($access['saleorderbatchdef']=='availdrop')?'readonly':'' ?>>
                                                                                </div>
                                                                                <div>
                                                                                    <span id="productexpdatespan1" style="display:none; font-size:11px;">
                                                                                        EXPIRY:
                                                                                    </span>
                                                                                    <input type="date" name="expdate[]" id="expdate1" class="form-control form-control-sm proitemselect" style="display:none;width: 94px !important;padding: 0px !important;height: 18px !important;" <?=(($access['saleorderbatexprequired']=='Yes')?'required':'')?> <?= ($access['saleorderbatchdef']=='availdrop')?'readonly':'' ?>>
                                                                            ...
                                                                            <td data-label="RATE" <?=(in_array('Rate', $fieldadd))?'':'style="display:none !important;"'?>>
                                                                                ...
                                                                                <div>
                                                                                    <div class="dvi saleorderbillgets" id="saleorderbillgets1" style="margin-top:-22px;display: none;width: 250px;border-radius: 0px !important;">
                                                                                        <table width="100%">
                                                                                            <tr style="border-bottom: 1px solid #cccccc;margin-bottom: 0px;">
                                                                                                <td align="center" style="border-right: 1px solid #cccccc;width: 50% !important;display: inline-block !important;text-align: center;">
                                                                                                    <span onclick="saleordergetfun(1,0)" id="saleorderonoff1">
                                                                                                ...
                                                                                                <input type="text" style="display: none;" value="saleorder" id="billorsaleorder1">
                                                                                            </tr>
                                                                                        </table>
                                                                                    </div>
                                                                            ...
                                                                            <td data-label="<?=($access['txtqtysaleorder'])?>" <?=(in_array('Quantity', $fieldadd))?'':'style="display:none !important;"'?>>
                                                                                <div>
                                                                                ...
                                                                                <div <?=(in_array('Pack', $fieldadd))?'':'style="display:none !important;"'?>>
                                                                                    ...
                                                                                    <input type="text" name="noofpacks[]" id="noofpacks1" class="form-control form-control-sm proitemselect" style="display:none;width: 33px !important;padding: 0px !important;height: 18px !important;">
                                                                            ...
                                                                            <td data-label="<?=$access['txttaxablesaleorder']?>" <?=(in_array('Taxable Value', $fieldadd))?'':'style="display:none;"'?>>
                                                                                ...
                                                                                <div style="<?=(in_array('Profit Margin', $fieldadd))?'':'display:none;'?>">
                                                                                    <span data-bs-toggle="modal" data-bs-target="#ViewMarginDetails" style="color: green;font-size:13px;display: none;" id="margintotal1" onclick="getmargindetails(1)">
                                                                                        Profit Margin : 
                                                                                        <span id="totalmargsaleorderalue1">
                                                                                            0
                                                                                        </span>
                                                                                    </span>
                                                ...
                                                <div class="modal-body" style="height: max-content;">
                                                    ...
                                                    <table class="table notforfinaloutputsave showhideload">
                                                        ...
                                                        <tr id="validpaidblock">
                                                            ...
                                                            <td style="background-color: #e9ecef;">
                                                                <input type="number" name="validpaidamount" id="validpaidamount" class="form-control form-control-sm finalsubmitrequired" placeholder="0.00" required oninput="paidamountcalc()" value="0">
                                                                <script>
                                                        ...
                                                        <input type="hidden" name="validbalances" id="validbalances" value="0">
                                                        <tr id="duedateblock">
                                                            <th style="text-align:left !important;background-color: #e9ecef;">
...
//                 $.ajax({
//                     type: "GET",
//                     url: 'financialyear.php?types=saleorder&finyear='+document.getElementById("salesorderdate").value.split('-')[0]+'',
//                     success: function (result) {
//                         // console.log(result);
//                         if (result=='exist') {
