...
if(isset($_POST['quotationno']))
{
	...
	$quotationamount=mysqli_real_escape_string($con, $_POST['quotationamount']);
	$paidamount=mysqli_real_escape_string($con, $_POST['paidamount']);
	$paymentterm=mysqli_real_escape_string($con, $_POST['paymentterm']);
	$sql=mysqli_query($con, "update pairquotations set cancelstatus='$cancelstatus' where quotationdate='$quotationdate' and quotationno='$quotationno'");
	/* $sql2=mysqli_query($con, "update pairquotationspayments set cancelstatus='$cancelstatus' where receiptdate='$quotationdate' and receiptno='$quotationno'"); */
	if($sql)
	{
...
$sqlp=mysqli_query($con, "select productid, quantity from pairquotations where createdid='$companymainid' and  quotationno='$quotationno' and quotationdate='$quotationdate'");
	...
	if($paidamount!='0')
{
$sqlq=mysqli_query($con, "select customerid, customername from pairquotations where createdid='$companymainid' and  quotationno='$quotationno' and quotationdate='$quotationdate'");	
...
if($sql)
{
	...
	if($sqle)
	{
if((float)$quotationamount==(float)$paidamount)
{
	$paidstatus='1';
}
else
