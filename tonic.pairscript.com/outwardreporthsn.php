...
$sqlismainaccessusercusts=$con->prepare("SELECT * FROM pairmainaccess WHERE franchiseid=? AND moduletype='Customers' ORDER BY id ASC");
$sqlismainaccessusercusts->bind_param("s", $_SESSION['franchisesession']);
$sqlismainaccessusercusts->execute();
$sqlismainaccessusercust = $sqlismainaccessusercusts->get_result();
$infomainaccessusercust=$sqlismainaccessusercust->fetch_array();
$sqlismainaccessusercust->close();
$sqlismainaccessusercusts->close();
...
if (isset($_POST['submitcustomizes'])) {
	...
	$sqlreportmoduless=$con->prepare("SELECT * FROM pairreportmodules WHERE types='ouwhsn' ORDER BY id ASC");
	...
	$sqlupreport = $con->prepare("UPDATE pairreports SET rowcolumns=? WHERE franchiseid=? AND createdid=? AND types='ouwhsn'");
	...
	$sqlreport = $con->prepare("UPDATE pairreports SET reportperiod=?,filtername=?,companyname=?,dateprepare=?,timeprepare=? WHERE franchiseid=? AND createdid=? AND types='ouwhsn'");
	...
	header("Location:outwardreporthsn.php?datefrom=".date($dateformatforurlredirection,strtotime($from))."&dateto=".date($dateformatforurlredirection,strtotime($to))."");
}

$sqlreportviews=$con->prepare("SELECT * FROM pairreports WHERE franchiseid=? AND createdid=? AND types='ouwhsn'");
									...
									<div class="row">
										<div class="col-lg-6">
											<p class="mb-3" style="color:black;font-size: 20px;margin-top: -8px;">
												...
												&gt;
												<a href="outwardreport.php?datefrom=<?=$datefrom?>&dateto=<?=$dateto?>" style="color: #1878F1">
																			...
																			<div id="customizerowcolumns" class="accordion-collapse collapse show" aria-labelledby="customizerowcolumn">
																				<div class="accordion-body text-sm">
																					<div class="row justify-content-center">
																						...
																						<?php
																							$newans=array();
																							$sqlreportaccesss=$con->prepare("SELECT * FROM pairreports WHERE franchiseid=? AND createdid=? AND types='ouwhsn'");
																								...
																								<div class="col-lg-6 my-1" style="padding-left: 0px !important;">
																									<div class="custom-control custom-checkbox mr-sm-2" style="padding: 0px 12px !important;">
																										<div class="input-group input-group-sm" id="flaghead">
																											...
																											<label class="custom-control-label custom-label" for="<?= $coltypemod; ?>ouwhsn">
											...
											<div class="tab-pane fade" id="nav-profile" role="tabpanel" aria-labelledby="nav-profile-tab">
												<div class="table-responsive m-3" id="histables">
													<table class="table table-bordered">
														...
														<tbody>
      												<?php
															$sqluse=$con->prepare("SELECT * FROM pairusehistory WHERE usetype = 'REPouwhsn' AND useid = ? ORDER BY createdon DESC");
																		...
																		<tr style="height:1px;">
																			<td width="100%" style="padding:10px;">
																				<table id="print-are1" style="border:1px solid #eee;height: 975px;" width="100%">
																					...
																					<tbody id="myTable">
																					<?php
																						$sqls=$con->prepare("SELECT * FROM pairinvoices WHERE franchisesession=? AND createdid=? AND (invoicedate>=? AND invoicedate<=?) AND cancelstatus='0' ORDER BY producthsn ASC, productname ASC LIMIT 36");
										      										...
										      										while($info=$sql->fetch_array()){
												      									...
												      									if($infoposics!=$infofranposics){
												      										$totigstamt += $info['taxvalue'];
												      									}
												      									elseif($infoposics!=$infofranposics){
												      										$totcgstamt += $info['cgstvat'];
												      										$totsgstamt += $info['sgstvat'];
												      									}
																					?>
																						<tr style="vertical-align: middle;height: 18px !important;" onclick="window.open('invoiceview.php?id=<?=$info['id']?>&invoiceno=<?=$info['invoiceno']?>&invoicedate=<?=$info['invoicedate']?>','_self')">
																							<td data-label="HSN/SAC" style="text-align: left !important;font-size:12px;color:black;border:1px solid #eee;padding-left: 10px;">
																								<span style="display: inline-flex;max-width: 90px;min-width: 90px;white-space: nowrap;overflow: hidden;">
																									<span>
																										<?=$info['producthsn']?>
																									</span>
																								</span>
																							...
																							<td data-label="Product Name" style="text-align: left !important;font-size:12px;color:black;border:1px solid #eee;padding-left: 10px;">
																								<span style="display: inline-flex;max-width: 100px;min-width: 100px;justify-content: start;">
																									<span>
																										<?=$info['productname']?>
																									</span>
																								</span>
																							...
																							<td data-label="Unit" style="text-align: left !important;font-size:12px;color:black;border:1px solid #eee;padding-left: 10px;">
																								<span style="display: inline-flex;max-width: 30px;min-width: 30px;white-space: nowrap;overflow: hidden;justify-content: start;">
																									<span>
																										<?=$info['unit']?>
																									</span>
																								</span>
																							...
																							<td data-label="Quantity" style="text-align: right !important;font-size:12px;color:black;border:1px solid #eee;padding-right: 10px;">
																								<span style="display: inline-flex;max-width: 30px;min-width: 30px;white-space: nowrap;overflow: hidden;justify-content: end;">
																									<span>
																										<?=$info['quantity']?>
																									</span>
																								</span>
																							</td>
																							<td data-label="Total Value" style="text-align: right !important;font-size:12px;color:black;border:1px solid #eee;padding-right: 10px;">
																								<span style="display: inline-flex;white-space: nowrap;overflow: hidden;justify-content: end;">
																									<span>
																										<?=$resmaincurrencyans.number_format((float)$info['productnetvalue'],2,'.',',')?>
																									</span>
																								</span>
																							</td>
																							...
																							<td data-label="Taxable Value" style="text-align: right !important;font-size:12px;color:black;border:1px solid #eee;padding-right: 10px;">
																								<span style="display: inline-flex;white-space: nowrap;overflow: hidden;justify-content: end;">
																									<span>
																										<?=$resmaincurrencyans.number_format((float)$info['productvalue'],2,'.',',')?>
																									</span>
																								</span>
																							</td>
																					...
																					<?php
																						...
																						$sqltotlistpages=$con->prepare("SELECT * FROM pairinvoices WHERE franchisesession=? AND createdid=? AND (invoicedate>=? AND invoicedate<=?) AND cancelstatus='0' ORDER BY producthsn ASC, productname ASC");
										      										...
										      										while($sqlfetlist=$sqltotlistpage->fetch_array()){
																						...
																						if (($checkspage>=1)&&($checkspage<=36)) {
																							...
																							$pagetotnumpage = ceil($checkspage/36);
																						}
																						if ($pagetotnumpage==1) {
																						?>
																						<tr style="height: 30px;font-weight: bold;">
																							<td style="border: 1px solid #eee;padding-left: 10px;padding-right: 10px;" colspan="4">
																								Total
																							</td>
																							<td style="border: 1px solid #eee;padding-right: 10px;padding-left: 10px;text-align: right;">
																								<?=number_format((float)$totvalamt,2,'.',',')?>
																							</td>
										...
										<table id="print-are3" style="border: 1px solid #eee;display: none;" width="100%">
											<thead>
												<tr>
													...
													<td class="text-uppercase" style="text-align: right !important;border:1px solid #eee;padding-right: 10px;"><span style="font-size:12px;color:black;"> IGST</span></td>
...
const pagechanges = (page, pageSize) => {
	...
	var totalpagesnumber = <?=ceil($checkspage/36)?>;
	if ((page=='')||(page==0)) {
		var isthisval = 1;
	}
	...
	ajaxRequestview = $.ajax({
		type: "GET",
		url: 'reportviewsearch.php?lastornone='+lastornone+'&term='+perpages+'&limitings='+($('#limitforpagenum').val())+'&datefrom=<?=$datefrom?>&dateto=<?=$dateto?>&dif=outwardhsn',
...
$(function() {
	...
	function(start, end, label) {
		...
		window.open("outwardreporthsn.php?datefrom="+datefrom+"&dateto="+dateto+"","_self");
	});
});
	...
	var finaltimer = setTimeout(function(){
		...
		doc.autoTable({ 
			...
			columnStyles: { 
				...
				2: { halign: 'left', cellWidth: 51},
				3: { halign: 'right', cellWidth: 51},
				4: { halign: 'right', cellWidth: 51},
				5: { halign: 'right', cellWidth: 51},
				6: { halign: 'right', cellWidth: 51},
				7: { halign: 'right', cellWidth: 51},
				8: { halign: 'right', cellWidth: 51},
				9: { halign: 'right', cellWidth: 51}
			}
		});
...
function downloadaspdf() {
	...
	var finaltimer = setTimeout(function(){
		...
		doc.save('HSN-wise-summary-of-outward-supplies-<?=$datefrom?>-To-<?=$dateto?>');
...
function downloadascsv() {
	...
	var finaltimer = setTimeout(function(){
		...
		data.push(["HSN-wise summary of outward supplies"]);
		data.push([fromtopdf]);
			...
			if (cells.length > 0) {
				for (var j = 0; j < cells.length; j++) {
					...
					if (computedStyle.display !== 'none' && computedStyle.visibility !== 'hidden') {
						...
						// Handle colspan
						if((parseInt(rows.length))!=(parseInt(i+1))){
							if (colspan > 1) {
		...
		link.setAttribute("download", "HSN-wise-summary-of-outward-supplies-Reports-<?=$datefrom?>-To-<?=$dateto?>");
...
document.addEventListener('DOMContentLoaded', function() {
	...
	function makeAjaxRequest() {
		ajaxRequest = $.ajax({
			type: "GET",
			url: 'check.php?term=outwardhsn&datefrom=<?=$datefrom?>&dateto=<?=$dateto?>&limitfrom=0&limitto=10000',
			success: function (result) {
				if (result != mytabprev) {
