                    ...
                    <div class="col-12">
                        <div class="card mb-4 mt-5">
                            <div class="card-body p-3" style="font-family:'Myriad Set Pro','Helvetica Neue',Helvetica,Arial,sans-serif;">
								...
								<?php
$sqlismainaccessuser=mysqli_query($con, "select * from pairmainaccess where franchiseid='".$_SESSION['franchisesession']."' and moduletype='Estimates' order by id  asc");
$infomainaccessuser=mysqli_fetch_array($sqlismainaccessuser);
...
/*
								...
								<a href="estimateadd.php" class="btn btn-sm btn-custom" style="padding: 0.2rem 0.4rem;"><i class="fa fa-plus"
 style="font-size:13px;"></i> &nbsp; New Estimate</a>
...
<div align="right" class=" p-2" style="margin-top: -60px;">
       ...
       <a href="estimateadd.php" class="btn btn-sm btn-custom add"  style="font-size: 13px;height: 24px;margin-bottom:1rem;margin-top: 9px;margin-right:0px;padding-right: 5px;"><p style="width: max-content;margin-top:-5px;margin-left: -6px;padding: 0px;"><i class="fa fa-plus" style="font-size:13px;padding: 0px;width: max-content;"></i> &nbsp; <span style="margin-left: -5px;width: max-content;"> New <?= $infomainaccessuser['modulename'] ?></span></p></a>
                                            <?php
                                          if ((in_array('Estimate Payments', $modulecolumns))) {
                                            ?>
	   <a href="estimatepayments.php" class="btn btn-sm btn-custom add"  style="font-size: 13px;height: 24px;margin-bottom:1rem;margin-top: 9px;margin-right:0px;padding-right: 5px;"><p style="width: max-content;margin-top:-5px;margin-left: -6px;padding: 0px;"><i class="fa fa-check" style="font-size:13px;padding: 0px;width: max-content;"></i> &nbsp; <span style="margin-left: -5px;width: max-content;"> <?= $infomainaccessuser['modulename'] ?> Payments</span></p></a>
                                            <?php
                                          }
                                            ?>
                                    ...
                                    <thead>
                                        <tr>
                                            ...
                                            if ((in_array('Date', $modulecolumns))) {
                                                ?>
					<td class="text-uppercase" style="width:9%;word-wrap: break-word;"><span style="font-size:13px;color:black;"> Date</span></td>
                    <?php
                                            }
                                                ?>
                                                <?php
                                            if ((in_array('No', $modulecolumns))) {
                                                ?>
                      <td class="text-uppercase" style="width:10%;word-wrap: break-word;color:black;"><span style="font-size:13px;"> Number</span></td>
                      <?php
                                            }
                                                ?>
                                                <?php
                                            // if ((in_array('Customer Name', $modulecolumns))) {
                                                ?>
                      <td class="text-uppercase" style="width:56%;word-wrap: break-word;color:black;"><span style="font-size:13px;"> Name</span></td>
                      <?php
                                            // }
                                                ?>
                                                <?php
                                            if ((in_array('Term', $modulecolumns))) {
                                                ?>
                      <!-- <td class="text-uppercase" style="width:50px;word-wrap: break-word;color:black;"><span style="font-size:13px;"> Term</span></td> -->
                      <?php
                                            }
                                                ?>
                                                <?php
                                            if ((in_array('Amount', $modulecolumns))) {
                                                ?>
                      <!--td class="text-uppercase" style="width:50px;word-wrap: break-word;"><span style="font-size:13px;">Due Date</span></td>-->
                      <td class="text-uppercase" style="width:10%;word-wrap: break-word;color:black;"><span style="font-size:13px;"> Amount</span></td>
                      <?php
                                            }
                                                ?>
                                                <?php
                                            if ((in_array('Print', $modulecolumns))) {
                                                ?>
                      <td class="text-uppercase" style="width:10%;word-wrap: break-word;color:black;"><span style="font-size:13px;">Print</span></td>
                      <?php
                                            }
                                                ?>
					  <!--td class="text-uppercase" style="width:50px;word-wrap: break-word;"><span style="font-size:13px;">Generate Invoice</span></td>>
                      <td class="text-uppercase" style="width:50px;word-wrap: break-word;"><span style="font-size:13px;">Cancel Estimate</span></td>-->
...
$sqltotlist = mysqli_query($con,"select COUNT(DISTINCT estimateno,estimatedate) from pairestimates where franchisesession='".$_SESSION['franchisesession']."' and createdid='$companymainid'");
...
<?php
if ($access['estpageload']=='pageauto') {
?>
...
$.ajax({
type: "GET",
url: 'listestsearch.php?term=' + sIndex + '',
...
$.ajax({
type: "GET",
url: 'listestsearch.php?term=<?=$i?>0',
success: function (result) {
$("#myTable").html(result);
},
error: function (error) {
...
<?php
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                Are you sure you want to Generate this Estimate as Invoice?
            </div>
 ...
 ?>
 <script>
 function deleteitem(estimateno,estimatedate,cancelstatus)
{
	$('#deleteconfirm-adddelete').modal('show');
	$("#deleteconfirm-adddelete #deleteitem").attr("href","estimatecancel.php?estimateno="+estimateno+"&estimatedate="+estimatedate+"&cancelstatus="+cancelstatus);
}
 </script>
</body>
