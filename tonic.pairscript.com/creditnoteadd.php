...
$creditnotemodtype = 'Credit Notes';
$sqlismainaccessfields->bind_param("is", $companymainid,$creditnotemodtype);
...
$sqlismainaccessusers->bind_param("is", $userid,$creditnotemodtype);
...
if(isset($_POST['grandtotal'])){
	...
	$sqlismainaccessusers->bind_param("is", $franchsess,$creditnotemodtype);
	$sqlismainaccessusers->execute();
	$sqlismainaccessuser = $sqlismainaccessusers->get_result();
	$infomainaccessuser=$sqlismainaccessuser->fetch_assoc();
	$sqlismainaccessuser->close();
	...
	$creditnoteno = htmlspecialchars($infomainaccessuser['moduleprefix'].(str_pad($infomainaccessuser['modulesuffix']+1, 0, "0", STR_PAD_LEFT)), ENT_QUOTES, 'UTF-8');
   ...
   $ordering = (str_pad($infomainaccessuser['modulesuffix']+1, 0, "0", STR_PAD_LEFT));
	...
	$sesunqwerty = $_SESSION['unqwerty'];
	$franchsess = $_SESSION["franchisesession"];
	$sql3 = $con->prepare("UPDATE pairmainaccess SET modulesuffix=modulesuffix + 1 WHERE franchiseid=? AND moduletype='Credit Notes'");
	...
	$sql2->bind_param("iiss", $franchsess, $companymainid, $creditnoteno, $creditnotedate);
	$sql2->execute();
	$sql2->store_result();
	...
	if($sql2->num_rows==0){
		...
		if(($creditnoteterm=='CASH')){
			...
			if($sqlsalepayins){
				...
				// if ($validpaidamount!=0) {
					...
					if($creditnotedate!=''){
						if($chforpayments!=''){
							$chforpayments.='<br> Date <span style="color:green;" id="prohisfromtospan">( '.date($datemainphp,strtotime($creditnotedate)).' ) </span>';
						}
						else{
							$chforpayments.=' Date <span style="color:green;" id="prohisfromtospan">( '.date($datemainphp,strtotime($creditnotedate)).' ) </span>';
						}					
					}
					if($creditnoteno!=''){
						if($chforpayments!=''){
							$chforpayments.='<br> Reference Number <span style="color:green;" id="prohisfromtospan">( '.$creditnoteno.' ) </span>';
						}
						else{
							$chforpayments.='Reference Number <span style="color:green;" id="prohisfromtospan">( '.$creditnoteno.' ) </span>';
						}					
					}
					if($creditnoteterm!=''){
						if($chforpayments!=''){
							$chforpayments.='<br> Mode of Payment <span style="color:green;" id="prohisfromtospan">( '.$creditnoteterm.' ) </span>';
						}
						else{
							$chforpayments.='Mode of Payment <span style="color:green;" id="prohisfromtospan">( '.$creditnoteterm.' ) </span>';
						}					
					}
					if($validpaidamount!=''){
						if($chforpayments!=''){
				...
				if ($creditnotearr[$termrun]>0) {
					...
					if($sqlsalepayins){
						...
						if ($creditnotearr[$termrun]>0) {
							...
							if(explode(',', $creditnoteterm)[$termrun]!=''){
								if($chforpayments!=''){
									$chforpayments.='<br> Mode of Payment <span style="color:green;" id="prohisfromtospan">( '.explode(',', $creditnoteterm)[$termrun].' ) </span>';
								}
								else{
									$chforpayments.='Mode of Payment <span style="color:green;" id="prohisfromtospan">( '.explode(',', $creditnoteterm)[$termrun].' ) </span>';
								}					
							}
							if($creditnotearr[$termrun]!=''){
								if($chforpayments!=''){
									$chforpayments.='<br> Amount Received <span style="color:green;" id="prohisfromtospan">( '.$creditnotearr[$termrun].' ) </span>';
								}
								else{
									$chforpayments.='Amount Received <span style="color:green;" id="prohisfromtospan">( '.$creditnotearr[$termrun].' ) </span>';
								}					
							}
							if($creditnotedate!=''){
								if($chforpayments!=''){
							...
							if($creditnotearr[$termrun]!=''){
								if($chforpayments!=''){
									$chforpayments.='<br> Payment <span style="color:green;" id="prohisfromtospan">( '.$creditnotearr[$termrun].' ) </span>';
								}
								else{
									$chforpayments.='Payment <span style="color:green;" id="prohisfromtospan">( '.$creditnotearr[$termrun].' ) </span>';
								}
							}
							if($creditnotearr[$termrun]!=''){
								if($chforpayments!=''){
									$chforpayments.='<br> Total <span style="color:green;" id="prohisfromtospan">( '.$creditnotearr[$termrun].' ) </span>';
								}
								else{
									$chforpayments.='Total <span style="color:green;" id="prohisfromtospan">( '.$creditnotearr[$termrun].' ) </span>';
								}
							}
							if($chforpayments!=''){
								$sqlusepayments = $con->prepare("INSERT INTO pairusehistory (usetype,createdon,createdby,useid,useremarks) VALUES ('CUSREFUND', ?, ?, ?, ?)");
		...
		if($creditnoteno!=''){
			if($creditnoteinfoch!=''){
				$creditnoteinfoch.='<br> '.$infomainaccessuser['modulename'].' Number <span style="color:green;" id="prohisfromtospan">( '.$creditnoteno.' ) </span>';
			}
			else{
				$creditnoteinfoch.='<span style="color:royalblue;">'.$infomainaccessuser['modulename'].' Information</span> <br> '.$infomainaccessuser['modulename'].' Number <span style="color:green;" id="prohisfromtospan">( '.$creditnoteno.' ) </span>';
			}                   
		}
		if($creditnotedate!=''){
			if($creditnoteinfoch!=''){
				$creditnoteinfoch.='<br> '.$infomainaccessuser['modulename'].' Date <span style="color:green;" id="prohisfromtospan">( '.date($date,strtotime($creditnotedate)).' ) </span>';
			}
			else{
				$creditnoteinfoch.='<span style="color:royalblue;">'.$infomainaccessuser['modulename'].' Information</span> <br> '.$infomainaccessuser['modulename'].' Date <span style="color:green;" id="prohisfromtospan">( '.date($date,strtotime($creditnotedate)).' ) </span>';
			}                   
		}
		if($reference!=''){
			if($creditnoteinfoch!=''){
				$creditnoteinfoch.='<br> Reference <span style="color:green;" id="prohisfromtospan">( '.$reference.' ) </span>';
			}
			else{
				$creditnoteinfoch.='<span style="color:royalblue;">'.$infomainaccessuser['modulename'].' Information</span> <br> Reference <span style="color:green;" id="prohisfromtospan">( '.$reference.' ) </span>';
			}                   
		}
		if($saleperson!=''){
			if($creditnoteinfoch!=''){
				$creditnoteinfoch.='<br> Sale Person <span style="color:green;" id="prohisfromtospan">( '.$saleperson.' ) </span>';
			}
			else{
				$creditnoteinfoch.='<span style="color:royalblue;">'.$infomainaccessuser['modulename'].' Information</span> <br> Sale Person <span style="color:green;" id="prohisfromtospan">( '.$saleperson.' ) </span>';
			}                   
		}
		if($preparedby!=''){
			if($creditnoteinfoch!=''){
				$creditnoteinfoch.='<br> Prepared By <span style="color:green;" id="prohisfromtospan">( '.$preparedby.' ) </span>';
			}
			else{
				$creditnoteinfoch.='<span style="color:royalblue;">'.$infomainaccessuser['modulename'].' Information</span> <br> Prepared By <span style="color:green;" id="prohisfromtospan">( '.$preparedby.' ) </span>';
			}                   
		}
		if($checkedby!=''){
			if($creditnoteinfoch!=''){
				$creditnoteinfoch.='<br> Checked By <span style="color:green;" id="prohisfromtospan">( '.$checkedby.' ) </span>';
			}
			else{
				$creditnoteinfoch.='<span style="color:royalblue;">'.$infomainaccessuser['modulename'].' Information</span> <br> Checked By <span style="color:green;" id="prohisfromtospan">( '.$checkedby.' ) </span>';
			}                   
		}
		...
		if($customername!=''){
			if($creditnotecustinfoch!=''){
				$creditnotecustinfoch.='<br> '.$infomainaccessusercus['modulename'].' Name <span style="color:green;" id="prohisfromtospan">( '.$customername.' ) </span>';
			}
			else{
				$creditnotecustinfoch.=''.(($creditnoteinfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessusercus['modulename'].' Information</span> <br> '.$infomainaccessusercus['modulename'].' Name <span style="color:green;" id="prohisfromtospan">( '.$customername.' ) </span>';
			}                   
		}
		if($twoage!=''){
			if($creditnotecustinfoch!=''){
				$creditnotecustinfoch.='<br> AGE <span style="color:green;" id="prohisfromtospan">( '.$twoage.' ) </span>';
			}
			else{
				$creditnotecustinfoch.=''.(($creditnoteinfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessusercus['modulename'].' Information</span> <br> AGE <span style="color:green;" id="prohisfromtospan">( '.$twoage.' ) </span>';
			}                   
		}
		if($twosex!=''){
			if($creditnotecustinfoch!=''){
				$creditnotecustinfoch.='<br> SEX <span style="color:green;" id="prohisfromtospan">( '.$twosex.' ) </span>';
			}
			else{
				$creditnotecustinfoch.=''.(($creditnoteinfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessusercus['modulename'].' Information</span> <br> SEX <span style="color:green;" id="prohisfromtospan">( '.$twosex.' ) </span>';
			}                   
		}
		if($area!=''){
			if($creditnotecustinfoch!=''){
				$creditnotecustinfoch.='<br> BILLING STREET <span style="color:green;" id="prohisfromtospan">( '.$area.' ) </span>';
			}
			else{
				$creditnotecustinfoch.=''.(($creditnoteinfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessusercus['modulename'].' Information</span> <br> BILLING STREET <span style="color:green;" id="prohisfromtospan">( '.$area.' ) </span>';
			}                   
		}
		if($city!=''){
			if($creditnotecustinfoch!=''){
				$creditnotecustinfoch.='<br> BILLING CITY/TOWN <span style="color:green;" id="prohisfromtospan">( '.$city.' ) </span>';
			}
			else{
				$creditnotecustinfoch.=''.(($creditnoteinfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessusercus['modulename'].' Information</span> <br> BILLING CITY/TOWN <span style="color:green;" id="prohisfromtospan">( '.$city.' ) </span>';
			}                   
		}
		if($state!=''){
			if($creditnotecustinfoch!=''){
				$creditnotecustinfoch.='<br> BILLING STATE <span style="color:green;" id="prohisfromtospan">( '.$state.' ) </span>';
			}
			else{
				$creditnotecustinfoch.=''.(($creditnoteinfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessusercus['modulename'].' Information</span> <br> BILLING STATE <span style="color:green;" id="prohisfromtospan">( '.$state.' ) </span>';
			}                   
		}
		if($district!=''){
			if($creditnotecustinfoch!=''){
				$creditnotecustinfoch.='<br> BILLING PIN <span style="color:green;" id="prohisfromtospan">( '.$district.' ) </span>';
			}
			else{
				$creditnotecustinfoch.=''.(($creditnoteinfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessusercus['modulename'].' Information</span> <br> BILLING PIN <span style="color:green;" id="prohisfromtospan">( '.$district.' ) </span>';
			}                   
		}
		if($pincode!=''){
			if($creditnotecustinfoch!=''){
				$creditnotecustinfoch.='<br> BILLING COUNTRY/REGION <span style="color:green;" id="prohisfromtospan">( '.$pincode.' ) </span>';
			}
			else{
				$creditnotecustinfoch.=''.(($creditnoteinfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessusercus['modulename'].' Information</span> <br> BILLING COUNTRY/REGION <span style="color:green;" id="prohisfromtospan">( '.$pincode.' ) </span>';
			}                   
		}
		if($sarea!=''){
			if($creditnotecustinfoch!=''){
				$creditnotecustinfoch.='<br> SHIPPING STREET <span style="color:green;" id="prohisfromtospan">( '.$sarea.' ) </span>';
			}
			else{
				$creditnotecustinfoch.=''.(($creditnoteinfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessusercus['modulename'].' Information</span> <br> SHIPPING STREET <span style="color:green;" id="prohisfromtospan">( '.$sarea.' ) </span>';
			}                   
		}
		if($scity!=''){
			if($creditnotecustinfoch!=''){
				$creditnotecustinfoch.='<br> SHIPPING CITY/TOWN <span style="color:green;" id="prohisfromtospan">( '.$scity.' ) </span>';
			}
			else{
				$creditnotecustinfoch.=''.(($creditnoteinfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessusercus['modulename'].' Information</span> <br> SHIPPING CITY/TOWN <span style="color:green;" id="prohisfromtospan">( '.$scity.' ) </span>';
			}                   
		}
		if($sstate!=''){
			if($creditnotecustinfoch!=''){
				$creditnotecustinfoch.='<br> SHIPPING STATE <span style="color:green;" id="prohisfromtospan">( '.$sstate.' ) </span>';
			}
			else{
				$creditnotecustinfoch.=''.(($creditnoteinfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessusercus['modulename'].' Information</span> <br> SHIPPING STATE <span style="color:green;" id="prohisfromtospan">( '.$sstate.' ) </span>';
			}                   
		}
		if($sdistrict!=''){
			if($creditnotecustinfoch!=''){
				$creditnotecustinfoch.='<br> SHIPPING PIN <span style="color:green;" id="prohisfromtospan">( '.$sdistrict.' ) </span>';
			}
			else{
				$creditnotecustinfoch.=''.(($creditnoteinfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessusercus['modulename'].' Information</span> <br> SHIPPING PIN <span style="color:green;" id="prohisfromtospan">( '.$sdistrict.' ) </span>';
			}                   
		}
		if($spincode!=''){
			if($creditnotecustinfoch!=''){
				$creditnotecustinfoch.='<br> SHIPPING COUNTRY/REGION <span style="color:green;" id="prohisfromtospan">( '.$spincode.' ) </span>';
			}
			else{
				$creditnotecustinfoch.=''.(($creditnoteinfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessusercus['modulename'].' Information</span> <br> SHIPPING COUNTRY/REGION <span style="color:green;" id="prohisfromtospan">( '.$spincode.' ) </span>';
			}                   
		}
		if($twoworkphone!=''){
			if($creditnotecustinfoch!=''){
				$creditnotecustinfoch.='<br> WORK PHONE <span style="color:green;" id="prohisfromtospan">( '.$twoworkphone.' ) </span>';
			}
			else{
				$creditnotecustinfoch.=''.(($creditnoteinfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessusercus['modulename'].' Information</span> <br> WORK PHONE <span style="color:green;" id="prohisfromtospan">( '.$twoworkphone.' ) </span>';
			}                   
		}
		if($twomobilephone!=''){
			if($creditnotecustinfoch!=''){
				$creditnotecustinfoch.='<br> MOBILE PHONE <span style="color:green;" id="prohisfromtospan">( '.$twomobilephone.' ) </span>';
			}
			else{
				$creditnotecustinfoch.=''.(($creditnoteinfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessusercus['modulename'].' Information</span> <br> MOBILE PHONE <span style="color:green;" id="prohisfromtospan">( '.$twomobilephone.' ) </span>';
			}                   
		}
		if($gstrtype!=''){
			if($creditnotecustinfoch!=''){
				$creditnotecustinfoch.='<br> GST TREATMENT <span style="color:green;" id="prohisfromtospan">( '.$gstrtype.' ) </span>';
			}
			else{
				$creditnotecustinfoch.=''.(($creditnoteinfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessusercus['modulename'].' Information</span> <br> GST TREATMENT <span style="color:green;" id="prohisfromtospan">( '.$gstrtype.' ) </span>';
			}                   
		}
		if($gstno!=''){
			if($creditnotecustinfoch!=''){
				$creditnotecustinfoch.='<br> GSTIN <span style="color:green;" id="prohisfromtospan">( '.$gstno.' ) </span>';
			}
			else{
				$creditnotecustinfoch.=''.(($creditnoteinfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessusercus['modulename'].' Information</span> <br> GSTIN <span style="color:green;" id="prohisfromtospan">( '.$gstno.' ) </span>';
			}                   
		}
		if($pos!=''){
			if($creditnotecustinfoch!=''){
				$creditnotecustinfoch.='<br> PLACE OF SUPPLY <span style="color:green;" id="prohisfromtospan">( '.$pos.' ) </span>';
			}
			else{
				$creditnotecustinfoch.=''.(($creditnoteinfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessusercus['modulename'].' Information</span> <br> PLACE OF SUPPLY <span style="color:green;" id="prohisfromtospan">( '.$pos.' ) </span>';
			}                   
		}
		if($dlno20!=''){
			if($creditnotecustinfoch!=''){
				$creditnotecustinfoch.='<br> DL No 20 <span style="color:green;" id="prohisfromtospan">( '.$dlno20.' ) </span>';
			}
			else{
				$creditnotecustinfoch.=''.(($creditnoteinfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessusercus['modulename'].' Information</span> <br> DL No 20 <span style="color:green;" id="prohisfromtospan">( '.$dlno20.' ) </span>';
			}                   
		}
		if($dlno21!=''){
			if($creditnotecustinfoch!=''){
				$creditnotecustinfoch.='<br> DL No 21 <span style="color:green;" id="prohisfromtospan">( '.$dlno21.' ) </span>';
			}
			else{
				$creditnotecustinfoch.=''.(($creditnoteinfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessusercus['modulename'].' Information</span> <br> DL No 21 <span style="color:green;" id="prohisfromtospan">( '.$dlno21.' ) </span>';
			}                   
		}
		...
		for($i=0; $i<count($_POST['productname']); $i++){
			...
			if($productname!=''){
				...
				if($expdate!=''){
					if($creditnoteiteminfoch!=''){
						$creditnoteiteminfoch.='<br> Expiry <span style="color:green;" id="prohisfromtospan">( '.$expdate.' ) </span>';
					}
					else{
						$creditnoteiteminfoch.=''.(($creditnoteinfoch!=''||$creditnotecustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Expiry <span style="color:green;" id="prohisfromtospan">( '.$expdate.' ) </span>';
					}                   
				}
				if($productrate!=''){
					if($creditnoteiteminfoch!=''){
		...
		if($totalitems!=''){
			if($creditnoteiteminfoch!=''){
				$creditnoteiteminfoch.='<br> Total Items <span style="color:green;" id="prohisfromtospan">( '.$totalitems.' ) </span>';
			}
			else{
				$creditnoteiteminfoch.=''.(($creditnoteinfoch!=''||$creditnotecustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Total Items <span style="color:green;" id="prohisfromtospan">( '.$totalitems.' ) </span>';
			}                   
		}
		if($totalquantity!=''){
			if($creditnoteiteminfoch!=''){
				$creditnoteiteminfoch.='<br> Total Qty <span style="color:green;" id="prohisfromtospan">( '.$totalquantity.' ) </span>';
			}
			else{
				$creditnoteiteminfoch.=''.(($creditnoteinfoch!=''||$creditnotecustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Total Qty <span style="color:green;" id="prohisfromtospan">( '.$totalquantity.' ) </span>';
			}                   
		}
		if($totalamount!=''){
			if($creditnoteiteminfoch!=''){
				$creditnoteiteminfoch.='<br> Sub Total <span style="color:green;" id="prohisfromtospan">( '.$totalamount.' ) </span>';
			}
			else{
				$creditnoteiteminfoch.=''.(($creditnoteinfoch!=''||$creditnotecustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Sub Total <span style="color:green;" id="prohisfromtospan">( '.$totalamount.' ) </span>';
			}                   
		}
		if($discountamount!=''){
			if($creditnoteiteminfoch!=''){
				$creditnoteiteminfoch.='<br> Discount <span style="color:green;" id="prohisfromtospan">( '.$discountamount.' ) </span>';
			}
			else{
				$creditnoteiteminfoch.=''.(($creditnoteinfoch!=''||$creditnotecustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Discount <span style="color:green;" id="prohisfromtospan">( '.$discountamount.' ) </span>';
			}                   
		}
		if($totalvatamount!=''){
			if($creditnoteiteminfoch!=''){
				$creditnoteiteminfoch.='<br> Total Tax <span style="color:green;" id="prohisfromtospan">( '.$totalvatamount.' ) </span>';
			}
			else{
				$creditnoteiteminfoch.=''.(($creditnoteinfoch!=''||$creditnotecustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Total Tax <span style="color:green;" id="prohisfromtospan">( '.$totalvatamount.' ) </span>';
			}                   
		}
		if($roundoff!=''){
			if($creditnoteiteminfoch!=''){
				$creditnoteiteminfoch.='<br> Round Off <span style="color:green;" id="prohisfromtospan">( '.$roundoff.' ) </span>';
			}
			else{
				$creditnoteiteminfoch.=''.(($creditnoteinfoch!=''||$creditnotecustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Round Off <span style="color:green;" id="prohisfromtospan">( '.$roundoff.' ) </span>';
			}                   
		}
		if($grandtotal!=''){
			if($creditnoteiteminfoch!=''){
				$creditnoteiteminfoch.='<br> Grand Total <span style="color:green;" id="prohisfromtospan">( '.$grandtotal.' ) </span>';
			}
			else{
				$creditnoteiteminfoch.=''.(($creditnoteinfoch!=''||$creditnotecustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Grand Total <span style="color:green;" id="prohisfromtospan">( '.$grandtotal.' ) </span>';
			}                   
		}
		if($description!=''){
			if($creditnoteiteminfoch!=''){
				$creditnoteiteminfoch.='<br> Description <span style="color:green;" id="prohisfromtospan">( '.$description.' ) </span>';
			}
			else{
				$creditnoteiteminfoch.=''.(($creditnoteinfoch!=''||$creditnotecustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Description <span style="color:green;" id="prohisfromtospan">( '.$description.' ) </span>';
			}                   
		}
		if($notes!=''){
			if($creditnoteiteminfoch!=''){
				$creditnoteiteminfoch.='<br> Notes <span style="color:green;" id="prohisfromtospan">( '.$notes.' ) </span>';
			}
			else{
				$creditnoteiteminfoch.=''.(($creditnoteinfoch!=''||$creditnotecustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Notes <span style="color:green;" id="prohisfromtospan">( '.$notes.' ) </span>';
			}                   
		}
		if($terms!=''){
			if($creditnoteiteminfoch!=''){
				$creditnoteiteminfoch.='<br> Terms And Conditions <span style="color:green;" id="prohisfromtospan">( '.$terms.' ) </span>';
			}
			else{
				$creditnoteiteminfoch.=''.(($creditnoteinfoch!=''||$creditnotecustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Terms And Conditions <span style="color:green;" id="prohisfromtospan">( '.$terms.' ) </span>';
			}                   
		}
		if($fileattach!=''){
			if($creditnoteiteminfoch!=''){
				$creditnoteiteminfoch.='<br> Attach <span style="color:green;" id="prohisfromtospan">( Added ) </span>';
			}
			else{
				$creditnoteiteminfoch.=''.(($creditnoteinfoch!=''||$creditnotecustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Attach <span style="color:green;" id="prohisfromtospan">( Added ) </span>';
			}                   
		}
		...
		if($invoiceno!=''){
			if($creditnoteiteminfoch!=''){
				$creditnoteiteminfoch.='<br> '.$infomainaccessuserinv['modulename'].' Number <span style="color:green;" id="prohisfromtospan">( '.$invoiceno.' ) </span>';
			}
			else{
				$creditnoteiteminfoch.=''.(($creditnoteinfoch!=''||$creditnotecustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> '.$infomainaccessuserinv['modulename'].' Number <span style="color:green;" id="prohisfromtospan">( '.$invoiceno.' ) </span>';
			}                   
		}
		if(($invoiceamount!='')&&($invoiceamount!='0')){
			if($creditnoteiteminfoch!=''){
				$creditnoteiteminfoch.='<br> '.$infomainaccessuserinv['modulename'].' Amount <span style="color:green;" id="prohisfromtospan">( '.$invoiceamount.' ) </span>';
			}
			else{
				$creditnoteiteminfoch.=''.(($creditnoteinfoch!=''||$creditnotecustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> '.$infomainaccessuserinv['modulename'].' Amount <span style="color:green;" id="prohisfromtospan">( '.$invoiceamount.' ) </span>';
			}                   
		}
		if(($invoicepaymentreceived!='')&&($invoicepaymentreceived!='0')){
			if($creditnoteiteminfoch!=''){
				$creditnoteiteminfoch.='<br> '.$infomainaccessuserinv['modulename'].' Payment Received <span style="color:green;" id="prohisfromtospan">( '.$invoicepaymentreceived.' ) </span>';
			}
			else{
				$creditnoteiteminfoch.=''.(($creditnoteinfoch!=''||$creditnotecustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> '.$infomainaccessuserinv['modulename'].' Payment Received <span style="color:green;" id="prohisfromtospan">( '.$invoicepaymentreceived.' ) </span>';
			}                   
		}
		if($refundoption!=''){
			if($creditnoteiteminfoch!=''){
				$creditnoteiteminfoch.='<br> Refund Option <span style="color:green;" id="prohisfromtospan">( '.(($refundoption=='refundnow')?'Refund Now':((($refundoption=='refundlater')?'Refund Later':'No Refund'))).' ) </span>';
			}
			else{
				$creditnoteiteminfoch.=''.(($creditnoteinfoch!=''||$creditnotecustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Refund Option <span style="color:green;" id="prohisfromtospan">( '.(($refundoption=='refundnow')?'Refund Now':((($refundoption=='refundlater')?'Refund Later':'No Refund'))).' ) </span>';
			}                   
		}
		if($creditnoteterm!=''){
			if($creditnoteiteminfoch!=''){
				$creditnoteiteminfoch.='<br> Refund Method <span style="color:green;" id="prohisfromtospan">( '.$creditnoteterm.' ) </span>';
			}
			else{
				$creditnoteiteminfoch.=''.(($creditnoteinfoch!=''||$creditnotecustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Refund Method <span style="color:green;" id="prohisfromtospan">( '.$creditnoteterm.' ) </span>';
			}                   
		}
		if($validpaidamount!=''&&$creditnoteterm=='CASH'){
			if($creditnoteiteminfoch!=''){
				$creditnoteiteminfoch.='<br> Refund Amount <span style="color:green;" id="prohisfromtospan">( '.$validpaidamount.' ) </span>';
			}
			else{
				$creditnoteiteminfoch.=''.(($creditnoteinfoch!=''||$creditnotecustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Refund Amount <span style="color:green;" id="prohisfromtospan">( '.$validpaidamount.' ) </span>';
			}                   
		}
		if($validpaidamount!=''&&$creditnoteterm=='CASH,CARD,GPAY'){
			if($creditnoteiteminfoch!=''){
				$creditnoteiteminfoch.='<br> Refund Amount <span style="color:green;" id="prohisfromtospan">( '.$validpaidamount.' ) </span>';
			}
			else{
		...
		// if($validbalance!=''&&$creditnoteterm=='CASH'){
		// 	if($creditnoteiteminfoch!=''){
		// 		$creditnoteiteminfoch.='<br> Balance Due <span style="color:green;" id="prohisfromtospan">( '.$validbalance.' ) </span>';
		// 	}
		// 	else{
		// 		$creditnoteiteminfoch.=''.(($creditnoteinfoch!=''||$creditnotecustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Balance Due <span style="color:green;" id="prohisfromtospan">( '.$validbalance.' ) </span>';
		// 	}                   
		// }
		// if($validbalance!=''&&$creditnoteterm=='CASH,CARD,GPAY'){
		// 	if($creditnoteiteminfoch!=''){
		// 		$creditnoteiteminfoch.='<br> Balance Due <span style="color:green;" id="prohisfromtospan">( '.$validbalance.' ) </span>';
		// 	}
		// 	else{
		...
		// 	}                   
		// }
		// if($duedates!=''&&$creditnoteterm=='CREDIT'){
		// 	if($creditnoteiteminfoch!=''){
		// 		$creditnoteiteminfoch.='<br> Due Term <span style="color:green;" id="prohisfromtospan">( '.$duedates.' ) </span>';
		// 	}
		// 	else{
		// 		$creditnoteiteminfoch.=''.(($creditnoteinfoch!=''||$creditnotecustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Due Term <span style="color:green;" id="prohisfromtospan">( '.$duedates.' ) </span>';
		// 	}                   
		// }
		// if($duedate!=''&&$creditnoteterm=='CREDIT'){
		// 	if($creditnoteiteminfoch!=''){
		// 		$creditnoteiteminfoch.='<br> Due Date <span style="color:green;" id="prohisfromtospan">( '.date($date,strtotime($duedate)).' ) </span>';
		// 	}
		// 	else{
		// 		$creditnoteiteminfoch.=''.(($creditnoteinfoch!=''||$creditnotecustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Due Date <span style="color:green;" id="prohisfromtospan">( '.date($date,strtotime($duedate)).' ) </span>';
		// 	}                   
		// }
		//FOR CREDITNOTE OTHER INFORMATIONS LIKE PAYMENT AND TERMS AND DESCRIPTION......
		$creditnotetotinfoch = "CREDITNOTE CREATED<br>".$creditnoteinfoch.$creditnotecustinfoch.$creditnoteiteminfoch;
    	...
    	$cussqlup->bind_param("ssssi", $creditnoteamount, $balanceamount, $currentamount, $overdueamount, $customerid);
    	$cussqlup->execute();
		$cussqlup->close();
		...
		if(isset($_POST['submit1'])){
			...
			echo '<script> window.location.href="creditnotes.php?remarks=Added Successfully";</script>';
		}
		else{
			$sqlusefinal = $con->prepare("INSERT INTO pairusehistory (usetype,createdon,createdby,useid,uniqueid,useremarks) VALUES ('CREDITNOTES', ?, ?, ?, ?, '<span style=\"color:green;\" id=\"prohisfromtospan\">Added Successfully</span>')");
    		$sqlusefinal->bind_param("ssss", $times, $_SESSION["unqwerty"], $creditnoteno, $salesid);
    		$sqlusefinal->execute();
    		$sqlusefinal->close();
			echo '<script> window.location.href="creditnoteview.php?id='.$salesid.'&creditnoteno='.$creditnoteno.'&creditnotedate='.$creditnotedate.'&remarks=Added Successfully";</script>';
		}
	}
}
else{
	header("Location: creditnotes.php?error=Error Data");
	//FOR IF THE CREDITNOTE IS ALREADY EXISTS IN THIS NUMBER AND DATE AND FRANCHISE AND COMPANY
}
}
$sqlismainaccessusercuss=$con->prepare("SELECT * FROM pairmainaccess WHERE userid=? AND moduletype='Customers' ORDER BY id ASC");
$sqlismainaccessusercuss->bind_param("s", $userid);
	...
	<div class="container-fluid py-4 bg-body">
		...
		<?php
			...
			if((isset($_GET['invoiceno']))&&(isset($_GET['invoicedate']))){
				$invoiceno=base64_decode( $_GET['invoiceno']);
				$invoicedate=base64_decode( $_GET['invoicedate']);
				...
				if ($sql->errno) {
					...
					$sql->close();
		?>
		<!-- FOR CREDITNOTE MODULE INFORMATIONS AND PREFERENCES -->
				...
				<div class="col-12">
					<div class="card mb-4 mt-5">
						...
						?>
							...
							<?php
								$oldarray=array( 'invoiceno','invoicedate','reference','saleperson','preparedby','checkedby','customername','age','sex','area','city','state','district','pincode','sarea','scity','sstate','sdistrict','spincode','workphone','mobile','gstrtype','gstno','pos','dlno20','dlno21' );
							...
							?>
								...
								<input type="hidden" value="<?=$rows[0]['invoiceterm']?>" name="oldesinvoiceterm">
								...
								<input type="hidden" name="oldinvoiceno" value="<?=$rows[0]['invoiceno']?>">
								<input type="hidden" name="oldinvoicedate" value="<?=$rows[0]['invoicedate']?>">
								<input type="hidden" name="creditnoteterm" value="<?=$rows[0]['invoiceterm']?>">
								<div class="row">
								<?php
									// if ((in_array('Credit Note Information', $fieldadd))) {
								?>
									<div class="col-lg-4">
														...
														<div class="row">
															<div class="col-lg-12">
																<div class="form-group row" style="align-items: center !important;">
																	...
																	<div class="col-lg-7">
																		<input type="text" class="form-control  form-control-sm" id="creditnoteno" name="creditnoteno" required  value="<?=$infomainaccessuser['moduleprefix']?><?=str_pad(($infomainaccessuser['modulesuffix']+1), 0, "0", STR_PAD_LEFT)?>" readonly>
															...
															<div class="col-lg-12">
																<div class="form-group row" style="align-items: center !important;">
																	...
																	<?php
																		...
																		$dt = new DateTime();
																	?>
																		<input type="date" class="form-control  form-control-sm" id="creditnotedate" name="creditnotedate" required value="<?= $dt->format('Y-m-d') ?>">
																	</div>
																</div>
															</div>
															<div class="col-lg-12" <?=((in_array('Credit Note Time', $fieldadd))?'':'style="display:none;"')?>>
																<div class="form-group row" style="align-items: center !important;">
																	<div class="col-lg-5">
																	...
																	<div class="col-lg-7">
																		...
																		?>
																		<input type="time" class="form-control form-control-sm" id="creditnotetime" name="creditnotetime" required value="<?php echo $current_time_def; ?>">
																	</div>
																</div>
															</div>
														<?php
														...
														<table class="table table-bordered" id="purchasetable">
															<thead>
																<tr>
																	...
																	<th <?=((in_array('Item Details', $fieldadd))||($access['batchexpiryval']==1)||(in_array('Rate', $fieldadd))||(in_array('Quantity', $fieldadd))||(in_array('Taxable Value', $fieldadd))||(in_array('Tax Value', $fieldadd))||(in_array('Amount', $fieldadd)))?'':'style="display:none !important;"'?>>
																	</th>
																<?php
														...
														?>
															...
															?>
																...
																<tr>
																	...
																	<td class="tdmove" <?=((in_array('Item Details', $fieldadd))||($access['batchexpiryval']==1)||(in_array('Rate', $fieldadd))||(in_array('Quantity', $fieldadd))||(in_array('Taxable Value', $fieldadd))||(in_array('Tax Value', $fieldadd))||(in_array('Amount', $fieldadd)))?'':'style="display:none !important;"'?>>
																	...
																	<td data-label="<?=($access['txtqtycreditnote'])?>" <?=(in_array('Quantity', $fieldadd))?'':'style="display:none !important;"'?>>
																		<div>
																	...
																	<td data-label="<?=$access['txttaxablecreditnote']?>" <?=(in_array('Taxable Value', $fieldadd))?'':'style="display:none;"'?>>
																		<div>
																			<span style="font-size:15px !important;">
																				<?php echo $resmaincurrencyans; ?>
																			</span>
																	...
																	<td <?=((in_array('Item Details', $fieldadd))||($access['batchexpiryval']==1)||(in_array('Rate', $fieldadd))||(in_array('Quantity', $fieldadd))||(in_array('Taxable Value', $fieldadd))||(in_array('Tax Value', $fieldadd))||(in_array('Amount', $fieldadd)))?'style="white-space:nowrap !important;"':'style="display:none !important;"'?>>
																		<div class="app-utility-item app-user-dropdown dropdown" style="margin-right: 0px !important; <?=(in_array('Additional Informations', $fieldadd))?'display:none !important;':'display:none !important;'?>">
																			<a href="javascript:;" class="p-0" id="dropdownadditionalinfo" data-bs-toggle="dropdown" aria-expanded="false">
																				<svg width="15" height="15" viewBox="0 0 512 512" xmlns="http://www.w3.org/2000/svg" class="icon icon-sm text-blue"><path d="M162 224c17.7 0 32 14.3 32 32s-14.3 32-32 32-32-14.3-32-32 14.3-32 32-32M256 224c17.7 0 32 14.3 32 32s-14.3 32-32 32-32-14.3-32-32 14.3-32 32-32M350 224c17.7 0 32 14.3 32 32s-14.3 32-32 32-32-14.3-32-32 14.3-32 32-32" id="Path"></path><path d="M256 480c123.712 0 224-100.288 224-224S379.712 32 256 32 32 132.288 32 256s100.288 224 224 224zm0 32C114.615 512 0 397.385 0 256S114.615 0 256 0s256 114.615 256 256-114.615 256-256 256z" id="Oval-1"></path></svg>
																			</a>
												...
												<div class="table-responsive mt-1" id="gsttablediv" <?=(in_array('Tax Informations', $fieldadd))?'':'style="display:none;"'?>>
													<table class="table table-bordered" id="gsttable" style="font-size:12px;">
														...
														<?php
															...
															$sqlitaxes=$con->prepare("SELECT * FROM pairinvoices WHERE franchisesession=? AND createdid=? AND invoiceno=? AND invoicedate=? ORDER BY id ASC");
															$sqlitaxes->bind_param("ssss", $_SESSION["franchisesession"], $companymainid, $invoiceno, $invoicedate);
															$sqlitaxes->execute();
											...
											<table class="table notforfinaloutputsave showhideload">
												...
												<tr>
													...
													<td style="background-color: #e9ecef;">
														<input type="text" name="validcreditnoteinvno" id="validcreditnoteinvno" class="form-control form-control-sm finalsubmitrequired" readonly required value="<?=base64_decode($_GET['invoiceno'])?>">
													</td>
												</tr>
												<tr>
													<th style="text-align:left !important;background-color: #e9ecef;">
													...
													<td style="background-color: #e9ecef;">
														<input type="number" name="invoiceamount" id="invoiceamount" class="form-control form-control-sm finalsubmitrequired" readonly required value="<?=base64_decode($_GET['totamt'])?>">
													</td>
												</tr>
												<?php
													$invoiceno = base64_decode($_GET['invoiceno']);
			      								...
			      								$sqlsantss->bind_param("sssss", $_SESSION['franchisesession'], $companymainid, $invoiceno, $invoicedate, $customerid);
			      								$sqlsantss->execute();
														...
														<div class="col-lg-12">
															...
															<?php
																...
																while($info=$sqli->fetch_array()){
																	if (($info['term']=='CASH')){
															?>
																<option value="<?= $info['term'] ?>" <?=($info['term']==$rows[0]['invoiceterm'])?'selected':''?>>
																	<?= $info['term'] ?>
																</option>
															<?php
																	}
												...
												<tr id="validpaidblock">
													...
													<td>
														<input type="number" name="validpaidamount" id="validpaidamount" class="form-control form-control-sm finalsubmitrequired" placeholder="0.00" required onchange="paidamountcalc()" value="<?=$paidamount - $paidamountcr?>" step="any" <?=((($paidamount - $paidamountcr)<=0)?'readonly':'')?>>
														<script>
																...
																<div class="form-group row" style="align-items: center !important;">
																	...
																	<div class="col-lg-7">
																		<?php
																			...
																			$dt = new DateTime();
																		?>
																		<input type="date" class="form-control form-control-sm" id="creditnotedate" name="creditnotedate" value="<?= $dt->format('Y-m-d') ?>" required>
																	</div>
																</div>
															</div>
															<div class="col-lg-12" <?=((in_array('Credit Note Time', $fieldadd))?'':'style="display:none;"')?>>
																<div class="form-group row" style="align-items: center !important;">
																	...
																	<div class="col-lg-7">
																		<?php
																			$current_time_def = date('H:i:s');
																		?>
																		<input type="time" class="form-control form-control-sm" value="<?php echo $current_time_def; ?>" id="creditnotetime" name="creditnotetime" required>
																	</div>
																</div>
															</div>
														<?php
														...
														<button class="accordion-button font-weight-bold" type="button" id="customerinfotoggler">
															...
															?>
																<div class="row">
																	<div class="col-lg-2 col-md-2 col-3">
																		<div class="custom-control custom-radio mr-sm-2">
																			<input type="radio" class="custom-control-input" name="customerinfodefault" id="onecustomerinfo" value="one" <?= ($infomainaccessuser['creditnotecustinfo']=='defone')?'checked':'' ?> onclick="onetwo()">
																			<label class="custom-control-label custom-label" for="onecustomerinfo">
																				B2B
																			</label>
																		</div>
																	...
																	<div class="col-lg-2 col-md-2 col-3">
																		<div class="custom-control custom-radio mr-sm-2">
																			<input type="radio" class="custom-control-input" name="customerinfodefault" id="twocustomerinfo" value="two" <?= ($infomainaccessuser['creditnotecustinfo']=='deftwo')?'checked':'' ?> onclick="onetwo()">
																			<label class="custom-control-label custom-label" for="twocustomerinfo">
																				B2C
																			</label>
																		</div>
														...
														?>
															...
															<div id="two">
																<div class="row">
																	...
																	<div class="<?=((in_array('Age', $fieldadd))&&(in_array('Sex', $fieldadd)))?'col-lg-4':(((in_array('Age', $fieldadd))||(in_array('Sex', $fieldadd)))?'col-lg-7':'col-lg-9')?>">
																		<input type="text" name="twocustomername" id="twocustomername" class="form-control form-control-sm" placeholder="<?= $infomainaccessusercus['modulename'] ?> Name" <?= ($access['creditnotebtwocnamerequired']=='Yes')? 'required': ' ' ?>>
																	</div>
																	<div class="col-lg-1" style="padding-right: 0px;<?=(in_array('Age', $fieldadd))?'':'display:none;'?>">
																		<label for="customername" class="custom-label">
																			AGE
																		...
																		<div class="row mb-0" id="custaddressdiv" style="background-color:#fbfafa; color:#777777; display:block;margin: 0px !important;padding: 9px 9px 0px 9px !important;">
																			...
																			<div class="row" style="padding:0px !important;margin: 0px !important;">
																				<div class="col-lg-6" <?=(in_array('Work Phone', $fieldadd))?'':'style="display:none;"'?>>
																					...
																					<input type="text" class="form-control form-control-sm" id="twoworkphone" name="twoworkphone" placeholder="Work Phone" <?= ($access['creditnotebtwocwphonerequired']=='Yes')? 'required': ' ' ?>>
																				</div>
																				<div class="col-lg-6" <?=(in_array('Mobile Phone', $fieldadd))?'':'style="display:none;"'?>>
																					<label for="twomobilephone" class="custom-label" style="margin:0px !important;font-size: 12px !important;color: #777777;">
																						MOBILE PHONE
																				...
																				<div class="col-lg-8" <?=(in_array('GSTIN', $fieldadd))?'':'style="display:none;"'?>>
																					<div class="row">
																						<div class="col-lg-6">
																							...
																							<input type="text" class="form-control form-control-sm" id="twogsttreatment" name="twogsttreatment" placeholder="GST TREATMENT" value="<?=$infomainaccessuser['creditnotetwogst']?>" readonly>
																						</div>
																						<div class="col-lg-6" id="twogstinblock">
																							<label for="twogstin" class="custom-label" style="margin:0px !important;font-size: 12px !important;color: #777777;">
																								GSTIN
																				...
																				<div class="col-lg-4" <?=(in_array('Place of Supply', $fieldadd))?'':'style="display:none;"'?>>
																					<div class="row">
																						<div class="col-lg-12" id="twoposblock">
																							...
																							<select name="twopos" id="twopos" class="select5 form-control form-control-sm" required onchange="twoposans()">
																								<option disabled value="" <?=($infomainaccessuser['creditnotetwopos']=='manual'||'auto')?'selected':''?>>
																									Select The Place
																								</option>
																								<option value="JAMMU AND KASHMIR (1)" <?=($infomainaccessuser['creditnotetwopos']=="JAMMU AND KASHMIR (1)")?'selected':''?>>
																									JAMMU AND KASHMIR (1)
																								</option>
																								<option value="ANDAMAN AND NICOBAR ISLANDS (35)" <?=($infomainaccessuser['creditnotetwopos']=="ANDAMAN AND NICOBAR ISLANDS (35)")?'selected':''?>>
																									ANDAMAN AND NICOBAR ISLANDS (35)
																								</option>
																								<option value="ANDHRA PRADESH (NEWLY ADDED) (37)" <?=($infomainaccessuser['creditnotetwopos']=="ANDHRA PRADESH (NEWLY ADDED) (37)")?'selected':''?>>
																									ANDHRA PRADESH (NEWLY ADDED) (37)
																								</option>
																								<option value="ANDHRA PRADESH(BEFORE DIVISION) (28)" <?=($infomainaccessuser['creditnotetwopos']=="ANDHRA PRADESH(BEFORE DIVISION) (28)")?'selected':''?>>
																									ANDHRA PRADESH(BEFORE DIVISION) (28)
																								</option>
																								<option value="ARUNACHAL PRADESH (12)" <?=($infomainaccessuser['creditnotetwopos']=="ARUNACHAL PRADESH (12)")?'selected':''?>>
																									ARUNACHAL PRADESH (12)
																								</option>
																								<option value="ASSAM (18)" <?=($infomainaccessuser['creditnotetwopos']=="ASSAM (18)")?'selected':''?>>
																									ASSAM (18)
																								</option>
																								<option value="BIHAR (10)" <?=($infomainaccessuser['creditnotetwopos']=="BIHAR (10)")?'selected':''?>>
																									BIHAR (10)
																								</option>
																								<option value="CENTRE JURISDICTION (99)" <?=($infomainaccessuser['creditnotetwopos']=="CENTRE JURISDICTION (99)")?'selected':''?>>
																									CENTRE JURISDICTION (99)
																								</option>
																								<option value="CHANDIGARH (4)" <?=($infomainaccessuser['creditnotetwopos']=="CHANDIGARH (4)")?'selected':''?>>
																									CHANDIGARH (4)
																								</option>
																								<option value="CHATTISGARH (22)" <?=($infomainaccessuser['creditnotetwopos']=="CHATTISGARH (22)")?'selected':''?>>
																									CHATTISGARH (22)
																								</option>
																								<option value="DADRA AND NAGAR HAVELI AND DAMAN AND DIU (NEWLY MERGED UT) (26*)" <?=($infomainaccessuser['creditnotetwopos']=="DADRA AND NAGAR HAVELI AND DAMAN AND DIU (NEWLY MERGED UT) (26*)")?'selected':''?>>
																									DADRA AND NAGAR HAVELI AND DAMAN AND DIU (NEWLY MERGED UT) (26*)
																								</option>
																								<option value="DELHI (7)" <?=($infomainaccessuser['creditnotetwopos']=="DELHI (7)")?'selected':''?>>
																									DELHI (7)
																								</option>
																								<option value="GOA (30)" <?=($infomainaccessuser['creditnotetwopos']=="GOA (30)")?'selected':''?>>
																									GOA (30)
																								</option>
																								<option value="GUJARAT (24)" <?=($infomainaccessuser['creditnotetwopos']=="GUJARAT (24)")?'selected':''?>>
																									GUJARAT (24)
																								</option>
																								<option value="HARYANA (6)" <?=($infomainaccessuser['creditnotetwopos']=="HARYANA (6)")?'selected':''?>>
																									HARYANA (6)
																								</option>
																								<option value="HIMACHAL PRADESH (2)" <?=($infomainaccessuser['creditnotetwopos']=="HIMACHAL PRADESH (2)")?'selected':''?>>
																									HIMACHAL PRADESH (2)
																								</option>
																								<option value="JHARKHAND (20)" <?=($infomainaccessuser['creditnotetwopos']=="JHARKHAND (20)")?'selected':''?>>
																									JHARKHAND (20)
																								</option>
																								<option value="KARNATAKA (29)" <?=($infomainaccessuser['creditnotetwopos']=="KARNATAKA (29)")?'selected':''?>>
																									KARNATAKA (29)
																								</option>
																								<option value="KERALA (32)" <?=($infomainaccessuser['creditnotetwopos']=="KERALA (32)")?'selected':''?>>
																									KERALA (32)
																								</option>
																								<option value="LADAKH (NEWLY ADDED) (38)" <?=($infomainaccessuser['creditnotetwopos']=="LADAKH (NEWLY ADDED) (38)")?'selected':''?>>
																									LADAKH (NEWLY ADDED) (38)
																								</option>
																								<option value="LAKSHADWEEP (31)" <?=($infomainaccessuser['creditnotetwopos']=="LAKSHADWEEP (31)")?'selected':''?>>
																									LAKSHADWEEP (31)
																								</option>
																								<option value="MADHYA PRADESH (23)" <?=($infomainaccessuser['creditnotetwopos']=="MADHYA PRADESH (23)")?'selected':''?>>
																									MADHYA PRADESH (23)
																								</option>
																								<option value="MAHARASHTRA (27)" <?=($infomainaccessuser['creditnotetwopos']=="MAHARASHTRA (27)")?'selected':''?>>
																									MAHARASHTRA (27)
																								</option>
																								<option value="MANIPUR (14)" <?=($infomainaccessuser['creditnotetwopos']=="MANIPUR (14)")?'selected':''?>>
																									MANIPUR (14)
																								</option>
																								<option value="MEGHALAYA (17)" <?=($infomainaccessuser['creditnotetwopos']=="MEGHALAYA (17)")?'selected':''?>>
																									MEGHALAYA (17)
																								</option>
																								<option value="MIZORAM (15)" <?=($infomainaccessuser['creditnotetwopos']=="MIZORAM (15)")?'selected':''?>>
																									MIZORAM (15)
																								</option>
																								<option value="NAGALAND (13)" <?=($infomainaccessuser['creditnotetwopos']=="NAGALAND (13)")?'selected':''?>>
																									NAGALAND (13)
																								</option>
																								<option value="ODISHA (21)" <?=($infomainaccessuser['creditnotetwopos']=="ODISHA (21)")?'selected':''?>>
																									ODISHA (21)
																								</option>
																								<option value="OTHER TERRITORY (97)" <?=($infomainaccessuser['creditnotetwopos']=="OTHER TERRITORY (97)")?'selected':''?>>
																									OTHER TERRITORY (97)
																								</option>
																								<option value="PUDUCHERRY (34)" <?=($infomainaccessuser['creditnotetwopos']=="PUDUCHERRY (34)")?'selected':''?>>
																									PUDUCHERRY (34)
																								</option>
																								<option value="PUNJAB (3)" <?=($infomainaccessuser['creditnotetwopos']=="PUNJAB (3)")?'selected':''?>>
																									PUNJAB (3)
																								</option>
																								<option value="RAJASTHAN (8)" <?=($infomainaccessuser['creditnotetwopos']=="RAJASTHAN (8)")?'selected':''?>>
																									RAJASTHAN (8)
																								</option>
																								<option value="SIKKIM (11)" <?=($infomainaccessuser['creditnotetwopos']=="SIKKIM (11)")?'selected':''?>>
																									SIKKIM (11)
																								</option>
																								<option value="TAMIL NADU (33)"  <?=($infomainaccessuser['creditnotetwopos']=="TAMIL NADU (33)")?'selected':''?>>
																									TAMIL NADU (33)
																								</option>
																								<option value="TELANGANA (36)" <?=($infomainaccessuser['creditnotetwopos']=="TELANGANA (36)")?'selected':''?>>
																									TELANGANA (36)
																								</option>
																								<option value="TRIPURA (16)" <?=($infomainaccessuser['creditnotetwopos']=="TRIPURA (16)")?'selected':''?>>
																									TRIPURA (16)
																								</option>
																								<option value="UTTAR PRADESH (9)" <?=($infomainaccessuser['creditnotetwopos']=="UTTAR PRADESH (9)")?'selected':''?>>
																									UTTAR PRADESH (9)
																								</option>
																								<option value="UTTARAKHAND (5)" <?=($infomainaccessuser['creditnotetwopos']=="UTTARAKHAND (5)")?'selected':''?>>
																									UTTARAKHAND (5)
																								</option>
																								<option value="WEST BENGAL (19)" <?=($infomainaccessuser['creditnotetwopos']=="WEST BENGAL (19)")?'selected':''?>>
																									WEST BENGAL (19)
																								</option>
																							</select>
																						</div>
																...
																<table class="table table-bordered" id="purchasetable">
																	...
																	<tbody>
																		<tr>
																			...
																			<td data-label="BATCH" <?=($access['batchexpiryval']==1)?'':'style="display:none;"'?>>
																				<div>
																					<input type="text" name="batch[]" maxlength="100" id="batch1" onClick="batchget(1);" onFocus="batchget(1);" class="form-control form-control-sm proitemselect productselectwidth" style="margin-bottom: 3px !important;padding: 0px !important;" list="" autocomplete="off" <?=(($access['creditnotebatexprequired']=='Yes')?'required':'')?> <?= ($access['creditnotebatchdef']=='availdrop')?'readonly':'' ?>>
																				</div>
																				<div>
																					<span id="productexpdatespan1" style="display:none; font-size:11px;">
																						EXPIRY:
																					</span>
																					<input type="date" name="expdate[]" id="expdate1" class="form-control form-control-sm proitemselect" style="display:none;width: 94px !important;padding: 0px !important;height: 18px !important;" <?=(($access['creditnotebatexprequired']=='Yes')?'required':'')?> <?= ($access['creditnotebatchdef']=='availdrop')?'readonly':'' ?>>
																			...
																			<td data-label="RATE" <?=(in_array('Rate', $fieldadd))?'':'style="display:none !important;"'?>>
																				...
																				<div>
																					<div class="dvi creditnotebillgets" id="creditnotebillgets1" style="margin-top:-22px;display: none;width: 250px;border-radius: 0px !important;">
																						<table width="100%">
																							<tr style="border-bottom: 1px solid #cccccc;margin-bottom: 0px;">
																								<td align="center" style="border-right: 1px solid #cccccc;width: 50% !important;display: inline-block !important;text-align: center;">
																									<span onclick="creditnotegetfun(1,0)" id="creditnoteonoff1">
																										<?= strtoupper($infomainaccessuser['modulename']) ?>
																									</span>
																								</td>
																								<td align="center" style="width: 50% !important;display: inline-block !important;text-align: center;">
																								...
																								<input type="text" style="display: none;" value="creditnote" id="billorcreditnote1">
																							</tr>
																						</table>
																					</div>
																			...
																			<td data-label="<?=$access['txttaxablecreditnote']?>" <?=(in_array('Taxable Value', $fieldadd))?'':'style="display:none;"'?>>
																				...
																				<div style="<?=(in_array('Profit Margin', $fieldadd))?'':'display:none;'?>">
																					<span data-bs-toggle="modal" data-bs-target="#ViewMarginDetails" style="color: green;font-size:13px;display: none;" id="margintotal1" onclick="getmargindetails(1)">
																						Profit Margin : 
																						<span id="totalmargcreditnotealue1">
																							0
																						</span>
																					</span>
											...
											<div class="modal-content">
												...
												<div class="modal-body" style="height: max-content;">
													...
													<table class="table notforfinaloutputsave showhideload">
														...
														<input type="hidden" name="invoicepaymentreceived" id="invoicepaymentreceived" value="0">
														<tr style="<?=(($access['defcreditnotepay']=='split')?'display: none;':'')?>" id="creditnotetermblock">
															<th style="text-align:left !important;">
																<label class="custom-label">
																	Refund Method
	...
	setTimeout(function() {
		...
		if (!allAreFilled) {
			...
			if(accessallow){
				...
				$.ajax({
					type: "GET",
					url: 'financialyear.php?types=creditnote&finyear='+document.getElementById("creditnotedate").value.split('-')[0]+'',
					success: function (result) {
						// console.log(result);
						if (result=='exist') {
...
?>
	$.get("ratesearch.php", {term: productid,custid: customerid,ratedef: creditnoteratedef,differ: 'inv'} , function(datas){
		// console.log("normal"+datas);
		$("#errrate"+lineNo).val(datas);
		var letters = "<br /><b>Warning</b>:  Undefined variable $ratedatas in";
		brrch = document.getElementById("errrate"+lineNo);
