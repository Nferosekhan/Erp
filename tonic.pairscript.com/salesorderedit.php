...
if((isset($_GET['salesorderno']))&&(isset($_GET['salesorderdate']))){
    $salesorderno=htmlspecialchars( $_GET['salesorderno'], ENT_QUOTES, 'UTF-8');
    $salesorderdate=htmlspecialchars( $_GET['salesorderdate'], ENT_QUOTES, 'UTF-8');
    $sql=$con->prepare("SELECT * FROM pairsalesorders WHERE franchisesession=? AND createdid=? AND salesorderno=? AND salesorderdate=? ORDER BY id ASC");
    $sql->bind_param("ssss", $_SESSION['franchisesession'], $companymainid, $salesorderno, $salesorderdate);
    $sql->execute();
    $sql->store_result();
    if($sql->num_rows>0){
        $sql->close();
        ...
        if(isset($_POST['grandtotal'])){
            ...
            if (($infomainaccessuser['saleordercustinfo']=='defone')||($infomainaccessuser['saleordercustinfo']=='deftwo')) {
            ...
            if (($infomainaccessuser['saleordercustinfo']=='one')||($scriptonetwo=='one')) {
            ...
            if($access['defsaleorderpay']=='split'){
                $salesorderterm = 'CASH,CARD,GPAY';
            }
            else{
                $salesorderterm = htmlspecialchars($_POST['salesorderterm'], ENT_QUOTES, 'UTF-8');
            }
            $oldsalesorderno=htmlspecialchars( $_GET['salesorderno'], ENT_QUOTES, 'UTF-8');
            $oldsalesorderdate=htmlspecialchars( $_GET['salesorderdate'], ENT_QUOTES, 'UTF-8');
            $salesorderno=htmlspecialchars( $_POST['salesorderno'], ENT_QUOTES, 'UTF-8');
            $salesorderdate=htmlspecialchars( $_POST['salesorderdate'], ENT_QUOTES, 'UTF-8');
            $salesordertime=htmlspecialchars( $_POST['salesordertime'], ENT_QUOTES, 'UTF-8');
            ...
            //FOR GET THE CURRENT SALE ORDER NO
            ...
            if ((in_array('Table Number', $fieldedit))) {
                if(isset($_POST['tablenumber'])){
                    $tablenumber = htmlspecialchars((explode('|||',$_POST['tablenumber'])[0]), ENT_QUOTES, 'UTF-8');
                    $tableid = htmlspecialchars((explode('|||',$_POST['tablenumber'])[1]), ENT_QUOTES, 'UTF-8');
                }
                else{
                    $tablenumber='';
                    $tableid='';
            ...
            $salesorderamount=htmlspecialchars( $_POST['grandtotal'], ENT_QUOTES, 'UTF-8');
            ...
            if (($infomainaccessuser['saleordercustinfo']=='one')||($scriptonetwo=='one')) {
                ...
                $custsqlup = $con->prepare("UPDATE paircustomers SET createdon=?, createdid=?, createdby=?, franchisesession=?,billstreet=?,billcity=?,billstate=?,billpincode=?,billcountry=?,shipstreet=?,shipcity=?,shipstate=?,shippincode=?,shipcountry=?,gstrtype=?,gstin=?,workphone=?,mobile=? WHERE id=? AND createdid=? AND moduletype='Customers' AND (franchisesession=? OR cvisiblity='PUBLIC')");
                $custsqlup->bind_param("ssssssssssssssssssiss", $times, $companymainid, $_SESSION["unqwerty"], $_SESSION["franchisesession"], $area, $city, $state, $pincode, $district, $sarea, $scity, $sstate, $spincode, $sdistrict, $gstrtype, $custgstin, $custworkphone, $custmobilephone, $custid, $companymainid, $_SESSION["franchisesession"]);
                $custsqlup->execute();
                $custsqlup->close();
                $custch='';
            ...
            $twoage = '';
            $twosex = '';
            if (($infomainaccessuser['saleordercustinfo']=='two')||($scriptonetwo=='two')) {
                $address1 = '';
                $address2 = '';
                $saddress1 = '';
                $saddress2 = '';
            ...
            for($i=0; $i<count($_POST['oldproductname']); $i++){
                ...
                if($productname!=''){
                    $sql = $con->prepare("DELETE FROM pairsalesorders WHERE createdid=? AND franchisesession=? AND salesorderno=? AND salesorderdate=?");
                    $sql->bind_param("ssss", $companymainid, $_SESSION["franchisesession"], $oldsalesorderno, $oldsalesorderdate);
                    $sql->execute();
                    // $sqldelledger = $con->prepare("DELETE FROM pairledgers WHERE createdid=? AND franchisesession=? AND ledgerno=? AND ledgerdate=? AND (type='salesorder' OR type='salesorder payment')");
                    // $sqldelledger->bind_param("ssss", $companymainid, $_SESSION["franchisesession"], $oldsalesorderno, $oldsalesorderdate);
                    // $sqldelledger->execute();
                    $sqls = $con->prepare("DELETE FROM pairsalespayments WHERE receiptno=? AND receiptdate=?");
                    $sqls->bind_param("ss", $oldsalesorderno, $oldsalesorderdate);
                    $sqls->execute();
                    $sqls->close();
                    $sqlsant = $con->prepare("DELETE FROM pairsalespayhistory WHERE invoiceno=? AND invoicedate=? AND type='salesorder'");
                    $sqlsant->bind_param("ss", $oldsalesorderno, $oldsalesorderdate);
                    $sqlsant->execute();
                    $sqlsant->close();
                    //  margins     
            ...
            $sql2=$con->prepare("SELECT id FROM pairsalesorders WHERE franchisesession=? AND createdid=? AND salesorderno=? AND salesorderdate=?");
            $sql2->bind_param("iiss", $_SESSION['franchisesession'], $companymainid, $oldsalesorderno, $oldsalesorderdate);
            $sql2->execute();
            $sql2->store_result();
            //FOR CHECK THE SALE ORDER IS IN OR NOT
            if($sql2->num_rows==0){
                for($i=0; $i<count($_POST['productname']); $i++){
                    ...
                    // $sqlaccmanuals->close();
                    if($productname!=''){
                        $sql = $con->prepare("INSERT INTO pairsalesorders (createdon,createdid,createdby,franchisesession,customername,customerid,address1,address2,area,city,state,district,pincode,saddress1,saddress2,sarea,scity,sstate,sdistrict,spincode,gstno,salesorderterm,salesorderno,salesorderdate,salesorderamount,duedate,duedates,productid,productname,manufacturer,producthsn,productnotes,productdescription,itemmodule,rack,batch,expdate,mrp,vat,quantity,unit,productrate,noofpacks,prodiscount,productvalue,taxvalue,cgstvat,sgstvat,productnetvalue,totalitems,orderdcno,saleperson,totalvatamount,totalamount,totalquantity,discounttype,discount,discountamount,freightamount,roundoff,grandtotal,taxtype,tax,cgst,sgst,igst,gst,gstpercent,csgstpercent,terms,notes,description,fileattach,pos,workphone,mobile,sameasbilling,customerinfodefault,gstrtype,validpaidamount,validbalance,icsgsthis,prodiscounttype,marginupdates,margintotalvalue,age,sex,accountid,accountname,salesordertime,cashreceived,cardreceived,gpayreceived,tablenumber,reference,preparedby,checkedby,tableid) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,?)");
                        $sql->bind_param("sssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssisssssssssi", $times, $companymainid, $_SESSION["unqwerty"], $_SESSION["franchisesession"], $customername, $customerid, $address1, $address2, $area, $city, $state, $district, $pincode, $saddress1, $saddress2, $sarea, $scity, $sstate, $sdistrict, $spincode, $gstno, $salesorderterm, $salesorderno, $salesorderdate, $salesorderamount, $duedate, $duedates, $productid, $productname, $manufacturer, $producthsn, $productnotes, $productdescription, $itemmodule, $rack, $batch, $expdate, $mrp, $vat, $quantity, $unit, $productrate, $noofpacks, $prodiscount, $productvalue, $taxvalue,$cgstvat,$sgstvat, $productnetvalue, $totalitems, $orderdcno, $saleperson, $totalvatamount, $totalamount, $totalquantity, $discounttype, $discount, $discountamount, $freightamount, $roundoff, $grandtotal, $taxtype, $tax, $cgst, $sgst, $igst, $gst, $gstpercent, $csgstpercent, $terms, $notes, $description, $fileattach, $pos, $twoworkphone, $twomobilephone, $twosameasbilling, $scriptonetwo, $gstrtype, $validpaidamount, $validbalance, $ansforsepgstval, $prodiscounttype, $marginupdates, $margintotalvalue, $twoage, $twosex, $chartaccountid, $accountname, $salesordertime, $cashreceived, $cardreceived, $gpayreceived, $tablenumber, $reference, $preparedby, $checkedby, $tableid);
                        //FOR INSERT THE NEW SALE ORDER
                        if($sql->execute()){
                            $sql->close();
                            $salesid=$con->insert_id;
                            // if($salesorderterm=='CASH'){
                            ...
                            //                 $sqlaccdefault->bind_param("sisisssssssssss", $times, $companymainid, $_SESSION["unqwerty"], $_SESSION["franchisesession"], $salesorderdate, $salesorderno, $defaccountnamediscount, $defaccountiddiscount, $customerid, $customername, $prodisvalueforledger, $prodisvalueforledger, $grandtotal, $publicidmanual, $privateidmanual);
                            ...
                            //             $sqlaccdefault = $con->prepare("INSERT INTO pairledgers (createdon,createdid,createdby,franchisesession,ledgerdate,ledgerno,chartaccountname,chartaccountid,customerid,customername,ledgerdebit,ledgercredit,subledgerdebit,subledgercredit,totalledgerdebit,totalledgercredit,balanceledgerdebit,balanceledgercredit,publicid,privateid,type) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, '0', ?, '0', ?, '0', ?, '0', '0', ?, ?, 'salesorder')");
                            //             $sqlaccdefault->bind_param("sisisssssssssss", $times, $companymainid, $_SESSION["unqwerty"], $_SESSION["franchisesession"], $salesorderdate, $salesorderno, $defaccountnametds, $defaccountidtds, $customerid, $customername, $taxvalue, $taxvalue, $grandtotal, $publicidmanual, $privateidmanual);
                            //             $sqlaccdefault->execute();
                            //             $sqlaccdefault->close();
                            //         }
                            ...
                            //         $sqlaccdefault = $con->prepare("INSERT INTO pairledgers (createdon,createdid,createdby,franchisesession,ledgerdate,ledgerno,chartaccountname,chartaccountid,customerid,customername,ledgerdebit,ledgercredit,subledgerdebit,subledgercredit,totalledgerdebit,totalledgercredit,balanceledgerdebit,balanceledgercredit,publicid,privateid,type) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, '0', ?, '0', ?, '0', ?, '0', '0', ?, ?, 'salesorder')");
                            //         $sqlaccdefault->bind_param("sisisssssssssss", $times, $companymainid, $_SESSION["unqwerty"], $_SESSION["franchisesession"], $salesorderdate, $salesorderno, $defaccountname, $defaccountid, $customerid, $customername, $productrate, $productrate, $grandtotal, $publicidmanual, $privateidmanual);
                            //         $sqlaccdefault->execute();
                            //         $sqlaccdefault->close();
                            ...
                            //         $sqlaccinspay = $con->prepare("INSERT INTO pairledgers (createdon,createdid,createdby,franchisesession,ledgerdate,ledgerno,chartaccountname,chartaccountid,customerid,customername,ledgerdebit,ledgercredit,subledgerdebit,subledgercredit,totalledgerdebit,totalledgercredit,balanceledgerdebit,balanceledgercredit,publicid,privateid,type) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, '0', ?, '0', ?, '0', '0', '0', ?, ?, 'salesorder payment')");
                            //         $sqlaccinspay->bind_param("sisisssssssssss", $times, $companymainid, $_SESSION["unqwerty"], $_SESSION["franchisesession"], $salesorderdate, $salesorderno, $defaccountnamepettycashpay, $defaccountidpettycashpay, $customerid, $customername, $productnetvalue, $productnetvalue, $grandtotal, $publicidmanual, $privateidmanual);
                            ...
                            //         $sqlaccdefaultpay->bind_param("sisisssssssssss", $times, $companymainid, $_SESSION["unqwerty"], $_SESSION["franchisesession"], $salesorderdate, $salesorderno, $defaccountnamepay, $defaccountidpay, $customerid, $customername, $productnetvalue, $productnetvalue, $grandtotal, $publicidmanual, $privateidmanual);
                            //         $sqlaccdefaultpay->execute();
                            //         $sqlaccdefaultpay->close();
                            //     }
                            // //FOR INSERT THE MANUAL JOURNAL
                            // }
                            // elseif($salesorderterm=='CASH,CARD,GPAY'){
                            //     $sqlismodulespublicnamemanuals=$con->prepare("SELECT * FROM pairmodules WHERE moduletype='Manual Journals' ORDER BY id ASC");
                            //     $sqlismodulespublicnamemanuals->execute();
                            //     $sqlismodulespublicnamemanual = $sqlismodulespublicnamemanuals->get_result();
                            //     $infomodulespublicnamemanual=$sqlismodulespublicnamemanual->fetch_array();
                            ...
                            //             $sqlaccinspay = $con->prepare("INSERT INTO pairledgers (createdon,createdid,createdby,franchisesession,ledgerdate,ledgerno,chartaccountname,chartaccountid,customerid,customername,ledgerdebit,ledgercredit,subledgerdebit,subledgercredit,totalledgerdebit,totalledgercredit,balanceledgerdebit,balanceledgercredit,publicid,privateid,type) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, '0', ?, '0', ?, '0', '0', '0', ?, ?, 'salesorder payment')");
                            //             $sqlaccinspay->bind_param("sisisssssssssss", $times, $companymainid, $_SESSION["unqwerty"], $_SESSION["franchisesession"], $salesorderdate, $salesorderno, $defaccountnamepettycashpay, $defaccountidpettycashpay, $customerid, $customername, $cashreceived, $cashreceived, $grandtotal, $publicidmanual, $privateidmanual);
                            //             $sqlaccinspay->execute();
                            //             $sqlaccinspay->close();
                            //             $sqlaccdefaultcashpays=$con->prepare("SELECT accountname,id FROM pairchartaccountings WHERE accountname='Accounts Receivable'");
                            //             $sqlaccdefaultcashpays->execute();
                            ...
                            //             $sqlaccdefaultpay = $con->prepare("INSERT INTO pairledgers (createdon,createdid,createdby,franchisesession,ledgerdate,ledgerno,chartaccountname,chartaccountid,customerid,customername,ledgerdebit,ledgercredit,subledgerdebit,subledgercredit,totalledgerdebit,totalledgercredit,balanceledgerdebit,balanceledgercredit,publicid,privateid,type) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, '0', ?, '0', ?, '0', ?, '0', '0', ?, ?, 'salesorder payment')");
                            //             $sqlaccdefaultpay->bind_param("sisisssssssssss", $times, $companymainid, $_SESSION["unqwerty"], $_SESSION["franchisesession"], $salesorderdate, $salesorderno, $defaccountnamepay, $defaccountidpay, $customerid, $customername, $cashreceived, $cashreceived, $grandtotal, $publicidmanual, $privateidmanual);
                            //             $sqlaccdefaultpay->execute();
                            //             $sqlaccdefaultpay->close();
                            //         }
                            //         if ($cardreceived>0) {
                            ...
                            //             $sqlaccinspay = $con->prepare("INSERT INTO pairledgers (createdon,createdid,createdby,franchisesession,ledgerdate,ledgerno,chartaccountname,chartaccountid,customerid,customername,ledgerdebit,ledgercredit,subledgerdebit,subledgercredit,totalledgerdebit,totalledgercredit,balanceledgerdebit,balanceledgercredit,publicid,privateid,type) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, '0', ?, '0', ?, '0', '0', '0', ?, ?, 'salesorder payment')");
                            //             $sqlaccinspay->bind_param("sisisssssssssss", $times, $companymainid, $_SESSION["unqwerty"], $_SESSION["franchisesession"], $salesorderdate, $salesorderno, $defaccountnamepettycashpay, $defaccountidpettycashpay, $customerid, $customername, $cardreceived, $cardreceived, $grandtotal, $publicidmanual, $privateidmanual);
                            //             $sqlaccinspay->execute();
                            //             $sqlaccinspay->close();
                            //             $sqlaccdefaultcashpays=$con->prepare("SELECT accountname,id FROM pairchartaccountings WHERE accountname='Accounts Receivable'");
                            //             $sqlaccdefaultcashpays->execute();
                            ...
                            //             $sqlaccdefaultpay = $con->prepare("INSERT INTO pairledgers (createdon,createdid,createdby,franchisesession,ledgerdate,ledgerno,chartaccountname,chartaccountid,customerid,customername,ledgerdebit,ledgercredit,subledgerdebit,subledgercredit,totalledgerdebit,totalledgercredit,balanceledgerdebit,balanceledgercredit,publicid,privateid,type) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, '0', ?, '0', ?, '0', ?, '0', '0', ?, ?, 'salesorder payment')");
                            //             $sqlaccdefaultpay->bind_param("sisisssssssssss", $times, $companymainid, $_SESSION["unqwerty"], $_SESSION["franchisesession"], $salesorderdate, $salesorderno, $defaccountnamepay, $defaccountidpay, $customerid, $customername, $cardreceived, $cardreceived, $grandtotal, $publicidmanual, $privateidmanual);
                            //             $sqlaccdefaultpay->execute();
                            //             $sqlaccdefaultpay->close();
                            //         }
                            //         if ($gpayreceived>0) {
                            ...
                            //             $sqlaccinspay = $con->prepare("INSERT INTO pairledgers (createdon,createdid,createdby,franchisesession,ledgerdate,ledgerno,chartaccountname,chartaccountid,customerid,customername,ledgerdebit,ledgercredit,subledgerdebit,subledgercredit,totalledgerdebit,totalledgercredit,balanceledgerdebit,balanceledgercredit,publicid,privateid,type) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, '0', ?, '0', ?, '0', '0', '0', ?, ?, 'salesorder payment')");
                            //             $sqlaccinspay->bind_param("sisisssssssssss", $times, $companymainid, $_SESSION["unqwerty"], $_SESSION["franchisesession"], $salesorderdate, $salesorderno, $defaccountnamepettycashpay, $defaccountidpettycashpay, $customerid, $customername, $gpayreceived, $gpayreceived, $grandtotal, $publicidmanual, $privateidmanual);
                            //             $sqlaccinspay->execute();
                            //             $sqlaccinspay->close();
                            //             $sqlaccdefaultcashpays=$con->prepare("SELECT accountname,id FROM pairchartaccountings WHERE accountname='Accounts Receivable'");
                            //             $sqlaccdefaultcashpays->execute();
                            ...
                            //             $sqlaccdefaultpay = $con->prepare("INSERT INTO pairledgers (createdon,createdid,createdby,franchisesession,ledgerdate,ledgerno,chartaccountname,chartaccountid,customerid,customername,ledgerdebit,ledgercredit,subledgerdebit,subledgercredit,totalledgerdebit,totalledgercredit,balanceledgerdebit,balanceledgercredit,publicid,privateid,type) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, '0', ?, '0', ?, '0', ?, '0', '0', ?, ?, 'salesorder payment')");
                            //             $sqlaccdefaultpay->bind_param("sisisssssssssss", $times, $companymainid, $_SESSION["unqwerty"], $_SESSION["franchisesession"], $salesorderdate, $salesorderno, $defaccountnamepay, $defaccountidpay, $customerid, $customername, $gpayreceived, $gpayreceived, $grandtotal, $publicidmanual, $privateidmanual);
                            //             $sqlaccdefaultpay->execute();
                            //             $sqlaccdefaultpay->close();
                            //         }
                            //     }
                                ...
                                if (strpos($runtheans[$ans], '|||')!==false) {
                                    ...
                                    if($selbillmargins->num_rows>0){
                                        ...
                                        $reduceqty = $runtheansnxt[4];
                                            $updbillmargins = $con->prepare("UPDATE pairmargins SET quantity=quantity - ? WHERE billerid=? AND productid=? AND createdid=? AND franchisesession=? AND type='buying' AND batch=? AND expiry=? AND billingno=? AND billingdate=? AND nowstatus='added'");
                                            $updbillmargins->bind_param("siiiissss", $reduceqty, $runtheansnxt[0], $productid, $companymainid, $_SESSION["franchisesession"], $batch, $expdate, $runtheansnxt[2], $runtheansnxt[3]);
                                            $updbillmargins->execute();
                                            $updbillmargins->close();
                                            $sqlsellingmargins = $con->prepare("INSERT INTO pairmargins (createdon,createdid,franchisesession,type,billername,billerid,billingno,billingdate,productid,productname,batch,expiry,quantity,mrp,rate,nowstatus) VALUES (?, ?, ?, 'selling', ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'added')");
                                            $sqlsellingmargins->bind_param("siisississssss", $times, $companymainid, $_SESSION["franchisesession"], $runtheansnxt[1], $runtheansnxt[0], $runtheansnxt[2], $runtheansnxt[3], $productid, $productname, $batch, $expdate, $reduceqty, $mrp, $productrate);
                ...
                $tid=$con->insert_id;
                if(($salesorderterm=='CASH')||($salesorderterm=='CASH,CARD,GPAY')){
                    if ($validpaidamount!=0) {
                        ...
                        $sqlselects->bind_param("ss", $salesorderno, $salesorderdate);
                        $sqlselects->execute();
                        ...
                        if ($sqlselects->num_rows==0) {
                            ...
                            // $sqlsalepayins = $con->prepare("INSERT INTO pairsalespayments (createdid,createdby,franchisesession,createdon,term,type,customername,customerid,receiptno,receiptdate,amount,paymentmode,notes,publicid,privateid) VALUES (?, ?, ?, ?, 'RECEIPT', 'salesorder', ?, ?, ?, ?, ?, ?, '-', ?, ?)");
                            // $sqlsalepayins->bind_param("ssssssssssss", $companymainid, $_SESSION["unqwerty"], $_SESSION["franchisesession"], $createdon, $customername, $customerid, $salesorderno, $salesorderdate, $validpaidamount, $salesorderterm, $publiccode, $privatecode);
                            // $sqlsalepayins->execute();
                            // if($sqlsalepayins){
                            //     $sqlsalepayins->close();
                            //     $paymentid=$con->insert_id;
                            //     $sqle = $con->prepare("INSERT INTO pairsalespayhistory (createdid,createdby,franchisesession,createdon,paymentid,customerid,invoiceno,invoicedate,amount,type) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'salesorder')");
                            //     $sqle->bind_param("sssssssss", $companymainid, $_SESSION["unqwerty"], $_SESSION["franchisesession"], $createdon, $paymentid, $customerid, $salesorderno, $salesorderdate, $validpaidamount);
                            //     $sqle->execute();
                            //     $sqle->close();
                            // }
                            ...
                            // $sqlbatchupd = $con->prepare("UPDATE pairsalespayments SET createdid=?, createdby=?, franchisesession=?, createdon=?,  term='RECEIPT', type='salesorder', customername=?, customerid=?, receiptno=?, receiptdate=?, amount=?, paymentmode=?, notes='-' WHERE receiptno=? AND receiptdate=?");
                            // $sqlbatchupd->bind_param("ssssssssssss", $companymainid, $_SESSION["unqwerty"], $_SESSION["franchisesession"], $createdon, $customername, $customerid, $salesorderno, $salesorderdate, $validpaidamount, $salesorderterm, $salesorderno, $salesorderdate);
                            // $sqlbatchupd->execute();
                            // if($sqlbatchupd){
                            //     $sqlbatchupd->close();
                            //     $sqle = $con->prepare("UPDATE pairsalespayhistory SET amount=? WHERE invoiceno=? AND invoicedate=? AND type='salesorder'");
                            //     $sqle->bind_param("sss", $validpaidamount, $salesorderno, $salesorderdate);
                            //     $sqle->execute();
                            //     $sqle->close();
                            // }
                        ...
                        // $sqlbatchupdant = $con->prepare("UPDATE pairsalespayments SET createdid=?, createdby=?, franchisesession=?, createdon=?,  term='RECEIPT', type='salesorder', customername=?, customerid=?, receiptno=?, receiptdate=?, amount=?, paymentmode=?, notes='-' WHERE receiptno=? AND receiptdate=?");
                        // $sqlbatchupdant->bind_param("ssssssssssss", $companymainid, $_SESSION["unqwerty"], $_SESSION["franchisesession"], $createdon, $customername, $customerid, $salesorderno, $salesorderdate, $validpaidamount, $salesorderterm, $salesorderno, $salesorderdate);
                        // $sqlbatchupdant->execute();
                        // if($sqlbatchupdant){
                        //     $sqlbatchupdant->close();
                        //     $sqleant = $con->prepare("UPDATE pairsalespayhistory SET amount=? WHERE invoiceno=? AND invoicedate=? AND type='salesorder'");
                        //     $sqleant->bind_param("sss", $validpaidamount, $salesorderno, $salesorderdate);
                        //     $sqleant->execute();
                        //     $sqleant->close();
                        // }
                ...
                elseif($salesorderterm=='CREDIT'){
                    $sqls = $con->prepare("DELETE FROM pairsalespayments WHERE receiptno=? AND receiptdate=?");
                    $sqls->bind_param("ss", $salesorderno, $salesorderdate);
                    $sqls->execute();
                    $sqls->close();
                    $sqlsant = $con->prepare("DELETE FROM pairsalespayhistory WHERE invoiceno=? AND invoicedate=? AND type='salesorder'");
                    $sqlsant->bind_param("ss", $salesorderno, $salesorderdate);
                    $sqlsant->execute();
                    $sqlsant->close();
                    //FOR UPDATE THE PAYMENT STATUS UNPAID
                    // $sqldelledgerpay = $con->prepare("DELETE FROM pairledgers WHERE createdid=? AND franchisesession=? AND ledgerno=? AND ledgerdate=? AND type='salesorder payment'");
                    // $sqldelledgerpay->bind_param("ssss", $companymainid, $_SESSION["franchisesession"], $salesorderno, $salesorderdate);
                    // $sqldelledgerpay->execute();
                }
                if($sql){
                    // $sqlisalesorders=$con->prepare("SELECT salesorderamount, salesorderdate, salesorderno FROM pairsalesorders WHERE franchisesession=? AND createdid=? AND customerid=? GROUP BY salesorderdate, salesorderno ORDER BY salesorderdate DESC, salesorderno DESC");
                    // $sqlisalesorders->bind_param("sss", $_SESSION['franchisesession'], $companymainid, $customerid);
                    // $sqlisalesorders->execute();
                    // $salesorderamount=0;
                    // $balanceamount=0;
                    // $currentamount=0;
                    // $overdueamount=0;
                    // $sqlisalesorder = $sqlisalesorders->get_result();
                    // while($infosalesorder=$sqlisalesorder->fetch_array()){
                    //     $salesorderamount+=(float)$infosalesorder['salesorderamount'];
                    //     $paidamount=0;
                    ...
                    //     $sqlsalespays->bind_param("sssss", $_SESSION['franchisesession'], $companymainid, $infosalesorder['salesorderno'], $infosalesorder['salesorderdate'], $customerid);
                    ...
                    //     }
                    //     $balanceamount+=((float)$infosalesorder['salesorderamount']-$paidamount);
                    //     $diff = abs(time() - strtotime($infosalesorder['salesorderdate']));
                    //     $days = floor(($diff)/ (60*60*24));
                    //     if($days>30){
                    //         $overdueamount+=((float)$infosalesorder['salesorderamount']-$paidamount);
                    //     }
                    //     else{
                    //         $currentamount+=((float)$infosalesorder['salesorderamount']-$paidamount);
                    //     }
                    // }
                    // $cussqlup = $con->prepare("UPDATE paircustomers SET invoiceamount=?,balanceamount=?,currentamount=?,overdueamount=? WHERE id=?");
                    // $cussqlup->bind_param("sssss", $salesorderamount, $balanceamount, $currentamount, $overdueamount, $customerid);
                    // $cussqlup->execute();
                    // $cussqlup->close();
                    if(isset($_POST['convertoinvoices'])){
                        echo '<script> window.open("converttoinvoice.php?salesorderno='.$salesorderno.'&salesorderdate='.$salesorderdate.'&type=salesorder", "_self");</script>';
                    }
                    else{
                        echo '<script> window.location.href="salesorderview.php?id='.$salesid.'&salesorderno='.$salesorderno.'&salesorderdate='.$salesorderdate.'&remarks=Updated Successfully";</script>';
                    }
                }
                //FOR CUSTOMER BALANCE UPDATION DETAILS
            }
            else{
                header("Location: salesorders.php?error=Error Data");
            }
            if ($sql) {
                $oldessalesorderno = htmlspecialchars( $_POST['oldessalesorderno'], ENT_QUOTES, 'UTF-8');
                $oldessalesorderdate = htmlspecialchars( $_POST['oldessalesorderdate'], ENT_QUOTES, 'UTF-8');
                ...
                $oldestablenumber = htmlspecialchars( $_POST['oldestablenumber'], ENT_QUOTES, 'UTF-8');
                ...
                $oldessalesorderterm = htmlspecialchars( $_POST['oldessalesorderterm'], ENT_QUOTES, 'UTF-8');
                ...
                if($salesorderno!=$oldessalesorderno){
                    if($saleorderinfoch!=''){
                        $saleorderinfoch.='<br> '.$infomainaccessuser['modulename'].' Number <span style="color:green;" id="prohisfromtospan">( From '.$oldessalesorderno.' To '.$salesorderno.' ) </span>';
                    }
                    else{
                        $saleorderinfoch.='<span style="color:royalblue;">'.$infomainaccessuser['modulename'].' Information</span> <br> '.$infomainaccessuser['modulename'].' Number <span style="color:green;" id="prohisfromtospan">( From '.$oldessalesorderno.' To '.$salesorderno.' ) </span>';
                    }                   
                }
                if($salesorderdate!=$oldessalesorderdate){
                    if($saleorderinfoch!=''){
                        $saleorderinfoch.='<br> '.$infomainaccessuser['modulename'].' Date <span style="color:green;" id="prohisfromtospan">( From '.date($date,strtotime($oldessalesorderdate)).' To '.date($date,strtotime($salesorderdate)).' ) </span>';
                    }
                    else{
                        $saleorderinfoch.='<span style="color:royalblue;">'.$infomainaccessuser['modulename'].' Information</span> <br> '.$infomainaccessuser['modulename'].' Date <span style="color:green;" id="prohisfromtospan">( From '.date($date,strtotime($oldessalesorderdate)).' To '.date($date,strtotime($salesorderdate)).' ) </span>';
                    }                   
                }
                if($reference!=$oldesreference){
                    if($saleorderinfoch!=''){
                        $saleorderinfoch.='<br> Reference <span style="color:green;" id="prohisfromtospan">( From '.$oldesreference.' To '.$reference.' ) </span>';
                    }
                    else{
                        $saleorderinfoch.='<span style="color:royalblue;">'.$infomainaccessuser['modulename'].' Information</span> <br> Reference <span style="color:green;" id="prohisfromtospan">( From '.$oldesreference.' To '.$reference.' ) </span>';
                    }                   
                }
                if($tablenumber!=$oldestablenumber){
                    if($saleorderinfoch!=''){
                        $saleorderinfoch.='<br> '.$access['txttablenumber'].' <span style="color:green;" id="prohisfromtospan">( From '.$oldestablenumber.' To '.$tablenumber.' ) </span>';
                    }
                    else{
                        $saleorderinfoch.='<span style="color:royalblue;">'.$infomainaccessuser['modulename'].' Information</span> <br> '.$access['txttablenumber'].' <span style="color:green;" id="prohisfromtospan">( From '.$oldestablenumber.' To '.$tablenumber.' ) </span>';
                    }                   
                }
                if($saleperson!=$oldessaleperson){
                    if($saleorderinfoch!=''){
                        $saleorderinfoch.='<br> '.$access['txtsaleperson'].' <span style="color:green;" id="prohisfromtospan">( From '.$oldessaleperson.' To '.$saleperson.' ) </span>';
                    }
                    else{
                        $saleorderinfoch.='<span style="color:royalblue;">'.$infomainaccessuser['modulename'].' Information</span> <br> '.$access['txtsaleperson'].' <span style="color:green;" id="prohisfromtospan">( From '.$oldessaleperson.' To '.$saleperson.' ) </span>';
                    }                   
                }
                if($preparedby!=$oldespreparedby){
                    if($saleorderinfoch!=''){
                        $saleorderinfoch.='<br> Prepared By <span style="color:green;" id="prohisfromtospan">( From '.$oldespreparedby.' To '.$preparedby.' ) </span>';
                    }
                    else{
                        $saleorderinfoch.='<span style="color:royalblue;">'.$infomainaccessuser['modulename'].' Information</span> <br> Prepared By <span style="color:green;" id="prohisfromtospan">( From '.$oldespreparedby.' To '.$preparedby.' ) </span>';
                    }                   
                }
                if($checkedby!=$oldescheckedby){
                    if($saleorderinfoch!=''){
                        $saleorderinfoch.='<br> Checked By <span style="color:green;" id="prohisfromtospan">( From '.$oldescheckedby.' To '.$checkedbfy.' ) </span>';
                    }
                    else{
                        $saleorderinfoch.='<span style="color:royalblue;">'.$infomainaccessuser['modulename'].' Information</span> <br> Checked By <span style="color:green;" id="prohisfromtospan">( From '.$oldescheckedby.' To '.$checkedby.' ) </span>';
                    }                   
                }
                //FOR SALE ORDER INFORMATIONS HISTORY DETAILS
                $saleordercustinfoch = '';
                ...
                if($customername!=$oldescustomername){
                    if($saleordercustinfoch!=''){
                        $saleordercustinfoch.='<br> '.$infomainaccessusercus['modulename'].' Name <span style="color:green;" id="prohisfromtospan">( From '.$oldescustomername.' To '.$customername.' ) </span>';
                    }
                    else{
                        $saleordercustinfoch.=''.(($saleorderinfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessusercus['modulename'].' Information</span> <br> '.$infomainaccessusercus['modulename'].' Name <span style="color:green;" id="prohisfromtospan">( From '.$oldescustomername.' To '.$customername.' ) </span>';
                    }                   
                }
                if($twoage!=$oldesage){
                    if($saleordercustinfoch!=''){
                        $saleordercustinfoch.='<br> AGE <span style="color:green;" id="prohisfromtospan">( From '.$oldesage.' To '.$twoage.' ) </span>';
                    }
                    else{
                        $saleordercustinfoch.=''.(($saleorderinfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessusercus['modulename'].' Information</span> <br> AGE <span style="color:green;" id="prohisfromtospan">( From '.$oldesage.' To '.$twoage.' ) </span>';
                    }                   
                }
                if($twosex!=$oldessex){
                    if($saleordercustinfoch!=''){
                        $saleordercustinfoch.='<br> SEX <span style="color:green;" id="prohisfromtospan">( From '.$oldessex.' To '.$twosex.' ) </span>';
                    }
                    else{
                        $saleordercustinfoch.=''.(($saleorderinfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessusercus['modulename'].' Information</span> <br> SEX <span style="color:green;" id="prohisfromtospan">( From '.$oldessex.' To '.$twosex.' ) </span>';
                    }                   
                }
                if($area!=$oldesarea){
                    if($saleordercustinfoch!=''){
                        $saleordercustinfoch.='<br> BILLING STREET <span style="color:green;" id="prohisfromtospan">( From '.$oldesarea.' To '.$area.' ) </span>';
                    }
                    else{
                        $saleordercustinfoch.=''.(($saleorderinfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessusercus['modulename'].' Information</span> <br> BILLING STREET <span style="color:green;" id="prohisfromtospan">( From '.$oldesarea.' To '.$area.' ) </span>';
                    }                   
                }
                if($city!=$oldescity){
                    if($saleordercustinfoch!=''){
                        $saleordercustinfoch.='<br> BILLING CITY/TOWN <span style="color:green;" id="prohisfromtospan">( From '.$oldescity.' To '.$city.' ) </span>';
                    }
                    else{
                        $saleordercustinfoch.=''.(($saleorderinfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessusercus['modulename'].' Information</span> <br> BILLING CITY/TOWN <span style="color:green;" id="prohisfromtospan">( From '.$oldescity.' To '.$city.' ) </span>';
                    }                   
                }
                if($state!=$oldesstate){
                    if($saleordercustinfoch!=''){
                        $saleordercustinfoch.='<br> BILLING STATE <span style="color:green;" id="prohisfromtospan">( From '.$oldesstate.' To '.$state.' ) </span>';
                    }
                    else{
                        $saleordercustinfoch.=''.(($saleorderinfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessusercus['modulename'].' Information</span> <br> BILLING STATE <span style="color:green;" id="prohisfromtospan">( From '.$oldesstate.' To '.$state.' ) </span>';
                    }                   
                }
                if($district!=$oldesdistrict){
                    if($saleordercustinfoch!=''){
                        $saleordercustinfoch.='<br> BILLING PIN <span style="color:green;" id="prohisfromtospan">( From '.$oldesdistrict.' To '.$district.' ) </span>';
                    }
                    else{
                        $saleordercustinfoch.=''.(($saleorderinfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessusercus['modulename'].' Information</span> <br> BILLING PIN <span style="color:green;" id="prohisfromtospan">( From '.$oldesdistrict.' To '.$district.' ) </span>';
                    }                   
                }
                if($pincode!=$oldespincode){
                    if($saleordercustinfoch!=''){
                        $saleordercustinfoch.='<br> BILLING COUNTRY/REGION <span style="color:green;" id="prohisfromtospan">( From '.$oldespincode.' To '.$pincode.' ) </span>';
                    }
                    else{
                        $saleordercustinfoch.=''.(($saleorderinfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessusercus['modulename'].' Information</span> <br> BILLING COUNTRY/REGION <span style="color:green;" id="prohisfromtospan">( From '.$oldespincode.' To '.$pincode.' ) </span>';
                    }                   
                }
                if($sarea!=$oldessarea){
                    if($saleordercustinfoch!=''){
                        $saleordercustinfoch.='<br> SHIPPING STREET <span style="color:green;" id="prohisfromtospan">( From '.$oldessarea.' To '.$sarea.' ) </span>';
                    }
                    else{
                        $saleordercustinfoch.=''.(($saleorderinfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessusercus['modulename'].' Information</span> <br> SHIPPING STREET <span style="color:green;" id="prohisfromtospan">( From '.$oldessarea.' To '.$sarea.' ) </span>';
                    }                   
                }
                if($scity!=$oldesscity){
                    if($saleordercustinfoch!=''){
                        $saleordercustinfoch.='<br> SHIPPING CITY/TOWN <span style="color:green;" id="prohisfromtospan">( From '.$oldesscity.' To '.$scity.' ) </span>';
                    }
                    else{
                        $saleordercustinfoch.=''.(($saleorderinfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessusercus['modulename'].' Information</span> <br> SHIPPING CITY/TOWN <span style="color:green;" id="prohisfromtospan">( From '.$oldesscity.' To '.$scity.' ) </span>';
                    }                   
                }
                if($sstate!=$oldessstate){
                    if($saleordercustinfoch!=''){
                        $saleordercustinfoch.='<br> SHIPPING STATE <span style="color:green;" id="prohisfromtospan">( From '.$oldessstate.' To '.$sstate.' ) </span>';
                    }
                    else{
                        $saleordercustinfoch.=''.(($saleorderinfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessusercus['modulename'].' Information</span> <br> SHIPPING STATE <span style="color:green;" id="prohisfromtospan">( From '.$oldessstate.' To '.$sstate.' ) </span>';
                    }                   
                }
                if($sdistrict!=$oldessdistrict){
                    if($saleordercustinfoch!=''){
                        $saleordercustinfoch.='<br> SHIPPING PIN <span style="color:green;" id="prohisfromtospan">( From '.$oldessdistrict.' To '.$sdistrict.' ) </span>';
                    }
                    else{
                        $saleordercustinfoch.=''.(($saleorderinfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessusercus['modulename'].' Information</span> <br> SHIPPING PIN <span style="color:green;" id="prohisfromtospan">( From '.$oldessdistrict.' To '.$sdistrict.' ) </span>';
                    }                   
                }
                if($spincode!=$oldesspincode){
                    if($saleordercustinfoch!=''){
                        $saleordercustinfoch.='<br> SHIPPING COUNTRY/REGION <span style="color:green;" id="prohisfromtospan">( From '.$oldesspincode.' To '.$spincode.' ) </span>';
                    }
                    else{
                        $saleordercustinfoch.=''.(($saleorderinfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessusercus['modulename'].' Information</span> <br> SHIPPING COUNTRY/REGION <span style="color:green;" id="prohisfromtospan">( From '.$oldesspincode.' To '.$spincode.' ) </span>';
                    }                   
                }
                if($twoworkphone!=$oldestwoworkphone){
                    if($saleordercustinfoch!=''){
                        $saleordercustinfoch.='<br> WORK PHONE <span style="color:green;" id="prohisfromtospan">( From '.$oldestwoworkphone.' To '.$twoworkphone.' ) </span>';
                    }
                    else{
                        $saleordercustinfoch.=''.(($saleorderinfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessusercus['modulename'].' Information</span> <br> WORK PHONE <span style="color:green;" id="prohisfromtospan">( From '.$oldestwoworkphone.' To '.$twoworkphone.' ) </span>';
                    }                   
                }
                if($twomobilephone!=$oldestwomobilephone){
                    if($saleordercustinfoch!=''){
                        $saleordercustinfoch.='<br> MOBILE PHONE <span style="color:green;" id="prohisfromtospan">( From '.$oldestwomobilephone.' To '.$twomobilephone.' ) </span>';
                    }
                    else{
                        $saleordercustinfoch.=''.(($saleorderinfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessusercus['modulename'].' Information</span> <br> MOBILE PHONE <span style="color:green;" id="prohisfromtospan">( From '.$oldestwomobilephone.' To '.$twomobilephone.' ) </span>';
                    }                   
                }
                if($gstrtype!=$oldesgstrtype){
                    if($saleordercustinfoch!=''){
                        $saleordercustinfoch.='<br> GST TREATMENT <span style="color:green;" id="prohisfromtospan">( From '.$oldesgstrtype.' To '.$gstrtype.' ) </span>';
                    }
                    else{
                        $saleordercustinfoch.=''.(($saleorderinfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessusercus['modulename'].' Information</span> <br> GST TREATMENT <span style="color:green;" id="prohisfromtospan">( From '.$oldesgstrtype.' To '.$gstrtype.' ) </span>';
                    }                   
                }
                if($gstno!=$oldesgstno){
                    if($saleordercustinfoch!=''){
                        $saleordercustinfoch.='<br> GSTIN <span style="color:green;" id="prohisfromtospan">( From '.$oldesgstno.' To '.$gstno.' ) </span>';
                    }
                    else{
                        $saleordercustinfoch.=''.(($saleorderinfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessusercus['modulename'].' Information</span> <br> GSTIN <span style="color:green;" id="prohisfromtospan">( From '.$oldesgstno.' To '.$gstno.' ) </span>';
                    }                   
                }
                if($pos!=$oldespos){
                    if($saleordercustinfoch!=''){
                        $saleordercustinfoch.='<br> PLACE OF SUPPLY <span style="color:green;" id="prohisfromtospan">( From '.$oldespos.' To '.$pos.' ) </span>';
                    }
                    else{
                        $saleordercustinfoch.=''.(($saleorderinfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessusercus['modulename'].' Information</span> <br> PLACE OF SUPPLY <span style="color:green;" id="prohisfromtospan">( From '.$oldespos.' To '.$pos.' ) </span>';
                    }                   
                }
                //FOR SALE ORDER CUSTOMER INFORMATIONS DETAILS HISTORY
                $saleorderiteminfoch = '';
                    ...
                    for($i=0; $i<count($_POST['oldesproductname']); $i++){
                        ...
                        if($productname!=''){
                            if($productname!=$oldesproductname){
                                if($saleorderiteminfoch!=''){
                                    $saleorderiteminfoch.='<br> '.$infomainaccessuserpro['modulename'].' Name <span style="color:green;" id="prohisfromtospan">( From '.$oldesproductname.' To '.$productname.' ) </span>';
                                }
                                else{
                                    $saleorderiteminfoch.=''.(($saleorderinfoch!=''||$saleordercustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> '.$infomainaccessuserpro['modulename'].' Name <span style="color:green;" id="prohisfromtospan">( From '.$oldesproductname.' To '.$productname.' ) </span>';
                                }                   
                            }
                            if($manufacturer!=$oldesmanufacturer){
                                if($saleorderiteminfoch!=''){
                                    $saleorderiteminfoch.='<br> '.$access['txtnamecategory'].' <span style="color:green;" id="prohisfromtospan">( From '.$oldesmanufacturer.' To '.$manufacturer.' ) </span>';
                                }
                                else{
                                    $saleorderiteminfoch.=''.(($saleorderinfoch!=''||$saleordercustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> '.$access['txtnamecategory'].' <span style="color:green;" id="prohisfromtospan">( From '.$oldesmanufacturer.' To '.$manufacturer.' ) </span>';
                                }                   
                            }
                            if($producthsn!=$oldesproducthsn){
                                if($saleorderiteminfoch!=''){
                                    $saleorderiteminfoch.='<br> HSN Code <span style="color:green;" id="prohisfromtospan">( From '.$oldesproducthsn.' To '.$producthsn.' ) </span>';
                                }
                                else{
                                    $saleorderiteminfoch.=''.(($saleorderinfoch!=''||$saleordercustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> HSN Code <span style="color:green;" id="prohisfromtospan">( From '.$oldesproducthsn.' To '.$producthsn.' ) </span>';
                                }                   
                            }
                            if($productdescription!=$oldesproductdescription){
                                if($saleorderiteminfoch!=''){
                                    $saleorderiteminfoch.='<br> Description <span style="color:green;" id="prohisfromtospan">( From '.$oldesproductdescription.' To '.$productdescription.' ) </span>';
                                }
                                else{
                                    $saleorderiteminfoch.=''.(($saleorderinfoch!=''||$saleordercustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Description <span style="color:green;" id="prohisfromtospan">( From '.$oldesproductdescription.' To '.$productdescription.' ) </span>';
                                }                   
                            }
                            if($batch!=$oldesbatch){
                                if($saleorderiteminfoch!=''){
                                    $saleorderiteminfoch.='<br> Batch <span style="color:green;" id="prohisfromtospan">( From '.$oldesbatch.' To '.$batch.' ) </span>';
                                }
                                else{
                                    $saleorderiteminfoch.=''.(($saleorderinfoch!=''||$saleordercustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Batch <span style="color:green;" id="prohisfromtospan">( From '.$oldesbatch.' To '.$batch.' ) </span>';
                                }                   
                            }
                            if($expdate!=$oldesexpdate){
                                if($saleorderiteminfoch!=''){
                                    $saleorderiteminfoch.='<br> Expiry <span style="color:green;" id="prohisfromtospan">( From '.date($date,strtotime($oldesexpdate)).' To '.date($date,strtotime($expdate)).' ) </span>';
                                }
                                else{
                                    $saleorderiteminfoch.=''.(($saleorderinfoch!=''||$saleordercustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Expiry <span style="color:green;" id="prohisfromtospan">( From '.date($date,strtotime($oldesexpdate)).' To '.date($date,strtotime($expdate)).' ) </span>';
                                }                   
                            }
                            if($productrate!=$oldesproductrate){
                                if($saleorderiteminfoch!=''){
                                    $saleorderiteminfoch.='<br> Rate <span style="color:green;" id="prohisfromtospan">( From '.$oldesproductrate.' To '.$productrate.' ) </span>';
                                }
                                else{
                                    $saleorderiteminfoch.=''.(($saleorderinfoch!=''||$saleordercustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Rate <span style="color:green;" id="prohisfromtospan">( From '.$oldesproductrate.' To '.$productrate.' ) </span>';
                                }                   
                            }
                            if($mrp!=$oldesmrp){
                                if($saleorderiteminfoch!=''){
                                    $saleorderiteminfoch.='<br> Mrp <span style="color:green;" id="prohisfromtospan">( From '.$oldesmrp.' To '.$mrp.' ) </span>';
                                }
                                else{
                                    $saleorderiteminfoch.=''.(($saleorderinfoch!=''||$saleordercustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Mrp <span style="color:green;" id="prohisfromtospan">( From '.$oldesmrp.' To '.$mrp.' ) </span>';
                                }                   
                            }
                            if($quantity!=$oldesquantity){
                                if($saleorderiteminfoch!=''){
                                    $saleorderiteminfoch.='<br> Quantity <span style="color:green;" id="prohisfromtospan">( From '.$oldesquantity.' To '.$quantity.' ) </span>';
                                }
                                else{
                                    $saleorderiteminfoch.=''.(($saleorderinfoch!=''||$saleordercustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Quantity <span style="color:green;" id="prohisfromtospan">( From '.$oldesquantity.' To '.$quantity.' ) </span>';
                                }                   
                            }
                            if($unit!=$oldesunit){
                                if($saleorderiteminfoch!=''){
                                    $saleorderiteminfoch.='<br> Unit <span style="color:green;" id="prohisfromtospan">( From '.$oldesunit.' To '.$unit.' ) </span>';
                                }
                                else{
                                    $saleorderiteminfoch.=''.(($saleorderinfoch!=''||$saleordercustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Unit <span style="color:green;" id="prohisfromtospan">( From '.$oldesunit.' To '.$unit.' ) </span>';
                                }                   
                            }
                            if($noofpacks!=$oldesnoofpacks){
                                if($saleorderiteminfoch!=''){
                                    $saleorderiteminfoch.='<br> Pack <span style="color:green;" id="prohisfromtospan">( From '.$oldesnoofpacks.' To '.$noofpacks.' ) </span>';
                                }
                                else{
                                    $saleorderiteminfoch.=''.(($saleorderinfoch!=''||$saleordercustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Pack <span style="color:green;" id="prohisfromtospan">( From '.$oldesnoofpacks.' To '.$noofpacks.' ) </span>';
                                }                   
                            }
                            if($productvalue!=$oldesproductvalue){
                                if($saleorderiteminfoch!=''){
                                    $saleorderiteminfoch.='<br> Taxable Value <span style="color:green;" id="prohisfromtospan">( From '.$oldesproductvalue.' To '.$productvalue.' ) </span>';
                                }
                                else{
                                    $saleorderiteminfoch.=''.(($saleorderinfoch!=''||$saleordercustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Taxable Value <span style="color:green;" id="prohisfromtospan">( From '.$oldesproductvalue.' To '.$productvalue.' ) </span>';
                                }                   
                            }
                            if($prodiscount!=$oldesprodiscount){
                                if($saleorderiteminfoch!=''){
                                    $saleorderiteminfoch.='<br> '.$access['txtprodissaleorder'].' <span style="color:green;" id="prohisfromtospan">( From '.$oldesprodiscount.' To '.$prodiscount.' ) </span>';
                                }
                                else{
                                    $saleorderiteminfoch.=''.(($saleorderinfoch!=''||$saleordercustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> '.$access['txtprodissaleorder'].' <span style="color:green;" id="prohisfromtospan">( From '.$oldesprodiscount.' To '.$prodiscount.' ) </span>';
                                }                   
                            }
                            if($prodiscounttype!=$oldesprodiscounttype){
                                if($saleorderiteminfoch!=''){
                                    $saleorderiteminfoch.='<br> Discounted By <span style="color:green;" id="prohisfromtospan">( From '.(($oldesprodiscounttype==0)?'%':'?').' To '.(($prodiscounttype==0)?'%':'?').' ) </span>';
                                }
                                else{
                                    $saleorderiteminfoch.=''.(($saleorderinfoch!=''||$saleordercustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Discounted By <span style="color:green;" id="prohisfromtospan">( From '.(($oldesprodiscounttype==0)?'%':'?').' To '.(($prodiscounttype==0)?'%':'?').' ) </span>';
                                }                   
                            }
                            if($taxvalue!=$oldestaxvalue){
                                if($saleorderiteminfoch!=''){
                                    $saleorderiteminfoch.='<br> Tax Value <span style="color:green;" id="prohisfromtospan">( From '.$oldestaxvalue.' To '.$taxvalue.' ) </span>';
                                }
                                else{
                                    $saleorderiteminfoch.=''.(($saleorderinfoch!=''||$saleordercustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Tax Value <span style="color:green;" id="prohisfromtospan">( From '.$oldestaxvalue.' To '.$taxvalue.' ) </span>';
                                }                   
                            }
                            if($vat!=$oldesvat){
                                if($saleorderiteminfoch!=''){
                                    $saleorderiteminfoch.='<br> Gst <span style="color:green;" id="prohisfromtospan">( From '.$oldesvat.' % ('.$oldesicsgsthis.') To '.$vat.' % ('.$ansforsepgstval.') ) </span>';
                                }
                                else{
                                    $saleorderiteminfoch.=''.(($saleorderinfoch!=''||$saleordercustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Gst <span style="color:green;" id="prohisfromtospan">( From '.$oldesvat.' % ('.$oldesicsgsthis.') To '.$vat.' % ('.$ansforsepgstval.') ) </span>';
                                }                   
                            }
                            if($productnetvalue!=$oldesproductnetvalue){
                                if($saleorderiteminfoch!=''){
                                    $saleorderiteminfoch.='<br> Amount <span style="color:green;" id="prohisfromtospan">( From '.$oldesproductnetvalue.' To '.$productnetvalue.' ) </span>';
                                }
                                else{
                                    $saleorderiteminfoch.=''.(($saleorderinfoch!=''||$saleordercustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Amount <span style="color:green;" id="prohisfromtospan">( From '.$oldesproductnetvalue.' To '.$productnetvalue.' ) </span>';
                                }                   
                            }
                        }
                    }
                    ...
                    for($i=0; $i<count($_POST['oldesproductname']); $i++){
                        ...
                        if($productname!=''){
                            ...
                            if($oldesproductname!=''){                    
                                if($oldesproductname!=''){
                                    if($saleorderiteminfoch!=''){
                                        $saleorderiteminfoch.='<br> '.$infomainaccessuserpro['modulename'].' Name <span style="color:red;" id="prohisfromtospan">( From '.$oldesproductname.' To &nbsp; ) </span>';
                                    }
                                    else{
                                        $saleorderiteminfoch.=''.(($saleorderinfoch!=''||$saleordercustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> '.$infomainaccessuserpro['modulename'].' Name <span style="color:red;" id="prohisfromtospan">( From '.$oldesproductname.' To &nbsp; ) </span>';
                                    }
                                }
                                if($oldesmanufacturer!=''){
                                    if($saleorderiteminfoch!=''){
                                        $saleorderiteminfoch.='<br> '.$access['txtnamecategory'].' <span style="color:red;" id="prohisfromtospan">( From '.$oldesmanufacturer.' To &nbsp; ) </span>';
                                    }
                                    else{
                                        $saleorderiteminfoch.=''.(($saleorderinfoch!=''||$saleordercustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> '.$access['txtnamecategory'].' <span style="color:red;" id="prohisfromtospan">( From '.$oldesmanufacturer.' To &nbsp; ) </span>';
                                    }
                                }
                                if($oldesproducthsn!=''){
                                    if($saleorderiteminfoch!=''){
                                        $saleorderiteminfoch.='<br> HSN Code <span style="color:red;" id="prohisfromtospan">( From '.$oldesproducthsn.' To &nbsp; ) </span>';
                                    }
                                    else{
                                        $saleorderiteminfoch.=''.(($saleorderinfoch!=''||$saleordercustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> HSN Code <span style="color:red;" id="prohisfromtospan">( From '.$oldesproducthsn.' To &nbsp; ) </span>';
                                    }
                                }
                                if($oldesproductdescription!=''){
                                    if($saleorderiteminfoch!=''){
                                        $saleorderiteminfoch.='<br> Description <span style="color:red;" id="prohisfromtospan">( From '.$oldesproductdescription.' To &nbsp; ) </span>';
                                    }
                                    else{
                                        $saleorderiteminfoch.=''.(($saleorderinfoch!=''||$saleordercustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Description <span style="color:red;" id="prohisfromtospan">( From '.$oldesproductdescription.' To &nbsp; ) </span>';
                                    }
                                }
                                if($oldesbatch!=''){
                                    if($saleorderiteminfoch!=''){
                                        $saleorderiteminfoch.='<br> Batch <span style="color:red;" id="prohisfromtospan">( From '.$oldesbatch.' To &nbsp; ) </span>';
                                    }
                                    else{
                                        $saleorderiteminfoch.=''.(($saleorderinfoch!=''||$saleordercustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Batch <span style="color:red;" id="prohisfromtospan">( From '.$oldesbatch.' To &nbsp; ) </span>';
                                    }
                                }
                                if($oldesexpdate!=''){
                                    if($saleorderiteminfoch!=''){
                                        $saleorderiteminfoch.='<br> Expiry <span style="color:red;" id="prohisfromtospan">( From '.date($date,strtotime($oldesexpdate)).' To &nbsp; ) </span>';
                                    }
                                    else{
                                        $saleorderiteminfoch.=''.(($saleorderinfoch!=''||$saleordercustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Expiry <span style="color:red;" id="prohisfromtospan">( From '.date($date,strtotime($oldesexpdate)).' To &nbsp; ) </span>';
                                    }
                                }
                                if($oldesproductrate!=''){
                                    if($saleorderiteminfoch!=''){
                                        $saleorderiteminfoch.='<br> Rate <span style="color:red;" id="prohisfromtospan">( From '.$oldesproductrate.' To &nbsp; ) </span>';
                                    }
                                    else{
                                        $saleorderiteminfoch.=''.(($saleorderinfoch!=''||$saleordercustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Rate <span style="color:red;" id="prohisfromtospan">( From '.$oldesproductrate.' To &nbsp; ) </span>';
                                    }
                                }
                                if($oldesmrp!=''){
                                    if($saleorderiteminfoch!=''){
                                        $saleorderiteminfoch.='<br> Mrp <span style="color:red;" id="prohisfromtospan">( From '.$oldesmrp.' To &nbsp; ) </span>';
                                    }
                                    else{
                                        $saleorderiteminfoch.=''.(($saleorderinfoch!=''||$saleordercustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Mrp <span style="color:red;" id="prohisfromtospan">( From '.$oldesmrp.' To &nbsp; ) </span>';
                                    }
                                }
                                if($oldesquantity!=''){
                                    if($saleorderiteminfoch!=''){
                                        $saleorderiteminfoch.='<br> Quantity <span style="color:red;" id="prohisfromtospan">( From '.$oldesquantity.' To &nbsp; ) </span>';
                                    }
                                    else{
                                        $saleorderiteminfoch.=''.(($saleorderinfoch!=''||$saleordercustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Quantity <span style="color:red;" id="prohisfromtospan">( From '.$oldesquantity.' To &nbsp; ) </span>';
                                    }
                                }
                                if($oldesunit!=''){
                                    if($saleorderiteminfoch!=''){
                                        $saleorderiteminfoch.='<br> Unit <span style="color:red;" id="prohisfromtospan">( From '.$oldesunit.' To &nbsp; ) </span>';
                                    }
                                    else{
                                        $saleorderiteminfoch.=''.(($saleorderinfoch!=''||$saleordercustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Unit <span style="color:red;" id="prohisfromtospan">( From '.$oldesunit.' To &nbsp; ) </span>';
                                    }
                                }
                                if($oldesnoofpacks!=''){
                                    if($saleorderiteminfoch!=''){
                                        $saleorderiteminfoch.='<br> Pack <span style="color:red;" id="prohisfromtospan">( From '.$oldesnoofpacks.' To &nbsp; ) </span>';
                                    }
                                    else{
                                        $saleorderiteminfoch.=''.(($saleorderinfoch!=''||$saleordercustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Pack <span style="color:red;" id="prohisfromtospan">( From '.$oldesnoofpacks.' To &nbsp; ) </span>';
                                    }
                                }
                                if($oldesproductvalue!=''){
                                    if($saleorderiteminfoch!=''){
                                        $saleorderiteminfoch.='<br> Taxable Value <span style="color:red;" id="prohisfromtospan">( From '.$oldesproductvalue.' To &nbsp; ) </span>';
                                    }
                                    else{
                                        $saleorderiteminfoch.=''.(($saleorderinfoch!=''||$saleordercustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Taxable Value <span style="color:red;" id="prohisfromtospan">( From '.$oldesproductvalue.' To &nbsp; ) </span>';
                                    }
                                }
                                if($oldesprodiscount!=''){
                                    if($saleorderiteminfoch!=''){
                                        $saleorderiteminfoch.='<br> Discount <span style="color:red;" id="prohisfromtospan">( From '.$oldesprodiscount.' To &nbsp; ) </span>';
                                    }
                                    else{
                                        $saleorderiteminfoch.=''.(($saleorderinfoch!=''||$saleordercustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Discount <span style="color:red;" id="prohisfromtospan">( From '.$oldesprodiscount.' To &nbsp; ) </span>';
                                    }
                                }
                                if($oldestaxvalue!=''){
                                    if($saleorderiteminfoch!=''){
                                        $saleorderiteminfoch.='<br> Tax Value <span style="color:red;" id="prohisfromtospan">( From '.$oldestaxvalue.' To &nbsp; ) </span>';
                                    }
                                    else{
                                        $saleorderiteminfoch.=''.(($saleorderinfoch!=''||$saleordercustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Tax Value <span style="color:red;" id="prohisfromtospan">( From '.$oldestaxvalue.' To &nbsp; ) </span>';
                                    }
                                }
                                if($oldesvat!=''){
                                    if($saleorderiteminfoch!=''){
                                        $saleorderiteminfoch.='<br> Gst <span style="color:red;" id="prohisfromtospan">( From '.$oldesvat.' % ('.$oldesicsgsthis.') To &nbsp; ) </span>';
                                    }
                                    else{
                                        $saleorderiteminfoch.=''.(($saleorderinfoch!=''||$saleordercustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Gst <span style="color:red;" id="prohisfromtospan">( From '.$oldesvat.' % ('.$oldesicsgsthis.') To &nbsp; ) </span>';
                                    }
                                }
                                if($oldesproductnetvalue!=''){
                                    if($saleorderiteminfoch!=''){
                                        $saleorderiteminfoch.='<br> Amount <span style="color:red;" id="prohisfromtospan">( From '.$oldesproductnetvalue.' To &nbsp; ) </span>';
                                    }
                                    else{
                                        $saleorderiteminfoch.=''.(($saleorderinfoch!=''||$saleordercustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Amount <span style="color:red;" id="prohisfromtospan">( From '.$oldesproductnetvalue.' To &nbsp; ) </span>';
                                    }
                                }
                            }
                        }
                ...
                elseif ($curans>$oldans) {
                    for($i=0; $i<count($_POST['productname']); $i++){
                        ...
                        if($oldesproductname!=''){
                            ...
                            if($productname!=''){
                                if($saleorderiteminfoch!=''){
                                    $saleorderiteminfoch.='<br> '.$infomainaccessuserpro['modulename'].' Name <span style="color:green;" id="prohisfromtospan">( '.$productname.' ) </span>';
                                }
                                else{
                                    $saleorderiteminfoch.=''.(($saleorderinfoch!=''||$saleordercustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> '.$infomainaccessuserpro['modulename'].' Name <span style="color:green;" id="prohisfromtospan">( '.$productname.' ) </span>';        
                                }
                            }
                            if($manufacturer!=''){
                                if($saleorderiteminfoch!=''){
                                    $saleorderiteminfoch.='<br> '.$access['txtnamecategory'].' <span style="color:green;" id="prohisfromtospan">( '.$manufacturer.' ) </span>';
                                }
                                else{
                                    $saleorderiteminfoch.=''.(($saleorderinfoch!=''||$saleordercustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> '.$access['txtnamecategory'].' <span style="color:green;" id="prohisfromtospan">( '.$manufacturer.' ) </span>';        
                                }
                            }
                            if($producthsn!=''){
                                if($saleorderiteminfoch!=''){
                                    $saleorderiteminfoch.='<br> HSN Code <span style="color:green;" id="prohisfromtospan">( '.$producthsn.' ) </span>';
                                }
                                else{
                                    $saleorderiteminfoch.=''.(($saleorderinfoch!=''||$saleordercustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> HSN Code <span style="color:green;" id="prohisfromtospan">( '.$producthsn.' ) </span>';        
                                }
                            }
                            if($productdescription!=''){
                                if($saleorderiteminfoch!=''){
                                    $saleorderiteminfoch.='<br> Description <span style="color:green;" id="prohisfromtospan">( '.$productdescription.' ) </span>';
                                }
                                else{
                                    $saleorderiteminfoch.=''.(($saleorderinfoch!=''||$saleordercustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Description <span style="color:green;" id="prohisfromtospan">( '.$productdescription.' ) </span>';        
                                }
                            }
                            if($batch!=''){
                                if($saleorderiteminfoch!=''){
                                    $saleorderiteminfoch.='<br> Batch <span style="color:green;" id="prohisfromtospan">( '.$batch.' ) </span>';
                                }
                                else{
                                    $saleorderiteminfoch.=''.(($saleorderinfoch!=''||$saleordercustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Batch <span style="color:green;" id="prohisfromtospan">( '.$batch.' ) </span>';        
                                }
                            }
                            if($expdate!=''){
                                if($saleorderiteminfoch!=''){
                                    $saleorderiteminfoch.='<br> Expiry <span style="color:green;" id="prohisfromtospan">( '.date($date,strtotime($expdate)).' ) </span>';
                                }
                                else{
                                    $saleorderiteminfoch.=''.(($saleorderinfoch!=''||$saleordercustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Expiry <span style="color:green;" id="prohisfromtospan">( '.date($date,strtotime($expdate)).' ) </span>';        
                                }
                            }
                            if($productrate!=''){
                                if($saleorderiteminfoch!=''){
                                    $saleorderiteminfoch.='<br> Rate <span style="color:green;" id="prohisfromtospan">( '.$productrate.' ) </span>';
                                }
                                else{
                                    $saleorderiteminfoch.=''.(($saleorderinfoch!=''||$saleordercustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Rate <span style="color:green;" id="prohisfromtospan">( '.$productrate.' ) </span>';        
                                }
                            }
                            if($mrp!=''){
                                if($saleorderiteminfoch!=''){
                                    $saleorderiteminfoch.='<br> Mrp <span style="color:green;" id="prohisfromtospan">( '.$mrp.' ) </span>';
                                }
                                else{
                                    $saleorderiteminfoch.=''.(($saleorderinfoch!=''||$saleordercustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Mrp <span style="color:green;" id="prohisfromtospan">( '.$mrp.' ) </span>';        
                                }
                            }
                            if($quantity!=''){
                                if($saleorderiteminfoch!=''){
                                    $saleorderiteminfoch.='<br> Quantity <span style="color:green;" id="prohisfromtospan">( '.$quantity.' ) </span>';
                                }
                                else{
                                    $saleorderiteminfoch.=''.(($saleorderinfoch!=''||$saleordercustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Quantity <span style="color:green;" id="prohisfromtospan">( '.$quantity.' ) </span>';        
                                }
                            }
                            if($unit!=''){
                                if($saleorderiteminfoch!=''){
                                    $saleorderiteminfoch.='<br> Unit <span style="color:green;" id="prohisfromtospan">( '.$unit.' ) </span>';
                                }
                                else{
                                    $saleorderiteminfoch.=''.(($saleorderinfoch!=''||$saleordercustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Unit <span style="color:green;" id="prohisfromtospan">( '.$unit.' ) </span>';        
                                }
                            }
                            if($noofpacks!=''){
                                if($saleorderiteminfoch!=''){
                                    $saleorderiteminfoch.='<br> Pack <span style="color:green;" id="prohisfromtospan">( '.$noofpacks.' ) </span>';
                                }
                                else{
                                    $saleorderiteminfoch.=''.(($saleorderinfoch!=''||$saleordercustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Pack <span style="color:green;" id="prohisfromtospan">( '.$noofpacks.' ) </span>';        
                                }
                            }
                            if($productvalue!=''){
                                if($saleorderiteminfoch!=''){
                                    $saleorderiteminfoch.='<br> Taxable Value <span style="color:green;" id="prohisfromtospan">( '.$productvalue.' ) </span>';
                                }
                                else{
                                    $saleorderiteminfoch.=''.(($saleorderinfoch!=''||$saleordercustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Taxable Value <span style="color:green;" id="prohisfromtospan">( '.$productvalue.' ) </span>';        
                                }
                            }
                            if($prodiscount!=''){
                                if($saleorderiteminfoch!=''){
                                    $saleorderiteminfoch.='<br> '.$access['txtprodissaleorder'].' <span style="color:green;" id="prohisfromtospan">( '.$prodiscount.' ) </span>';
                                }
                                else{
                                    $saleorderiteminfoch.=''.(($saleorderinfoch!=''||$saleordercustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> '.$access['txtprodissaleorder'].' <span style="color:green;" id="prohisfromtospan">( '.$prodiscount.' ) </span>';        
                                }
                            }
                            if($prodiscounttype!=''){
                                if($saleorderiteminfoch!=''){
                                    $saleorderiteminfoch.='<br> Discounted By <span style="color:green;" id="prohisfromtospan">( '.(($prodiscounttype==0)?'%':'?').' ) </span>';
                                }
                                else{
                                    $saleorderiteminfoch.=''.(($saleorderinfoch!=''||$saleordercustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Discounted By <span style="color:green;" id="prohisfromtospan">( '.(($prodiscounttype==0)?'%':'?').' ) </span>';        
                                }
                            }
                            if($taxvalue!=''){
                                if($saleorderiteminfoch!=''){
                                    $saleorderiteminfoch.='<br> Tax Value <span style="color:green;" id="prohisfromtospan">( '.$taxvalue.' ) </span>';
                                }
                                else{
                                    $saleorderiteminfoch.=''.(($saleorderinfoch!=''||$saleordercustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Tax Value <span style="color:green;" id="prohisfromtospan">( '.$taxvalue.' ) </span>';        
                                }
                            }
                            if($vat!=''){
                                if($saleorderiteminfoch!=''){
                                    $saleorderiteminfoch.='<br> Gst <span style="color:green;" id="prohisfromtospan">( '.$vat.' % ('.$ansforsepgstval.') ) </span>';
                                }
                                else{
                                    $saleorderiteminfoch.=''.(($saleorderinfoch!=''||$saleordercustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Gst <span style="color:green;" id="prohisfromtospan">( '.$vat.' % ('.$ansforsepgstval.') ) </span>';        
                                }
                            }
                            if($productnetvalue!=''){
                                if($saleorderiteminfoch!=''){
                                    $saleorderiteminfoch.='<br> Amount <span style="color:green;" id="prohisfromtospan">( '.$productnetvalue.' ) </span>';
                                }
                                else{
                                    $saleorderiteminfoch.=''.(($saleorderinfoch!=''||$saleordercustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Amount <span style="color:green;" id="prohisfromtospan">( '.$productnetvalue.' ) </span>';        
                                }
                            }
                        }
                    }
                ...
                if($totalitems!=$oldestotalitems){
                    if($saleorderiteminfoch!=''){
                        $saleorderiteminfoch.='<br> Total Items <span style="color:green;" id="prohisfromtospan">( From '.$oldestotalitems.' To '.$totalitems.' ) </span>';
                    }
                    else{
                        $saleorderiteminfoch.=''.(($saleorderinfoch!=''||$saleordercustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Total Items <span style="color:green;" id="prohisfromtospan">( From '.$oldestotalitems.' To '.$totalitems.' ) </span>';
                    }
                }
                if($totalquantity!=$oldestotalquantity){
                    if($saleorderiteminfoch!=''){
                        $saleorderiteminfoch.='<br> Total Qty <span style="color:green;" id="prohisfromtospan">( From '.$oldestotalquantity.' To '.$totalquantity.' ) </span>';
                    }
                    else{
                        $saleorderiteminfoch.=''.(($saleorderinfoch!=''||$saleordercustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Total Qty <span style="color:green;" id="prohisfromtospan">( From '.$oldestotalquantity.' To '.$totalquantity.' ) </span>';
                    }
                }
                if($totalamount!=$oldestotalamount){
                    if($saleorderiteminfoch!=''){
                        $saleorderiteminfoch.='<br> Sub Total <span style="color:green;" id="prohisfromtospan">( From '.$oldestotalamount.' To '.$totalamount.' ) </span>';
                    }
                    else{
                        $saleorderiteminfoch.=''.(($saleorderinfoch!=''||$saleordercustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Sub Total <span style="color:green;" id="prohisfromtospan">( From '.$oldestotalamount.' To '.$totalamount.' ) </span>';
                    }
                }
                if($discountamount!=$oldesdiscountamount){
                    if($saleorderiteminfoch!=''){
                        $saleorderiteminfoch.='<br> Discount <span style="color:green;" id="prohisfromtospan">( From '.$oldesdiscountamount.' To '.$discountamount.' ) </span>';
                    }
                    else{
                        $saleorderiteminfoch.=''.(($saleorderinfoch!=''||$saleordercustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Discount <span style="color:green;" id="prohisfromtospan">( From '.$oldesdiscountamount.' To '.$discountamount.' ) </span>';
                    }
                }
                if($totalvatamount!=$oldestotalvatamount){
                    if($saleorderiteminfoch!=''){
                        $saleorderiteminfoch.='<br> Total Tax <span style="color:green;" id="prohisfromtospan">( From '.$oldestotalvatamount.' To '.$totalvatamount.' ) </span>';
                    }
                    else{
                        $saleorderiteminfoch.=''.(($saleorderinfoch!=''||$saleordercustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Total Tax <span style="color:green;" id="prohisfromtospan">( From '.$oldestotalvatamount.' To '.$totalvatamount.' ) </span>';
                    }
                }
                if($roundoff!=$oldesroundoff){
                    if($saleorderiteminfoch!=''){
                        $saleorderiteminfoch.='<br> Round Off <span style="color:green;" id="prohisfromtospan">( From '.$oldesroundoff.' To '.$roundoff.' ) </span>';
                    }
                    else{
                        $saleorderiteminfoch.=''.(($saleorderinfoch!=''||$saleordercustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Round Off <span style="color:green;" id="prohisfromtospan">( From '.$oldesroundoff.' To '.$roundoff.' ) </span>';
                    }
                }
                if($grandtotal!=$oldesgrandtotal){
                    if($saleorderiteminfoch!=''){
                        $saleorderiteminfoch.='<br> Grand Total <span style="color:green;" id="prohisfromtospan">( From '.$oldesgrandtotal.' To '.$grandtotal.' ) </span>';
                    }
                    else{
                        $saleorderiteminfoch.=''.(($saleorderinfoch!=''||$saleordercustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Grand Total <span style="color:green;" id="prohisfromtospan">( From '.$oldesgrandtotal.' To '.$grandtotal.' ) </span>';
                    }
                }
                if($description!=$oldesdescription){
                    if($saleorderiteminfoch!=''){
                        $saleorderiteminfoch.='<br> Description <span style="color:green;" id="prohisfromtospan">( From '.$oldesdescription.' To '.$description.' ) </span>';
                    }
                    else{
                        $saleorderiteminfoch.=''.(($saleorderinfoch!=''||$saleordercustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Description <span style="color:green;" id="prohisfromtospan">( From '.$oldesdescription.' To '.$description.' ) </span>';
                    }
                }
                if($notes!=$oldesnotes){
                    if($saleorderiteminfoch!=''){
                        $saleorderiteminfoch.='<br> Notes <span style="color:green;" id="prohisfromtospan">( From '.$oldesnotes.' To '.$notes.' ) </span>';
                    }
                    else{
                        $saleorderiteminfoch.=''.(($saleorderinfoch!=''||$saleordercustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Notes <span style="color:green;" id="prohisfromtospan">( From '.$oldesnotes.' To '.$notes.' ) </span>';
                    }
                }
                if($terms!=$oldesterms){
                    if($saleorderiteminfoch!=''){
                        $saleorderiteminfoch.='<br> Terms And Conditions <span style="color:green;" id="prohisfromtospan">( From '.$oldesterms.' To '.$terms.' ) </span>';
                    }
                    else{
                        $saleorderiteminfoch.=''.(($saleorderinfoch!=''||$saleordercustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Terms And Conditions <span style="color:green;" id="prohisfromtospan">( From '.$oldesterms.' To '.$terms.' ) </span>';
                    }
                }
                if($fileattach!=$oldesfileattach){
                    if($saleorderiteminfoch!=''){
                        $saleorderiteminfoch.='<br> Attach <span style="color:green;" id="prohisfromtospan">( Changed ) </span>';
                    }
                    else{
                        $saleorderiteminfoch.=''.(($saleorderinfoch!=''||$saleordercustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Attach <span style="color:green;" id="prohisfromtospan">( Changed ) </span>';
                    }
                }
                if($salesorderterm!=$oldessalesorderterm){
                    if($saleorderiteminfoch!=''){
                        $saleorderiteminfoch.='<br> Payment Method <span style="color:green;" id="prohisfromtospan">( From '.$oldessalesorderterm.' To '.$salesorderterm.' ) </span>';
                    }
                    else{
                        $saleorderiteminfoch.=''.(($saleorderinfoch!=''||$saleordercustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Payment Method <span style="color:green;" id="prohisfromtospan">( From '.$oldessalesorderterm.' To '.$salesorderterm.' ) </span>';
                    }
                }
                if($validpaidamount!=$oldesvalidpaidamount&&$salesorderterm=='CASH,CARD,GPAY'){
                    if($saleorderiteminfoch!=''){
                        $saleorderiteminfoch.='<br> '.(($access['defsaleorderpay']=='split')?'Amount Received':'Cash Received').' <span style="color:green;" id="prohisfromtospan">( From '.$oldesvalidpaidamount.' To '.$validpaidamount.' ) </span>';
                    }
                    else{
                        $saleorderiteminfoch.=''.(($saleorderinfoch!=''||$saleordercustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> '.(($access['defsaleorderpay']=='split')?'Amount Received':'Cash Received').' <span style="color:green;" id="prohisfromtospan">( From '.$oldesvalidpaidamount.' To '.$validpaidamount.' ) </span>';
                    }
                }
                if($validpaidamount!=$oldesvalidpaidamount&&$salesorderterm=='CASH'){
                    if($saleorderiteminfoch!=''){
                        $saleorderiteminfoch.='<br> '.(($access['defsaleorderpay']=='split')?'Amount Received':'Cash Received').' <span style="color:green;" id="prohisfromtospan">( From '.$oldesvalidpaidamount.' To '.$validpaidamount.' ) </span>';
                    }
                    else{
                ...
                if($validbalance!=$oldesvalidbalance&&$salesorderterm=='CASH'){
                    if($saleorderiteminfoch!=''){
                        $saleorderiteminfoch.='<br> Change Due <span style="color:green;" id="prohisfromtospan">( From '.$oldesvalidbalance.' To '.$validbalance.' ) </span>';
                    }
                    else{
                        $saleorderiteminfoch.=''.(($saleorderinfoch!=''||$saleordercustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Change Due <span style="color:green;" id="prohisfromtospan">( From '.$oldesvalidbalance.' To '.$validbalance.' ) </span>';
                    }
                }
                if($validbalance!=$oldesvalidbalance&&$salesorderterm=='CASH,CARD,GPAY'){
                    if($saleorderiteminfoch!=''){
                        $saleorderiteminfoch.='<br> Change Due <span style="color:green;" id="prohisfromtospan">( From '.$oldesvalidbalance.' To '.$validbalance.' ) </span>';
                    }
                    else{
                ...
                if($duedates!=$oldesduedates&&$salesorderterm=='CREDIT'){
                    if($saleorderiteminfoch!=''){
                        $saleorderiteminfoch.='<br> Due Term <span style="color:green;" id="prohisfromtospan">( From '.$oldesduedates.' To '.$duedates.' ) </span>';
                    }
                    else{
                        $saleorderiteminfoch.=''.(($saleorderinfoch!=''||$saleordercustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Due Term <span style="color:green;" id="prohisfromtospan">( From '.$oldesduedates.' To '.$duedates.' ) </span>';
                    }
                }
                if($duedate!=$oldesduedate&&$salesorderterm=='CREDIT'){
                    if($saleorderiteminfoch!=''){
                        $saleorderiteminfoch.='<br> Due Date <span style="color:green;" id="prohisfromtospan">( From '.date($date,strtotime($oldesduedate)).' To '.date($date,strtotime($duedate)).' ) </span>';
                    }
                    else{
                        $saleorderiteminfoch.=''.(($saleorderinfoch!=''||$saleordercustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Due Date <span style="color:green;" id="prohisfromtospan">( From '.date($date,strtotime($oldesduedate)).' To '.date($date,strtotime($duedate)).' ) </span>';
                    }
                }
                //FOR SALE ORDER OTHER INFORMATIONS LIKE PAYMENT AND TERMS AND DESCRIPTION......
                $saleordertotinfoch = $saleorderinfoch.$saleordercustinfoch.$saleorderiteminfoch;
                if($saleordertotinfoch!=''){
                    $sqluse = $con->prepare("INSERT INTO pairusehistory (usetype,createdon,createdby,useid,uniqueid,useremarks) VALUES ('SALE ORDERS', ?, ?, ?, ?, ?)");
                    $sqluse->bind_param("sssss", $times, $_SESSION["unqwerty"], $salesorderno, $salesid, $saleordertotinfoch);
                    $sqluse->execute();
                    $sqluse->close();
                }
                //FOR SALE ORDER HISTORY
                $sqlusehisupdqry = $con->prepare("UPDATE pairusehistory SET uniqueid=? WHERE useid=? AND uniqueid=?");
                $sqlusehisupdqry->bind_param("sss", $salesid, $salesorderno, $_GET['id']);
...
<main class="main-content position-relative max-height-vh-100 h-100 mt-0 " style="overflow-y: scroll !important;">
    ...
    <?php
        ...
        $sqlismainaccessusers=$con->prepare("SELECT moduleno, moduleprefix, modulesuffix,modulename FROM pairmainaccess WHERE franchiseid=? AND moduletype='Sales Orders' ORDER BY id ASC");
                ...
                <div class="col-12">
                    <div class="card mb-4 mt-5">
                        ...
                        ?>
                            ...
                            <!-- CUSTOMER ADD MODAL -->
                            <form autocomplete="off" id="salesorderform" action="" onsubmit="return checkvalidate()" method="post" enctype="multipart/form-data" class="form-horizontal" role="form">
                            ...
                            <?php
                                $oldarray=array( 'salesorderno','salesorderdate','reference','saleperson','preparedby','checkedby','customername','age','sex','area','city','state','district','pincode','sarea','scity','sstate','sdistrict','spincode','workphone','mobile','gstrtype','gstno','pos','tablenumber' );
                            ...
                            ?>
                                ...
                                <input type="hidden" value="<?=$rows[0]['salesorderterm']?>" name="oldessalesorderterm">
                                ...
                                <input type="hidden" name="oldsalesorderno" value="<?=$rows[0]['salesorderno']?>">
                                <input type="hidden" name="oldsalesorderdate" value="<?=$rows[0]['salesorderdate']?>">
                                <input type="hidden" name="salesorderterm" value="<?=$rows[0]['salesorderterm']?>">
                                <div class="row">
                                <?php
                                    // if ((in_array('Sales Order Information', $fieldedit))) {
                                ?>
                                    <div class="col-lg-4">
                                                        ...
                                                        <div class="row">
                                                            <div class="col-lg-12">
                                                                <div class="form-group row" style="align-items: center !important;">
                                                                    <div class="col-lg-5">
                                                                        <label for="salesorderno" class="custom-label">
                                                                    ...
                                                                    <div class="col-lg-7">
                                                                        <input type="text" class="form-control  form-control-sm" id="salesorderno" name="salesorderno" required  value="<?=$rows[0]['salesorderno']?>" readonly>
                                                                    </div>
                                                                </div>
                                                            </div>
                                                            <div class="col-lg-12">
                                                                <div class="form-group row" style="align-items: center !important;">
                                                                    <div class="col-lg-5">
                                                                        <label for="salesorderdate" class="custom-label">
                                                                            <span class="text-danger">
                                                                    ...
                                                                    <?php
                                                                        ...
                                                                        $dateformats->close();
                                                                    ?>
                                                                        <input type="date" class="form-control  form-control-sm" id="salesorderdate" name="salesorderdate" required value="<?=$rows[0]['salesorderdate']?>">
                                                                    </div>
                                                                </div>
                                                            </div>
                                                            <div class="col-lg-12" <?=((in_array('Sales Order Time', $fieldedit))?'':'style="display:none;"')?>>
                                                                <div class="form-group row" style="align-items: center !important;">
                                                                    <div class="col-lg-5">
                                                                        <label for="salesordertime" class="custom-label">
                                                                            <span class="text-danger">
                                                                    ...
                                                                    <div class="col-lg-7">
                                                                        ...
                                                                        ?>
                                                                        <input type="time" class="form-control form-control-sm" id="salesordertime" name="salesordertime" required value="<?=$rows[0]['salesordertime']?>">
                                                                    </div>
                                                                </div>
                                                            </div>
                                                            <div class="col-lg-12" <?=((in_array('Table Number', $fieldedit))?'':'style="display:none;"')?>>
                                                                <div class="form-group row" style="align-items: center !important;">
                                                                    <div class="col-lg-5">
                                                                        <label for="tablenumber" class="custom-label">
                                                                            <?=$access['txttablenumber']?>
                                                                        </label>
                                                                    </div>
                                                                    <div class="col-lg-7">
                                                                        <select class="select2-field form-control  form-control-sm" name="tablenumber" id="tablenumber">
                                                                            <option selected disabled>Select</option>
                                                                        <?php
                                                                            $sqlis=$con->prepare("SELECT tablenumber,id FROM pairtables WHERE (createdid=? OR createdid='0') ORDER BY tablenumber ASC");
                                                                            $sqlis->bind_param("s", $companymainid);
                                                                            $sqlis->execute();
                                                                            $sqli = $sqlis->get_result();
                                                                            while($info=$sqli->fetch_array()){
                                                                        ?>
                                                                            <option value="<?= $info['tablenumber'].'|||'.$info['id'] ?>" <?=($rows[0]['tableid']==$info['id'])?'selected':''?>><?= $info['tablenumber'] ?></option>
                                                                        <?php
                                                                            }
                                                                            $sqli->close();
                                                                            $sqlis->close();
                                            ...
                                            <div class="accordion-item mb-1">
                                                        ...
                                                        <div class="row" style="width:100% !important;">
                                                            ...
                                                            ?>
                                                                ...
                                                                $(document).ready(function () {
                                                                    ...
                                                                    <?php
                                                                        if ($access['saleorderbtwocnamerequired']=='Yes') {
                                                                    ?>
                                                                        $("#twocustomername").removeAttr("required");
                                                                    <?php
                                                                        }
                                                                        if ($access['saleorderbtwocwphonerequired']=='Yes') {
                                                                    ?>
                                                ...
                                                <div id="collapseOne" class="accordion-collapse collapse show" aria-labelledby="headingOne" style="padding-left:7px !important;padding-right: 7px !important;">
                                                    ...
                                                    <?php
                                                        ...
                                                        if ($infomainaccessuser['saleordercustinfo']=='one'||(($infomainaccessuser['saleordercustinfo']=='defone')||($infomainaccessuser['saleordercustinfo']=='deftwo'))) {
                                                    ?>
                                                        <div id="one">
                                                            <div class="row">
                                                                <div class="col-lg-12">
                                                    ...
                                                    <?php
                                                        ...
                                                        if ($infomainaccessuser['saleordercustinfo']=='two'||(($infomainaccessuser['saleordercustinfo']=='defone')||($infomainaccessuser['saleordercustinfo']=='deftwo'))) {
                                                    ?>
                                                        <script type="text/javascript">
                                                        function twoposans() {
                                                        ...
                                                        <div id="two">
                                                            <div class="row">
                                                                <div class="<?=((!in_array('Age', $fieldedit))&&(!in_array('Sex', $fieldedit)))?'col-lg-3':'col-lg-2'?>" style="padding-right: 0px;">
                                                                    <label for="customername" class="custom-label">
                                                                        <?= ($access['saleorderbtwocnamerequired']=='Yes')? '<span class="text-danger">'. $infomainaccessusercus['modulename'] . ' Name * </span>': $infomainaccessusercus['modulename'] . ' Name ' ?>
                                                                    </label>
                                                                </div>
                                                                <div class="<?=((in_array('Age', $fieldedit))&&(in_array('Sex', $fieldedit)))?'col-lg-4':(((in_array('Age', $fieldedit))||(in_array('Sex', $fieldedit)))?'col-lg-7':'col-lg-9')?>">
                                                                    <input type="text" name="twocustomername" id="twocustomername" class="form-control form-control-sm" placeholder="<?= $infomainaccessusercus['modulename'] ?> Name" value="<?= $rows[0]['customername'] ?>" <?= ($access['saleorderbtwocnamerequired']=='Yes')? 'required': ' ' ?>>
                                                                        ...
                                                                        <div class="row" style="padding:0px !important;margin: 0px !important;">
                                                                            <div class="col-lg-6" <?=(in_array('Work Phone', $fieldedit))?'':'style="display:none;"'?>>
                                                                                <label for="twoworkphone" class="custom-label" style="margin:0px !important;font-size: 12px !important;color: #777777;">
                                                                                    <?= ($access['saleorderbtwocwphonerequired']=='Yes')? '<span class="text-danger"> WORK PHONE * </span>': ' WORK PHONE ' ?>
                                                                                </label>
                                                                                <input type="text" class="form-control form-control-sm" id="twoworkphone" name="twoworkphone" placeholder="Work Phone" value="<?=$rows[0]['workphone']?>" <?= ($access['saleorderbtwocwphonerequired']=='Yes')? 'required': ' ' ?>>
                                                    ...
                                                    <div class="table-responsive" style="overflow-y: hidden;">
                                                        <table class="table table-bordered" id="purchasetable">
                                                            <thead>
                                                                ...
                                                                ?>
                                                                    <th width="13%">
                                                                        BATCH <?=(($access['saleorderbatexprequired']=='Yes')?'<span class="text-danger"> *</span>':'')?>
                                                                    </th>
                                                                <?php
                                                                    }
                                                                ?>
                                                                ...
                                                                ?>
                                                                    <th style="width: 87px !important;text-align: right !important;">
                                                                        <?=($access['txtqtysaleorder'])?><span class="text-danger"> *</span>
                                                                    </th>
                                                                <?php
                                                                    }
                                                                ?>
                                                            ...
                                                            ?>
                                                                ...
                                                                <tr>
                                                                    ...
                                                                    <td data-label="RATE" <?=(in_array('Rate', $fieldedit))?'':'style="display:none !important;"'?>>
                                                                        ...
                                                                        <div>
                                                                            <div class="dvi saleorderbillgets" id="saleorderbillgets<?=$i?>" style="margin-top:-22px;display: none;width: 250px;border-radius: 0px !important;">
                                                                                <table width="100%">
                                                                                    <tr style="border-bottom: 1px solid #cccccc;margin-bottom: 0px;">
                                                                                        <td align="center" style="border-right: 1px solid #cccccc;width: 50% !important;display: inline-block !important;text-align: center;">
                                                                                            <span onclick="saleordergetfun(<?=$i?>,0)" id="saleorderonoff<?=$i?>">
                                                                                        ...
                                                                                        <input type="text" style="display: none;" value="saleorder" id="billorsaleorder<?=$i?>">
                                                                                    </tr>
                                                                                </table>
                                                                            </div>
                                                                    ...
                                                                    <td data-label="<?=($access['txtqtysaleorder'])?>" <?=(in_array('Quantity', $fieldedit))?'':'style="display:none !important;"'?>>
                                                                        <div>
                                                                    ...
                                                                    <td data-label="<?=$access['txttaxablesaleorder']?>" <?=(in_array('Taxable Value', $fieldedit))?'':'style="display:none;"'?>>
                                                                        <div>
                                                                            <span style="font-size:15px !important;">
                                                                                <?php echo $resmaincurrencyans; ?>
                                                                            </span>
                                                                        ...
                                                                        <div style="<?=(in_array('Profit Margin', $fieldedit))?'':'display:none;'?>">
                                                                            <span data-bs-toggle="modal" data-bs-target="#ViewMarginDetails" style="color: green;font-size:13px;" id="margintotal<?=$i?>" onclick="getmargindetails(<?=$i?>)">
                                                                                Profit Margin :
                                                                                <span id="totalmargsaleorderalue<?=$i?>">
                                                            ...
                                                            ?>
                                                                ...
                                                                setTimeout(function() {
                                                                    ...
                                                                    function getoldmargins(lineidno) {
                                                                        $("#totalmargsaleorderalue"+lineidno).html($("#oldmargintotalvalue"+lineidno).val());
                                                                    }
                                                                },9000);
                                                                setTimeout(function() {
                                                                    ...
                                                                    // <?php
                                                                    //  if ($access['saleorderratedef']=='avail') {
                                                                    // ?>
                                                                    //  var saleorderratedef = 'and quantity>0 ';
                                                                    // <?php
                                                                    //  }
                                                                    //  else{
                                                                    // ?>
                                                                    //  var saleorderratedef = '';
                                                                    // <?php
                                                                    //  }
                                                                    // ?>
                                                                    //  $.get("ratesearch.php", {term: productid,custid: customerid,ratedef: saleorderratedef,differ: 'saleorder'} , function(datas){
                                                                    ...
                                                                    //              // console.log("key"+key);
                                                                    //              check+="<div id='option"+lineNo+objbatch[key].salesorderno+"saleorder' style='border:1px solid #cccccc;border-top: none !important;'><table width='100%' style='table-layout:fixed;'><tr style='border-bottom:none;margin-bottom:0px;'><td align='left' style='border:none;overflow:hidden;white-space:nowrap;' title='<?=strtoupper($infomainaccessuser['modulename'])?>'><span style='background-color:#1BBC9B;color:white;padding:2px 3px; border-radius:5px; text-transform: uppercase;' class='saleorderbilltxt'><?=strtoupper($infomainaccessuser['modulename'])?></span></td><td align='right' id='saleorderno"+objbatch[key].salesorderno+"saleorder' style='border:none;overflow:hidden;white-space:nowrap;' title='"+objbatch[key].salesorderno+"'>  "+objbatch[key].salesorderno+" </td><td align='right' id='saleorderdt"+objbatch[key].salesorderno+"saleorder' style='border:none;overflow:hidden;white-space:nowrap;' title='"+objbatch[key].salesorderdate+"'>"+objbatch[key].salesorderdate+" </td></tr><tr style='border-bottom:none;border-top:none;margin-bottom:0px;'><td align='left' colspan='2' id='rate"+objbatch[key].productrate+"saleorder' style='border:none;overflow:hidden;white-space:nowrap;' title='"+objbatch[key].productrate+"'>Rate : "+objbatch[key].productrate+" </td><td align='right' id='dis"+objbatch[key].productdiscount+"saleorder' style='border:none;overflow:hidden;white-space:nowrap;' title='"+objbatch[key].productdiscount+"'><?=$access['txtprodissaleorder']?> : "+objbatch[key].productdiscount+" </td></tr></table></div>";
                                                                    //          }
                                                                    //          // console.log(check);
                                                                    ...
                                                                    //  var productid= $('#product'+lineNo).val();
                                                                    // <?php
                                                                    //  if ($access['saleorderbatchdef']=='avail') {
                                                                    // ?>
                                                                    //  var saleorderbatchdef = 'and quantity>0 ';
                                                                    // <?php
                                                                    //  }
                                                                    //  else{
                                                                    // ?>
                                                                    //  var saleorderbatchdef = '';
                                                                    // <?php
                                                                    //  }
                                                                    // ?>
                                                                    //  $.get("batchsearch.php", {term: productid,batchdef: saleorderbatchdef} , function(datas){
                                                                    //      // console.log("normal"+datas);
                                ...
                                <div class="row">
                                                ...
                                                <div class="table-responsive mt-1" id="gsttablediv" <?=(in_array('Tax Informations', $fieldedit))?'':'style="display:none;"'?>>
                                                    <table class="table table-bordered" id="gsttable" style="font-size:12px;">
                                                        ...
                                                        <?php
                                                            ...
                                                            $sqlitaxes=$con->prepare("SELECT * FROM pairsalesorders WHERE franchisesession=? AND createdid=? AND salesorderno=? AND salesorderdate=? ORDER BY id ASC");
                                                            $sqlitaxes->bind_param("ssss", $_SESSION["franchisesession"], $companymainid, $salesorderno, $salesorderdate);
                                                            $sqlitaxes->execute();
                                    ...
                                    <div class="col-lg-4 mb-1" style="background-color:#fbfafa; font-size: 0.85rem;margin-top:-10px;" id="nextcontentdesignforweb">
                                        <div class="p-1">
                                            ...
                                            <div class="row mb-1" style="font-size:18px; display:none" >
                                                <div class="col-6" >
                                                    <label class="custom-label" for="balancedue">Change Due</label>
                                                </div>
                                                <div class="col-6">
                                                ...
                                                <div class="col" style="width:34px !important;margin-top: 1px !important;margin-left: -9px !important;">
                                                    <div class="form-group row">
                                                        <div class="col-7 col-sm-6">
                                                            ...
                                                            <a class="btn btn-primary btn-sm btn-custom-grey" href="salesorders.php" style="margin-top:-3px !important;">
                                                                Cancel
                                                            </a>
                                                            <button class="btn btn-primary btn-sm btn-custom arlina-button expand-left" id="convertoinvoices" name="convertoinvoices" type="submit" <?=(($rows[0]['convertstatus']=='0')?'':'')?> style="margin-top:-3px !important;<?=(($rows[0]['tdelete']=='0')?'':'display: none;')?>">
                                                                <?=(($rows[0]['convertstatus']=='0')?'Convert To '.$infomainaccessinvoice['modulename'].'':''.$infomainaccessinvoice['modulename'].' Converted')?>
                                                            </button>
                                                        </div>
                                        ...
                                        <div class="modal-body" style="height: max-content;">
                                            ...
                                            <table class="table notforfinaloutputsave showhideload">
                                                <tr>
                                                    ...
                                                    <td>
                                                        <input type="text" name="validsalesorderno" id="validsalesorderno" class="form-control form-control-sm finalsubmitrequired" readonly required>
                                                    </td>
                                                </tr>
                                                <tr>
                                                    <th style="text-align:left !important">
                                                    ...
                                                    <td>
                                                        <input type="date" name="validsalesorderdate" id="validsalesorderdate" class="form-control form-control-sm finalsubmitrequired" readonly required>
                                                    </td>
                                                </tr>
                                                <tr>
                                                    <th style="text-align:left !important">
                                                    ...
                                                    <td>
                                                        <input type="number" name="validsalesorderamount" id="validsalesorderamount" class="form-control form-control-sm finalsubmitrequired" readonly required>
                                                    </td>
                                                </tr>
                                                <tr style="<?=(($access['defsaleorderpay']=='split')?'':'display: none;')?>">
                                                ...
                                                <tr style="<?=(($access['defsaleorderpay']=='split')?'display: none;':'')?>">
                                                    ...
                                                    <td style="background-color: #e9ecef;">
                                                        <div class="col-lg-12">
                                                            <select class="select2-field form-control  form-control-sm finalsubmitrequired" name="salesorderterm" id="salesorderterm" required onchange="ihaveblock()">
                                                            <?php
                                                            ...
                                                            ?>
                                                                <option value="<?= $info['term'] ?>" <?=($info['term']==$rows[0]['salesorderterm'])?'selected':''?>>
                                                                    <?= $info['term'] ?>
                                                                </option>
                                                            <?php
                                                                    }
                                                ...
                                                <tr id="validpaidblock">
                                                    <th style="text-align:left !important;background-color: #e9ecef;">
                                                        <label class="custom-label text-danger">
                                                            <?=(($access['defsaleorderpay']=='split')?'Amount Received':'Cash Received')?> *
                                                        </label>
                                                    </th>
                                                    <td style="background-color: #e9ecef;">
                                                        ...
                                                        function paidamountcalc(){
                                                            var validsalesorderamount=$("#validsalesorderamount").val();
                                                            var validpaidamount=$("#validpaidamount").val();
                                                            // $("#validpaidamount").val(validsalesorderamount);
                                                            if((validsalesorderamount!='')&&(validpaidamount!='')){
                                                                var balance=parseFloat(validsalesorderamount)-parseFloat(validpaidamount);
                                                ...
                                                <tr id="duedateblock">
                                                    ...
                                                    <td style="background-color: #e9ecef;">
                                                        ...
                                                        function splitchecking() {
                                                            ...
                                                            $("#validbalance").val(((parseFloat($("#validsalesorderamount").val())-parseFloat($("#validpaidamount").val()))*-1));
                                                            $("#validbalances").val(((parseFloat($("#validsalesorderamount").val())-parseFloat($("#validpaidamount").val()))*-1));
                                                            if (($("#validsalesorderamount").val()) == (parseFloat($("#cashreceived").val()) + parseFloat($("#cardreceived").val()) + parseFloat($("#gpayreceived").val()))) {
                                                        ...
                                                        function funduedates(){
                                                            ...
                                                            var today="";
                                                            if(duedates!=''){
                                                                // console.log(salesorderdate);
                                                                // console.log(duedates);
                                                                today=addDays(salesorderdate, duedates);
                                            ...
                                            <script type="text/javascript">
                                            var ihaveterm = document.getElementById("salesorderterm").value;
...
$("#customer").on("select2:open", function() {
<?php
    if($access['saleordernewcustomerdef']=='1'){
?>
    $("#configureunits").show();
...
$(".product1").on("select2:open", function() {
<?php
    if($access['saleordernewproductdef']=='1'){
?>
    $("#configureunits").show();
...
function getmargindetails(linidno) {
    ...
    $.ajax({
        type: "GET",
        url: 'marginchecking.php?productid='+marginproductid+'&batch='+marginbatchval+'&expiry='+marginexpval+'&prorate='+marginprorate+'&proqty='+marginproqty+'',
        success: function (result) {
    ...
    if(id != ''){
        $.get("prosearch1.php", {term: id} , function(data){
            ...
            if (proalreadychnxtlvl==0) {
                ...
                <?php
                    if($access['saleorderbatexprequired']=='Yes'){
                ?>
                ...
                <?php
                    ...
                    if($access['saleorderbatchdef']=='availdrop'){
                ?>
    ...
    if(fetchedit){
        ...
        $.ajax({
            ...
            success: function (result) {
                ...
                $("#totalmargsaleorderalue"+id+"").html(resultdata.margintotal);
...
<div class="modal fade" id="AddNewSalesOrderTerm" tabindex="-1" role="dialog" style="z-index: 1051;">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <span type="button" onclick="funessalesorderterm()" class="close" data-dismiss="modal" aria-label="Close">
                ...
                <form method="post" action="">
                    <div class="row justify-content-center">
                        <div class="col-lg-12">
                            <div class="form-group row">
                                <div class="col-sm-6">
                                    <label for="missingsalesorderterm" class="custom-label">
                                        <span class="text-danger">
                                ...
                                <div class="col-sm-6">
                                    <input type="text" name="salesorderterm" class="form-control form-control-sm mb-4" id="missingsalesorderterm" placeholder="Name" required>
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            <div class="modal-footer " style="margin-top: 10px !important;">
                <div class="col">
                    <button onclick="funaddsalesorderterm()" class="btn btn-primary btn-sm btn-custom arlina-button expand-left"   type="submit"  name="submitsalesorderterm" value="Submit">
                        <span class="label">Save</span>
                        <span class="spinner"></span>
                    </button>
                    <button type="button" class="btn btn-primary btn-sm btn-custom-grey" onclick="funessalesorderterm()">
                        Cancel
                    </button>
                </div>
            </div>
...
<script>
function funaddsalesorderterm() {
    var missingsalesorderterm = document.getElementById('missingsalesorderterm');
    if (missingsalesorderterm.value == '') {
        alert('Please Enter New <?= $infomainaccessuser['modulename'] ?> Term Name');
        missingsalesorderterm.focus();
        return false;
    }
    else {
        $.ajax({
            ...
            url: "termadds.php",
            data: {
                term: $("#missingsalesorderterm").val(),
                submit: "Submit"
            },
            success:function(result){
                const resarray = result.split("|");
                ...
                if(resarray[1]=='0'){}
                    ...
                    $('#salesorderterm').append('<option value="' + missingsalesorderterm.value + '">' + missingsalesorderterm.value +'</option>');
                    $('#salesorderterm').val(missingsalesorderterm.value).change();
                    $("#salesorderterm").select2();
                    $('#AddNewSalesOrderTerm').modal('hide');
                    return false;
                }
            }
        });
...
function funessalesorderterm() {
    $("#salesorderterm").select2();
    $('#AddNewSalesOrderTerm').modal('hide');
    return false;
}
$("#tablenumber").on("select2:open", function() {
    $("#configureunits").attr("data-bs-target","#AddNewTablenumber");
});
$("#tablenumber").on("select2:open", function() {
    document.getElementById("configureunits").innerHTML = "New <?=$access['txttablenumber']?>";
});
$("#saleperson").on("select2:open", function() {
    $("#configureunits").attr("data-bs-target","#AddNewSaleperson");
});
$("#saleperson").on("select2:open", function() {
    document.getElementById("configureunits").innerHTML = "New <?=$access['txtsaleperson']?>";
});
$("#salesorderterm").on("select2:open", function() {
    $("#configureunits").attr("data-bs-target","#AddNewSalesOrderTerm");
});
$("#salesorderterm").on("select2:open", function() {
    document.getElementById("configureunits").innerHTML = "New <?= $infomainaccessuser['modulename'] ?> Term";
});
</script>
<div class="modal fade" id="AddNewTablenumber" tabindex="-1" role="dialog" style="z-index: 1051;">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">
                    New <?=$access['txttablenumber']?>
                </h5>
...
function funaddtablenumber() {
    ...
    if (missingtablenumber.value == '') {
        alert('Please Enter New <?=$access['txttablenumber']?>');
        ...
        $.ajax({
            ...
            success:function(result){
                ...
                if(resarray[1]=='0'){}
                    ...
                    $('#tablenumber').append('<option value="' + missingtablenumber.value + '|||' + resarray[1] + '">' + missingtablenumber.value +'</option>');
                    $('#tablenumber').val(missingtablenumber.value + '|||' + resarray[1]).change();
                    $("#tablenumber").select2();
                    $('#AddNewTablenumber').modal('hide');
                    return false;
                }
            }
        });
                            ...
                            <div class="form-group row">
                                <div class="col-sm-5">
                                    <label for="missingsaleperson" class="custom-label">
                                        <span class="text-danger"> <?=$access['txtsaleperson']?> *</span>
                                    </label>
                                </div>
                                <div class="col-sm-7">
...
function funaddsaleperson() {
    ...
    if (missingsaleperson.value == '') {
        alert('Please Enter New <?=$access['txtsaleperson']?> Name');
...
// FOR GST TYPE
// function triggerpayment(salesorderno,salesorderdate,salesorderamount,cancelstatus,whatdonext){
//     let allAreFilled = true;
//     document.getElementById("salesorderform").querySelectorAll("[required]").forEach(function(i) {
//         if (!allAreFilled) return;
//         if (i.type === "radio") {
//             let radioValueCheck = false;
//             document.getElementById("salesorderform").querySelectorAll(`[name=${i.name}]`).forEach(function(r) {
...
//                 isValidFinal = true;
//                 checkvalidate();
//                 $('#validsalesorderno').val($('#salesorderno').val());
//                 $('#validsalesorderdate').val($('#salesorderdate').val());
//                 $('#validsalesorderamount').val($('#grandtotal').val());
//                 var validsalesorderamount=$("#validsalesorderamount").val();
//                 var validpaidamount=$("#validpaidamount").val();
//                 $("#validpaidamount").val(validsalesorderamount);
//                 <?php
...
function saleordergetfun(lineNo,boolean) {
    $("#saleorderonoff"+lineNo).css("background-color","#1BBC9B");
    $("#saleorderonoff"+lineNo).css("color","#fff");
    $("#billonoff"+lineNo).css("background-color","#fff");
    $("#billonoff"+lineNo).css("color","#000");
    $("#billorsaleorder"+lineNo).val('saleorder');
    var productid= $('#product'+lineNo).val();
    var customerid = $("#customer").val();
<?php
    if ($access['saleorderratedef']=='avail') {
...
// FOR GET OLD RATE OF THE CUSTOMER AND PRODUCT IN SALE ORDER
function billgetfun(lineNo) {
    $("#saleorderonoff"+lineNo).css("background-color","#fff");
    $("#saleorderonoff"+lineNo).css("color","#000");
    $("#billonoff"+lineNo).css("background-color","#1BBC9B");
    $("#billonoff"+lineNo).css("color","#fff");
    $("#billorsaleorder"+lineNo).val('bill');
    var productid= $('#product'+lineNo).val();
    var customerid = $("#customer").val();
<?php
    if ($access['saleorderratedef']=='avail') {
...
?>
    $.get("ratesearch.php", {term: productid,custid: customerid,ratedef: saleorderratedef,differ: 'bill'} , function(datas){
        // console.log("normal"+datas);
        $("#errrate"+lineNo).val(datas);
        var letters = "<br /><b>Warning</b>:  Undefined variable $ratedatas in";
        brrch = document.getElementById("errrate"+lineNo);
        if (brrch.value.includes(letters)||brrch.value=='') {
            ...
            for (var key in objbatch) {
                // console.log("key"+key);
                check+="<div id='option"+lineNo+objbatch[key].salesorderno+"saleorder' style='border:1px solid #cccccc;border-top: none !important;'><table width='100%' style='table-layout:fixed;'><tr style='border-bottom:none;margin-bottom:0px;'><td align='left' style='border:none;overflow:hidden;white-space:nowrap;' title='<?=strtoupper($infomainaccessuser['modulename'])?>'><span style='background-color:#1BBC9B;color:white;padding:2px 3px; border-radius:5px; text-transform: uppercase;' class='saleorderbilltxt'><?=strtoupper($infomainaccessuserbill['modulename'])?></span></td><td align='right' id='saleorderno"+objbatch[key].salesorderno+"saleorder' style='border:none;overflow:hidden;white-space:nowrap;' title='"+objbatch[key].salesorderno+"'>  "+objbatch[key].salesorderno+" </td><td align='right' id='saleorderdt"+objbatch[key].salesorderno+"saleorder' style='border:none;overflow:hidden;white-space:nowrap;' title='"+objbatch[key].salesorderdate+"'>"+objbatch[key].salesorderdate+" </td></tr><tr style='border-bottom:none;border-top:none;margin-bottom:0px;'><td align='left' colspan='2' id='rate"+objbatch[key].productrate+"saleorder' style='border:none;overflow:hidden;white-space:nowrap;' title='"+objbatch[key].productrate+"'>Rate : "+objbatch[key].productrate+" </td><td align='right' id='dis"+objbatch[key].productdiscount+"saleorder' style='border:none;overflow:hidden;white-space:nowrap;' title='"+objbatch[key].productdiscount+"'><?=$access['txtprodissaleorder']?> : "+objbatch[key].productdiscount+" </td></tr></table></div>";
            }
            ...
            browsers.style.border = 'none';
            $("#productrate"+lineNo).focus();
            for(i=0;i<document.getElementsByClassName("saleorderbilltxt").length;i++){
                document.getElementsByClassName("saleorderbilltxt")[i].innerHTML = "<?=strtoupper($infomainaccessuserbill['modulename'])?>";
            }
        }
    });
}
...
function batchget(id){
    if($("#productid"+id).val()==''){
        ...
        var browserssaleorderbill = document.getElementById("saleorderbillgets"+id);
        browsersrate.style.display = 'none';
        browserssaleorderbill.style.display = 'none';
    ...
    <?php
        if (($access['saleorderbatchdef']=='avail')||($access['saleorderbatchdef']=='availdrop')) {
    ?>
        var saleorderbatchdef = 'and quantity>0 ';
    <?php
        }
...
function rateget(id){
    if($("#productid"+id).val()==''){
        ...
        if (firstornonerate) {
            ...
            if (document.getElementById("billorsaleorder"+id).value=='saleorder') {
                saleordergetfun(id,0);
            }
            else{
                billgetfun(id);
            }
        ...
        var browserssaleorderbill = document.getElementById("saleorderbillgets"+id);
        browsers.style.display = 'block';
        browserssaleorderbill.style.display = 'block';
        var productid= $('#product'+id).val();
        var customerid = $("#customer").val();
        input.onclick = function () {
            browsers.style.display = 'block';
    ...
    ?>
        if (document.getElementById("billorsaleorder"+id).value=='saleorder') {
            var saleorderorbills = 'saleorder';
        }
        else{
            var saleorderorbills = 'bill';
        }
        $.get("ratesearch.php", {term: productid,custid: customerid,ratedef: saleorderratedef,differ: saleorderorbills} , function(datas){
            const objbatch = JSON.parse(datas);
            option='';
            saleorderno='';
            saleorderdt='';
            rate='';
            for (var key in objbatch) {
                option+='option'+id+objbatch[key].salesorderno+'saleorder;';
                saleorderno+='saleorderno'+objbatch[key].salesorderno+'saleorder;';
                saleorderdt+='saleorderdt'+objbatch[key].salesorderdate+'saleorder;';
                rate+='rate'+objbatch[key].productrate+'saleorder;';
            }
            optionspl = option.split(';');
            saleordernospl = saleorderno.split(';');
            saleorderdtspl = saleorderdt.split(';');
            ratespl = rate.split(';');
            for (var i = 0; i <= optionspl.length; i++) {
                var optionans = document.getElementById(optionspl[i]);
                $("#"+optionspl[i]).on("click",function() {
                    ...
                    const childone = child[0].children;
                    const childtwo = childone[0].children;
                    var saleordernosaleorderdt = childtwo[0].innerHTML;
                    var saleordernosaleorderdtspl = saleordernosaleorderdt.split(" ");
...
setTimeout(function() {
    $.get("refersearch.php", {term: 'saleorderref'} , function(datas){
...
$.ajax({
type: "GET",
url: 'editfetch.php?term=' + sIndexEdit + '&limitings=5&salesorderno=<?=$_GET['salesorderno']?>&salesorderdate=<?=$_GET['salesorderdate']?>&type=salesorder&i='+ivalue+'',
success: function (result) {
...
<?php
        ...
        header("Location:salesorders.php?error=No Information Found");
    }
}
else{
    header("Location:salesorders.php?error=No Information Found");  
