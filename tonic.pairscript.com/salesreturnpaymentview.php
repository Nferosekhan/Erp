...
$sql=mysqli_query($con,"select id, createdon, term, receiptdate, receiptno, cancelstatus, customerid, customername, paymentmode, amount, notes, publicid, privateid from pairsalesreturnpayments where franchisesession='".$_SESSION['franchisesession']."' and createdid='$companymainid' and id='".$_GET['id']."'");
...
<div class="row">
<div class="col-lg-6">
 <p class="mb-3" id="viewpro"><i class="fa fa-eye"></i> <?= $infomainaccessuser['modulename'] ?></p>
 <?php
...
?>
<a class="btn btn-primary btn-sm btn-custom-grey" href="salesreturnpaymentedit.php?id=<?=$id?>" id="btngopage"><i class="fa fa-pencil-alt"></i> <span class="mobreswords "> Edit</span></a>
  <?php 
}
?>
  </span>
...
<div class="tab-content" id="nav-tabContent">
    <div class="tab-pane fade" id="nav-profile" role="tabpanel" aria-labelledby="nav-profile-tab">
      ...
      <tbody>
        <?php
      $sqluse=mysqli_query($con, "select * from pairusehistory where usetype='SALESPAYMADE' and useid='$id' order by createdon desc");
...
<!-- <p class="m-3" id="infoheadsall"><?= $infomainaccessuser['modulename'] ?></p>	 -->
...
Customer Name: <strong><?=$info['customername']?></strong><br>
...
?>
             ...
             <p align="right" class="mt-3" style="margin-right:0px; cursor:pointer" id="templatetext">Template: '<?=($infofranch['salesreturnpaymenttemplate']=='1')?'Advanced':'Standard'?> Template' <a data-bs-toggle="modal" data-bs-target="#changesalesreturnpaymentModal" class="text-blue">Change</a></p>
             
        </div>
        <!-- Modal -->
<div class="modal fade" id="changesalesreturnpaymentModal" tabindex="-1" role="dialog" aria-labelledby="changesalesreturnpaymentModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="changesalesreturnpaymentModalLabel">Choose Template</h5>
        <button type="button" class="btn-close text-dark" data-bs-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
...
if (r == true) {
    window.location.href='changetemplatereceipt.php?template=salesreturnpayment&id=<?=$id?>&publicid=<?=$info['publicid']?>&receiptdate=<?=$info['receiptdate']?>&temptype=0&submt=submt';
} else {
...
var r = confirm('Are you sure want to Change this Template?');
if (r == true) {
    window.location.href='changetemplatereceipt.php?template=salesreturnpayment&id=<?=$id?>&publicid=<?=$info['publicid']?>&receiptdate=<?=$info['receiptdate']?>&temptype=1&submt=submt';
} else {
...
function downloadpdf() {
    ...
    doc.html(pdfjs, {
        callback: function(doc) {
            //doc.output('dataurlnewwindow');
            doc.save("Payments Made For Sales Return-<?=$info['publicid']?>-<?=$info['receiptdate']?>.pdf");
            //document.getElementById("pdfObj").data = doc.output("bloburl");
            //window.open(doc.output("bloburl"), "_blank","toolbar=no,status=no,menubar=no,scrollbars=no,resizable=no,modal=yes,top=200,left=350,width=600,height=400");
