...
if (isset($_POST['submitcustomizes'])) {
	...
	$sqlreportmoduless=$con->prepare("SELECT * FROM pairreportmodules WHERE types='crnote' ORDER BY id ASC");
	...
	foreach ($newmodules as $newmoduleskey) {
		...
		$modcolumncol=$coltypemod."crnote";
	...
	$sqlupreport = $con->prepare("UPDATE pairreports SET rowcolumns=? WHERE franchiseid=? AND createdid=? AND types='crnote'");
	...
	$sqlreportfav = $con->prepare("UPDATE pairreportfavourites SET reporturl=? WHERE franchisesession=? AND createdid=? AND reportnames='crnote'");
	$sqlreportfav->bind_param("sss", $newReportUrl, $_SESSION['franchisesession'], $companymainid);
	$sqlreportfav->execute();
	$sqlreportfav->close();

	$sqlreport = $con->prepare("UPDATE pairreports SET reportperiod=?,filtername=?,companyname=?,dateprepare=?,timeprepare=?,gstrule=? WHERE franchiseid=? AND createdid=? AND types='crnote'");
	...
	$sqlreportcon = $con->prepare("UPDATE pairreports SET gstrule=? WHERE franchiseid=? AND createdid=? AND (types='crnoteregistered' OR types='crnoteconsumer')");
	$sqlreportcon->bind_param("sss", $gstrule, $_SESSION['franchisesession'], $companymainid);
	$sqlreportcon->execute();
	$sqlreportcon->close();

	header("Location:reportcrnote.php?datefrom=".date($dateformatforurlredirection,strtotime($from))."&dateto=".date($dateformatforurlredirection,strtotime($to))."");
}

$sqlreportviews=$con->prepare("SELECT * FROM pairreports WHERE franchiseid=? AND createdid=? AND types='crnote'");
									...
									<div class="row">
										<div class="col-lg-6">
											<p class="mb-3" style="color:black;font-size: 20px;margin-top: -8px;">
												...
												&gt;
												Summary of <?= $infomainaccessusercreditnote['modulename']; ?>
											</p>
										</div>
										<div class="col-lg-6">
																			...
																			<div id="customizerowcolumns" class="accordion-collapse collapse show" aria-labelledby="customizerowcolumn">
																				<div class="accordion-body text-sm">
																					<div class="row justify-content-center">
																						...
																						<?php
																							$newans=array();
																							$sqlreportaccesss=$con->prepare("SELECT * FROM pairreports WHERE franchiseid=? AND createdid=? AND types='crnote'");
																								...
																								<div class="col-lg-6 my-1" style="padding-left: 0px !important;">
																									<div class="custom-control custom-checkbox mr-sm-2" style="padding: 0px 12px !important;">
																										<div class="input-group input-group-sm" id="flaghead">
																											...
																											<input type="checkbox" class="custom-control-input" name="<?= $coltypemod; ?>crnote" id="<?= $coltypemod; ?>crnote" checked disabled>
																											<label class="custom-control-label custom-label" for="<?= $coltypemod; ?>crnote">
																												<?= str_replace(" or ", " / ",str_replace("Credit Note",$infomainaccessusercreditnote['modulename'],$newmoduleskey)) ?>
																											</label>
																										</div>
																									</div>
																								</div>
																								...
																								<div class="col-lg-6 my-1">
																									<div class="custom-control custom-checkbox mr-sm-2">
																										...
																										<label class="custom-control-label custom-label" for="gstrule">
																											If the creditnote does not contain a GSTIN, it will be forwarded to the consumer for display and calculation. Conversely, if the creditnote contains a GSTIN, it will be directed to the registered persons for display and calculation.
																										</label>
																									</div>
																								</div>
																							</div>
											...
											<div class="tab-pane fade" id="nav-profile" role="tabpanel" aria-labelledby="nav-profile-tab">
												<div class="table-responsive m-3" id="histables">
													<table class="table table-bordered">
														...
														<tbody>
      												<?php
															$sqluse=$con->prepare("SELECT * FROM pairusehistory WHERE usetype = 'REPcrnote' AND useid = ? ORDER BY createdon DESC");
																			...
																			<td width="100%" style="padding:10px;">
																				<div style="height: 975px;">
																					<table id="print-are1" style="border: 1px solid #eee;" width="100%">
																						<thead>
																							<tr>
																								...
																								<td class="text-uppercase" style="text-align: right !important;width: 16%;border:1px solid #eee;padding-right: 10px;">
																									<span style="font-size:13px;color:black;">
																										<?=$infomainaccessusercreditnote["modulename"]?> Total
																									</span>
																								</td>
																							</tr>
																						</thead>
																						<tbody id="myTable">
																						<?php
																							$sqlregisters=$con->prepare("SELECT * FROM paircreditnotes WHERE franchisesession=? AND createdid=? AND (creditnotedate>=? AND creditnotedate<=?) AND cancelstatus='0' AND ".(($sqlviewreport['gstrule']=='0')?'gstrtype="Registered Business - Regular"':'gstrtype="Registered Business - Regular" and gstno!="" ')." GROUP BY creditnotedate, creditnoteno");
										      											...
										      											$sqlregister = $sqlregisters->get_result();
																							...
																							if($registercreditnotetotal!=0){
																						?>
																							<tr onclick="window.open('reportcrnoteregistered.php?datefrom=<?=$datefrom?>&dateto=<?=$dateto?>','_self')">
																								<td data-label="DESCRIPTION" style="border:1px solid #eee;padding-left: 10px;">
																									<span style="display: inline-flex;max-width: 295px;min-width: 295px;white-space: nowrap;overflow: hidden;color: royalblue;">
																										Taxable credit note supplies made to registered persons
																									</span>
																								</td>
																								...
																								<td data-label="<?=$infomainaccessusercreditnote["modulename"]?> TOTAL" style="text-align: right !important;border:1px solid #eee;padding-right: 10px;">
																									<span style="display: inline-flex;max-width: 90px;min-width: 90px;white-space: nowrap;overflow: hidden;justify-content: end;<?= (($registercreditnotetotal>0)?'color:royalblue;':'color:black;') ?>">
																										<?= (($registercreditnotetotal==0)?number_format((float)0,2,'.',','):number_format((float)$registercreditnotetotal,2,'.',',')) ?>
																									</span>
																								</td>
																							</tr>
																						<?php
																							}
																							$sqlconsumers=$con->prepare("SELECT * FROM paircreditnotes WHERE franchisesession=? AND createdid=? AND (creditnotedate>=? AND creditnotedate<=?) AND cancelstatus='0' AND ".(($sqlviewreport['gstrule']=='0')?'gstrtype="Consumer"':'gstrtype="Registered Business - Regular" and gstno="" ')." GROUP BY creditnotedate, creditnoteno");
										      											...
										      											$sqlconsumer = $sqlconsumers->get_result();
																							...
																							if($consumercreditnotetotal!=0){
																						?>
																							<tr onclick="window.open('reportcrnoteconsumer.php?datefrom=<?=$datefrom?>&dateto=<?=$dateto?>','_self')">
																								<td data-label="DESCRIPTION" style="border:1px solid #eee;padding-left: 10px;">
																									<span style="display: inline-flex;max-width: 295px;min-width: 295px;white-space: nowrap;overflow: hidden;color: royalblue;">
																										Taxable credit note supplies to consumer
																									</span>
																								</td>
																								<td data-label="IGST AMOUNT" style="text-align: right !important;border:1px solid #eee;padding-right: 10px;">
																								...
																								<td data-label="<?=$infomainaccessusercreditnote["modulename"]?> TOTAL" style="text-align: right !important;border:1px solid #eee;padding-right: 10px;">
																									<span style="display: inline-flex;max-width: 90px;min-width: 90px;white-space: nowrap;overflow: hidden;justify-content: end;<?= (($consumercreditnotetotal>0)?'color:royalblue;':'color:black;') ?>">
																										<?= (($consumercreditnotetotal==0)?number_format((float)0,2,'.',','):number_format((float)$consumercreditnotetotal,2,'.',',')) ?>
																									</span>
																								</td>
																							</tr>
																						<?php
										...
										<table id="print-are3" style="border: 1px solid #eee;display: none;" width="100%">
											<thead>
												<tr>
													...
													<td class="text-uppercase" style="text-align: right !important;width: 16%;border:1px solid #eee;padding-right: 10px;"><span style="font-size:13px;color:black;"> <?=$infomainaccessusercreditnote["modulename"]?> Total</span></td>
												</tr>
											</thead>
											<tbody id="myTables">
											</tbody>
...
const pagechanges = (page, pageSize) => {
	...
	ajaxRequestview = $.ajax({
		type: "GET",
		url: 'reportviewsearch.php?term='+perpages+'&limitings='+($('#limitforpagenum').val())+'&datefrom=<?=$datefrom?>&dateto=<?=$dateto?>&dif=crnote',
...
$(function() {
	...
	function(start, end, label) {
		...
		window.open("reportcrnote.php?datefrom="+datefrom+"&dateto="+dateto+"","_self");
	});
});
...
function converHTMLFileToPDF() {
	...
	var finaltimer = setTimeout(function(){
		...
		var header = function(data) {
			...
			var headtext1 = "Summary of <?= $infomainaccessusercreditnote['modulename']; ?>";
			var fontSize = doc.internal.getFontSize();
			var pageWidth = doc.internal.pageSize.width;
		...
		$.ajax({
			type: "GET",
			url: 'reporthistory.php?types=crnote&titles=Summary of <?= $infomainaccessusercreditnote['modulename']; ?>&printdownpdfcsv=Printed PDF&fromto='+fromtopdf+'',
			success: function (result) {
				console.log(result);
			},
			error: function (error) {
...
function downloadaspdf() {
	...
	var finaltimer = setTimeout(function(){
		...
		$.ajax({
			type: "GET",
			url: 'reporthistory.php?types=crnote&titles=Summary of <?= $infomainaccessusercreditnote['modulename']; ?>&printdownpdfcsv=Downloaded PDF&fromto='+fromtopdf+'',
			success: function (result) {
				console.log(result);
			},
			error: function (error) {
		...
		doc.save('Credit-Note-Reports-<?=$datefrom?>-To-<?=$dateto?>');
...
function downloadascsv() {
	...
	var finaltimer = setTimeout(function(){
		...
		data.push(["Summary of <?= $infomainaccessusercreditnote['modulename']; ?>"]);
		data.push([fromtopdf]);
		...
		link.setAttribute("download", "Credit-Note-Reports-<?=$datefrom?>-To-<?=$dateto?>");
		document.body.appendChild(link);
		link.click();
		$.ajax({
			type: "GET",
			url: 'reporthistory.php?types=crnote&titles=Summary of <?= $infomainaccessusercreditnote['modulename']; ?>&printdownpdfcsv=Downloaded CSV&fromto='+fromtopdf+'',
			success: function (result) {
				console.log(result);
			},
			error: function (error) {
...
document.addEventListener('DOMContentLoaded', function() {
	...
	function makeAjaxRequest() {
		ajaxRequest = $.ajax({
			type: "GET",
			url: 'check.php?term=crnote&datefrom=<?=$datefrom?>&dateto=<?=$dateto?>&limitfrom=0&limitto=10000',
			success: function (result) {
				if (result != mytabprev) {
