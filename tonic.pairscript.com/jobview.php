<?php
include('lcheck.php');
 if((isset($_GET['jobno']))&&(isset($_GET['jobdate'])))
 {
     $jobno=mysqli_real_escape_string($con, $_GET['jobno']);
     $jobdate=mysqli_real_escape_string($con, $_GET['jobdate']);
 $sql=mysqli_query($con, "select * from pairjobs where franchisesession='".$_SESSION['franchisesession']."' and createdid='$companymainid' and jobno='$jobno' and jobdate='$jobdate' order by id asc");
...
$datefetch = mysqli_fetch_array($dateformat);
if ($datefetch['date']=='DD/MM/YYYY') {
if (($access['jobprintexpdate']=='0')&&($access['jobprintexpmonth']=='0')&&($access['jobprintexpyear']=='0')) {
$dateforexp = '';
$widthforexp = '5%';
}
if (($access['jobprintexpdate']=='1')&&($access['jobprintexpmonth']=='1')&&($access['jobprintexpyear']=='1')) {
$dateforexp = 'd/m/Y';
$widthforexp = '10%';
}
if (($access['jobprintexpdate']=='0')&&($access['jobprintexpmonth']=='1')&&($access['jobprintexpyear']=='1')) {
$dateforexp = 'm/Y';
$widthforexp = '7%';
}
if (($access['jobprintexpdate']=='1')&&($access['jobprintexpmonth']=='0')&&($access['jobprintexpyear']=='1')) {
$dateforexp = 'd/Y';
$widthforexp = '7%';
}
if (($access['jobprintexpdate']=='1')&&($access['jobprintexpmonth']=='1')&&($access['jobprintexpyear']=='0')) {
$dateforexp = 'd/m';
$widthforexp = '7%';
}
if (($access['jobprintexpdate']=='1')&&($access['jobprintexpmonth']=='0')&&($access['jobprintexpyear']=='0')) {
$dateforexp = 'd';
$widthforexp = '5%';
}
if (($access['jobprintexpdate']=='0')&&($access['jobprintexpmonth']=='1')&&($access['jobprintexpyear']=='0')) {
$dateforexp = 'm';
$widthforexp = '5%';
}
if (($access['jobprintexpdate']=='0')&&($access['jobprintexpmonth']=='0')&&($access['jobprintexpyear']=='1')) {
$dateforexp = 'Y';
$widthforexp = '5%';
}
}
...
<a style="margin:4.5px 4.5px !important;" class="btn btn-primary btn-sm btn-custom-grey" onclick="downloadpdf()"><i class="fa fa-download" aria-hidden="true"></i> <span class="mobreswords "> Download</span></a>
          <a style="margin:4.5px 4.5px !important;" class="btn btn-primary btn-sm btn-custom-grey" href="salespaymentadd.php?no=<?=$jobno?>&date=<?=$jobdate?>&type=job" id="recordpaymentlink"><span style="font-weight: 700;"><?php echo $rescurrency[0]; ?></span> <span class="mobreswords " id="recordpaymentspan"> Record Payment</span></a>
          <?php
...
?>
<a class="btn btn-primary btn-sm btn-custom-grey" href="jobedit.php?jobno=<?=$jobno?>&jobdate=<?=$jobdate?>" style="margin:4.5px 4.5px !important;"><i class="fa fa-pencil-alt"></i> <span class="mobreswords"> Edit</span></a>
 <?php 
}
?>
<a href="javascript:;" class="btn btn-primary btn-sm btn-custom-grey" id="dropdownMenuButton" data-bs-toggle="dropdown" aria-expanded="false" style="margin:4.5px 4.5px !important;padding-bottom: 3px !important;" onclick="more()">
              ...
              <i class="fa fa-caret-down" id="more" style="color: #3c3c46 !important;position: relative;top: -12px;left: 130px;"></i>   
               <li class="nav-item" style="margin-top:-22px !important;">
                  <a class="nav-link" href="jobcancel.php?jobno=<?=$jobno?>&jobdate=<?=$jobdate?>&cancelstatus=<?=($rows[0]['cancelstatus']=='0')?'1':'0'?>" onclick="return confirm('Are you sure want to <?=($rows[0]['cancelstatus']=='0')?'Cancel':'Enable'?> this Job?')">
...
<div class="tab-content" id="nav-tabContent">
    <div class="tab-pane fade" id="nav-profile" role="tabpanel" aria-labelledby="nav-profile-tab">
      ...
      <tbody>
        <?php
      $sqluse=mysqli_query($con, "select * from pairusehistory where usetype='JOBS' and useid='$jobno' order by createdon desc");
...
<?php
$sqlismainaccessuser=mysqli_query($con, "select moduleno, moduleprefix, modulesuffix,jobprintgsttreatment,jobprintgstin,modulename,jobprintpos from pairmainaccess where franchiseid='".$_SESSION['franchisesession']."' and moduletype='Jobs' order by id  asc");
$infomainaccessuser=mysqli_fetch_array($sqlismainaccessuser);
  ...
  <div class="modal-dialog modal-dialog-centered" role="document">
    ...
    <div class="row d-flex justify-content-center">
        <div class="col-lg-8 col-md-12 justify-content-center">
            ...
            $paidamount=0;
                $currentbalance=0;
                $currentamount=(float)$rows[0]['jobamount'];
                $sqlsalespay=mysqli_query($con,"select amount from pairsalespayhistory where franchisesession='".$_SESSION['franchisesession']."' and createdid='$companymainid' and invoiceno='".$jobno."' and invoicedate='".$jobdate."' order by id desc");
                while($infosalespay=mysqli_fetch_array($sqlsalespay))
                {
                    $paidamount+=(float)$infosalespay['amount'];
                }
                $currentbalance=((float)$rows[0]['jobamount']-$paidamount);
            if($currentbalance==0)
            {
                ?>
...
<td style="">
<table width="100%">
<tr style="<?=($access['jobbranchphone']=='0')?'display:none;':''?>">
...
<tr>
<td width="10%" style="font-size:12px;padding: 0px !important;line-height: 15px !important;<?=($access['jobbranchemail']=='0')?'visibility:hidden;':''?>">E-mail </td>
<td width="90%" style="font-size:12px;padding: 0px !important;line-height: 15px !important;"><input type="text" readonly value=": <?=$info['email']?>" style="padding:0px;color:black;width: 206px !important;height: 18px !important;border: none !important;outline: none !important;display: inline-flex;font-size: 12px;<?=($access['jobbranchemail']=='0')?'visibility:hidden;':''?>" class="form-control form-control-sm"><span style="<?=(($access['jobprintdlno20']=='1')?'display:inline-flex;':'display:none;')?>white-space: nowrap;width: 169px;overflow: hidden;float: right;margin-right: 9px;">DL No 20 &nbsp; : <?=$info['dlno20']?></span></td>
</tr>
<tr>
<td width="10%" style="font-size:12px;padding: 0px !important;line-height: 15px !important;<?=($access['jobbranchgstin']=='0')?'visibility:hidden;':''?>">GSTIN </td>
<td width="90%" style="font-size:12px;padding: 0px !important;line-height: 15px !important;"><span style="display: inline-flex;white-space: nowrap;width: 134px;overflow: hidden;<?=($access['jobbranchgstin']=='0')?'visibility:hidden;':''?>">: <?=$info['gstno']?></span><span style="<?=(($access['jobprintdlno21']=='1')?'display:inline-flex;':'display:none;')?>white-space: nowrap;width: 169px;overflow: hidden;float: right;margin-right: 9px;">DL No 21 &nbsp; : <?=$info['dlno21']?></span></td>
</tr>
</table>
</td>
</tr>
...
<td width="34.5%">
<table width="100%">
<tr <?=(($access['jobbank']=='1')?'':'style="display:none;"')?>>
...
<tr <?=(($access['jobname']=='1')?'':'style="display:none;"')?>>
...
<tr <?=(($access['jobaccnumber']=='1')?'':'style="display:none;"')?>>
...
<tr <?=(($access['jobifsccode']=='1')?'':'style="display:none;"')?>>
...
<tr <?=(($access['jobbranchandcity']=='1')?'':'style="display:none;"')?>>
...
<?php
if (($infomainaccessuser['jobprintgstin']=='show')||($rows[0]['gstno']!='')&&($infomainaccessuser['jobprintgstin']!='hide')) {
?>
...
<td width="62%" style="font-size:12px;padding: 0px !important;line-height: 15px !important;">: 
<?php
if (($infomainaccessuser['jobprintpos']=='show')||($rows[0]['pos']!='')&&($infomainaccessuser['jobprintpos']!='hide')) {
?>
...
<?php
if ((in_array('Job Information', $fieldview))) {
?>
<tr>
...
<td style="padding: 0px !important;font-size: 14px !important;line-height: 15px !important;text-align: left !important;" width="45%">: <b><?=$rows[0]['jobno']?></b></td>
</tr>
<tr>
...
<?php
    ...
    ?>
<td style="padding: 0px !important;font-size: 14px !important;line-height: 15px !important;text-align: left !important;" width="45%">: <b><?=date($date,strtotime($rows[0]['jobdate']))?></b></td>
</tr>
<tr>
...
<td style="padding: 0px !important;font-size: 14px !important;line-height: 15px !important;text-align: left !important;" width="45%">: <b><?=$rows[0]['jobterm']?></b></td>
</tr>
<?php
}
?>
...
$job = '<tr><td style="padding: 0px !important;font-size: 14px !important;line-height: 15px !important;" width="55%"><input type="text" readonly value="'. $infomainaccessuser['modulename'] .'" style="padding:0px;color:black;width: 56px !important;height: 18px !important;border: none !important;outline: none !important;display: inline-flex;font-size: 14px;" class="form-control form-control-sm"> <input type="text" readonly value="Number" style="padding:0px;color:black;width: 54px !important;height: 18px !important;border: none !important;outline: none !important;display: inline-flex;font-size: 14px;" class="form-control form-control-sm"> </td><td style="padding: 0px !important;font-size: 14px !important;line-height: 15px !important;text-align: left !important;" width="45%">: <b>'.$rows[0]['jobno'].'</b></td></tr><tr><td style="padding: 0px !important;font-size: 14px !important;line-height: 15px !important;" width="55%"><input type="text" readonly value="'. $infomainaccessuser['modulename'] .'" style="padding:0px;color:black;width: 56px !important;height: 18px !important;border: none !important;outline: none !important;display: inline-flex;font-size: 14px;" class="form-control form-control-sm"> <input type="text" readonly value="Date" style="padding:0px;color:black;width: 54px !important;height: 18px !important;border: none !important;outline: none !important;display: inline-flex;font-size: 14px;" class="form-control form-control-sm"> </td> </td><td style="padding: 0px !important;font-size: 14px !important;line-height: 15px !important;text-align: left !important;" width="45%">: <b>'.date($date,strtotime($rows[0]['jobdate'])).'</b></td></tr><tr><td style="padding: 0px !important;font-size: 14px !important;line-height: 15px !important;" width="55%"><input type="text" readonly value="Payment" style="padding:0px;color:black;width: 56px !important;height: 18px !important;border: none !important;outline: none !important;display: inline-flex;font-size: 14px;" class="form-control form-control-sm"> <input type="text" readonly value="Term" style="padding:0px;color:black;width: 54px !important;height: 18px !important;border: none !important;outline: none !important;display: inline-flex;font-size: 14px;" class="form-control form-control-sm"> </td><td style="padding: 0px !important;font-size: 14px !important;line-height: 15px !important;text-align: left !important;" width="45%">: <b>'.$rows[0]['jobterm'].'</b></td></tr>';
}
$taxable ='';
$tax ='';
if ((in_array('Taxable Value', $fieldview))) {
...
</tr><tr><td style="padding: 0px !important;"><table width="100%"><tr '.(((in_array('Work Phone', $fieldview))?'':'style="display:none;"')).'><td width="38%" style="font-size:12px;padding:0px 6px !important;line-height: 15px !important;">Work Phone </td><td width="62%" style="font-size:12px;padding: 0px !important;line-height: 15px !important;">: <b style="display: inline-flex;white-space: nowrap;width: 100px;overflow: hidden;">'.$rows[0]['workphone'].'</b></td></tr><tr '.(((in_array('GSTIN', $fieldview))?'':'style="display:none;"')).'><td width="38%" style="font-size:12px;padding:0px 6px !important;line-height: 15px !important;">GSTIN </td><td width="62%" style="font-size:12px;padding: 0px !important;line-height: 15px !important;">:'.$gstin.'</td></tr></table></td></tr></table></td><td width="33%" style="white-space: normal !important;vertical-align: top;border-right: 1px solid #cccccc;padding: 0px !important;"><table width="100%"><tr style="background-color: #eee;"><td style="padding: 0px !important;"><strong style="padding:0px 6px !important;">Shipping Address</strong></td></tr><tr>'.$custname.'</tr><tr><td style="padding: 0px 6px !important;line-height: 15px !important;white-space: nowrap !important;">'.$ship1.$ship2.'</td></tr><tr><td style="padding: 0px !important;"><table width="100%"><tr '.((in_array('Mobile Phone', $fieldview))?'':'style="display:none;"').'><td width="38%" style="font-size:12px;padding:0px 6px !important;line-height: 15px !important;">Mobile Phone </td><td width="62%" style="font-size:12px;padding: 0px !important;line-height: 15px !important;">: <b style="display: inline-flex;white-space: nowrap;width: 100px;overflow: hidden;">'.$rows[0]['mobile'].'</b></td></tr><tr '.((in_array('Place of Supply', $fieldview))?'':'style="display:none;"').'><td width="38%" style="font-size:12px;padding:0px 6px !important;line-height: 15px !important;white-space: nowrap !important;">Place Of Supply </td><td width="62%" style="font-size:12px;padding: 0px !important;line-height: 15px !important;">: '.$pos.'</td></tr></table></td></tr></table></td><td width="33%" style="white-space: normal !important;vertical-align: middle;padding: 0px !important;"><table width="100%" style="text-align: center !important;">'.$job.'</table></td></tr></table></td></tr><tr style="height:1px !important;"><td style="padding: 0px !important;"><table width="100%"><tr style="vertical-align: top !important;"><td style="padding: 0px !important;height:176px !important;"><div style="max-height: 176px !important;min-height: 176px !important;overflow: hidden;"><table width="100%" style="border:1px solid #cccccc 0px 0px 0px !important;line-height: 13px !important;height: 176px;"><thead style="background-color: #eee;"><tr style="height:1px !important;"><th width="20%" style="border: 1px solid #cccccc;font-size: 12px !important;padding:0px 6px !important;">ITEM DETAILS</th><th width="8%" style="border: 1px solid #cccccc;font-size: 12px !important;padding:0px 6px !important;'.((in_array('Product Category', $fieldview))?'':'display:none !important;').'"><span style="display: block;overflow: hidden;max-width: 78px;max-height: 13px;">'.$access['txtnamecategory'].'</span></th><th width="10%" style="border: 1px solid #cccccc;font-size: 12px !important;padding:0px 6px !important;'.(($access['batchexpiryval']=='1')?'':'display:none !important;').'">BATCH</th><th width="'.$widthforexp.'" style="border: 1px solid #cccccc;font-size: 12px !important;padding:0px 6px !important;'.(($access['batchexpiryval']=='1')?'':'display:none !important;').'">EXPIRY</th><th width="7%" style="border: 1px solid #cccccc;font-size: 12px !important;padding:0px 6px !important;">HSN/SAC</th><th width="9%" style="border: 1px solid #cccccc;font-size: 12px !important;padding:0px 6px !important;text-align:right !important;'.((in_array('Product Mrp', $fieldview))?'':'display:none !important;').'">MRP</th><th width="9%" style="border: 1px solid #cccccc;font-size: 12px !important;padding:0px 6px !important;text-align: right !important;">RATE</th><th width="7%" style="border: 1px solid #cccccc;font-size: 12px !important;padding:0px 6px !important;text-align: right !important;">QUANTITY</th>'.$taxable.$tax.'</tr></thead><tbody>';
$finalrow=4;
if ((($item-1)%$finalrow)==0) {
if ((($countt-1)-$totpros)!=0) {
echo $merge;
...
?>
             ...
             <p align="right" class="mt-3" style="margin-right:-25px; cursor:pointer" id="templatetext">Template: '<?=($info['jobtemplate']=='1')?'24.13cm X 15.75cm':''?><?=($info['jobtemplate']=='0')?'Standard A4 Portrait':''?><?=($info['jobtemplate']=='2')?'Standard A5 Landscape':''?> ' <a data-bs-toggle="modal" data-bs-target="#changejobModal" class="text-blue">Change</a></p>
             </div>
...
<!-- Modal -->
<div class="modal fade" id="changejobModal" tabindex="-1" role="dialog" aria-labelledby="changejobModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="changejobModalLabel">Choose Template</h5>
        <button type="button" class="btn-close text-dark" data-bs-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
...
if (r == true) {
    window.location.href='changetemplate.php?template=job&no=<?=$jobno?>&date=<?=$jobdate?>&temptype=0&submt=submt';
} else {
...
var r = confirm('Are you sure want to Change this Template?');
if (r == true) {
    window.location.href='changetemplate.php?template=job&no=<?=$jobno?>&date=<?=$jobdate?>&temptype=2&submt=submt';
} else {
...
var r = confirm('Are you sure want to Change this Template?');
if (r == true) {
    window.location.href='changetemplate.php?template=job&no=<?=$jobno?>&date=<?=$jobdate?>&temptype=1&submt=submt';
} else {
...
$.ajax({
type: "GET",
url: 'jobdompdf.php?term=job&names=Jobs-<?=$_GET['jobno']?>-<?=$_GET['jobdate']?>&jobno=<?=$_GET['jobno']?>&jobdate=<?=$_GET['jobdate']?>&sizes=a4',
...
$("#time").html(Timer);
 var req = new XMLHttpRequest();
  req.open("GET", "dompdf/Jobs-<?=$_GET['jobno']?>-<?=$_GET['jobdate']?>.pdf", true);
...
$("#timedownload").html(Timer);
 var req = new XMLHttpRequest();
  ...
  req.onload = function (event) {
    ...
    link.download="Jobs-<?=$_GET['jobno']?>-<?=$_GET['jobdate']?>.pdf";
    link.click();
  };
...
$.ajax({
type: "GET",
url: 'jobdompdf.php?term=job&names=Jobs-<?=$_GET['jobno']?>-<?=$_GET['jobdate']?>&jobno=<?=$_GET['jobno']?>&jobdate=<?=$_GET['jobdate']?>&sizes=a5',
success: function (result) {
if (result && result.length > 0) {       
result=$.parseJSON( result );
Timer=result.timer;
...
$.ajax({
type: "GET",
url: 'jobdompdf.php?term=job&names=Jobs-<?=$_GET['jobno']?>-<?=$_GET['jobdate']?>&jobno=<?=$_GET['jobno']?>&jobdate=<?=$_GET['jobdate']?>&sizes=dt',
success: function (result) {
if (result && result.length > 0) {       
result=$.parseJSON( result );
Timer=result.timer;
...
header("Location:jobs.php?error=No Information Found");
}
}
else{
header("Location:jobs.php?error=No Information Found");  
