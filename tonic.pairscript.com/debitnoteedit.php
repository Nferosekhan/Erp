...
if((isset($_GET['debitnoteno']))&&(isset($_GET['debitnotedate']))){
	 $debitnoteno=htmlspecialchars($_GET['debitnoteno'], ENT_QUOTES, 'UTF-8');
	 $debitnotedate=htmlspecialchars($_GET['debitnotedate'], ENT_QUOTES, 'UTF-8');
	 $sql=$con->prepare("SELECT * FROM pairdebitnotes WHERE franchisesession=? AND createdid=? AND debitnoteno=? AND debitnotedate=? ORDER BY id ASC");
	 $sql->bind_param("ssss", $_SESSION['franchisesession'], $companymainid, $debitnoteno, $debitnotedate);
	 $sql->execute();
	 $sql->store_result();
	 if($sql->num_rows>0){
		  $sql->close();
		  ...
		  if(isset($_POST['grandtotal'])){
				...
				$olddebitnoteno=htmlspecialchars($_GET['debitnoteno'], ENT_QUOTES, 'UTF-8');
				$olddebitnotedate=htmlspecialchars($_GET['debitnotedate'], ENT_QUOTES, 'UTF-8');
				$debitnoteno=htmlspecialchars($_POST['debitnoteno'], ENT_QUOTES, 'UTF-8');
				//FOR GET THE CURRENT INVOICE NO
				$debitnotedate=htmlspecialchars($_POST['debitnotedate'], ENT_QUOTES, 'UTF-8');
				$paymentidforupdpayhis = '';
				$publicsqlforupdhis=$con->prepare("SELECT id FROM pairdebitnotepayments WHERE receiptno=? AND receiptdate=?");
				$publicsqlforupdhis->bind_param("ss", $debitnoteno, $debitnotedate);
				...
				for($i=0; $i<count($_POST['oldproductname']); $i++){
					 ...
					 if($productname!=''){
						  $sql = $con->prepare("DELETE FROM pairdebitnotes WHERE createdid=? AND franchisesession=? AND debitnoteno=? AND debitnotedate=?");
						  $sql->bind_param("ssss", $companymainid, $_SESSION["franchisesession"], $olddebitnoteno, $olddebitnotedate);
						  $sql->execute();
						  $sqldelledger = $con->prepare("DELETE FROM pairledgers WHERE createdid=? AND franchisesession=? AND ledgerno=? AND ledgerdate=? AND (type='debitnote' OR type='debitnote payment')");
						  $sqldelledger->bind_param("ssss", $companymainid, $_SESSION["franchisesession"], $olddebitnoteno, $olddebitnotedate);
						  $sqldelledger->execute();
						  $sqls = $con->prepare("DELETE FROM pairdebitnotepayments WHERE receiptno=? AND receiptdate=?");
						  $sqls->bind_param("ss", $olddebitnoteno, $olddebitnotedate);
						  $sqls->execute();
						  $sqls->close();
						  $sqlsant = $con->prepare("DELETE FROM pairdebitnotepayhistory WHERE debitnoteno=? AND debitnotedate=? AND type='debitnote'");
						  $sqlsant->bind_param("ss", $olddebitnoteno, $olddebitnotedate);
						  $sqlsant->execute();
						  $sqlsant->close();
						  ...
						  $sqlmarginsdel->bind_param("iiss",$companymainid, $_SESSION["franchisesession"], $olddebitnoteno, $olddebitnotedate);
				...
				$sql2->bind_param("iiss", $_SESSION['franchisesession'], $companymainid, $olddebitnoteno, $olddebitnotedate);
				$sql2->execute();
				$sql2->store_result();
				...
				if($sql2->num_rows==0){
					 ...
					 if($debitnoteterm=='CASH'){
						  if ($validpaidamount!=0) {
								...
								$sqlselects=$con->prepare("SELECT id FROM pairdebitnotepayments WHERE receiptno=? AND receiptdate=?");
								$sqlselects->bind_param("ss", $debitnoteno, $debitnotedate);
								$sqlselects->execute();
								if ($sqlselects->num_rows==0) {
									 $sqlselectsql = $sqlselects->get_result();
									 $sqlselect=$sqlselectsql->fetch_array();
									 $sqlpurchasepayins = $con->prepare("INSERT INTO pairdebitnotepayments (createdid,createdby,franchisesession,createdon,term,type,vendorname,vendorid,receiptno,receiptdate,amount,paymentmode,notes,publicid,privateid) VALUES (?, ?, ?, ?, 'RECEIPT', 'debitnote', ?, ?, ?, ?, ?, ?, '-', ?, ?)");
									 $sqlpurchasepayins->bind_param("ssssssssssss", $companymainid, $_SESSION["unqwerty"], $_SESSION["franchisesession"], $createdon, $vendorname, $vendorid, $debitnoteno, $debitnotedate, $validpaidamount, $debitnoteterm, $publiccode, $privatecode);
									 ...
									 if($sqlpurchasepayins){
										  ...
										  $sqle->execute();
										  $sqle->close();
											$oldesdebitnoteno = htmlspecialchars( $_POST['oldesdebitnoteno'], ENT_QUOTES, 'UTF-8');
											$oldesdebitnotedate = htmlspecialchars( $_POST['oldesdebitnotedate'], ENT_QUOTES, 'UTF-8');
											$oldesvendorname = htmlspecialchars( $_POST['oldesvendorname'], ENT_QUOTES, 'UTF-8');
											$oldesdebitnoteterm = htmlspecialchars( $_POST['oldesdebitnoteterm'], ENT_QUOTES, 'UTF-8');
											...
											if($debitnotedate!=$oldesdebitnotedate){
												if($chforpayments!=''){
													$chforpayments.='<br> Date <span style="color:green;" id="prohisfromtospan">( From '.date($datemainphp,strtotime($debitnotedate)).' To '.date($datemainphp,strtotime($olddebitnotedate)).' ) </span>';
												}
												else{
													$chforpayments.=' Date <span style="color:green;" id="prohisfromtospan">( From '.date($datemainphp,strtotime($debitnotedate)).' To '.date($datemainphp,strtotime($olddebitnotedate)).' ) </span>';
												}					
											}
											if($debitnoteno!=$oldesdebitnoteno){
												if($chforpayments!=''){
													$chforpayments.='<br> Reference Number <span style="color:green;" id="prohisfromtospan">( From '.$olddebitnoteno.' To '.$debitnoteno.' ) </span>';
												}
												else{
													$chforpayments.='Reference Number <span style="color:green;" id="prohisfromtospan">( From '.$olddebitnoteno.' To '.$debitnoteno.' ) </span>';
												}					
											}
											if($debitnoteterm!=$oldesdebitnoteterm){
												if($chforpayments!=''){
													$chforpayments.='<br> Mode of Payment <span style="color:green;" id="prohisfromtospan">( From '.$oldesdebitnoteterm.' To '.$debitnoteterm.' ) </span>';
												}
												else{
													$chforpayments.='Mode of Payment <span style="color:green;" id="prohisfromtospan">( From '.$oldesdebitnoteterm.' To '.$debitnoteterm.' ) </span>';
												}					
											}
									 ...
									 $sqlbatchupd = $con->prepare("UPDATE pairdebitnotepayments SET createdid=?, createdby=?, franchisesession=?, createdon=?,  term='RECEIPT', type='debitnote', vendorname=?, vendorid=?, receiptno=?, receiptdate=?, amount=?, paymentmode=?, notes='-' WHERE receiptno=? AND receiptdate=?");
									 $sqlbatchupd->bind_param("ssssssssssss", $companymainid, $_SESSION["unqwerty"], $_SESSION["franchisesession"], $createdon, $vendorname, $vendorid, $debitnoteno, $debitnotedate, $validpaidamount, $debitnoteterm, $debitnoteno, $debitnotedate);
									 $sqlbatchupd->execute();
									 if($sqlbatchupd){
										  $sqlbatchupd->close();
										  $sqle = $con->prepare("UPDATE pairdebitnotepayhistory SET amount=? WHERE debitnoteno=? AND debitnotedate=? AND type='debitnote'");
										  $sqle->bind_param("sss", $validpaidamount, $debitnoteno, $debitnotedate);
										  $sqle->execute();
										  $sqle->close();
									 }
								...
								$sqlbatchupdant = $con->prepare("UPDATE pairdebitnotepayments SET createdid=?, createdby=?, franchisesession=?, createdon=?,  term='RECEIPT', type='debitnote', vendorname=?, vendorid=?, receiptno=?, receiptdate=?, amount=?, paymentmode=?, notes='-' WHERE receiptno=? AND receiptdate=?");
								$sqlbatchupdant->bind_param("ssssssssssss", $companymainid, $_SESSION["unqwerty"], $_SESSION["franchisesession"], $createdon, $vendorname, $vendorid, $debitnoteno, $debitnotedate, $validpaidamount, $debitnoteterm, $debitnoteno, $debitnotedate);
								$sqlbatchupdant->execute();
								if($sqlbatchupdant){
									 $sqlbatchupdant->close();
									 $sqleant = $con->prepare("UPDATE pairdebitnotepayhistory SET amount=? WHERE debitnoteno=? AND debitnotedate=? AND type='debitnote'");
									 $sqleant->bind_param("sss", $validpaidamount, $debitnoteno, $debitnotedate);
									 $sqleant->execute();
									 $sqleant->close();
								}
					 ...
					 elseif($debitnoteterm=='CREDIT'){
						  $sqls = $con->prepare("DELETE FROM pairdebitnotepayments WHERE receiptno=? AND receiptdate=?");
						  $sqls->bind_param("ss", $debitnoteno, $debitnotedate);
						  $sqls->execute();
						  $sqls->close();
						  $sqlsant = $con->prepare("DELETE FROM pairdebitnotepayhistory WHERE debitnoteno=? AND debitnotedate=? AND type='debitnote'");
						  $sqlsant->bind_param("ss", $debitnoteno, $debitnotedate);
						  $sqlsant->execute();
						  $sqlsant->close();
						  //FOR UPDATE THE PAYMENT STATUS UNPAID
						  $sqler = $con->prepare("UPDATE pairdebitnotes SET createdid=?, createdby=?, franchisesession=?, createdon=?, paidstatus='0' WHERE debitnoteno=? AND debitnotedate=? AND vendorid=? AND franchisesession=? AND createdid=?");
						  $sqler->bind_param("sssssssss", $companymainid, $_SESSION["unqwerty"], $_SESSION["franchisesession"], $createdon, $debitnoteno, $debitnotedate, $vendorid, $_SESSION['franchisesession'], $companymainid);
						  $sqler->execute();
						  $sqler->close();
						  //FOR UPDATE THE PAYMENT STATUS UNPAID
						  $sqldelledgerpay = $con->prepare("DELETE FROM pairledgers WHERE createdid=? AND franchisesession=? AND ledgerno=? AND ledgerdate=? AND type='debitnote payment'");
						  $sqldelledgerpay->bind_param("ssss", $companymainid, $_SESSION["franchisesession"], $debitnoteno, $debitnotedate);
					 ...
					 if($sql){
						  ...
						  while($infodebitnote=$sqlidebitnote->fetch_array()){
								...
								$paidamount=0;
								$sqlpurchasespays=$con->prepare("SELECT amount FROM pairdebitnotepayhistory WHERE franchisesession=? AND createdid=? AND debitnoteno=? AND debitnotedate=? AND vendorid=? ORDER BY id DESC");
						...
						$sqlibills->close();
						  ...
						  $cussqlup->bind_param("sssss", $debitnoteamount, $balanceamount, $currentamount, $overdueamount, $vendorid);
						  $cussqlup->execute();
						  $cussqlup->close();
						  if(isset($_POST['submit1'])){
								echo '<script> window.open("debitnoteprint.php?debitnoteno='.$debitnoteno.'&debitnotedate='.$debitnotedate.'", "_blank");</script>';
								echo '<script> window.location.href="debitnotes.php?remarks=Updated Successfully";</script>'; 
						  }
						  else{
								$sqluse = $con->prepare("INSERT INTO pairusehistory (usetype,createdon,createdby,useid,uniqueid,useremarks) VALUES ('DEBITNOTES', ?, ?, ?, ?, '<span style=\"color:green;\" id=\"prohisfromtospan\">Updated Successfully</span>')");
								$sqluse->bind_param("ssss", $times, $_SESSION["unqwerty"], $debitnoteno, $purchaseid);
								$sqluse->execute();
								$sqluse->close();
								echo '<script> window.location.href="debitnoteview.php?id='.$purchaseid.'&debitnoteno='.$debitnoteno.'&debitnotedate='.$debitnotedate.'&remarks=Updated Successfully";</script>';
						  }
					 }
				...
				if ($sql) {
					 ...
					 $oldesbillamount = htmlspecialchars( $_POST['oldesbillamount'], ENT_QUOTES, 'UTF-8');
					 $oldesbillpaymentreceived = htmlspecialchars( $_POST['oldesbillpaymentreceived'], ENT_QUOTES, 'UTF-8');
					 ...
					 if($debitnoteno!=$oldesdebitnoteno){
						  if($debitnoteinfoch!=''){
								$debitnoteinfoch.='<br> '.$infomainaccessuser['modulename'].' Number <span style="color:green;" id="prohisfromtospan">( From '.$oldesdebitnoteno.' To '.$debitnoteno.' ) </span>';
						  }
						  else{
								$debitnoteinfoch.='<span style="color:royalblue;">'.$infomainaccessuser['modulename'].' Information</span> <br> '.$infomainaccessuser['modulename'].' Number <span style="color:green;" id="prohisfromtospan">( From '.$oldesdebitnoteno.' To '.$debitnoteno.' ) </span>';
						  }                   
					 }
					 if($invnumber!=$oldesinvnumber){
						  if($debitnoteinfoch!=''){
								$debitnoteinfoch.='<br> Invoice Number <span style="color:green;" id="prohisfromtospan">( From '.$oldesinvnumber.' To '.$invnumber.' ) </span>';
						  }
						  else{
								$debitnoteinfoch.='<span style="color:royalblue;">'.$infomainaccessuser['modulename'].' Information</span> <br> Invoice Number <span style="color:green;" id="prohisfromtospan">( From '.$oldesinvnumber.' To '.$invnumber.' ) </span>';
						  }                   
					 }
					 if($debitnotedate!=$oldesdebitnotedate){
						  if($debitnoteinfoch!=''){
								$debitnoteinfoch.='<br> '.$infomainaccessuser['modulename'].' Date <span style="color:green;" id="prohisfromtospan">( From '.date($date,strtotime($oldesdebitnotedate)).' To '.date($date,strtotime($debitnotedate)).' ) </span>';
						  }
						  else{
								$debitnoteinfoch.='<span style="color:royalblue;">'.$infomainaccessuser['modulename'].' Information</span> <br> '.$infomainaccessuser['modulename'].' Date <span style="color:green;" id="prohisfromtospan">( From '.date($date,strtotime($oldesdebitnotedate)).' To '.date($date,strtotime($debitnotedate)).' ) </span>';
						  }                   
					 }
					 if($reference!=$oldesreference){
						  if($debitnoteinfoch!=''){
								$debitnoteinfoch.='<br> Reference <span style="color:green;" id="prohisfromtospan">( From '.$oldesreference.' To '.$reference.' ) </span>';
						  }
						  else{
								$debitnoteinfoch.='<span style="color:royalblue;">'.$infomainaccessuser['modulename'].' Information</span> <br> Reference <span style="color:green;" id="prohisfromtospan">( From '.$oldesreference.' To '.$reference.' ) </span>';
						  }                   
					 }
					 if($saleperson!=$oldessaleperson){
						  if($debitnoteinfoch!=''){
								$debitnoteinfoch.='<br> Sale Person <span style="color:green;" id="prohisfromtospan">( From '.$oldessaleperson.' To '.$saleperson.' ) </span>';
						  }
						  else{
								$debitnoteinfoch.='<span style="color:royalblue;">'.$infomainaccessuser['modulename'].' Information</span> <br> Sale Person <span style="color:green;" id="prohisfromtospan">( From '.$oldessaleperson.' To '.$saleperson.' ) </span>';
						  }                   
					 }
					 if($preparedby!=$oldespreparedby){
						  if($debitnoteinfoch!=''){
								$debitnoteinfoch.='<br> Prepared By <span style="color:green;" id="prohisfromtospan">( From '.$oldespreparedby.' To '.$preparedby.' ) </span>';
						  }
						  else{
								$debitnoteinfoch.='<span style="color:royalblue;">'.$infomainaccessuser['modulename'].' Information</span> <br> Prepared By <span style="color:green;" id="prohisfromtospan">( From '.$oldespreparedby.' To '.$preparedby.' ) </span>';
						  }                   
					 }
					 if($checkedby!=$oldescheckedby){
						  if($debitnoteinfoch!=''){
								$debitnoteinfoch.='<br> Checked By <span style="color:green;" id="prohisfromtospan">( From '.$oldescheckedby.' To '.$checkedby.' ) </span>';
						  }
						  else{
								$debitnoteinfoch.='<span style="color:royalblue;">'.$infomainaccessuser['modulename'].' Information</span> <br> Checked By <span style="color:green;" id="prohisfromtospan">( From '.$oldescheckedby.' To '.$checkedby.' ) </span>';
						  }                   
					 }
					 if($invgrandtotal!=$oldesinvgrandtotal){
						  if($debitnoteinfoch!=''){
								$debitnoteinfoch.='<br> Grand Total <span style="color:green;" id="prohisfromtospan">( From '.$oldesinvgrandtotal.' To '.$invgrandtotal.' ) </span>';
						  }
						  else{
								$debitnoteinfoch.='<span style="color:royalblue;">'.$infomainaccessuser['modulename'].' Information</span> <br> Grand Total <span style="color:green;" id="prohisfromtospan">( From '.$oldesinvgrandtotal.' To '.$invgrandtotal.' ) </span>';
						  }                   
					 }
					 ...
					 if($vendorname!=$oldesvendorname){
						  if($debitnoteveninfoch!=''){
								$debitnoteveninfoch.='<br> '.$infomainaccessuserven['modulename'].' Name <span style="color:green;" id="prohisfromtospan">( From '.$oldesvendorname.' To '.$vendorname.' ) </span>';
						  }
						  else{
								$debitnoteveninfoch.=''.(($debitnoteinfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessuserven['modulename'].' Information</span> <br> '.$infomainaccessuserven['modulename'].' Name <span style="color:green;" id="prohisfromtospan">( From '.$oldesvendorname.' To '.$vendorname.' ) </span>';
						  }                   
					 }
					 if($area!=$oldesarea){
						  if($debitnoteveninfoch!=''){
								$debitnoteveninfoch.='<br> BILLING STREET <span style="color:green;" id="prohisfromtospan">( From '.$oldesarea.' To '.$area.' ) </span>';
						  }
						  else{
								$debitnoteveninfoch.=''.(($debitnoteinfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessuserven['modulename'].' Information</span> <br> BILLING STREET <span style="color:green;" id="prohisfromtospan">( From '.$oldesarea.' To '.$area.' ) </span>';
						  }                   
					 }
					 if($city!=$oldescity){
						  if($debitnoteveninfoch!=''){
								$debitnoteveninfoch.='<br> BILLING CITY/TOWN <span style="color:green;" id="prohisfromtospan">( From '.$oldescity.' To '.$city.' ) </span>';
						  }
						  else{
								$debitnoteveninfoch.=''.(($debitnoteinfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessuserven['modulename'].' Information</span> <br> BILLING CITY/TOWN <span style="color:green;" id="prohisfromtospan">( From '.$oldescity.' To '.$city.' ) </span>';
						  }                   
					 }
					 if($state!=$oldesstate){
						  if($debitnoteveninfoch!=''){
								$debitnoteveninfoch.='<br> BILLING STATE <span style="color:green;" id="prohisfromtospan">( From '.$oldesstate.' To '.$state.' ) </span>';
						  }
						  else{
								$debitnoteveninfoch.=''.(($debitnoteinfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessuserven['modulename'].' Information</span> <br> BILLING STATE <span style="color:green;" id="prohisfromtospan">( From '.$oldesstate.' To '.$state.' ) </span>';
						  }                   
					 }
					 if($district!=$oldesdistrict){
						  if($debitnoteveninfoch!=''){
								$debitnoteveninfoch.='<br> BILLING PIN <span style="color:green;" id="prohisfromtospan">( From '.$oldesdistrict.' To '.$district.' ) </span>';
						  }
						  else{
								$debitnoteveninfoch.=''.(($debitnoteinfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessuserven['modulename'].' Information</span> <br> BILLING PIN <span style="color:green;" id="prohisfromtospan">( From '.$oldesdistrict.' To '.$district.' ) </span>';
						  }                   
					 }
					 if($pincode!=$oldespincode){
						  if($debitnoteveninfoch!=''){
								$debitnoteveninfoch.='<br> BILLING COUNTRY/REGION <span style="color:green;" id="prohisfromtospan">( From '.$oldespincode.' To '.$pincode.' ) </span>';
						  }
						  else{
								$debitnoteveninfoch.=''.(($debitnoteinfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessuserven['modulename'].' Information</span> <br> BILLING COUNTRY/REGION <span style="color:green;" id="prohisfromtospan">( From '.$oldespincode.' To '.$pincode.' ) </span>';
						  }                   
					 }
					 if($sarea!=$oldessarea){
						  if($debitnoteveninfoch!=''){
								$debitnoteveninfoch.='<br> SHIPPING STREET <span style="color:green;" id="prohisfromtospan">( From '.$oldessarea.' To '.$sarea.' ) </span>';
						  }
						  else{
								$debitnoteveninfoch.=''.(($debitnoteinfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessuserven['modulename'].' Information</span> <br> SHIPPING STREET <span style="color:green;" id="prohisfromtospan">( From '.$oldessarea.' To '.$sarea.' ) </span>';
						  }                   
					 }
					 if($scity!=$oldesscity){
						  if($debitnoteveninfoch!=''){
								$debitnoteveninfoch.='<br> SHIPPING CITY/TOWN <span style="color:green;" id="prohisfromtospan">( From '.$oldesscity.' To '.$scity.' ) </span>';
						  }
						  else{
								$debitnoteveninfoch.=''.(($debitnoteinfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessuserven['modulename'].' Information</span> <br> SHIPPING CITY/TOWN <span style="color:green;" id="prohisfromtospan">( From '.$oldesscity.' To '.$scity.' ) </span>';
						  }                   
					 }
					 if($sstate!=$oldessstate){
						  if($debitnoteveninfoch!=''){
								$debitnoteveninfoch.='<br> SHIPPING STATE <span style="color:green;" id="prohisfromtospan">( From '.$oldessstate.' To '.$sstate.' ) </span>';
						  }
						  else{
								$debitnoteveninfoch.=''.(($debitnoteinfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessuserven['modulename'].' Information</span> <br> SHIPPING STATE <span style="color:green;" id="prohisfromtospan">( From '.$oldessstate.' To '.$sstate.' ) </span>';
						  }                   
					 }
					 if($sdistrict!=$oldessdistrict){
						  if($debitnoteveninfoch!=''){
								$debitnoteveninfoch.='<br> SHIPPING PIN <span style="color:green;" id="prohisfromtospan">( From '.$oldessdistrict.' To '.$sdistrict.' ) </span>';
						  }
						  else{
								$debitnoteveninfoch.=''.(($debitnoteinfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessuserven['modulename'].' Information</span> <br> SHIPPING PIN <span style="color:green;" id="prohisfromtospan">( From '.$oldessdistrict.' To '.$sdistrict.' ) </span>';
						  }                   
					 }
					 if($spincode!=$oldesspincode){
						  if($debitnoteveninfoch!=''){
								$debitnoteveninfoch.='<br> SHIPPING COUNTRY/REGION <span style="color:green;" id="prohisfromtospan">( From '.$oldesspincode.' To '.$spincode.' ) </span>';
						  }
						  else{
								$debitnoteveninfoch.=''.(($debitnoteinfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessuserven['modulename'].' Information</span> <br> SHIPPING COUNTRY/REGION <span style="color:green;" id="prohisfromtospan">( From '.$oldesspincode.' To '.$spincode.' ) </span>';
						  }                   
					 }
					 if($twoworkphone!=$oldestwoworkphone){
						  if($debitnoteveninfoch!=''){
								$debitnoteveninfoch.='<br> WORK PHONE <span style="color:green;" id="prohisfromtospan">( From '.$oldestwoworkphone.' To '.$twoworkphone.' ) </span>';
						  }
						  else{
								$debitnoteveninfoch.=''.(($debitnoteinfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessuserven['modulename'].' Information</span> <br> WORK PHONE <span style="color:green;" id="prohisfromtospan">( From '.$oldestwoworkphone.' To '.$twoworkphone.' ) </span>';
						  }                   
					 }
					 if($twomobilephone!=$oldestwomobilephone){
						  if($debitnoteveninfoch!=''){
								$debitnoteveninfoch.='<br> MOBILE PHONE <span style="color:green;" id="prohisfromtospan">( From '.$oldestwomobilephone.' To '.$twomobilephone.' ) </span>';
						  }
						  else{
								$debitnoteveninfoch.=''.(($debitnoteinfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessuserven['modulename'].' Information</span> <br> MOBILE PHONE <span style="color:green;" id="prohisfromtospan">( From '.$oldestwomobilephone.' To '.$twomobilephone.' ) </span>';
						  }                   
					 }
					 if($gstrtype!=$oldesgstrtype){
						  if($debitnoteveninfoch!=''){
								$debitnoteveninfoch.='<br> GST TREATMENT <span style="color:green;" id="prohisfromtospan">( From '.$oldesgstrtype.' To '.$gstrtype.' ) </span>';
						  }
						  else{
								$debitnoteveninfoch.=''.(($debitnoteinfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessuserven['modulename'].' Information</span> <br> GST TREATMENT <span style="color:green;" id="prohisfromtospan">( From '.$oldesgstrtype.' To '.$gstrtype.' ) </span>';
						  }                   
					 }
					 if($gstno!=$oldesgstno){
						  if($debitnoteveninfoch!=''){
								$debitnoteveninfoch.='<br> GSTIN <span style="color:green;" id="prohisfromtospan">( From '.$oldesgstno.' To '.$gstno.' ) </span>';
						  }
						  else{
								$debitnoteveninfoch.=''.(($debitnoteinfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessuserven['modulename'].' Information</span> <br> GSTIN <span style="color:green;" id="prohisfromtospan">( From '.$oldesgstno.' To '.$gstno.' ) </span>';
						  }                   
					 }
					 if($pos!=$oldespos){
						  if($debitnoteveninfoch!=''){
								$debitnoteveninfoch.='<br> PLACE OF SUPPLY <span style="color:green;" id="prohisfromtospan">( From '.$oldespos.' To '.$pos.' ) </span>';
						  }
						  else{
								$debitnoteveninfoch.=''.(($debitnoteinfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessuserven['modulename'].' Information</span> <br> PLACE OF SUPPLY <span style="color:green;" id="prohisfromtospan">( From '.$oldespos.' To '.$pos.' ) </span>';
						  }                   
					 }
					 if($dlno20!=$oldesdlno20){
						  if($debitnoteveninfoch!=''){
								$debitnoteveninfoch.='<br> DL No 20 <span style="color:green;" id="prohisfromtospan">( From '.$oldesdlno20.' To '.$dlno20.' ) </span>';
						  }
						  else{
								$debitnoteveninfoch.=''.(($debitnoteinfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessuserven['modulename'].' Information</span> <br> DL No 20 <span style="color:green;" id="prohisfromtospan">( From '.$oldesdlno20.' To '.$dlno20.' ) </span>';
						  }                   
					 }
					 if($dlno21!=$oldesdlno21){
						  if($debitnoteveninfoch!=''){
								$debitnoteveninfoch.='<br> DL No 21 <span style="color:green;" id="prohisfromtospan">( From '.$oldesdlno21.' To '.$dlno21.' ) </span>';
						  }
						  else{
								$debitnoteveninfoch.=''.(($debitnoteinfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessuserven['modulename'].' Information</span> <br> DL No 21 <span style="color:green;" id="prohisfromtospan">( From '.$oldesdlno21.' To '.$dlno21.' ) </span>';
						  }                   
					 }
								...
								if($productname!=''){
									 if($productname!=$oldesproductname){
										  if($debitnoteiteminfoch!=''){
												$debitnoteiteminfoch.='<br> '.$infomainaccessuserpro['modulename'].' Name <span style="color:green;" id="prohisfromtospan">( From '.$oldesproductname.' To '.$productname.' ) </span>';
										  }
										  else{
												$debitnoteiteminfoch.=''.(($debitnoteinfoch!=''||$debitnoteveninfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> '.$infomainaccessuserpro['modulename'].' Name <span style="color:green;" id="prohisfromtospan">( From '.$oldesproductname.' To '.$productname.' ) </span>';
										  }                   
									 }
									 if($manufacturer!=$oldesmanufacturer){
										  if($debitnoteiteminfoch!=''){
												$debitnoteiteminfoch.='<br> '.$access['txtnamecategory'].' <span style="color:green;" id="prohisfromtospan">( From '.$oldesmanufacturer.' To '.$manufacturer.' ) </span>';
										  }
										  else{
												$debitnoteiteminfoch.=''.(($debitnoteinfoch!=''||$debitnoteveninfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> '.$access['txtnamecategory'].' <span style="color:green;" id="prohisfromtospan">( From '.$oldesmanufacturer.' To '.$manufacturer.' ) </span>';
										  }                   
									 }
									 if($producthsn!=$oldesproducthsn){
										  if($debitnoteiteminfoch!=''){
												$debitnoteiteminfoch.='<br> HSN Code <span style="color:green;" id="prohisfromtospan">( From '.$oldesproducthsn.' To '.$producthsn.' ) </span>';
										  }
										  else{
												$debitnoteiteminfoch.=''.(($debitnoteinfoch!=''||$debitnoteveninfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> HSN Code <span style="color:green;" id="prohisfromtospan">( From '.$oldesproducthsn.' To '.$producthsn.' ) </span>';
										  }                   
									 }
									 if($productdescription!=$oldesproductdescription){
										  if($debitnoteiteminfoch!=''){
												$debitnoteiteminfoch.='<br> Description <span style="color:green;" id="prohisfromtospan">( From '.$oldesproductdescription.' To '.$productdescription.' ) </span>';
										  }
										  else{
												$debitnoteiteminfoch.=''.(($debitnoteinfoch!=''||$debitnoteveninfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Description <span style="color:green;" id="prohisfromtospan">( From '.$oldesproductdescription.' To '.$productdescription.' ) </span>';
										  }                   
									 }
									 if($batch!=$oldesbatch){
										  if($debitnoteiteminfoch!=''){
												$debitnoteiteminfoch.='<br> Batch <span style="color:green;" id="prohisfromtospan">( From '.$oldesbatch.' To '.$batch.' ) </span>';
										  }
										  else{
												$debitnoteiteminfoch.=''.(($debitnoteinfoch!=''||$debitnoteveninfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Batch <span style="color:green;" id="prohisfromtospan">( From '.$oldesbatch.' To '.$batch.' ) </span>';
										  }                   
									 }
									 if($expdate!=$oldesexpdate){
										  if($debitnoteiteminfoch!=''){
												$debitnoteiteminfoch.='<br> Expiry <span style="color:green;" id="prohisfromtospan">( From '.date($date,strtotime($oldesexpdate)).' To '.date($date,strtotime($expdate)).' ) </span>';
										  }
										  else{
												$debitnoteiteminfoch.=''.(($debitnoteinfoch!=''||$debitnoteveninfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Expiry <span style="color:green;" id="prohisfromtospan">( From '.date($date,strtotime($oldesexpdate)).' To '.date($date,strtotime($expdate)).' ) </span>';
										  }                   
									 }
									 if($productrate!=$oldesproductrate){
										  if($debitnoteiteminfoch!=''){
												$debitnoteiteminfoch.='<br> Rate <span style="color:green;" id="prohisfromtospan">( From '.$oldesproductrate.' To '.$productrate.' ) </span>';
										  }
										  else{
												$debitnoteiteminfoch.=''.(($debitnoteinfoch!=''||$debitnoteveninfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Rate <span style="color:green;" id="prohisfromtospan">( From '.$oldesproductrate.' To '.$productrate.' ) </span>';
										  }                   
									 }
									 if($mrp!=$oldesmrp){
										  if($debitnoteiteminfoch!=''){
												$debitnoteiteminfoch.='<br> Mrp <span style="color:green;" id="prohisfromtospan">( From '.$oldesmrp.' To '.$mrp.' ) </span>';
										  }
										  else{
												$debitnoteiteminfoch.=''.(($debitnoteinfoch!=''||$debitnoteveninfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Mrp <span style="color:green;" id="prohisfromtospan">( From '.$oldesmrp.' To '.$mrp.' ) </span>';
										  }                   
									 }
									 if($quantity!=$oldesquantity){
										  if($debitnoteiteminfoch!=''){
												$debitnoteiteminfoch.='<br> Quantity <span style="color:green;" id="prohisfromtospan">( From '.$oldesquantity.' To '.$quantity.' ) </span>';
										  }
										  else{
												$debitnoteiteminfoch.=''.(($debitnoteinfoch!=''||$debitnoteveninfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Quantity <span style="color:green;" id="prohisfromtospan">( From '.$oldesquantity.' To '.$quantity.' ) </span>';
										  }                   
									 }
									 if($unit!=$oldesunit){
										  if($debitnoteiteminfoch!=''){
												$debitnoteiteminfoch.='<br> Unit <span style="color:green;" id="prohisfromtospan">( From '.$oldesunit.' To '.$unit.' ) </span>';
										  }
										  else{
												$debitnoteiteminfoch.=''.(($debitnoteinfoch!=''||$debitnoteveninfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Unit <span style="color:green;" id="prohisfromtospan">( From '.$oldesunit.' To '.$unit.' ) </span>';
										  }                   
									 }
									 if($noofpacks!=$oldesnoofpacks){
										  if($debitnoteiteminfoch!=''){
												$debitnoteiteminfoch.='<br> Pack <span style="color:green;" id="prohisfromtospan">( From '.$oldesnoofpacks.' To '.$noofpacks.' ) </span>';
										  }
										  else{
												$debitnoteiteminfoch.=''.(($debitnoteinfoch!=''||$debitnoteveninfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Pack <span style="color:green;" id="prohisfromtospan">( From '.$oldesnoofpacks.' To '.$noofpacks.' ) </span>';
										  }                   
									 }
									 if($productvalue!=$oldesproductvalue){
										  if($debitnoteiteminfoch!=''){
												$debitnoteiteminfoch.='<br> Taxable Value <span style="color:green;" id="prohisfromtospan">( From '.$oldesproductvalue.' To '.$productvalue.' ) </span>';
										  }
										  else{
												$debitnoteiteminfoch.=''.(($debitnoteinfoch!=''||$debitnoteveninfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Taxable Value <span style="color:green;" id="prohisfromtospan">( From '.$oldesproductvalue.' To '.$productvalue.' ) </span>';
										  }                   
									 }
									 if($prodiscount!=$oldesprodiscount){
										  if($debitnoteiteminfoch!=''){
												$debitnoteiteminfoch.='<br> '.$access['txtprodisdebitnote'].' <span style="color:green;" id="prohisfromtospan">( From '.$oldesprodiscount.' To '.$prodiscount.' ) </span>';
										  }
										  else{
												$debitnoteiteminfoch.=''.(($debitnoteinfoch!=''||$debitnoteveninfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> '.$access['txtprodisdebitnote'].' <span style="color:green;" id="prohisfromtospan">( From '.$oldesprodiscount.' To '.$prodiscount.' ) </span>';
										  }                   
									 }
									 if($prodiscounttype!=$oldesprodiscounttype){
										  if($debitnoteiteminfoch!=''){
												$debitnoteiteminfoch.='<br> Discounted By <span style="color:green;" id="prohisfromtospan">( From '.(($oldesprodiscounttype==0)?'%':'?').' To '.(($prodiscounttype==0)?'%':'?').' ) </span>';
										  }
										  else{
												$debitnoteiteminfoch.=''.(($debitnoteinfoch!=''||$debitnoteveninfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Discounted By <span style="color:green;" id="prohisfromtospan">( From '.(($oldesprodiscounttype==0)?'%':'?').' To '.(($prodiscounttype==0)?'%':'?').' ) </span>';
										  }                   
									 }
									 if($taxvalue!=$oldestaxvalue){
										  if($debitnoteiteminfoch!=''){
												$debitnoteiteminfoch.='<br> Tax Value <span style="color:green;" id="prohisfromtospan">( From '.$oldestaxvalue.' To '.$taxvalue.' ) </span>';
										  }
										  else{
												$debitnoteiteminfoch.=''.(($debitnoteinfoch!=''||$debitnoteveninfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Tax Value <span style="color:green;" id="prohisfromtospan">( From '.$oldestaxvalue.' To '.$taxvalue.' ) </span>';
										  }                   
									 }
									 if($vat!=$oldesvat){
										  if($debitnoteiteminfoch!=''){
												$debitnoteiteminfoch.='<br> Gst <span style="color:green;" id="prohisfromtospan">( From '.$oldesvat.' % ('.$oldesicsgsthis.') To '.$vat.' % ('.$ansforsepgstval.') ) </span>';
										  }
										  else{
												$debitnoteiteminfoch.=''.(($debitnoteinfoch!=''||$debitnoteveninfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Gst <span style="color:green;" id="prohisfromtospan">( From '.$oldesvat.' % ('.$oldesicsgsthis.') To '.$vat.' % ('.$ansforsepgstval.') ) </span>';
										  }                   
									 }
									 if($productnetvalue!=$oldesproductnetvalue){
										  if($debitnoteiteminfoch!=''){
												$debitnoteiteminfoch.='<br> Amount <span style="color:green;" id="prohisfromtospan">( From '.$oldesproductnetvalue.' To '.$productnetvalue.' ) </span>';
										  }
										  else{
												$debitnoteiteminfoch.=''.(($debitnoteinfoch!=''||$debitnoteveninfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Amount <span style="color:green;" id="prohisfromtospan">( From '.$oldesproductnetvalue.' To '.$productnetvalue.' ) </span>';
										  }                   
									 }
									 if($salequantity!=$oldessalequantity){
										  if($debitnoteiteminfoch!=''){
												$debitnoteiteminfoch.='<br> '.($access['debitnotetxtsqty']).' <span style="color:green;" id="prohisfromtospan">( From '.$oldessalequantity.' To '.$salequantity.' ) </span>';
										  }
										  else{
												$debitnoteiteminfoch.=''.(($debitnoteinfoch!=''||$debitnoteveninfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> '.($access['debitnotetxtsqty']).' <span style="color:green;" id="prohisfromtospan">( From '.$oldessalequantity.' To '.$salequantity.' ) </span>';
										  }                   
									 }
									 if($productwithouttax!=$oldesproductwithouttax){
										  if($debitnoteiteminfoch!=''){
												$debitnoteiteminfoch.='<br> Sale Rate/Unit(Without Gst) <span style="color:green;" id="prohisfromtospan">( From '.$oldesproductwithouttax.' To '.$productwithouttax.' ) </span>';
										  }
										  else{
												$debitnoteiteminfoch.=''.(($debitnoteinfoch!=''||$debitnoteveninfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Sale Rate/Unit(Without Gst) <span style="color:green;" id="prohisfromtospan">( From '.$oldesproductwithouttax.' To '.$productwithouttax.' ) </span>';
										  }                   
									 }
									 if($productwithtax!=$oldesproductwithtax){
										  if($debitnoteiteminfoch!=''){
												$debitnoteiteminfoch.='<br> Sale Rate/Unit(With Gst) <span style="color:green;" id="prohisfromtospan">( From '.$oldesproductwithtax.' To '.$productwithtax.' ) </span>';
										  }
										  else{
												$debitnoteiteminfoch.=''.(($debitnoteinfoch!=''||$debitnoteveninfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Sale Rate/Unit(With Gst) <span style="color:green;" id="prohisfromtospan">( From '.$oldesproductwithtax.' To '.$productwithtax.' ) </span>';
										  }                   
									 }
								}
						  }
						  ...
						  for($i=0; $i<count($_POST['oldesproductname']); $i++){
								...
								if($productname!=''){
									 ...
									 if($oldesproductname!=''){                          
										  if($oldesproductname!=''){
												if($debitnoteiteminfoch!=''){
													 $debitnoteiteminfoch.='<br> '.$infomainaccessuserpro['modulename'].' Name <span style="color:red;" id="prohisfromtospan">( From '.$oldesproductname.' To &nbsp; ) </span>';
												}
												else{
													 $debitnoteiteminfoch.=''.(($debitnoteinfoch!=''||$debitnoteveninfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> '.$infomainaccessuserpro['modulename'].' Name <span style="color:red;" id="prohisfromtospan">( From '.$oldesproductname.' To &nbsp; ) </span>';
												}
										  }
										  if($oldesmanufacturer!=''){
												if($debitnoteiteminfoch!=''){
													 $debitnoteiteminfoch.='<br> '.$access['txtnamecategory'].' <span style="color:red;" id="prohisfromtospan">( From '.$oldesmanufacturer.' To &nbsp; ) </span>';
												}
												else{
													 $debitnoteiteminfoch.=''.(($debitnoteinfoch!=''||$debitnoteveninfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> '.$access['txtnamecategory'].' <span style="color:red;" id="prohisfromtospan">( From '.$oldesmanufacturer.' To &nbsp; ) </span>';
												}
										  }
										  if($oldesproducthsn!=''){
												if($debitnoteiteminfoch!=''){
													 $debitnoteiteminfoch.='<br> HSN Code <span style="color:red;" id="prohisfromtospan">( From '.$oldesproducthsn.' To &nbsp; ) </span>';
												}
												else{
													 $debitnoteiteminfoch.=''.(($debitnoteinfoch!=''||$debitnoteveninfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> HSN Code <span style="color:red;" id="prohisfromtospan">( From '.$oldesproducthsn.' To &nbsp; ) </span>';
												}
										  }
										  if($oldesproductdescription!=''){
												if($debitnoteiteminfoch!=''){
													 $debitnoteiteminfoch.='<br> Description <span style="color:red;" id="prohisfromtospan">( From '.$oldesproductdescription.' To &nbsp; ) </span>';
												}
												else{
													 $debitnoteiteminfoch.=''.(($debitnoteinfoch!=''||$debitnoteveninfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Description <span style="color:red;" id="prohisfromtospan">( From '.$oldesproductdescription.' To &nbsp; ) </span>';
												}
										  }
										  if($oldesbatch!=''){
												if($debitnoteiteminfoch!=''){
													 $debitnoteiteminfoch.='<br> Batch <span style="color:red;" id="prohisfromtospan">( From '.$oldesbatch.' To &nbsp; ) </span>';
												}
												else{
													 $debitnoteiteminfoch.=''.(($debitnoteinfoch!=''||$debitnoteveninfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Batch <span style="color:red;" id="prohisfromtospan">( From '.$oldesbatch.' To &nbsp; ) </span>';
												}
										  }
										  if($oldesexpdate!=''){
												if($debitnoteiteminfoch!=''){
													 $debitnoteiteminfoch.='<br> Expiry <span style="color:red;" id="prohisfromtospan">( From '.date($date,strtotime($oldesexpdate)).' To &nbsp; ) </span>';
												}
												else{
													 $debitnoteiteminfoch.=''.(($debitnoteinfoch!=''||$debitnoteveninfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Expiry <span style="color:red;" id="prohisfromtospan">( From '.date($date,strtotime($oldesexpdate)).' To &nbsp; ) </span>';
												}
										  }
										  if($oldesproductrate!=''){
												if($debitnoteiteminfoch!=''){
													 $debitnoteiteminfoch.='<br> Rate <span style="color:red;" id="prohisfromtospan">( From '.$oldesproductrate.' To &nbsp; ) </span>';
												}
												else{
													 $debitnoteiteminfoch.=''.(($debitnoteinfoch!=''||$debitnoteveninfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Rate <span style="color:red;" id="prohisfromtospan">( From '.$oldesproductrate.' To &nbsp; ) </span>';
												}
										  }
										  if($oldesmrp!=''){
												if($debitnoteiteminfoch!=''){
													 $debitnoteiteminfoch.='<br> Mrp <span style="color:red;" id="prohisfromtospan">( From '.$oldesmrp.' To &nbsp; ) </span>';
												}
												else{
													 $debitnoteiteminfoch.=''.(($debitnoteinfoch!=''||$debitnoteveninfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Mrp <span style="color:red;" id="prohisfromtospan">( From '.$oldesmrp.' To &nbsp; ) </span>';
												}
										  }
										  if($oldesquantity!=''){
												if($debitnoteiteminfoch!=''){
													 $debitnoteiteminfoch.='<br> Quantity <span style="color:red;" id="prohisfromtospan">( From '.$oldesquantity.' To &nbsp; ) </span>';
												}
												else{
													 $debitnoteiteminfoch.=''.(($debitnoteinfoch!=''||$debitnoteveninfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Quantity <span style="color:red;" id="prohisfromtospan">( From '.$oldesquantity.' To &nbsp; ) </span>';
												}
										  }
										  if($oldesunit!=''){
												if($debitnoteiteminfoch!=''){
													 $debitnoteiteminfoch.='<br> Unit <span style="color:red;" id="prohisfromtospan">( From '.$oldesunit.' To &nbsp; ) </span>';
												}
												else{
													 $debitnoteiteminfoch.=''.(($debitnoteinfoch!=''||$debitnoteveninfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Unit <span style="color:red;" id="prohisfromtospan">( From '.$oldesunit.' To &nbsp; ) </span>';
												}
										  }
										  if($oldesnoofpacks!=''){
												if($debitnoteiteminfoch!=''){
													 $debitnoteiteminfoch.='<br> Pack <span style="color:red;" id="prohisfromtospan">( From '.$oldesnoofpacks.' To &nbsp; ) </span>';
												}
												else{
													 $debitnoteiteminfoch.=''.(($debitnoteinfoch!=''||$debitnoteveninfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Pack <span style="color:red;" id="prohisfromtospan">( From '.$oldesnoofpacks.' To &nbsp; ) </span>';
												}
										  }
										  if($oldesproductvalue!=''){
												if($debitnoteiteminfoch!=''){
													 $debitnoteiteminfoch.='<br> Taxable Value <span style="color:red;" id="prohisfromtospan">( From '.$oldesproductvalue.' To &nbsp; ) </span>';
												}
												else{
													 $debitnoteiteminfoch.=''.(($debitnoteinfoch!=''||$debitnoteveninfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Taxable Value <span style="color:red;" id="prohisfromtospan">( From '.$oldesproductvalue.' To &nbsp; ) </span>';
												}
										  }
										  if($oldesprodiscount!=''){
												if($debitnoteiteminfoch!=''){
													 $debitnoteiteminfoch.='<br> Discount <span style="color:red;" id="prohisfromtospan">( From '.$oldesprodiscount.' To &nbsp; ) </span>';
												}
												else{
													 $debitnoteiteminfoch.=''.(($debitnoteinfoch!=''||$debitnoteveninfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Discount <span style="color:red;" id="prohisfromtospan">( From '.$oldesprodiscount.' To &nbsp; ) </span>';
												}
										  }
										  if($oldestaxvalue!=''){
												if($debitnoteiteminfoch!=''){
													 $debitnoteiteminfoch.='<br> Tax Value <span style="color:red;" id="prohisfromtospan">( From '.$oldestaxvalue.' To &nbsp; ) </span>';
												}
												else{
													 $debitnoteiteminfoch.=''.(($debitnoteinfoch!=''||$debitnoteveninfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Tax Value <span style="color:red;" id="prohisfromtospan">( From '.$oldestaxvalue.' To &nbsp; ) </span>';
												}
										  }
										  if($oldesvat!=''){
												if($debitnoteiteminfoch!=''){
													 $debitnoteiteminfoch.='<br> Gst <span style="color:red;" id="prohisfromtospan">( From '.$oldesvat.' % ('.$oldesicsgsthis.') To &nbsp; ) </span>';
												}
												else{
													 $debitnoteiteminfoch.=''.(($debitnoteinfoch!=''||$debitnoteveninfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Gst <span style="color:red;" id="prohisfromtospan">( From '.$oldesvat.' % ('.$oldesicsgsthis.') To &nbsp; ) </span>';
												}
										  }
										  if($oldesproductnetvalue!=''){
												if($debitnoteiteminfoch!=''){
													 $debitnoteiteminfoch.='<br> Amount <span style="color:red;" id="prohisfromtospan">( From '.$oldesproductnetvalue.' To &nbsp; ) </span>';
												}
												else{
													 $debitnoteiteminfoch.=''.(($debitnoteinfoch!=''||$debitnoteveninfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Amount <span style="color:red;" id="prohisfromtospan">( From '.$oldesproductnetvalue.' To &nbsp; ) </span>';
												}
										  }
										  if($oldessalequantity!=''){
												if($debitnoteiteminfoch!=''){
													 $debitnoteiteminfoch.='<br> '.($access['debitnotetxtsqty']).' <span style="color:red;" id="prohisfromtospan">( From '.$oldessalequantity.' To &nbsp; ) </span>';
												}
												else{
													 $debitnoteiteminfoch.=''.(($debitnoteinfoch!=''||$debitnoteveninfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> '.($access['debitnotetxtsqty']).' <span style="color:red;" id="prohisfromtospan">( From '.$oldessalequantity.' To &nbsp; ) </span>';
												}
										  }
										  if($oldesproductwithouttax!=''){
												if($debitnoteiteminfoch!=''){
													 $debitnoteiteminfoch.='<br> Sale Rate/Unit(Without Gst) <span style="color:red;" id="prohisfromtospan">( From '.$oldesproductwithouttax.' To &nbsp; ) </span>';
												}
												else{
													 $debitnoteiteminfoch.=''.(($debitnoteinfoch!=''||$debitnoteveninfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Sale Rate/Unit(Without Gst) <span style="color:red;" id="prohisfromtospan">( From '.$oldesproductwithouttax.' To &nbsp; ) </span>';
												}
										  }
										  if($oldesproductwithtax!=''){
												if($debitnoteiteminfoch!=''){
													 $debitnoteiteminfoch.='<br> Sale Rate/Unit(With Gst) <span style="color:red;" id="prohisfromtospan">( From '.$oldesproductwithtax.' To &nbsp; ) </span>';
												}
												else{
													 $debitnoteiteminfoch.=''.(($debitnoteinfoch!=''||$debitnoteveninfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Sale Rate/Unit(With Gst) <span style="color:red;" id="prohisfromtospan">( From '.$oldesproductwithtax.' To &nbsp; ) </span>';
												}
										  }
									 }
								}
					 ...
					 elseif ($curans>$oldans) {
						  for($i=0; $i<count($_POST['productname']); $i++){
								...
								if($oldesproductname!=''){
									 ...
									 if($expdate!=''){
										  if($debitnoteiteminfoch!=''){
												$debitnoteiteminfoch.='<br> Expiry <span style="color:green;" id="prohisfromtospan">( '.date($date,strtotime($expdate)).' ) </span>';
										  }
										  else{
												$debitnoteiteminfoch.=''.(($debitnoteinfoch!=''||$debitnoteveninfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Expiry <span style="color:green;" id="prohisfromtospan">( '.date($date,strtotime($expdate)).' ) </span>';        
										  }
									 }
									 if($productrate!=''){
										  if($debitnoteiteminfoch!=''){
					 ...
					 if($totalitems!=$oldestotalitems){
						  if($debitnoteiteminfoch!=''){
								$debitnoteiteminfoch.='<br> Total Items <span style="color:green;" id="prohisfromtospan">( From '.$oldestotalitems.' To '.$totalitems.' ) </span>';
						  }
						  else{
								$debitnoteiteminfoch.=''.(($debitnoteinfoch!=''||$debitnoteveninfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Total Items <span style="color:green;" id="prohisfromtospan">( From '.$oldestotalitems.' To '.$totalitems.' ) </span>';
						  }
					 }
					 if($totalquantity!=$oldestotalquantity){
						  if($debitnoteiteminfoch!=''){
								$debitnoteiteminfoch.='<br> Total Qty <span style="color:green;" id="prohisfromtospan">( From '.$oldestotalquantity.' To '.$totalquantity.' ) </span>';
						  }
						  else{
								$debitnoteiteminfoch.=''.(($debitnoteinfoch!=''||$debitnoteveninfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Total Qty <span style="color:green;" id="prohisfromtospan">( From '.$oldestotalquantity.' To '.$totalquantity.' ) </span>';
						  }
					 }
					 if($totalamount!=$oldestotalamount){
						  if($debitnoteiteminfoch!=''){
								$debitnoteiteminfoch.='<br> Sub Total <span style="color:green;" id="prohisfromtospan">( From '.$oldestotalamount.' To '.$totalamount.' ) </span>';
						  }
						  else{
								$debitnoteiteminfoch.=''.(($debitnoteinfoch!=''||$debitnoteveninfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Sub Total <span style="color:green;" id="prohisfromtospan">( From '.$oldestotalamount.' To '.$totalamount.' ) </span>';
						  }
					 }
					 if($discountamount!=$oldesdiscountamount){
						  if($debitnoteiteminfoch!=''){
								$debitnoteiteminfoch.='<br> Discount <span style="color:green;" id="prohisfromtospan">( From '.$oldesdiscountamount.' To '.$discountamount.' ) </span>';
						  }
						  else{
								$debitnoteiteminfoch.=''.(($debitnoteinfoch!=''||$debitnoteveninfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Discount <span style="color:green;" id="prohisfromtospan">( From '.$oldesdiscountamount.' To '.$discountamount.' ) </span>';
						  }
					 }
					 if($totalvatamount!=$oldestotalvatamount){
						  if($debitnoteiteminfoch!=''){
								$debitnoteiteminfoch.='<br> Total Tax <span style="color:green;" id="prohisfromtospan">( From '.$oldestotalvatamount.' To '.$totalvatamount.' ) </span>';
						  }
						  else{
								$debitnoteiteminfoch.=''.(($debitnoteinfoch!=''||$debitnoteveninfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Total Tax <span style="color:green;" id="prohisfromtospan">( From '.$oldestotalvatamount.' To '.$totalvatamount.' ) </span>';
						  }
					 }
					 if($roundoff!=$oldesroundoff){
						  if($debitnoteiteminfoch!=''){
								$debitnoteiteminfoch.='<br> Round Off <span style="color:green;" id="prohisfromtospan">( From '.$oldesroundoff.' To '.$roundoff.' ) </span>';
						  }
						  else{
								$debitnoteiteminfoch.=''.(($debitnoteinfoch!=''||$debitnoteveninfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Round Off <span style="color:green;" id="prohisfromtospan">( From '.$oldesroundoff.' To '.$roundoff.' ) </span>';
						  }
					 }
					 if($grandtotal!=$oldesgrandtotal){
						  if($debitnoteiteminfoch!=''){
								$debitnoteiteminfoch.='<br> Grand Total <span style="color:green;" id="prohisfromtospan">( From '.$oldesgrandtotal.' To '.$grandtotal.' ) </span>';
						  }
						  else{
								$debitnoteiteminfoch.=''.(($debitnoteinfoch!=''||$debitnoteveninfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Grand Total <span style="color:green;" id="prohisfromtospan">( From '.$oldesgrandtotal.' To '.$grandtotal.' ) </span>';
						  }
					 }
					 if($description!=$oldesdescription){
						  if($debitnoteiteminfoch!=''){
								$debitnoteiteminfoch.='<br> Description <span style="color:green;" id="prohisfromtospan">( From '.$oldesdescription.' To '.$description.' ) </span>';
						  }
						  else{
								$debitnoteiteminfoch.=''.(($debitnoteinfoch!=''||$debitnoteveninfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Description <span style="color:green;" id="prohisfromtospan">( From '.$oldesdescription.' To '.$description.' ) </span>';
						  }
					 }
					 if($notes!=$oldesnotes){
						  if($debitnoteiteminfoch!=''){
								$debitnoteiteminfoch.='<br> Notes <span style="color:green;" id="prohisfromtospan">( From '.$oldesnotes.' To '.$notes.' ) </span>';
						  }
						  else{
								$debitnoteiteminfoch.=''.(($debitnoteinfoch!=''||$debitnoteveninfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Notes <span style="color:green;" id="prohisfromtospan">( From '.$oldesnotes.' To '.$notes.' ) </span>';
						  }
					 }
					 if($terms!=$oldesterms){
						  if($debitnoteiteminfoch!=''){
								$debitnoteiteminfoch.='<br> Terms And Conditions <span style="color:green;" id="prohisfromtospan">( From '.$oldesterms.' To '.$terms.' ) </span>';
						  }
						  else{
								$debitnoteiteminfoch.=''.(($debitnoteinfoch!=''||$debitnoteveninfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Terms And Conditions <span style="color:green;" id="prohisfromtospan">( From '.$oldesterms.' To '.$terms.' ) </span>';
						  }
					 }
					 if($fileattach!=$oldesfileattach){
						  if($debitnoteiteminfoch!=''){
								$debitnoteiteminfoch.='<br> Attach <span style="color:green;" id="prohisfromtospan">( Changed ) </span>';
						  }
						  else{
								$debitnoteiteminfoch.=''.(($debitnoteinfoch!=''||$debitnoteveninfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Attach <span style="color:green;" id="prohisfromtospan">( Changed ) </span>';
						  }
					 }
					...
					//FOR GET BILL MODULENAME AND OTHER DETAILS
					 if($billno!=$oldesbillno){
						  if($debitnoteiteminfoch!=''){
								$debitnoteiteminfoch.='<br> '.$infomainaccessuserbill['modulename'].' Number <span style="color:green;" id="prohisfromtospan">( From '.$oldesbillno.' To '.$billno.' ) </span>';
						  }
						  else{
								$debitnoteiteminfoch.=''.(($debitnoteinfoch!=''||$debitnoteveninfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> '.$infomainaccessuserbill['modulename'].' Number <span style="color:green;" id="prohisfromtospan">( From '.$oldesbillno.' To '.$billno.' ) </span>';
						  }
					 }
					 if($billamount!=$oldesbillamount){
						  if($debitnoteiteminfoch!=''){
								$debitnoteiteminfoch.='<br> '.$infomainaccessuserbill['modulename'].' Amount <span style="color:green;" id="prohisfromtospan">( From '.$oldesbillamount.' To '.$billamount.' ) </span>';
						  }
						  else{
								$debitnoteiteminfoch.=''.(($debitnoteinfoch!=''||$debitnoteveninfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> '.$infomainaccessuserbill['modulename'].' Amount <span style="color:green;" id="prohisfromtospan">( From '.$oldesbillamount.' To '.$billamount.' ) </span>';
						  }
					 }
					 if($billpaymentreceived!=$oldesbillpaymentreceived){
						  if($debitnoteiteminfoch!=''){
								$debitnoteiteminfoch.='<br> '.$infomainaccessuserbill['modulename'].' Payment Received <span style="color:green;" id="prohisfromtospan">( From '.$oldesbillpaymentreceived.' To '.$billpaymentreceived.' ) </span>';
						  }
						  else{
								$debitnoteiteminfoch.=''.(($debitnoteinfoch!=''||$debitnoteveninfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> '.$infomainaccessuserbill['modulename'].' Payment Received <span style="color:green;" id="prohisfromtospan">( From '.$oldesbillpaymentreceived.' To '.$billpaymentreceived.' ) </span>';
						  }
					 }
					 if($refundoption!=$oldesrefundoption){
						  if($debitnoteiteminfoch!=''){
								$debitnoteiteminfoch.='<br> Refund Option <span style="color:green;" id="prohisfromtospan">( From '.$oldesrefundoption.' To '.$refundoption.' ) </span>';
						  }
						  else{
								$debitnoteiteminfoch.=''.(($debitnoteinfoch!=''||$debitnoteveninfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Refund Option <span style="color:green;" id="prohisfromtospan">( From '.$oldesrefundoption.' To '.$refundoption.' ) </span>';
						  }
					 }
					 if($debitnoteterm!=$oldesdebitnoteterm){
						  if($debitnoteiteminfoch!=''){
								$debitnoteiteminfoch.='<br> Refund Method <span style="color:green;" id="prohisfromtospan">( From '.$oldesdebitnoteterm.' To '.$debitnoteterm.' ) </span>';
						  }
						  else{
								$debitnoteiteminfoch.=''.(($debitnoteinfoch!=''||$debitnoteveninfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Refund Method <span style="color:green;" id="prohisfromtospan">( From '.$oldesdebitnoteterm.' To '.$debitnoteterm.' ) </span>';
						  }
					 }
					 if($validpaidamount!=$oldesvalidpaidamount&&$debitnoteterm=='CASH'){
						  if($debitnoteiteminfoch!=''){
								$debitnoteiteminfoch.='<br> Refund Amount <span style="color:green;" id="prohisfromtospan">( From '.$oldesvalidpaidamount.' To '.$validpaidamount.' ) </span>';
						  }
						  else{
								$debitnoteiteminfoch.=''.(($debitnoteinfoch!=''||$debitnoteveninfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Refund Amount <span style="color:green;" id="prohisfromtospan">( From '.$oldesvalidpaidamount.' To '.$validpaidamount.' ) </span>';
						  }
					 }
					 // if($validbalance!=$oldesvalidbalance&&$debitnoteterm=='CASH'){
						//   if($debitnoteiteminfoch!=''){
						// 		$debitnoteiteminfoch.='<br> Balance Due <span style="color:green;" id="prohisfromtospan">( From '.$oldesvalidbalance.' To '.$validbalance.' ) </span>';
						//   }
						//   else{
						// 		$debitnoteiteminfoch.=''.(($debitnoteinfoch!=''||$debitnoteveninfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Balance Due <span style="color:green;" id="prohisfromtospan">( From '.$oldesvalidbalance.' To '.$validbalance.' ) </span>';
						//   }
					 // }
					 // if($duedates!=$oldesduedates&&$debitnoteterm=='CREDIT'){
						//   if($debitnoteiteminfoch!=''){
						// 		$debitnoteiteminfoch.='<br> Due Term <span style="color:green;" id="prohisfromtospan">( From '.$oldesduedates.' To '.$duedates.' ) </span>';
						//   }
						//   else{
						// 		$debitnoteiteminfoch.=''.(($debitnoteinfoch!=''||$debitnoteveninfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Due Term <span style="color:green;" id="prohisfromtospan">( From '.$oldesduedates.' To '.$duedates.' ) </span>';
						//   }
					 // }
					 // if($duedate!=$oldesduedate&&$debitnoteterm=='CREDIT'){
						//   if($debitnoteiteminfoch!=''){
						// 		$debitnoteiteminfoch.='<br> Due Date <span style="color:green;" id="prohisfromtospan">( From '.date($date,strtotime($oldesduedate)).' To '.date($date,strtotime($duedate)).' ) </span>';
						//   }
						//   else{
						// 		$debitnoteiteminfoch.=''.(($debitnoteinfoch!=''||$debitnoteveninfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Due Date <span style="color:green;" id="prohisfromtospan">( From '.date($date,strtotime($oldesduedate)).' To '.date($date,strtotime($duedate)).' ) </span>';
						//   }
					 // }
					 //FOR DEBITNOTE OTHER INFORMATIONS LIKE PAYMENT AND TERMS AND DESCRIPTION......
					 $debitnotetotinfoch = $debitnoteinfoch.$debitnoteveninfoch.$debitnoteiteminfoch;
					 ...
					 $sqlusehisupdqry->bind_param("sss", $purchaseid, $debitnoteno, $_GET['id']);
...
?>
	 ...
	 <?php
		  ...
		  if((isset($_GET['debitnoteno']))&&(isset($_GET['debitnotedate']))){
				$debitnoteno=htmlspecialchars( $_GET['debitnoteno'], ENT_QUOTES, 'UTF-8');
				$debitnotedate=htmlspecialchars( $_GET['debitnotedate'], ENT_QUOTES, 'UTF-8');
				$sql=$con->prepare("SELECT * FROM pairdebitnotes WHERE franchisesession=? AND createdid=? AND debitnoteno=? AND debitnotedate=? ORDER BY id ASC LIMIT 5");
				$sql->bind_param("ssss", $_SESSION['franchisesession'], $companymainid, $debitnoteno, $debitnotedate);
				$sql->execute();
				$count=1;
						  ...
						  <div class="card mb-4 mt-5">
								...
								?>
									 ...
									 <form autocomplete="off" id="debitnoteform" action="" onsubmit="return checkvalidate()" method="post" enctype="multipart/form-data" class="form-horizontal" role="form">
										  <input type="hidden" name="billno" id="billno" value="<?=$rows[0]['billno']?>">
										  <input type="hidden" name="billdate" id="billdate" value="<?=$rows[0]['billdate']?>">
									 ...
									 <?php
										  $oldarray=array( 'debitnoteno','invnumber','debitnotedate','reference','saleperson','preparedby','checkedby','invgrandtotal','vendorname','area','city','state','district','pincode','sarea','scity','sstate','sdistrict','spincode','workphone','mobile','gstrtype','gstno','pos','salequantity','dlno20','dlno21' );
									 ...
									 ?>
										  ...
										  <input type="hidden" value="<?=$rows[0]['debitnoteterm']?>" name="oldesdebitnoteterm">
										  <input type="hidden" value="<?= $rows[0]['validpaidamount'] ?>" name="oldesvalidpaidamount">
										  <input type="hidden" value="<?= $rows[0]['billamount'] ?>" name="oldesbillamount">
										  <input type="hidden" value="<?= $rows[0]['billpaymentreceived'] ?>" name="oldesbillpaymentreceived">
										  <input type="hidden" value="<?= $rows[0]['billno'] ?>" name="oldesbillno">
										  ...
										  <input type="hidden" name="olddebitnoteno" value="<?=$rows[0]['debitnoteno']?>">
										  <input type="hidden" name="olddebitnotedate" value="<?=$rows[0]['debitnotedate']?>">
										  <input type="hidden" name="debitnoteterm" value="<?=$rows[0]['debitnoteterm']?>">
										  <div class="row">
										  <?php
																		  ...
																		  <div class="row">
																				<div class="col-lg-12">
																					 <div class="form-group row" style="align-items: center !important;">
																						  ...
																						  <div class="col-lg-7">
																								<input type="text" class="form-control  form-control-sm" id="debitnoteno" name="debitnoteno" required  value="<?=$rows[0]['debitnoteno']?>" readonly>
																				...
																				<div class="col-lg-12">
																					 <div class="form-group row" style="align-items: center !important;">
																						  ...
																						  <?php
																								...
																								$dateformats->close();
																						  ?>
																								<input type="date" class="form-control  form-control-sm" id="debitnotedate" name="debitnotedate" required value="<?=$rows[0]['debitnotedate']?>">
																						  </div>
																					 </div>
																				</div>
																		  <?php
																		  ...
																		  ?>
																				...
																				?>
																					 ...
																					 <tr>
																						  ...
																						  <td data-label="<?=($access['txtqtydebitnote'])?>" <?=(in_array('Quantity', $fieldedit))?'':'style="display:none !important;"'?>>
																								<div>
																						  ...
																						  <td data-label="<?=($access['txttaxabledebitnote'])?>" <?=(in_array('Taxable Value', $fieldedit))?'':'style="display:none;"'?>>
																								<div>
																									 <span style="font-size:15px !important;">
																										  <?php echo $resmaincurrencyans; ?>
																									 </span>
																						  ...
																						  <td data-label="<?=$access['debitnotetxtsqty']?>" <?=(in_array('Sale Quantity', $fieldedit))?'':'style="display:none;"'?>>
																								<div>
																...
																<div class="table-responsive mt-1" id="gsttablediv" <?=(in_array('Tax Informations', $fieldedit))?'':'style="display:none;"'?>>
																	 <table class="table table-bordered" id="gsttable" style="font-size:12px;">
																		  ...
																		  <?php
																				...
																				$sqlitaxes=$con->prepare("SELECT * FROM pairdebitnotes WHERE franchisesession=? AND createdid=? AND debitnoteno=? AND debitnotedate=? ORDER BY id ASC");
																				$sqlitaxes->bind_param("ssss", $_SESSION["franchisesession"], $companymainid, $debitnoteno, $debitnotedate);
																				$sqlitaxes->execute();
												...
												<div class="modal-content">
													 ...
													 <div class="modal-body" style="height: max-content;">
														  ...
														  <table class="table notforfinaloutputsave showhideload">
																...
																<tr style="<?=(($rows[0]['billno']!='')?'':'display: none;')?>">
																	 ...
																	 <td style="background-color: #e9ecef;">
																		  <input type="text" name="validdebitnotebillno" id="validdebitnotebillno" class="form-control form-control-sm finalsubmitrequired" readonly required value="<?=$rows[0]['billno']?>">
																	 </td>
																</tr>
																<tr style="<?=(($rows[0]['billno']!='')?'':'display: none;')?>">
																	<th style="text-align:left !important;background-color: #e9ecef;">
																	...
																	<td style="background-color: #e9ecef;">
																		<input type="number" name="billamount" id="billamount" class="form-control form-control-sm finalsubmitrequired" readonly required value="<?=$rows[0]['billamount']?>">
																	</td>
																</tr>
																<tr style="<?=(($rows[0]['billno']!='')?'':'display: none;')?>">
																	<?php
																		...
																		$paidamountdr=0;
																		$currentbalance=0;
																		$sqlpurchasespays=$con->prepare("SELECT amount FROM pairpurchasepayhistory WHERE franchisesession=? AND createdid=? AND billno=? AND billdate=? ORDER BY id DESC");
																		$sqlpurchasespays->bind_param("ssss", $_SESSION['franchisesession'], $companymainid, $rows[0]['billno'], $rows[0]['billdate']);
																		$sqlpurchasespays->execute();
																		$sqlpurchasespay = $sqlpurchasespays->get_result();
																		while($infopurchasespay=$sqlpurchasespay->fetch_array()){
																			$paidamount+=(float)$infopurchasespay['amount'];
								      								...
								      								$sqlsantss->bind_param("sssss", $_SESSION['franchisesession'], $companymainid, $rows[0]['billno'], $rows[0]['billdate'], $rows[0]['vendorid']);
								      								$sqlsantss->execute();
																	...
																	<td>
																		<div class="col-lg-12">
																			<select class="select4 form-control  form-control-sm finalsubmitrequired" name="refundoption" maxlength="100" id="refundoption" required onchange="checktherefund()">
																				...
																				?>
																				<option value="norefund" selecteds>
																					No Refund
																				</option>
																				<?php
																					}
																		  ...
																		  <div class="col-lg-12">
																				...
																				<?php
																					 ...
																					 while($info=$sqli->fetch_array()){
																						  if (($info['term']=='CASH')) {
																				?>
																					 <option value="<?= $info['term'] ?>" <?=($info['term']==$rows[0]['debitnoteterm'])?'selected':''?>>
																						  <?= $info['term'] ?>
																					 </option>
																				<?php
																						  }
																...
																<tr id="validpaidblock">
																	 ...
																	 <td>
																		  <input type="number" name="validpaidamount" id="validpaidamount" class="form-control form-control-sm finalsubmitrequired" placeholder="0.00" required onchange="paidamountcalc()" value="<?= $rows[0]['validpaidamount'] ?>" step="any" <?=(($rows[0]['billno']!='')?'':'readonly')?> <?=(($rows[0]['refundoption']=='norefund')||(($paidamount - $paidamountdr)<=0))?'readonly':''?>>
																		  <script>
...
$.ajax({
type: "GET",
url: 'editfetch.php?term=' + sIndexEdit + '&limitings=5&debitnoteno=<?=$_GET['debitnoteno']?>&debitnotedate=<?=$_GET['debitnotedate']?>&type=debitnote&i='+ivalue+'',
success: function (result) {
...
<?php
		  ...
		  header("Location:debitnotes.php?error=No Information Found");
	 }
}
else{
	 header("Location:debitnotes.php?error=No Information Found");  
