...
if((isset($_GET['creditnoteno']))&&(isset($_GET['creditnotedate']))){
	$creditnoteno=htmlspecialchars( $_GET['creditnoteno'], ENT_QUOTES, 'UTF-8');
	$creditnotedate=htmlspecialchars( $_GET['creditnotedate'], ENT_QUOTES, 'UTF-8');
	$sql=$con->prepare("SELECT * FROM paircreditnotes WHERE franchisesession=? AND createdid=? AND creditnoteno=? AND creditnotedate=? ORDER BY id ASC");
	$sql->bind_param("ssss", $_SESSION['franchisesession'], $companymainid, $creditnoteno, $creditnotedate);
	$sql->execute();
	$sql->store_result();
	if($sql->num_rows>0){
		$sql->close();
		$sqlismainaccessfields=$con->prepare("SELECT * FROM pairmainaccess WHERE userid=? AND moduletype='Credit Notes' ORDER BY id ASC");
		...
		$sqlismainaccessusers=$con->prepare("SELECT * FROM pairmainaccess WHERE userid=? AND moduletype='Credit Notes' ORDER BY id ASC");
		...
		if(isset($_POST['grandtotal'])){
			...
			if (($infomainaccessuser['creditnotecustinfo']=='defone')||($infomainaccessuser['creditnotecustinfo']=='deftwo')) {
			...
			$dlno20 = '';
			$dlno21 = '';
			if (($infomainaccessuser['creditnotecustinfo']=='one')||($scriptonetwo=='one')) {
			...
			if($access['defcreditnotepay']=='split'){
				$creditnoteterm = 'CASH,CARD,GPAY';
			}
			else{
				$creditnoteterm = htmlspecialchars($_POST['creditnoteterm'], ENT_QUOTES, 'UTF-8');
			}
			$refundoption = htmlspecialchars($_POST['refundoption'], ENT_QUOTES, 'UTF-8');
			$invoiceamount = htmlspecialchars($_POST['invoiceamount'], ENT_QUOTES, 'UTF-8');
			$invoicepaymentreceived = htmlspecialchars($_POST['invoicepaymentreceived'], ENT_QUOTES, 'UTF-8');
			$cancelstatuscheck = htmlspecialchars($_POST['cancelstatuscheck'], ENT_QUOTES, 'UTF-8');
			$oldcreditnoteno=htmlspecialchars( $_GET['creditnoteno'], ENT_QUOTES, 'UTF-8');
			$oldcreditnotedate=htmlspecialchars( $_GET['creditnotedate'], ENT_QUOTES, 'UTF-8');
			$creditnoteno=htmlspecialchars( $_POST['creditnoteno'], ENT_QUOTES, 'UTF-8');
			$creditnotedate=htmlspecialchars( $_POST['creditnotedate'], ENT_QUOTES, 'UTF-8');
			$paymentidforupdpayhis = '';
			$publicsqlforupdhis=$con->prepare("SELECT id FROM paircreditnotepayments WHERE receiptno=? AND receiptdate=?");
			$publicsqlforupdhis->bind_param("ss", $creditnoteno, $creditnotedate);
			...
			$creditnotetime=htmlspecialchars( $_POST['creditnotetime'], ENT_QUOTES, 'UTF-8');
			$invoiceno=htmlspecialchars($_POST['invoiceno'], ENT_QUOTES, 'UTF-8');
			...
			//FOR GET THE CURRENT CREDITNOTE NO
			...
			$creditnoteamount=htmlspecialchars( $_POST['grandtotal'], ENT_QUOTES, 'UTF-8');
			...
			if (($infomainaccessuser['creditnotecustinfo']=='one')||($scriptonetwo=='one')) {
				...
				$custsqlcushiss=$con->prepare("SELECT * FROM paircustomers WHERE (franchisesession=? OR cvisiblity='PUBLIC') AND (createdid=? AND moduletype='Customers') AND id=? ORDER BY id ASC");
				$custsqlcushiss->bind_param("sss", $_SESSION["franchisesession"], $companymainid, $custid);
				$custsqlcushiss->execute();
				$custsqlcushis = $custsqlcushiss->get_result();
				$custsqlanscus=$custsqlcushis->fetch_array();
				...
				$custworkphone = htmlspecialchars( $_POST['workworkphone'], ENT_QUOTES, 'UTF-8');
				$custmobilephone = htmlspecialchars( $_POST['workmobile'], ENT_QUOTES, 'UTF-8');
				...
				$custsqlup = $con->prepare("UPDATE paircustomers SET dlno20=?, dlno21=?, createdon=?, createdid=?, createdby=?, franchisesession=?,billstreet=?,billcity=?,billstate=?,billpincode=?,billcountry=?,shipstreet=?,shipcity=?,shipstate=?,shippincode=?,shipcountry=?,gstrtype=?,gstin=?,workphone=?,mobile=? WHERE id=? AND createdid=? AND moduletype='Customers' AND (franchisesession=? OR cvisiblity='PUBLIC')");
				$custsqlup->bind_param("ssssssssssssssssssssiss", $dlno20, $dlno21, $times, $companymainid, $_SESSION["unqwerty"], $_SESSION["franchisesession"], $area, $city, $state, $pincode, $district, $sarea, $scity, $sstate, $spincode, $sdistrict, $gstrtype, $custgstin, $custworkphone, $custmobilephone, $custid, $companymainid, $_SESSION["franchisesession"]);
				$custsqlup->execute();
				$custsqlup->close();
				$custch='';
			...
			$twoage = '';
			$twosex = '';
			if (($infomainaccessuser['creditnotecustinfo']=='two')||($scriptonetwo=='two')) {
				$address1 = '';
				$address2 = '';
				$saddress1 = '';
				$saddress2 = '';
				...
				$customerid = htmlspecialchars( $_POST['oldcustomername'], ENT_QUOTES, 'UTF-8');
				$sqlup = $con->prepare("UPDATE paircustomers SET dlno20=?, dlno21=?, createdon=?,createdid=?,createdby=?,franchisesession=?,customername=?,billstreet=?,billcity=?,billstate=?,billpincode=?,billcountry=?,shipstreet=?,shipcity=?,shipstate=?,shippincode=?,shipcountry=?,sameasbilling=?,gstrtype=?,gstin=?,moduletype='Customers',placeos=?,workphone=?,mobile=?,customercode=?,publicid=?,privateid=?,cvisiblity=?,age=?,sex=? WHERE id=?");
				$sqlup->bind_param("sssssssssssssssssisssssssssssi", $dlno20, $dlno21, $times, $companymainid, $_SESSION["unqwerty"], $_SESSION["franchisesession"], $customername, $area, $city, $state, $pincode, $district, $sarea, $scity, $sstate, $spincode, $sdistrict, $twosameasbilling, $gstrtype, $gstno, $pos, $twoworkphone, $twomobilephone, $custcustomercode, $custpubliccode, $custprivatecode, $infomainaccesspublicnamecust['customervisible'], $twoage, $twosex,$customerid);
				$sqlup->execute();
				$sqlup->close();
			//FOR B2C CUSTOMER CHANGES
			}
			$sqlismainaccessusers=$con->prepare("SELECT * FROM pairmainaccess WHERE userid=? AND moduletype='Credit Notes' ORDER BY id ASC");
			$sqlismainaccessusers->bind_param("i", $userid);
			$sqlismainaccessusers->execute();
			...
			for($i=0; $i<count($_POST['oldproductname']); $i++){
				...
				$oldmarginupdates=htmlspecialchars( $_POST['oldmarginupdates'][$i], ENT_QUOTES, 'UTF-8');
				$oldmargintotalvalue=htmlspecialchars( $_POST['oldmargintotalvalue'][$i], ENT_QUOTES, 'UTF-8');
				if($productname!=''){
					$sql = $con->prepare("DELETE FROM paircreditnotes WHERE createdid=? AND franchisesession=? AND creditnoteno=? AND creditnotedate=?");
					$sql->bind_param("ssss", $companymainid, $_SESSION["franchisesession"], $oldcreditnoteno, $oldcreditnotedate);
					$sql->execute();
					$sqldelledger = $con->prepare("DELETE FROM pairledgers WHERE createdid=? AND franchisesession=? AND ledgerno=? AND ledgerdate=? AND (type='creditnote' OR type='creditnote payment')");
					$sqldelledger->bind_param("ssss", $companymainid, $_SESSION["franchisesession"], $oldcreditnoteno, $oldcreditnotedate);
					$sqldelledger->execute();
					$sqls = $con->prepare("DELETE FROM paircreditnotepayments WHERE receiptno=? AND receiptdate=?");
					$sqls->bind_param("ss", $oldcreditnoteno, $oldcreditnotedate);
					$sqls->execute();
					$sqls->close();
					$sqlsant = $con->prepare("DELETE FROM paircreditnotepayhistory WHERE creditnoteno=? AND creditnotedate=? AND type='creditnote'");
					$sqlsant->bind_param("ss", $oldcreditnoteno, $oldcreditnotedate);
					$sqlsant->execute();
					$sqlsant->close();
					//  margins     
					$runtheans = explode(';', $oldmarginupdates);
					$emptyqty = 0;
					$margintotal = 0;
					$showtheempty=true;
					for($ans=0;$ans<count($runtheans);$ans++){
						if (strpos($runtheans[$ans], '|||')!==false) {
							...
							$delselbillmargins=$con->prepare("SELECT id FROM pairmargins WHERE billerid=? AND productid=? AND createdid=? AND franchisesession=? AND type='selling' AND billingno=? AND billingdate=? AND nowstatus='added' GROUP BY billingdate, billingno ORDER BY billingdate ASC, billingno ASC");
							$delselbillmargins->bind_param("iiiiss", $runtheansnxt[0], $productid, $companymainid, $_SESSION["franchisesession"], $runtheansnxt[2], $runtheansnxt[3]);
							$delselbillmargins->execute();
							$delselbillmargins->store_result();
							if ($delselbillmargins->num_rows>0) {
								$delbillmargins = $con->prepare("UPDATE pairmargins SET nowstatus='deleted' WHERE billerid=? AND productid=? AND createdid=? AND franchisesession=? AND type='selling' AND billingno=? AND billingdate=? AND nowstatus='added'");
								$delbillmargins->bind_param("iiiiss", $runtheansnxt[0], $productid, $companymainid, $_SESSION["franchisesession"], $runtheansnxt[2], $runtheansnxt[3]);
								$delbillmargins->execute();
								$delbillmargins->close();
							}
							$delselbillmargins->close();
						}
					}
					//  margins     
					if($sql){
						...
						$sql4->bind_param("di", $quantity, $productid);
						$sql4->execute();
						if($sql4){
							$sql4->close();
							...
							if($sqlibatch->num_rows>0){
								...
								$sqlbatchins->bind_param("ssssssssssssssisss", $times, $companymainid, $_SESSION["unqwerty"], $_SESSION["franchisesession"], $productid, $productname, $manufacturer, $producthsn, $productnotes, $productdescription, $batch, $expdate, $mrp, $vat, $quantity, $productrate, $noofpacks, $prodiscount);
								$sqlbatchins->execute();
								$sqlbatchins->close();
							}
						}
			...
			$sql2=$con->prepare("SELECT id FROM paircreditnotes WHERE franchisesession=? AND createdid=? AND creditnoteno=? AND creditnotedate=?");
			$sql2->bind_param("iiss", $_SESSION['franchisesession'], $companymainid, $oldcreditnoteno, $oldcreditnotedate);
			$sql2->execute();
			$sql2->store_result();
			//FOR CHECK THE CREDITNOTE IS IN OR NOT
			if($sql2->num_rows==0){
				for($i=0; $i<count($_POST['productname']); $i++){
					...
					$margintotalvalue=htmlspecialchars( $_POST['margintotalvalue'][$i], ENT_QUOTES, 'UTF-8');
								...
								if($accountname!=''){
				    				...
				    				if($sqlaccins->execute()){
				    					if ($prodisvalueforledger>0) {
						    				...
						    				$sqlaccdefault->bind_param("sisisssssssssss", $times, $companymainid, $_SESSION["unqwerty"], $_SESSION["franchisesession"], $creditnotedate, $creditnoteno, $defaccountnamediscount, $defaccountiddiscount, $customerid, $customername, $prodisvalueforledger, $prodisvalueforledger, $grandtotal, $publicidmanual, $privateidmanual);
				    				...
				    				if ($taxvalue>0) {
										...
										$sqlaccdefault = $con->prepare("INSERT INTO pairledgers (createdon,createdid,createdby,franchisesession,ledgerdate,ledgerno,chartaccountname,chartaccountid,customerid,customername,ledgerdebit,ledgercredit,subledgerdebit,subledgercredit,totalledgerdebit,totalledgercredit,balanceledgerdebit,balanceledgercredit,publicid,privateid,type) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, '0', ?, '0', ?, '0', ?, '0', '0', ?, ?, 'creditnote')");
					    				$sqlaccdefault->bind_param("sisisssssssssss", $times, $companymainid, $_SESSION["unqwerty"], $_SESSION["franchisesession"], $creditnotedate, $creditnoteno, $defaccountnametds, $defaccountidtds, $customerid, $customername, $taxvalue, $taxvalue, $grandtotal, $publicidmanual, $privateidmanual);
					    				$sqlaccdefault->execute();
					    				$sqlaccdefault->close();
					    			}
									...
									$sqlaccdefault = $con->prepare("INSERT INTO pairledgers (createdon,createdid,createdby,franchisesession,ledgerdate,ledgerno,chartaccountname,chartaccountid,customerid,customername,ledgerdebit,ledgercredit,subledgerdebit,subledgercredit,totalledgerdebit,totalledgercredit,balanceledgerdebit,balanceledgercredit,publicid,privateid,type) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, '0', ?, '0', ?, '0', ?, '0', '0', ?, ?, 'creditnote')");
				    				$sqlaccdefault->bind_param("sisisssssssssss", $times, $companymainid, $_SESSION["unqwerty"], $_SESSION["franchisesession"], $creditnotedate, $creditnoteno, $defaccountname, $defaccountid, $customerid, $customername, $productvalue, $productvalue, $grandtotal, $publicidmanual, $privateidmanual);
				    				$sqlaccdefault->execute();
				    				$sqlaccdefault->close();
									...
									$sqlaccinspay = $con->prepare("INSERT INTO pairledgers (createdon,createdid,createdby,franchisesession,ledgerdate,ledgerno,chartaccountname,chartaccountid,customerid,customername,ledgerdebit,ledgercredit,subledgerdebit,subledgercredit,totalledgerdebit,totalledgercredit,balanceledgerdebit,balanceledgercredit,publicid,privateid,type) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, '0', ?, '0', ?, '0', '0', '0', ?, ?, 'creditnote payment')");
				    				$sqlaccinspay->bind_param("sisisssssssssss", $times, $companymainid, $_SESSION["unqwerty"], $_SESSION["franchisesession"], $creditnotedate, $creditnoteno, $defaccountnamepettycashpay, $defaccountidpettycashpay, $customerid, $customername, $productnetvalue, $productnetvalue, $grandtotal, $publicidmanual, $privateidmanual);
				    				...
				    				$sqlaccdefaultpay->bind_param("sisisssssssssss", $times, $companymainid, $_SESSION["unqwerty"], $_SESSION["franchisesession"], $creditnotedate, $creditnoteno, $defaccountnamepay, $defaccountidpay, $customerid, $customername, $productnetvalue, $productnetvalue, $grandtotal, $publicidmanual, $privateidmanual);
				    				$sqlaccdefaultpay->execute();
				    				$sqlaccdefaultpay->close();
								}
							//FOR INSERT THE MANUAL JOURNAL
							}
							elseif($creditnoteterm=='CASH,CARD,GPAY'){
								$sqlismodulespublicnamemanuals=$con->prepare("SELECT * FROM pairmodules WHERE moduletype='Manual Journals' ORDER BY id ASC");
								$sqlismodulespublicnamemanuals->execute();
								$sqlismodulespublicnamemanual = $sqlismodulespublicnamemanuals->get_result();
								$infomodulespublicnamemanual=$sqlismodulespublicnamemanual->fetch_array();
								...
								if($accountname!=''){
				    				...
				    				if ($cashreceived>0) {
										...
										$sqlaccinspay = $con->prepare("INSERT INTO pairledgers (createdon,createdid,createdby,franchisesession,ledgerdate,ledgerno,chartaccountname,chartaccountid,customerid,customername,ledgerdebit,ledgercredit,subledgerdebit,subledgercredit,totalledgerdebit,totalledgercredit,balanceledgerdebit,balanceledgercredit,publicid,privateid,type) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, '0', ?, '0', ?, '0', '0', '0', ?, ?, 'creditnote payment')");
					    				$sqlaccinspay->bind_param("sisisssssssssss", $times, $companymainid, $_SESSION["unqwerty"], $_SESSION["franchisesession"], $creditnotedate, $creditnoteno, $defaccountnamepettycashpay, $defaccountidpettycashpay, $customerid, $customername, $cashreceived, $cashreceived, $grandtotal, $publicidmanual, $privateidmanual);
					    				$sqlaccinspay->execute();
					    				$sqlaccinspay->close();
										$sqlaccdefaultcashpays=$con->prepare("SELECT accountname,id FROM pairchartaccountings WHERE accountname='Accounts Receivable'");
										$sqlaccdefaultcashpays->execute();
										...
										$sqlaccdefaultpay = $con->prepare("INSERT INTO pairledgers (createdon,createdid,createdby,franchisesession,ledgerdate,ledgerno,chartaccountname,chartaccountid,customerid,customername,ledgerdebit,ledgercredit,subledgerdebit,subledgercredit,totalledgerdebit,totalledgercredit,balanceledgerdebit,balanceledgercredit,publicid,privateid,type) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, '0', ?, '0', ?, '0', ?, '0', '0', ?, ?, 'creditnote payment')");
					    				$sqlaccdefaultpay->bind_param("sisisssssssssss", $times, $companymainid, $_SESSION["unqwerty"], $_SESSION["franchisesession"], $creditnotedate, $creditnoteno, $defaccountnamepay, $defaccountidpay, $customerid, $customername, $cashreceived, $cashreceived, $grandtotal, $publicidmanual, $privateidmanual);
					    				$sqlaccdefaultpay->execute();
					    				$sqlaccdefaultpay->close();
				    				}
				    				if ($cardreceived>0) {
										...
										$sqlaccinspay = $con->prepare("INSERT INTO pairledgers (createdon,createdid,createdby,franchisesession,ledgerdate,ledgerno,chartaccountname,chartaccountid,customerid,customername,ledgerdebit,ledgercredit,subledgerdebit,subledgercredit,totalledgerdebit,totalledgercredit,balanceledgerdebit,balanceledgercredit,publicid,privateid,type) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, '0', ?, '0', ?, '0', '0', '0', ?, ?, 'creditnote payment')");
					    				$sqlaccinspay->bind_param("sisisssssssssss", $times, $companymainid, $_SESSION["unqwerty"], $_SESSION["franchisesession"], $creditnotedate, $creditnoteno, $defaccountnamepettycashpay, $defaccountidpettycashpay, $customerid, $customername, $cardreceived, $cardreceived, $grandtotal, $publicidmanual, $privateidmanual);
					    				$sqlaccinspay->execute();
					    				$sqlaccinspay->close();
										$sqlaccdefaultcashpays=$con->prepare("SELECT accountname,id FROM pairchartaccountings WHERE accountname='Accounts Receivable'");
										$sqlaccdefaultcashpays->execute();
										...
										$sqlaccdefaultpay = $con->prepare("INSERT INTO pairledgers (createdon,createdid,createdby,franchisesession,ledgerdate,ledgerno,chartaccountname,chartaccountid,customerid,customername,ledgerdebit,ledgercredit,subledgerdebit,subledgercredit,totalledgerdebit,totalledgercredit,balanceledgerdebit,balanceledgercredit,publicid,privateid,type) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, '0', ?, '0', ?, '0', ?, '0', '0', ?, ?, 'creditnote payment')");
					    				$sqlaccdefaultpay->bind_param("sisisssssssssss", $times, $companymainid, $_SESSION["unqwerty"], $_SESSION["franchisesession"], $creditnotedate, $creditnoteno, $defaccountnamepay, $defaccountidpay, $customerid, $customername, $cardreceived, $cardreceived, $grandtotal, $publicidmanual, $privateidmanual);
					    				$sqlaccdefaultpay->execute();
					    				$sqlaccdefaultpay->close();
				    				}
				    				if ($gpayreceived>0) {
										...
										$sqlaccinspay = $con->prepare("INSERT INTO pairledgers (createdon,createdid,createdby,franchisesession,ledgerdate,ledgerno,chartaccountname,chartaccountid,customerid,customername,ledgerdebit,ledgercredit,subledgerdebit,subledgercredit,totalledgerdebit,totalledgercredit,balanceledgerdebit,balanceledgercredit,publicid,privateid,type) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, '0', ?, '0', ?, '0', '0', '0', ?, ?, 'creditnote payment')");
					    				$sqlaccinspay->bind_param("sisisssssssssss", $times, $companymainid, $_SESSION["unqwerty"], $_SESSION["franchisesession"], $creditnotedate, $creditnoteno, $defaccountnamepettycashpay, $defaccountidpettycashpay, $customerid, $customername, $gpayreceived, $gpayreceived, $grandtotal, $publicidmanual, $privateidmanual);
					    				$sqlaccinspay->execute();
					    				$sqlaccinspay->close();
										$sqlaccdefaultcashpays=$con->prepare("SELECT accountname,id FROM pairchartaccountings WHERE accountname='Accounts Receivable'");
										$sqlaccdefaultcashpays->execute();
										...
										$sqlaccdefaultpay = $con->prepare("INSERT INTO pairledgers (createdon,createdid,createdby,franchisesession,ledgerdate,ledgerno,chartaccountname,chartaccountid,customerid,customername,ledgerdebit,ledgercredit,subledgerdebit,subledgercredit,totalledgerdebit,totalledgercredit,balanceledgerdebit,balanceledgercredit,publicid,privateid,type) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, '0', ?, '0', ?, '0', ?, '0', '0', ?, ?, 'creditnote payment')");
					    				$sqlaccdefaultpay->bind_param("sisisssssssssss", $times, $companymainid, $_SESSION["unqwerty"], $_SESSION["franchisesession"], $creditnotedate, $creditnoteno, $defaccountnamepay, $defaccountidpay, $customerid, $customername, $gpayreceived, $gpayreceived, $grandtotal, $publicidmanual, $privateidmanual);
					    				$sqlaccdefaultpay->execute();
					    				$sqlaccdefaultpay->close();
				    				}
								}
				...
				if(($creditnoteterm=='CASH')){
					if ($validpaidamount!=0) {
						$sqlismodulespublicnames=$con->prepare("SELECT * FROM pairmodules WHERE moduletype='Customer Refunds' ORDER BY id ASC");
						...
						$sqlismainaccesspublicnames=$con->prepare("SELECT * FROM pairmainaccess WHERE createdid=? AND moduletype='Customer Refunds' AND franchiseid=? ORDER BY id ASC");
						...
						$publicsqls=$con->prepare("SELECT count(publicid) FROM paircreditnotepayments WHERE createdid=?");
						...
						$privatesqls=$con->prepare("SELECT count(privateid) FROM paircreditnotepayments WHERE createdid=? AND franchisesession=?");
						...
						$sqlselects=$con->prepare("SELECT id FROM paircreditnotepayments WHERE receiptno=? AND receiptdate=?");
						$sqlselects->bind_param("ss", $creditnoteno, $creditnotedate);
						$sqlselects->execute();
						...
						if ($sqlselects->num_rows==0) {
							...
							$sqlsalepayins = $con->prepare("INSERT INTO paircreditnotepayments (createdid,createdby,franchisesession,createdon,term,type,customername,customerid,receiptno,receiptdate,amount,paymentmode,notes,publicid,privateid) VALUES (?, ?, ?, ?, 'RECEIPT', 'creditnote', ?, ?, ?, ?, ?, ?, '-', ?, ?)");
							$sqlsalepayins->bind_param("ssssssssssss", $companymainid, $_SESSION["unqwerty"], $_SESSION["franchisesession"], $createdon, $customername, $customerid, $creditnoteno, $creditnotedate, $validpaidamount, $creditnoteterm, $publiccode, $privatecode);
							$sqlsalepayins->execute();
							if($sqlsalepayins){
								$sqlsalepayins->close();
								$paymentid=$con->insert_id;
								$sqle = $con->prepare("INSERT INTO paircreditnotepayhistory (createdid,createdby,franchisesession,createdon,paymentid,customerid,creditnoteno,creditnotedate,amount,type) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'creditnote')");
								$sqle->bind_param("sssssssss", $companymainid, $_SESSION["unqwerty"], $_SESSION["franchisesession"], $createdon, $paymentid, $customerid, $creditnoteno, $creditnotedate, $validpaidamount);
								$sqle->execute();
								$sqle->close();
								$oldescreditnoteno = htmlspecialchars( $_POST['oldescreditnoteno'], ENT_QUOTES, 'UTF-8');
								$oldescreditnotedate = htmlspecialchars( $_POST['oldescreditnotedate'], ENT_QUOTES, 'UTF-8');
								$oldescustomername = htmlspecialchars( $_POST['oldescustomername'], ENT_QUOTES, 'UTF-8');
								$oldescreditnoteterm = htmlspecialchars( $_POST['oldescreditnoteterm'], ENT_QUOTES, 'UTF-8');
								...
								if($customername!=$oldescustomername){
									if($chforpayments!=''){
										$chforpayments.='<br> '.$infomainaccessusercus['modulename'].' Name <span style="color:green;" id="prohisfromtospan">( From '.$oldcustomername.' To '.$customername.' ) </span>';
									}
									else{
										$chforpayments.=''.$infomainaccessusercus['modulename'].' Name <span style="color:green;" id="prohisfromtospan">( From '.$oldcustomername.' To '.$customername.' ) </span>';
									}					
								}
								if($creditnotedate!=$oldescreditnotedate){
									if($chforpayments!=''){
										$chforpayments.='<br> Date <span style="color:green;" id="prohisfromtospan">( From '.date($datemainphp,strtotime($creditnotedate)).' To '.date($datemainphp,strtotime($oldcreditnotedate)).' ) </span>';
									}
									else{
										$chforpayments.=' Date <span style="color:green;" id="prohisfromtospan">( From '.date($datemainphp,strtotime($creditnotedate)).' To '.date($datemainphp,strtotime($oldcreditnotedate)).' ) </span>';
									}					
								}
								if($creditnoteno!=$oldescreditnoteno){
									if($chforpayments!=''){
										$chforpayments.='<br> Reference Number <span style="color:green;" id="prohisfromtospan">( From '.$oldcreditnoteno.' To '.$creditnoteno.' ) </span>';
									}
									else{
										$chforpayments.='Reference Number <span style="color:green;" id="prohisfromtospan">( From '.$oldcreditnoteno.' To '.$creditnoteno.' ) </span>';
									}					
								}
								if($creditnoteterm!=$oldescreditnoteterm){
									if($chforpayments!=''){
										$chforpayments.='<br> Mode of Payment <span style="color:green;" id="prohisfromtospan">( From '.$oldescreditnoteterm.' To '.$creditnoteterm.' ) </span>';
									}
									else{
										$chforpayments.='Mode of Payment <span style="color:green;" id="prohisfromtospan">( From '.$oldescreditnoteterm.' To '.$creditnoteterm.' ) </span>';
									}					
								}
								...
								if($validpaidamount!=$oldesvalidpaidamount){
									if($creditnotedate!=''){
										if($chforpayments!=''){
											$chforpayments.='<br> <span style="color:royalblue;">'.$infomainaccessuser['modulename'].' Information</span> <br> (Selected) <br> '.$infomainaccessuser['modulename'].' Date <span style="color:green;" id="prohisfromtospan">( '.date($datemainphp,strtotime($creditnotedate)).' ) </span>';
										}
										else{
											$chforpayments.='<span style="color:royalblue;">'.$infomainaccessuser['modulename'].' Information</span> <br> (Selected) <br> '.$infomainaccessuser['modulename'].' Date <span style="color:green;" id="prohisfromtospan">( '.date($datemainphp,strtotime($creditnotedate)).' ) </span>';
										}
									}
									if($creditnoteno!=''){
										if($chforpayments!=''){
											$chforpayments.='<br> '.$infomainaccessuser['modulename'].' Number <span style="color:green;" id="prohisfromtospan">( '.$creditnoteno.' ) </span>';
										}
										else{
											$chforpayments.=''.$infomainaccessuser['modulename'].' Number <span style="color:green;" id="prohisfromtospan">( '.$creditnoteno.' ) </span>';
										}
									}
									if($grandtotal!=''){
										if($chforpayments!=''){
								...
								if($chforpayments!=''){
									$sqlusepayments = $con->prepare("INSERT INTO pairusehistory (usetype,createdon,createdby,useid,useremarks) VALUES ('CUSREFUND', ?, ?, ?, ?)");
							...
							$sqlbatchupd = $con->prepare("UPDATE paircreditnotepayments SET createdid=?, createdby=?, franchisesession=?, createdon=?,  term='RECEIPT', type='creditnote', customername=?, customerid=?, receiptno=?, receiptdate=?, amount=?, paymentmode=?, notes='-' WHERE receiptno=? AND receiptdate=?");
							$sqlbatchupd->bind_param("ssssssssssss", $companymainid, $_SESSION["unqwerty"], $_SESSION["franchisesession"], $createdon, $customername, $customerid, $creditnoteno, $creditnotedate, $validpaidamount, $creditnoteterm, $creditnoteno, $creditnotedate);
							$sqlbatchupd->execute();
							if($sqlbatchupd){
								$sqlbatchupd->close();
								$sqle = $con->prepare("UPDATE paircreditnotepayhistory SET amount=? WHERE creditnoteno=? AND creditnotedate=? AND type='creditnote'");
								$sqle->bind_param("sss", $validpaidamount, $creditnoteno, $creditnotedate);
								$sqle->execute();
								$sqle->close();
							}
						...
						$sqlbatchupdant = $con->prepare("UPDATE paircreditnotepayments SET createdid=?, createdby=?, franchisesession=?, createdon=?,  term='RECEIPT', type='creditnote', customername=?, customerid=?, receiptno=?, receiptdate=?, amount=?, paymentmode=?, notes='-' WHERE receiptno=? AND receiptdate=?");
						$sqlbatchupdant->bind_param("ssssssssssss", $companymainid, $_SESSION["unqwerty"], $_SESSION["franchisesession"], $createdon, $customername, $customerid, $creditnoteno, $creditnotedate, $validpaidamount, $creditnoteterm, $creditnoteno, $creditnotedate);
						$sqlbatchupdant->execute();
						if($sqlbatchupdant){
							$sqlbatchupdant->close();
							$sqleant = $con->prepare("UPDATE paircreditnotepayhistory SET amount=? WHERE creditnoteno=? AND creditnotedate=? AND type='creditnote'");
							$sqleant->bind_param("sss", $validpaidamount, $creditnoteno, $creditnotedate);
							$sqleant->execute();
							$sqleant->close();
						}
					...
					if($validpaidamount==$grandtotal){
						$sqler = $con->prepare("UPDATE paircreditnotes SET createdid=?, createdby=?, franchisesession=?, createdon=?, paidstatus='1' WHERE creditnoteno=? AND creditnotedate=? AND customerid=? AND franchisesession=? AND createdid=?");
	    				$sqler->bind_param("sssssssss", $companymainid, $_SESSION["unqwerty"], $_SESSION["franchisesession"], $createdon, $creditnoteno, $creditnotedate, $customerid, $_SESSION['franchisesession'], $companymainid);
	    				$sqler->execute();
						$sqler->close();
						//FOR UPDATE THE PAYMENT STATUS PAID
					}
					else{
						$sqler = $con->prepare("UPDATE paircreditnotes SET createdid=?, createdby=?, franchisesession=?, createdon=?, paidstatus='2' WHERE creditnoteno=? AND creditnotedate=? AND customerid=? AND franchisesession=? AND createdid=?");
	    				$sqler->bind_param("sssssssss", $companymainid, $_SESSION["unqwerty"], $_SESSION["franchisesession"], $createdon, $creditnoteno, $creditnotedate, $customerid, $_SESSION['franchisesession'], $companymainid);
	    				$sqler->execute();
						$sqler->close();
				...
				if(($creditnoteterm=='CASH,CARD,GPAY')){
					$creditnotearr = [$cashreceived,$cardreceived,$gpayreceived];
					for($termrun = 0;$termrun<count(explode(',', $creditnoteterm));$termrun++){
						if ($creditnotearr[$termrun]>0) {
							$sqlismodulespublicnames=$con->prepare("SELECT * FROM pairmodules WHERE moduletype='Customer Refunds' ORDER BY id ASC");
							$sqlismodulespublicnames->execute();
							$sqlismodulespublicname = $sqlismodulespublicnames->get_result();
							$infomodulespublicname=$sqlismodulespublicname->fetch_array();
							...
							if ($sqlselects->num_rows==0) {
								...
								$sqlselect=$sqlselectsql->fetch_array();
								$sqlsalepayins = $con->prepare("INSERT INTO paircreditnotepayments (createdid,createdby,franchisesession,createdon,term,type,customername,customerid,receiptno,receiptdate,amount,paymentmode,notes,publicid,privateid) VALUES (?, ?, ?, ?, 'RECEIPT', 'creditnote', ?, ?, ?, ?, ?, ?, '-', ?, ?)");
								$sqlsalepayins->bind_param("ssssssssssss", $companymainid, $_SESSION["unqwerty"], $_SESSION["franchisesession"], $createdon, $customername, $customerid, $creditnoteno, $creditnotedate, $creditnotearr[$termrun], explode(',', $creditnoteterm)[$termrun], $publiccode, $privatecode);
								$sqlsalepayins->execute();
								if($sqlsalepayins){
									$sqlsalepayins->close();
									$paymentid=$con->insert_id;
									$sqle = $con->prepare("INSERT INTO paircreditnotepayhistory (createdid,createdby,franchisesession,createdon,paymentid,customerid,creditnoteno,creditnotedate,amount,type) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'creditnote')");
									$sqle->bind_param("sssssssss", $companymainid, $_SESSION["unqwerty"], $_SESSION["franchisesession"], $createdon, $paymentid, $customerid, $creditnoteno, $creditnotedate, $creditnotearr[$termrun]);
									$sqle->execute();
									$sqle->close();
									$oldescreditnoteno = htmlspecialchars( $_POST['oldescreditnoteno'], ENT_QUOTES, 'UTF-8');
									$oldescreditnotedate = htmlspecialchars( $_POST['oldescreditnotedate'], ENT_QUOTES, 'UTF-8');
									...
									if(explode(',', $creditnoteterm)[$termrun]!=$oldescreditnoteterm){
										if($chforpayments!=''){
											$chforpayments.='<br> Mode of Payment <span style="color:green;" id="prohisfromtospan">( From '.$oldescreditnoteterm.' To '.explode(',', $creditnoteterm)[$termrun].' ) </span>';
										}
										else{
											$chforpayments.='Mode of Payment <span style="color:green;" id="prohisfromtospan">( From '.$oldescreditnoteterm.' To '.explode(',', $creditnoteterm)[$termrun].' ) </span>';
										}					
									}
									if($creditnotearr[$termrun]!=$oldesvalidpaidamount){
										if($chforpayments!=''){
											$chforpayments.='<br> Amount Received <span style="color:green;" id="prohisfromtospan">( From '.$oldesvalidpaidamount.' To '.$creditnotearr[$termrun].' ) </span>';
										}
										else{
											$chforpayments.='Amount Received <span style="color:green;" id="prohisfromtospan">( From '.$oldesvalidpaidamount.' To '.$creditnotearr[$termrun].' ) </span>';
										}					
									}
									if($creditnotearr[$termrun]!=$oldesvalidpaidamount){
										if($creditnotedate!=''){
										...
										if($chforpayments!=''){
											$chforpayments.='<br> Payment <span style="color:green;" id="prohisfromtospan">( From '.$oldesvalidpaidamount.' To '.$creditnotearr[$termrun].' ) </span>';
										}
										else{
											$chforpayments.='Payment <span style="color:green;" id="prohisfromtospan">( From '.$oldesvalidpaidamount.' To '.$creditnotearr[$termrun].' ) </span>';
										}					
									}
									if($creditnotearr[$termrun]!=$oldesvalidpaidamount){
										if($chforpayments!=''){
											$chforpayments.='<br> Total <span style="color:green;" id="prohisfromtospan">( From '.$oldesvalidpaidamount.' To '.$creditnotearr[$termrun].' ) </span>';
										}
										else{
											$chforpayments.='Total <span style="color:green;" id="prohisfromtospan">( From '.$oldesvalidpaidamount.' To '.$creditnotearr[$termrun].' ) </span>';
										}					
									}
									if($chforpayments!=''){
										$sqlusepayments = $con->prepare("INSERT INTO pairusehistory (usetype,createdon,createdby,useid,useremarks) VALUES ('CUSREFUND', ?, ?, ?, ?)");
								...
								$sqlbatchupd = $con->prepare("UPDATE paircreditnotepayments SET createdid=?, createdby=?, franchisesession=?, createdon=?,  term='RECEIPT', type='creditnote', customername=?, customerid=?, receiptno=?, receiptdate=?, amount=?, paymentmode=?, notes='-' WHERE receiptno=? AND receiptdate=?");
								$sqlbatchupd->bind_param("ssssssssssss", $companymainid, $_SESSION["unqwerty"], $_SESSION["franchisesession"], $createdon, $customername, $customerid, $creditnoteno, $creditnotedate, $creditnotearr[$termrun], explode(',', $creditnoteterm)[$termrun], $creditnoteno, $creditnotedate);
								$sqlbatchupd->execute();
								if($sqlbatchupd){
									$sqlbatchupd->close();
									$sqle = $con->prepare("UPDATE paircreditnotepayhistory SET amount=? WHERE creditnoteno=? AND creditnotedate=? AND type='creditnote'");
									$sqle->bind_param("sss", $creditnotearr[$termrun], $creditnoteno, $creditnotedate);
									$sqle->execute();
									$sqle->close();
								}
								//FOR UPDATE THE PAYMENT DETAILS
							...
							$publicsqlsdel=$con->prepare("SELECT id FROM paircreditnotepayments WHERE receiptno=? AND receiptdate=? AND paymentmode=?");
							$publicsqlsdel->bind_param("sss", $creditnoteno, $creditnotedate, explode(',', $creditnoteterm)[$termrun]);
							...
							if($publicsqldel->num_rows > 0){
								...
								$publicsqldel->close();
								$publicsqlsdel->close();
								$sqls = $con->prepare("DELETE FROM paircreditnotepayments WHERE receiptno=? AND receiptdate=? AND paymentmode=?");
								$sqls->bind_param("sss", $creditnoteno, $creditnotedate, explode(',', $creditnoteterm)[$termrun]);
								$sqls->execute();
								$sqls->close();
								$sqlsant = $con->prepare("DELETE FROM paircreditnotepayhistory WHERE creditnoteno=? AND creditnotedate=? AND type='creditnote' AND paymentid=?");
								$sqlsant->bind_param("sss", $creditnoteno, $creditnotedate, $paymentidfordelete);
								$sqlsant->execute();
								$sqlsant->close();
								// $sqlbatchupdant = $con->prepare("UPDATE paircreditnotepayments SET createdid=?, createdby=?, franchisesession=?, createdon=?,  term='RECEIPT', type='creditnote', customername=?, customerid=?, receiptno=?, receiptdate=?, amount=?, paymentmode=?, notes='-' WHERE receiptno=? AND receiptdate=?");
								// $sqlbatchupdant->bind_param("ssssssssssss", $companymainid, $_SESSION["unqwerty"], $_SESSION["franchisesession"], $createdon, $customername, $customerid, $creditnoteno, $creditnotedate, $creditnotearr[$termrun], explode(',', $creditnoteterm)[$termrun], $creditnoteno, $creditnotedate);
								// $sqlbatchupdant->execute();
								// if($sqlbatchupdant){
								// 	$sqlbatchupdant->close();
								// 	$sqleant = $con->prepare("UPDATE paircreditnotepayhistory SET amount=? WHERE creditnoteno=? AND creditnotedate=? AND type='creditnote'");
								// 	$sqleant->bind_param("sss", $creditnotearr[$termrun], $creditnoteno, $creditnotedate);
								// 	$sqleant->execute();
								// 	$sqleant->close();
								// }
								//FOR UPDATE THE PAYMENT DETAILS HISTORY
				...
				elseif($creditnoteterm=='CREDIT'){
					$sqls = $con->prepare("DELETE FROM paircreditnotepayments WHERE receiptno=? AND receiptdate=?");
					$sqls->bind_param("ss", $creditnoteno, $creditnotedate);
					$sqls->execute();
					$sqls->close();
					$sqlsant = $con->prepare("DELETE FROM paircreditnotepayhistory WHERE creditnoteno=? AND creditnotedate=? AND type='creditnote'");
					$sqlsant->bind_param("ss", $creditnoteno, $creditnotedate);
					$sqlsant->execute();
					$sqlsant->close();
					$sqler = $con->prepare("UPDATE paircreditnotes SET createdid=?, createdby=?, franchisesession=?, createdon=?, paidstatus='0' WHERE creditnoteno=? AND creditnotedate=? AND customerid=? AND franchisesession=? AND createdid=?");
	    			$sqler->bind_param("sssssssss", $companymainid, $_SESSION["unqwerty"], $_SESSION["franchisesession"], $createdon, $creditnoteno, $creditnotedate, $customerid, $_SESSION['franchisesession'], $companymainid);
	    			$sqler->execute();
					$sqler->close();
					//FOR UPDATE THE PAYMENT STATUS UNPAID
					$sqldelledgerpay = $con->prepare("DELETE FROM pairledgers WHERE createdid=? AND franchisesession=? AND ledgerno=? AND ledgerdate=? AND type='creditnote payment'");
					$sqldelledgerpay->bind_param("ssss", $companymainid, $_SESSION["franchisesession"], $creditnoteno, $creditnotedate);
				...
				if($sql){
					...
					while($infocreditnote=$sqlicreditnote->fetch_array()){
						$creditnoteamount+=(float)$infocreditnote['creditnoteamount'];
						$paidamount=0;
						$sqlsalespays=$con->prepare("SELECT amount FROM paircreditnotepayhistory WHERE franchisesession=? AND createdid=? AND creditnoteno=? AND creditnotedate=? AND customerid=? ORDER BY id DESC");
					...
					while($infoinvoice=$sqliinvoice->fetch_array()){
						$creditnoteamount+=(float)$infoinvoice['invoiceamount'];
						$paidamount=0;
						...
						$balanceamount-=((float)$infoinvoice['invoiceamount']);
						$diff = abs(time() - strtotime($infoinvoice['invoicedate']));
						$days = floor(($diff)/ (60*60*24));
						if($days>30){
							$overdueamount-=((float)$infoinvoice['invoiceamount']);
						}
						else{
							$currentamount-=((float)$infoinvoice['invoiceamount']);
						}
						$sqlsalespay->close();
						$sqlsalespays->close();
					}
					...
					$cussqlup->bind_param("sssss", $creditnoteamount, $balanceamount, $currentamount, $overdueamount, $customerid);
					$cussqlup->execute();
					$cussqlup->close();
					if(isset($_POST['submit1'])){
						echo '<script> window.open("creditnoteprint.php?creditnoteno='.$creditnoteno.'&creditnotedate='.$creditnotedate.'", "_blank");</script>';
						echo '<script> window.location.href="creditnotes.php?remarks=Updated Successfully";</script>'; 
					}
					else{
						$sqlusefinal = $con->prepare("INSERT INTO pairusehistory (usetype,createdon,createdby,useid,uniqueid,useremarks) VALUES ('CREDITNOTES', ?, ?, ?, ?, '<span style=\"color:green;\" id=\"prohisfromtospan\">Updated Successfully</span>')");
			    		$sqlusefinal->bind_param("ssss", $times, $_SESSION["unqwerty"], $creditnoteno, $salesid);
			    		$sqlusefinal->execute();
			    		$sqlusefinal->close();
						echo '<script> window.location.href="creditnoteview.php?id='.$salesid.'&creditnoteno='.$creditnoteno.'&creditnotedate='.$creditnotedate.'&remarks=Updated Successfully";</script>';
					}
				}
				//FOR CUSTOMER BALANCE UPDATION DETAILS
			}
			else{
				header("Location: creditnotes.php?error=Error Data");
			}
			if ($sql) {
				$oldescreditnoteno = htmlspecialchars( $_POST['oldescreditnoteno'], ENT_QUOTES, 'UTF-8');
				$oldescreditnotedate = htmlspecialchars( $_POST['oldescreditnotedate'], ENT_QUOTES, 'UTF-8');
				...
				$oldescustomername = htmlspecialchars( $_POST['oldescustomername'], ENT_QUOTES, 'UTF-8');
				$oldesage = htmlspecialchars( $_POST['oldesage'], ENT_QUOTES, 'UTF-8');
				$oldessex = htmlspecialchars( $_POST['oldessex'], ENT_QUOTES, 'UTF-8');
				...
				$oldescreditnoteterm = htmlspecialchars( $_POST['oldescreditnoteterm'], ENT_QUOTES, 'UTF-8');
				$oldesinvoiceno = htmlspecialchars( $_POST['oldesinvoiceno'], ENT_QUOTES, 'UTF-8');
				$oldesinvoiceamount = htmlspecialchars( $_POST['oldesinvoiceamount'], ENT_QUOTES, 'UTF-8');
				$oldesinvoicepaymentreceived = htmlspecialchars( $_POST['oldesinvoicepaymentreceived'], ENT_QUOTES, 'UTF-8');
				$oldesrefundoption = htmlspecialchars( $_POST['oldesrefundoption'], ENT_QUOTES, 'UTF-8');
				$oldesvalidpaidamount = htmlspecialchars( $_POST['oldesvalidpaidamount'], ENT_QUOTES, 'UTF-8');
				...
				if($creditnoteno!=$oldescreditnoteno){
					if($creditnoteinfoch!=''){
						$creditnoteinfoch.='<br> '.$infomainaccessuser['modulename'].' Number <span style="color:green;" id="prohisfromtospan">( From '.$oldescreditnoteno.' To '.$creditnoteno.' ) </span>';
					}
					else{
						$creditnoteinfoch.='<span style="color:royalblue;">'.$infomainaccessuser['modulename'].' Information</span> <br> '.$infomainaccessuser['modulename'].' Number <span style="color:green;" id="prohisfromtospan">( From '.$oldescreditnoteno.' To '.$creditnoteno.' ) </span>';
					}                   
				}
				if($creditnotedate!=$oldescreditnotedate){
					if($creditnoteinfoch!=''){
						$creditnoteinfoch.='<br> '.$infomainaccessuser['modulename'].' Date <span style="color:green;" id="prohisfromtospan">( From '.date($date,strtotime($oldescreditnotedate)).' To '.date($date,strtotime($creditnotedate)).' ) </span>';
					}
					else{
						$creditnoteinfoch.='<span style="color:royalblue;">'.$infomainaccessuser['modulename'].' Information</span> <br> '.$infomainaccessuser['modulename'].' Date <span style="color:green;" id="prohisfromtospan">( From '.date($date,strtotime($oldescreditnotedate)).' To '.date($date,strtotime($creditnotedate)).' ) </span>';
					}                   
				}
				if($reference!=$oldesreference){
					if($creditnoteinfoch!=''){
						$creditnoteinfoch.='<br> Reference <span style="color:green;" id="prohisfromtospan">( From '.$oldesreference.' To '.$reference.' ) </span>';
					}
					else{
						$creditnoteinfoch.='<span style="color:royalblue;">'.$infomainaccessuser['modulename'].' Information</span> <br> Reference <span style="color:green;" id="prohisfromtospan">( From '.$oldesreference.' To '.$reference.' ) </span>';
					}                   
				}
				if($saleperson!=$oldessaleperson){
					if($creditnoteinfoch!=''){
						$creditnoteinfoch.='<br> Sale Person <span style="color:green;" id="prohisfromtospan">( From '.$oldessaleperson.' To '.$saleperson.' ) </span>';
					}
					else{
						$creditnoteinfoch.='<span style="color:royalblue;">'.$infomainaccessuser['modulename'].' Information</span> <br> Sale Person <span style="color:green;" id="prohisfromtospan">( From '.$oldessaleperson.' To '.$saleperson.' ) </span>';
					}                   
				}
				if($preparedby!=$oldespreparedby){
					if($creditnoteinfoch!=''){
						$creditnoteinfoch.='<br> Prepared By <span style="color:green;" id="prohisfromtospan">( From '.$oldespreparedby.' To '.$preparedby.' ) </span>';
					}
					else{
						$creditnoteinfoch.='<span style="color:royalblue;">'.$infomainaccessuser['modulename'].' Information</span> <br> Prepared By <span style="color:green;" id="prohisfromtospan">( From '.$oldespreparedby.' To '.$preparedby.' ) </span>';
					}                   
				}
				if($checkedby!=$oldescheckedby){
					if($creditnoteinfoch!=''){
						$creditnoteinfoch.='<br> Checked By <span style="color:green;" id="prohisfromtospan">( From '.$oldescheckedby.' To '.$checkedby.' ) </span>';
					}
					else{
						$creditnoteinfoch.='<span style="color:royalblue;">'.$infomainaccessuser['modulename'].' Information</span> <br> Checked By <span style="color:green;" id="prohisfromtospan">( From '.$oldescheckedby.' To '.$checkedby.' ) </span>';
					}                   
				}
				//FOR CREDITNOTE INFORMATIONS HISTORY DETAILS
				$creditnotecustinfoch = '';
				$sqlismainaccessusercuss=$con->prepare("SELECT * FROM pairmainaccess WHERE userid=? AND moduletype='Customers' ORDER BY id ASC");
				$sqlismainaccessusercuss->bind_param("i", $userid);
				$sqlismainaccessusercuss->execute();
				$sqlismainaccessusercus = $sqlismainaccessusercuss->get_result();
				$infomainaccessusercus=$sqlismainaccessusercus->fetch_array();
				if($customername!=$oldescustomername){
					if($creditnotecustinfoch!=''){
						$creditnotecustinfoch.='<br> '.$infomainaccessusercus['modulename'].' Name <span style="color:green;" id="prohisfromtospan">( From '.$oldescustomername.' To '.$customername.' ) </span>';
					}
					else{
						$creditnotecustinfoch.=''.(($creditnoteinfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessusercus['modulename'].' Information</span> <br> '.$infomainaccessusercus['modulename'].' Name <span style="color:green;" id="prohisfromtospan">( From '.$oldescustomername.' To '.$customername.' ) </span>';
					}                   
				}
				if($twoage!=$oldesage){
					if($creditnotecustinfoch!=''){
						$creditnotecustinfoch.='<br> AGE <span style="color:green;" id="prohisfromtospan">( From '.$oldesage.' To '.$twoage.' ) </span>';
					}
					else{
						$creditnotecustinfoch.=''.(($creditnoteinfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessusercus['modulename'].' Information</span> <br> AGE <span style="color:green;" id="prohisfromtospan">( From '.$oldesage.' To '.$twoage.' ) </span>';
					}                   
				}
				if($twosex!=$oldessex){
					if($creditnotecustinfoch!=''){
						$creditnotecustinfoch.='<br> SEX <span style="color:green;" id="prohisfromtospan">( From '.$oldessex.' To '.$twosex.' ) </span>';
					}
					else{
						$creditnotecustinfoch.=''.(($creditnoteinfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessusercus['modulename'].' Information</span> <br> SEX <span style="color:green;" id="prohisfromtospan">( From '.$oldessex.' To '.$twosex.' ) </span>';
					}                   
				}
				if($area!=$oldesarea){
					if($creditnotecustinfoch!=''){
						$creditnotecustinfoch.='<br> BILLING STREET <span style="color:green;" id="prohisfromtospan">( From '.$oldesarea.' To '.$area.' ) </span>';
					}
					else{
						$creditnotecustinfoch.=''.(($creditnoteinfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessusercus['modulename'].' Information</span> <br> BILLING STREET <span style="color:green;" id="prohisfromtospan">( From '.$oldesarea.' To '.$area.' ) </span>';
					}                   
				}
				if($city!=$oldescity){
					if($creditnotecustinfoch!=''){
						$creditnotecustinfoch.='<br> BILLING CITY/TOWN <span style="color:green;" id="prohisfromtospan">( From '.$oldescity.' To '.$city.' ) </span>';
					}
					else{
						$creditnotecustinfoch.=''.(($creditnoteinfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessusercus['modulename'].' Information</span> <br> BILLING CITY/TOWN <span style="color:green;" id="prohisfromtospan">( From '.$oldescity.' To '.$city.' ) </span>';
					}                   
				}
				if($state!=$oldesstate){
					if($creditnotecustinfoch!=''){
						$creditnotecustinfoch.='<br> BILLING STATE <span style="color:green;" id="prohisfromtospan">( From '.$oldesstate.' To '.$state.' ) </span>';
					}
					else{
						$creditnotecustinfoch.=''.(($creditnoteinfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessusercus['modulename'].' Information</span> <br> BILLING STATE <span style="color:green;" id="prohisfromtospan">( From '.$oldesstate.' To '.$state.' ) </span>';
					}                   
				}
				if($district!=$oldesdistrict){
					if($creditnotecustinfoch!=''){
						$creditnotecustinfoch.='<br> BILLING PIN <span style="color:green;" id="prohisfromtospan">( From '.$oldesdistrict.' To '.$district.' ) </span>';
					}
					else{
						$creditnotecustinfoch.=''.(($creditnoteinfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessusercus['modulename'].' Information</span> <br> BILLING PIN <span style="color:green;" id="prohisfromtospan">( From '.$oldesdistrict.' To '.$district.' ) </span>';
					}                   
				}
				if($pincode!=$oldespincode){
					if($creditnotecustinfoch!=''){
						$creditnotecustinfoch.='<br> BILLING COUNTRY/REGION <span style="color:green;" id="prohisfromtospan">( From '.$oldespincode.' To '.$pincode.' ) </span>';
					}
					else{
						$creditnotecustinfoch.=''.(($creditnoteinfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessusercus['modulename'].' Information</span> <br> BILLING COUNTRY/REGION <span style="color:green;" id="prohisfromtospan">( From '.$oldespincode.' To '.$pincode.' ) </span>';
					}                   
				}
				if($sarea!=$oldessarea){
					if($creditnotecustinfoch!=''){
						$creditnotecustinfoch.='<br> SHIPPING STREET <span style="color:green;" id="prohisfromtospan">( From '.$oldessarea.' To '.$sarea.' ) </span>';
					}
					else{
						$creditnotecustinfoch.=''.(($creditnoteinfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessusercus['modulename'].' Information</span> <br> SHIPPING STREET <span style="color:green;" id="prohisfromtospan">( From '.$oldessarea.' To '.$sarea.' ) </span>';
					}                   
				}
				if($scity!=$oldesscity){
					if($creditnotecustinfoch!=''){
						$creditnotecustinfoch.='<br> SHIPPING CITY/TOWN <span style="color:green;" id="prohisfromtospan">( From '.$oldesscity.' To '.$scity.' ) </span>';
					}
					else{
						$creditnotecustinfoch.=''.(($creditnoteinfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessusercus['modulename'].' Information</span> <br> SHIPPING CITY/TOWN <span style="color:green;" id="prohisfromtospan">( From '.$oldesscity.' To '.$scity.' ) </span>';
					}                   
				}
				if($sstate!=$oldessstate){
					if($creditnotecustinfoch!=''){
						$creditnotecustinfoch.='<br> SHIPPING STATE <span style="color:green;" id="prohisfromtospan">( From '.$oldessstate.' To '.$sstate.' ) </span>';
					}
					else{
						$creditnotecustinfoch.=''.(($creditnoteinfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessusercus['modulename'].' Information</span> <br> SHIPPING STATE <span style="color:green;" id="prohisfromtospan">( From '.$oldessstate.' To '.$sstate.' ) </span>';
					}                   
				}
				if($sdistrict!=$oldessdistrict){
					if($creditnotecustinfoch!=''){
						$creditnotecustinfoch.='<br> SHIPPING PIN <span style="color:green;" id="prohisfromtospan">( From '.$oldessdistrict.' To '.$sdistrict.' ) </span>';
					}
					else{
						$creditnotecustinfoch.=''.(($creditnoteinfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessusercus['modulename'].' Information</span> <br> SHIPPING PIN <span style="color:green;" id="prohisfromtospan">( From '.$oldessdistrict.' To '.$sdistrict.' ) </span>';
					}                   
				}
				if($spincode!=$oldesspincode){
					if($creditnotecustinfoch!=''){
						$creditnotecustinfoch.='<br> SHIPPING COUNTRY/REGION <span style="color:green;" id="prohisfromtospan">( From '.$oldesspincode.' To '.$spincode.' ) </span>';
					}
					else{
						$creditnotecustinfoch.=''.(($creditnoteinfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessusercus['modulename'].' Information</span> <br> SHIPPING COUNTRY/REGION <span style="color:green;" id="prohisfromtospan">( From '.$oldesspincode.' To '.$spincode.' ) </span>';
					}                   
				}
				if($twoworkphone!=$oldestwoworkphone){
					if($creditnotecustinfoch!=''){
						$creditnotecustinfoch.='<br> WORK PHONE <span style="color:green;" id="prohisfromtospan">( From '.$oldestwoworkphone.' To '.$twoworkphone.' ) </span>';
					}
					else{
						$creditnotecustinfoch.=''.(($creditnoteinfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessusercus['modulename'].' Information</span> <br> WORK PHONE <span style="color:green;" id="prohisfromtospan">( From '.$oldestwoworkphone.' To '.$twoworkphone.' ) </span>';
					}                   
				}
				if($twomobilephone!=$oldestwomobilephone){
					if($creditnotecustinfoch!=''){
						$creditnotecustinfoch.='<br> MOBILE PHONE <span style="color:green;" id="prohisfromtospan">( From '.$oldestwomobilephone.' To '.$twomobilephone.' ) </span>';
					}
					else{
						$creditnotecustinfoch.=''.(($creditnoteinfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessusercus['modulename'].' Information</span> <br> MOBILE PHONE <span style="color:green;" id="prohisfromtospan">( From '.$oldestwomobilephone.' To '.$twomobilephone.' ) </span>';
					}                   
				}
				if($gstrtype!=$oldesgstrtype){
					if($creditnotecustinfoch!=''){
						$creditnotecustinfoch.='<br> GST TREATMENT <span style="color:green;" id="prohisfromtospan">( From '.$oldesgstrtype.' To '.$gstrtype.' ) </span>';
					}
					else{
						$creditnotecustinfoch.=''.(($creditnoteinfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessusercus['modulename'].' Information</span> <br> GST TREATMENT <span style="color:green;" id="prohisfromtospan">( From '.$oldesgstrtype.' To '.$gstrtype.' ) </span>';
					}                   
				}
				if($gstno!=$oldesgstno){
					if($creditnotecustinfoch!=''){
						$creditnotecustinfoch.='<br> GSTIN <span style="color:green;" id="prohisfromtospan">( From '.$oldesgstno.' To '.$gstno.' ) </span>';
					}
					else{
						$creditnotecustinfoch.=''.(($creditnoteinfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessusercus['modulename'].' Information</span> <br> GSTIN <span style="color:green;" id="prohisfromtospan">( From '.$oldesgstno.' To '.$gstno.' ) </span>';
					}                   
				}
				if($pos!=$oldespos){
					if($creditnotecustinfoch!=''){
						$creditnotecustinfoch.='<br> PLACE OF SUPPLY <span style="color:green;" id="prohisfromtospan">( From '.$oldespos.' To '.$pos.' ) </span>';
					}
					else{
						$creditnotecustinfoch.=''.(($creditnoteinfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessusercus['modulename'].' Information</span> <br> PLACE OF SUPPLY <span style="color:green;" id="prohisfromtospan">( From '.$oldespos.' To '.$pos.' ) </span>';
					}                   
				}
				if($dlno20!=$oldesdlno20){
					if($creditnotecustinfoch!=''){
						$creditnotecustinfoch.='<br> DL No 20 <span style="color:green;" id="prohisfromtospan">( From '.$oldesdlno20.' To '.$dlno20.' ) </span>';
					}
					else{
						$creditnotecustinfoch.=''.(($creditnoteinfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessusercus['modulename'].' Information</span> <br> DL No 20 <span style="color:green;" id="prohisfromtospan">( From '.$oldesdlno20.' To '.$dlno20.' ) </span>';
					}                   
				}
				if($dlno21!=$oldesdlno21){
					if($creditnotecustinfoch!=''){
						$creditnotecustinfoch.='<br> DL No 21 <span style="color:green;" id="prohisfromtospan">( From '.$oldesdlno21.' To '.$dlno21.' ) </span>';
					}
					else{
						$creditnotecustinfoch.=''.(($creditnoteinfoch!='')?'<br>':'').'<span style="color:royalblue;">'.$infomainaccessusercus['modulename'].' Information</span> <br> DL No 21 <span style="color:green;" id="prohisfromtospan">( From '.$oldesdlno21.' To '.$dlno21.' ) </span>';
					}                   
				}
				//FOR CREDITNOTE CUSTOMER INFORMATIONS DETAILS HISTORY
				$creditnoteiteminfoch = '';
					...
					for($i=0; $i<count($_POST['oldesproductname']); $i++){
						...
						if($productname!=''){
							if($productname!=$oldesproductname){
								if($creditnoteiteminfoch!=''){
									$creditnoteiteminfoch.='<br> '.$infomainaccessuserpro['modulename'].' Name <span style="color:green;" id="prohisfromtospan">( From '.$oldesproductname.' To '.$productname.' ) </span>';
								}
								else{
									$creditnoteiteminfoch.=''.(($creditnoteinfoch!=''||$creditnotecustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> '.$infomainaccessuserpro['modulename'].' Name <span style="color:green;" id="prohisfromtospan">( From '.$oldesproductname.' To '.$productname.' ) </span>';
								}                   
							}
							if($manufacturer!=$oldesmanufacturer){
								if($creditnoteiteminfoch!=''){
									$creditnoteiteminfoch.='<br> '.$access['txtnamecategory'].' <span style="color:green;" id="prohisfromtospan">( From '.$oldesmanufacturer.' To '.$manufacturer.' ) </span>';
								}
								else{
									$creditnoteiteminfoch.=''.(($creditnoteinfoch!=''||$creditnotecustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> '.$access['txtnamecategory'].' <span style="color:green;" id="prohisfromtospan">( From '.$oldesmanufacturer.' To '.$manufacturer.' ) </span>';
								}                   
							}
							if($producthsn!=$oldesproducthsn){
								if($creditnoteiteminfoch!=''){
									$creditnoteiteminfoch.='<br> HSN Code <span style="color:green;" id="prohisfromtospan">( From '.$oldesproducthsn.' To '.$producthsn.' ) </span>';
								}
								else{
									$creditnoteiteminfoch.=''.(($creditnoteinfoch!=''||$creditnotecustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> HSN Code <span style="color:green;" id="prohisfromtospan">( From '.$oldesproducthsn.' To '.$producthsn.' ) </span>';
								}                   
							}
							if($productdescription!=$oldesproductdescription){
								if($creditnoteiteminfoch!=''){
									$creditnoteiteminfoch.='<br> Description <span style="color:green;" id="prohisfromtospan">( From '.$oldesproductdescription.' To '.$productdescription.' ) </span>';
								}
								else{
									$creditnoteiteminfoch.=''.(($creditnoteinfoch!=''||$creditnotecustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Description <span style="color:green;" id="prohisfromtospan">( From '.$oldesproductdescription.' To '.$productdescription.' ) </span>';
								}                   
							}
							if($batch!=$oldesbatch){
								if($creditnoteiteminfoch!=''){
									$creditnoteiteminfoch.='<br> Batch <span style="color:green;" id="prohisfromtospan">( From '.$oldesbatch.' To '.$batch.' ) </span>';
								}
								else{
									$creditnoteiteminfoch.=''.(($creditnoteinfoch!=''||$creditnotecustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Batch <span style="color:green;" id="prohisfromtospan">( From '.$oldesbatch.' To '.$batch.' ) </span>';
								}                   
							}
							if($expdate!=$oldesexpdate){
								if($creditnoteiteminfoch!=''){
									$creditnoteiteminfoch.='<br> Expiry <span style="color:green;" id="prohisfromtospan">( From '.date($date,strtotime($oldesexpdate)).' To '.date($date,strtotime($expdate)).' ) </span>';
								}
								else{
									$creditnoteiteminfoch.=''.(($creditnoteinfoch!=''||$creditnotecustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Expiry <span style="color:green;" id="prohisfromtospan">( From '.date($date,strtotime($oldesexpdate)).' To '.date($date,strtotime($expdate)).' ) </span>';
								}                   
							}
							if($productrate!=$oldesproductrate){
								if($creditnoteiteminfoch!=''){
									$creditnoteiteminfoch.='<br> Rate <span style="color:green;" id="prohisfromtospan">( From '.$oldesproductrate.' To '.$productrate.' ) </span>';
								}
								else{
									$creditnoteiteminfoch.=''.(($creditnoteinfoch!=''||$creditnotecustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Rate <span style="color:green;" id="prohisfromtospan">( From '.$oldesproductrate.' To '.$productrate.' ) </span>';
								}                   
							}
							if($mrp!=$oldesmrp){
								if($creditnoteiteminfoch!=''){
									$creditnoteiteminfoch.='<br> Mrp <span style="color:green;" id="prohisfromtospan">( From '.$oldesmrp.' To '.$mrp.' ) </span>';
								}
								else{
									$creditnoteiteminfoch.=''.(($creditnoteinfoch!=''||$creditnotecustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Mrp <span style="color:green;" id="prohisfromtospan">( From '.$oldesmrp.' To '.$mrp.' ) </span>';
								}                   
							}
							if($quantity!=$oldesquantity){
								if($creditnoteiteminfoch!=''){
									$creditnoteiteminfoch.='<br> Quantity <span style="color:green;" id="prohisfromtospan">( From '.$oldesquantity.' To '.$quantity.' ) </span>';
								}
								else{
									$creditnoteiteminfoch.=''.(($creditnoteinfoch!=''||$creditnotecustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Quantity <span style="color:green;" id="prohisfromtospan">( From '.$oldesquantity.' To '.$quantity.' ) </span>';
								}                   
							}
							if($unit!=$oldesunit){
								if($creditnoteiteminfoch!=''){
									$creditnoteiteminfoch.='<br> Unit <span style="color:green;" id="prohisfromtospan">( From '.$oldesunit.' To '.$unit.' ) </span>';
								}
								else{
									$creditnoteiteminfoch.=''.(($creditnoteinfoch!=''||$creditnotecustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Unit <span style="color:green;" id="prohisfromtospan">( From '.$oldesunit.' To '.$unit.' ) </span>';
								}                   
							}
							if($noofpacks!=$oldesnoofpacks){
								if($creditnoteiteminfoch!=''){
									$creditnoteiteminfoch.='<br> Pack <span style="color:green;" id="prohisfromtospan">( From '.$oldesnoofpacks.' To '.$noofpacks.' ) </span>';
								}
								else{
									$creditnoteiteminfoch.=''.(($creditnoteinfoch!=''||$creditnotecustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Pack <span style="color:green;" id="prohisfromtospan">( From '.$oldesnoofpacks.' To '.$noofpacks.' ) </span>';
								}                   
							}
							if($productvalue!=$oldesproductvalue){
								if($creditnoteiteminfoch!=''){
									$creditnoteiteminfoch.='<br> Taxable Value <span style="color:green;" id="prohisfromtospan">( From '.$oldesproductvalue.' To '.$productvalue.' ) </span>';
								}
								else{
									$creditnoteiteminfoch.=''.(($creditnoteinfoch!=''||$creditnotecustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Taxable Value <span style="color:green;" id="prohisfromtospan">( From '.$oldesproductvalue.' To '.$productvalue.' ) </span>';
								}                   
							}
							if($prodiscount!=$oldesprodiscount){
								if($creditnoteiteminfoch!=''){
									$creditnoteiteminfoch.='<br> '.$access['txtprodiscreditnote'].' <span style="color:green;" id="prohisfromtospan">( From '.$oldesprodiscount.' To '.$prodiscount.' ) </span>';
								}
								else{
									$creditnoteiteminfoch.=''.(($creditnoteinfoch!=''||$creditnotecustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> '.$access['txtprodiscreditnote'].' <span style="color:green;" id="prohisfromtospan">( From '.$oldesprodiscount.' To '.$prodiscount.' ) </span>';
								}                   
							}
							if($prodiscounttype!=$oldesprodiscounttype){
								if($creditnoteiteminfoch!=''){
									$creditnoteiteminfoch.='<br> Discounted By <span style="color:green;" id="prohisfromtospan">( From '.(($oldesprodiscounttype==0)?'%':'?').' To '.(($prodiscounttype==0)?'%':'?').' ) </span>';
								}
								else{
									$creditnoteiteminfoch.=''.(($creditnoteinfoch!=''||$creditnotecustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Discounted By <span style="color:green;" id="prohisfromtospan">( From '.(($oldesprodiscounttype==0)?'%':'?').' To '.(($prodiscounttype==0)?'%':'?').' ) </span>';
								}                   
							}
							if($taxvalue!=$oldestaxvalue){
								if($creditnoteiteminfoch!=''){
									$creditnoteiteminfoch.='<br> Tax Value <span style="color:green;" id="prohisfromtospan">( From '.$oldestaxvalue.' To '.$taxvalue.' ) </span>';
								}
								else{
									$creditnoteiteminfoch.=''.(($creditnoteinfoch!=''||$creditnotecustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Tax Value <span style="color:green;" id="prohisfromtospan">( From '.$oldestaxvalue.' To '.$taxvalue.' ) </span>';
								}                   
							}
							if($vat!=$oldesvat){
								if($creditnoteiteminfoch!=''){
									$creditnoteiteminfoch.='<br> Gst <span style="color:green;" id="prohisfromtospan">( From '.$oldesvat.' % ('.$oldesicsgsthis.') To '.$vat.' % ('.$ansforsepgstval.') ) </span>';
								}
								else{
									$creditnoteiteminfoch.=''.(($creditnoteinfoch!=''||$creditnotecustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Gst <span style="color:green;" id="prohisfromtospan">( From '.$oldesvat.' % ('.$oldesicsgsthis.') To '.$vat.' % ('.$ansforsepgstval.') ) </span>';
								}                   
							}
							if($productnetvalue!=$oldesproductnetvalue){
								if($creditnoteiteminfoch!=''){
									$creditnoteiteminfoch.='<br> Amount <span style="color:green;" id="prohisfromtospan">( From '.$oldesproductnetvalue.' To '.$productnetvalue.' ) </span>';
								}
								else{
									$creditnoteiteminfoch.=''.(($creditnoteinfoch!=''||$creditnotecustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Amount <span style="color:green;" id="prohisfromtospan">( From '.$oldesproductnetvalue.' To '.$productnetvalue.' ) </span>';
								}                   
							}
						}
					}
					...
					for($i=0; $i<count($_POST['oldesproductname']); $i++){
						...
						if($productname!=''){
							...
							if($oldesproductname!=''){                    
								if($oldesproductname!=''){
									if($creditnoteiteminfoch!=''){
										$creditnoteiteminfoch.='<br> '.$infomainaccessuserpro['modulename'].' Name <span style="color:red;" id="prohisfromtospan">( From '.$oldesproductname.' To &nbsp; ) </span>';
									}
									else{
										$creditnoteiteminfoch.=''.(($creditnoteinfoch!=''||$creditnotecustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> '.$infomainaccessuserpro['modulename'].' Name <span style="color:red;" id="prohisfromtospan">( From '.$oldesproductname.' To &nbsp; ) </span>';
									}
								}
								if($oldesmanufacturer!=''){
									if($creditnoteiteminfoch!=''){
										$creditnoteiteminfoch.='<br> '.$access['txtnamecategory'].' <span style="color:red;" id="prohisfromtospan">( From '.$oldesmanufacturer.' To &nbsp; ) </span>';
									}
									else{
										$creditnoteiteminfoch.=''.(($creditnoteinfoch!=''||$creditnotecustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> '.$access['txtnamecategory'].' <span style="color:red;" id="prohisfromtospan">( From '.$oldesmanufacturer.' To &nbsp; ) </span>';
									}
								}
								if($oldesproducthsn!=''){
									if($creditnoteiteminfoch!=''){
										$creditnoteiteminfoch.='<br> HSN Code <span style="color:red;" id="prohisfromtospan">( From '.$oldesproducthsn.' To &nbsp; ) </span>';
									}
									else{
										$creditnoteiteminfoch.=''.(($creditnoteinfoch!=''||$creditnotecustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> HSN Code <span style="color:red;" id="prohisfromtospan">( From '.$oldesproducthsn.' To &nbsp; ) </span>';
									}
								}
								if($oldesproductdescription!=''){
									if($creditnoteiteminfoch!=''){
										$creditnoteiteminfoch.='<br> Description <span style="color:red;" id="prohisfromtospan">( From '.$oldesproductdescription.' To &nbsp; ) </span>';
									}
									else{
										$creditnoteiteminfoch.=''.(($creditnoteinfoch!=''||$creditnotecustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Description <span style="color:red;" id="prohisfromtospan">( From '.$oldesproductdescription.' To &nbsp; ) </span>';
									}
								}
								if($oldesbatch!=''){
									if($creditnoteiteminfoch!=''){
										$creditnoteiteminfoch.='<br> Batch <span style="color:red;" id="prohisfromtospan">( From '.$oldesbatch.' To &nbsp; ) </span>';
									}
									else{
										$creditnoteiteminfoch.=''.(($creditnoteinfoch!=''||$creditnotecustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Batch <span style="color:red;" id="prohisfromtospan">( From '.$oldesbatch.' To &nbsp; ) </span>';
									}
								}
								if($oldesexpdate!=''){
									if($creditnoteiteminfoch!=''){
										$creditnoteiteminfoch.='<br> Expiry <span style="color:red;" id="prohisfromtospan">( From '.date($date,strtotime($oldesexpdate)).' To &nbsp; ) </span>';
									}
									else{
										$creditnoteiteminfoch.=''.(($creditnoteinfoch!=''||$creditnotecustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Expiry <span style="color:red;" id="prohisfromtospan">( From '.date($date,strtotime($oldesexpdate)).' To &nbsp; ) </span>';
									}
								}
								if($oldesproductrate!=''){
									if($creditnoteiteminfoch!=''){
										$creditnoteiteminfoch.='<br> Rate <span style="color:red;" id="prohisfromtospan">( From '.$oldesproductrate.' To &nbsp; ) </span>';
									}
									else{
										$creditnoteiteminfoch.=''.(($creditnoteinfoch!=''||$creditnotecustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Rate <span style="color:red;" id="prohisfromtospan">( From '.$oldesproductrate.' To &nbsp; ) </span>';
									}
								}
								if($oldesmrp!=''){
									if($creditnoteiteminfoch!=''){
										$creditnoteiteminfoch.='<br> Mrp <span style="color:red;" id="prohisfromtospan">( From '.$oldesmrp.' To &nbsp; ) </span>';
									}
									else{
										$creditnoteiteminfoch.=''.(($creditnoteinfoch!=''||$creditnotecustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Mrp <span style="color:red;" id="prohisfromtospan">( From '.$oldesmrp.' To &nbsp; ) </span>';
									}
								}
								if($oldesquantity!=''){
									if($creditnoteiteminfoch!=''){
										$creditnoteiteminfoch.='<br> Quantity <span style="color:red;" id="prohisfromtospan">( From '.$oldesquantity.' To &nbsp; ) </span>';
									}
									else{
										$creditnoteiteminfoch.=''.(($creditnoteinfoch!=''||$creditnotecustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Quantity <span style="color:red;" id="prohisfromtospan">( From '.$oldesquantity.' To &nbsp; ) </span>';
									}
								}
								if($oldesunit!=''){
									if($creditnoteiteminfoch!=''){
										$creditnoteiteminfoch.='<br> Unit <span style="color:red;" id="prohisfromtospan">( From '.$oldesunit.' To &nbsp; ) </span>';
									}
									else{
										$creditnoteiteminfoch.=''.(($creditnoteinfoch!=''||$creditnotecustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Unit <span style="color:red;" id="prohisfromtospan">( From '.$oldesunit.' To &nbsp; ) </span>';
									}
								}
								if($oldesnoofpacks!=''){
									if($creditnoteiteminfoch!=''){
										$creditnoteiteminfoch.='<br> Pack <span style="color:red;" id="prohisfromtospan">( From '.$oldesnoofpacks.' To &nbsp; ) </span>';
									}
									else{
										$creditnoteiteminfoch.=''.(($creditnoteinfoch!=''||$creditnotecustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Pack <span style="color:red;" id="prohisfromtospan">( From '.$oldesnoofpacks.' To &nbsp; ) </span>';
									}
								}
								if($oldesproductvalue!=''){
									if($creditnoteiteminfoch!=''){
										$creditnoteiteminfoch.='<br> Taxable Value <span style="color:red;" id="prohisfromtospan">( From '.$oldesproductvalue.' To &nbsp; ) </span>';
									}
									else{
										$creditnoteiteminfoch.=''.(($creditnoteinfoch!=''||$creditnotecustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Taxable Value <span style="color:red;" id="prohisfromtospan">( From '.$oldesproductvalue.' To &nbsp; ) </span>';
									}
								}
								if($oldesprodiscount!=''){
									if($creditnoteiteminfoch!=''){
										$creditnoteiteminfoch.='<br> Discount <span style="color:red;" id="prohisfromtospan">( From '.$oldesprodiscount.' To &nbsp; ) </span>';
									}
									else{
										$creditnoteiteminfoch.=''.(($creditnoteinfoch!=''||$creditnotecustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Discount <span style="color:red;" id="prohisfromtospan">( From '.$oldesprodiscount.' To &nbsp; ) </span>';
									}
								}
								if($oldestaxvalue!=''){
									if($creditnoteiteminfoch!=''){
										$creditnoteiteminfoch.='<br> Tax Value <span style="color:red;" id="prohisfromtospan">( From '.$oldestaxvalue.' To &nbsp; ) </span>';
									}
									else{
										$creditnoteiteminfoch.=''.(($creditnoteinfoch!=''||$creditnotecustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Tax Value <span style="color:red;" id="prohisfromtospan">( From '.$oldestaxvalue.' To &nbsp; ) </span>';
									}
								}
								if($oldesvat!=''){
									if($creditnoteiteminfoch!=''){
										$creditnoteiteminfoch.='<br> Gst <span style="color:red;" id="prohisfromtospan">( From '.$oldesvat.' % ('.$oldesicsgsthis.') To &nbsp; ) </span>';
									}
									else{
										$creditnoteiteminfoch.=''.(($creditnoteinfoch!=''||$creditnotecustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Gst <span style="color:red;" id="prohisfromtospan">( From '.$oldesvat.' % ('.$oldesicsgsthis.') To &nbsp; ) </span>';
									}
								}
								if($oldesproductnetvalue!=''){
									if($creditnoteiteminfoch!=''){
										$creditnoteiteminfoch.='<br> Amount <span style="color:red;" id="prohisfromtospan">( From '.$oldesproductnetvalue.' To &nbsp; ) </span>';
									}
									else{
										$creditnoteiteminfoch.=''.(($creditnoteinfoch!=''||$creditnotecustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Amount <span style="color:red;" id="prohisfromtospan">( From '.$oldesproductnetvalue.' To &nbsp; ) </span>';
									}
								}
							}
						}
				...
				elseif ($curans>$oldans) {
					for($i=0; $i<count($_POST['productname']); $i++){
						...
						if($oldesproductname!=''){
							...
							if($productname!=''){
								if($creditnoteiteminfoch!=''){
									$creditnoteiteminfoch.='<br> '.$infomainaccessuserpro['modulename'].' Name <span style="color:green;" id="prohisfromtospan">( '.$productname.' ) </span>';
								}
								else{
									$creditnoteiteminfoch.=''.(($creditnoteinfoch!=''||$creditnotecustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> '.$infomainaccessuserpro['modulename'].' Name <span style="color:green;" id="prohisfromtospan">( '.$productname.' ) </span>';        
								}
							}
							if($manufacturer!=''){
								if($creditnoteiteminfoch!=''){
									$creditnoteiteminfoch.='<br> '.$access['txtnamecategory'].' <span style="color:green;" id="prohisfromtospan">( '.$manufacturer.' ) </span>';
								}
								else{
									$creditnoteiteminfoch.=''.(($creditnoteinfoch!=''||$creditnotecustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> '.$access['txtnamecategory'].' <span style="color:green;" id="prohisfromtospan">( '.$manufacturer.' ) </span>';        
								}
							}
							if($producthsn!=''){
								if($creditnoteiteminfoch!=''){
									$creditnoteiteminfoch.='<br> HSN Code <span style="color:green;" id="prohisfromtospan">( '.$producthsn.' ) </span>';
								}
								else{
									$creditnoteiteminfoch.=''.(($creditnoteinfoch!=''||$creditnotecustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> HSN Code <span style="color:green;" id="prohisfromtospan">( '.$producthsn.' ) </span>';        
								}
							}
							if($productdescription!=''){
								if($creditnoteiteminfoch!=''){
									$creditnoteiteminfoch.='<br> Description <span style="color:green;" id="prohisfromtospan">( '.$productdescription.' ) </span>';
								}
								else{
									$creditnoteiteminfoch.=''.(($creditnoteinfoch!=''||$creditnotecustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Description <span style="color:green;" id="prohisfromtospan">( '.$productdescription.' ) </span>';        
								}
							}
							if($batch!=''){
								if($creditnoteiteminfoch!=''){
									$creditnoteiteminfoch.='<br> Batch <span style="color:green;" id="prohisfromtospan">( '.$batch.' ) </span>';
								}
								else{
									$creditnoteiteminfoch.=''.(($creditnoteinfoch!=''||$creditnotecustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Batch <span style="color:green;" id="prohisfromtospan">( '.$batch.' ) </span>';        
								}
							}
							if($expdate!=''){
								if($creditnoteiteminfoch!=''){
									$creditnoteiteminfoch.='<br> Expiry <span style="color:green;" id="prohisfromtospan">( '.date($date,strtotime($expdate)).' ) </span>';
								}
								else{
									$creditnoteiteminfoch.=''.(($creditnoteinfoch!=''||$creditnotecustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Expiry <span style="color:green;" id="prohisfromtospan">( '.date($date,strtotime($expdate)).' ) </span>';        
								}
							}
							if($productrate!=''){
								if($creditnoteiteminfoch!=''){
									$creditnoteiteminfoch.='<br> Rate <span style="color:green;" id="prohisfromtospan">( '.$productrate.' ) </span>';
								}
								else{
									$creditnoteiteminfoch.=''.(($creditnoteinfoch!=''||$creditnotecustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Rate <span style="color:green;" id="prohisfromtospan">( '.$productrate.' ) </span>';        
								}
							}
							if($mrp!=''){
								if($creditnoteiteminfoch!=''){
									$creditnoteiteminfoch.='<br> Mrp <span style="color:green;" id="prohisfromtospan">( '.$mrp.' ) </span>';
								}
								else{
									$creditnoteiteminfoch.=''.(($creditnoteinfoch!=''||$creditnotecustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Mrp <span style="color:green;" id="prohisfromtospan">( '.$mrp.' ) </span>';        
								}
							}
							if($quantity!=''){
								if($creditnoteiteminfoch!=''){
									$creditnoteiteminfoch.='<br> Quantity <span style="color:green;" id="prohisfromtospan">( '.$quantity.' ) </span>';
								}
								else{
									$creditnoteiteminfoch.=''.(($creditnoteinfoch!=''||$creditnotecustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Quantity <span style="color:green;" id="prohisfromtospan">( '.$quantity.' ) </span>';        
								}
							}
							if($unit!=''){
								if($creditnoteiteminfoch!=''){
									$creditnoteiteminfoch.='<br> Unit <span style="color:green;" id="prohisfromtospan">( '.$unit.' ) </span>';
								}
								else{
									$creditnoteiteminfoch.=''.(($creditnoteinfoch!=''||$creditnotecustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Unit <span style="color:green;" id="prohisfromtospan">( '.$unit.' ) </span>';        
								}
							}
							if($noofpacks!=''){
								if($creditnoteiteminfoch!=''){
									$creditnoteiteminfoch.='<br> Pack <span style="color:green;" id="prohisfromtospan">( '.$noofpacks.' ) </span>';
								}
								else{
									$creditnoteiteminfoch.=''.(($creditnoteinfoch!=''||$creditnotecustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Pack <span style="color:green;" id="prohisfromtospan">( '.$noofpacks.' ) </span>';        
								}
							}
							if($productvalue!=''){
								if($creditnoteiteminfoch!=''){
									$creditnoteiteminfoch.='<br> Taxable Value <span style="color:green;" id="prohisfromtospan">( '.$productvalue.' ) </span>';
								}
								else{
									$creditnoteiteminfoch.=''.(($creditnoteinfoch!=''||$creditnotecustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Taxable Value <span style="color:green;" id="prohisfromtospan">( '.$productvalue.' ) </span>';        
								}
							}
							if($prodiscount!=''){
								if($creditnoteiteminfoch!=''){
									$creditnoteiteminfoch.='<br> '.$access['txtprodiscreditnote'].' <span style="color:green;" id="prohisfromtospan">( '.$prodiscount.' ) </span>';
								}
								else{
									$creditnoteiteminfoch.=''.(($creditnoteinfoch!=''||$creditnotecustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> '.$access['txtprodiscreditnote'].' <span style="color:green;" id="prohisfromtospan">( '.$prodiscount.' ) </span>';        
								}
							}
							if($prodiscounttype!=''){
								if($creditnoteiteminfoch!=''){
									$creditnoteiteminfoch.='<br> Discounted By <span style="color:green;" id="prohisfromtospan">( '.(($prodiscounttype==0)?'%':'?').' ) </span>';
								}
								else{
									$creditnoteiteminfoch.=''.(($creditnoteinfoch!=''||$creditnotecustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Discounted By <span style="color:green;" id="prohisfromtospan">( '.(($prodiscounttype==0)?'%':'?').' ) </span>';        
								}
							}
							if($taxvalue!=''){
								if($creditnoteiteminfoch!=''){
									$creditnoteiteminfoch.='<br> Tax Value <span style="color:green;" id="prohisfromtospan">( '.$taxvalue.' ) </span>';
								}
								else{
									$creditnoteiteminfoch.=''.(($creditnoteinfoch!=''||$creditnotecustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Tax Value <span style="color:green;" id="prohisfromtospan">( '.$taxvalue.' ) </span>';        
								}
							}
							if($vat!=''){
								if($creditnoteiteminfoch!=''){
									$creditnoteiteminfoch.='<br> Gst <span style="color:green;" id="prohisfromtospan">( '.$vat.' % ('.$ansforsepgstval.') ) </span>';
								}
								else{
									$creditnoteiteminfoch.=''.(($creditnoteinfoch!=''||$creditnotecustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Gst <span style="color:green;" id="prohisfromtospan">( '.$vat.' % ('.$ansforsepgstval.') ) </span>';        
								}
							}
							if($productnetvalue!=''){
								if($creditnoteiteminfoch!=''){
									$creditnoteiteminfoch.='<br> Amount <span style="color:green;" id="prohisfromtospan">( '.$productnetvalue.' ) </span>';
								}
								else{
									$creditnoteiteminfoch.=''.(($creditnoteinfoch!=''||$creditnotecustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Amount <span style="color:green;" id="prohisfromtospan">( '.$productnetvalue.' ) </span>';        
								}
							}
						}
					}
				...
				if($totalitems!=$oldestotalitems){
					if($creditnoteiteminfoch!=''){
						$creditnoteiteminfoch.='<br> Total Items <span style="color:green;" id="prohisfromtospan">( From '.$oldestotalitems.' To '.$totalitems.' ) </span>';
					}
					else{
						$creditnoteiteminfoch.=''.(($creditnoteinfoch!=''||$creditnotecustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Total Items <span style="color:green;" id="prohisfromtospan">( From '.$oldestotalitems.' To '.$totalitems.' ) </span>';
					}
				}
				if($totalquantity!=$oldestotalquantity){
					if($creditnoteiteminfoch!=''){
						$creditnoteiteminfoch.='<br> Total Qty <span style="color:green;" id="prohisfromtospan">( From '.$oldestotalquantity.' To '.$totalquantity.' ) </span>';
					}
					else{
						$creditnoteiteminfoch.=''.(($creditnoteinfoch!=''||$creditnotecustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Total Qty <span style="color:green;" id="prohisfromtospan">( From '.$oldestotalquantity.' To '.$totalquantity.' ) </span>';
					}
				}
				if($totalamount!=$oldestotalamount){
					if($creditnoteiteminfoch!=''){
						$creditnoteiteminfoch.='<br> Sub Total <span style="color:green;" id="prohisfromtospan">( From '.$oldestotalamount.' To '.$totalamount.' ) </span>';
					}
					else{
						$creditnoteiteminfoch.=''.(($creditnoteinfoch!=''||$creditnotecustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Sub Total <span style="color:green;" id="prohisfromtospan">( From '.$oldestotalamount.' To '.$totalamount.' ) </span>';
					}
				}
				if($discountamount!=$oldesdiscountamount){
					if($creditnoteiteminfoch!=''){
						$creditnoteiteminfoch.='<br> Discount <span style="color:green;" id="prohisfromtospan">( From '.$oldesdiscountamount.' To '.$discountamount.' ) </span>';
					}
					else{
						$creditnoteiteminfoch.=''.(($creditnoteinfoch!=''||$creditnotecustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Discount <span style="color:green;" id="prohisfromtospan">( From '.$oldesdiscountamount.' To '.$discountamount.' ) </span>';
					}
				}
				if($totalvatamount!=$oldestotalvatamount){
					if($creditnoteiteminfoch!=''){
						$creditnoteiteminfoch.='<br> Total Tax <span style="color:green;" id="prohisfromtospan">( From '.$oldestotalvatamount.' To '.$totalvatamount.' ) </span>';
					}
					else{
						$creditnoteiteminfoch.=''.(($creditnoteinfoch!=''||$creditnotecustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Total Tax <span style="color:green;" id="prohisfromtospan">( From '.$oldestotalvatamount.' To '.$totalvatamount.' ) </span>';
					}
				}
				if($roundoff!=$oldesroundoff){
					if($creditnoteiteminfoch!=''){
						$creditnoteiteminfoch.='<br> Round Off <span style="color:green;" id="prohisfromtospan">( From '.$oldesroundoff.' To '.$roundoff.' ) </span>';
					}
					else{
						$creditnoteiteminfoch.=''.(($creditnoteinfoch!=''||$creditnotecustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Round Off <span style="color:green;" id="prohisfromtospan">( From '.$oldesroundoff.' To '.$roundoff.' ) </span>';
					}
				}
				if($grandtotal!=$oldesgrandtotal){
					if($creditnoteiteminfoch!=''){
						$creditnoteiteminfoch.='<br> Grand Total <span style="color:green;" id="prohisfromtospan">( From '.$oldesgrandtotal.' To '.$grandtotal.' ) </span>';
					}
					else{
						$creditnoteiteminfoch.=''.(($creditnoteinfoch!=''||$creditnotecustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Grand Total <span style="color:green;" id="prohisfromtospan">( From '.$oldesgrandtotal.' To '.$grandtotal.' ) </span>';
					}
				}
				if($description!=$oldesdescription){
					if($creditnoteiteminfoch!=''){
						$creditnoteiteminfoch.='<br> Description <span style="color:green;" id="prohisfromtospan">( From '.$oldesdescription.' To '.$description.' ) </span>';
					}
					else{
						$creditnoteiteminfoch.=''.(($creditnoteinfoch!=''||$creditnotecustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Description <span style="color:green;" id="prohisfromtospan">( From '.$oldesdescription.' To '.$description.' ) </span>';
					}
				}
				if($notes!=$oldesnotes){
					if($creditnoteiteminfoch!=''){
						$creditnoteiteminfoch.='<br> Notes <span style="color:green;" id="prohisfromtospan">( From '.$oldesnotes.' To '.$notes.' ) </span>';
					}
					else{
						$creditnoteiteminfoch.=''.(($creditnoteinfoch!=''||$creditnotecustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Notes <span style="color:green;" id="prohisfromtospan">( From '.$oldesnotes.' To '.$notes.' ) </span>';
					}
				}
				if($terms!=$oldesterms){
					if($creditnoteiteminfoch!=''){
						$creditnoteiteminfoch.='<br> Terms And Conditions <span style="color:green;" id="prohisfromtospan">( From '.$oldesterms.' To '.$terms.' ) </span>';
					}
					else{
						$creditnoteiteminfoch.=''.(($creditnoteinfoch!=''||$creditnotecustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Terms And Conditions <span style="color:green;" id="prohisfromtospan">( From '.$oldesterms.' To '.$terms.' ) </span>';
					}
				}
				if($fileattach!=$oldesfileattach){
					if($creditnoteiteminfoch!=''){
						$creditnoteiteminfoch.='<br> Attach <span style="color:green;" id="prohisfromtospan">( Changed ) </span>';
					}
					else{
						$creditnoteiteminfoch.=''.(($creditnoteinfoch!=''||$creditnotecustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Attach <span style="color:green;" id="prohisfromtospan">( Changed ) </span>';
					}
				}
				...
				//FOR GET INVOICE MODULENAME AND OTHER DETAILS
				if($invoiceno!=$oldesinvoiceno){
					if($creditnoteiteminfoch!=''){
						$creditnoteiteminfoch.='<br> '.$infomainaccessuserinv['modulename'].' Number <span style="color:green;" id="prohisfromtospan">( From '.$oldesinvoiceno.' To '.$invoiceno.' ) </span>';
					}
					else{
						$creditnoteiteminfoch.=''.(($creditnoteinfoch!=''||$creditnotecustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> '.$infomainaccessuserinv['modulename'].' Number <span style="color:green;" id="prohisfromtospan">( From '.$oldesinvoiceno.' To '.$invoiceno.' ) </span>';
					}
				}
				if($invoiceamount!=$oldesinvoiceamount){
					if($creditnoteiteminfoch!=''){
						$creditnoteiteminfoch.='<br> '.$infomainaccessuserinv['modulename'].' Amount <span style="color:green;" id="prohisfromtospan">( From '.$oldesinvoiceamount.' To '.$invoiceamount.' ) </span>';
					}
					else{
						$creditnoteiteminfoch.=''.(($creditnoteinfoch!=''||$creditnotecustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> '.$infomainaccessuserinv['modulename'].' Amount <span style="color:green;" id="prohisfromtospan">( From '.$oldesinvoiceamount.' To '.$invoiceamount.' ) </span>';
					}
				}
				if($invoicepaymentreceived!=$oldesinvoicepaymentreceived){
					if($creditnoteiteminfoch!=''){
						$creditnoteiteminfoch.='<br> '.$infomainaccessuserinv['modulename'].' Payment Received <span style="color:green;" id="prohisfromtospan">( From '.$oldesinvoicepaymentreceived.' To '.$invoicepaymentreceived.' ) </span>';
					}
					else{
						$creditnoteiteminfoch.=''.(($creditnoteinfoch!=''||$creditnotecustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> '.$infomainaccessuserinv['modulename'].' Payment Received <span style="color:green;" id="prohisfromtospan">( From '.$oldesinvoicepaymentreceived.' To '.$invoicepaymentreceived.' ) </span>';
					}
				}
				if($refundoption!=$oldesrefundoption){
					if($creditnoteiteminfoch!=''){
						$creditnoteiteminfoch.='<br> Refund Option <span style="color:green;" id="prohisfromtospan">( From '.$oldesrefundoption.' To '.$refundoption.' ) </span>';
					}
					else{
						$creditnoteiteminfoch.=''.(($creditnoteinfoch!=''||$creditnotecustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Refund Option <span style="color:green;" id="prohisfromtospan">( From '.$oldesrefundoption.' To '.$refundoption.' ) </span>';
					}
				}
				if($creditnoteterm!=$oldescreditnoteterm){
					if($creditnoteiteminfoch!=''){
						$creditnoteiteminfoch.='<br> Refund Method <span style="color:green;" id="prohisfromtospan">( From '.$oldescreditnoteterm.' To '.$creditnoteterm.' ) </span>';
					}
					else{
						$creditnoteiteminfoch.=''.(($creditnoteinfoch!=''||$creditnotecustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Refund Method <span style="color:green;" id="prohisfromtospan">( From '.$oldescreditnoteterm.' To '.$creditnoteterm.' ) </span>';
					}
				}
				if($validpaidamount!=$oldesvalidpaidamount&&$creditnoteterm=='CASH,CARD,GPAY'){
					if($creditnoteiteminfoch!=''){
						$creditnoteiteminfoch.='<br> Refund Amount <span style="color:green;" id="prohisfromtospan">( From '.$oldesvalidpaidamount.' To '.$validpaidamount.' ) </span>';
					}
					else{
						$creditnoteiteminfoch.=''.(($creditnoteinfoch!=''||$creditnotecustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Refund Amount <span style="color:green;" id="prohisfromtospan">( From '.$oldesvalidpaidamount.' To '.$validpaidamount.' ) </span>';
					}
				}
				if($validpaidamount!=$oldesvalidpaidamount&&$creditnoteterm=='CASH'){
					if($creditnoteiteminfoch!=''){
						$creditnoteiteminfoch.='<br> Refund Amount <span style="color:green;" id="prohisfromtospan">( From '.$oldesvalidpaidamount.' To '.$validpaidamount.' ) </span>';
					}
					else{
				...
				// if($validbalance!=$oldesvalidbalance&&$creditnoteterm=='CASH'){
				// 	if($creditnoteiteminfoch!=''){
				// 		$creditnoteiteminfoch.='<br> Balance Due <span style="color:green;" id="prohisfromtospan">( From '.$oldesvalidbalance.' To '.$validbalance.' ) </span>';
				// 	}
				// 	else{
				// 		$creditnoteiteminfoch.=''.(($creditnoteinfoch!=''||$creditnotecustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Balance Due <span style="color:green;" id="prohisfromtospan">( From '.$oldesvalidbalance.' To '.$validbalance.' ) </span>';
				// 	}
				// }
				// if($validbalance!=$oldesvalidbalance&&$creditnoteterm=='CASH,CARD,GPAY'){
				// 	if($creditnoteiteminfoch!=''){
				// 		$creditnoteiteminfoch.='<br> Balance Due <span style="color:green;" id="prohisfromtospan">( From '.$oldesvalidbalance.' To '.$validbalance.' ) </span>';
				// 	}
				// 	else{
				...
				// 	}
				// }
				// if($duedates!=$oldesduedates&&$creditnoteterm=='CREDIT'){
				// 	if($creditnoteiteminfoch!=''){
				// 		$creditnoteiteminfoch.='<br> Due Term <span style="color:green;" id="prohisfromtospan">( From '.$oldesduedates.' To '.$duedates.' ) </span>';
				// 	}
				// 	else{
				// 		$creditnoteiteminfoch.=''.(($creditnoteinfoch!=''||$creditnotecustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Due Term <span style="color:green;" id="prohisfromtospan">( From '.$oldesduedates.' To '.$duedates.' ) </span>';
				// 	}
				// }
				// if($duedate!=$oldesduedate&&$creditnoteterm=='CREDIT'){
				// 	if($creditnoteiteminfoch!=''){
				// 		$creditnoteiteminfoch.='<br> Due Date <span style="color:green;" id="prohisfromtospan">( From '.date($date,strtotime($oldesduedate)).' To '.date($date,strtotime($duedate)).' ) </span>';
				// 	}
				// 	else{
				// 		$creditnoteiteminfoch.=''.(($creditnoteinfoch!=''||$creditnotecustinfoch!='')?'<br>':'').'<span style="color:royalblue;">Item Information</span> <br> Due Date <span style="color:green;" id="prohisfromtospan">( From '.date($date,strtotime($oldesduedate)).' To '.date($date,strtotime($duedate)).' ) </span>';
				// 	}
				// }
				//FOR CREDITNOTE OTHER INFORMATIONS LIKE PAYMENT AND TERMS AND DESCRIPTION......
				$creditnotetotinfoch = $creditnoteinfoch.$creditnotecustinfoch.$creditnoteiteminfoch;
				if($creditnotetotinfoch!=''){
					$sqluse = $con->prepare("INSERT INTO pairusehistory (usetype,createdon,createdby,useid,uniqueid,useremarks) VALUES ('CREDITNOTES', ?, ?, ?, ?, ?)");
					$sqluse->bind_param("sssss", $times, $_SESSION["unqwerty"], $creditnoteno, $salesid, $creditnotetotinfoch);
					$sqluse->execute();
					$sqluse->close();
				}
				//FOR CREDITNOTE HISTORY
				$sqlusehisupdqry = $con->prepare("UPDATE pairusehistory SET uniqueid=? WHERE useid=? AND uniqueid=?");
				$sqlusehisupdqry->bind_param("sss", $salesid, $creditnoteno, $_GET['id']);
...
<main class="main-content position-relative max-height-vh-100 h-100 mt-0 " style="overflow-y: scroll !important;">
	...
	<?php
		...
		$sqlismainaccessusers=$con->prepare("SELECT moduleno, moduleprefix, modulesuffix,modulename FROM pairmainaccess WHERE franchiseid=? AND moduletype='Credit Notes' ORDER BY id ASC");
		...
		if((isset($_GET['creditnoteno']))&&(isset($_GET['creditnotedate']))){
			$creditnoteno=htmlspecialchars( $_GET['creditnoteno'], ENT_QUOTES, 'UTF-8');
			$creditnotedate=htmlspecialchars( $_GET['creditnotedate'], ENT_QUOTES, 'UTF-8');
			$sql=$con->prepare("SELECT * FROM paircreditnotes WHERE franchisesession=? AND createdid=? AND creditnoteno=? AND creditnotedate=? ORDER BY id ASC LIMIT 5");
			$sql->bind_param("ssss", $_SESSION['franchisesession'], $companymainid, $creditnoteno, $creditnotedate);
			$sql->execute();
			if ($sql->errno) {
				echo "Execution failed: (" . $sql->errno . ") " . $sql->error;
			}
			else {
				$count=1;
				...
				<div class="col-12">
					<div class="card mb-4 mt-5">
						...
						?>
								...
								<div class="modal-dialog modal-lg" role="document">
									<div class="modal-content">
										...
										<form action="" method="post" enctype="multipart/form-data" class="form-horizontal mt-0" role="form">
											...
											<div class="modal-footer mfsub" style="margin: 0px 9px !important;border-top: 1px solid #b6bcc5 !important;"></div>
										</form>
									</div>
								</div>
							</div>
							...
							<!-- CUSTOMER ADD MODAL -->
							<form autocomplete="off" id="creditnoteform" action="" onsubmit="return checkvalidate()" method="post" enctype="multipart/form-data" class="form-horizontal" role="form">
								<input type="hidden" name="invoiceno" id="invoiceno" value="<?=$rows[0]['invoiceno']?>">
								<input type="hidden" name="invoicedate" id="invoicedate" value="<?=$rows[0]['invoicedate']?>">
							...
							<?php
								$oldarray=array( 'creditnoteno','creditnotedate','reference','saleperson','preparedby','checkedby','customername','age','sex','area','city','state','district','pincode','sarea','scity','sstate','sdistrict','spincode','workphone','mobile','gstrtype','gstno','pos','dlno20','dlno21' );
							...
							?>
								...
								<input type="hidden" value="<?=$rows[0]['creditnoteterm']?>" name="oldescreditnoteterm">
								<input type="hidden" value="<?= $rows[0]['validpaidamount'] ?>" name="oldesvalidpaidamount">
								<input type="hidden" value="<?= $rows[0]['invoiceamount'] ?>" name="oldesinvoiceamount">
								<input type="hidden" value="<?= $rows[0]['invoicepaymentreceived'] ?>" name="oldesinvoicepaymentreceived">
								<input type="hidden" value="<?= $rows[0]['invoiceno'] ?>" name="oldesinvoiceno">
								<input type="hidden" value="<?= $rows[0]['refundoption'] ?>" name="oldesrefundoption">
								...
								<input type="hidden" name="oldcreditnoteno" value="<?=$rows[0]['creditnoteno']?>">
								<input type="hidden" name="oldcreditnotedate" value="<?=$rows[0]['creditnotedate']?>">
								<input type="hidden" name="creditnoteterm" value="<?=$rows[0]['creditnoteterm']?>">
								<div class="row">
								<?php
									// if ((in_array('Credit Note Information', $fieldedit))) {
								?>
									<div class="col-lg-4">
											...
											<div class="accordion-item mb-1">
												...
												<div id="collapseTwo" class="accordion-collapse collapse show" aria-labelledby="headingTwo">
													<div class="accordion-body text-sm" style="background-color:#fbfafa;" id="modulegreydesign">
														<div class="row">
															<div class="col-lg-12">
																<div class="form-group row" style="align-items: center !important;">
																	<div class="col-lg-5">
																		<label for="creditnoteno" class="custom-label">
																	...
																	<div class="col-lg-7">
																		<input type="text" class="form-control  form-control-sm" id="creditnoteno" name="creditnoteno" required  value="<?=$rows[0]['creditnoteno']?>" readonly>
															...
															<div class="col-lg-12">
																<div class="form-group row" style="align-items: center !important;">
																	<div class="col-lg-5">
																		<label for="creditnotedate" class="custom-label">
																			<span class="text-danger">
																	...
																	<?php
																		...
																		$dateformats->close();
																	?>
																		<input type="date" class="form-control  form-control-sm" id="creditnotedate" name="creditnotedate" required value="<?=$rows[0]['creditnotedate']?>">
																	</div>
																</div>
															</div>
															<div class="col-lg-12" <?=((in_array('Credit Note Time', $fieldedit))?'':'style="display:none;"')?>>
																<div class="form-group row" style="align-items: center !important;">
																	<div class="col-lg-5">
																		<label for="creditnotetime" class="custom-label">
																			<span class="text-danger">
																	...
																	<div class="col-lg-7">
																		...
																		?>
																		<input type="time" class="form-control form-control-sm" id="creditnotetime" name="creditnotetime" required value="<?=$rows[0]['creditnotetime']?>">
																	</div>
																</div>
															</div>
														<?php
														...
														?>
															<div class="col-lg-12">
																<div class="form-group row" style="align-items: center !important;">
																	...
																	<div class="col-lg-7">
																		<input type="text" class="form-control  form-control-sm" id="reference" name="reference" value="<?=$rows[0]['reference']?>" oninput="referget()" autocomplete="off">
														...
														?>
															<div class="col-lg-12" <?=((in_array('Reason', $fieldedit))?'':'style="display:none;"')?>>
																<div class="form-group row" style="align-items: center !important;">
																	<div class="col-lg-5">
																		<label for="saleperson" class="custom-label">
																			<span class="text-danger">
																	...
																	<div class="col-lg-7">
																		<select class="select2-field form-control  form-control-sm" name="reason" id="reason" <?=((in_array('Reason', $fieldedit))?'required':'')?>>
																			<option value="" selected disabled>Select</option>
																		<?php
																			$sqlireasons=$con->prepare("SELECT reason FROM pairreasons WHERE (createdid=? OR createdid='0') AND itemmodule='creditnote' ORDER BY reason ASC");
																			$sqlireasons->bind_param("s", $companymainid);
																			$sqlireasons->execute();
																			$sqlireason = $sqlireasons->get_result();
																			while($info=$sqlireason->fetch_array()){
																		?>
																			<option value="<?= $info['reason'] ?>" <?=($rows[0]['reason']==$info['reason'])?'selected':''?>>
																				<?= $info['reason'] ?>
																			</option>
																		<?php
																			}
											...
											<div class="accordion-item mb-1">
														...
														<div class="row" style="width:100% !important;">
															...
															?>
																<div class="row">
																	<div class="col-lg-2 col-md-2 col-3">
																		<div class="custom-control custom-radio mr-sm-2">
																			<input type="radio" class="custom-control-input" name="customerinfodefault" id="onecustomerinfo" value="one" <?= ($rows[0]['customerinfodefault']=='one')?'checked':'' ?> onclick="onetwo()">
																	...
																	<div class="col-lg-2 col-md-2 col-3">
																		<div class="custom-control custom-radio mr-sm-2">
																			<input type="radio" class="custom-control-input" name="customerinfodefault" id="twocustomerinfo" value="two" <?= ($rows[0]['customerinfodefault']=='two')?'checked':'' ?> onclick="onetwo()">
																...
																$(document).ready(function () {
																	...
																	<?php
																		if ($access['creditnotebtwocnamerequired']=='Yes') {
																	?>
																		$("#twocustomername").removeAttr("required");
																	<?php
																		}
																		if ($access['creditnotebtwocwphonerequired']=='Yes') {
																	?>
												...
												<div id="collapseOne" class="accordion-collapse collapse show" aria-labelledby="headingOne" style="padding-left:7px !important;padding-right: 7px !important;">
													...
													<?php
														...
														if ($infomainaccessuser['creditnotecustinfo']=='one'||(($infomainaccessuser['creditnotecustinfo']=='defone')||($infomainaccessuser['creditnotecustinfo']=='deftwo'))) {
													?>
														<div id="one">
															<div class="row">
																<div class="col-lg-12">
																	<div class="form-group row" style="align-items: center !important;">
																		...
																		<div class="col-sm-9" onclick="andus()">
																			...
																			<?php
																				$sqlis=$con->prepare("SELECT id, customername, billcity, mobile, workphone FROM paircustomers WHERE franchisesession=? AND (createdid=? AND moduletype='Customers') AND id=? ORDER BY customername ASC");
																				$sqlis->bind_param("ssi", $_SESSION["franchisesession"], $companymainid, $rows[0]['customerid']);
																				$sqlis->execute();
																				$sqli = $sqlis->get_result();
																				while($info=$sqli->fetch_array()){
																			?>
																				<option value="<?=$info['id']?>" <?=($rows[0]['customerid']==$info['id'])?'selected':''?>><?=$info['customername']?></option>
																			<?php
																				}
																				$sqlis->close();
																				$sqli->close();
																...
																<div class="col-lg-9">
																	<div class="row mb-0" id="custaddressdiv" style="background-color:#fbfafa; color:#777777; display:none;margin: 0px !important;">
																		...
																		<div class="col-lg-6" style="padding-left: 3px !important;margin-top: 0px ;<?=(in_array('GSTIN', $fieldedit))?'':'display:none;'?>" id="mphonemob">
																			...
																			<div class="col-lg-3" style="padding-left: 3px !important;<?=(in_array('DL No 20', $fieldedit))?'':'display:none;'?>">
																			...
																			<div class="col-lg-3" style="padding-left: 3px !important;<?=(in_array('DL No 21', $fieldedit))?'':'display:none;'?>">
													...
													<?php
														...
														if ($infomainaccessuser['creditnotecustinfo']=='two'||(($infomainaccessuser['creditnotecustinfo']=='defone')||($infomainaccessuser['creditnotecustinfo']=='deftwo'))) {
													?>
														<script type="text/javascript">
														function twoposans() {
														...
														<div id="two">
															<div class="row">
																<div class="<?=((!in_array('Age', $fieldedit))&&(!in_array('Sex', $fieldedit)))?'col-lg-3':'col-lg-2'?>" style="padding-right: 0px;">
																	<label for="customername" class="custom-label">
																		<?= ($access['creditnotebtwocnamerequired']=='Yes')? '<span class="text-danger">'. $infomainaccessusercus['modulename'] . ' Name * </span>': $infomainaccessusercus['modulename'] . ' Name ' ?>
																	</label>
																</div>
																<div class="<?=((in_array('Age', $fieldedit))&&(in_array('Sex', $fieldedit)))?'col-lg-4':(((in_array('Age', $fieldedit))||(in_array('Sex', $fieldedit)))?'col-lg-7':'col-lg-9')?>">
																	<input type="text" name="twocustomername" id="twocustomername" class="form-control form-control-sm" placeholder="<?= $infomainaccessusercus['modulename'] ?> Name" value="<?= $rows[0]['customername'] ?>" <?= ($access['creditnotebtwocnamerequired']=='Yes')? 'required': ' ' ?>>
																	<input type="hidden" name="oldcustomername" value="<?=$rows[0]['customerid']?>">
																</div>
																<div class="col-lg-1" style="padding-right: 0px;<?=(in_array('Age', $fieldedit))?'':'display:none;'?>">
																	<label for="customername" class="custom-label">
																		AGE
																	</label>
																</div>
																<div class="col-lg-2" style="<?=(in_array('Age', $fieldedit))?'':'display:none;'?>">
																	<input type="text" class="form-control form-control-sm" id="twoage" name="twoage" placeholder="AGE" value="<?=$rows[0]['age']?>">
																</div>
																<div class="col-lg-1" style="padding-right: 0px;<?=(in_array('Sex', $fieldedit))?'':'display:none;'?>">
																	<label for="customername" class="custom-label">
																		SEX
																	</label>
																</div>
																<div class="col-lg-2" style="<?=(in_array('Sex', $fieldedit))?'':'display:none;'?>">
																	<input type="text" class="form-control form-control-sm" id="twosex" name="twosex" placeholder="SEX" value="<?=$rows[0]['sex']?>">
																</div>
															</div>
															<div class="row mt-1">
																<div class="col-lg-12">
																		...
																		<div class="row" style="padding:0px !important;margin: 0px !important;">
																			<div class="col-lg-6" <?=(in_array('Work Phone', $fieldedit))?'':'style="display:none;"'?>>
																				<label for="twoworkphone" class="custom-label" style="margin:0px !important;font-size: 12px !important;color: #777777;">
																					<?= ($access['creditnotebtwocwphonerequired']=='Yes')? '<span class="text-danger"> WORK PHONE * </span>': ' WORK PHONE ' ?>
																				</label>
																				<input type="text" class="form-control form-control-sm" id="twoworkphone" name="twoworkphone" placeholder="Work Phone" value="<?=$rows[0]['workphone']?>" <?= ($access['creditnotebtwocwphonerequired']=='Yes')? 'required': ' ' ?>>
													...
													<div class="table-responsive" style="overflow-y: hidden;">
														<table class="table table-bordered" id="purchasetable">
															<thead>
																<tr>
																	...
																	<th <?=((in_array('Item Details', $fieldedit))||($access['batchexpiryval']==1)||(in_array('Rate', $fieldedit))||(in_array('Quantity', $fieldedit))||(in_array('Taxable Value', $fieldedit))||(in_array('Tax Value', $fieldedit))||(in_array('Amount', $fieldedit)))?'':'style="display:none !important;"'?>>
																	</th>
																<?php
																...
																?>
																	<th width="13%">
																		BATCH <?=(($access['creditnotebatexprequired']=='Yes')?'<span class="text-danger"> *</span>':'')?>
																	</th>
																<?php
																	}
																?>
																...
																?>
																	<th style="width: 87px !important;text-align: right !important;">
																		<?=($access['txtqtycreditnote'])?><span class="text-danger"> *</span>
																	</th>
																<?php
																	}
																?>
														...
														?>
															...
															?>
																...
																<input type="hidden" name="oldmarginupdates[]" id="oldformarginupdates<?=$i?>" value="<?=$row['marginupdates']?>">
																<input type="hidden" name="oldmargintotalvalue[]" id="oldmargintotalvalue<?=$i?>" value="<?=$row['margintotalvalue']?>">
																<tr>
																	<td class="priority" style="display:none">
																		<?=$i?>
																	</td>
																	<td class="tdmove" <?=((in_array('Item Details', $fieldedit))||($access['batchexpiryval']==1)||(in_array('Rate', $fieldedit))||(in_array('Quantity', $fieldedit))||(in_array('Taxable Value', $fieldedit))||(in_array('Tax Value', $fieldedit))||(in_array('Amount', $fieldedit)))?'':'style="display:none !important;"'?>>
																	...
																	<td data-label="BATCH" <?=($access['batchexpiryval']==1)?'':'style="display:none;"'?>>
																		<div>
																			<input type="text" <?= ($access['creditnotebatchdef']=='availdrop')?'readonly':'' ?> <?=(($access['creditnotebatexprequired']=='Yes')?'required':'')?> name="batch[]" maxlength="100" id="batch<?=$i?>" onClick="batchget(<?=$i?>);" onFocus="batchget(<?=$i?>);"  class="form-control form-control-sm proitemselect productselectwidth" style="margin-bottom: 3px !important;padding: 0px !important;" value="<?=$row['batch']?>" autocomplete="off">
																		</div>
																		<div>
																		<?php
																			$dateformats=$con->prepare("SELECT * FROM paricountry");
																		...
																		?>
																			...
																			<input type="date" <?= ($access['creditnotebatchdef']=='availdrop')?'readonly':'' ?> <?=(($access['creditnotebatexprequired']=='Yes')?'required':'')?> name="expdate[]" id="expdate<?=$i?>" class="form-control form-control-sm proitemselect" style="display:none;width: 94px !important;padding: 0px !important;height: 18px !important;" value="<?=$row['expdate']?>" onChange="productcalc(<?=$i?>)">
																	...
																	<td data-label="RATE" <?=(in_array('Rate', $fieldedit))?'':'style="display:none !important;"'?>>
																		...
																		<div>
																			<div class="dvi creditnotebillgets" id="creditnotebillgets<?=$i?>" style="margin-top:-22px;display: none;width: 250px;border-radius: 0px !important;">
																				<table width="100%">
																					<tr style="border-bottom: 1px solid #cccccc;margin-bottom: 0px;">
																						<td align="center" style="border-right: 1px solid #cccccc;width: 50% !important;display: inline-block !important;text-align: center;">
																							<span onclick="creditnotegetfun(<?=$i?>,0)" id="creditnoteonoff<?=$i?>">
																						...
																						<input type="text" style="display: none;" value="creditnote" id="billorcreditnote<?=$i?>">
																					</tr>
																				</table>
																			</div>
																	...
																	<td data-label="<?=($access['txtqtycreditnote'])?>" <?=(in_array('Quantity', $fieldedit))?'':'style="display:none !important;"'?>>
																		<div>
																	...
																	<td data-label="<?=$access['txttaxablecreditnote']?>" <?=(in_array('Taxable Value', $fieldedit))?'':'style="display:none;"'?>>
																		<div>
																			<span style="font-size:15px !important;">
																				<?php echo $resmaincurrencyans; ?>
																			</span>
																		...
																		<div style="<?=(in_array('Profit Margin', $fieldedit))?'':'display:none;'?>">
																			<span data-bs-toggle="modal" data-bs-target="#ViewMarginDetails" style="color: green;font-size:13px;" id="margintotal<?=$i?>" onclick="getmargindetails(<?=$i?>)">
																				Profit Margin :
																				<span id="totalmargcreditnotealue<?=$i?>">
																	...
																	<td <?=((in_array('Item Details', $fieldedit))||($access['batchexpiryval']==1)||(in_array('Rate', $fieldedit))||(in_array('Quantity', $fieldedit))||(in_array('Taxable Value', $fieldedit))||(in_array('Tax Value', $fieldedit))||(in_array('Amount', $fieldedit)))?'style="white-space:nowrap !important;"':'style="display:none !important;"'?>>
																		<div class="app-utility-item app-user-dropdown dropdown" style="margin-right: 0px !important; <?=(in_array('Additional Informations', $fieldedit))?'display:none !important;':'display:none !important;'?>">
																			<a href="javascript:;" class="p-0" id="dropdownadditionalinfo" data-bs-toggle="dropdown" aria-expanded="false">
																				<svg width="15" height="15" viewBox="0 0 512 512" xmlns="http://www.w3.org/2000/svg" class="icon icon-sm text-blue"><path d="M162 224c17.7 0 32 14.3 32 32s-14.3 32-32 32-32-14.3-32-32 14.3-32 32-32M256 224c17.7 0 32 14.3 32 32s-14.3 32-32 32-32-14.3-32-32 14.3-32 32-32M350 224c17.7 0 32 14.3 32 32s-14.3 32-32 32-32-14.3-32-32 14.3-32 32-32" id="Path"></path><path d="M256 480c123.712 0 224-100.288 224-224S379.712 32 256 32 32 132.288 32 256s100.288 224 224 224zm0 32C114.615 512 0 397.385 0 256S114.615 0 256 0s256 114.615 256 256-114.615 256-256 256z" id="Oval-1"></path></svg>
																			</a>
															...
															?>
																...
																setTimeout(function() {
																	for(i=0;i<document.getElementsByClassName("oldaccnames").length;i++){
																		...
																		getoldmargins(lineNo);
																	}
																	function getoldmargins(lineidno) {
																		$("#totalmargcreditnotealue"+lineidno).html($("#oldmargintotalvalue"+lineidno).val());
																	}
																},9000);
																setTimeout(function() {
																	...
																	// <?php
																	// 	if ($access['creditnoteratedef']=='avail') {
																	// ?>
																	// 	var creditnoteratedef = 'and quantity>0 ';
																	// <?php
																	// 	}
																	// 	else{
																	// ?>
																	// 	var creditnoteratedef = '';
																	// <?php
																	// 	}
																	// ?>
																	// 	$.get("ratesearch.php", {term: productid,custid: customerid,ratedef: creditnoteratedef,differ: 'creditnote'} , function(datas){
																	...
																	// 				// console.log("key"+key);
																	// 				check+="<div id='option"+lineNo+objbatch[key].creditnoteno+"creditnote' style='border:1px solid #cccccc;border-top: none !important;'><table width='100%' style='table-layout:fixed;'><tr style='border-bottom:none;margin-bottom:0px;'><td align='left' style='border:none;overflow:hidden;white-space:nowrap;' title='<?=strtoupper($infomainaccessuser['modulename'])?>'><span style='background-color:#1BBC9B;color:white;padding:2px 3px; border-radius:5px; text-transform: uppercase;' class='creditnotebilltxt'><?=strtoupper($infomainaccessuser['modulename'])?></span></td><td align='right' id='creditnoteno"+objbatch[key].creditnoteno+"creditnote' style='border:none;overflow:hidden;white-space:nowrap;' title='"+objbatch[key].creditnoteno+"'>  "+objbatch[key].creditnoteno+" </td><td align='right' id='creditnotedt"+objbatch[key].creditnoteno+"creditnote' style='border:none;overflow:hidden;white-space:nowrap;' title='"+objbatch[key].creditnotedate+"'>"+objbatch[key].creditnotedate+" </td></tr><tr style='border-bottom:none;border-top:none;margin-bottom:0px;'><td align='left' colspan='2' id='rate"+objbatch[key].productrate+"creditnote' style='border:none;overflow:hidden;white-space:nowrap;' title='"+objbatch[key].productrate+"'>Rate : "+objbatch[key].productrate+" </td><td align='right' id='dis"+objbatch[key].productdiscount+"creditnote' style='border:none;overflow:hidden;white-space:nowrap;' title='"+objbatch[key].productdiscount+"'><?=$access['txtprodiscreditnote']?> : "+objbatch[key].productdiscount+" </td></tr></table></div>";
																	// 			}
																	// 			// console.log(check);
																	...
																	// 	var productid= $('#product'+lineNo).val();
																	// <?php
																	// 	if ($access['creditnotebatchdef']=='avail') {
																	// ?>
																	// 	var creditnotebatchdef = 'and quantity>0 ';
																	// <?php
																	// 	}
																	// 	else{
																	// ?>
																	// 	var creditnotebatchdef = '';
																	// <?php
																	// 	}
																	// ?>
																	// 	$.get("batchsearch.php", {term: productid,batchdef: creditnotebatchdef} , function(datas){
																	// 		// console.log("normal"+datas);
												...
												<div class="table-responsive mt-1" id="gsttablediv" <?=(in_array('Tax Informations', $fieldedit))?'':'style="display:none;"'?>>
													<table class="table table-bordered" id="gsttable" style="font-size:12px;">
														...
														<?php
															...
															$sqlitaxes=$con->prepare("SELECT * FROM paircreditnotes WHERE franchisesession=? AND createdid=? AND creditnoteno=? AND creditnotedate=? ORDER BY id ASC");
															$sqlitaxes->bind_param("ssss", $_SESSION["franchisesession"], $companymainid, $creditnoteno, $creditnotedate);
															$sqlitaxes->execute();
								...
								<div class="app-header-inner">
									<div class="py-2 px-3">
										<div class="app-header-content" style="margin-left: -45px;" id="ilu">
											<div class="row">
												<div class="col-auto pt-1" style="width:34px !important;margin-top: -8px !important;height: 100px;margin-left: -18px !important;border-top: 1px solid #eff0f4;"></div>
												<div class="col" style="width:34px !important;margin-top: 1px !important;margin-left: -9px !important;">
													<div class="form-group row">
														<div class="col-7 col-sm-6">
															...
															<a class="btn btn-primary btn-sm btn-custom-grey" href="creditnotes.php" style="margin-top:-3px !important;">
																Cancel
															</a>
														</div>
										...
										<div class="modal-body" style="height: max-content;">
											...
											<table class="table notforfinaloutputsave showhideload">
												<tr>
													...
													<td>
														<input type="text" name="validcreditnoteno" id="validcreditnoteno" class="form-control form-control-sm finalsubmitrequired" readonly required>
													</td>
												</tr>
												<tr>
													<th style="text-align:left !important">
													...
													<td>
														<input type="date" name="validcreditnotedate" id="validcreditnotedate" class="form-control form-control-sm finalsubmitrequired" readonly required>
													</td>
												</tr>
												<tr>
													<th style="text-align:left !important">
													...
													<td>
														<input type="number" name="validcreditnoteamount" id="validcreditnoteamount" class="form-control form-control-sm finalsubmitrequired" readonly required>
													</td>
												</tr>
												<tr style="<?=(($rows[0]['invoiceno']!='')?'':'display: none;')?>">
													<th style="text-align:left !important;background-color: #e9ecef;">
														<label class="custom-label">
															<?= $infomainaccessuserinv['modulename'] ?> Number
														</label>
													</th>
													<td style="background-color: #e9ecef;">
														<input type="text" name="validcreditnoteinvno" id="validcreditnoteinvno" class="form-control form-control-sm finalsubmitrequired" readonly required value="<?=$rows[0]['invoiceno']?>">
													</td>
												</tr>
												<tr style="<?=(($rows[0]['invoiceno']!='')?'':'display: none;')?>">
													<th style="text-align:left !important;background-color: #e9ecef;">
														<label class="custom-label">
															<?= $infomainaccessuserinv['modulename'] ?> Amount
														</label>
													</th>
													<td style="background-color: #e9ecef;">
														<input type="number" name="invoiceamount" id="invoiceamount" class="form-control form-control-sm finalsubmitrequired" readonly required value="<?=$rows[0]['invoiceamount']?>">
													</td>
												</tr>
												<?php
													$paidamount=0;
													...
													$sqlsalespays->bind_param("ssss", $_SESSION['franchisesession'], $companymainid, $rows[0]['invoiceno'], $rows[0]['invoicedate']);
													$sqlsalespays->execute();
													$sqlsalespay = $sqlsalespays->get_result();
													while($infosalespay=$sqlsalespay->fetch_array()){
														$paidamount+=(float)$infosalespay['amount'];
													}
													$currentbalance=((float)$rows[0]['invoiceamount']-$paidamount);
			      								...
			      								$sqlsantss->bind_param("sssss", $_SESSION['franchisesession'], $companymainid, $rows[0]['invoiceno'], $rows[0]['invoicedate'], $rows[0]['customerid']);
			      								$sqlsantss->execute();
												...
												<tr style="<?=(($rows[0]['invoiceno']!='')?'':'display: none;')?>">
													<th style="text-align:left !important;background-color: #e9ecef;">
														<label class="custom-label">
															<?= $infomainaccessuserinv['modulename'] ?> Payment Received
														</label>
													</th>
													<td style="background-color: #e9ecef;">
														<input type="number" name="invoicepaymentreceived" id="invoicepaymentreceived" class="form-control form-control-sm finalsubmitrequired" readonly required value="<?=$paidamount - $paidamountcr?>">
													</td>
												</tr>
												<tr>
													<th style="text-align:left !important">
													...
													<td>
														<div class="col-lg-12">
															<select class="select4 form-control  form-control-sm finalsubmitrequired" name="refundoption" maxlength="100" id="refundoption" required onchange="checktherefund()">
																...
																?>
																<option value="refundnow" <?=($rows[0]['refundoption']=='refundnow')?'selected':''?>>
																	Refund Now
																</option>
																<option value="refundlater" <?=(($rows[0]['refundoption']=='refundlater')||($rows[0]['refundoption']=='norefund'))?'selected':''?>>
																	Refund Later
																</option>
																<?php
																	}
												...
												<tr style="<?=(($access['defcreditnotepay']=='split')?'display: none;':'display: none;')?>">
													<th style="text-align:left !important;background-color: #e9ecef;">
														<label class="custom-label">
															Cash Received
														</label>
													...
													<td style="background-color: #e9ecef;">
														<input type="number" name="cashreceived" id="cashreceived" oninput="splitchecking()" class="form-control form-control-sm finalsubmitrequired" required step="any" value="<?=$rows[0]['cashreceived']?>">
													</td>
												</tr>
												<tr style="<?=(($access['defcreditnotepay']=='split')?'display: none;':'display: none;')?>">
													<th style="text-align:left !important;background-color: #e9ecef;">
													...
													<td style="background-color: #e9ecef;">
														<input type="number" name="cardreceived" id="cardreceived" oninput="splitchecking()" class="form-control form-control-sm finalsubmitrequired" required step="any" value="<?=$rows[0]['cardreceived']?>">
													</td>
												</tr>
												<tr style="<?=(($access['defcreditnotepay']=='split')?'display: none;':'display: none;')?>">
													<th style="text-align:left !important;background-color: #e9ecef;">
													...
													<td style="background-color: #e9ecef;">
														<input type="number" name="gpayreceived" id="gpayreceived" oninput="splitchecking()" class="form-control form-control-sm finalsubmitrequired" required step="any" value="<?=$rows[0]['gpayreceived']?>">
													</td>
												</tr>
												<tr style="<?=(($access['defcreditnotepay']=='split')?'display: none;':'')?>" id="creditnotetermblock">
													...
													<td>
														<div class="col-lg-12">
															<select class="select2-field form-control  form-control-sm finalsubmitrequired" name="creditnoteterm" maxlength="100" id="creditnoteterm" required onchange="ihaveblock()">
															<?php
																...
																while($info=$sqli->fetch_array()){
																	if (($info['term']=='CASH')){
															?>
																<option value="<?= $info['term'] ?>" <?=($info['term']==$rows[0]['creditnoteterm'])?'selected':''?>>
																	<?= $info['term'] ?>
																</option>
															<?php
																	}
												...
												<tr id="validpaidblock">
													...
													<td>
														<input type="number" name="validpaidamount" id="validpaidamount" class="form-control form-control-sm finalsubmitrequired" placeholder="0.00" required onchange="paidamountcalc()" value="<?= $rows[0]['validpaidamount'] ?>" step="any" <?=(($rows[0]['invoiceno']!='')?'':'readonly')?> <?=(($rows[0]['refundoption']=='norefund')||(($paidamount - $paidamountcr)<=0))?'readonly':''?>>
														<script>
														function paidamountcalc(){
															var validcreditnoteamount=$("#validcreditnoteamount").val();
															var validpaidamount=$("#validpaidamount").val();
														<?php
															if($rows[0]['invoiceno']==''){
														?>
															$("#validpaidamount").val(validcreditnoteamount);
														<?php
															}
														?>
															if((validcreditnoteamount!='')&&(validpaidamount!='')){
																var balance=parseFloat(validcreditnoteamount)-parseFloat(validpaidamount);
												...
												<tr id="duedateblock">
													...
													<td>
														...
														function splitchecking() {
															...
															if (($("#validcreditnoteamount").val()) == (parseFloat($("#cashreceived").val()) + parseFloat($("#cardreceived").val()) + parseFloat($("#gpayreceived").val()))) {
														...
														function funduedates(){
															...
															if(duedates!=''){
																...
																today=addDays(creditnotedate, duedates);
											...
											<script type="text/javascript">
											var ihaveterm = document.getElementById("creditnoteterm").value;
											...
											function checktherefund() {
												if(document.getElementById("refundoption").value=='refundnow'){
													document.getElementById("creditnoteterm").value = 'CASH';
													$("#creditnoteterm").change();
													ihavereceive();
													document.getElementById("creditnotetermblock").style.display='table-row';
													$('select[name^="creditnoteterm"] option[value="CASH"]').attr("selected","selected");
													$('#creditnoteterm').val('CASH').change();
													$('select[name^="creditnoteterm"] option[value="CREDIT"]').remove();
												}
												else if(document.getElementById("refundoption").value=='refundlater'){
													$('#creditnoteterm').append('<option value="CREDIT">CREDIT</option>');
													$('select[name^="creditnoteterm"] option[value="CREDIT"]').attr("selected","selected");
													$('#creditnoteterm').val('CREDIT').change();
													document.getElementById("duedateblock").style.display='none';
													document.getElementById("creditnotetermblock").style.display='none';
													document.getElementById("validpaidblock").style.display='none';
												}
											}
...
$("#customer").on("select2:open", function() {
<?php
	if($access['creditnotenewcustomerdef']=='1'){
?>
	$("#configureunits").show();
...
$(".product1").on("select2:open", function() {
<?php
	if($access['creditnotenewproductdef']=='1'){
?>
	$("#configureunits").show();
	...
	if(id != ''){
		$.get("prosearch1.php", {term: id} , function(data){
			...
			if (proalreadychnxtlvl==0) {
				...
				<?php
					if($access['creditnotebatexprequired']=='Yes'){
				?>
				...
				<?php
					...
					if($access['creditnotebatchdef']=='availdrop'){
				?>
	...
	marginprodiscounttype = $("#prodiscounttype"+id+"").val();
		$.ajax({
			...
			success: function (result) {
				...
				$("#totalmargcreditnotealue"+id+"").html(resultdata.margintotal);
...
<!-- Start New Reason -->
<div class="modal fade" id="AddNewCreditNoteTerm" tabindex="-1" role="dialog" style="z-index: 1051;">
	<div class="modal-dialog" role="document">
		<div class="modal-content">
			<div class="modal-header">
				<h5 class="modal-title">
				...
				<span type="button" onclick="funescreditnoteterm()" class="close" data-dismiss="modal" aria-label="Close">
					<span aria-hidden="true" id="closeicon">&times;</span>
				</span>
			</div>
			<div class="modal-body">
				<form method="post" action="">
					<div class="row justify-content-center">
						<div class="col-lg-12">
							<div class="form-group row">
								<div class="col-sm-6">
									<label for="missingcreditnoteterm" class="custom-label">
										<span class="text-danger">
								...
								<div class="col-sm-6">
									<input type="text" name="creditnoteterm" class="form-control form-control-sm mb-4" id="missingcreditnoteterm" placeholder="Name" required>
								</div>
							</div>
						</div>
					</div>
			...
			<div class="modal-footer " style="margin-top: 10px !important;">
				<div class="col">
					<button onclick="funaddcreditnoteterm()" class="btn btn-primary btn-sm btn-custom arlina-button expand-left"   type="submit"  name="submitcreditnoteterm" value="Submit">
						<span class="label">Save</span>
						<span class="spinner"></span>
					</button>
					<button type="button" class="btn btn-primary btn-sm btn-custom-grey" onclick="funescreditnoteterm()">
						Cancel
					</button>
				</div>
			</div>
...
<script>
function funaddcreditnoteterm() {
	var missingcreditnoteterm = document.getElementById('missingcreditnoteterm');
	if (missingcreditnoteterm.value == '') {
		alert('Please Enter New <?= $infomainaccessuser['modulename'] ?> Term Name');
		missingcreditnoteterm.focus();
		return false;
	}
	else {
		$.ajax({
			...
			url: "termadds.php",
			data: {
				term: $("#missingcreditnoteterm").val(),
				submit: "Submit"
			},
			success:function(result){
				const resarray = result.split("|");
				...
				if(resarray[1]=='0'){}
					...
					$('#creditnoteterm').append('<option value="' + missingcreditnoteterm.value + '">' + missingcreditnoteterm.value +'</option>');
					$('#creditnoteterm').val(missingcreditnoteterm.value).change();
					$("#creditnoteterm").select2();
					$('#AddNewCreditNoteTerm').modal('hide');
					return false;
				}
			}
		});
...
$("#creditnoteterm").on("select2:open", function() {
	$("#configureunits").attr("data-bs-target","#AddNewCreditNoteTerm");
});
$("#creditnoteterm").on("select2:open", function() {
...
// FOR GST TYPE
function triggerpayment(creditnoteno,creditnotedate,creditnoteamount,cancelstatus,whatdonext){
	let allAreFilled = true;
	document.getElementById("creditnoteform").querySelectorAll("[required]").forEach(function(i) {
		if (!allAreFilled) return;
		if (i.type === "radio") {
			let radioValueCheck = false;
			document.getElementById("creditnoteform").querySelectorAll(`[name=${i.name}]`).forEach(function(r) {
	...
	setTimeout(function() {
		...
		if (!allAreFilled) {
			...
			if(accessallow){
				...
				isValidFinal = true;
				checkvalidate();
				$('#validcreditnoteno').val($('#creditnoteno').val());
				$('#validcreditnotedate').val($('#creditnotedate').val());
				$('#validcreditnoteamount').val($('#grandtotal').val());
				var validcreditnoteamount=$("#validcreditnoteamount").val();
			<?php
				if($rows[0]['invoiceno']==''){
			?>
			...
			?>
				if(parseFloat($("#invoiceamount").val()) - parseFloat($("#invoicepaymentreceived").val()) - parseFloat($("#validcreditnoteamount").val())<0){
					$("#validpaidamount").val(Math.abs(parseFloat($("#invoiceamount").val()) - parseFloat($("#invoicepaymentreceived").val()) - parseFloat($("#validcreditnoteamount").val())));
				}
				else{
...
function creditnotegetfun(lineNo,boolean) {
	$("#creditnoteonoff"+lineNo).css("background-color","#1BBC9B");
	$("#creditnoteonoff"+lineNo).css("color","#fff");
	$("#billonoff"+lineNo).css("background-color","#fff");
	$("#billonoff"+lineNo).css("color","#000");
	$("#billorcreditnote"+lineNo).val('creditnote');
	var productid= $('#product'+lineNo).val();
	var customerid = $("#customer").val();
<?php
	if ($access['creditnoteratedef']=='avail') {
...
// FOR GET OLD RATE OF THE CUSTOMER AND PRODUCT IN CREDITNOTE
function billgetfun(lineNo) {
	$("#creditnoteonoff"+lineNo).css("background-color","#fff");
	$("#creditnoteonoff"+lineNo).css("color","#000");
	$("#billonoff"+lineNo).css("background-color","#1BBC9B");
	$("#billonoff"+lineNo).css("color","#fff");
	$("#billorcreditnote"+lineNo).val('bill');
	var productid= $('#product'+lineNo).val();
	var customerid = $("#customer").val();
<?php
	if ($access['creditnoteratedef']=='avail') {
...
?>
	$.get("ratesearch.php", {term: productid,custid: customerid,ratedef: creditnoteratedef,differ: 'bill'} , function(datas){
		// console.log("normal"+datas);
		$("#errrate"+lineNo).val(datas);
		var letters = "<br /><b>Warning</b>:  Undefined variable $ratedatas in";
		brrch = document.getElementById("errrate"+lineNo);
		if (brrch.value.includes(letters)||brrch.value=='') {
			...
			for (var key in objbatch) {
				// console.log("key"+key);
				check+="<div id='option"+lineNo+objbatch[key].creditnoteno+"creditnote' style='border:1px solid #cccccc;border-top: none !important;'><table width='100%' style='table-layout:fixed;'><tr style='border-bottom:none;margin-bottom:0px;'><td align='left' style='border:none;overflow:hidden;white-space:nowrap;' title='<?=strtoupper($infomainaccessuser['modulename'])?>'><span style='background-color:#1BBC9B;color:white;padding:2px 3px; border-radius:5px; text-transform: uppercase;' class='creditnotebilltxt'><?=strtoupper($infomainaccessuserbill['modulename'])?></span></td><td align='right' id='creditnoteno"+objbatch[key].creditnoteno+"creditnote' style='border:none;overflow:hidden;white-space:nowrap;' title='"+objbatch[key].creditnoteno+"'>  "+objbatch[key].creditnoteno+" </td><td align='right' id='creditnotedt"+objbatch[key].creditnoteno+"creditnote' style='border:none;overflow:hidden;white-space:nowrap;' title='"+objbatch[key].creditnotedate+"'>"+objbatch[key].creditnotedate+" </td></tr><tr style='border-bottom:none;border-top:none;margin-bottom:0px;'><td align='left' colspan='2' id='rate"+objbatch[key].productrate+"creditnote' style='border:none;overflow:hidden;white-space:nowrap;' title='"+objbatch[key].productrate+"'>Rate : "+objbatch[key].productrate+" </td><td align='right' id='dis"+objbatch[key].productdiscount+"creditnote' style='border:none;overflow:hidden;white-space:nowrap;' title='"+objbatch[key].productdiscount+"'><?=$access['txtprodiscreditnote']?> : "+objbatch[key].productdiscount+" </td></tr></table></div>";
			}
			...
			browsers.style.border = 'none';
			$("#productrate"+lineNo).focus();
			for(i=0;i<document.getElementsByClassName("creditnotebilltxt").length;i++){
				document.getElementsByClassName("creditnotebilltxt")[i].innerHTML = "<?=strtoupper($infomainaccessuserbill['modulename'])?>";
			}
		}
	});
}
...
function batchget(id){
	if($("#productid"+id).val()==''){
		...
		var browserscreditnotebill = document.getElementById("creditnotebillgets"+id);
		browsersrate.style.display = 'none';
		browserscreditnotebill.style.display = 'none';
	...
	<?php
		if (($access['creditnotebatchdef']=='avail')||($access['creditnotebatchdef']=='availdrop')) {
	?>
		var creditnotebatchdef = 'and quantity>0 ';
	<?php
		}
...
function rateget(id){
	if($("#productid"+id).val()==''){
		...
		if (firstornonerate) {
			...
			if (document.getElementById("billorcreditnote"+id).value=='creditnote') {
				creditnotegetfun(id,0);
			}
			else{
				billgetfun(id);
			}
		...
		var browserscreditnotebill = document.getElementById("creditnotebillgets"+id);
		browsers.style.display = 'block';
		browserscreditnotebill.style.display = 'block';
		var productid= $('#product'+id).val();
		var customerid = $("#customer").val();
		input.onclick = function () {
			browsers.style.display = 'block';
	...
	?>
		if (document.getElementById("billorcreditnote"+id).value=='creditnote') {
			var creditnoteorbills = 'creditnote';
		}
		else{
			var creditnoteorbills = 'bill';
		}
		$.get("ratesearch.php", {term: productid,custid: customerid,ratedef: creditnoteratedef,differ: creditnoteorbills} , function(datas){
			const objbatch = JSON.parse(datas);
			option='';
			creditnoteno='';
			creditnotedt='';
			rate='';
			for (var key in objbatch) {
				option+='option'+id+objbatch[key].creditnoteno+'creditnote;';
				creditnoteno+='creditnoteno'+objbatch[key].creditnoteno+'creditnote;';
				creditnotedt+='creditnotedt'+objbatch[key].creditnotedate+'creditnote;';
				rate+='rate'+objbatch[key].productrate+'creditnote;';
			}
			optionspl = option.split(';');
			creditnotenospl = creditnoteno.split(';');
			creditnotedtspl = creditnotedt.split(';');
			ratespl = rate.split(';');
			for (var i = 0; i <= optionspl.length; i++) {
				var optionans = document.getElementById(optionspl[i]);
				$("#"+optionspl[i]).on("click",function() {
					...
					const childone = child[0].children;
					const childtwo = childone[0].children;
					var creditnotenocreditnotedt = childtwo[0].innerHTML;
					var creditnotenocreditnotedtspl = creditnotenocreditnotedt.split(" ");
...
setTimeout(function() {
	$.get("refersearch.php", {term: 'creditnoteref'} , function(datas){
...
$.ajax({
type: "GET",
url: 'editfetch.php?term=' + sIndexEdit + '&limitings=5&creditnoteno=<?=$_GET['creditnoteno']?>&creditnotedate=<?=$_GET['creditnotedate']?>&type=creditnote&i='+ivalue+'',
success: function (result) {
...
<?php
		...
		header("Location:creditnotes.php?error=No Information Found");
	}
}
else{
	header("Location:creditnotes.php?error=No Information Found");  
