...
if (isset($_POST['submitcustomizes'])) {
	...
	$sqlreportmoduless=$con->prepare("SELECT * FROM pairreportmodules WHERE types='cust' ORDER BY id ASC");
	...
	$sqlupreport = $con->prepare("UPDATE pairreports SET rowcolumns=? WHERE franchiseid=? AND createdid=? AND types='cust'");
	...
	$sqlreportfav = $con->prepare("UPDATE pairreportfavourites SET reporturl=? WHERE franchisesession=? AND createdid=? AND reportnames='cust'");
	$sqlreportfav->bind_param("sss", $newReportUrl, $_SESSION['franchisesession'], $companymainid);
	$sqlreportfav->execute();
	$sqlreportfav->close();

	$sqlreport = $con->prepare("UPDATE pairreports SET reportperiod=?,filtername=?,companyname=?,dateprepare=?,timeprepare=?,gstrule=? WHERE franchiseid=? AND createdid=? AND types='cust'");
	...
	header("Location:reportcustbalance.php?datefrom=".date($dateformatforurlredirection,strtotime($from))."&dateto=".date($dateformatforurlredirection,strtotime($to))."");
}

$sqlreportviews=$con->prepare("SELECT * FROM pairreports WHERE franchiseid=? AND createdid=? AND types='cust'");
								...
								<form action="" method="post" enctype="multipart/form-data" class="form-horizontal mt-0" role="form">
									<div class="row">
																			...
																			<div id="customizerowcolumns" class="accordion-collapse collapse show" aria-labelledby="customizerowcolumn">
																				<div class="accordion-body text-sm">
																					<div class="row justify-content-center">
																						...
																						<?php
																							$newans=array();
																							$sqlreportaccesss=$con->prepare("SELECT * FROM pairreports WHERE franchiseid=? AND createdid=? AND types='cust'");
																								...
																								<div class="col-lg-6 my-1" style="padding-left: 0px !important;">
																									<div class="custom-control custom-checkbox mr-sm-2" style="padding: 0px 12px !important;">
																										<div class="input-group input-group-sm" id="flaghead">
																											...
																											<input type="checkbox" class="custom-control-input" name="<?= $coltypemod; ?>cust" id="<?= $coltypemod; ?>cust" checked disabled>
																											<label class="custom-control-label custom-label" for="<?= $coltypemod; ?>cust">
											...
											<div class="tab-pane fade" id="nav-profile" role="tabpanel" aria-labelledby="nav-profile-tab">
												<div class="table-responsive m-3" id="histables">
													<table class="table table-bordered">
														...
														<tbody>
														<?php
															$sqluse=$con->prepare("SELECT * FROM pairusehistory WHERE usetype = 'REPcust' AND useid = ? ORDER BY createdon DESC");
																		...
																		<tr style="height:1px;">
																			<td width="100%" style="padding:10px;">
																				<table id="print-are1" style="border: 1px solid #eee;" width="100%">
																					<thead>
																						<tr>
																							<td class="text-uppercase" style="width:64%;border:1px solid #eee;padding-left: 10px;">
																					...
																					<tbody id="myTable">
																					<?php
																						$sqliinvoicefrts=$con->prepare("SELECT * FROM pairinvoices WHERE franchisesession=? AND createdid=? AND (invoicedate>=? AND invoicedate<=?) GROUP BY customerid LIMIT 36");
																						$sqliinvoicefrts->bind_param("ssss", $_SESSION['franchisesession'], $companymainid, $datefrom, $dateto);
																						$sqliinvoicefrts->execute();
																						$sqliinvoicefrt = $sqliinvoicefrts->get_result();
																						while($infoinvoicefrt=$sqliinvoicefrt->fetch_array()){
																							$sqliinvoices=$con->prepare("SELECT customername, customerid,invoiceamount, invoicedate, invoiceno FROM pairinvoices WHERE franchisesession=? AND createdid=? AND customerid=? GROUP BY invoicedate, invoiceno ORDER BY invoicedate DESC, invoiceno DESC");
																							$sqliinvoices->bind_param("sss", $_SESSION['franchisesession'], $companymainid, $infoinvoicefrt['customerid']);
																							...
																							while($infoinvoice=$sqliinvoice->fetch_array()){
																								...
																								$paidamount=0;
																								$sqlsalespays=$con->prepare("SELECT amount FROM pairsalespayhistory WHERE franchisesession=? AND createdid=? AND customerid=? AND invoiceno=? AND invoicedate=? ORDER BY id DESC");
																								$sqlsalespays->bind_param("sssss", $_SESSION['franchisesession'], $companymainid, $customerid, $infoinvoice['invoiceno'], $infoinvoice['invoicedate']);
																					...
																					?>
																						<tr style="height: 30px;" onclick="window.open('reportcustdetails.php?datefrom=<?=$datefrom?>&dateto=<?=$dateto?>&customerid=<?=$customerid?>','_self')">
																					...
																					<?php
																						...
																						if (mysqli_num_rows($sqliinvoice)>0) {
																							$sqliinvoicefrtfortots=$con->prepare("SELECT * FROM pairinvoices WHERE franchisesession=? AND createdid=? AND (invoicedate>=? AND invoicedate<=?) GROUP BY customerid");
																							$sqliinvoicefrtfortots->bind_param("ssss", $_SESSION['franchisesession'], $companymainid, $datefrom, $dateto);
																							$sqliinvoicefrtfortots->execute();
																							$sqliinvoicefrtfortot = $sqliinvoicefrtfortots->get_result();
																							$crb=0;
																							$int=0;
																							while($infoinvoicefrtfortot=$sqliinvoicefrtfortot->fetch_array()){
																								$sqliinvoicefortots=$con->prepare("SELECT customername, customerid,invoiceamount, invoicedate, invoiceno FROM pairinvoices WHERE franchisesession=? AND createdid=? AND customerid=? GROUP BY invoicedate, invoiceno ORDER BY invoicedate DESC, invoiceno DESC");
																								$sqliinvoicefortots->bind_param("sss", $_SESSION['franchisesession'], $companymainid, $infoinvoicefrtfortot['customerid']);
																								$sqliinvoicefortots->execute();
																								$sqliinvoicefortot = $sqliinvoicefortots->get_result();
																								$invoiceamountfortot=0;
																								$balanceamountfortot=0;
																								while($infoinvoicefortot=$sqliinvoicefortot->fetch_array()){
																									$customerid = $infoinvoicefortot['customerid'];
																									$customername = $infoinvoicefortot['customername'];
																									$invoiceamountfortot+=(float)$infoinvoicefortot['invoiceamount'];
																									$paidamountfortot=0;
																									$sqlsalespayfortots=$con->prepare("SELECT amount FROM pairsalespayhistory WHERE franchisesession=? AND createdid=? AND customerid=? AND invoiceno=? AND invoicedate=? ORDER BY id DESC");
																									$sqlsalespayfortots->bind_param("sssss", $_SESSION['franchisesession'], $companymainid, $customerid, $infoinvoicefortot['invoiceno'], $infoinvoicefortot['invoicedate']);
																									$sqlsalespayfortots->execute();
																									$sqlsalespayfortot = $sqlsalespayfortots->get_result();
																									while($infosalespayfortot=$sqlsalespayfortot->fetch_array()){
																										$paidamountfortot+=(float)$infosalespayfortot['amount'];
																									}
																									$balanceamountfortot+=((float)$infoinvoicefortot['invoiceamount']-$paidamountfortot);
																								}
																								$crb+=$balanceamountfortot;
																								$int+=$invoiceamountfortot;
																							}
																							$sqltotlistpages=$con->prepare("SELECT customername, customerid,invoiceamount FROM pairinvoices WHERE franchisesession=? AND createdid=? AND (invoicedate>=? AND invoicedate<=?) GROUP BY customerid");
																					...
																					?>
																						<tr style="height: 30px;font-weight: bold;">
																							<td style="border: 1px solid #eee;padding-left: 10px;">
																								Total
																							</td>
										...
										<table id="print-are3" style="border: 1px solid #eee;display: none;" width="100%">
											<thead>
												<tr>
													<td class="text-uppercase" style="width:64%;border:1px solid #eee;padding-left: 10px;"><span style="font-size:13px;color:black;"> Customer Name</span></td>
													...
													<td class="text-uppercase" style="text-align: right !important;width: 16%;border:1px solid #eee;padding-right: 10px;"><span style="font-size:13px;color:black;white-space: nowrap;overflow: hidden;"> Balance</span></td>
												</tr>
											</thead>
											<tbody id="myTables">
											</tbody>
...
const pagechanges = (page, pageSize) => {
	...
	ajaxRequestview = $.ajax({
		type: "GET",
		url: 'reportviewsearch.php?term='+perpages+'&limitings='+($('#limitforpagenum').val())+'&datefrom=<?=$datefrom?>&dateto=<?=$dateto?>&dif=cust&lastornone='+lastornone+'',
...
$(function() {
	...
	function(start, end, label) {
		...
		window.open("reportcustbalance.php?datefrom="+datefrom+"&dateto="+dateto+"","_self");
	});
});
...
function converHTMLFileToPDF() {
	...
	var finaltimer = setTimeout(function(){
		...
		$.ajax({
			type: "GET",
			url: 'reporthistory.php?types=cust&titles=Balance&printdownpdfcsv=Printed PDF&fromto='+fromtopdf+'',
			success: function (result) {
				console.log(result);
			},
			error: function (error) {
...
function downloadaspdf() {
	...
	var finaltimer = setTimeout(function(){
		...
		$.ajax({
			type: "GET",
			url: 'reporthistory.php?types=cust&titles=Balance&printdownpdfcsv=Downloaded PDF&fromto='+fromtopdf+'',
			success: function (result) {
				console.log(result);
			},
			error: function (error) {
		...
		doc.save('Balance-<?=$datefrom?>-To-<?=$dateto?>');
...
function downloadascsv() {
	...
	var finaltimer = setTimeout(function(){
		...
		link.setAttribute("download", "Balance-<?=$datefrom?>-To-<?=$dateto?>");
		document.body.appendChild(link);
		link.click();
		$.ajax({
			type: "GET",
			url: 'reporthistory.php?types=cust&titles=Balance&printdownpdfcsv=Downloaded CSV&fromto='+fromtopdf+'',
			success: function (result) {
				console.log(result);
			},
			error: function (error) {
...
document.addEventListener('DOMContentLoaded', function() {
	...
	function makeAjaxRequest() {
		ajaxRequest = $.ajax({
			type: "GET",
			url: 'check.php?term=cust&datefrom=<?=$datefrom?>&dateto=<?=$dateto?>&limitfrom=0&limitto=10000',
			success: function (result) {
				if (result != mytabprev) {
