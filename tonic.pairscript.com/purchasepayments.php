	 ...
	 <div style="max-width: 1650px;">
              ...
              <div class="row">
                ...
                <div class="col-lg-3" style="margin-bottom: 57px !important;"> 
                  <input class="form-control" id="purpaysearch" type="text" placeholder="Search.." style="height:24px;">
                </div>
              </div>
                                <?php 
$sqlismainaccessuser=mysqli_query($con, "select * from pairmainaccess where userid='$userid' and moduletype='Payments Made' order by id  asc");
...
if (($infomainaccessuser['useraccesscreate']==1&&$infomainaccessuser['createdid']!=0)||($infomainaccessuser['createdid']==0)) {
       ...
       <?php
                            ...
                            ?>
			<?php
$sqlismainaccessuser=mysqli_query($con, "select * from pairmainaccess where franchiseid='".$_SESSION['franchisesession']."' and moduletype='Payments Made' order by id  asc");
$infomainaccessuser=mysqli_fetch_array($sqlismainaccessuser);
      ...
      <div class="tab-pane fade show active p-3" id="nav-general" role="tabpanel" aria-labelledby="nav-general-tab">
                      ...
                      <?php
                                            ...
                                            // if ((in_array('Vendor Name', $modulecolumns))) {
                                                ?>
                      <td class="text-uppercase" style="width:31%;"><span style="font-size:13px;color:black;">Name</span></td>
                      <?php
                                            // }
                                                ?>
                                                <?php
                      ...
                      <?php
                                            ...
                                            if ((in_array('Payment Types', $modulecolumns))) {
                                                ?>
                      <td class="text-uppercase" style="width:15%;"><span style="font-size:13px;color:black;">Payment Types</span></td>
                      <?php
                                            }
                                                ?>
                                              <!--   <?php
...
$sqltotlist = mysqli_query($con,"select COUNT(DISTINCT id) from pairpurchasepayments where franchisesession='".$_SESSION['franchisesession']."' and createdid='$companymainid' ".$typesforlisting."");
...
<?php
if ($access['purpaypageload']=='pageauto') {
?>
<script type="text/javascript">
...
$.ajax({
type: "GET",
url: 'listpurpaymentsearch.php?term=' + sIndex + '&limitings=15&typesforlisting=<?=$typesforlisting?>',
...
$.ajax({
type: "GET",
url: 'listpurpaymentsearch.php?term='+perpages+'&limitings='+($('#limitforpagenum').val())+'&typesforlisting=<?=$typesforlisting?>',
success: function (result) {
$("#myTable").html(result);
},
error: function (error) {
...
function submitthesorting(value) {
  $.ajax({
    type: "GET",
    url: 'listingsortfilters.php?moduletype=Payments Made&typesforlisting='+value+'',
    success: function (result) {
      console.log(result);
      window.location = 'purchasepayments.php?remarks='+result+'';
    },
    error: function (error) {
      console.log(error);
    }
