...
if(isset($_POST['enquiryno']))
{
	...
	$enquiryamount=mysqli_real_escape_string($con, $_POST['enquiryamount']);
	$paidamount=mysqli_real_escape_string($con, $_POST['paidamount']);
	$paymentterm=mysqli_real_escape_string($con, $_POST['paymentterm']);
	$sql=mysqli_query($con, "update pairenquirys set cancelstatus='$cancelstatus' where enquirydate='$enquirydate' and enquiryno='$enquiryno'");
	/* $sql2=mysqli_query($con, "update pairenquiryspayments set cancelstatus='$cancelstatus' where receiptdate='$enquirydate' and receiptno='$enquiryno'"); */
	if($sql)
	{
...
$sqlp=mysqli_query($con, "select productid, quantity from pairenquirys where createdid='$companymainid' and  enquiryno='$enquiryno' and enquirydate='$enquirydate'");
	...
	if($paidamount!='0')
{
$sqlq=mysqli_query($con, "select customerid, customername from pairenquirys where createdid='$companymainid' and  enquiryno='$enquiryno' and enquirydate='$enquirydate'");	
...
if($sql)
{
	...
	if($sqle)
	{
if((float)$enquiryamount==(float)$paidamount)
{
	$paidstatus='1';
}
else
