...
if (isset($_POST['submitcustomizes'])) {
	...
	$sqlreportmoduless=$con->prepare("SELECT * FROM pairreportmodules WHERE types='drnoteconsumer' ORDER BY id ASC");
	...
	foreach ($newmodules as $newmoduleskey) {
		...
		$modcolumncol=$coltypemod."drnoteconsumer";
	...
	$sqlupreport = $con->prepare("UPDATE pairreports SET rowcolumns=? WHERE franchiseid=? AND createdid=? AND types='drnoteconsumer'");
	...
	$sqlreport = $con->prepare("UPDATE pairreports SET reportperiod=?,filtername=?,companyname=?,dateprepare=?,timeprepare=? WHERE franchiseid=? AND createdid=? AND types='drnoteconsumer'");
	...
	header("Location:reportdrnoteconsumer.php?datefrom=".date($dateformatforurlredirection,strtotime($from))."&dateto=".date($dateformatforurlredirection,strtotime($to))."");
}

$sqlreportviews=$con->prepare("SELECT * FROM pairreports WHERE franchiseid=? AND createdid=? AND types='drnoteconsumer'");
									...
									<div class="row">
										<div class="col-lg-6">
											<p class="mb-3" style="color:black;font-size: 20px;margin-top: -8px;">
												...
												&gt;
												<a href="reportdrnote.php?datefrom=<?=$datefrom?>&dateto=<?=$dateto?>" style="color: #1878F1">
																			...
																			<div id="customizerowcolumns" class="accordion-collapse collapse show" aria-labelledby="customizerowcolumn">
																				<div class="accordion-body text-sm">
																					<div class="row justify-content-center">
																						...
																						<?php
																							$newans=array();
																							$sqlreportaccesss=$con->prepare("SELECT * FROM pairreports WHERE franchiseid=? AND createdid=? AND types='drnoteconsumer'");
																								...
																								<div class="col-lg-6 my-1" style="padding-left: 0px !important;">
																									<div class="custom-control custom-checkbox mr-sm-2" style="padding: 0px 12px !important;">
																										<div class="input-group input-group-sm" id="flaghead">
																											...
																											<input type="checkbox" class="custom-control-input" name="<?= $coltypemod; ?>drnoteconsumer" id="<?= $coltypemod; ?>drnoteconsumer" checked disabled>
																											<label class="custom-control-label custom-label" for="<?= $coltypemod; ?>drnoteconsumer">
																												<?= str_replace(" or ", " / ",str_replace("Debit Note",$infomainaccessuserdebitnote['modulename'],str_replace("Taxable Amount",$access['txttaxabledebitnote'],$newmoduleskey))) ?>
																											</label>
																										</div>
																									</div>
																								</div>
											...
											<div class="tab-pane fade" id="nav-profile" role="tabpanel" aria-labelledby="nav-profile-tab">
												<div class="table-responsive m-3" id="histables">
													<table class="table table-bordered">
														...
														<tbody>
      												<?php
															$sqluse=$con->prepare("SELECT * FROM pairusehistory WHERE usetype = 'REPdrnoteconsumer' AND useid = ? ORDER BY createdon DESC");
																				...
																				<table id="print-are1" style="border:1px solid #eee;height: 975px;" width="100%">
																					<thead>
																						<tr>
																							...
																							<td class="text-uppercase" style="text-align: left !important;border:1px solid #eee;padding-left: 10px;">
																								<span style="font-size:12px;color:black;">
																										<?=$infomainaccessuserdebitnote["modulename"]?> DATE
																								</span>
																							</td>
																					...
																					<tbody id="myTable">
																					<?php
																						$sqls=$con->prepare("SELECT * FROM pairdebitnotes WHERE franchisesession=? AND createdid=? AND (debitnotedate>=? AND debitnotedate<=?) AND cancelstatus='0' AND ".(($sqlviewreport['gstrule']=='0')?'gstrtype="Consumer"':'(gstrtype="Registered Business - Regular" OR gstrtype="Consumer") AND gstno="" ')." GROUP BY debitnotedate, debitnoteno ORDER BY debitnotedate ASC, debitnoteno ASC LIMIT 15");
																					...
																					?>
																						<tr style="vertical-align: middle;height: 18px !important;" onclick="window.open('debitnoteview.php?id=<?=$info['id']?>&debitnoteno=<?=$info['debitnoteno']?>&debitnotedate=<?=$info['debitnotedate']?>','_self')">
																							<td data-label="PLACE OF SUPPLY" style="text-align: left !important;font-size:12px;color:black;border:1px solid #eee;padding-left: 10px;">
																								...
																								<br>
																								<span style="display: inline-flex;max-width: 190px;min-width: 190px;white-space: nowrap;overflow: hidden;color: grey;"><?=$infomainaccessuserdebitnote["modulename"]?> Number : 
																									<span style="color: black;">
																										<?=$info['debitnoteno']?>
																									</span>
																								</span>
																							...
																							<td data-label="<?=$infomainaccessuserdebitnote["modulename"]?> DATE" style="text-align: left !important;font-size:12px;color:black;border:1px solid #eee;padding-left: 10px;">
																								<span style="display: inline-flex;max-width: 70px;min-width: 70px;white-space: nowrap;overflow: hidden;">
																									<span>
																										<?=date($datemainphp,strtotime($info['debitnotedate']))?>
																									</span>
																								</span>
																							</td>
																					...
																					<?php
																						...
																						$sqltotlistpages=$con->prepare("SELECT * FROM pairdebitnotes WHERE franchisesession=? AND createdid=? AND (debitnotedate>=? AND debitnotedate<=?) AND cancelstatus='0' AND ".(($sqlviewreport['gstrule']=='0')?'gstrtype="Consumer"':'(gstrtype="Registered Business - Regular" OR gstrtype="Consumer") AND gstno="" ')." GROUP BY debitnotedate, debitnoteno");
										...
										<table id="print-are3" style="border: 1px solid #eee;display: none;" width="100%">
											<thead>
												<tr>
													...
													<td class="text-uppercase" style="text-align: left !important;border:1px solid #eee;padding-left: 10px;"><span style="font-size:12px;color:black;"> <?=$infomainaccessuserdebitnote["modulename"]?> DATE</span></td>
													<td class="text-uppercase" style="text-align: right !important;border:1px solid #eee;padding-right: 10px;"><span style="font-size:12px;color:black;"> <?=($access["txttaxabledebitnote"])?></span></td>
										...
										<table id="csvprint-are3" style="border: 1px solid #eee;display: none;" width="100%">
											<thead>
												<tr>
													...
													<td class="text-uppercase" style="text-align: left !important;border:1px solid #eee;padding-left: 10px;"><span style="font-size:12px;color:black;"> <?=$infomainaccessuserdebitnote["modulename"]?> NUMBER</span></td>
													<td class="text-uppercase" style="text-align: left !important;border:1px solid #eee;padding-left: 10px;"><span style="font-size:12px;color:black;"> <?=$infomainaccessuserdebitnote["modulename"]?> DATE</span></td>
													...
													<td class="text-uppercase" style="text-align: left !important;border:1px solid #eee;padding-left: 10px;"><span style="font-size:12px;color:black;"> Place of Supply</span></td>
													<td class="text-uppercase" style="text-align: right !important;border:1px solid #eee;padding-right: 10px;"><span style="font-size:12px;color:black;"> Total <?=($access["txttaxabledebitnote"])?></span></td>
...
const pagechanges = (page, pageSize) => {
	...
	ajaxRequestview = $.ajax({
		type: "GET",
		url: 'reportviewsearch.php?lastornone='+lastornone+'&term='+perpages+'&limitings='+($('#limitforpagenum').val())+'&datefrom=<?=$datefrom?>&dateto=<?=$dateto?>&dif=drnoteconsumer',
...
$(function() {
	...
	function(start, end, label) {
		...
		window.open("reportdrnoteconsumer.php?datefrom="+datefrom+"&dateto="+dateto+"","_self");
	});
});
...
function converHTMLFileToPDF() {
	...
	var finaltimer = setTimeout(function(){
		...
		var header = function(data) {
			...
			var headtext1 = "Purchases Received From Unregistered (Consumer) Taxpayers";
			var fontSize = doc.internal.getFontSize();
			var pageWidth = doc.internal.pageSize.width;
		...
		$.ajax({
			type: "GET",
			url: 'reporthistory.php?types=drnoteconsumer&titles=Purchases Received From Unregistered (Consumer) Taxpayers&printdownpdfcsv=Printed PDF&fromto='+fromtopdf+'',
			success: function (result) {
				console.log(result);
			},
			error: function (error) {
...
function downloadaspdf() {
	...
	var finaltimer = setTimeout(function(){
		...
		$.ajax({
			type: "GET",
			url: 'reporthistory.php?types=drnoteconsumer&titles=Purchases Received From Unregistered (Consumer) Taxpayers&printdownpdfcsv=Downloaded PDF&fromto='+fromtopdf+'',
			success: function (result) {
				console.log(result);
			},
			error: function (error) {
		...
		doc.save('Purchases-Received-From-Unregistered-(Consumer)-Taxpayers-<?=$datefrom?>-To-<?=$dateto?>');
...
function downloadascsv() {
	...
	var finaltimer = setTimeout(function(){
		...
		data.push(["Purchases Received From Unregistered (Consumer) Taxpayers"]);
		data.push([fromtopdf]);
		...
		link.setAttribute("download", "Purchases-Received-From-Unregistered-(Consumer)-Taxpayers-Reports-<?=$datefrom?>-To-<?=$dateto?>");
		document.body.appendChild(link);
		link.click();
		$.ajax({
			type: "GET",
			url: 'reporthistory.php?types=drnoteconsumer&titles=Purchases Received From Unregistered (Consumer) Taxpayers&printdownpdfcsv=Downloaded CSV&fromto='+fromtopdf+'',
			success: function (result) {
				console.log(result);
			},
			error: function (error) {
...
document.addEventListener('DOMContentLoaded', function() {
	...
	function makeAjaxRequest() {
		ajaxRequest = $.ajax({
			type: "GET",
			url: 'check.php?term=drnoteconsumer&datefrom=<?=$datefrom?>&dateto=<?=$dateto?>&limitfrom=0&limitto=10000',
			success: function (result) {
				if (result != mytabprev) {
		...
		ajaxCsvRequest = $.ajax({
			type: "GET",
			url: 'reportcsv.php?term=drnoteconsumer&datefrom=<?=$datefrom?>&dateto=<?=$dateto?>',
			success: function (result) {
