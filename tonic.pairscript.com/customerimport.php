...
if(!empty($_FILES['customerfile'] ['name']) && in_array($_FILES['customerfile'] ['type'], $csvMimes)){
...
if($sqlcus){
header("Location: customers.php?remarks=Imported Successfully");
}
else{
header("Location: customers.php?error=".mysqli_error($con));
}
}
}
else{
// $all="invalid customer type";
}
}
...
// $customercode=mysqli_real_escape_string($con, $line[5]);
...
// $customername=mysqli_real_escape_string($con, $line[10]);
...
// $sqlcustomer="INSERT INTO paircustomers set franchisesession='$franchisesession',createdon='$createdon',createdby='$createdby',createdid='$createdid',moduletype='$moduletype',customercode='$customercode',publicid='$publicid',privateid='$privateid',primarysalutation='$primarysalutation',primarycontact='$primarycontact',customername='$customername',billstreet='$billstreet',billcity='$billcity',billstate='$billstate',billpincode='$billpincode',billcountry='$billcountry',sameasbilling='$sameasbilling',shipstreet='$shipstreet',shipcity='$shipcity',shipstate='$shipstate',shippincode='$shippincode',shipcountry='$shipcountry',mobile='$mobile',email='$email',website='$website',workphone='$workphone',gstrtype='$gstrtype',gstin='$gstin',placeos='$placeos',cvisiblity='$cvisiblity',dlno20='$dlno20',dlno21='$dlno21',cstno='$cstno'";
...
<div class="card-body p-3">
 ...
 <p class="mb-3" style="color:black;font-size:20px;margin-top: -8px;"> <i class="fa fa-file-import"></i> Customer Import</p>

<div class="alert alert-info text-white">
Kindly Note that the New Customer List should contains existing Customer Name, otherwise it has been Overrided.
</div>
...
<div class="col-sm-4">
<label for="customerfile" class="custom-label"><span class="text-danger">Customers CSV File *</span></label>
</div>
<div class="col-sm-8">
<input type="file" class="form-control  form-control-sm" required id="customerfile" name="customerfile" accept=".csv">
</div>
</div>
</div>
</div>
...
<input class="btn btn-primary btn-sm btn-custom" type="submit" name="customersubmit" value="Save">  <a class="btn btn-primary btn-sm btn-custom-grey" href="customers.php">Cancel</a>
</div>
</div>
</form>
