...
if (isset($_POST['submitcustomizes'])) {
	...
	$sqlreportmoduless=$con->prepare("SELECT * FROM pairreportmodules WHERE types='accounttrans' ORDER BY id ASC");
	...
	foreach ($newmodules as $newmoduleskey) {
		...
		$modcolumncol=$coltypemod."accounttrans";
	...
	$sqlupreport = $con->prepare("UPDATE pairreports SET rowcolumns=? WHERE franchiseid=? AND createdid=? AND types='accounttrans'");
	...
	$sqlreportfav = $con->prepare("UPDATE pairreportfavourites SET reporturl=? WHERE franchisesession=? AND createdid=? AND reportnames='accounttrans'");
	$sqlreportfav->bind_param("sss", $newReportUrl, $_SESSION['franchisesession'], $companymainid);
	$sqlreportfav->execute();
	$sqlreportfav->close();

	$sqlreport = $con->prepare("UPDATE pairreports SET reportperiod=?,filtername=?,companyname=?,dateprepare=?,timeprepare=?,gstrule=? WHERE franchiseid=? AND createdid=? AND types='accounttrans'");
	...
	header("Location:reportaccounttrans.php?datefrom=".date($dateformatforurlredirection,strtotime($from))."&dateto=".date($dateformatforurlredirection,strtotime($to))."");
}

$sqlreportviews=$con->prepare("SELECT * FROM pairreports WHERE franchiseid=? AND createdid=? AND types='accounttrans'");
																			...
																			<div id="customizerowcolumns" class="accordion-collapse collapse show" aria-labelledby="customizerowcolumn">
																				<div class="accordion-body text-sm">
																					<div class="row justify-content-center">
																						...
																						<?php
																							$newans=array();
																							$sqlreportaccesss=$con->prepare("SELECT * FROM pairreports WHERE franchiseid=? AND createdid=? AND types='accounttrans'");
																								...
																								<div class="col-lg-6 my-1" style="padding-left: 0px !important;">
																									<div class="custom-control custom-checkbox mr-sm-2" style="padding: 0px 12px !important;">
																										<div class="input-group input-group-sm" id="flaghead">
																											...
																											<input type="checkbox" class="custom-control-input" name="<?= $coltypemod; ?>accounttrans" id="<?= $coltypemod; ?>accounttrans" checked disabled>
																											<label class="custom-control-label custom-label" for="<?= $coltypemod; ?>accounttrans">
											...
											<div class="tab-pane fade" id="nav-profile" role="tabpanel" aria-labelledby="nav-profile-tab">
												<div class="table-responsive m-3" id="histables">
													<table class="table table-bordered">
														...
														<tbody>
														<?php
															$sqluse=$con->prepare("SELECT * FROM pairusehistory WHERE usetype = 'REPaccounttrans' AND useid = ? ORDER BY createdon DESC");
																		...
																		<tr style="height:1px;">
																			<td width="100%" style="padding:10px;">
																				<table id="print-are1" style="border: 1px solid #eee;" width="100%">
																					...
																					<tbody id="myTable">
																					<?php
																						$sqlijournals=$con->prepare("SELECT customerid, chartaccountname, customername, SUM(ledgerdebit) AS ledgerdebit, SUM(ledgercredit) AS ledgercredit, notes,ledgerdate, ledgerno, referenceno, referencedate, totalledgerdebit, totalledgercredit, id, type FROM pairledgers WHERE franchisesession=? AND createdid=? AND (ledgerdate>=? AND ledgerdate<=?) GROUP BY chartaccountid,ledgerdate,ledgerno,type ORDER BY ledgerdate,ledgerno,type ASC LIMIT 36");
																						$sqlijournals->bind_param("ssss", $_SESSION['franchisesession'], $companymainid, $datefrom, $dateto);
																					...
																					?>
																						<tr style="height: 30px;">
																							<td style="border: 1px solid #eee;padding-left: 10px;font-size: 13px;">
																								<?=date($datemainphp,strtotime($infojournal['ledgerdate']))?>
																							</td>
																							<td style="border: 1px solid #eee;padding-left: 10px;font-size: 13px;">
																							...
																							<td style="border: 1px solid #eee;padding-left: 10px;font-size: 13px;">
																								<?=ucfirst($infojournal['type'])?>
																							</td>
																							<td style="border: 1px solid #eee;padding-left: 10px;font-size: 13px;">
																								<?=$infojournal['ledgerno']?>
																							</td>
																							<td style="border: 1px solid #eee;padding-left: 10px;font-size: 13px;">
																								<?=$infojournal['referenceno']?>
																							</td>
																							...
																							<td style="border: 1px solid #eee;padding-right: 10px;font-size: 13px;text-align: right;">
																								<?=$resmaincurrencyans.number_format(($infojournal['ledgercredit']>0)?$infojournal['ledgercredit']:$infojournal['ledgerdebit'],2,'.',',')?>
																							</td>
																						</tr>
																					<?php
																						}
																						...
																						$pageinitnumpage = 1;
																						$pagetotnumpage = 1;
																						if (mysqli_num_rows($sqlijournal)>0) {
										...
										<table id="print-are3" style="border: 1px solid #eee;display: none;" width="100%">
											<thead>
												<tr>
													...
													<td class="text-uppercase" style="width:10%;border:1px solid #eee;padding-left: 10px;"><span style="font-size:13px;color:black;">Account</span></td>
													<td class="text-uppercase" style="width:13%;border:1px solid #eee;padding-left: 10px;"><span style="font-size:13px;color:black;">Tr Details</span></td>
													<td class="text-uppercase" style="width:10%;border:1px solid #eee;padding-left: 10px;"><span style="font-size:13px;color:black;">Tr Type</span></td>
													<td class="text-uppercase" style="width:10%;border:1px solid #eee;padding-left: 10px;"><span style="font-size:13px;color:black;">Tr Number</span></td>
													<td class="text-uppercase" style="width:15%;border:1px solid #eee;padding-left: 10px;"><span style="font-size:13px;color:black;">Ref Number</span></td>
													<td class="text-uppercase" style="text-align: right !important;width: 10%;border:1px solid #eee;padding-right: 10px;"><span style="font-size:13px;color:black;white-space: nowrap;overflow: hidden;">Debit</span></td>
													<td class="text-uppercase" style="text-align: right !important;width: 10%;border:1px solid #eee;padding-right: 10px;"><span style="font-size:13px;color:black;white-space: nowrap;overflow: hidden;">Credit</span></td>
													<td class="text-uppercase" style="text-align: right !important;width: 10%;border:1px solid #eee;padding-right: 10px;"><span style="font-size:13px;color:black;white-space: nowrap;overflow: hidden;">Amount</span></td>
												</tr>
											</thead>
											<tbody id="myTables">
											</tbody>
...
const pagechanges = (page, pageSize) => {
	...
	ajaxRequestview = $.ajax({
		type: "GET",
		url: 'reportviewsearch.php?term='+perpages+'&limitings='+($('#limitforpagenum').val())+'&datefrom=<?=$datefrom?>&dateto=<?=$dateto?>&dif=accounttrans&lastornone='+lastornone+'',
...
$(function() {
	...
	function(start, end, label) {
		...
		window.open("reportaccounttrans.php?datefrom="+datefrom+"&dateto="+dateto+"","_self");
	});
});
...
function converHTMLFileToPDF() {
	...
	var finaltimer = setTimeout(function(){
		...
		var header = function(data) {
			...
			var headtext1 = "Accounts Transactions";
			var fontSize = doc.internal.getFontSize();
			var pageWidth = doc.internal.pageSize.width;
		...
		doc.autoTable({
			...
			didDrawPage: header, 
			columnStyles: {
				0: { halign: 'left', cellWidth: 57},
				1: { halign: 'left', cellWidth: 57},
				2: { halign: 'left', cellWidth: 57},
				3: { halign: 'left', cellWidth: 57},
				4: { halign: 'left', cellWidth: 57},
				5: { halign: 'left', cellWidth: 57},
				6: { halign: 'right', cellWidth: 57},
				7: { halign: 'right', cellWidth: 57},
				8: { halign: 'right', cellWidth: 57}
			}
		});
		obj = '<object id="pdfObjObj" data="'+doc.output('bloburi')+'" type="application/pdf" width="90%" height="550"></object>';
		$.ajax({
			type: "GET",
			url: 'reporthistory.php?types=accounttrans&titles=Accounts Transactions&printdownpdfcsv=Printed PDF&fromto='+fromtopdf+'',
			success: function (result) {
				console.log(result);
			},
			error: function (error) {
...
function downloadaspdf() {
	...
	var finaltimer = setTimeout(function(){
		...
		$.ajax({
			type: "GET",
			url: 'reporthistory.php?types=accounttrans&titles=Accounts Transactions&printdownpdfcsv=Downloaded PDF&fromto='+fromtopdf+'',
			success: function (result) {
				console.log(result);
			},
			error: function (error) {
		...
		doc.save('Accounts-Transactions-<?=$datefrom?>-To-<?=$dateto?>');
...
function downloadascsv() {
	...
	var finaltimer = setTimeout(function(){
		...
		data.push(["Accounts Transactions"]);
		data.push([fromtopdf]);
		...
		link.setAttribute("download", "Accounts-Transactions-<?=$datefrom?>-To-<?=$dateto?>");
		document.body.appendChild(link);
		link.click();
		$.ajax({
			type: "GET",
			url: 'reporthistory.php?types=accounttrans&titles=Accounts Transactions&printdownpdfcsv=Downloaded CSV&fromto='+fromtopdf+'',
			success: function (result) {
				console.log(result);
			},
			error: function (error) {
...
document.addEventListener('DOMContentLoaded', function() {
	...
	function makeAjaxRequest() {
		ajaxRequest = $.ajax({
			type: "GET",
			url: 'check.php?term=accounttrans&datefrom=<?=$datefrom?>&dateto=<?=$dateto?>&limitfrom=0&limitto=10000',
			success: function (result) {
				if (result != mytabprev) {
