...
$sqlismainaccessuserinvoices=$con->prepare("SELECT * FROM pairmainaccess WHERE franchiseid=? AND moduletype='Invoices' ORDER BY id ASC");
$sqlismainaccessuserinvoices->bind_param("s", $_SESSION['franchisesession']);
$sqlismainaccessuserinvoices->execute();
$sqlismainaccessuserinvoice = $sqlismainaccessuserinvoices->get_result();
$infomainaccessuserinv=$sqlismainaccessuserinvoice->fetch_array();
$sqlismainaccessuserinvoice->close();
$sqlismainaccessuserinvoices->close();
...
if (isset($_POST['submitcustomizes'])) {
	...
	$sqlreportmoduless=$con->prepare("SELECT * FROM pairreportmodules WHERE types='ouw' ORDER BY id ASC");
	...
	$sqlupreport = $con->prepare("UPDATE pairreports SET rowcolumns=? WHERE franchiseid=? AND createdid=? AND types='ouw'");
	...
	$sqlreport = $con->prepare("UPDATE pairreports SET reportperiod=?,filtername=?,companyname=?,dateprepare=?,timeprepare=?,gstrule=? WHERE franchiseid=? AND createdid=? AND types='ouw'");
	...
	$sqlreportcon = $con->prepare("UPDATE pairreports SET gstrule=? WHERE franchiseid=? AND createdid=? AND (types='ouwreg' OR types='ouwcon')");
	$sqlreportcon->bind_param("sss", $gstrule, $_SESSION['franchisesession'], $companymainid);
	$sqlreportcon->execute();
	$sqlreportcon->close();

	header("Location:outwardreport.php?datefrom=".date($dateformatforurlredirection,strtotime($from))."&dateto=".date($dateformatforurlredirection,strtotime($to))."");
}

$sqlreportviews=$con->prepare("SELECT * FROM pairreports WHERE franchiseid=? AND createdid=? AND types='ouw'");
																			...
																			<div id="customizerowcolumns" class="accordion-collapse collapse show" aria-labelledby="customizerowcolumn">
																				<div class="accordion-body text-sm">
																					<div class="row justify-content-center">
																						...
																						<?php
																							$newans=array();
																							$sqlreportaccesss=$con->prepare("SELECT * FROM pairreports WHERE franchiseid=? AND createdid=? AND types='ouw'");
																								...
																								<div class="col-lg-6 my-1" style="padding-left: 0px !important;">
																									<div class="custom-control custom-checkbox mr-sm-2" style="padding: 0px 12px !important;">
																										<div class="input-group input-group-sm" id="flaghead">
																											...
																											<label class="custom-control-label custom-label" for="<?= $coltypemod; ?>ouw">
																												<?= str_replace(" or ", " / ",str_replace("Invoice",$infomainaccessuserinv['modulename'],$newmoduleskey)) ?>
																											</label>
																										</div>
																									</div>
																								</div>
											...
											<div class="tab-pane fade" id="nav-profile" role="tabpanel" aria-labelledby="nav-profile-tab">
												<div class="table-responsive m-3" id="histables">
													<table class="table table-bordered">
														...
														<tbody>
      												<?php
															$sqluse=$con->prepare("SELECT * FROM pairusehistory WHERE usetype = 'REPouw' AND useid = ? ORDER BY createdon DESC");
																			...
																			<td width="100%" style="padding:10px;">
																				<div style="height: 975px;">
																					<table id="print-are1" style="border: 1px solid #eee;" width="100%">
																						<thead>
																							<tr>
																								...
																								<td class="text-uppercase" style="text-align: right !important;width: 16%;border:1px solid #eee;padding-right: 10px;">
																									<span style="font-size:13px;color:black;">
																										<?=$infomainaccessuserinv["modulename"]?> Total
																									</span>
																								</td>
																							</tr>
																						</thead>
																						<tbody id="myTable">
																						<?php
																							$sqlregisters=$con->prepare("SELECT * FROM pairinvoices WHERE franchisesession=? AND createdid=? AND (invoicedate>=? AND invoicedate<=?) AND cancelstatus='0' AND ".(($sqlviewreport['gstrule']=='0')?'gstrtype="Registered Business - Regular"':'gstrtype="Registered Business - Regular" AND gstno!="" ')." GROUP BY invoicedate, invoiceno");
																						...
																						?>
																							<tr onclick="window.open('outwardreportregistered.php?datefrom=<?=$datefrom?>&dateto=<?=$dateto?>','_self')">
																								...
																								<td data-label="<?=$infomainaccessuserinv["modulename"]?> TOTAL" style="text-align: right !important;border:1px solid #eee;padding-right: 10px;">
																									<span style="display: inline-flex;max-width: 90px;min-width: 90px;white-space: nowrap;overflow: hidden;justify-content: end;<?= (($registerinvoicetotal>0)?'color:royalblue;':'color:black;') ?>">
																										<?= (($registerinvoicetotal==0)?number_format((float)0,2,'.',','):number_format((float)$registerinvoicetotal,2,'.',',')) ?>
																									</span>
																								</td>
																							</tr>
																						<?php
																							}
																							$sqlconsumers=$con->prepare("SELECT * FROM pairinvoices WHERE franchisesession=? AND createdid=? AND (invoicedate>=? AND invoicedate<=?) AND cancelstatus='0' AND ".(($sqlviewreport['gstrule']=='0')?'gstrtype="Consumer"':'(gstrtype="Consumer" OR gstrtype="Registered Business - Regular") AND gstno="" ')." GROUP BY invoicedate, invoiceno");
																						...
																						?>
																							<tr onclick="window.open('outwardreportconsumer.php?datefrom=<?=$datefrom?>&dateto=<?=$dateto?>','_self')">
																								<td data-label="DESCRIPTION" style="border:1px solid #eee;padding-left: 10px;">
																									<span style="display: inline-flex;max-width: 295px;min-width: 295px;white-space: nowrap;overflow: hidden;color: royalblue;">
																								...
																								<td data-label="<?=$infomainaccessuserinv["modulename"]?> TOTAL" style="text-align: right !important;border:1px solid #eee;padding-right: 10px;">
																									<span style="display: inline-flex;max-width: 90px;min-width: 90px;white-space: nowrap;overflow: hidden;justify-content: end;<?= (($consumerinvoicetotal>0)?'color:royalblue;':'color:black;') ?>">
																										<?= (($consumerinvoicetotal==0)?number_format((float)0,2,'.',','):number_format((float)$consumerinvoicetotal,2,'.',',')) ?>
																									</span>
																								</td>
																							</tr>
																						<?php
																							}
																							$sqlhsns=$con->prepare("SELECT * FROM pairinvoices WHERE franchisesession=? AND createdid=? AND (invoicedate>=? AND invoicedate<=?) AND cancelstatus='0' GROUP BY invoicedate, invoiceno");
																						...
																						?>
																							<tr onclick="window.open('outwardreporthsn.php?datefrom=<?=$datefrom?>&dateto=<?=$dateto?>','_self')">
																								<td data-label="DESCRIPTION" style="border:1px solid #eee;padding-left: 10px;">
																									<span style="display: inline-flex;max-width: 295px;min-width: 295px;white-space: nowrap;overflow: hidden;color: royalblue;">
																								...
																								<td data-label="<?=$infomainaccessuserinv["modulename"]?> TOTAL" style="text-align: right !important;border:1px solid #eee;padding-right: 10px;">
																									<span style="display: inline-flex;max-width: 90px;min-width: 90px;white-space: nowrap;overflow: hidden;justify-content: end;<?= (($hsninvoicetotal>0)?'color:royalblue;':'color:black;') ?>">
																										<?= (($hsninvoicetotal==0)?number_format((float)0,2,'.',','):number_format((float)$hsninvoicetotal,2,'.',',')) ?>
																									</span>
																								</td>
																							</tr>
																						<?php
...
const pagechanges = (page, pageSize) => {
	...
	ajaxRequestview = $.ajax({
		type: "GET",
		url: 'reportviewsearch.php?term='+perpages+'&limitings='+($('#limitforpagenum').val())+'&datefrom=<?=$datefrom?>&dateto=<?=$dateto?>&dif=outward',
...
$(function() {
	...
	function(start, end, label) {
		...
		window.open("outwardreport.php?datefrom="+datefrom+"&dateto="+dateto+"","_self");
	});
});
...
function downloadaspdf() {
	...
	var finaltimer = setTimeout(function(){
		...
		doc.save('Outward-Reports-<?=$datefrom?>-To-<?=$dateto?>');
...
function downloadascsv() {
	...
	var finaltimer = setTimeout(function(){
		...
		data.push(["Summary of Outward Supplies (GSTR-1)"]);
		data.push([fromtopdf]);
		...
		link.setAttribute("download", "Outward-Reports-<?=$datefrom?>-To-<?=$dateto?>");
...
document.addEventListener('DOMContentLoaded', function() {
	...
	function makeAjaxRequest() {
		ajaxRequest = $.ajax({
			type: "GET",
			url: 'check.php?term=outward&datefrom=<?=$datefrom?>&dateto=<?=$dateto?>&limitfrom=0&limitto=10000',
			success: function (result) {
				if (result != mytabprev) {
