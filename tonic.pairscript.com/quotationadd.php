<?php
include('lcheck.php');
$sqlismainaccessfield=mysqli_query($con, "select * from pairmainaccess where userid='$companymainid' and moduletype='Quotations' order by id  asc");
...
$rescurrency=explode('-',$ans);
if(isset($_POST['grandtotal']))
{
date_default_timezone_set('Asia/Calcutta');
if (($infomainaccessuser['quotationcustinfo']=='defone')||($infomainaccessuser['quotationcustinfo']=='deftwo')) {
...
$sqlismainaccessuser=mysqli_query($con, "select * from pairmainaccess where userid='$userid' and moduletype='Quotations' order by id  asc");
$infomainaccessuser=mysqli_fetch_array($sqlismainaccessuser);
if ($infomainaccessuser['quotationcustinfo']=='one'||($scriptonetwo=='one')) {
...
$quotationterm=mysqli_real_escape_string($con, $_POST['quotationterm']);

$sqlismainaccessuser=mysqli_query($con, "select moduleno, moduleprefix, modulesuffix from pairmainaccess where franchiseid='".$_SESSION['franchisesession']."' and moduletype='Quotations' order by id  asc");
$infomainaccessuser=mysqli_fetch_array($sqlismainaccessuser);
$quotationno=mysqli_real_escape_string($con, $infomainaccessuser['moduleprefix'].(str_pad($infomainaccessuser['modulesuffix']+1, 0, "0", STR_PAD_LEFT)));


$quotationdate=mysqli_real_escape_string($con, $_POST['quotationdate']);
...
$quotationamount=mysqli_real_escape_string($con, $_POST['grandtotal']);
...
$custsqluse=mysqli_query($con, "insert into pairusehistory set usetype='CUSTOMERS', createdon='$times',  createdby='".$_SESSION["unqwerty"]."', useid='$custid', useremarks='".$custch."' ");
               ...
               if ($infomainaccessuser['quotationcustinfo']=='two'||($scriptonetwo=='two')) {
$address1 = '';
$address2 = '';
$saddress1 = '';
$saddress2 = '';
...
$sql2=mysqli_query($con, "select id from pairquotations where franchisesession='".$_SESSION['franchisesession']."' and createdid='$companymainid' and quotationno='$quotationno' and quotationdate='$quotationdate'");
...
$tid=mysqli_insert_id($con);
mysqli_query($con, "INSERT INTO pairhistory (user, ipaddress, times, remarks, tid) VALUES ('{$username}', '{$ip}', '{$times}', 'Insert A Quotation', '{$tid}')");

$sql3=mysqli_query($con, "update pairmainaccess set modulesuffix=modulesuffix+1 where franchiseid='".$_SESSION['franchisesession']."' and moduletype='Quotations'");
if($sql3)
{
 if(isset($_POST['submit1']))
{
echo '<script> window.open("quotationprint.php?quotationno='.$quotationno.'&quotationdate='.$quotationdate.'", "_blank");</script>';
echo '<script> window.location.href="quotations.php?remarks=Added Successfully";</script>'; 
}
else
{
echo '<script> window.location.href="quotationview.php?quotationno='.$quotationno.'&quotationdate='.$quotationdate.'&remarks=Added Successfully";</script>';
}
}
}
else
{
header("Location: quotations.php?error=Error Data");
}
}
...
<?php
$sqlismainaccessuser=mysqli_query($con, "select moduleno, moduleprefix, modulesuffix,modulename from pairmainaccess where franchiseid='".$_SESSION['franchisesession']."' and moduletype='Quotations' order by id  asc");
$infomainaccessuser=mysqli_fetch_array($sqlismainaccessuser);
...
<!-------------------customer add info end ------------------------------->
<form id="quotationform" action="" onsubmit="return checkvalidate()" method="post" enctype="multipart/form-data" class="form-horizontal" role="form">
<div class="row">
<!-- <div class="col-lg-6">
...
<?php
     if ((in_array('Quotation Information', $fieldadd))) {
        ?>
<div class="col-lg-4">
<div class="accordion" id="accordionRental">
<div class="accordion-item mb-1">
...
<label for="quotationno" class="custom-label"><span class="text-danger"><?= $infomainaccessuser['modulename'] ?> Number *</span></label>
</div>
<div class="col-lg-7">
<input type="text" class="form-control  form-control-sm" id="quotationno" name="quotationno" required value="<?=$infomainaccessuser['moduleprefix']?><?=str_pad(($infomainaccessuser['modulesuffix']+1), 0, "0", STR_PAD_LEFT)?>" readonly>
</div>
</div>
</div>
<div class="col-lg-12">
...
<label for="quotationdate" class="custom-label"><span class="text-danger"><?= $infomainaccessuser['modulename'] ?> Date *</span></label>
</div>
<div class="col-lg-7">
<?php
...
$dt = new DateTime();
?>
<input type="date" class="form-control  form-control-sm" id="quotationdate" name="quotationdate" value="<?= $dt->format('Y-m-d') ?>" required>
</div>
</div>
</div>
...
<div class="form-group row" style="align-items: center !important;">
<div class="col-lg-5" style="padding-right:0px">
<label for="quotationdate" class="custom-label"><span class="text-danger"><?= $infomainaccessuser['modulename'] ?> Term *</span></label>
</div>
<div class="col-lg-7">
<select class="select2-field form-control  form-control-sm" name="quotationterm" id="quotationterm" required>
...
today=addDays(quotationdate, duedates);
...
<input type="radio" class="custom-control-input" name="customerinfodefault" id="onecustomerinfo" value="one" <?= ($infomainaccessuser['quotationcustinfo']=='defone')?'checked':'' ?> onclick="onetwo()">
<label class="custom-control-label custom-label" for="onecustomerinfo">B2B</label>
</div>
</div>
<div class="col-lg-2 col-md-2 col-3">
<div class="custom-control custom-radio mr-sm-2">
<input type="radio" class="custom-control-input" name="customerinfodefault" id="twocustomerinfo" value="two" <?= ($infomainaccessuser['quotationcustinfo']=='deftwo')?'checked':'' ?> onclick="onetwo()">
...
<?php
if ($access['quotbtwocnamerequired']=='Yes') {
?>
$("#twocustomername").removeAttr("required");
<?php
}
if ($access['quotbtwocwphonerequired']=='Yes') {
?>
...
$sqlismainaccessuser=mysqli_query($con, "select * from pairmainaccess where userid='$userid' and moduletype='Quotations' order by id  asc");
$infomainaccessuser=mysqli_fetch_array($sqlismainaccessuser);
if ($infomainaccessuser['quotationcustinfo']=='one'||(($infomainaccessuser['quotationcustinfo']=='defone')||($infomainaccessuser['quotationcustinfo']=='deftwo'))) {
?>
<div id="one">
<div class="row">
<div class="col-lg-12">
...
if ($infomainaccessuser['quotationcustinfo']=='two'||(($infomainaccessuser['quotationcustinfo']=='defone')||($infomainaccessuser['quotationcustinfo']=='deftwo'))) {
?>
<script type="text/javascript">
function twoposans() {
...
<?= ($access['quotbtwocnamerequired']=='Yes')? '<span class="text-danger">'. $infomainaccessusercus['modulename'] . ' Name * </span>': $infomainaccessusercus['modulename'] . ' Name ' ?>
</label>
</div>
<div class="col-lg-9">
<input type="text" name="twocustomername" id="twocustomername" class="form-control form-control-sm" placeholder="<?= $infomainaccessusercus['modulename'] ?> Name" <?= ($access['quotbtwocnamerequired']=='Yes')? 'required': ' ' ?>>
</div>
</div>
<div class="row mt-1">
<div class="col-lg-12">
...
<?= ($access['quotbtwocwphonerequired']=='Yes')? '<span class="text-danger"> WORK PHONE * </span>': ' WORK PHONE ' ?>
</label>
<input type="text" class="form-control form-control-sm" id="twoworkphone" name="twoworkphone" placeholder="Work Phone" <?= ($access['quotbtwocwphonerequired']=='Yes')? 'required': ' ' ?>>
</div>
...
<input type="text" class="form-control form-control-sm" id="twogsttreatment" name="twogsttreatment" placeholder="GST TREATMENT" value="<?= $infomainaccessuser['quotationtwogst'] ?>" readonly>
</div>
...
<option disabled value="" <?=($infomainaccessuser['quottwopos']=='manual'||'auto')?'selected':''?>>Select The Place</option>
<option value="JAMMU AND KASHMIR (1)" <?=($infomainaccessuser['quottwopos']=="JAMMU AND KASHMIR (1)")?'selected':''?>>JAMMU AND KASHMIR (1)</option>
<option value="ANDAMAN AND NICOBAR ISLANDS (35)" <?=($infomainaccessuser['quottwopos']=="ANDAMAN AND NICOBAR ISLANDS (35)")?'selected':''?>>ANDAMAN AND NICOBAR ISLANDS (35)</option>
<option value="ANDHRA PRADESH (NEWLY ADDED) (37)" <?=($infomainaccessuser['quottwopos']=="ANDHRA PRADESH (NEWLY ADDED) (37)")?'selected':''?>>ANDHRA PRADESH (NEWLY ADDED) (37)</option>
<option value="ANDHRA PRADESH(BEFORE DIVISION) (28)" <?=($infomainaccessuser['quottwopos']=="ANDHRA PRADESH(BEFORE DIVISION) (28)")?'selected':''?>>ANDHRA PRADESH(BEFORE DIVISION) (28)</option>
<option value="ARUNACHAL PRADESH (12)" <?=($infomainaccessuser['quottwopos']=="ARUNACHAL PRADESH (12)")?'selected':''?>>ARUNACHAL PRADESH (12)</option>
<option value="ASSAM (18)" <?=($infomainaccessuser['quottwopos']=="ASSAM (18)")?'selected':''?>>ASSAM (18)</option>
<option value="BIHAR (10)" <?=($infomainaccessuser['quottwopos']=="BIHAR (10)")?'selected':''?>>BIHAR (10)</option>
<option value="CENTRE JURISDICTION (99)" <?=($infomainaccessuser['quottwopos']=="CENTRE JURISDICTION (99)")?'selected':''?>>CENTRE JURISDICTION (99)</option>
<option value="CHANDIGARH (4)" <?=($infomainaccessuser['quottwopos']=="CHANDIGARH (4)")?'selected':''?>>CHANDIGARH (4)</option>
<option value="CHATTISGARH (22)" <?=($infomainaccessuser['quottwopos']=="CHATTISGARH (22)")?'selected':''?>>CHATTISGARH (22)</option>
<option value="DADRA AND NAGAR HAVELI AND DAMAN AND DIU (NEWLY MERGED UT) (26*)" <?=($infomainaccessuser['quottwopos']=="DADRA AND NAGAR HAVELI AND DAMAN AND DIU (NEWLY MERGED UT) (26*)")?'selected':''?>>DADRA AND NAGAR HAVELI AND DAMAN AND DIU (NEWLY MERGED UT) (26*)</option>
<option value="DELHI (7)" <?=($infomainaccessuser['quottwopos']=="DELHI (7)")?'selected':''?>>DELHI (7)</option>
<option value="GOA (30)" <?=($infomainaccessuser['quottwopos']=="GOA (30)")?'selected':''?>>GOA (30)</option>
<option value="GUJARAT (24)" <?=($infomainaccessuser['quottwopos']=="GUJARAT (24)")?'selected':''?>>GUJARAT (24)</option>
<option value="HARYANA (6)" <?=($infomainaccessuser['quottwopos']=="HARYANA (6)")?'selected':''?>>HARYANA (6)</option>
<option value="HIMACHAL PRADESH (2)" <?=($infomainaccessuser['quottwopos']=="HIMACHAL PRADESH (2)")?'selected':''?>>HIMACHAL PRADESH (2)</option>
<option value="JHARKHAND (20)" <?=($infomainaccessuser['quottwopos']=="JHARKHAND (20)")?'selected':''?>>JHARKHAND (20)</option>
<option value="KARNATAKA (29)" <?=($infomainaccessuser['quottwopos']=="KARNATAKA (29)")?'selected':''?>>KARNATAKA (29)</option>
<option value="KERALA (32)" <?=($infomainaccessuser['quottwopos']=="KERALA (32)")?'selected':''?>>KERALA (32)</option>
<option value="LADAKH (NEWLY ADDED) (38)" <?=($infomainaccessuser['quottwopos']=="LADAKH (NEWLY ADDED) (38)")?'selected':''?>>LADAKH (NEWLY ADDED) (38)</option>
<option value="LAKSHADWEEP (31)" <?=($infomainaccessuser['quottwopos']=="LAKSHADWEEP (31)")?'selected':''?>>LAKSHADWEEP (31)</option>
<option value="MADHYA PRADESH (23)" <?=($infomainaccessuser['quottwopos']=="MADHYA PRADESH (23)")?'selected':''?>>MADHYA PRADESH (23)</option>
<option value="MAHARASHTRA (27)" <?=($infomainaccessuser['quottwopos']=="MAHARASHTRA (27)")?'selected':''?>>MAHARASHTRA (27)</option>
<option value="MANIPUR (14)" <?=($infomainaccessuser['quottwopos']=="MANIPUR (14)")?'selected':''?>>MANIPUR (14)</option>
<option value="MEGHALAYA (17)" <?=($infomainaccessuser['quottwopos']=="MEGHALAYA (17)")?'selected':''?>>MEGHALAYA (17)</option>
<option value="MIZORAM (15)" <?=($infomainaccessuser['quottwopos']=="MIZORAM (15)")?'selected':''?>>MIZORAM (15)</option>
<option value="NAGALAND (13)" <?=($infomainaccessuser['quottwopos']=="NAGALAND (13)")?'selected':''?>>NAGALAND (13)</option>
<option value="ODISHA (21)" <?=($infomainaccessuser['quottwopos']=="ODISHA (21)")?'selected':''?>>ODISHA (21)</option>
<option value="OTHER TERRITORY (97)" <?=($infomainaccessuser['quottwopos']=="OTHER TERRITORY (97)")?'selected':''?>>OTHER TERRITORY (97)</option>
<option value="PUDUCHERRY (34)" <?=($infomainaccessuser['quottwopos']=="PUDUCHERRY (34)")?'selected':''?>>PUDUCHERRY (34)</option>
<option value="PUNJAB (3)" <?=($infomainaccessuser['quottwopos']=="PUNJAB (3)")?'selected':''?>>PUNJAB (3)</option>
<option value="RAJASTHAN (8)" <?=($infomainaccessuser['quottwopos']=="RAJASTHAN (8)")?'selected':''?>>RAJASTHAN (8)</option>
<option value="SIKKIM (11)" <?=($infomainaccessuser['quottwopos']=="SIKKIM (11)")?'selected':''?>>SIKKIM (11)</option>
<option value="TAMIL NADU (33)"  <?=($infomainaccessuser['quottwopos']=="TAMIL NADU (33)")?'selected':''?>>TAMIL NADU (33)</option>
<option value="TELANGANA (36)" <?=($infomainaccessuser['quottwopos']=="TELANGANA (36)")?'selected':''?>>TELANGANA (36)</option>
<option value="TRIPURA (16)" <?=($infomainaccessuser['quottwopos']=="TRIPURA (16)")?'selected':''?>>TRIPURA (16)</option>
<option value="UTTAR PRADESH (9)" <?=($infomainaccessuser['quottwopos']=="UTTAR PRADESH (9)")?'selected':''?>>UTTAR PRADESH (9)</option>
<option value="UTTARAKHAND (5)" <?=($infomainaccessuser['quottwopos']=="UTTARAKHAND (5)")?'selected':''?>>UTTARAKHAND (5)</option>
<option value="WEST BENGAL (19)" <?=($infomainaccessuser['quottwopos']=="WEST BENGAL (19)")?'selected':''?>>WEST BENGAL (19)</option>
</select>
</div>
</div>
</div>
...
href="quotations.php" style="margin-top:-3px !important;">Cancel</a>
...
$("#customer").on("select2:open", function() {
<?php
if($access['quotnewcustomerdef']=='1'){
?>
$("#configureunits").show();
...
<script type="text/javascript">
$(function() {
$("#quotationsuffix").autocomplete({
source: 'quotationsuffixsearch.php',
select: function(event, ui) {
$("#quotationsuffix").val(ui.item.quotationsuffix);
...
<!------------------------------------------- pro script end ------------------------------------------->
<!--term start-->
<div class="modal fade" id="AddNewQuotationTerm" tabindex="-1" role="dialog" style="z-index: 1051;">
<div class="modal-dialog" role="document">
<div class="modal-content">
<div class="modal-header">
<h5 class="modal-title">New <?= $infomainaccessuser['modulename'] ?> Term</h5>
<span type="button" onclick="funesquotationterm()" class="close" data-dismiss="modal"
aria-label="Close">
...
<div class="form-group row">
<div class="col-sm-5">
<label for="missingquotationterm" class="custom-label"><span class="text-danger">
<?= $infomainaccessuser['modulename'] ?> Term *</span></label>
</div>
<div class="col-sm-7">
<input type="text" name="quotationterm" class="form-control form-control-sm mb-4" id="missingquotationterm" placeholder="Name" required>
</div>
</div>
</div>
</div>
...
<div class="modal-footer ">
<div class="col">
<button   onclick="funaddquotationterm()" class="btn btn-primary btn-sm btn-custom arlina-button expand-left"   type="submit"  name="submitquotationterm" value="Submit">
<span class="label">Save</span> <span class="spinner"></span>
</button>
<button type="button"
class="btn btn-primary btn-sm btn-custom-grey"
onclick="funesquotationterm()">Cancel</button> </div>
</div>
</div>
</div>
</div>
<script>
$("#quotationterm").on("change", function() {
var sOptionVal = $(this).val();
if (sOptionVal == '#AddNewQuotationTerm') {
$('#AddNewQuotationTerm').modal('show');
}
});
function funaddquotationterm() {
var missingquotationterm = document.getElementById('missingquotationterm');
if (missingquotationterm.value == '') {
alert('Please Enter New QuotationTerm Name');
missingquotationterm.focus();
return false;
} else {
...
{
$('#quotationterm').append('<option value="' + missingquotationterm.value + '">' + missingquotationterm.value +
'</option>');
$('#quotationterm').val(missingquotationterm.value).change();
$("#quotationterm").select2();
$('#AddNewQuotationTerm').modal('hide');
return false;
}
}});
...
function funesquotationterm() {
//$('#quotationterm').val('').change();
$("#quotationterm").select2();
$('#AddNewQuotationTerm').modal('hide');
return false;
}
...
$("#quotationterm").on("select2:open", function() {
$("#configureunits").attr("data-bs-target","#AddNewQuotationTerm");
});
$("#quotationterm").on("select2:open", function() {
document.getElementById("configureunits").innerHTML = "New QuotationTerm";
});
</script>
...
<!---payment confirm---->
<script>
function triggerpayment(quotationno,quotationdate,quotationamount,cancelstatus)
{
let allAreFilled = true;
document.getElementById("quotationform").querySelectorAll("[required]").forEach(function(i) {
if (!allAreFilled) return;
if (i.type === "radio") {
let radioValueCheck = false;
document.getElementById("quotationform").querySelectorAll(`[name=${i.name}]`).forEach(function(r) {
...
{
$('#validquotationno').val($('#quotationno').val());
$('#validquotationdate').val($('#quotationdate').val());
$('#validquotationamount').val($('#grandtotal').val());
