...
?>
  ...
  <?php
    ...
    if((isset($_GET['salesorderno']))&&(isset($_GET['salesorderdate']))){
      $salesorderno=htmlspecialchars($_GET['salesorderno'], ENT_QUOTES, 'UTF-8');
      $salesorderdate=htmlspecialchars($_GET['salesorderdate'], ENT_QUOTES, 'UTF-8');
      $sqls=$con->prepare("SELECT * FROM pairsalesorders WHERE franchisesession=? AND createdid=? AND salesorderno=? AND salesorderdate=? ORDER BY id ASC");
      $sqls->bind_param("ssss", $_SESSION['franchisesession'], $companymainid, $salesorderno, $salesorderdate);
      $sqls->execute();
        $sql = $sqls->get_result();
      $count=1;
      if($sql->num_rows>0){
            ...
            <div class="card-body p-3" style="font-family:'Myriad Set Pro','Helvetica Neue',Helvetica,Arial,sans-serif;">
              <div class="row">
                ...
                <div class="col-lg-6">
                  <span style="float:right">
                    <a style="margin:4.5px 4.5px !important;<?=(($rows[0]['tdelete']=='0')?'':'display: none;')?>" class="btn btn-primary btn-sm btn-custom-grey" <?=(($rows[0]['convertstatus']=='0')?'href="converttoinvoice.php?salesorderno='.$salesorderno.'&salesorderdate='.$salesorderdate.'&type=salesorder"':'')?>>
                      <i class="fa fa-file-import"></i>
                      <span class="mobreswords "> <?=(($rows[0]['convertstatus']=='0')?'Convert To '.$infomainaccessinvoice['modulename'].'':''.$infomainaccessinvoice['modulename'].' Converted')?></span>
                    </a>
                    ...
                    <a style="margin:4.5px 4.5px !important;display: none;" class="btn btn-primary btn-sm btn-custom-grey" href="salespaymentadd.php?no=<?=$salesorderno?>&date=<?=$salesorderdate?>&type=salesorder" id="recordpaymentlink">
                  ...
                  ?>
                    <a class="btn btn-primary btn-sm btn-custom-grey" href="salesorderedit.php?id=<?=$_GET['id']?>&salesorderno=<?=$salesorderno?>&salesorderdate=<?=$salesorderdate?>" style="margin:4.5px 4.5px !important;">
                  ...
                  <?php
                    ...
                    $sqlsalespays=$con->prepare("SELECT amount FROM pairsalespayhistory WHERE franchisesession=? AND createdid=? AND invoiceno=? AND invoicedate=? ORDER BY id DESC");
                      $sqlsalespays->bind_param("ssss", $_SESSION['franchisesession'], $companymainid, $salesorderno, $salesorderdate);
                    ...
                    <ul class="dropdown-menu  dropdown-menu-end customdropdown me-sm-n4 mt-1" aria-labelledby="dropdownMenuButton">   
                      <div style="background-color: #3c3c46;margin-top: 0px !important;cursor: pointer;"> 
                        ...
                        <li class="nav-item" style="margin-top:-22px !important;">
                          <a class="nav-link" href="salesordercancel.php?salesorderno=<?=$salesorderno?>&salesorderdate=<?=$salesorderdate?>&tdelete=<?=($rows[0]['tdelete']=='0')?'1':'0'?>" onclick="return confirm('Are you sure want to <?=($rows[0]['tdelete']=='0')?'Delete':'Enable'?> this Sales Order?')">
                            <span class="nav-link-text ms-2"> Delete</span>
                          </a>
                        </li>  
                      </div>
                    </ul>
              ...
              <nav>
                <div class="nav nav-tabs" id="nav-tab" role="tablist">
                  ...
                  <button class="nav-link" id="nav-journal-tab" data-bs-toggle="tab" data-bs-target="#nav-journal" type="button" role="tab" aria-controls="nav-journal" aria-selected="false" style="display: none;">
                    <div class="customcont-header ml-0"> 
              ...
              <div class="tab-content" id="nav-tabContent">
                <div class="tab-pane fade" id="nav-journal" role="tabpanel" aria-labelledby="nav-journal-tab">
                  <div class="table-responsive m-3" id="journaltables">
                    <table class="table table-bordered">
                      <tbody>
                      <?php
                        $sqlijournals=$con->prepare("SELECT customerid, chartaccountname, customername, sum(ledgerdebit) AS ledgerdebit, sum(ledgercredit) AS ledgercredit, notes,ledgerdate, ledgerno, referenceno, referencedate, totalledgerdebit, totalledgercredit, id, type FROM pairledgers WHERE franchisesession=? AND createdid=? AND ledgerno=? AND ledgerdate=? AND (type='salesorder' OR type='salesorder payment') GROUP BY type,chartaccountid,ledgerdate,ledgerno ORDER BY ledgerdate,ledgerno,type ASC");
                          $sqlijournals->bind_param("ssss", $_SESSION['franchisesession'], $companymainid, $salesorderno, $salesorderdate);
                ...
                <div class="tab-pane fade" id="nav-profile" role="tabpanel" aria-labelledby="nav-profile-tab">
                  <div class="table-responsive m-3" id="histables">
                    <table class="table table-bordered">
                      ...
                      <tbody>
                      <?php
                        $sqluse=$con->prepare("SELECT * FROM pairusehistory WHERE (usetype = 'SALE ORDERS' AND (useid = ? AND (uniqueid = ? OR uniqueid = ?))) ORDER BY createdon DESC");
                          $sqluse->bind_param("sss", $salesorderno, $_GET['id'], $salesorderno);
                          $sqluse->execute();
                      ...
                      ?>
                        <tr>
                          ...
                          <td data-label="DETAILS">
                            ...
                            <br>
                            <span>
                              <?=((str_contains($infouse['useremarks'],'SALE ORDER CREATED'))?'Created By':'Changed By')?>
                            </span>
                            <span  id="chhis">
                              <?=$infouse['createdby']?>
                            </span>
                ...
                <?php
                  $sqlismainaccessusers=$con->prepare("SELECT moduleno, moduleprefix, modulesuffix,saleorderprintgsttreatment,saleorderprintgstin,modulename,saleorderprintpos FROM pairmainaccess WHERE franchiseid = ? AND moduletype = 'Sales Orders' ORDER BY id ASC");
                    ...
                    <div>
                      <div class="row d-flex justify-content-center">
                        <div class="col-lg-8 col-md-12 justify-content-center">
                          ...
                          <?php
                            if($rows[0]['convertstatus']=="1"){
                          ?>
                            <div class="ribbon-wrapper">
                              <div class="ribbon">Converted</div>
                            </div>
                          <?php
                            }
                            else{
                                          ...
                                          <td style="text-align:center;height: 80px !important;">
                                            <table width="100%">
                                              <tr>
                                                ...
                                                <td width="51.5%" style="text-align:left;vertical-align: top;padding-left: 10px !important;border-right: 0px solid #cccccc;">
                                                  <table width="100%">
                                                    ...
                                                    <tr>
                                                      <td>
                                                        <table width="100%">
                                                          <tr style="<?=($access['saleorderbranchphone']=='0')?'display:none;':''?>">
                                                          ...
                                                          <tr>
                                                            <td width="10%" style="font-size:12px;padding: 0px !important;line-height: 15px !important;<?=($access['saleorderbranchemail']=='0')?'visibility:hidden;':''?>">
                                                              E-mail
                                                            </td>
                                                            <td width="90%" style="font-size:12px;padding: 0px !important;line-height: 15px !important;">
                                                              <input type="text" readonly value=": <?=$info['email']?>" style="padding:0px;color:black;width: 206px !important;height: 18px !important;border: none !important;outline: none !important;display: inline-flex;font-size: 12px;<?=($access['saleorderbranchemail']=='0')?'visibility:hidden;':''?>" class="form-control form-control-sm">
                                                              <span style="<?=(($access['saleorderprintdlno20']=='1')?'display:inline-flex;':'display:none;')?>white-space: nowrap;width: 169px;overflow: hidden;float: right;margin-right: 9px;">
                                                                DL No 20 &nbsp; : <?=$info['dlno20']?>
                                                              </span>
                                                            </td>
                                                          </tr>
                                                          <tr>
                                                            <td width="10%" style="font-size:12px;padding: 0px !important;line-height: 15px !important;<?=($access['saleorderbranchgstin']=='0')?'visibility:hidden;':''?>">
                                                              GSTIN
                                                            </td>
                                                            <td width="90%" style="font-size:12px;padding: 0px !important;line-height: 15px !important;">
                                                              <span style="display: inline-flex;white-space: nowrap;width: 134px;overflow: hidden;<?=($access['saleorderbranchgstin']=='0')?'visibility:hidden;':''?>">
                                                                : <?=$info['gstno']?>
                                                              </span>
                                                              <span style="<?=(($access['saleorderprintdlno21']=='1')?'display:inline-flex;':'display:none;')?>white-space: nowrap;width: 169px;overflow: hidden;float: right;margin-right: 9px;">
                                                ...
                                                <td width="34.5%">
                                                  <table width="100%">
                                                    <tr <?=(($access['saleorderbank']=='1')?'':'style="display:none;"')?>>
                                                    ...
                                                    <tr <?=(($access['saleordername']=='1')?'':'style="display:none;"')?>>
                                                      <td style="padding: 0px 6px !important;font-size: 11px !important;line-height: 15px !important;text-align: left;" width="35%">
                                                    ...
                                                    <tr <?=(($access['saleorderaccnumber']=='1')?'':'style="display:none;"')?>>
                                                      <td style="padding: 0px 6px !important;font-size: 11px !important;line-height: 15px !important;text-align: left;" width="35%">
                                                    ...
                                                    <tr <?=(($access['saleorderifsccode']=='1')?'':'style="display:none;"')?>>
                                                      <td style="padding: 0px 6px !important;font-size: 11px !important;line-height: 15px !important;text-align: left;" width="35%">
                                                    ...
                                                    <tr <?=(($access['saleorderbranchandcity']=='1')?'':'style="display:none;"')?>>
                                                      <td style="padding: 0px 6px !important;font-size: 11px !important;line-height: 15px !important;text-align: left;" width="35%">
                                      ...
                                      <table width="100%">
                                        <tr>
                                                ...
                                                <td style="padding: 0px !important;">
                                                  <table width="100%">
                                                    ...
                                                    <tr <?=((in_array('GSTIN', $fieldview))?'':'style="display:none;"')?>>
                                                      <td width="38%" style="<?=((($infomainaccessuser['saleorderprintgstin']=='show')||($rows[0]['gstno']!='')&&($infomainaccessuser['saleorderprintgstin']!='hide'))?'':'display:none;')?>font-size:12px;padding:0px 6px !important;line-height: 15px !important;">
                                                        GSTIN
                                                      </td>
                                                      <td width="62%" style="<?=((($infomainaccessuser['saleorderprintgstin']=='show')||($rows[0]['gstno']!='')&&($infomainaccessuser['saleorderprintgstin']!='hide'))?'':'display:none;')?>font-size:12px;padding: 0px !important;line-height: 15px !important;">
                                                        : 
                                                      <?php
                                                        if (($infomainaccessuser['saleorderprintgstin']=='show')||($rows[0]['gstno']!='')&&($infomainaccessuser['saleorderprintgstin']!='hide')) {
                                                      ?>
                                                ...
                                                <td style="padding: 0px !important;">
                                                  <table width="100%">
                                                    ...
                                                    <tr <?=(in_array('Place of Supply', $fieldview))?'':'style="display:none;"'?>>
                                                      <td width="38%" style="<?=(($infomainaccessuser['saleorderprintpos']=='show')||($rows[0]['pos']!='')&&($infomainaccessuser['saleorderprintpos']!='hide'))?'':'display:none;'?>font-size:12px;padding:0px 6px !important;line-height: 15px !important;white-space: nowrap !important;">
                                                        Place Of Supply
                                                      </td>
                                                      <td width="62%" style="<?=(($infomainaccessuser['saleorderprintpos']=='show')||($rows[0]['pos']!='')&&($infomainaccessuser['saleorderprintpos']!='hide'))?'':'display:none;'?>font-size:12px;padding: 0px !important;line-height: 15px !important;">: 
                                                      <?php
                                                        if (($infomainaccessuser['saleorderprintpos']=='show')||($rows[0]['pos']!='')&&($infomainaccessuser['saleorderprintpos']!='hide')) {
                                                      ?>
                                          ...
                                          <td width="33%" style="white-space: normal !important;vertical-align: middle;padding: 0px !important;">
                                            ...
                                            <?php
                                              // if ((in_array('Sales Order Information', $fieldview))) {
                                            ?>
                                              <tr>
                                              ...
                                              ?>
                                                <td style="padding: 0px !important;font-size: 14px !important;line-height: 15px !important;text-align: left !important;" width="45%">
                                                  :   <b>
                                                      <?=date($date,strtotime($rows[0]['salesorderdate']))?>
                                                    </b>
                                                </td>
                                              </tr>
                                              <tr>
                                                ...
                                                <td style="padding: 0px !important;">
                                                  <table id="gsttable" width="100%" style="line-height: 13px !important;padding: 0px !important;border: 1px solid #cccccc;border-collapse: collapse;">
                                                    ...
                                                    <?php
                                                      ...
                                                      $sqlitaxess=$con->prepare("SELECT * FROM pairsalesorders WHERE franchisesession = ? AND createdid = ? AND salesorderno=? and salesorderdate=? ORDER BY id ASC");
                                                        $sqlitaxess->bind_param("ssss", $_SESSION['franchisesession'], $companymainid, $salesorderno, $salesorderdate);
                                                        $sqlitaxess->execute();
                          ...
                          <?php
                            ...
                            elseif($info['salesordertemplate']=='4'){
                              //FOR 3 Inch PAPER
                          ?>
                            <div id="printalble">
                              <div class="table-responsive" style="width: max-content !important;height: max-content !important;max-width:max-content !important; max-height:max-content !important;min-width:max-content !important; min-height:max-content !important;">
                                <table id="printarea" style="border:1px solid #cccccc;margin-bottom: -13px !important;width: 7.62cm !important;max-width:7.62cm !important;min-width:7.62cm !important;">
                                  ...
                                  <tr style="height:10px !important;<?=($access['saleorderbranchphone']=='0')?'display:none;':''?>">
                                    <td style="border-bottom: 1px solid #cccccc;">
                                      <table width="100%">
                                        <tr>
                                          <td style="text-align:center;line-height: 9px !important;">
                                        ...
                                        <tr>
                                          ...
                                          <td style="text-align:right;line-height: 13px !important;" width="50%">
                                            <b style="font-size:10px !important;display: inline-flex;white-space: nowrap;width: max-content;overflow: hidden;text-align: right;">
                                              Date : <?=date($date,strtotime($rows[0]['salesorderdate']))?>
                                            </b>
                                          </td>
                                        </tr>
                                      </table>
                                              ...
                                              <table width="100%" style="border:1px solid #cccccc 0px 0px 0px !important;line-height: 13px !important;">
                                                ...
                                                ?>
                                                  <tr>
                                                    ...
                                                    <td style="border: 1px solid #cccccc;<?=(in_array('Amount', $fieldview))?'':'display:none !important;'?>">
                                                      &nbsp;
                                                    </td>
                                                  </tr>
                                                </tbody>
                          ...
                          <?php
                            ...
                            elseif($info['salesordertemplate']=='0'){
                                                      ...
                                                      <td>
                                                        <table width="100%">
                                                          ...
                                                          <tr>
                                                            ...
                                                            <td width="90%" style="font-size:12px;padding: 0px !important;line-height: 15px !important;">
                                                              <input type="text" readonly value=": <?=$info['email']?>" style="padding:0px;color:black;width: 150px !important;height: 18px !important;border: none !important;outline: none !important;display: inline-flex;font-size: 12px;<?=($access['saleorderbranchemail']=='0')?'visibility:hidden;':''?>" class="form-control form-control-sm">
                                                              <span style="<?=(($access['saleorderprintdlno20']=='1')?'display:inline-flex;':'display:none;')?>white-space: nowrap;width: 174px;overflow: hidden;float: right;margin-right: 9px;">
                                                                DL No 20 &nbsp; : <?=$info['dlno20']?>
                                                              </span>
                                                            </td>
                                                          </tr>
                                                          <tr>
                                                            ...
                                                            <td width="90%" style="font-size:12px;padding: 0px !important;line-height: 15px !important;">
                                                              <span style="display: inline-flex;white-space: nowrap;width: 113px;overflow: hidden;<?=($access['saleorderbranchgstin']=='0')?'visibility:hidden;':''?>">
                                                                : <?=$info['gstno']?>
                                                              </span>
                                                              <span style="<?=(($access['saleorderprintdlno21']=='1')?'display:inline-flex;':'display:none;')?>white-space: nowrap;width: 174px;overflow: hidden;float: right;margin-right: 9px;">
                                                                DL No 21 &nbsp; : <?=$info['dlno21']?>
                                                              </span>
                                                            </td>
                                                          </tr>
                                                ...
                                                <td width="34.5%">
                                                  <table width="100%">
                                                    ...
                                                    <tr <?=(($access['saleordername']=='1')?'':'style="display:none;"')?>>
                                                      ...
                                                      <td style="padding: 0px !important;font-size: 11px !important;line-height: 15px !important;text-align: left !important;white-space: nowrap !important;overflow: hidden !important;" width="65%">
                                                        :   <b style="display:inline-flex;max-width: 163px !important;<?=(($access['saleorderbank']=='1')?'overflow: hidden;':'white-space: pre-line;')?>"><?=$info['names']?></b>
                                                      </td>
                                                    </tr>
                                                    <tr <?=(($access['saleorderaccnumber']=='1')?'':'style="display:none;"')?>>
                                                      <td style="padding: 0px 6px !important;font-size: 11px !important;line-height: 15px !important;text-align: left;" width="35%">
                                              ...
                                              <table width="100%" style="border:1px solid #cccccc 0px 0px 0px !important;line-height: 13px !important;height: 723px;">
                                                <thead style="background-color: #eee;">
                                                  ...
                                                  <?php
                                                    if ((in_array('Taxable Value', $fieldview))) {
                                                  ?>
                                                    <th width="5%" style="white-space: nowrap;overflow: hidden;border: 1px solid #cccccc;font-size: 12px !important;padding:0px 6px !important;text-align: right !important;max-width: 104px;min-width: 104px;"><?=($access['txttaxablesaleorder'])?></th>
                                                  <?php
                                                    }
                                                  ?>
                                                  <?php
                          ...
                          <?php
                            ...
                            elseif($info['salesordertemplate']=='2'){
                                                      ...
                                                      <td>
                                                        <table width="100%">
                                                          ...
                                                          <tr>
                                                            ...
                                                            <td width="90%" style="font-size:12px;padding: 0px !important;line-height: 15px !important;">
                                                              <input type="text" readonly value=": <?=$info['email']?>" style="padding:0px;color:black;width: 186px !important;height: 18px !important;border: none !important;outline: none !important;display: inline-flex;font-size: 12px;<?=($access['saleorderbranchemail']=='0')?'visibility:hidden;':''?>" class="form-control form-control-sm">
                                                              <span style="<?=(($access['saleorderprintdlno20']=='1')?'display:inline-flex;':'display:none;')?>white-space: nowrap;width: 139px;overflow: hidden;float: right;margin-right: 9px;">
                                                                DL No 20 &nbsp; : <?=$info['dlno20']?>
                                                              </span>
                                                            </td>
                                                          </tr>
                                                          <tr>
                                                            ...
                                                            <td width="90%" style="font-size:12px;padding: 0px !important;line-height: 15px !important;">
                                                              ...
                                                              <span style="<?=(($access['saleorderprintdlno21']=='1')?'display:inline-flex;':'display:none;')?>white-space: nowrap;width: 139px;overflow: hidden;float: right;margin-right: 9px;">
                                                                DL No 21 &nbsp; : <?=$info['dlno21']?>
                                                              </span>
                                                            </td>
                                                          </tr>
                                              ...
                                              <table width="100%" style="border:1px solid #cccccc 0px 0px 0px !important;line-height: 13px !important;height: 176px;">
                                                ...
                                                ?>
                                                  ...
                                                  <?php
                                                    ...
                                                    $saleorder = '<tr><td style="padding: 0px !important;font-size: 14px !important;line-height: 15px !important;" width="55%"><input type="text" readonly value="'. $infomainaccessuser['modulename'] .'" style="padding:0px;color:black;width: 56px !important;height: 18px !important;border: none !important;outline: none !important;display: inline-flex;font-size: 14px;" class="form-control form-control-sm"> <input type="text" readonly value="Number" style="padding:0px;color:black;width: 54px !important;height: 18px !important;border: none !important;outline: none !important;display: inline-flex;font-size: 14px;" class="form-control form-control-sm"> </td><td style="padding: 0px !important;font-size: 14px !important;line-height: 15px !important;text-align: left !important;" width="45%">: <b>'.$rows[0]['salesorderno'].'</b></td></tr><tr><td style="padding: 0px !important;font-size: 14px !important;line-height: 15px !important;" width="55%"><input type="text" readonly value="'. $infomainaccessuser['modulename'] .'" style="padding:0px;color:black;width: 56px !important;height: 18px !important;border: none !important;outline: none !important;display: inline-flex;font-size: 14px;" class="form-control form-control-sm"> <input type="text" readonly value="Date" style="padding:0px;color:black;width: 54px !important;height: 18px !important;border: none !important;outline: none !important;display: inline-flex;font-size: 14px;" class="form-control form-control-sm"> </td> </td><td style="padding: 0px !important;font-size: 14px !important;line-height: 15px !important;text-align: left !important;" width="45%">: <b>'.date($date,strtotime($rows[0]['salesorderdate'])).'</b></td></tr><tr><td style="padding: 0px !important;font-size: 14px !important;line-height: 15px !important;" width="55%"><input type="text" readonly value="Payment" style="padding:0px;color:black;width: 56px !important;height: 18px !important;border: none !important;outline: none !important;display: inline-flex;font-size: 14px;" class="form-control form-control-sm"> <input type="text" readonly value="Term" style="padding:0px;color:black;width: 54px !important;height: 18px !important;border: none !important;outline: none !important;display: inline-flex;font-size: 14px;" class="form-control form-control-sm"> </td><td style="padding: 0px !important;font-size: 14px !important;line-height: 15px !important;text-align: left !important;" width="45%">: <b>'.$rows[0]['salesorderterm'].'</b></td></tr>';
                                                    // }
                                                    $merge = '</tbody></table></div></td></tr></table></td></tr><tr><td><div style="min-height:92px !important;max-height:92px !important;"><div></td></tr><tr><td><div style="min-height:25px !important;max-height:25px !important;padding: 0px !important;"><div></td></tr><tr style="height:40px !important;"><td style="padding:0px !important;border-bottom: none;"><table width="100%"><tr><td width="30%" style="padding: 0px !important;border-right: 1px solid #cccccc;"><table width="100%"><tr><td style="'.(((in_array('Printed On', $fieldview)))?'':'visibility:hidden;').'vertical-align:middle !important;text-align: center !important;padding-top: 7px !important;"><div style="text-align:center;margin-top: -10px !important;font-size: 12px !important;"><span>Printed On :'.date($dateforfooter,strtotime($dates)).'</span></div><div style="text-align:center;line-height: 7px !important;font-size: 12px !important;"><b>(Page '.$begin.'/'.$finish.')</b></div></td></tr></table></td></tr></table></td></tr></table><span><span style="background-color:#fff !important;font-size: 8px !important;color: #ccc !important;">PAIRSCRIPT</span></span></div><div class="table-responsive" style="margin-top:0.75rem;width: max-content !important;height: max-content !important;max-width:max-content !important; max-height:max-content !important;min-width:max-content !important; min-height:max-content !important;"><table id="printarea" style="border:1px solid #cccccc;margin-bottom: -13px !important;width: 21cm !important;height: 14.8cm !important;max-width:21cm !important; max-height:14.8cm !important;min-width:21cm !important; min-height:14.8cm !important;"><tr style="height:10px !important;"><td style="border-bottom: 1px solid #cccccc;"><table width="100%"><tr><td style="text-align:center;line-height: 13px !important;"><b style="font-size:15px !important;display: inline-flex;white-space: nowrap;width: max-content;overflow: hidden;text-align: center;max-width: 166px;">'.$infomainaccessuser['modulename'].'</b></td></tr></table></td></tr><tr style="height:10px !important;"><td><table width="100%"><tr><td style="text-align:center;height: 80px !important;"><table width="100%"><tr><td width="16%" style="text-align:right;">'.$imgans.'</td><td width="51.5%" style="text-align:left;vertical-align: top;padding-left: 30px !important;"><table width="100%"><tr style="padding:0px !important;"><td><table width="100%" style="padding:0px !important;"><tr style="padding:0px !important;"><td style="padding:0px !important;line-height: 15px !important;"><strong style="font-size:14px;vertical-align: top;display: inline-flex;white-space: nowrap;width: 361px;overflow: hidden;">'.$info['franchisename'].'</strong></td></tr><tr style="padding:0px !important;"><td style="padding:0px !important;line-height: 15px !important;margin-bottom: -3px;"><span style="font-size:12px;margin: 0px !important;display: inline-flex;width: 361px;overflow: hidden;white-space: nowrap;">'.$info['street'].' '.$info['city'].' '.$info['pincode'].' '.$info['state'].' '.$info['country'].'</span></td></tr></table></td></tr><tr><td><table width="100%"><tr style="'.(($access['saleorderbranchphone']=='0')?'display:none;':'').'"><td width="10%" style="font-size:12px;padding: 0px !important;line-height: 13px !important;">Phone </td><td width="90%" style="font-size:12px;padding: 0px !important;line-height: 13px !important;">: <span style="display: inline-flex;white-space: nowrap;width: 253px;overflow: hidden;">'.$info['mobile'].'</span></td></tr><tr><td width="10%" style="font-size:12px;padding: 0px !important;line-height: 15px !important;'.(($access['saleorderbranchemail']=='0')?'visibility:hidden;':'').'">E-mail </td><td width="90%" style="font-size:12px;padding: 0px !important;line-height: 15px !important;"><input type="text" readonly value=": '.$info['email'].'" style="padding:0px;color:black;width: 186px !important;height: 18px !important;border: none !important;outline: none !important;display: inline-flex;font-size: 12px;'.(($access['saleorderbranchemail']=='0')?'visibility:hidden;':'').'" class="form-control form-control-sm"><span style="'.(($access['saleorderprintdlno20']=='1')?'display:inline-flex;':'display:none;').'white-space: nowrap;width: 139px;overflow: hidden;float: right;margin-right: 9px;">DL No 20 &nbsp; : '.$info['dlno20'].'</span></td></tr><tr><td width="10%" style="font-size:12px;padding: 0px !important;line-height: 15px !important;'.(($access['saleorderbranchgstin']=='0')?'visibility:hidden;':'').'">GSTIN </td><td width="90%" style="font-size:12px;padding: 0px !important;line-height: 15px !important;"><span style="display: inline-flex;white-space: nowrap;width: 134px;overflow: hidden;'.(($access['saleorderbranchgstin']=='0')?'visibility:hidden;':'').'">: '.$info['gstno'].'</span><span style="'.(($access['saleorderprintdlno21']=='1')?'display:inline-flex;':'display:none;').'white-space: nowrap;width: 139px;overflow: hidden;float: right;margin-right: 9px;">DL No 21 &nbsp; : '.$info['dlno21'].'</span></td></tr></table></td></tr></table></td><td width="34.5%"><table width="100%"><tr '.(($access['billbank']=='1')?'':'style="display:none;"').'><td style="padding: 0px 6px !important;font-size: 11px !important;line-height: 15px !important;text-align: left;" width="35%"><input type="text" readonly value="Bank" style="padding:0px;color:black;width: 113px !important;height: 15px !important;border: none !important;outline: none !important;display: inline-flex;font-size: 11px;" class="form-control form-control-sm"></td><td style="padding: 0px !important;font-size: 11px !important;line-height: 15px !important;text-align: left !important;white-space: nowrap !important;overflow: hidden !important;" width="65%">: <b style="display:inline-flex;max-width: 163px !important;overflow: hidden;">'.$info['bank'].'</b></td></tr><tr '.(($access['billname']=='1')?'':'style="display:none;"').'><td style="padding: 0px 6px !important;font-size: 11px !important;line-height: 15px !important;text-align: left;" width="35%"><input type="text" readonly value="Name" style="padding:0px;color:black;width: 113px !important;height: 15px !important;border: none !important;outline: none !important;display: inline-flex;font-size: 11px;" class="form-control form-control-sm"></td><td style="padding: 0px !important;font-size: 11px !important;line-height: 15px !important;text-align: left !important;white-space: nowrap !important;overflow: hidden !important;" width="65%">: <b style="display:inline-flex;max-width: 163px !important;overflow: hidden;">'.$info['names'].'</b></td></tr><tr '.(($access['billaccnumber']=='1')?'':'style="display:none;"').'><td style="padding: 0px 6px !important;font-size: 11px !important;line-height: 15px !important;text-align: left;" width="35%"><input type="text" readonly value="Account Number" style="padding:0px;color:black;width: 113px !important;height: 15px !important;border: none !important;outline: none !important;display: inline-flex;font-size: 11px;" class="form-control form-control-sm"></td><td style="padding: 0px !important;font-size: 11px !important;line-height: 15px !important;text-align: left !important;white-space: nowrap !important;overflow: hidden !important;" width="65%">: <b style="display:inline-flex;max-width: 163px !important;overflow: hidden;">'.$info['accountnumber'].'</b></td></tr><tr '.(($access['billifsccode']=='1')?'':'style="display:none;"').'><td style="padding: 0px 6px !important;font-size: 11px !important;line-height: 15px !important;text-align: left;" width="35%"><input type="text" readonly value="IFSC Code" style="padding:0px;color:black;width: 113px !important;height: 15px !important;border: none !important;outline: none !important;display: inline-flex;font-size: 11px;" class="form-control form-control-sm"></td><td style="padding: 0px !important;font-size: 11px !important;line-height: 15px !important;text-align: left !important;white-space: nowrap !important;overflow: hidden !important;" width="65%">: <b style="display:inline-flex;max-width: 163px !important;overflow: hidden;">'.$info['ifsccode'].'</b></td></tr><tr '.(($access['billbranchandcity']=='1')?'':'style="display:none;"').'><td style="padding: 0px 6px !important;font-size: 11px !important;line-height: 15px !important;text-align: left;" width="35%"><input type="text" readonly value="Branch & City" style="padding:0px;color:black;width: 100px !important;height: 16px !important;border: none !important;outline: none !important;display: inline-flex;font-size: 11px;" class="form-control form-control-sm"></td><td style="padding: 0px !important;font-size: 11px !important;line-height: 15px !important;text-align: left !important;white-space: nowrap !important;overflow: hidden !important;" width="65%">: <b style="display:inline-flex;max-width: 163px !important;overflow: hidden;">'.$info['branchandcity'].'</b></td></tr></table></td></tr></table></td></tr></table></td></tr><tr style="height:1px !important;border: 1px solid #cccccc;"><td style="padding: 0px !important;"><table width="100%"><tr><td width="33%" style="white-space: normal !important;vertical-align: top;border-right: 1px solid #cccccc;padding: 0px !important;height: 98px !important;"><table width="100%"><tr style="background-color: #eee;"><td style="padding: 0px !important;"><strong style="padding:0px 6px !important;">Billing Address</strong></td></tr><tr>'.$custname.'</tr><tr><td style="padding: 0px 6px !important;line-height: 15px !important;white-space: nowrap !important;">'.$bill1.$bill2.'</td></tr><tr><td style="padding: 0px !important;"><table width="100%"><tr '.(((in_array('Work Phone', $fieldview))?'':'style="display:none;"')).'><td width="38%" style="font-size:12px;padding:0px 6px !important;line-height: 15px !important;">Work Phone </td><td width="62%" style="font-size:12px;padding: 0px !important;line-height: 15px !important;">: <b style="display: inline-flex;white-space: nowrap;width: 100px;overflow: hidden;">'.$rows[0]['workphone'].'</b></td></tr><tr '.(((in_array('GSTIN', $fieldview))?'':'style="display:none;"')).'><td width="38%" style="'.((($infomainaccessuser['saleorderprintgstin']=='show')||($rows[0]['gstno']!='')&&($infomainaccessuser['saleorderprintgstin']!='hide'))?'':'display:none;').'font-size:12px;padding:0px 6px !important;line-height: 15px !important;">GSTIN </td><td width="62%" style="'.((($infomainaccessuser['saleorderprintgstin']=='show')||($rows[0]['gstno']!='')&&($infomainaccessuser['saleorderprintgstin']!='hide'))?'':'display:none;').'font-size:12px;padding: 0px !important;line-height: 15px !important;">:'.$gstin.'</td></tr></table></td></tr></table></td><td width="33%" style="white-space: normal !important;vertical-align: top;border-right: 1px solid #cccccc;padding: 0px !important;"><table width="100%"><tr style="background-color: #eee;"><td style="padding: 0px !important;"><strong style="padding:0px 6px !important;">Shipping Address</strong></td></tr><tr>'.$custname.'</tr><tr><td style="padding: 0px 6px !important;line-height: 15px !important;white-space: nowrap !important;">'.$ship1.$ship2.'</td></tr><tr><td style="padding: 0px !important;"><table width="100%"><tr '.((in_array('Mobile Phone', $fieldview))?'':'style="display:none;"').'><td width="38%" style="font-size:12px;padding:0px 6px !important;line-height: 15px !important;">Mobile Phone </td><td width="62%" style="font-size:12px;padding: 0px !important;line-height: 15px !important;">: <b style="display: inline-flex;white-space: nowrap;width: 100px;overflow: hidden;">'.$rows[0]['mobile'].'</b></td></tr><tr '.((in_array('Place of Supply', $fieldview))?'':'style="display:none;"').'><td width="38%" style="'.((($infomainaccessuser['saleorderprintpos']=='show')||($rows[0]['pos']!='')&&($infomainaccessuser['saleorderprintpos']!='hide'))?'':'display:none;').'font-size:12px;padding:0px 6px !important;line-height: 15px !important;white-space: nowrap !important;">Place Of Supply </td><td width="62%" style="'.((($infomainaccessuser['saleorderprintpos']=='show')||($rows[0]['pos']!='')&&($infomainaccessuser['saleorderprintpos']!='hide'))?'':'display:none;').'font-size:12px;padding: 0px !important;line-height: 15px !important;">: '.$pos.'</td></tr></table></td></tr></table></td><td width="33%" style="white-space: normal !important;vertical-align: middle;padding: 0px !important;"><table width="100%" style="text-align: center !important;">'.$saleorder.'</table></td></tr></table></td></tr><tr style="height:1px !important;"><td style="padding: 0px !important;"><table width="100%"><tr style="vertical-align: top !important;"><td style="padding: 0px !important;height:176px !important;"><div style="max-height: 176px !important;min-height: 176px !important;overflow: hidden;"><table width="100%" style="border:1px solid #cccccc 0px 0px 0px !important;line-height: 13px !important;height: 176px;"><thead style="background-color: #eee;"><tr style="height:1px !important;"><th width="20%" style="white-space: nowrap;overflow: hidden;border: 1px solid #cccccc;font-size: 12px !important;padding:0px 6px !important;'.((in_array('Item Details', $fieldview))?'':'display:none !important;').'">ITEM DETAILS</th><th width="7%" style="white-space: nowrap;overflow: hidden;border: 1px solid #cccccc;font-size: 12px !important;padding:0px 6px !important;'.((in_array('Category', $fieldview))?'':'display:none !important;').'max-width: 40px;min-width: 40px;">'.($access['txtnamecategory']).'</th><th width="7%" style="white-space: nowrap;overflow: hidden;border: 1px solid #cccccc;font-size: 12px !important;padding:0px 6px !important;'.((in_array('Hsn or Sac', $fieldview))?'':'display:none !important;').'max-width: 56px;min-width: 56px;">HSN/SAC</th><th width="7%" style="white-space: nowrap;overflow: hidden;border: 1px solid #cccccc;font-size: 12px !important;padding:0px 6px !important;'.(($access['batchexpiryval']==1)?'':'display:none !important;').'min-width:59px !important;max-width:59px !important;">BATCH</th><th width="7%" style="white-space: nowrap;overflow: hidden;border: 1px solid #cccccc;font-size: 12px !important;padding:0px 6px !important;'.(($access['batchexpiryval']==1)?'':'display:none !important;').'">EXPIRY</th><th width="9%" style="white-space: nowrap;overflow: hidden;border: 1px solid #cccccc;font-size: 12px !important;padding:0px 6px !important;text-align: right !important; '.((in_array('Mrp', $fieldview))?'':'display:none !important;').'">MRP</th><th width="9%" style="white-space: nowrap;overflow: hidden;border: 1px solid #cccccc;font-size: 12px !important;padding:0px 6px !important;text-align: right !important; '.((in_array('Rate', $fieldview))?'':'display:none !important;').'">RATE</th><th width="7%" style="white-space: nowrap;overflow: hidden;border: 1px solid #cccccc;font-size: 12px !important;padding:0px 6px !important;text-align: right !important;min-width:63px;max-width:63px;'.((in_array('Quantity', $fieldview))?'':'display:none !important;').'">'.($access['txtqtysaleorder']).'</th><th width="7%" style="white-space: nowrap;overflow: hidden;border: 1px solid #cccccc;font-size: 12px !important;padding:0px 6px !important;text-align: right !important;'.((in_array('GST Percentage', $fieldview))?'':'display:none !important;').'">GST %</th><th width="5%" style="white-space: nowrap;overflow: hidden;border: 1px solid #cccccc;font-size: 12px !important;padding:0px 6px !important;text-align: right !important;max-width: 104px;min-width: 104px;'.((in_array('Taxable Value', $fieldview))?'':'display:none !important;').'">'.(($access['txttaxablesaleorder'])).'</th></tr></thead><tbody>';
                                                    $finalrow=3;
                                                    if ((($item-1)%$finalrow)==0) {
                                                      if ((($countt-1)-$totpros)!=0) {
                                                        if (($item-1)!=0) {
                          ...
                          <?php
                            ...
                            elseif($info['salesordertemplate']=='3'){
                                          ...
                                          <td style="text-align:center;height: 80px !important;">
                                            <table width="100%">
                                              <tr>
                                                ...
                                                <td width="<?=(($access['saleorderbank']=='0' && $access['saleordername']=='0' && $access['saleorderaccnumber']=='0' && $access['saleorderifsccode']=='0' && $access['saleorderbranchandcity']=='0')?'86%':'56%')?>" style="text-align:left;vertical-align: top;padding-left: 30px !important;">
                                                  <table width="100%">
                                                    <tr style="padding:0px !important;">
                                                      <td>
                                                        <table width="100%" style="padding:0px !important;">
                                                          ...
                                                          <tr style="padding:0px !important;">
                                                            <td style="padding:0px !important;line-height: 15px !important;margin-bottom: -3px;">
                                                              <span style="font-size:12px;margin: 0px !important;display: inline-flex;width: <?=(($access['saleorderbank']=='0' && $access['saleordername']=='0' && $access['saleorderaccnumber']=='0' && $access['saleorderifsccode']=='0' && $access['saleorderbranchandcity']=='0')?'618px':'361px')?>;overflow: hidden;white-space: nowrap;">
                                                                <?=$info['street']?> <?=$info['city']?> <?=$info['pincode']?> <?=$info['state']?> <?=$info['country']?>
                                                              </span>
                                                            </td>
                                                          </tr>
                                                      ...
                                                      <td>
                                                        <table width="100%">
                                                          ...
                                                          <tr>
                                                            ...
                                                            <td width="90%" style="font-size:12px;padding: 0px !important;line-height: 15px !important;">
                                                              <input type="text" readonly value=": <?=$info['email']?>" style="padding:0px;color:black;width: 186px !important;height: 18px !important;border: none !important;outline: none !important;display: inline-flex;font-size: 12px;<?=($access['saleorderbranchemail']=='0')?'visibility:hidden;':''?>" class="form-control form-control-sm">
                                                              <span style="<?=(($access['saleorderprintdlno20']=='1')?'display:inline-flex;':'display:none;')?>white-space: nowrap;width: 163px;overflow: hidden;float: right;margin-right: 9px;">
                                                                DL No 20 &nbsp; : <?=$info['dlno20']?>
                                                              </span>
                                                            </td>
                                                          </tr>
                                                          <tr>
                                                            ...
                                                            <td width="90%" style="font-size:12px;padding: 0px !important;line-height: 15px !important;">
                                                              ...
                                                              <span style="<?=(($access['saleorderprintdlno21']=='1')?'display:inline-flex;':'display:none;')?>white-space: nowrap;width: 163px;overflow: hidden;float: right;margin-right: 9px;">
                                                                DL No 21 &nbsp; : <?=$info['dlno21']?>
                                                              </span>
                                                            </td>
                                                          </tr>
                                                ...
                                                <td width="30%" style="<?=(($access['saleorderbank']=='0' && $access['saleordername']=='0' && $access['saleorderaccnumber']=='0' && $access['saleorderifsccode']=='0' && $access['saleorderbranchandcity']=='0')?'display: none;':'')?>">
                                                  <table width="100%">
                                                    <tr <?=(($access['saleorderbank']=='1')?'':'style="display:none;"')?>>
                                                      <td style="padding: 0px 6px !important;font-size: 11px !important;line-height: 15px !important;text-align: left;" width="35%">
                                                        <input type="text" readonly value="Bank" style="padding:0px;color:black;width: 100px !important;height: 15px !important;border: none !important;outline: none !important;display: inline-flex;font-size: 11px;" class="form-control form-control-sm">
                                        ...
                                        <tr>
                                              ...
                                              <tr>
                                                <td style="padding: 0px !important;">
                                                  <table width="100%">
                                                    ...
                                                    <tr>
                                                      <td width="38%" style="<?=(((($infomainaccessuser['saleorderprintgstin']=='show')||($rows[0]['gstno']!='')&&($infomainaccessuser['saleorderprintgstin']!='hide'))||(in_array('Mobile Phone', $fieldview)))?'':'display:none;')?>font-size:12px;padding:0px 6px !important;line-height: 15px !important;">
                                                        <?=((($infomainaccessuser['saleorderprintgstin']=='show')||($rows[0]['gstno']!='')&&($infomainaccessuser['saleorderprintgstin']!='hide'))?'GSTIN':'')?>
                                                      </td>
                                                      <td width="62%" style="<?=(((($infomainaccessuser['saleorderprintgstin']=='show')||($rows[0]['gstno']!='')&&($infomainaccessuser['saleorderprintgstin']!='hide'))||(in_array('Mobile Phone', $fieldview)))?'':'display:none;')?>font-size:12px;padding: 0px !important;line-height: 15px !important;">
                                                      <?php
                                                        if (($infomainaccessuser['saleorderprintgstin']=='show')||($rows[0]['gstno']!='')&&($infomainaccessuser['saleorderprintgstin']!='hide')) {
                                                      ?>
                                                        : 
                                          ...
                                          <td width="33%" style="white-space: normal !important;vertical-align: middle;padding: 0px !important;">
                                            ...
                                            ?>
                                              ...
                                              <tr style="<?=((in_array('Sales Order Time', $fieldview))?'':'display:none;')?>">
                                                <td style="padding: 0px !important;font-size: 14px !important;line-height: 15px !important;" width="55%">
                                                  <input type="text" readonly value="<?= $infomainaccessuser['modulename'] ?>" style="padding:0px;color:black;width: 56px !important;height: 18px !important;border: none !important;outline: none !important;display: inline-flex;font-size: 14px;" class="form-control form-control-sm">
                                              ...
                                              ?>
                                                <td style="padding: 0px !important;font-size: 14px !important;line-height: 15px !important;text-align: left !important;" width="45%">
                                                  :   <b>
                                                      <?=date('h:i:s A',strtotime($rows[0]['salesordertime']))?>
                                                    </b>
                                                </td>
                                              </tr>
                                              <tr>
                                              ...
                                              <table width="100%" style="border:1px solid #cccccc 0px 0px 0px !important;line-height: 13px !important;height: 176px;">
                                                ...
                                                ?>
                                                  ...
                                                  <?php
                                                    ...
                                                    //FOR DATE FORMATS
                                                    $saleorder = '<tr><td style="padding: 0px !important;font-size: 14px !important;line-height: 15px !important;" width="55%"><input type="text" readonly value="'. $infomainaccessuser['modulename'] .'" style="padding:0px;color:black;width: 56px !important;height: 18px !important;border: none !important;outline: none !important;display: inline-flex;font-size: 14px;" class="form-control form-control-sm"> <input type="text" readonly value="Number" style="padding:0px;color:black;width: 54px !important;height: 18px !important;border: none !important;outline: none !important;display: inline-flex;font-size: 14px;" class="form-control form-control-sm"> </td><td style="padding: 0px !important;font-size: 14px !important;line-height: 15px !important;text-align: left !important;" width="45%">: <b>'.$rows[0]['salesorderno'].'</b></td></tr><tr><td style="padding: 0px !important;font-size: 14px !important;line-height: 15px !important;" width="55%"><input type="text" readonly value="'. $infomainaccessuser['modulename'] .'" style="padding:0px;color:black;width: 56px !important;height: 18px !important;border: none !important;outline: none !important;display: inline-flex;font-size: 14px;" class="form-control form-control-sm"> <input type="text" readonly value="Date" style="padding:0px;color:black;width: 54px !important;height: 18px !important;border: none !important;outline: none !important;display: inline-flex;font-size: 14px;" class="form-control form-control-sm"> </td> </td><td style="padding: 0px !important;font-size: 14px !important;line-height: 15px !important;text-align: left !important;" width="45%">: <b>'.date($date,strtotime($rows[0]['salesorderdate'])).'</b></td></tr><tr style="'.(((in_array('Sales Order Time', $fieldview))?'':'display:none;')).'"><td style="padding: 0px !important;font-size: 14px !important;line-height: 15px !important;" width="55%"><input type="text" readonly value="'. $infomainaccessuser['modulename'] .'" style="padding:0px;color:black;width: 56px !important;height: 18px !important;border: none !important;outline: none !important;display: inline-flex;font-size: 14px;" class="form-control form-control-sm"> <input type="text" readonly value="Time" style="padding:0px;color:black;width: 54px !important;height: 18px !important;border: none !important;outline: none !important;display: inline-flex;font-size: 14px;" class="form-control form-control-sm"> </td> </td><td style="padding: 0px !important;font-size: 14px !important;line-height: 15px !important;text-align: left !important;" width="45%">: <b>'.date('h:i:s A',strtotime($rows[0]['salesordertime'])).'</b></td></tr><tr><td style="padding: 0px !important;font-size: 14px !important;line-height: 15px !important;" width="55%"><input type="text" readonly value="Payment" style="padding:0px;color:black;width: 56px !important;height: 18px !important;border: none !important;outline: none !important;display: inline-flex;font-size: 14px;" class="form-control form-control-sm"> <input type="text" readonly value="Term" style="padding:0px;color:black;width: 54px !important;height: 18px !important;border: none !important;outline: none !important;display: inline-flex;font-size: 14px;" class="form-control form-control-sm"> </td><td style="padding: 0px !important;font-size: 14px !important;line-height: 15px !important;text-align: left !important;" width="45%">: <b>'.$rows[0]['salesorderterm'].'</b></td></tr>';
                                                    // }
                                                    $merge = '</tbody></table></div></td></tr></table></td></tr><tr><td><div style="min-height:92px !important;max-height:92px !important;"><div></td></tr><tr><td><div style="min-height:25px !important;max-height:25px !important;padding: 0px !important;"><div></td></tr><tr style="height:40px !important;"><td style="padding:0px !important;border-bottom: none;"><table width="100%"><tr><td width="30%" style="padding: 0px !important;border-right: 1px solid #cccccc;"><table width="100%"><tr><td style="'.(((in_array('Printed On', $fieldview)))?'':'visibility:hidden;').'vertical-align:middle !important;text-align: center !important;padding-top: 7px !important;"><div style="text-align:center;margin-top: -10px !important;font-size: 12px !important;"><span>Printed On :'.date($dateforfooter,strtotime($dates)).'</span></div><div style="text-align:center;line-height: 7px !important;font-size: 12px !important;"><b>(Page '.$begin.'/'.$finish.')</b></div></td></tr></table></td></tr></table></td></tr></table><span><span style="background-color:#fff !important;font-size: 8px !important;color: #ccc !important;">PAIRSCRIPT</span></span></div><div class="table-responsive" style="margin-top:0.75rem;width: max-content !important;height: max-content !important;max-width:max-content !important; max-height:max-content !important;min-width:max-content !important; min-height:max-content !important;"><table id="printarea" style="border:1px solid #cccccc;margin-bottom: -13px !important;width: 21cm !important;height: 14.8cm !important;max-width:21cm !important; max-height:14.8cm !important;min-width:21cm !important; min-height:14.8cm !important;"><tr style="height:10px !important;"><td style="border-bottom: 1px solid #cccccc;"><table width="100%"><tr><td style="text-align:center;line-height: 13px !important;"><b style="font-size:15px !important;display: inline-flex;white-space: nowrap;width: max-content;overflow: hidden;text-align: center;max-width: 166px;">'.$infomainaccessuser['modulename'].'</b></td></tr></table></td></tr><tr style="height:10px !important;"><td><table width="100%"><tr><td style="text-align:center;height: 80px !important;"><table width="100%"><tr><td width="16%" style="text-align:right;">'.$imgans.'</td><td width="'.((($access['saleorderbank']=='0' && $access['saleordername']=='0' && $access['saleorderaccnumber']=='0' && $access['saleorderifsccode']=='0' && $access['saleorderbranchandcity']=='0')?'86%':'56%')).'" style="text-align:left;vertical-align: top;padding-left: 30px !important;"><table width="100%"><tr style="padding:0px !important;"><td><table width="100%" style="padding:0px !important;"><tr style="padding:0px !important;"><td style="padding:0px !important;line-height: 15px !important;"><strong style="font-size:14px;vertical-align: top;display: inline-flex;white-space: nowrap;width: 361px;overflow: hidden;">'.$info['franchisename'].'</strong></td></tr><tr style="padding:0px !important;"><td style="padding:0px !important;line-height: 15px !important;margin-bottom: -3px;"><span style="font-size:12px;margin: 0px !important;display: inline-flex;width: '.((($access['saleorderbank']=='0' && $access['saleordername']=='0' && $access['saleorderaccnumber']=='0' && $access['saleorderifsccode']=='0' && $access['saleorderbranchandcity']=='0')?'618px':'361px')).';overflow: hidden;white-space: nowrap;">'.$info['street'].' '.$info['city'].' '.$info['pincode'].' '.$info['state'].' '.$info['country'].'</span></td></tr></table></td></tr><tr><td><table width="100%"><tr style="'.(($access['saleorderbranchphone']=='0')?'display:none;':'').'"><td width="10%" style="font-size:12px;padding: 0px !important;line-height: 13px !important;">Phone </td><td width="90%" style="font-size:12px;padding: 0px !important;line-height: 13px !important;">: <span style="display: inline-flex;white-space: nowrap;width: 253px;overflow: hidden;">'.$info['mobile'].'</span></td></tr><tr><td width="10%" style="font-size:12px;padding: 0px !important;line-height: 15px !important;'.(($access['saleorderbranchemail']=='0')?'visibility:hidden;':'').'">E-mail </td><td width="90%" style="font-size:12px;padding: 0px !important;line-height: 15px !important;"><input type="text" readonly value=": '.$info['email'].'" style="padding:0px;color:black;width: 186px !important;height: 18px !important;border: none !important;outline: none !important;display: inline-flex;font-size: 12px;'.(($access['saleorderbranchemail']=='0')?'visibility:hidden;':'').'" class="form-control form-control-sm"><span style="'.(($access['saleorderprintdlno20']=='1')?'display:inline-flex;':'display:none;').'white-space: nowrap;width: 163px;overflow: hidden;float: right;margin-right: 9px;">DL No 20 &nbsp; : '.$info['dlno20'].'</span></td></tr><tr><td width="10%" style="font-size:12px;padding: 0px !important;line-height: 15px !important;'.(($access['saleorderbranchgstin']=='0')?'visibility:hidden;':'').'">GSTIN </td><td width="90%" style="font-size:12px;padding: 0px !important;line-height: 15px !important;"><span style="display: inline-flex;white-space: nowrap;width: 134px;overflow: hidden;'.(($access['saleorderbranchgstin']=='0')?'visibility:hidden;':'').'">: '.$info['gstno'].'</span><span style="'.(($access['saleorderprintdlno21']=='1')?'display:inline-flex;':'display:none;').'white-space: nowrap;width: 163px;overflow: hidden;float: right;margin-right: 9px;">DL No 21 &nbsp; : '.$info['dlno21'].'</span></td></tr></table></td></tr></table></td><td width="30%" style="'.((($access['saleorderbank']=='0' && $access['saleordername']=='0' && $access['saleorderaccnumber']=='0' && $access['saleorderifsccode']=='0' && $access['saleorderbranchandcity']=='0')?'display: none;':'')).'"><table width="100%"><tr '.(($access['billbank']=='1')?'':'style="display:none;"').'><td style="padding: 0px 6px !important;font-size: 11px !important;line-height: 15px !important;text-align: left;" width="35%"><input type="text" readonly value="Bank" style="padding:0px;color:black;width: 113px !important;height: 15px !important;border: none !important;outline: none !important;display: inline-flex;font-size: 11px;" class="form-control form-control-sm"></td><td style="padding: 0px !important;font-size: 11px !important;line-height: 15px !important;text-align: left !important;white-space: nowrap !important;overflow: hidden !important;" width="65%">: <b style="display:inline-flex;max-width: 163px !important;overflow: hidden;">'.$info['bank'].'</b></td></tr><tr '.(($access['billname']=='1')?'':'style="display:none;"').'><td style="padding: 0px 6px !important;font-size: 11px !important;line-height: 15px !important;text-align: left;" width="35%"><input type="text" readonly value="Name" style="padding:0px;color:black;width: 113px !important;height: 15px !important;border: none !important;outline: none !important;display: inline-flex;font-size: 11px;" class="form-control form-control-sm"></td><td style="padding: 0px !important;font-size: 11px !important;line-height: 15px !important;text-align: left !important;white-space: nowrap !important;overflow: hidden !important;" width="65%">: <b style="display:inline-flex;max-width: 163px !important;overflow: hidden;">'.$info['names'].'</b></td></tr><tr '.(($access['billaccnumber']=='1')?'':'style="display:none;"').'><td style="padding: 0px 6px !important;font-size: 11px !important;line-height: 15px !important;text-align: left;" width="35%"><input type="text" readonly value="Account Number" style="padding:0px;color:black;width: 113px !important;height: 15px !important;border: none !important;outline: none !important;display: inline-flex;font-size: 11px;" class="form-control form-control-sm"></td><td style="padding: 0px !important;font-size: 11px !important;line-height: 15px !important;text-align: left !important;white-space: nowrap !important;overflow: hidden !important;" width="65%">: <b style="display:inline-flex;max-width: 163px !important;overflow: hidden;">'.$info['accountnumber'].'</b></td></tr><tr '.(($access['billifsccode']=='1')?'':'style="display:none;"').'><td style="padding: 0px 6px !important;font-size: 11px !important;line-height: 15px !important;text-align: left;" width="35%"><input type="text" readonly value="IFSC Code" style="padding:0px;color:black;width: 113px !important;height: 15px !important;border: none !important;outline: none !important;display: inline-flex;font-size: 11px;" class="form-control form-control-sm"></td><td style="padding: 0px !important;font-size: 11px !important;line-height: 15px !important;text-align: left !important;white-space: nowrap !important;overflow: hidden !important;" width="65%">: <b style="display:inline-flex;max-width: 163px !important;overflow: hidden;">'.$info['ifsccode'].'</b></td></tr><tr '.(($access['billbranchandcity']=='1')?'':'style="display:none;"').'><td style="padding: 0px 6px !important;font-size: 11px !important;line-height: 15px !important;text-align: left;" width="35%"><input type="text" readonly value="Branch & City" style="padding:0px;color:black;width: 100px !important;height: 16px !important;border: none !important;outline: none !important;display: inline-flex;font-size: 11px;" class="form-control form-control-sm"></td><td style="padding: 0px !important;font-size: 11px !important;line-height: 15px !important;text-align: left !important;white-space: nowrap !important;overflow: hidden !important;" width="65%">: <b style="display:inline-flex;max-width: 163px !important;overflow: hidden;">'.$info['branchandcity'].'</b></td></tr></table></td></tr></table></td></tr></table></td></tr><tr style="height:1px !important;border: 1px solid #cccccc;"><td style="padding: 0px !important;"><table width="100%"><tr><td width="66%" style="white-space: normal !important;vertical-align: top;border-right: 1px solid #cccccc;padding: 0px !important;height: 98px !important;"><table width="100%"><tr style="background-color: #eee;"><td style="padding: 0px !important;"><strong style="padding:0px 6px !important;">Billing Address</strong></td></tr><tr>'.$custname.'</tr><tr><td style="padding: 0px 6px !important;line-height: 15px !important;white-space: nowrap !important;">'.$bill1.$bill2.'</td></tr><tr><td style="padding: 0px !important;"><table width="100%"><tr '.(((in_array('Work Phone', $fieldview))?'':'style="display:none;"')).'><td width="38%" style="font-size:12px;padding:0px 6px !important;line-height: 15px !important;">Work Phone </td><td width="62%" style="font-size:12px;padding: 0px !important;line-height: 15px !important;">: <b style="display: inline-flex;white-space: nowrap;width: 100px;overflow: hidden;">'.$rows[0]['workphone'].'</b></td></tr><tr><td width="38%" style="'.(((($infomainaccessuser['saleorderprintgstin']=='show')||($rows[0]['gstno']!='')&&($infomainaccessuser['saleorderprintgstin']!='hide'))||(in_array('Mobile Phone', $fieldview)))?'':'display:none;').'font-size:12px;padding:0px 6px !important;line-height: 15px !important;">'.(((($infomainaccessuser['saleorderprintgstin']=='show')||($rows[0]['gstno']!='')&&($infomainaccessuser['saleorderprintgstin']!='hide'))?'GSTIN':'')).' </td><td width="62%" style="'.(((($infomainaccessuser['saleorderprintgstin']=='show')||($rows[0]['gstno']!='')&&($infomainaccessuser['saleorderprintgstin']!='hide'))||(in_array('Mobile Phone', $fieldview)))?'':'display:none;').'font-size:12px;padding: 0px !important;line-height: 15px !important;"> '.(((($infomainaccessuser['saleorderprintgstin']=='show')||($rows[0]['gstno']!='')&&($infomainaccessuser['saleorderprintgstin']!='hide'))?' :'.$gstin:'')).' <span style=" '.((in_array('Mobile Phone', $fieldview))?'display: inline-flex;':'display:none;').'white-space: nowrap;width: 175px;overflow: hidden;float:right;"><span style="display: block;width: 100%;"><span style="display: inline-flex;width: 50%;"> Mobile Phone</span><b style="display: inline-flex;width: 50%;"> : '.($rows[0]['mobile']).'</b></span></span></td></tr></table></td></tr></table></td><td width="33%" style="white-space: normal !important;vertical-align: middle;padding: 0px !important;"><table width="100%" style="text-align: center !important;">'.$saleorder.'</table></td></tr></table></td></tr><tr style="height:1px !important;"><td style="padding: 0px !important;"><table width="100%"><tr style="vertical-align: top !important;"><td style="padding: 0px !important;height:176px !important;"><div style="max-height: 176px !important;min-height: 176px !important;overflow: hidden;"><table width="100%" style="border:1px solid #cccccc 0px 0px 0px !important;line-height: 13px !important;height: 176px;"><thead style="background-color: #eee;"><tr style="height:1px !important;"><th width="20%" style="white-space: nowrap;overflow: hidden;border: 1px solid #cccccc;font-size: 12px !important;padding:0px 6px !important;'.((in_array('Item Details', $fieldview))?'':'display:none !important;').'">ITEM DETAILS</th><th width="7%" style="white-space: nowrap;overflow: hidden;border: 1px solid #cccccc;font-size: 12px !important;padding:0px 6px !important;'.((in_array('Category', $fieldview))?'':'display:none !important;').'max-width: 40px;min-width: 40px;">'.($access['txtnamecategory']).'</th><th width="7%" style="white-space: nowrap;overflow: hidden;border: 1px solid #cccccc;font-size: 12px !important;padding:0px 6px !important;'.((in_array('Hsn or Sac', $fieldview))?'':'display:none !important;').'max-width: 56px;min-width: 56px;">HSN/SAC</th><th width="7%" style="white-space: nowrap;overflow: hidden;border: 1px solid #cccccc;font-size: 12px !important;padding:0px 6px !important;'.(($access['batchexpiryval']==1)?'':'display:none !important;').'min-width:59px !important;max-width:59px !important;">BATCH</th><th width="7%" style="white-space: nowrap;overflow: hidden;border: 1px solid #cccccc;font-size: 12px !important;padding:0px 6px !important;'.(($access['batchexpiryval']==1)?'':'display:none !important;').'">EXPIRY</th><th width="9%" style="white-space: nowrap;overflow: hidden;border: 1px solid #cccccc;font-size: 12px !important;padding:0px 6px !important;text-align: right !important; '.((in_array('Mrp', $fieldview))?'':'display:none !important;').'">MRP</th><th width="9%" style="white-space: nowrap;overflow: hidden;border: 1px solid #cccccc;font-size: 12px !important;padding:0px 6px !important;text-align: right !important; '.((in_array('Rate', $fieldview))?'':'display:none !important;').'">RATE</th><th width="7%" style="white-space: nowrap;overflow: hidden;border: 1px solid #cccccc;font-size: 12px !important;padding:0px 6px !important;text-align: right !important;min-width:63px;max-width:63px;'.((in_array('Quantity', $fieldview))?'':'display:none !important;').'">'.($access['txtqtysaleorder']).'</th><th width="7%" style="white-space: nowrap;overflow: hidden;border: 1px solid #cccccc;font-size: 12px !important;padding:0px 6px !important;text-align: right !important;'.((in_array('GST Percentage', $fieldview))?'':'display:none !important;').'">GST %</th><th width="5%" style="white-space: nowrap;overflow: hidden;border: 1px solid #cccccc;font-size: 12px !important;padding:0px 6px !important;text-align: right !important;max-width: 104px;min-width: 104px;'.((in_array('Taxable Value', $fieldview))?'':'display:none !important;').'">'.(($access['txttaxablesaleorder'])).'</th></tr></thead><tbody>';
                                                    $finalrow=3;
                                                    if ((($item-1)%$finalrow)==0) {
                                                      if ((($countt-1)-$totpros)!=0) {
                                                        if (($item-1)!=0) {
                          ...
                          <p align="right" class="mt-3" style="margin-right:-25px; cursor:pointer" id="templatetext">
                            Template: '<?=($info['salesordertemplate']=='1')?'24.13cm X 15.75cm':''?><?=($info['salesordertemplate']=='0')?'Standard A4 Portrait':''?><?=($info['salesordertemplate']=='2')?'Standard A5 Landscape':''?><?=($info['salesordertemplate']=='3')?'A5 Landscape ()':''?><?=($info['salesordertemplate']=='4')?'3 Inch Paper':''?> ' 
                            <a data-bs-toggle="modal" data-bs-target="#changesalesorderModal" class="text-blue">
                              Change
                            </a>
                          </p>
                        </div>
                        <div class="modal fade" id="changesalesorderModal" tabindex="-1" role="dialog" aria-labelledby="changesalesorderModalLabel" aria-hidden="true">
                            <div class="modal-dialog modal-dialog-centered" role="document">
                            <div class="modal-content">
                              <div class="modal-header">
                                  <h5 class="modal-title" id="changesalesorderModalLabel">
                                    Choose Template
                                  </h5>
                                  <button type="button" class="btn-close text-dark" data-bs-dismiss="modal" aria-label="Close">
                                  <span aria-hidden="true">&times;</span>
                              ...
                              <div class="modal-body">
                                ...
                                function standandclick(){
                                  ...
                                  setTimeout(function() {
                                    ...
                                    if (r == true) {
                                      window.location.href='changetemplate.php?template=salesorder&id=<?=$_GET['id']?>&no=<?=$salesorderno?>&date=<?=$salesorderdate?>&temptype=0&submt=submt';
                                    }
                                  }, 100);
                                }
                                function standardafiveclick(){
                                  ...
                                  setTimeout(function() {
                                    var r = confirm('Are you sure want to Change this Template?');
                                    if (r == true) {
                                      window.location.href='changetemplate.php?template=salesorder&id=<?=$_GET['id']?>&no=<?=$salesorderno?>&date=<?=$salesorderdate?>&temptype=2&submt=submt';
                                    }
                                  }, 100);
                                }
                                function standardafive2click(){
                                  ...
                                  setTimeout(function() {
                                    var r = confirm('Are you sure want to Change this Template?');
                                    if (r == true) {
                                      window.location.href='changetemplate.php?template=salesorder&id=<?=$_GET['id']?>&no=<?=$salesorderno?>&date=<?=$salesorderdate?>&temptype=3&submt=submt';
                                    }
                                  }, 100);
                                }
                                function advancedclick(){
                                  ...
                                  setTimeout(function() {
                                    var r = confirm('Are you sure want to Change this Template?');
                                    if (r == true) {
                                      window.location.href='changetemplate.php?template=salesorder&id=<?=$_GET['id']?>&no=<?=$salesorderno?>&date=<?=$salesorderdate?>&temptype=1&submt=submt';
                                    }
                                  }, 100);
                                }
                                function threeinchclick(){
                                  ...
                                  setTimeout(function() {
                                    var r = confirm('Are you sure want to Change this Template?');
                                    if (r == true) {
                                      window.location.href='changetemplate.php?template=salesorder&id=<?=$_GET['id']?>&no=<?=$salesorderno?>&date=<?=$salesorderdate?>&temptype=4&submt=submt';
                                    }
                                  }, 100);
                                }
                              <?php
...
function converHTMLFileToPDF() {
  ...
  $.ajax({
    type: "GET",
    url: 'salesorderprint.php?term=salesorder&icsgst='+finalforprint+'&names=Sales Orders-<?=$_GET['salesorderno']?>-<?=$_GET['salesorderdate']?>&salesorderno=<?=$_GET['salesorderno']?>&salesorderdate=<?=$_GET['salesorderdate']?>&sizes=a4',
    success: function (result) {
      ...
      var req = new XMLHttpRequest();
        req.open("GET", "dompdf/Sales Orders-<?=$_GET['salesorderno']?>-<?=$_GET['salesorderdate']?>.pdf", true);
  ...
  $.ajax({
    ...
    success: function (result) {
      ...
      var req = new XMLHttpRequest();
        ...
        link.download="Sales Orders-<?=$_GET['salesorderno']?>-<?=$_GET['salesorderdate']?>.pdf";
        link.click();
        };
        req.send();
...
function converHTMLFileToPDF() {
  ...
  $.ajax({
    type: "GET",
    url: 'salesorderprint.php?term=salesorder&icsgst='+finalforprint+'&names=Sales Orders-<?=$_GET['salesorderno']?>-<?=$_GET['salesorderdate']?>&salesorderno=<?=$_GET['salesorderno']?>&salesorderdate=<?=$_GET['salesorderdate']?>&sizes=a5',
    success: function (result) {
      if (result && result.length > 0) {       
        result=$.parseJSON( result );
        Timer=result.timer;
...
function converHTMLFileToPDF() {
  ...
  $.ajax({
    type: "GET",
    url: 'salesorderprint.php?term=salesorder&icsgst='+finalforprint+'&names=Sales Orders-<?=$_GET['salesorderno']?>-<?=$_GET['salesorderdate']?>&salesorderno=<?=$_GET['salesorderno']?>&salesorderdate=<?=$_GET['salesorderdate']?>&sizes=dt',
    success: function (result) {
      if (result && result.length > 0) {       
        result=$.parseJSON( result );
        Timer=result.timer;
...
function converHTMLFileToPDF() {
  ...
  $.ajax({
    type: "GET",
    url: 'salesorderprint.php?term=salesorder&icsgst='+finalforprint+'&names=Sales Orders-<?=$_GET['salesorderno']?>-<?=$_GET['salesorderdate']?>&salesorderno=<?=$_GET['salesorderno']?>&salesorderdate=<?=$_GET['salesorderdate']?>&sizes=a52',
    success: function (result) {
      if (result && result.length > 0) {       
        result=$.parseJSON( result );
        Timer=result.timer;
...
function converHTMLFileToPDF() {
  ...
  $.ajax({
    type: "GET",
    url: 'salesorderprint.php?term=salesorder&icsgst='+finalforprint+'&names=Sales Orders-<?=$_GET['salesorderno']?>-<?=$_GET['salesorderdate']?>&salesorderno=<?=$_GET['salesorderno']?>&salesorderdate=<?=$_GET['salesorderdate']?>&sizes=threeinch',
    success: function (result) {
      if (result && result.length > 0) {       
        result=$.parseJSON( result );
        Timer=result.timer;
