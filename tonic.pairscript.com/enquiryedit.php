...
$oldenquiryno=mysqli_real_escape_string($con, $_GET['enquiryno']);
$oldenquirydate=mysqli_real_escape_string($con, $_GET['enquirydate']);
$enquiryno=mysqli_real_escape_string($con, $_POST['enquiryno']);
...
if($productname!='')
{
$sql=mysqli_query($con, "delete from pairenquirys where createdid='$companymainid' and franchisesession='".$_SESSION["franchisesession"]."' and enquiryno='$oldenquiryno' and enquirydate='$oldenquirydate'");
if($sql)
{
}
else
...
$sql2=mysqli_query($con, "select id from pairenquirys where franchisesession='".$_SESSION['franchisesession']."' and createdid='$companymainid' and enquiryno='$oldenquiryno' and enquirydate='$oldenquirydate'");
...
echo '<script> window.location.href="enquirys.php?remarks=Updated Successfully";</script>'; 
}
else
{
...
if ($sql) {
$hissql=mysqli_query($con, "insert into pairusehistory set usetype='ENQUIRYS', createdon='$times',  createdby='".$_SESSION["unqwerty"]."', useid='$enquiryno', useremarks='Enquiry Updated' ");
}
}
...
<input type="hidden" name="oldenquiryno" value="<?=$rows[0]['enquiryno']?>">
<input type="hidden" name="oldenquirydate" value="<?=$rows[0]['enquirydate']?>">
<input type="hidden" name="enquiryterm" value="<?=$rows[0]['enquiryterm']?>">

<div class="row">
        <?php
     if ((in_array('Enquiry Information', $fieldedit))) {
        ?>
<div class="col-lg-4">
...
<div class="col-lg-7">
<input type="text" class="form-control  form-control-sm" id="enquiryno" name="enquiryno" required  value="<?=$rows[0]['enquiryno']?>" readonly>
</div>
</div>
</div>
<div class="col-lg-12">
...
<div class="col-lg-7">
<input type="date" class="form-control  form-control-sm" id="enquirydate" name="enquirydate" required value="<?=$rows[0]['enquirydate']?>">
</div>
</div>
</div>
...
?>
<option value="<?= $info['term'] ?>" <?=($info['term']==$rows[0]['enquiryterm'])?'selected':''?>><?= $info['term'] ?></option>
<?php
}
?>
</select>
...
<input type="radio" class="custom-control-input" name="customerinfodefault" id="onecustomerinfo" value="one" <?= ($rows[0]['customerinfodefault']=='one'||$infomainaccessuser['enqcustinfo']=='defone')?'checked':'' ?> onclick="onetwo()">
<label class="custom-control-label custom-label" for="onecustomerinfo">B2B</label>
</div>
</div>
<div class="col-lg-2 col-md-2 col-3">
<div class="custom-control custom-radio mr-sm-2">
<input type="radio" class="custom-control-input" name="customerinfodefault" id="twocustomerinfo" value="two" <?= ($rows[0]['customerinfodefault']=='two'||$infomainaccessuser['enqcustinfo']=='deftwo')?'checked':'' ?> onclick="onetwo()">
...
while ($info = mysqli_fetch_array($sqli))
{
$sqlienquiry=mysqli_query($con, "select enquiryamount, enquirydate, enquiryno from pairenquirys where franchisesession='".$_SESSION['franchisesession']."' and createdid='$companymainid' and customerid='".$info['id']."' GROUP BY enquirydate, enquiryno order by enquirydate desc, enquiryno desc");
$enquiryamount=0;
$balanceamount=0;
$currentamount=0;
$overdueamount=0;
while($infoenquiry=mysqli_fetch_array($sqlienquiry))
{
$enquiryamount+=(float)$infoenquiry['enquiryamount'];
$paidamount=0;
$sqlsalespay=mysqli_query($con,"select amount from pairsalespayhistory where franchisesession='".$_SESSION['franchisesession']."' and createdid='$companymainid' and invoiceno='".$infoenquiry['enquiryno']."' and invoicedate='".$infoenquiry['enquirydate']."' and customerid='".$info['id']."' order by id desc");
while($infosalespay=mysqli_fetch_array($sqlsalespay))
{
$paidamount+=(float)$infosalespay['amount'];
}
$balanceamount+=((float)$infoenquiry['enquiryamount']-$paidamount);
$diff = abs(time() - strtotime($infoenquiry['enquirydate']));
$days = floor(($diff)/ (60*60*24));
if($days>30)
{
$overdueamount+=((float)$infoenquiry['enquiryamount']-$paidamount);
}
else
{
$currentamount+=((float)$infoenquiry['enquiryamount']-$paidamount);
}
}
?>
...
<div class="col-lg-9">
<input type="text" name="twocustomername" id="twocustomername" class="form-control form-control-sm" placeholder="<?= $infomainaccessusercus['modulename'] ?> Name" value="<?= $rows[0]['customername'] ?>" <?= ($access['enqbtwocnamerequired']=='Yes')? 'required': ' ' ?>>
...
<input type="text" class="form-control form-control-sm" id="twoworkphone" name="twoworkphone" placeholder="Work Phone" value="<?=$rows[0]['workphone']?>" <?= ($access['enqbtwocwphonerequired']=='Yes')? 'required': ' ' ?>>
