...
<div class="col-lg-6">
 ...
 <?php
$sqlismainaccessuser=mysqli_query($con, "select moduleno, moduleprefix, modulesuffix, modulename from pairmainaccess where franchiseid='".$_SESSION['franchisesession']."' and moduletype='Payments Received' order by id  asc");
$infomainaccessuser=mysqli_fetch_array($sqlismainaccessuser);
...
?>
<a class="btn btn-primary btn-sm btn-custom-grey" href="salespaymentedit.php?id=<?=$id?>" id="btngopage"><i class="fa fa-pencil-alt"></i> <span class="mobreswords "> Edit</span></a>
  <?php 
}
?>
  </span>
...
$sql = "SELECT invoiceno, invoicedate FROM pairsalespayhistory WHERE franchisesession = ? AND createdid = ? AND customerid = ? AND paymentid = ?";
$stmt = $con->prepare($sql);
$stmt->bind_param("ssss", $franchisesession, $companymainid, $customerid, $payid);
$stmt->execute();
...
$sqls=mysqli_query($con,"select invoiceno, invoicedate, grandtotal, paidstatus, customerid, customername from pairinvoices where franchisesession='".$_SESSION['franchisesession']."' and createdid='$companymainid' and customerid='$customerid' and  invoiceno='".$invoice['invoiceno']."' and invoicedate='".$invoice['invoicedate']."' GROUP BY invoicedate, invoiceno  order by invoicedate asc, invoiceno asc");
...
foreach ($invoices as $invoice) {
    ...
    while($infos=mysqli_fetch_array($sqls))
    {
        ...
        else
        {
            $sta='2';
                        ...
                        $sqlijournals=$con->prepare("SELECT customerid, chartaccountname, customername, sum(ledgerdebit) AS ledgerdebit, sum(ledgercredit) AS ledgercredit, notes,ledgerdate, ledgerno, referenceno, referencedate, totalledgerdebit, totalledgercredit, id, type FROM pairledgers WHERE franchisesession=? AND createdid=? AND ledgerno=? AND ledgerdate=? AND type='invoice payment' GROUP BY type,chartaccountid,ledgerdate,ledgerno ORDER BY ledgerdate,ledgerno,type ASC");
                          $sqlijournals->bind_param("ssss", $_SESSION['franchisesession'], $companymainid, $infos['invoiceno'], $infos['invoicedate']);
    ...
    <div class="tab-pane fade" id="nav-profile" role="tabpanel" aria-labelledby="nav-profile-tab">
      ...
      <tbody>
        <?php
      $sqluse=mysqli_query($con, "select * from pairusehistory where usetype='SALESPAY' and useid='$id' order by createdon desc");
...
<?php
if($infofranch['salespaymenttemplate']=='1')
{
?>
<div id="printalble">
...
<?php
// if (($infomainaccessuser['invoiceprintgstin']=='show')||($sqlcusfetch['gstno']!='')&&($infomainaccessuser['invoiceprintgstin']!='hide')) {
?>
...
<th style="border:1px solid #ddd !important;vertical-align:middle !important;padding: 0px 6px !important;height: 13px !important;"><span style="display:inline-block;width:100%;color:black !important;font-size:12px !important;"><?=strtoupper($infomainaccessinvoice['modulename'])?> DATE</span></th>
<th style="border:1px solid #ddd !important;vertical-align:middle !important;padding: 0px 6px !important;height: 13px !important;"><span style="display:inline-block;width:100%;color:black !important;font-size:12px !important;"><?=strtoupper($infomainaccessinvoice['modulename'])?> NUMBER</span></th>
...
<th style="border:1px solid #ddd !important;vertical-align:middle !important;padding: 0px 6px !important;height: 13px !important;"><span style="display:inline-block;width:100%;color:black !important;font-size:12px !important;text-align:right;"><?=strtoupper($infomainaccessinvoice['modulename'])?> AMOUNT</span></th>
...
foreach ($invoices as $invoice) {
    ...
    while($infos=mysqli_fetch_array($sqls))
    {
        ...
        echo '<input type="hidden" name="nos[]" id="invoice'.$i.'" value="'.$infos['invoiceno'].'">
        ...
        echo '<td data-label="'. strtoupper($infomainaccessinvoice['modulename']) .' DATE" style="font-size:12px !important;vertical-align:middle !important;padding: 1px 6px !important;text-align:left;"><input type="text" readonly value="'.date('d/m/Y',strtotime($infos['invoicedate'])).'" style="text-align: left;padding:0px;color:black;height: 18px !important;border: none !important;outline: none !important;display: inline-flex;font-size: 12px;" class="form-control form-control-sm"></td>';
        echo '<td data-label="'. strtoupper($infomainaccessinvoice['modulename']) .' NUMBER" style="font-size:12px !important;vertical-align:middle !important;padding: 1px 6px !important;text-align:left;"><input type="text" readonly value="'.$infos['invoiceno'].'" style="text-align: left;padding:0px;color:black;height: 18px !important;border: none !important;outline: none !important;display: inline-flex;font-size: 12px;" class="form-control form-control-sm"></td>';
        ...
        echo '<td data-label="'. strtoupper($infomainaccessinvoice['modulename']) .' AMOUNT" style="font-size:12px !important;vertical-align:middle !important;padding: 1px 6px !important;text-align:right;"><span >'.$rescurrency[0].'</span><input type="text" readonly value="'.(number_format((float)$infos['grandtotal'],2,'.','')).'" style="text-align: right;padding:0px;color:black;height: 18px !important;border: none !important;outline: none !important;display: inline-flex;font-size: 12px;width:90%;" class="form-control form-control-sm"></td>';
...
<td data-label="DESCRIPTION" style="font-size:12px !important;vertical-align:middle !important;padding: 1px 6px !important;text-align:left;"><input type="text" readonly value="<?=strtoupper($infomainaccessusercus['modulename'])?> ADVANCE RECEIVED" style="text-align: left;padding:0px;color:black;height: 18px !important;border: none !important;outline: none !important;display: inline-flex;font-size: 12px;" class="form-control form-control-sm"></td>
...
<p align="right" class="mt-3" style="margin-right:-25px; cursor:pointer" id="templatetext">Template: '<?=($infofranch['salespaymenttemplate']=='0')?'Standard A5 Landscape':''?><?=($infofranch['salespaymenttemplate']=='1')?'Standard A4 Portrait':''?> ' <a data-bs-toggle="modal" data-bs-target="#changesalespaymentModal" class="text-blue">Change</a></p>
</div>
 <!-- Modal -->
<div class="modal fade" id="changesalespaymentModal" tabindex="-1" role="dialog" aria-labelledby="changesalespaymentModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="changesalespaymentModalLabel" style="font-weight:normal;">Choose Template</h5>
        <button type="button" class="btn-close text-dark" data-bs-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
...
if (r == true) {
    window.location.href='changetemplatereceipt.php?template=salespayment&id=<?=$id?>&publicid=<?=$info['publicid']?>&receiptno=<?=$_GET['receiptno']?>&receiptdate=<?=$info['receiptdate']?>&temptype=0&submt=submt';
} else {
...
var r = confirm('Are you sure want to Change this Template?');
if (r == true) {
    window.location.href='changetemplatereceipt.php?template=salespayment&id=<?=$id?>&publicid=<?=$info['publicid']?>&receiptno=<?=$_GET['receiptno']?>&receiptdate=<?=$info['receiptdate']?>&temptype=1&submt=submt';
} else {
...
$.ajax({
type: "GET",
url: 'salespaydompdf.php?term=salespayment&names=<?= $infomainaccessuser['modulename'] ?>-<?=$info['privateid']?>-<?=$_GET['receiptdate']?>&receiptno=<?=$_GET['receiptno']?>&receiptdate=<?=$_GET['receiptdate']?>&sizes=a4&id=<?=$_GET['id']?>',
...
if($infofranch['salespaymenttemplate']=='0'){
?>
<script>
function converHTMLFileToPDF() {
$('#pdfObj').html('<div class="text-center"><span id="timer">Please wait <span id="time"></span> Seconds</span></div><br><img src="loading.gif" width="100%">');
...
$.ajax({
type: "GET",
url: 'salespaydompdf.php?term=salespayment&names=<?= $infomainaccessuser['modulename'] ?>-<?=$info['privateid']?>-<?=$_GET['receiptdate']?>&receiptno=<?=$_GET['receiptno']?>&receiptdate=<?=$_GET['receiptdate']?>&sizes=a5&id=<?=$_GET['id']?>',
success: function (result) {
if (result && result.length > 0) {       
result=$.parseJSON( result );
Timer=result.timer;
...
header("Location:salespayments.php?error=No Information Found");
}
}
else{
header("Location:salespayments.php?error=No Information Found");  
