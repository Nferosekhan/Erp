...
if (isset($_POST['submitcustomizes'])) {
  ...
  $sqlreportmodules=mysqli_query($con, "select * from pairreportmodules where types='ouwreg' order by id  asc");
...
$modcolumncol=$coltypemod."ouwreg";
...
$sqlupreport = "update pairreports set rowcolumns='$modcolumncolchanges' where franchiseid='".$_SESSION['franchisesession']."' and types='ouwreg' and createdid='$companymainid'"; 
$sqlreportup = mysqli_query($con, $sqlupreport);
  ...
  $sqlreport = mysqli_query($con,"update pairreports set reportperiod='$reportperiod',filtername='$filtername',companyname='$companyname',dateprepare='$dateprepare',timeprepare='$timeprepare' where franchiseid='".$_SESSION['franchisesession']."' and types='ouwreg' and createdid='$companymainid'");
  header("Location:outwardreportregistered.php?invfrom=".date($datenxt,strtotime($from))."&invto=".date($datenxt,strtotime($to))."");
}
...
<?php
$newans=array();
$sqlreportaccess=mysqli_query($con, "select * from pairreports where franchiseid='".$_SESSION['franchisesession']."' and types='ouwreg' and createdid='$companymainid' order by id  asc");
...
<input type="checkbox" class="custom-control-input" name="<?= $coltypemod; ?>ouwreg" id="<?= $coltypemod; ?>ouwreg" checked disabled>
<label class="custom-control-label custom-label" for="<?= $coltypemod; ?>ouwreg"> <?= str_replace(" or ", " / ",$newmoduleskey) ?></label>
</div>
</div>                  
</div>
</div>
...
<div class="tab-pane fade" id="nav-profile" role="tabpanel" aria-labelledby="nav-profile-tab">
  <div class="table-responsive m-3" id="histables">
    <table class="table table-bordered">
      ...
      <tbody>
      <?php
      $sqluse=mysqli_query($con, "select * from pairusehistory where usetype='REPouwreg' and useid='$companymainid' order by createdon desc");
...
<tr>
<td>Taxable outward supplies made to registered persons</td>
</tr>
<tr>
<td id="datefromto"></td>
</tr>
...
$pagetotnumpage = ceil($chekcspage/12);
}
...
const App = () => (React.createElement(Pagination, { total: <?=ceil($chekcs)?>, showSizeChanger: false, showQuickJumper: true, showTotal: (total, range) => `Total ${range[0]} - ${range[1]} of ${total} items`,
        onChange: pagechanges,defaultPageSize: 12 }));
...
var totalpagesnumber = <?=ceil($chekcs/12)?>;
if ((page=='')||(page==0)) {
var isthisval = 1;
}
...
$.ajax({
type: "GET",
url: 'reportviewsearch.php?term='+perpages+'&limitings='+($('#limitforpagenum').val())+'&invfrom=<?=$invfrom?>&invto=<?=$invto?>&dif=outwardreg',
...
$(function() {
  ...
  $('#calendars').daterangepicker({
    ...
    let invto = $("#invto").val();
    window.open("outwardreportregistered.php?invfrom="+invfrom+"&invto="+invto+"","_self");
  });
});
</script>
<script type="text/javascript">
...
var finaltimer = setTimeout(function() 
  {
   ...
   doc.setFont(undefined, 'bold');
    ...
    var headtext1 = "Taxable outward supplies made to registered persons";
     var fontSize = doc.internal.getFontSize();
     var pageWidth = doc.internal.pageSize.width;
...
// }            // doc.save('table.pdf');
    obj = '<object id="pdfObjObj" data="'+doc.output('bloburi')+'" type="application/pdf" width="90%" height="350"></object>';
$.ajax({
type: "GET",
url: 'reporthistory.php?types=ouwreg&titles=Taxable outward supplies made to registered persons&printdownpdfcsv=Printed PDF&fromto='+fromtopdf+'',
success: function (result) {
console.log(result);
},
error: function (error) {
...
$.ajax({
type: "GET",
url: 'check.php?term=outwardreg&invfrom=<?=$invfrom?>&invto=<?=$invto?>&limitfrom='+sIndex+'&limitto='+dt+'',
success: function (result) {
    if (result!=mytabprev) {
...
$.ajax({
type: "GET",
url: 'reporthistory.php?types=ouwreg&titles=Taxable outward supplies made to registered persons&printdownpdfcsv=Downloaded PDF&fromto='+fromtopdf+'',
success: function (result) {
console.log(result);
},
error: function (error) {
console.log(error);
    ...
    doc.save('Outward-Registered-Reports-<?=$invfrom?>-To-<?=$invto?>');
...
$.ajax({
type: "GET",
url: 'reportcsv.php?term=outwardreg&from=<?=$invfrom?>&to=<?=$invto?>&branch=<?=(($sqlviewreport['companyname']=='1')?$branch['franchisename']:'')?>&type=Taxable outward supplies made to registered persons&fromto='+fromtopdf+'',
success: function (result) {
if(checkdown){
...
$.ajax({
type: "GET",
url: 'reporthistory.php?types=ouwreg&titles=Taxable outward supplies made to registered persons&printdownpdfcsv=Downloaded CSV&fromto='+fromtopdf+'',
success: function (result) {
console.log(result);
},
error: function (error) {
