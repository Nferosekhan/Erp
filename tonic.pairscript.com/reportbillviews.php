...
if (isset($_POST['submitcustomizes'])) {
	...
	$sqlreportmoduless=$con->prepare("SELECT * FROM pairreportmodules WHERE types='bill' ORDER BY id ASC");
	...
	$sqlupreport = $con->prepare("UPDATE pairreports SET rowcolumns=?, printtype=? WHERE franchiseid=? AND createdid=? AND types='bill'");
	...
	if($names!='all'){
		...
		$sqlicusname->close();
		$sqlicusnames->close();
		$vendorname = $infocusname['customername'];
	}

	$prefix = "CONCAT(SUBSTRING_INDEX(reporturl, '&period=', 1),'&period=')";
	$newReportUrl = $prefix . $reportperiod;

	$sqlreportfav = $con->prepare("UPDATE pairreportfavourites SET reporturl=? WHERE franchisesession=? AND createdid=? AND reportnames='billlist'");
	$sqlreportfav->bind_param("sss", $newReportUrl, $_SESSION['franchisesession'], $companymainid);
	$sqlreportfav->execute();
	$sqlreportfav->close();

	$sqlreport = $con->prepare("UPDATE pairreports SET cusvenname=?,reportperiod=?,filtername=?,companyname=?,dateprepare=?,timeprepare=?,names=?,payterm=? WHERE franchiseid=? AND createdid=? AND types='bill'");
	$sqlreport->bind_param("ssssssssss", $vendorname, $reportperiod, $filtername, $companyname, $dateprepare, $timeprepare, $names, $payterm, $_SESSION['franchisesession'], $companymainid);
	$sqlreport->execute();
	$sqlreport->close();

	header("Location:reportbillviews.php?names=".$names."&datefrom=".date($dateformatforurlredirection,strtotime($from))."&dateto=".date($dateformatforurlredirection,strtotime($to))."&payterm=".$payterm."&amttype=".$amttype."");
}
									...
									<div class="row">
										...
										<div class="col-lg-6">
											<span style="float:right">
																			...
																			<div id="customizerowcolumns" class="accordion-collapse collapse show" aria-labelledby="customizerowcolumn">
																				<div class="accordion-body text-sm">
																					<div class="row justify-content-center">
																						...
																						<?php
																							$newans=array();
																							$sqlreportaccesss=$con->prepare("SELECT * FROM pairreports WHERE franchiseid=? AND createdid=? AND types='bill'");
																								...
																								<div class="col-lg-6 my-1" style="padding-left: 0px !important;">
																									<div class="custom-control custom-checkbox mr-sm-2" style="padding: 0px 12px !important;">
																										<div class="input-group input-group-sm" id="flaghead">
																											...
																											<input type="checkbox" class="custom-control-input" name="<?= $coltypemod; ?>bill" id="<?= $coltypemod; ?>bill" <?=((($newmoduleskey=='Payment Term')||($newmoduleskey=='City'))?(((in_array($newmoduleskey, $newans)))?'checked':''):'checked')?> <?=((($newmoduleskey=='Payment Term')||($newmoduleskey=='City'))?'':'disabled')?>>
																											<label class="custom-control-label custom-label" for="<?= $coltypemod; ?>bill">
																								...
																								<div class="col-lg-6 my-1">
																									<div class="custom-control custom-checkbox mr-sm-2">
																										...
																										<label class="custom-control-label custom-label" for="filtername">
																											<?=$infomainaccessuserven['modulename']?>(PRIVATE)
																										</label>
																									</div>                  
																								</div>
												...
												<a style="margin:4.5px 4.5px !important;<?= ($sqlviewreport['printtype']=='0')?'':'display: none;'?>" class="btn btn-primary btn-sm btn-custom-grey" onclick="window.open('reportbillviewsprint.php?names=<?=$_GET['names']?>&datefrom=<?=$_GET['datefrom']?>&dateto=<?=$_GET['dateto']?>&payterm=<?=$_GET['payterm']?>&amttype=<?=$_GET['amttype']?>','_blank')">
											...
											<div class="tab-pane fade" id="nav-profile" role="tabpanel" aria-labelledby="nav-profile-tab">
												<div class="table-responsive m-3" id="histables">
													<table class="table table-bordered">
														...
														<tbody>
														<?php
															$sqluse=$con->prepare("SELECT * FROM pairusehistory WHERE usetype = 'REPbill' AND useid = ? ORDER BY createdon DESC");
																			...
																			<td width="100%" style="padding: 10px;">
																				<table id="print-are1" style="border: 1px solid #eee;" width="100%">
																					...
																					<?php
																						...
																						$sqltotlistpages=$con->prepare("SELECT * FROM pairbills WHERE franchisesession=? AND createdid=? AND (billdate>=? AND billdate<=?) ".(($sqlviewreport['names']=='all')?' ':' and vendorid="'.$sqlviewreport['names'].'"')."".(($sqlviewreport['payterm']=='all')?' ':' AND billterm="'.$sqlviewreport['payterm'].'"')." GROUP BY billdate, billno");
										...
										<table id="print-are3" style="border: 1px solid #eee;display: none;" width="100%">
											<thead>
												<tr>
													...
													<td class="text-uppercase" style="border: 1px solid #eee;padding-left: 10px;"><span style="font-size:13px;color:black;"> Status</span></td>
													<td class="text-uppercase" style="border: 1px solid #eee;padding-left: 10px;"><span style="font-size:13px;color:black;"> Date</span></td>
													<td class="text-uppercase" style="border: 1px solid #eee;padding-left: 10px;display: none;"><span style="font-size:13px;color:black;"> Payment Days</span></td>
													...
													<td class="text-uppercase" style="border: 1px solid #eee;padding-left: 10px;<?=((($sqlviewreport['filtername']=='1')||($sqlviewreport['filtername']=='0'))?'':'display:none;')?>"><span style="font-size:13px;color:black;"> <?=$infomainaccessuserven['modulename']?> Name</span></td>
													<td class="text-uppercase" style="border: 1px solid #eee;padding-left: 10px;<?=(((in_array('City', $newans)))?'':'display: none;')?>"><span style="font-size:13px;color:black;"> City</span></td>
													<td class="text-uppercase" style="border: 1px solid #eee;padding-right: 10px;text-align: right;"><span style="font-size:13px;color:black;"> Total</span></td>
													...
													<td class="text-uppercase" style="border: 1px solid #eee;padding-right: 10px;text-align: right;"><span style="font-size:13px;color:black;"> Balance</span></td>
												</tr>
											</thead>
											<tbody id="myTables">
											</tbody>
...
const pagechanges = (page, pageSize) => {
	...
	var totalpagesnumber = <?=ceil($chekcspage/36)?>;
	if ((page=='')||(page==0)) {
		var isthisval = 1;
	}
	...
	ajaxRequestview = $.ajax({
		type: "GET",
		url: 'reportviewsearch.php?term='+perpages+'&limitings='+($('#limitforpagenum').val())+'&datefrom=<?=$datefrom?>&dateto=<?=$dateto?>&dif=billview&names=<?=$_GET['names']?>&lastornone='+lastornone+'&payterm=<?=$_GET['payterm']?>&amttype=<?=$_GET['amttype']?>',
...
$(function() {
	...
	function(start, end, label) {
		...
		window.open("reportbillviews.php?names=<?=$_GET['names']?>&datefrom="+datefrom+"&dateto="+dateto+"&payterm=<?=$_GET['payterm']?>&amttype=<?=$_GET['amttype']?>","_self");
	});
});
...
function converHTMLFileToPDF() {
	...
	var finaltimer = setTimeout(function(){
		...
		var header = function(data) {
			...
			var headtext1 = "<?=$infomainaccessuserbill["modulename"]?> List/Details";
			var fontSize = doc.internal.getFontSize();
			var pageWidth = doc.internal.pageSize.width;
		...
		$.ajax({
			type: "GET",
			url: 'reporthistory.php?types=bill&titles=Bill List/Details&printdownpdfcsv=Printed PDF&fromto='+fromtopdf+'',
			success: function (result) {
				console.log(result);
			},
			error: function (error) {
...
function downloadaspdf() {
	...
	var finaltimer = setTimeout(function(){
		...
		$.ajax({
			type: "GET",
			url: 'reporthistory.php?types=bill&titles=Bill List/Details&printdownpdfcsv=Downloaded PDF&fromto='+fromtopdf+'',
			success: function (result) {
				console.log(result);
			},
			error: function (error) {
		...
		doc.save('Bill-Reports-<?=$datefrom?>-To-<?=$dateto?>');
...
function downloadascsv() {
	...
	var finaltimer = setTimeout(function(){
		...
		data.push(["<?=$infomainaccessuserbill["modulename"]?> List/Details"]);
		data.push([fromtopdf]);
		...
		link.setAttribute("download", "Bill-Reports-<?=$datefrom?>-To-<?=$dateto?>");
		document.body.appendChild(link);
		link.click();
		$.ajax({
			type: "GET",
			url: 'reporthistory.php?types=bill&titles=Bill List/Details&printdownpdfcsv=Downloaded CSV&fromto='+fromtopdf+'',
			success: function (result) {
				console.log(result);
			},
			error: function (error) {
...
document.addEventListener('DOMContentLoaded', function() {
	...
	var ajaxRequest;
	var intervalingopt;
	var counteringforoptions = <?=((ceil($chekcspage/100)>0)?ceil($chekcspage/100):1)?>;
	intervalingopt = setInterval(function() {
		if ((counteringforoptions <= 0)&&($('#myTables').html()!='')) {
			if (mytabprev=='empty') {
				...
				if(mytabprev==''){
					counteringforoptions = <?=((ceil($chekcspage/200)>0)?ceil($chekcspage/200):1)?>;
					setInterval();
				}
				else{
...
?>
	function makeAjaxRequest() {
		ajaxRequest = $.ajax({
			type: "GET",
			url: 'check.php?term=billview&datefrom=<?=$datefrom?>&dateto=<?=$dateto?>&limitfrom=0&limitto=10000&names=<?=$_GET['names']?>&payterm=<?=$_GET['payterm']?>&amttype=<?=$_GET['amttype']?>',
			success: function (result) {
				if (result=='') {
					mytabprev = 'empty';
				}
