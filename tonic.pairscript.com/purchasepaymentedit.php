...
$infomainaccessuserbill=mysqli_fetch_array($sqlismainaccessuserbill);
     if(isset($_GET['id']))
{
$sql=mysqli_query($con,"select id, createdon, term, receiptdate, receiptno, cancelstatus, vendorid, vendorname, paymentmode, amount, notes,publicid,privateid,type from pairpurchasepayments where franchisesession='".$_SESSION['franchisesession']."' and createdid='$companymainid' and id='".$_GET['id']."'");
...
function checkDueDate($billdateans, $duedateans) {
    ...
    echo '<th style="border:1px solid #ddd !important;vertical-align:middle !important;"><span style="display:inline-block;font-size:13px;color:black;font-weight: normal;">'. strtoupper($infomainaccesspurchasebill['modulename']) .' DATE</span></th>';
    echo '<th style="border:1px solid #ddd !important;vertical-align:middle !important;"><span style="display:inline-block;font-size:13px;color:black;font-weight: normal;">'. strtoupper($infomainaccesspurchasebill['modulename']) .' NUMBER</span></th>';
    echo '<th style="border:1px solid #ddd !important;vertical-align:middle !important;"><span style="display:inline-block;font-size:13px;color:black;font-weight: normal;">INVOICE NUMBER</span></th>';
    ...
    echo '<th style="border:1px solid #ddd !important;vertical-align:middle !important;width:215px;"><span style="display:inline-block;font-size:13px;color:black;font-weight: normal;text-align:right;width:100%;">'. strtoupper($infomainaccesspurchasebill['modulename']) .' AMOUNT</span></th>';
...
foreach ($bills as $bill) {
    ...
    $sqls=mysqli_query($con,"select duedate,invnumber,billno, billdate, grandtotal, paidstatus, vendorid, vendorname from pairbills where franchisesession='".$_SESSION['franchisesession']."' and createdid='$companymainid' and vendorid='$vendorid' and  billno='".$bill['billno']."' and billdate='".$bill['billdate']."' GROUP BY billdate, billno  order by billdate asc, billno asc");
    while($infos=mysqli_fetch_array($sqls))
    {
        ...
        <input type="hidden" name="invnos[]" id="inv'.$i.'" value="'.$infos['invnumber'].'">
        ...
        echo '<td data-label="'. strtoupper($infomainaccesspurchasebill['modulename']) .' DATE" style="vertical-align:middle !important;text-align:left;">'.date('d/m/Y',strtotime($infos['billdate'])).'<br>'.checkDueDate($infos['billdate'], $infos['duedate']).'</td>';
        echo '<td data-label="'. strtoupper($infomainaccesspurchasebill['modulename']) .' NUMBER" style="vertical-align:middle !important;text-align:left;">'.$infos['billno'].'</td>';
...
<!-- <button name="submit"
                                                          ...
                                                          <a class="btn btn-primary btn-sm btn-custom-grey" href="purchasepayments.php" style="font-family:'Myriad Set Pro','Helvetica Neue',Helvetica,Arial,sans-serif;">Cancel</a>
    </div>
</div>
...
?>
  ...
  <div class="row justify-content-center">
    <div class="col-lg-6">
        <div class="form-group row">
            <div class="col-sm-4">
            <label for="advvendorname" class="custom-label" style="font-size:13.6px;font-family:'Myriad Set Pro','Helvetica Neue',Helvetica,Arial,sans-serif;"><span class="text-danger">Vendor Name *</span></label>
            </div>
            <div class="col-sm-8">
              <div class="col-sm-12" onclick="andus()">
...
<input type="hidden" name="advid" id="advid" value="<?=$info['id']?>">
  ...
  <div class="row justify-content-center">
    <div class="col-lg-6">
        <div class="form-group row">
            <div class="col-sm-4">
            <label for="advvendorname" class="custom-label text-danger" style="font-family:'Myriad Set Pro','Helvetica Neue',Helvetica,Arial,sans-serif;">Date *</label>
            </div>
            <div class="col-sm-8">
...
<input type="number" step="any" class="form-control  form-control-sm" id="advamount" name="advamount" placeholder="Amount Received" min='1' required value="<?=$info['amount']?>" style="border-left: 0px solid #ced4da !important;border-radius: 0px !important;margin-left: 0px !important;padding-left: 3px;">
      ...
      <div class="form-group row" <?=((in_array('Notes', $fieldedit))?'':'style="display:none;"')?>>
        ...
        <div class="row justify-content-center" style="margin-bottom: -14px;">
    <div class="col-lg-12"><hr>
        <button name="advadvvenpayeditsubmit" class="btn btn-primary btn-sm btn-custom arlina-button expand-left" type="submit" id="advadvvenpayeditsubmit" value="Save" style="margin-bottom: 15px;font-family:'Myriad Set Pro','Helvetica Neue',Helvetica,Arial,sans-serif;">
                                                            <span class="label">Save</span> <span
                                                                class="spinner"></span>
