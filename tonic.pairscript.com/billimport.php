...
if(!empty($_FILES['billfile'] ['name']) && in_array($_FILES['billfile'] ['type'], $csvMimes)){
if(is_uploaded_file($_FILES['billfile'] ['tmp_name'])){
$csvFile=fopen($_FILES['billfile'] ['tmp_name'],'r');
fgetcsv($csvFile);
while(($line=fgetcsv($csvFile))!==FALSE){

$billdate=mysqli_real_escape_string($con, $line[0]);    
$billno=mysqli_real_escape_string($con, $line[1]);
// $billterm=mysqli_real_escape_string($con, $line[2]);
$duedate=mysqli_real_escape_string($con, $line[3]);
$orderdcno=mysqli_real_escape_string($con, $line[4]);
$billamount=mysqli_real_escape_string($con, $line[5]);
...
$billitemmodule=mysqli_real_escape_string($con, $line[55]);
...
$cussel = mysqli_query($con,"select * from paircustomers where customername='$customername' and franchisesession='".$_SESSION["franchisesession"]."' and moduletype='Vendors'");
...
$sqlins = mysqli_query($con, "select count(customercode) from paircustomers where moduletype='Vendors'");
$ansins = mysqli_fetch_array($sqlins);
$oldid = $ansins[0];
$customercode = $oldid + 1;
$publicsql=mysqli_query($con,"select count(publicid) from paircustomers where createdid='$companymainid' and moduletype='Vendors'");
$publicans=mysqli_fetch_array($publicsql);
$oldcodepublic=$publicans[0];
$publiccode=$infomodulespublicnamecust['publiccolumn'] . $oldcodepublic+1;
$privatesql=mysqli_query($con,"select count(privateid) from paircustomers where createdid='$companymainid' and moduletype='Vendors' and franchisesession='".$_SESSION['franchisesession']."'");
$privateans=mysqli_fetch_array($privatesql);
$oldcodeprivate=$privateans[0];
$privatecode=$infomainaccesspublicnamecust['moduleprefix'] . $oldcodeprivate+1;

$sqlup = mysqli_query($con,"insert into paircustomers set createdon='$times', createdid='$companymainid', createdby='" . $_SESSION["unqwerty"] . "', franchisesession='" . $_SESSION["franchisesession"] . "',customername='$customername',cvisiblity='PRIVATE',billstreet='$area',billcity='$city',billstate='$state',billpincode='$pincode',billcountry='$district',customercode='$customercode',gstrtype='Registered Business - Regular',sameasbilling='0',moduletype='Vendors',publicid='$publiccode',privateid='$privatecode'");
$customerid=mysqli_insert_id($con);
}
else{
$customerid=$cusids['id'];
}
$selcuspos = mysqli_query($con,"select * from paircustomers where createdid='$companymainid' and moduletype='Vendors' and id='$customerid'");
$fetcuspos = mysqli_fetch_array($selcuspos);
$pos = $fetcuspos['placeos'];
$sqlbill = mysqli_query($con,"insert into pairbills set createdon='$times', createdid='$companymainid', createdby='".$_SESSION["unqwerty"]."', franchisesession='".$_SESSION["franchisesession"]."', vendorname='$customername', vendorid='$customerid', area='$area', city='$city', state='$state', district='$district', pincode='$pincode', gstno='$gstno', billterm='CASH', billno='$billno', billdate='$billdate', billamount='$billamount', duedate='$duedate', productid='$productid', productname='$productname', manufacturer='$manufacturer', producthsn='$producthsn', itemmodule='$billitemmodule', batch='$batch', expdate='$expdate', mrp='$mrp', vat='$vat', quantity='$quantity',unit='PCS', productrate='$productrate', noofpacks='$noofpacks', prodiscount='$prodiscount', productvalue='$productvalue',  taxvalue='0.00',  productnetvalue='$productvalue', totalitems='$totalitems', totalvatamount='$totalvatamount', totalamount='$totalamount', totalquantity='$totalitems', discounttype='0', discount='$discount', discountamount='$discountamount', freightamount='$freightamount', roundoff='$roundoff', grandtotal='$grandtotal', taxtype='$taxtype', tax='$tax',cgst='$cgst',sgst='$sgst', igst='$igst',gst='$gst',gstpercent='$gstpercent',csgstpercent='$csgstpercent',pos='$pos',sameasbilling='0',vendorinfodefault='two',gstrtype='Registered Business - Regular',validpaidamount='$grandtotal',validbalance='0',cstno='$cstno',paidstatus='$paidstatus',dlno20='$dlno20',dlno21='$dlno21',salequantity='$quantity'");
// $sql4=mysqli_query($con, "update pairproducts set openingstock=openingstock+$quantity where id='$productid'");
if ($sqlbill) {
$sqlismodulespublicnamepayments=mysqli_query($con, "select * from pairmodules where moduletype='Payments Made' order by id  asc");
$infomodulespublicnamepaymentspayments=mysqli_fetch_array($sqlismodulespublicnamepayments);
$sqlismainaccesspublicnamepayments=mysqli_query($con, "select * from pairmainaccess where createdid='$companymainid' and moduletype='Payments Made' and franchiseid='".$_SESSION['franchisesession']."' order by id  asc");
$infomainaccesspublicnamepayments=mysqli_fetch_array($sqlismainaccesspublicnamepayments);
$publicsqlpayments=mysqli_query($con,"select count(publicid) from pairpurchasepayments where createdid='$companymainid'");
...
$privatesqlpayments=mysqli_query($con,"select count(privateid) from pairpurchasepayments where createdid='$companymainid' and franchisesession='".$_SESSION['franchisesession']."'");
...
$selpayments = mysqli_query($con,"select receiptno,id from pairpurchasepayments where receiptno='$billno' and receiptdate='$billdate'");
$selpaymentfetch = mysqli_fetch_array($selpayments);
if (mysqli_num_rows($selpayments)>0) {
$sqlpay=mysqli_query($con, "update pairpurchasepayments set createdid='$companymainid', createdby='".$_SESSION["unqwerty"]."', franchisesession='".$_SESSION["franchisesession"]."', createdon='$times',  term='RECEIPT', type='bill', vendorname='$customername', vendorid='$customerid', receiptno='$billno', receiptdate='$billdate', amount='$grandtotal', paymentmode='CASH', notes='-' where receiptno='$billno' and receiptdate='$billdate'");
$paymentid=$selpaymentfetch['id'];
}
else{
$sqlpay=mysqli_query($con, "insert into pairpurchasepayments set createdid='$companymainid', createdby='".$_SESSION["unqwerty"]."', franchisesession='".$_SESSION["franchisesession"]."', createdon='$times',  term='RECEIPT', type='bill', vendorname='$customername', vendorid='$customerid', receiptno='$billno', receiptdate='$billdate', amount='$grandtotal', paymentmode='CASH', notes='-',publicid='$publiccodepayments',privateid='$privatecodepayments'");
$paymentid=mysqli_insert_id($con);
}
if($sqlpay)
{
$selpayhis = mysqli_query($con,"select billno from pairpurchasepayhistory where billno='$billno' and billdate='$billdate'");
if (mysqli_num_rows($selpayhis)>0) {
$sqlhis=mysqli_query($con,"update pairpurchasepayhistory set createdid='$companymainid', createdby='".$_SESSION["unqwerty"]."', franchisesession='".$_SESSION["franchisesession"]."', createdon='$times', vendorid='$customerid', billno='$billno', billdate='$billdate', amount='$grandtotal' where billno='$billno' and billdate='$billdate'");
}
else{
$sqlhis=mysqli_query($con,"insert into pairpurchasepayhistory set createdid='$companymainid', createdby='".$_SESSION["unqwerty"]."', franchisesession='".$_SESSION["franchisesession"]."', createdon='$times', paymentid='$paymentid', vendorid='$customerid', billno='$billno', billdate='$billdate', amount='$grandtotal'");
}
}
}
}
...
if($sqlbill){
header("Location: bills.php?remarks=Imported Successfully");
}
else{
header("Location: bills.php?error=".mysqli_error($con));
}
}
}
else{
// $all="invalid bill type";
}
}
...
// $billdate=mysqli_real_escape_string($con, $line[5]);
// $billno=mysqli_real_escape_string($con, $line[6]);
// $billterm=mysqli_real_escape_string($con, $line[7]);
// $duedate=mysqli_real_escape_string($con, $line[8]);
// $duedates=mysqli_real_escape_string($con, $line[9]);
// $billamount=mysqli_real_escape_string($con, $line[10]);
// $vendorname=mysqli_real_escape_string($con, $line[11]);
// $vendorid=mysqli_real_escape_string($con, $line[12]);
...
// $vendorinfodefault=mysqli_real_escape_string($con, $line[61]);
// $sameasbilling=mysqli_real_escape_string($con, $line[62]);
// $pos=mysqli_real_escape_string($con, $line[63]);
// $unit=mysqli_real_escape_string($con, $line[64]);
// $invnumber=mysqli_real_escape_string($con, $line[65]);
// $invgrandtotal=mysqli_real_escape_string($con, $line[66]);
// $validpaidamount=mysqli_real_escape_string($con, $line[67]);
// $validbalance=mysqli_real_escape_string($con, $line[68]);
// $dlno20=mysqli_real_escape_string($con, $line[69]);
// $dlno21=mysqli_real_escape_string($con, $line[70]);
// $cstno=mysqli_real_escape_string($con, $line[71]);
// $manufacturer=mysqli_real_escape_string($con, $line[72]);
// $mrp=mysqli_real_escape_string($con, $line[73]);
// $noofpacks=mysqli_real_escape_string($con, $line[74]);
// $prodiscount=mysqli_real_escape_string($con, $line[75]);
// $discount=mysqli_real_escape_string($con, $line[76]);
// $discountamount=mysqli_real_escape_string($con, $line[77]);
// $freightamount=mysqli_real_escape_string($con, $line[78]);
// $roundoff=mysqli_real_escape_string($con, $line[79]);
// $editedon=mysqli_real_escape_string($con, $line[80]);
// $sqlbill="INSERT INTO pairbills set franchisesession='$franchisesession',createdon='$createdon',createdid='$createdid',createdby='$createdby',paidstatus='$paidstatus',billdate='$billdate',billno='$billno',billterm='$billterm',duedate='$duedate',duedates='$duedates',billamount='$billamount',vendorname='$vendorname',vendorid='$vendorid',area='$area',city='$city',pincode='$pincode',state='$state',district='$district',sarea='$sarea',scity='$scity',sdistrict='$sdistrict',sstate='$sstate',spincode='$spincode',gstno='$gstno',gstrtype='$gstrtype',mobile='$mobile',workphone='$workphone',batch='$batch',expdate='$expdate',productid='$productid',productname='$productname',producthsn='$producthsn',itemmodule='$itemmodule',vat='$vat',quantity='$quantity',productrate='$productrate',productvalue='$productvalue',taxvalue='$taxvalue',productnetvalue='$productnetvalue',totalitems='$totalitems',totalvatamount='$totalvatamount',taxtype='$taxtype',cgst25='$cgst25',sgst25='$sgst25',gst25='$gst25',cgst6='$cgst6',sgst6='$sgst6',gst6='$gst6',cgst9='$cgst9',sgst9='$sgst9',gst9='$gst9',cgst14='$cgst14',sgst14='$sgst14',gst14='$gst14',tax25='$tax25',tax6='$tax6',tax9='$tax9',tax14='$tax14',totalamount='$totalamount',totalquantity='$totalquantity',grandtotal='$grandtotal',pos='$pos',sameasbilling='$sameasbilling',vendorinfodefault='$vendorinfodefault',unit='$unit',invnumber='$invnumber',invgrandtotal='$invgrandtotal',validpaidamount='$validpaidamount',validbalance='$validbalance',dlno20='$dlno20',dlno21='$dlno21',cstno='$cstno',manufacturer='$manufacturer',mrp='$mrp',noofpacks='$noofpacks',prodiscount='$prodiscount',discount='$discount',discountamount='$discountamount',freightamount='$freightamount',roundoff='$roundoff',editedon='$editedon'";
// $resultbill=mysqli_query($con,$sqlbill);
if (isset($_POST['billmigratesubmit'])) {
	$csvMimes = array('text/x-comma-separated-values', 'text/comma-separated-values', 'application/octet-stream', 'application/vnd.ms-excel', 'application/x-csv', 'text/x-csv', 'text/csv', 'application/csv', 'application/excel', 'application/vnd.msexcel', 'text/plain');
	if(!empty($_FILES['migrationbillfile'] ['name']) && in_array($_FILES['migrationbillfile'] ['type'], $csvMimes)){
		if(is_uploaded_file($_FILES['migrationbillfile'] ['tmp_name'])){
			$csvFile=fopen($_FILES['migrationbillfile'] ['tmp_name'],'r');
			fgetcsv($csvFile);
			while(($line=fgetcsv($csvFile))!==FALSE){
				$sesunqwerty = $_SESSION['unqwerty'];
				$franchsess = $_SESSION["franchisesession"];
				$vendorname = mysqli_real_escape_string($con, $line[4]);
				$vendorid = mysqli_real_escape_string($con, $line[5]);
				...
				$billterm = mysqli_real_escape_string($con, $line[21]);
				$billno = mysqli_real_escape_string($con, $line[22]);
				$billdate = mysqli_real_escape_string($con, (date('Y-m-d',strtotime($line[23]))));
				$billamount = mysqli_real_escape_string($con, $line[24]);
				...
				$invnumber = mysqli_real_escape_string($con, $line[86]);
				$invgrandtotal = mysqli_real_escape_string($con, $line[87]);
				$salequantity = mysqli_real_escape_string($con, $line[88]);
				$productwithouttax = mysqli_real_escape_string($con, $line[89]);
				$productwithtax = mysqli_real_escape_string($con, $line[90]);
				$chartaccountid = mysqli_real_escape_string($con, $line[91]);
				$accountname = mysqli_real_escape_string($con, $line[92]);
				$ordering = mysqli_real_escape_string($con, $line[93]);
				$dlno20 = mysqli_real_escape_string($con, $line[94]);
				$dlno21 = mysqli_real_escape_string($con, $line[95]);
				$cancelstatus = mysqli_real_escape_string($con, $line[96]);
				...
				if($productname!=''){
					$sql = $con->prepare("INSERT INTO pairbills (createdon,createdid,createdby,franchisesession,vendorname,vendorid,address1,address2,area,city,state,district,pincode,saddress1,saddress2,sarea,scity,sstate,sdistrict,spincode,gstno,billterm,billno,billdate,billamount,duedate,duedates,productid,productname,manufacturer,producthsn,productnotes,productdescription,itemmodule,rack,batch,expdate,mrp,vat,quantity,unit,productrate,noofpacks,prodiscount,productvalue,taxvalue,cgstvat,sgstvat,productnetvalue,totalitems,orderdcno,reference,saleperson,totalvatamount,totalamount,totalquantity,discounttype,discount,discountamount,freightamount,roundoff,grandtotal,preparedby,checkedby,taxtype,tax,cgst,sgst,igst,gst,gstpercent,csgstpercent,terms,notes,description,fileattach,pos,workphone,mobile,sameasbilling,vendorinfodefault,gstrtype,validpaidamount,validbalance,icsgsthis,prodiscounttype,invnumber,invgrandtotal,salequantity,productwithouttax,productwithtax,accountid,accountname,ordering,dlno20,dlno21,cancelstatus) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
				    $sql->bind_param("sssssssssssssssssssssssssssssssssssssssisssssssssssssssssssssssssssssssssssssssssssssssssssissssi", $times,$companymainid,$sesunqwerty,$franchsess,$vendorname,$vendorid,$address1,$address2,$area,$city,$state,$district,$pincode,$saddress1,$saddress2,$sarea,$scity,$sstate,$sdistrict,$spincode,$gstno,$billterm,$billno,$billdate,$billamount,$duedate,$duedates,$productid,$productname,$manufacturer,$producthsn,$productnotes,$productdescription,$itemmodule,$rack,$batch,$expdate,$mrp,$vat,$quantity,$unit,$productrate,$noofpacks,$prodiscount,$productvalue,$taxvalue,$cgstvat,$sgstvat,$productnetvalue,$totalitems,$orderdcno,$reference,$saleperson,$totalvatamount,$totalamount,$totalquantity,$discounttype,$discount,$discountamount,$freightamount,$roundoff,$grandtotal,$preparedby,$checkedby,$taxtype,$tax,$cgst,$sgst,$igst,$gst,$gstpercent,$csgstpercent,$terms,$notes,$description,$fileattach,$pos,$workphone,$mobile,$sameasbilling,$vendorinfodefault,$gstrtype,$validpaidamount,$validbalance,$icsgsthis,$prodiscounttype,$invnumber,$invgrandtotal,$salequantity,$productwithouttax,$productwithtax,$chartaccountid,$accountname,$ordering,$dlno20,$dlno21,$cancelstatus);
					if($sql->execute()){
						header("Location: bills.php?remarks=Imported Successfully");
					}
					else{
...
<div class="card-body p-3">
 ...
 <p class="mb-3" style="color:black;font-size:20px;margin-top: -8px;"> <i class="fa fa-file-import"></i> Bill Import</p>

<div class="alert alert-info text-white">
Kindly Note that the New Bill List should contains existing Bill Name, otherwise it has been Overrided.
</div>
...
<div class="col-sm-4">
<label for="billfile" class="custom-label"><span class="text-danger">Bills CSV File *</span></label>
</div>
<div class="col-sm-8">
<input type="file" class="form-control  form-control-sm" required id="billfile" name="billfile" accept=".csv">
</div>
</div>
</div>
</div>
...
<input class="btn btn-primary btn-sm btn-custom" type="submit" name="billsubmit" value="Save">  <a class="btn btn-primary btn-sm btn-custom-grey" href="bills.php">Cancel</a>
</div>
</div>
</form>
...
<div class="customcont-header ml-0 mb-1">
<a class="customcont-heading">Bill Migrate</a>	      
</div>       
</button>
</h5>
...
<div class="form-group row">
<div class="col-sm-4">
<label for="migrationbillfile" class="custom-label"><span class="text-danger">Bills CSV File *</span></label>
</div>
<div class="col-sm-8">
<input type="file" class="form-control  form-control-sm" required id="migrationbillfile" name="migrationbillfile" accept=".csv">
</div>
</div>
</div>
</div>
...
<div class="row justify-content-center">
<div class="col-lg-12"><hr>
<input class="btn btn-primary btn-sm btn-custom" type="submit" name="billmigratesubmit" value="Save">  <a class="btn btn-primary btn-sm btn-custom-grey" href="bills.php">Cancel</a>
</div>
</div>
</form>   
</div>
