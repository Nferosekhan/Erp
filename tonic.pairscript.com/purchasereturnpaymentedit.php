...
<form action="purchasereturnpaymentadds.php" onsubmit="return checkvalidate()" method="post" enctype="multipart/form-data" class="form-horizontal" role="form">
...
<?php
$sqli = mysqli_query($con, "SELECT id, customername, city, mobile, workphone From paircustomers WHERE (franchisesession='".$_SESSION["franchisesession"]."' or cvisiblity='PUBLIC') and (createdid='$companymainid' and moduletype='Vendors') order by customername asc");
...
?>
    ...
    <div class="col-lg-6">
        <div class="form-group row">
            <div class="col-sm-4">
            <label for="vendorname" class="custom-label" style="font-family:'Myriad Set Pro','Helvetica Neue',Helvetica,Arial,sans-serif;">Date</label>
            </div>
            <div class="col-sm-8">
...
if($vendorid!='')
{
    ...
    echo '<thead>';
	echo '<tr class="info">';
	echo '<th style="border:1px solid #ddd !important;vertical-align:middle !important;"><span style="display:inline-block;width:77px;">'. strtoupper($infomainaccesspurchasereturn['modulename']) .' NO</span></th>';
	echo '<th style="border:1px solid #ddd !important;vertical-align:middle !important;"><span style="display:inline-block;width:77px;">'. strtoupper($infomainaccesspurchasereturn['modulename']) .' DATE</span></th>';
	...
	echo '<th style="border:1px solid #ddd !important;vertical-align:middle !important;"><span style="display:inline-block;width:77px;">'. strtoupper($infomainaccesspurchasereturn['modulename']) .' AMOUNT</span></th>';
    ...
    echo '</thead>';
	$i=0;
	$sqls=mysqli_query($con,"select purchasereturnno, purchasereturndate, grandtotal, paidstatus, vendorid, vendorname from pairpurchasereturns where franchisesession='".$_SESSION['franchisesession']."' and createdid='$companymainid' and vendorid='$vendorid' GROUP BY purchasereturndate, purchasereturnno  order by purchasereturndate asc, purchasereturnno asc");
	while($infos=mysqli_fetch_array($sqls))
	{
		$s=0;
		if($infos['paidstatus']=='1')
		{
			$sqlse=mysqli_query($con,"select paymentid from pairpurchasereturnpayhistory where franchisesession='".$_SESSION['franchisesession']."' and createdid='$companymainid' and  purchasereturnno='".$infos['purchasereturnno']."' and purchasereturndate='".$infos['purchasereturndate']."' and vendorid='$vendorid' and paymentid='".$_GET['id']."'");
		...
		$p='';
		$p1=0;
		$sqlse=mysqli_query($con,"select amount, paymentid from pairpurchasereturnpayhistory where franchisesession='".$_SESSION['franchisesession']."' and createdid='$companymainid' and  purchasereturnno='".$infos['purchasereturnno']."' and purchasereturndate='".$infos['purchasereturndate']."' and vendorid='$vendorid'");
		...
		echo '<input type="hidden" name="nos[]" id="purchasereturn'.$i.'" value="'.$infos['purchasereturnno'].'">
		<input type="hidden" name="dates[]" id="date'.$i.'" value="'.$infos['purchasereturndate'].'">
		<input type="hidden" name="status[]" id="status'.$i.'" value="'.$sta.'">';
        echo '<tr>';
        echo '<td data-label="'. strtoupper($infomainaccesspurchasereturn['modulename']) .' NO" style="vertical-align:middle !important;">'.$infos['purchasereturnno'].'</td>';
        echo '<td data-label="'. strtoupper($infomainaccesspurchasereturn['modulename']) .' DATE" style="vertical-align:middle !important;">'.date('d/m/Y',strtotime($infos['purchasereturndate'])).'</td>';
...
function getdata()
{
  ...
  $.ajax({ /* THEN THE AJAX CALL */
        ...
        url: "purchasereturngetdata.php", /* PAGE WHERE WE WILL PASS THE DATA */
