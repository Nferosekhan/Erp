...
$oldprojectno=mysqli_real_escape_string($con, $_POST['oldprojectno']);
$oldprojectdate=mysqli_real_escape_string($con, $_POST['oldprojectdate']);
...
$sql2=mysqli_query($con, "delete from pairprojects where franchisesession='".$_SESSION['franchisesession']."' and createdid='$companymainid' and projectno='$oldprojectno' and projectdate='$oldprojectdate'");
...
echo '<script> window.location.href="projects.php?remarks=Updated Successfully";</script>'; 
...
$infomainaccessuser=mysqli_fetch_array($sqlismainaccessuser);
     ?>
 <?php
 if((isset($_GET['projectno']))&&(isset($_GET['projectdate'])))
 {
	 $projectno=mysqli_real_escape_string($con, $_GET['projectno']);
	 $projectdate=mysqli_real_escape_string($con, $_GET['projectdate']);
 $sql=mysqli_query($con, "select * from pairprojects where franchisesession='".$_SESSION['franchisesession']."' and createdid='$companymainid' and projectno='$projectno' and projectdate='$projectdate' order by id asc");
$count=1;
...
<p class="mb-3" style="font-size: 20px;"><i class="fa fa-pencil-square-o"></i> Edit <?= $infomainaccessuser['modulename'] ?></p>
<?php
$sqlismainaccessuser=mysqli_query($con, "select * from pairmainaccess where franchiseid='".$_SESSION['franchisesession']."' and moduletype='Projects' order by id  asc");
$infomainaccessuser=mysqli_fetch_array($sqlismainaccessuser);
...
<hr>
<input type="hidden" name="oldprojectno" value="<?=$rows[0]['projectno']?>">
<input type="hidden" name="oldprojectdate" value="<?=$rows[0]['projectdate']?>">
...
<div class="accordion" id="accordionRental">
	...
	<div class="form-group row">
            ...
            <div class="col-sm-8">
              <input type="date" class="form-control  form-control-sm" id="projectdate" name="projectdate"  required value="<?=$rows[0]['projectdate']?>">
		  ...
		  <div class="form-group row">
            ...
            <div class="col-sm-8">
              <input type="text" class="form-control  form-control-sm" id="projectno" name="projectno" required value="<?=$rows[0]['projectno']?>" readonly>	  
            </div>
			</div>
        ...
        <div class="form-group row">
            ...
            <div class="col-sm-8">
              <input type="text" class="form-control  form-control-sm" id="projectname" name="projectname" placeholder="Name" required value="<?=$rows[0]['projectname']?>">
            ...
            <div class="col-sm-8">
              <textarea class="form-control  form-control-sm" id="projectdescription1" name="projectdescription" placeholder="Description" style="height:100px;"><?=$rows[0]['projectdescription']?></textarea>
            ...
            <div class="col-sm-8">
              <input type="text" class="form-control  form-control-sm" id="customername" name="customername" placeholder="Customer Name" required value="<?=$rows[0]['customername']?>">
		  ...
		  <div class="form-group row">
            ...
            <div class="col-sm-8">
			  ...
			  <option value="Fixed Cost for Project" <?=($rows[0]['billingmethod']=='Fixed Cost for Project')?'selected':''?>>Fixed Cost for Project</option>
			  <option value="Based on Project Hours" <?=($rows[0]['billingmethod']=='Based on Project Hours')?'selected':''?>>Based on Project Hours</option>
			  <option value="Based on Task Hours" <?=($rows[0]['billingmethod']=='Based on Task Hours')?'selected':''?>>Based on Task Hours</option>
			  <option value="Based on Staff Hours" <?=($rows[0]['billingmethod']=='Based on Staff Hours')?'selected':''?>>Based on Staff Hours</option>			  
			  </select>
            </div>
			</div>
			<script>
    ...
    for (var i = 0; i < elements.length; i++){
        elements[i].style.display = displayState;
            ...
            <div class="col-sm-8">
              <input type="number" min="0" step="0.01" class="form-control  form-control-sm" id="projectcost" name="projectcost" placeholder="Total Project Cost" value="<?=$rows[0]['projectcost']?>">
            ...
            <div class="col-sm-8">
              <input type="number" min="0" step="0.01" class="form-control  form-control-sm" id="rateperhour" name="rateperhour" placeholder="Rate Per Hour" value="<?=$rows[0]['rateperhour']?>">
...
<td><input type="hidden" name="userid[]" id="userid<?=$i?>" value="<?=$row['userid']?>"><input type="text" name="username[]" id="username<?=$i?>" required class="form-control form-control-sm bordernoneinput" value="<?=$row['username']?>" <?=($row['userid']==$companymainid)?'disabled':''?> ></td>
<td><input type="text" name="useremail[]" id="useremail<?=$i?>" class="form-control form-control-sm bordernoneinput" required value="<?=$row['useremail']?>" <?=($row['userid']==$companymainid)?'disabled':''?>></td>
<td><input type="text" name="userrateperhour[]" id="userrateperhour<?=$i?>" class="form-control form-control-sm bordernoneinput" value="<?=$row['userrateperhour']?>"></td>
...
<tr>
<td class="priority" style="display:none"> <?=$i?></td>
<td><svg version="1.1" id="Layer_<?=$i?>" xmlns="http://www.w3.org/2000/svg" x="0" y="0" viewBox="0 0 512 512" xml:space="preserve" class="icon icon-drag align-text-bottom" style="color:#cccccc"><circle cx="153.6" cy="451" r="61"></circle><circle cx="153.6" cy="256" r="61"></circle><circle cx="153.6" cy="61" r="61"></circle><circle cx="358.4" cy="256" r="61"></circle><circle cx="358.4" cy="61" r="61"></circle><circle cx="358.4" cy="451" r="61"></circle></svg></td>
<td><input type="text" name="taskname[]" id="taskname<?=$i?>" required class="form-control form-control-sm bordernoneinput" value="<?=$row['taskname']?>" ></td>
<td><input type="text" name="taskdescription[]" id="taskdescription<?=$i?>" class="form-control form-control-sm bordernoneinput" required value="<?=$row['taskdescription']?>"></td>
<td><input type="text" name="taskrateperhour[]" id="taskrateperhour<?=$i?>" class="form-control form-control-sm bordernoneinput" value="<?=$row['taskrateperhour']?>"></td>
<td><select name="taskbillable[]" id="taskbillable<?=$i?>" class="form-control form-control-sm bordernoneinput"><option value="Non-Billable" <?=($row['taskbillable']=='Non-Billable')?'selected':''?>>Non-Billable</option><option value="Billable" <?=($row['taskbillable']=='Billable')?'selected':''?>>Billable</option></select></td>
<td><a class="btn-delete" style="cursor:pointer"><img src="assets/img/delete-row.png" width="15" height="15" style="border-radius: 10px;"></a></td>
</tr>
<?php
$i++;
