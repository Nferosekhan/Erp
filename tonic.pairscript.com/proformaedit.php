...
$oldproformano=mysqli_real_escape_string($con, $_GET['proformano']);
$oldproformadate=mysqli_real_escape_string($con, $_GET['proformadate']);
$proformano=mysqli_real_escape_string($con, $_POST['proformano']);
...
if($productname!='')
{
$sql=mysqli_query($con, "delete from pairproformas where createdid='$companymainid' and franchisesession='".$_SESSION["franchisesession"]."' and proformano='$oldproformano' and proformadate='$oldproformadate'");
if($sql)
{
}
else
...
$sql2=mysqli_query($con, "select id from pairproformas where franchisesession='".$_SESSION['franchisesession']."' and createdid='$companymainid' and proformano='$oldproformano' and proformadate='$oldproformadate'");
...
echo '<script> window.location.href="proformas.php?remarks=Updated Successfully";</script>'; 
}
else
{
echo '<script> window.location.href="proformaview.php?proformano='.$proformano.'&proformadate='.$proformadate.'&remarks=Updated Successfully";</script>';
}
}
}
else
...
if ($sql) {
$hissql=mysqli_query($con, "insert into pairusehistory set usetype='PROFORMAS', createdon='$times',  createdby='".$_SESSION["unqwerty"]."', useid='$proformano', useremarks='Proforma Invoice Updated' ");
}
}
...
<input type="hidden" name="oldproformano" value="<?=$rows[0]['proformano']?>">
<input type="hidden" name="oldproformadate" value="<?=$rows[0]['proformadate']?>">
<input type="hidden" name="proformaterm" value="<?=$rows[0]['proformaterm']?>">

<div class="row">
        <?php
     if ((in_array('Proforma Invoice Information', $fieldedit))) {
        ?>
<div class="col-lg-4">
...
<div class="col-lg-7">
<input type="text" class="form-control  form-control-sm" id="proformano" name="proformano" required  value="<?=$rows[0]['proformano']?>" readonly>
</div>
</div>
</div>
<div class="col-lg-12">
...
<div class="col-lg-7">
<input type="date" class="form-control  form-control-sm" id="proformadate" name="proformadate" required value="<?=$rows[0]['proformadate']?>">
</div>
</div>
</div>
...
?>
<option value="<?= $info['term'] ?>" <?=($info['term']==$rows[0]['proformaterm'])?'selected':''?>><?= $info['term'] ?></option>
<?php
}
?>
</select>
...
<input type="radio" class="custom-control-input" name="customerinfodefault" id="onecustomerinfo" value="one" <?= ($rows[0]['customerinfodefault']=='one'||$infomainaccessuser['proformacustinfo']=='defone')?'checked':'' ?> onclick="onetwo()">
<label class="custom-control-label custom-label" for="onecustomerinfo">B2B</label>
</div>
</div>
<div class="col-lg-2 col-md-2 col-3">
<div class="custom-control custom-radio mr-sm-2">
<input type="radio" class="custom-control-input" name="customerinfodefault" id="twocustomerinfo" value="two" <?= ($rows[0]['customerinfodefault']=='two'||$infomainaccessuser['proformacustinfo']=='deftwo')?'checked':'' ?> onclick="onetwo()">
...
while ($info = mysqli_fetch_array($sqli))
{
$sqliproforma=mysqli_query($con, "select proformaamount, proformadate, proformano from pairproformas where franchisesession='".$_SESSION['franchisesession']."' and createdid='$companymainid' and customerid='".$info['id']."' GROUP BY proformadate, proformano order by proformadate desc, proformano desc");
$proformaamount=0;
$balanceamount=0;
$currentamount=0;
$overdueamount=0;
...
{
$proformaamount+=(float)$infoproforma['proformaamount'];
$paidamount=0;
$sqlsalespay=mysqli_query($con,"select amount from pairsalespayhistory where franchisesession='".$_SESSION['franchisesession']."' and createdid='$companymainid' and invoiceno='".$infoproforma['proformano']."' and invoicedate='".$infoproforma['proformadate']."' and customerid='".$info['id']."' order by id desc");
while($infosalespay=mysqli_fetch_array($sqlsalespay))
{
$paidamount+=(float)$infosalespay['amount'];
}
$balanceamount+=((float)$infoproforma['proformaamount']-$paidamount);
$diff = abs(time() - strtotime($infoproforma['proformadate']));
$days = floor(($diff)/ (60*60*24));
if($days>30)
{
$overdueamount+=((float)$infoproforma['proformaamount']-$paidamount);
}
else
{
$currentamount+=((float)$infoproforma['proformaamount']-$paidamount);
}
}
?>
...
<div class="col-lg-9">
<input type="text" name="twocustomername" id="twocustomername" class="form-control form-control-sm" placeholder="<?= $infomainaccessusercus['modulename'] ?> Name" value="<?= $rows[0]['customername'] ?>" <?= ($access['proformabtwocnamerequired']=='Yes')? 'required': ' ' ?>>
...
<input type="text" class="form-control form-control-sm" id="twoworkphone" name="twoworkphone" placeholder="Work Phone" value="<?=$rows[0]['workphone']?>" <?= ($access['proformabtwocwphonerequired']=='Yes')? 'required': ' ' ?>>
