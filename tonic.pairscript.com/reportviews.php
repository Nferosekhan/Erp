...
if (isset($_POST['submitcustomizes'])) {
  ...
  $timefrom = htmlspecialchars( $_GET['timefrom'], ENT_QUOTES, 'UTF-8');
  $timeto = htmlspecialchars( $_GET['timeto'], ENT_QUOTES, 'UTF-8');
  ...
  $sqlreportmoduless=$con->prepare("SELECT * FROM pairreportmodules WHERE types='inv' ORDER BY id ASC");
  ...
  $sqlupreport = $con->prepare("UPDATE pairreports SET rowcolumns=?, printtype=? WHERE franchiseid=? AND createdid=? AND types='inv'");
  ...
  $names = htmlspecialchars($_POST['names'], ENT_QUOTES, 'UTF-8');
  $payterm = htmlspecialchars($_POST['payterm'], ENT_QUOTES, 'UTF-8');

  $customername = '';
  if($names!='all'){
	 $sqlicusnames=$con->prepare("SELECT customername FROM paircustomers WHERE franchisesession=? AND createdid=? AND moduletype='Customers' AND id=? ORDER BY customername ASC");
	 $sqlicusnames->bind_param("ssi", $_SESSION["franchisesession"], $companymainid, $names);
	 $sqlicusnames->execute();
	 $sqlicusname = $sqlicusnames->get_result();
	 $infocusname=$sqlicusname->fetch_array();
	 $sqlicusname->close();
	 $sqlicusnames->close();
	 $customername = $infocusname['customername'];
  }

  $prefix = "CONCAT(SUBSTRING_INDEX(reporturl, '&period=', 1),'&period=')";
  $newReportUrl = $prefix . $reportperiod;

  $sqlreportfav = $con->prepare("UPDATE pairreportfavourites SET reporturl=? WHERE franchisesession=? AND createdid=? AND reportnames='invlist'");
  $sqlreportfav->bind_param("sss", $newReportUrl, $_SESSION['franchisesession'], $companymainid);
  $sqlreportfav->execute();
  $sqlreportfav->close();

  $sqlreport = $con->prepare("UPDATE pairreports SET cusvenname=?,reportperiod=?,filtername=?,companyname=?,dateprepare=?,timeprepare=?,names=?,payterm=? WHERE franchiseid=? AND createdid=? AND types='inv'");
  $sqlreport->bind_param("ssssssssss", $customername, $reportperiod, $filtername, $companyname, $dateprepare, $timeprepare, $names, $payterm, $_SESSION['franchisesession'], $companymainid);
  $sqlreport->execute();
  $sqlreport->close();

  header("Location:reportviews.php?names=".$names."&datefrom=".date($dateformatforurlredirection,strtotime($from))."&dateto=".date($dateformatforurlredirection,strtotime($to))."&timefrom=".$timefrom."&timeto=".$timeto."&payterm=".$payterm."&amttype=".$amttype."");
}
if (isset($_POST['submittempcustomizes'])) {
  $dateformatforurlredirection = 'Y-m-d';
  $from = htmlspecialchars($_POST['from'], ENT_QUOTES, 'UTF-8');
  $to = htmlspecialchars($_POST['to'], ENT_QUOTES, 'UTF-8');
  $timefrom = htmlspecialchars( $_GET['timefrom'], ENT_QUOTES, 'UTF-8');
  ...
  $names = htmlspecialchars($_POST['names'], ENT_QUOTES, 'UTF-8');
  $payterm = htmlspecialchars($_POST['payterm'], ENT_QUOTES, 'UTF-8');
  $amttype = htmlspecialchars($_POST['amttype'], ENT_QUOTES, 'UTF-8');
  ...
  $sqlreport = $con->prepare("UPDATE pairreports SET cusvenname=?,names=?,payterm=? WHERE franchiseid=? AND createdid=? AND types='inv'");
  $sqlreport->bind_param("sssss", $customername, $names, $payterm, $_SESSION['franchisesession'], $companymainid);
  $sqlreport->execute();
  $sqlreport->close();
									...
									<div class="row">
										...
										<div class="col-lg-6">
											<span style="float:right">
												...
												<a style="margin:4.5px 4.5px !important;" class="btn btn-primary btn-sm btn-custom-grey" onclick="quickcustom()">
													<i class="fa fa-filter" aria-hidden="true"></i>
													<span class="mobreswords "> Filter</span>
												</a>
												<!-- Quick Customize Start -->
												<div class="modal fade" id="quickcustomModal" tabindex="-1" role="dialog" aria-labelledby="quickcustomModalLabel" aria-hidden="true">
													<div class="modal-dialog modal-dialog-centered" role="document">
														<div class="modal-content">
															<div class="modal-header">
																<h5 class="modal-title" id="quickcustomModalLabel" style="font-weight: normal;">
																	Filter
																</h5>
																				...
																				<div class="accordion-body text-sm">
																					<div class="row justify-content-center">
																						<div class="col-lg-12">
																							<div class="row mb-3">
																								<div class="col-lg-6 my-1">
																									<label class="custom-label" for="names">
																										<?=$infomainaccessusercus['modulename']?>
																									</label>
																								...
																								<div class="col-lg-6 automodalchecking">
																									<select class="form-control form-control-sm select4 reportcustsearch" id="names" name="names">
																									<?php
																										if ($sqlviewreport['names']=='all'){
																									?>
																									...
																									<?php
																											...
																											$sqli = mysqli_query($con, "SELECT id, customername, billcity, mobile, workphone FROM paircustomers WHERE franchisesession='".$_SESSION["franchisesession"]."' and (createdid='$companymainid' and moduletype='Customers') and id='".$sqlviewreport['names']."' order by customername asc");
																											while ($info = mysqli_fetch_array($sqli)){
																									?>
																										<option value="<?=$info['id']?>" <?=($sqlviewreport['names']==$info['id'])?'selected':''?>>
																											<?=$info['customername']?>
																										</option>
																									<?php
																											}
																							...
																							<div class="row mb-3">
																								<div class="col-lg-6 my-1">
																									<label class="custom-label" for="payterm">
																										Payment Method
																									</label>
																								</div>
																								<div class="col-lg-6 automodalchecking">
																									<select class="form-control form-control-sm select4 reportpaytermsearch" id="payterm" name="payterm">
																									<?php
																										if ($sqlviewreport['payterm']=='all'){
																									?>
																										<option value="all" data-foo="" data-receivable="" selected>
																											All
																										</option>
																									<?php
																											...
																											while ($info = mysqli_fetch_array($sqli)){
																									?>
																										<option value="<?=$info['term']?>" <?=($sqlviewreport['payterm']==$info['term'])?'selected':''?>>
																											<?=$info['term']?>
																										</option>
																									<?php
																											}
																							...
																							<div class="row">
																								<div class="col-lg-6 my-1">
																									<label class="custom-label" for="amttype">
																										Balance
																									</label>
																								</div>
																								<div class="col-lg-6 automodalchecking">
																									<select class="form-control form-control-sm select4" id="amttype" name="amttype">
																										<option value="all" <?=(($_GET['amttype']=='all')?'selected':'')?>>
																											All
																										</option>
																										<option value="balance" <?=(($_GET['amttype']=='balance')?'selected':'')?>>
																											Balance Only
																										</option>
																									</select>
																								</div>
															...
															<div class="modal-footer" style="margin-top: 33px !important;">
																<div class="col">
																	<button class="btn btn-primary btn-sm btn-custom arlina-button expand-left" type="submit"  name="submittempcustomizes" id="submittempcustomizes" value="Submit">
																		<span class="label">
																			Run Report
																		</span>
																		<span class="spinner"></span>
																	</button>
																	<button type="button" class="btn btn-primary btn-sm btn-custom-grey" data-bs-toggle="modal" data-bs-dismiss="modal">
																		Cancel
																	</button>
																</div>
															</div>
												...
												<!-- Quick Customize End -->
												<a style="margin:4.5px 4.5px !important;<?= ($sqlviewreport['printtype']=='0')?'':'display: none;'?>" class="btn btn-primary btn-sm btn-custom-grey" onclick="window.open('reportviewsprint.php?names=<?=$_GET['names']?>&datefrom=<?=$_GET['datefrom']?>&dateto=<?=$_GET['dateto']?>&timefrom=<?=$_GET['timefrom']?>&timeto=<?=$_GET['timeto']?>&payterm=<?=$_GET['payterm']?>&amttype=<?=$_GET['amttype']?>','_blank')">
												...
												<a style="margin:4.5px 4.5px !important;" id="dropdownMenuButtonforcust" data-bs-toggle="dropdown" aria-expanded="false" class="btn btn-primary btn-sm btn-custom-grey">
													<i class="fa fa-ellipsis-h" aria-hidden="true"></i>
												</a>
												<div class="dropdown-menu  dropdown-menu-end customdropdown  me-sm-2" aria-labelledby="dropdownMenuButtonforcust">
													<i class="fa fa-caret-down" id="reparup" style="color: #3c3c46 !important;position: relative;top: -13px;left: 240px;cursor: pointer !important;"></i>
													<div style="background-color: #3c3c46;margin-top: -22px !important;cursor: pointer !important;">
														<a style="margin:4.5px 4.5px !important;color: #fff;padding: 4.5px;cursor: pointer !important;" id="customizes" data-bs-toggle="modal" data-bs-target="#Customizesmodal" class="nav-link">
															<i class="fa fa-sliders" aria-hidden="true"></i>
															<span class="mobreswords nav-link-text"> Customize Report</span>
														</a>
													</div>
												</div>
																			...
																			<div id="customizerowcolumns" class="accordion-collapse collapse show" aria-labelledby="customizerowcolumn">
																				<div class="accordion-body text-sm">
																					<div class="row justify-content-center">
																						...
																						<?php
																							$newans=array();
																							$sqlreportaccesss=$con->prepare("SELECT * FROM pairreports WHERE franchiseid=? AND createdid=? AND types='inv'");
																								...
																								<div class="col-lg-6 my-1" style="padding-left: 0px !important;">
																									<div class="custom-control custom-checkbox mr-sm-2" style="padding: 0px 12px !important;">
																										<div class="input-group input-group-sm" id="flaghead">
																											...
																											<input type="checkbox" class="custom-control-input" name="<?= $coltypemod; ?>inv" id="<?= $coltypemod; ?>inv" <?=((($newmoduleskey=='Payment Term')||($newmoduleskey=='City'))?(((in_array($newmoduleskey, $newans)))?'checked':''):'checked')?> <?=((($newmoduleskey=='Payment Term')||($newmoduleskey=='City'))?'':'disabled')?>>
																											<label class="custom-control-label custom-label" for="<?= $coltypemod; ?>inv">
																												<?= (($newmoduleskey=='Sno')?'(#) Row Number':(str_replace(" or ", " / ",(str_replace("Payment Term", "(Term) Payment Term",$newmoduleskey))))) ?>
																											</label>
																										</div>
																									</div>                  
																								</div>
											...
											<div class="tab-pane fade" id="nav-profile" role="tabpanel" aria-labelledby="nav-profile-tab">
												<div class="table-responsive m-3" id="histables">
													<table class="table table-bordered">
														...
														<tbody>
														<?php
															$sqluse=$con->prepare("SELECT * FROM pairusehistory WHERE usetype = 'REPinv' AND useid = ? ORDER BY createdon DESC");
																			...
																			<td width="100%" style="padding: 10px;">
																				<table id="print-are1" style="border: 1px solid #eee;" width="100%">
										      									...
										      									if($sqlinfoe->num_rows<=2500){
																					...
																					?>
																						<div style="text-align: center;position: relative;top: 35px;margin-bottom: 5px;">
																							The number of records exceeds 2500 rows. To improve performance, please apply filters or reduce the data range.
																						</div>
																					<?php
																						}
																						if (mysqli_num_rows($sqlinfoe)==0) {
																					?>
										...
										<table id="print-are3" style="border: 1px solid #eee;display: none;" width="100%">
											<thead>
												<tr>
													<td class="text-uppercase" style="border: 1px solid #eee;padding-left: 10px;max-width: 50px !important;min-width: 50px !important;white-space: nowrap;overflow: hidden;"><span style="font-size:13px;color:black;"> Sno</span></td>
													<td class="text-uppercase" style="border: 1px solid #eee;padding-left: 10px;max-width: 87px !important;min-width: 87px !important;white-space: nowrap;overflow: hidden;"><span style="font-size:13px;color:black;"> Status</span></td>
													<td class="text-uppercase" style="border: 1px solid #eee;padding-left: 10px;max-width: 80px !important;min-width: 80px !important;white-space: nowrap;overflow: hidden;"><span style="font-size:13px;color:black;"> Date</span></td>
													<td class="text-uppercase" style="border: 1px solid #eee;padding-left: 10px;max-width: 80px !important;min-width: 80px !important;white-space: nowrap;overflow: hidden;display: none;"><span style="font-size:13px;color:black;"> Payment Days</span></td>
													<td class="text-uppercase" style="border: 1px solid #eee;padding-left: 10px;max-width: 73px !important;min-width: 73px !important;white-space: nowrap;overflow: hidden;"><span style="font-size:13px;color:black;"> Number</span></td>
													<td class="text-uppercase" style="border: 1px solid #eee;padding-left: 10px;max-width: 100px !important;min-width: 100px !important;white-space: nowrap;overflow: hidden;<?=((($sqlviewreport['filtername']=='1')||($sqlviewreport['filtername']=='0'))?'':'display:none;')?>"><span style="font-size:13px;color:black;"> <?=$infomainaccessusercus['modulename']?> Name</span></td>
													<td class="text-uppercase" style="border: 1px solid #eee;padding-left: 10px;max-width: 80px !important;min-width: 80px !important;white-space: nowrap;overflow: hidden;<?=(((in_array('City', $newans)))?'':'display: none;')?>"><span style="font-size:13px;color:black;"> City</span></td>
													<td class="text-uppercase" style="border: 1px solid #eee;padding-right: 10px;max-width: 76px !important;min-width: 76px !important;white-space: nowrap;overflow: hidden;text-align: right;"><span style="font-size:13px;color:black;"> Total</span></td>
													<td class="text-uppercase" style="<?=(((in_array('Payment Term', $newans)))?'':'display: none;')?>border: 1px solid #eee;padding-left: 10px;max-width: 50px !important;min-width: 50px !important;white-space: nowrap;overflow: hidden;"><span style="font-size:13px;color:black;"> Term</span></td>
													<td class="text-uppercase" style="border: 1px solid #eee;padding-right: 10px;max-width: 76px !important;min-width: 76px !important;white-space: nowrap;overflow: hidden;text-align: right;"><span style="font-size:13px;color:black;"> Balance</span></td>
												</tr>
											</thead>
											<tbody id="myTables">
											</tbody>
...
const pagechanges = (page, pageSize) => {
	...
	var totalpagesnumber = <?=ceil($chekcs/36)?>;
	if ((page=='')||(page==0)) {
		var isthisval = 1;
	}
	...
	var perpages = ''+((parseInt(isthisval)-1)*36);
	...
	ajaxRequestview = $.ajax({
		type: "GET",
		url: 'reportviewsearch.php?term='+perpages+'&limitings='+($('#limitforpagenum').val())+'&datefrom=<?=$datefrom?>&dateto=<?=$dateto?>&dif=invview&names=<?=$_GET['names']?>&timefrom=<?=$timefrom?>&timeto=<?=$timeto?>&payterm=<?=$_GET['payterm']?>&amttype=<?=$_GET['amttype']?>&lastornone='+lastornone+'',
...
$(function() {
	...
	const fromTimeString = $("#timefrom").val();
	const [fromyear, frommonth, fromday] = fromDateString.split('-').map(Number);
	const [fromhours, fromminutes, fromseconds] = fromTimeString.split(':').map(Number);
	var start = new Date(fromyear, frommonth - 1, fromday, fromhours, fromminutes, fromseconds);
	const toDateString = $("#dateto").val();
	const toTimeString = $("#timeto").val();
	const [toyear, tomonth, today] = toDateString.split('-').map(Number);
	const [tohours, tominutes, toseconds] = toTimeString.split(':').map(Number);
	var end = new Date(toyear, tomonth - 1, today, tohours, tominutes, toseconds);
	...
	function(start, end, label) {
		...
		var fromParts = start.format('HH:mm:ss').split(':');
		var fromTime = new Date();
		fromTime.setHours(fromParts[0]);
		fromTime.setMinutes(fromParts[1]);
		fromTime.setSeconds(fromParts[2]);
		var fromFormattedTime = fromTime.toLocaleString('en-US', { hour: 'numeric', minute: 'numeric', second: 'numeric', hour12: true });
		var toParts = end.format('HH:mm:ss').split(':');
		var toTime = new Date();
		toTime.setHours(toParts[0]);
		toTime.setMinutes(toParts[1]);
		toTime.setSeconds(toParts[2]);
		var toFormattedTime = toTime.toLocaleString('en-US', { hour: 'numeric', minute: 'numeric', second: 'numeric', hour12: true });
		$("#datefromto").html("From " + startfinalday + " " + startfinalmonth + " " + startfinalyear + " " + fromFormattedTime + ' To ' + endfinalday + " " + endfinalmonth + " " + endfinalyear + " " + toFormattedTime);
		...
		let timefromonspots;
		let timetoonspots;
		if ($("#timefrom").val()!=start.format('HH:mm:ss')) {
			timefromonspots = start.format('HH:mm:ss');
		}
		else{
			timefromonspots = "00:00:00";
		}
		if ($("#timeto").val()!=end.format('HH:mm:ss')) {
			timetoonspots = end.format('HH:mm:ss');
		}
		else{
			timetoonspots = "23:59:59";
		}
		window.open("reportviews.php?names=<?=$_GET['names']?>&datefrom="+datefrom+"&dateto="+dateto+"&timefrom="+timefromonspots+"&timeto="+timetoonspots+"&payterm=<?=$_GET['payterm']?>&amttype=<?=$_GET['amttype']?>","_self");
	});
	$("#timefrom").val(start.format('HH:mm:ss'));
	$("#timeto").val(end.format('HH:mm:ss'));
});
...
$(document).ready(function() {
	...
	var fromParts = $("#timefrom").val().split(':');
	var fromTime = new Date();
	fromTime.setHours(fromParts[0]);
	fromTime.setMinutes(fromParts[1]);
	fromTime.setSeconds(fromParts[2]);
	...
	$("#datefromto").html("From " + startday + " " + startfinalmonths + " " + startyear + " " + fromFormattedTime + ' To ' + endday + " " + endfinalmonths + " " + endyear + " " + toFormattedTime);
})
</script>
...
function converHTMLFileToPDF() {
	...
	var finaltimer = setTimeout(function(){
		...
		var header = function(data) {
			...
			var headtext1 = "<?=$infomainaccessuserinv["modulename"]?> List/Details";
			var fontSize = doc.internal.getFontSize();
			var pageWidth = doc.internal.pageSize.width;
		...
		?>
			...
			var totalrowscount = 'Total Rows : '+$("#totalrowscount").val();
			var fontSizetotal = doc.internal.getFontSize();
			var pageWidthtotal = doc.internal.pageSize.width;
			txtWidthtotal = doc.getStringUnitWidth(totalrowscount)*fontSizetotal/doc.internal.scaleFactor;
			x = 35;
			doc.text(totalrowscount, x, pageHeight - 35);
		};
		var botforprint = 50;
		doc.autoTable({
			html: '#print-are3',
			...
			columnStyles: { 
			<?php
				if ((in_array('Payment Term', $newans))&&(in_array('City', $newans))){
			?>
				0: { halign: 'left', cellWidth: 39},
				1: { halign: 'left', cellWidth: 40},
				2: { halign: 'left', cellWidth: 50},
				3: { halign: 'left', cellWidth: 50},
				4: { halign: 'left', cellWidth: 60},
				5: { halign: 'left', cellWidth: 91},
				6: { halign: 'left', cellWidth: 46},
				7: { halign: 'right', cellWidth: 51},
				8: { halign: 'left', cellWidth: 40},
				9: { halign: 'right', cellWidth: 55}
			<?php
				}
				elseif ((!in_array('Payment Term', $newans))&&(in_array('City', $newans))) {
			?>
				0: { halign: 'left', cellWidth: 39},
				1: { halign: 'left', cellWidth: 40},
				2: { halign: 'left', cellWidth: 50},
				3: { halign: 'left', cellWidth: 50},
				4: { halign: 'left', cellWidth: 60},
				5: { halign: 'left', cellWidth: 121},
				6: { halign: 'left', cellWidth: 46},
				7: { halign: 'right', cellWidth: 61},
		...
		$.ajax({
			type: "GET",
			url: 'reporthistory.php?types=inv&titles=Invoice List/Details&printdownpdfcsv=Printed PDF&fromto='+fromtopdf+'',
			success: function (result) {
				console.log(result);
			},
			error: function (error) {
...
function downloadaspdf() {
	...
	var finaltimer = setTimeout(function(){
		...
		$.ajax({
			type: "GET",
			url: 'reporthistory.php?types=inv&titles=Invoice List/Details&printdownpdfcsv=Downloaded PDF&fromto='+fromtopdf+'',
			success: function (result) {
				console.log(result);
			},
			error: function (error) {
		...
		doc.save('Invoice-Reports-<?=$datefrom?>-To-<?=$dateto?>');
...
function downloadascsv() {
	...
	var finaltimer = setTimeout(function(){
		...
		data.push(["<?=$infomainaccessuserinv["modulename"]?> List/Details"]);
		data.push([fromtopdf]);
		...
		link.setAttribute("download", "Invoice-Reports-<?=$datefrom?>-To-<?=$dateto?>");
		document.body.appendChild(link);
		link.click();
		$.ajax({
			type: "GET",
			url: 'reporthistory.php?types=inv&titles=Invoice List/Details&printdownpdfcsv=Downloaded CSV&fromto='+fromtopdf+'',
			success: function (result) {
				console.log(result);
			},
			error: function (error) {
	...
	intervalingopt = setInterval(function() {
		if ((counteringforoptions <= 0)&&($('#myTables').html()!='')) {
			if (mytabprev=='empty') {
				...
				if(mytabprev==''){
					counteringforoptions = <?=((ceil($chekcs/200)>0)?ceil($chekcs/200):1)?>;
					setInterval();
				}
				else{
...
?>
	function makeAjaxRequest() {
		ajaxRequest = $.ajax({
			type: "GET",
			url: 'check.php?term=invview&datefrom=<?=$datefrom?>&dateto=<?=$dateto?>&limitfrom=0&limitto=10000&names=<?=$_GET['names']?>&timefrom=<?=$timefrom?>&timeto=<?=$timeto?>&payterm=<?=$_GET['payterm']?>&amttype=<?=$_GET['amttype']?>',
			success: function (result) {
				if (result=='') {
					mytabprev = 'empty';
				}
...
// 			columnStyles: { 
// 			<?php
// 				if ((in_array('Payment Term', $newans))&&(in_array('Sno', $newans))&&(in_array('City', $newans))&&(in_array('Customer Name', $newans))&&(in_array('Balance Due', $newans))){
// 			?>
// 				0: { halign: 'left', cellWidth: 39},
// 				1: { halign: 'left', cellWidth: 40},
// 				2: { halign: 'left', cellWidth: 50},
// 				3: { halign: 'left', cellWidth: 60},
// 				4: { halign: 'left', cellWidth: 131},
...
// 			<?php
// 				}
// 				elseif ((!in_array('Payment Term', $newans))&&(in_array('Sno', $newans))&&(!in_array('City', $newans))&&(!in_array('Customer Name', $newans))&&(!in_array('Balance Due', $newans))) {
// 			?>
// 				0: { halign: 'left', cellWidth: 39},
// 				1: { halign: 'left', cellWidth: 71},
...
// 				1: { halign: 'left', cellWidth: 89},
// 				2: { halign: 'left', cellWidth: 121},
// 				3: { halign: 'right', cellWidth: 141},
// 				4: { halign: 'left', cellWidth: 111}
// 			<?php
// 				}
// 				elseif ((!in_array('Payment Term', $newans))&&(!in_array('Sno', $newans))&&(in_array('City', $newans))&&(!in_array('Customer Name', $newans))&&(!in_array('Balance Due', $newans))){
// 			?>
...
// 				2: { halign: 'left', cellWidth: 99},
// 				3: { halign: 'left', cellWidth: 173},
// 				4: { halign: 'right', cellWidth: 145}
// 			<?php
// 				}
// 				elseif ((!in_array('Payment Term', $newans))&&(!in_array('Sno', $newans))&&(!in_array('City', $newans))&&(in_array('Customer Name', $newans))&&(!in_array('Balance Due', $newans))){
// 			?>
...
// 				3: { halign: 'left', cellWidth: 195},
// 				4: { halign: 'right', cellWidth: 130}
// 			<?php
// 				}
// 				elseif ((!in_array('Payment Term', $newans))&&(!in_array('Sno', $newans))&&(!in_array('City', $newans))&&(!in_array('Customer Name', $newans))&&(in_array('Balance Due', $newans))){
// 			?>
// 				0: { halign: 'left', cellWidth: 79},
// 				1: { halign: 'left', cellWidth: 81},
// 				2: { halign: 'left', cellWidth: 160},
// 				3: { halign: 'right', cellWidth: 107},
...
// 				3: { halign: 'left', cellWidth: 170},
// 				4: { halign: 'left', cellWidth: 46},
// 				5: { halign: 'right', cellWidth: 61},
...
// 				2: { halign: 'left', cellWidth: 50},
// 				3: { halign: 'left', cellWidth: 60},
// 				4: { halign: 'left', cellWidth: 157},
// 				5: { halign: 'right', cellWidth: 71},
// 				6: { halign: 'left', cellWidth: 40},
