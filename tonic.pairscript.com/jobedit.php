...
$oldjobno=mysqli_real_escape_string($con, $_GET['jobno']);
$oldjobdate=mysqli_real_escape_string($con, $_GET['jobdate']);
...
if($productname!='')
{
$sql=mysqli_query($con, "delete from pairjobs where createdid='$companymainid' and franchisesession='".$_SESSION["franchisesession"]."' and jobno='$oldjobno' and jobdate='$oldjobdate'");
if($sql)
{
}
else
...
$sql2=mysqli_query($con, "select id from pairjobs where franchisesession='".$_SESSION['franchisesession']."' and createdid='$companymainid' and jobno='$oldjobno' and jobdate='$oldjobdate'");
...
echo '<script> window.location.href="jobs.php?remarks=Updated Successfully";</script>'; 
}
else
{
echo '<script> window.location.href="jobview.php?jobno='.$jobno.'&jobdate='.$jobdate.'&remarks=Updated Successfully";</script>';
}
}
}
else
...
if ($sql) {
$hissql=mysqli_query($con, "insert into pairusehistory set usetype='JOBS', createdon='$times',  createdby='".$_SESSION["unqwerty"]."', useid='$jobno', useremarks='Job Updated' ");
}
}
...
<input type="hidden" name="oldjobno" value="<?=$rows[0]['jobno']?>">
<input type="hidden" name="oldjobdate" value="<?=$rows[0]['jobdate']?>">
<input type="hidden" name="jobterm" value="<?=$rows[0]['jobterm']?>">

<div class="row">
        <?php
     if ((in_array('Job Information', $fieldedit))) {
        ?>
<div class="col-lg-4">
...
<div class="col-lg-7">
<input type="text" class="form-control  form-control-sm" id="jobno" name="jobno" required  value="<?=$rows[0]['jobno']?>" readonly>
</div>
</div>
</div>
<div class="col-lg-12">
...
<div class="col-lg-7">
<input type="date" class="form-control  form-control-sm" id="jobdate" name="jobdate" required value="<?=$rows[0]['jobdate']?>">
</div>
</div>
</div>
...
?>
<option value="<?= $info['term'] ?>" <?=($info['term']==$rows[0]['jobterm'])?'selected':''?>><?= $info['term'] ?></option>
<?php
}
?>
</select>
...
<input type="radio" class="custom-control-input" name="customerinfodefault" id="onecustomerinfo" value="one" <?= ($rows[0]['customerinfodefault']=='one'||$infomainaccessuser['jobcustinfo']=='defone')?'checked':'' ?> onclick="onetwo()">
<label class="custom-control-label custom-label" for="onecustomerinfo">B2B</label>
</div>
</div>
<div class="col-lg-2 col-md-2 col-3">
<div class="custom-control custom-radio mr-sm-2">
<input type="radio" class="custom-control-input" name="customerinfodefault" id="twocustomerinfo" value="two" <?= ($rows[0]['customerinfodefault']=='two'||$infomainaccessuser['jobcustinfo']=='deftwo')?'checked':'' ?> onclick="onetwo()">
...
while ($info = mysqli_fetch_array($sqli))
{
$sqlijob=mysqli_query($con, "select jobamount, jobdate, jobno from pairjobs where franchisesession='".$_SESSION['franchisesession']."' and createdid='$companymainid' and customerid='".$info['id']."' GROUP BY jobdate, jobno order by jobdate desc, jobno desc");
$jobamount=0;
$balanceamount=0;
$currentamount=0;
$overdueamount=0;
while($infojob=mysqli_fetch_array($sqlijob))
{
$jobamount+=(float)$infojob['jobamount'];
$paidamount=0;
$sqlsalespay=mysqli_query($con,"select amount from pairsalespayhistory where franchisesession='".$_SESSION['franchisesession']."' and createdid='$companymainid' and invoiceno='".$infojob['jobno']."' and invoicedate='".$infojob['jobdate']."' and customerid='".$info['id']."' order by id desc");
while($infosalespay=mysqli_fetch_array($sqlsalespay))
{
$paidamount+=(float)$infosalespay['amount'];
}
$balanceamount+=((float)$infojob['jobamount']-$paidamount);
$diff = abs(time() - strtotime($infojob['jobdate']));
$days = floor(($diff)/ (60*60*24));
if($days>30)
{
$overdueamount+=((float)$infojob['jobamount']-$paidamount);
}
else
{
$currentamount+=((float)$infojob['jobamount']-$paidamount);
}
}
?>
...
<div class="col-lg-9">
<input type="text" name="twocustomername" id="twocustomername" class="form-control form-control-sm" placeholder="<?= $infomainaccessusercus['modulename'] ?> Name" value="<?= $rows[0]['customername'] ?>" <?= ($access['jobbtwocnamerequired']=='Yes')? 'required': ' ' ?>>
...
<input type="text" class="form-control form-control-sm" id="twoworkphone" name="twoworkphone" placeholder="Work Phone" value="<?=$rows[0]['workphone']?>" <?= ($access['jobbtwocwphonerequired']=='Yes')? 'required': ' ' ?>>
