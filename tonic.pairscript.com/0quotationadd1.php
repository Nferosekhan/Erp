...
if(isset($_POST['grandtotal']))
{
    ...
    $billpin=mysqli_real_escape_string($con, $_POST['billpin']);
    ...
    $shippin=mysqli_real_escape_string($con, $_POST['shippin']);
...
if($customerid=='')
{
$sqlcon = "SELECT id From paircontacts WHERE franchisesession='".$_SESSION["franchisesession"]."' and customerdisplayname = '{$customername}'";
...
if($rowCountcon == 0) 
{   
$sqlup = "insert into paircontacts set createdon='$times', createdid='$companymainid', createdby='".$_SESSION["unqwerty"]."', franchisesession='".$_SESSION["franchisesession"]."', customerdisplayname='$customername', billstreet='$billstreet', billcity='$billcity', billstate='$billstate', billpincode='$billpin', billcountry='$billcountry', shipstreet='$shipstreet', shipcity='$shipcity', shipstate='$shipstate', shippincode='$shippin', shipcountry='$shipcountry'";
$queryup = mysqli_query($con, $sqlup);
$sql=mysqli_query($con, "insert into pairquotations set createdon='$times', createdid='$companymainid', createdby='".$_SESSION["unqwerty"]."', franchisesession='".$_SESSION["franchisesession"]."', customername='$customername', customerid='$customerid', billcity='$billcity', billstate='$billstate', billpincode='$billpin', billcountry='$billcountry', shipstreet='$shipstreet', shipcity='$shipcity', shipstate='$shipstate', shippincode='$shippin', shipcountry='$shipcountry'");
if(!$queryup){
   die("SQL query failed: " . mysqli_error($con));
}
else
...
<p class="mb-3" style="font-size:20px;margin-top: -8px;margin-bottom: -10px;"><i class="fa fa-file-import"></i> New
Quotation</p>
<?php
...
<button class="accordion-button font-weight-bold" type="button">
<div class="customcont-header ml-0 mb-1" style="height: 30px;">
<a class="customcont-heading" style="padding: 7px 0 7px;">Quotation Information</a>
</div>
</button>
</h5>
...
<div class="col-lg-8">
<input type="text" class="form-control  form-control-sm" id="quotationno" name="quotationno" required value="<?=$infoet['quotationprefix']?><?=str_pad((float)$infoet['quotationsuffix']+1, 0, "0", STR_PAD_LEFT)?>" readonly>
</div>
  </div>
</div>
<div class="col-lg-12">
...
<label for="quotationdate" class="custom-label">Quotation Date</label>
  ...
  <div class="col-lg-8">
<input type="date" class="form-control  form-control-sm" id="quotationdate" name="quotationdate" readonly required value="<?=date('Y-m-d')?>">
  </div>
  </div>
</div>
<div class="col-lg-12">
...
<label for="quotationterm" class="custom-label">Quotation Term</label>
  ...
  <div class="col-lg-8">
 <select required class="form-control  form-control-sm" name="quotationterm" id="quotationterm" >
...
<td><div class="input-group"><div class="input-group-prepend"><div class="input-group-text" style="color:#cccccc"><i class="fa fa-rupee"></i></div></div><input type="number" min="0" step="0.01" name="productrate[]" readonly  required id="productrate1" class="form-control form-control-sm bordernoneinput" onChange="productcalc(1)"></div></td>
...
<td><div class="input-group"><div class="input-group-prepend"><div class="input-group-text" style="color:#cccccc"><i class="fa fa-rupee"></i></div></div><input type="number" min="0" step="0.01" name="productvalue[]" id="productvalue1" class="form-control form-control-sm bordernoneinput"style="background:none;" readonly ></div></td>
