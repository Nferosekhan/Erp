...
if (isset($_POST['submitcustomizes'])) {
	...
	$sqlreportmoduless=$con->prepare("SELECT * FROM pairreportmodules WHERE types='prosalecus' ORDER BY id ASC");
	...
	foreach ($newmodules as $newmoduleskey) {
		...
		$modcolumncol=$coltypemod."prosalecus";
	...
	$sqlupreport = $con->prepare("UPDATE pairreports SET rowcolumns=? WHERE franchiseid=? AND createdid=? AND types='prosalecus'");
	...
	if($names!='all'){
		$sqlicusnames=$con->prepare("SELECT customername FROM paircustomers WHERE franchisesession=? AND createdid=? AND id=? AND moduletype='Customers' ORDER BY customername ASC");
	...
	$sqlreportfav = $con->prepare("UPDATE pairreportfavourites SET reporturl=? WHERE franchisesession=? AND createdid=? AND reportnames='prosalecus'");
	$sqlreportfav->bind_param("sss", $newReportUrl, $_SESSION['franchisesession'], $companymainid);
	$sqlreportfav->execute();
	$sqlreportfav->close();

	$sqlreport = $con->prepare("UPDATE pairreports SET commision=?,category=?,categoryname=?,names=?,cusvenname=?,reportperiod=?,filtername=?,companyname=?,dateprepare=?,timeprepare=?,gstrule=? WHERE franchiseid=? AND createdid=? AND types='prosalecus'");
	$sqlreport->bind_param("issssssssssss", $commision, $category, $categoryname, $names, $customername, $reportperiod, $filtername, $companyname, $dateprepare, $timeprepare, $gstrule, $_SESSION['franchisesession'], $companymainid);
	$sqlreport->execute();
	$sqlreport->close();

	header("Location:reportprosalecus.php?datefrom=".date($dateformatforurlredirection,strtotime($from))."&dateto=".date($dateformatforurlredirection,strtotime($to))."&names=".$names."&commision=".$commision."&category=".$category."");
}

$sqlreportviews=$con->prepare("SELECT * FROM pairreports WHERE franchiseid=? AND createdid=? AND types='prosalecus'");
																			...
																			<div id="customizerowcolumns" class="accordion-collapse collapse show" aria-labelledby="customizerowcolumn">
																				<div class="accordion-body text-sm">
																					<div class="row justify-content-center">
																						...
																						<?php
																							$newans=array();
																							$sqlreportaccesss=$con->prepare("SELECT * FROM pairreports WHERE franchiseid=? AND createdid=? AND types='prosalecus'");
																								...
																								<div class="col-lg-6 my-1" style="padding-left: 0px !important;">
																									<div class="custom-control custom-checkbox mr-sm-2" style="padding: 0px 12px !important;">
																										<div class="input-group input-group-sm" id="flaghead">
																											...
																											<input type="checkbox" class="custom-control-input" name="<?= $coltypemod; ?>prosalecus" id="<?= $coltypemod; ?>prosalecus" checked disabled>
																											<label class="custom-control-label custom-label" for="<?= $coltypemod; ?>prosalecus">
																					...
																					<div class="row justify-content-center">
																						<div class="col-lg-8">
																							...
																							<div class="row mb-1">
																								<div class="col-lg-6 my-1">
																									<div class="custom-control custom-checkbox mr-sm-2">
																										<input type="checkbox" class="custom-control-input" name="productname" id="productname" checked disabled>
																										<label class="custom-control-label custom-label" for="productname">
																											Products
																										</label>
																									</div>
																								</div>
																								<div class="col-lg-6">
																									<input type="text" value="All" class="form-control form-control-sm" readonly style="background-color: #e9ecef;">
																								</div>
																							</div>
																							<div class="row mb-1">
																								<div class="col-lg-6 my-1">
																									<div class="custom-control custom-checkbox mr-sm-2">
																										<input type="checkbox" class="custom-control-input" name="servicename" id="servicename" disabled>
																										<label class="custom-control-label custom-label" for="servicename">
																											Services
																										</label>
																									</div>
																								</div>
											...
											<div class="tab-pane fade" id="nav-profile" role="tabpanel" aria-labelledby="nav-profile-tab">
												<div class="table-responsive m-3" id="histables">
													<table class="table table-bordered">
														...
														<tbody>
														<?php
															$sqluse=$con->prepare("SELECT * FROM pairusehistory WHERE usetype = 'REPprosalecus' AND useid = ? ORDER BY createdon DESC");
																		...
																		<tr style="height:1px;">
																			<td width="100%" style="padding:10px;">
																				<table id="print-are1" style="border: 1px solid #eee;" width="100%">
																					<thead>
																						<tr>
																							<td class="text-uppercase" style="width:10%;border:1px solid #eee;padding-left: 10px;">
																							...
																							<td class="text-uppercase" style="width:13%;border:1px solid #eee;padding-left: 10px;">
																								<span style="font-size:13px;color:black;">
																									Customer Name
																								</span>
																							</td>
																							...
																							<td class="text-uppercase" style="width:6%;border:1px solid #eee;padding-left: 10px;">
																								<span style="font-size:13px;color:black;">
																									<?=$access['txtqtyinv']?>
																								</span>
																							</td>
																					...
																					<?php
																						$totalValuetot = 0;
																						$totalRatetot = 0;
																						$seldatas=$con->prepare("SELECT * FROM pairinvoices WHERE franchisesession=? AND createdid=? AND (invoicedate>=? AND invoicedate<=?) AND cancelstatus='0' ".(($sqlviewreport['names']=='all')?' ':' and customerid="'.$sqlviewreport['names'].'"')." ".(($sqlviewreport['category']=='all')?' ':' and manufacturer="'.$sqlviewreport['categoryname'].'"')." ORDER BY productname ASC LIMIT 35");
																					...
																					?>
																						<tr style="vertical-align: middle; height: 30px !important;">
																							...
																							?>
																						<tr>
																							<td colspan="8" style="text-align: left !important; font-size:12px; color:black; border:1px solid #eee; padding-left: 10px;">
																								<?= $info['productname'] ?>
																							</td>
																						</tr>
																						<?php
																						...
																						<tr style="vertical-align: middle; height: 30px !important;">
																							...
																							<td style="text-align: right !important; font-size:12px; color:black; border:1px solid #eee; padding-right: 10px;">
																								<?= (($info['productrate']>0)?$info['quantity']:'') ?>
																							</td>
																							<td style="text-align: right !important; font-size:12px; color:black; border:1px solid #eee; padding-right: 10px;">
																								<?= (($info['productrate']>0)?'':$info['quantity']) ?>
																							</td>
																							<td style="text-align: right !important; font-size:12px; color:black; border:1px solid #eee; padding-right: 10px;">
																								<?= number_format((float)($info['productrate'])+(($info['productrate'] * ($_GET['commision']) /100)),2,'.',',') ?>
																							</td>
																							<td style="text-align: right !important; font-size:12px; color:black; border:1px solid #eee; padding-right: 10px;">
																					...
																					<?php
																						...
																						$sqltotlistpages=$con->prepare("SELECT * FROM pairinvoices WHERE franchisesession=? AND createdid=? AND (invoicedate>=? AND invoicedate<=?) AND cancelstatus='0' ".(($sqlviewreport['names']=='all')?' ':' and customerid="'.$sqlviewreport['names'].'"')." ".(($sqlviewreport['category']=='all')?' ':' and manufacturer="'.$sqlviewreport['categoryname'].'"')." ORDER BY productname ASC");
																					...
																					?>
																						<div style="text-align: center;position: relative;top: 39px;margin-bottom: -25px;">
...
const pagechanges = (page, pageSize) => {
	...
	ajaxRequestview = $.ajax({
		type: "GET",
		url: 'reportviewsearch.php?term='+perpages+'&limitings='+($('#limitforpagenum').val())+'&datefrom=<?=$datefrom?>&dateto=<?=$dateto?>&dif=prosalecus&names=<?=$_GET['names']?>&commision=<?=$_GET['commision']?>&category=<?=$_GET['category']?>&lastornone='+lastornone+'',
...
$(function() {
	...
	function(start, end, label) {
		...
		window.open("reportprosalecus.php?datefrom="+datefrom+"&dateto="+dateto+"&names=<?=$_GET['names']?>&commision=<?=$_GET['commision']?>&category=<?=$_GET['category']?>","_self");
	});
});
...
function converHTMLFileToPDF() {
	...
	var finaltimer = setTimeout(function(){
		...
		var header = function(data) {
			...
			var headtext1 = "Product Customer Wise Sales";
			var fontSize = doc.internal.getFontSize();
			var pageWidth = doc.internal.pageSize.width;
		...
		$.ajax({
			type: "GET",
			url: 'reporthistory.php?types=prosalecus&titles=Product Customer Wise Sales&printdownpdfcsv=Printed PDF&fromto='+fromtopdf+'',
			success: function (result) {
				console.log(result);
			},
			error: function (error) {
...
function downloadaspdf() {
	...
	var finaltimer = setTimeout(function(){
		...
		$.ajax({
			type: "GET",
			url: 'reporthistory.php?types=prosalecus&titles=Product Customer Wise Sales&printdownpdfcsv=Downloaded PDF&fromto='+fromtopdf+'',
			success: function (result) {
				console.log(result);
			},
			error: function (error) {
		...
		doc.save('Product-Customer-Wise-Sales-<?=$datefrom?>-To-<?=$dateto?>');
...
function downloadascsv() {
	...
	var finaltimer = setTimeout(function(){
		...
		data.push(["Product Customer Wise Sales"]);
		...
		link.setAttribute("download", "Product-Customer-Wise-Sales-<?=$datefrom?>-To-<?=$dateto?>");
		document.body.appendChild(link);
		link.click();
		$.ajax({
			type: "GET",
			url: 'reporthistory.php?types=prosalecus&titles=Product Customer Wise Sales&printdownpdfcsv=Downloaded CSV&fromto='+fromtopdf+'',
			success: function (result) {
				console.log(result);
			},
			error: function (error) {
...
document.addEventListener('DOMContentLoaded', function() {
	...
	function makeAjaxRequest() {
		ajaxRequest = $.ajax({
			type: "GET",
			url: 'check.php?term=prosalecus&datefrom=<?=$datefrom?>&dateto=<?=$dateto?>&limitfrom=0&limitto=10000&names=<?=$_GET['names']?>&commision=<?=$_GET['commision']?>&category=<?=$_GET['category']?>',
			success: function (result) {
				if (result != mytabprev) {
