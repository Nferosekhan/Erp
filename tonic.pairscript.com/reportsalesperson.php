...
if (isset($_POST['submitcustomizes'])) {
	...
	$sqlreportmoduless=$con->prepare("SELECT * FROM pairreportmodules WHERE types='salesperson' ORDER BY id ASC");
	...
	foreach ($newmodules as $newmoduleskey) {
		...
		$modcolumncol=$coltypemod."salesperson";
	...
	$sqlupreport = $con->prepare("UPDATE pairreports SET rowcolumns=? WHERE franchiseid=? AND createdid=? AND types='salesperson'");
	...
	$sqlreportfav = $con->prepare("UPDATE pairreportfavourites SET reporturl=? WHERE franchisesession=? AND createdid=? AND reportnames='salesperson'");
	$sqlreportfav->bind_param("sss", $newReportUrl, $_SESSION['franchisesession'], $companymainid);
	$sqlreportfav->execute();
	$sqlreportfav->close();

	$sqlreport = $con->prepare("UPDATE pairreports SET reportperiod=?,filtername=?,companyname=?,dateprepare=?,timeprepare=?,gstrule=?,preparedby=?,checkedby=? WHERE franchiseid=? AND createdid=? AND types='salesperson'");
	$sqlreport->bind_param("ssssssssss", $reportperiod, $filtername, $companyname, $dateprepare, $timeprepare, $gstrule, $preparedby, $checkedby, $_SESSION['franchisesession'], $companymainid);
	$sqlreport->execute();
	$sqlreport->close();

	header("Location:reportsalesperson.php?datefrom=".date($dateformatforurlredirection,strtotime($from))."&dateto=".date($dateformatforurlredirection,strtotime($to))."&preparedby=".$preparedby."&checkedby=".$checkedby."");
}

$sqlreportviews=$con->prepare("SELECT * FROM pairreports WHERE franchiseid=? AND createdid=? AND types='salesperson'");
								...
								<form action="" method="post" enctype="multipart/form-data" class="form-horizontal mt-0" role="form">
									<div class="row">
																			...
																			<div id="customizerowcolumns" class="accordion-collapse collapse show" aria-labelledby="customizerowcolumn">
																				<div class="accordion-body text-sm">
																					<div class="row justify-content-center">
																						...
																						<?php
																							$newans=array();
																							$sqlreportaccesss=$con->prepare("SELECT * FROM pairreports WHERE franchiseid=? AND createdid=? AND types='salesperson'");
																								...
																								<div class="col-lg-6 my-1" style="padding-left: 0px !important;">
																									<div class="custom-control custom-checkbox mr-sm-2" style="padding: 0px 12px !important;">
																										<div class="input-group input-group-sm" id="flaghead">
																											...
																											<input type="checkbox" class="custom-control-input" name="<?= $coltypemod; ?>salesperson" id="<?= $coltypemod; ?>salesperson" checked disabled>
																											<label class="custom-control-label custom-label" for="<?= $coltypemod; ?>salesperson">
																					...
																					<div class="row justify-content-center">
																						<div class="col-lg-8">
																							<div class="row mb-3">
																								<div class="col-lg-6 my-1">
																									<div class="custom-control custom-checkbox mr-sm-2">
																										<input type="checkbox" class="custom-control-input" name="filterprepared" id="filterprepared" checked disabled>
																										<label class="custom-control-label custom-label" for="filterprepared">
																											Prepared By
																										</label>
																									</div>                  
																								</div>
																								<div class="col-lg-6 automodalchecking">
																									<select class="form-control form-control-sm select4 reportpreparedsearch" id="preparedby" name="preparedby">
																									<?php
																										if ($sqlviewreport['preparedby']=='all') {
																									?>
																									...
																									<?php
																											...
																											$sqlipro = mysqli_query($con, "SELECT preparedby FROM pairinvoices WHERE preparedby='".$sqlviewreport['preparedby']."' GROUP BY preparedby ORDER BY preparedby ASC");
																											while ($infopro = mysqli_fetch_array($sqlipro)){
																									?>
																										<option value="<?=$infopro['preparedby']?>" <?=($sqlviewreport['preparedby']==$infopro['preparedby'])?'selected':''?>>
																											<?=$infopro['preparedby']?>
																										</option>
																									<?php
																											}
																							...
																							<div class="row mb-3">
																								<div class="col-lg-6 my-1">
																									<div class="custom-control custom-checkbox mr-sm-2">
																										<input type="checkbox" class="custom-control-input" name="filterchecked" id="filterchecked" checked disabled>
																										<label class="custom-control-label custom-label" for="filterchecked">
																											Checked By
																										</label>
																									</div>                  
																								</div>
																								<div class="col-lg-6 automodalchecking">
																									<select class="form-control form-control-sm select4 reportcheckedsearch" id="checkedby" name="checkedby">
																									<?php
																										if ($sqlviewreport['checkedby']=='all') {
																									?>
																										<option value="all" data-foo="" data-receivable="" selected>
																											All
																										</option>
																									<?php
																											...
																											$sqlipro = mysqli_query($con, "SELECT checkedby FROM pairinvoices WHERE checkedby='".$sqlviewreport['checkedby']."' GROUP BY checkedby ORDER BY checkedby ASC");
																											while ($infopro = mysqli_fetch_array($sqlipro)){
																									?>
																										<option value="<?=$infopro['checkedby']?>" <?=($sqlviewreport['checkedby']==$infopro['checkedby'])?'selected':''?>>
																											<?=$infopro['checkedby']?>
																										</option>
																									<?php
																											}
											...
											<div class="tab-pane fade" id="nav-profile" role="tabpanel" aria-labelledby="nav-profile-tab">
												<div class="table-responsive m-3" id="histables">
													<table class="table table-bordered">
														...
														<tbody>
														<?php
															$sqluse=$con->prepare("SELECT * FROM pairusehistory WHERE usetype = 'REPsalesperson' AND useid = ? ORDER BY createdon DESC");
															...
															<div class="card" id="zoomforprint"  style="box-shadow: 0 4px 8px 0 rgba(0, 0, 0, 0.2), 0 6px 20px 0 rgba(0, 0, 0, 0.19); padding:10px; width:max-content;height: max-content;padding-bottom: 0px !important;" align="center">
																<div class="table-responsive" style="width: max-content !important;height: max-content !important;max-width:max-content !important; max-height:max-content !important;min-width:max-content !important; min-height:max-content !important;">
																	...
																	?>
																		...
																		<tr style="height:1px;">
																			<td width="100%" style="padding:10px;">
																				<table id="print-are1" style="border: 1px solid #eee;" width="100%">
																					<thead>
																						<tr>
																							<td class="text-uppercase" style="width:8%;border:1px solid #eee;padding-left: 10px;">
																							...
																							<td class="text-uppercase" style="width:8%;border:1px solid #eee;padding-left: 10px;">
																								<span style="font-size:13px;color:black;">
																									<?=$infomainaccessuserinvoice['modulename']?> Date
																								</span>
																							</td>
																							<td class="text-uppercase" style="width:8%;border:1px solid #eee;padding-left: 10px;">
																								<span style="font-size:13px;color:black;">
																									<?=$infomainaccessuserinvoice['modulename']?> Number
																								</span>
																							</td>
																							<td class="text-uppercase" style="width:8%;border:1px solid #eee;padding-left: 10px;">
																								<span style="font-size:13px;color:black;">
																									<?=$infomainaccessusercust['modulename']?> Name
																								</span>
																							</td>
																							<td class="text-uppercase" style="width:8%;border:1px solid #eee;padding-left: 10px;">
																								<span style="font-size:13px;color:black;">
																									<?=$infomainaccessusercust['modulename']?> City
																								</span>
																							</td>
																							<td class="text-uppercase" style="text-align: right !important;width: 8%;border:1px solid #eee;padding-right: 10px;">
																								<span style="font-size:13px;color:black;">
																					...
																					<tbody id="myTable">
																					<?php
																						$seldatas=$con->prepare("SELECT * FROM pairinvoices WHERE franchisesession=? AND createdid=? AND (invoicedate>=? AND invoicedate<=?) AND cancelstatus='0' ".(($sqlviewreport['preparedby']=='all')?' ':' AND preparedby="'.$sqlviewreport['preparedby'].'"')." ".(($sqlviewreport['checkedby']=='all')?' ':' AND checkedby="'.$sqlviewreport['checkedby'].'"')." GROUP BY invoicedate, invoiceno ORDER BY invoicedate,invoiceno ASC LIMIT 36");
																					...
																					?>
																						<tr style="vertical-align: middle; height: 30px !important;">
																							<td onclick="window.open('invoiceview.php?id=<?=$info['id']?>&invoiceno=<?=$info['invoiceno']?>&invoicedate=<?=$info['invoicedate']?>','_self')" style="text-align: left !important; font-size:12px; color:black; border:1px solid #eee; padding-left: 10px;">
																								Sales
																							</td>
																							<td onclick="window.open('invoiceview.php?id=<?=$info['id']?>&invoiceno=<?=$info['invoiceno']?>&invoicedate=<?=$info['invoicedate']?>','_self')" style="text-align: left !important; font-size:12px; color:black; border:1px solid #eee; padding-left: 10px;">
																							...
																							<td onclick="window.open('invoiceview.php?id=<?=$info['id']?>&invoiceno=<?=$info['invoiceno']?>&invoicedate=<?=$info['invoicedate']?>','_self')" style="text-align: right !important; font-size:12px; color:black; border:1px solid #eee; padding-right: 10px;">
																								<?= number_format((float)$info['totalamount'],2,'.',',') ?>
																								<input type="hidden" id="inputfortaxable<?=$rowinpid?>" class="form-control form-control-sm" value="<?= $info['totalamount'] ?>">
																							</td>
																							<td onclick="window.open('invoiceview.php?id=<?=$info['id']?>&invoiceno=<?=$info['invoiceno']?>&invoicedate=<?=$info['invoicedate']?>','_self')" style="text-align: right !important; font-size:12px; color:black; border:1px solid #eee; padding-right: 10px;">
																								<?= number_format((float)$info['totalvatamount'],2,'.',',') ?>
																							</td>
																							<td onclick="window.open('invoiceview.php?id=<?=$info['id']?>&invoiceno=<?=$info['invoiceno']?>&invoicedate=<?=$info['invoicedate']?>','_self')" style="text-align: right !important; font-size:12px; color:black; border:1px solid #eee; padding-right: 10px;">
																							...
																							<td style="text-align: right !important; font-size:12px; color:black; border:1px solid #eee; padding-right: 10px;padding-left: 10px;">
																								<input style="font-size:12px;padding: 3px;height: 20px;" type="number" id="inputforpercent<?=$rowinpid?>" step="any" class="form-control form-control-sm" value="0.00" onchange="calcpercent('<?=$rowinpid?>')" oninput="calcpercent('<?=$rowinpid?>')">
																							</td>
																							<td style="text-align: right !important; font-size:12px; color:black; border:1px solid #eee; padding-right: 10px;padding-left: 10px;">
																								<input style="font-size:12px;padding: 3px;height: 20px;" type="text" id="inputforamount<?=$rowinpid?>" class="form-control form-control-sm" readonly value="0.00">
																							</td>
																						</tr>
																					<?php
																						}
																						...
																						if (mysqli_num_rows($seldata)>0) {
																							$sqltotlistpages=$con->prepare("SELECT * FROM pairinvoices WHERE franchisesession=? AND createdid=? AND (invoicedate>=? AND invoicedate<=?) AND cancelstatus='0' ".(($sqlviewreport['preparedby']=='all')?' ':' AND preparedby="'.$sqlviewreport['preparedby'].'"')." ".(($sqlviewreport['checkedby']=='all')?' ':' AND checkedby="'.$sqlviewreport['checkedby'].'"')." GROUP BY invoicedate, invoiceno ORDER BY invoicedate,invoiceno ASC");
																		...
																		<?php
																			$seldatass=$con->prepare("SELECT invoiceno,invoicedate,totalamount FROM pairinvoices WHERE franchisesession=? AND createdid=? AND (invoicedate>=? AND invoicedate<=?) AND cancelstatus='0' ".(($sqlviewreport['preparedby']=='all')?' ':' AND preparedby="'.$sqlviewreport['preparedby'].'"')." ".(($sqlviewreport['checkedby']=='all')?' ':' AND checkedby="'.$sqlviewreport['checkedby'].'"')." GROUP BY invoicedate, invoiceno ORDER BY invoicedate,invoiceno ASC");
																			$seldatass->bind_param("ssss", $_SESSION['franchisesession'], $companymainid, $datefrom, $dateto);
																			$seldatass->execute();
																			$seldatas = $seldatass->get_result();
																			while($infos=$seldatas->fetch_array()){
																				$rowinpidpages = $infos['invoiceno'].str_replace('-', '', $infos['invoicedate']);
																		?>
																		<input type="hidden" id="taxablepage<?=$rowinpidpages?>" value="<?=$infos['totalamount']?>" class="taxablepagecls">
																		<input type="hidden" id="percentpage<?=$rowinpidpages?>" value="0.00" class="percentpagecls">
																		<input type="hidden" id="amountpage<?=$rowinpidpages?>" value="0.00" class="amountpagecls">
																		<?php
																			}
																		?>
																		<script type="text/javascript">
																		function calcpercent(idval) {
																			if ($("#inputforpercent" + idval).length) {
																				if ($("#inputforpercent"+idval).val()!='') {
																					$("#inputforamount"+idval).val((((parseFloat($("#inputforpercent"+idval).val()))*(parseFloat($("#inputfortaxable"+idval).val())))/100).toFixed(2));
																					$("#amountpage"+idval).val((((parseFloat($("#inputforpercent"+idval).val()))*(parseFloat($("#inputfortaxable"+idval).val())))/100).toFixed(2));
																					$("#percentpage"+idval).val(parseFloat($("#inputforpercent"+idval).val()));
																				}
																				else{
																					$("#inputforamount"+idval).val('0.00');
																					$("#amountpage"+idval).val('0.00');
																					$("#percentpage"+idval).val('0.00');
																				}
																			}
																		}
																		</script>
										...
										<table id="print-are3" style="border: 1px solid #eee;display: none;" width="100%">
											<thead>
												<tr>
													<td class="text-uppercase" style="width:8%;border:1px solid #eee;padding-left: 10px;"><span style="font-size:13px;color:black;">Type</span></td>
													<td class="text-uppercase" style="width:8%;border:1px solid #eee;padding-left: 10px;"><span style="font-size:13px;color:black;"><?=$infomainaccessuserinvoice['modulename']?> Date</span></td>
													<td class="text-uppercase" style="width:8%;border:1px solid #eee;padding-left: 10px;"><span style="font-size:13px;color:black;"><?=$infomainaccessuserinvoice['modulename']?> Number</span></td>
													<td class="text-uppercase" style="width:8%;border:1px solid #eee;padding-left: 10px;"><span style="font-size:13px;color:black;"><?=$infomainaccessusercust['modulename']?> Name</span></td>
													<td class="text-uppercase" style="width:8%;border:1px solid #eee;padding-left: 10px;"><span style="font-size:13px;color:black;"><?=$infomainaccessusercust['modulename']?> City</span></td>
													<td class="text-uppercase" style="text-align: right !important;width: 8%;border:1px solid #eee;padding-right: 10px;"><span style="font-size:13px;color:black;"> <?=$access['txttaxableinv']?></span></td>
													<td class="text-uppercase" style="text-align: right !important;width: 8%;border:1px solid #eee;padding-right: 10px;"><span style="font-size:13px;color:black;">Tax Value</span></td>
													<td class="text-uppercase" style="text-align: right !important;width: 8%;border:1px solid #eee;padding-right: 10px;"><span style="font-size:13px;color:black;">Total Value</span></td>
													<td class="text-uppercase" style="text-align: right !important;width: 8%;border:1px solid #eee;padding-right: 10px;"><span style="font-size:13px;color:black;">Prepared By</span></td>
													<td class="text-uppercase" style="text-align: right !important;width: 8%;border:1px solid #eee;padding-right: 10px;"><span style="font-size:13px;color:black;">Checked By</span></td>
													<td class="text-uppercase" style="text-align: right !important;width: 8%;border:1px solid #eee;padding-right: 10px;"><span style="font-size:13px;color:black;">Taxable %</span></td>
													<td class="text-uppercase" style="text-align: right !important;width: 8%;border:1px solid #eee;padding-right: 10px;"><span style="font-size:13px;color:black;">Amount</span></td>
												</tr>
											</thead>
											<tbody id="myTables">
											</tbody>
...
const pagechanges = (page, pageSize) => {
	...
	ajaxRequestview = $.ajax({
		type: "GET",
		url: 'reportviewsearch.php?term='+perpages+'&limitings='+($('#limitforpagenum').val())+'&datefrom=<?=$datefrom?>&dateto=<?=$dateto?>&dif=salesperson&preparedby=<?=$_GET['preparedby']?>&checkedby=<?=$_GET['checkedby']?>&lastornone='+lastornone+'',
		success: function (result) {
			$("#myTable").html(result);
			var oldpercentassings = document.getElementsByClassName("percentpagecls");
			for(i=0;i<=oldpercentassings.length;i++){
				idval = oldpercentassings[i].id.split('percentpage')[1];
				$("#inputforpercent"+idval).val($("#percentpage"+idval).val());
				$("#inputforamount"+idval).val($("#amountpage"+idval).val());
				if ($("#inputforpercent" + idval).length) {
					if ($("#inputforpercent"+idval).val()!='') {
						$("#inputforamount"+idval).val((((parseFloat($("#inputforpercent"+idval).val()))*(parseFloat($("#inputfortaxable"+idval).val())))/100).toFixed(2));
						$("#amountpage"+idval).val((((parseFloat($("#inputforpercent"+idval).val()))*(parseFloat($("#inputfortaxable"+idval).val())))/100).toFixed(2));
...
$(function() {
	...
	function(start, end, label) {
		...
		window.open("reportsalesperson.php?datefrom="+datefrom+"&dateto="+dateto+"&preparedby=<?=$_GET['preparedby']?>&checkedby=<?=$_GET['checkedby']?>","_self");
	});
});
...
function converHTMLFileToPDF() {
	...
	var finaltimer = setTimeout(function(){
		...
		for(i=0;i<oldpercentassings.length;i++){
			...
			$("#inputforpercent"+idval).val($("#percentpage"+idval).val());
			$("#inputforamount"+idval).val($("#amountpage"+idval).val());
			if ($("#percentpage" + idval).length) {
				if ($("#percentpage"+idval).val()!='') {
					$("#inputforamount"+idval).val((((parseFloat($("#percentpage"+idval).val()))*(parseFloat($("#taxablepage"+idval).val())))/100).toFixed(2));
					$("#amountpage"+idval).val((((parseFloat($("#percentpage"+idval).val()))*(parseFloat($("#taxablepage"+idval).val())))/100).toFixed(2));
					$("#percentpage"+idval).val(parseFloat($("#percentpage"+idval).val()));
					$("#inputforamountpdf"+idval).html((((parseFloat($("#percentpage"+idval).val()))*(parseFloat($("#taxablepage"+idval).val())))/100).toFixed(2));
					$("#inputforpercentpdf"+idval).html(parseFloat($("#percentpage"+idval).val()));
				}
				else{
					$("#inputforamount"+idval).val('0.00');
					$("#amountpage"+idval).val('0.00');
					$("#percentpage"+idval).val('0.00');
					$("#inputforpercentpdf"+idval).html('0.00');
					$("#inputforamountpdf"+idval).html('0.00');
				}
			}
		}
		...
		doc.autoTable({
			...
			didDrawPage: header, 
			columnStyles: {
				0: { halign: 'left', cellWidth: 42.75},
				1: { halign: 'left', cellWidth: 42.75},
				2: { halign: 'left', cellWidth: 42.75},
				3: { halign: 'left', cellWidth: 42.75},
				4: { halign: 'left', cellWidth: 42.75},
				5: { halign: 'right', cellWidth: 42.75},
				6: { halign: 'right', cellWidth: 42.75},
				7: { halign: 'right', cellWidth: 42.75},
				8: { halign: 'left', cellWidth: 42.75},
				9: { halign: 'left', cellWidth: 42.75},
				10: { halign: 'right', cellWidth: 42.75},
				11: { halign: 'right', cellWidth: 42.75}
			}
		});
		obj = '<object id="pdfObjObj" data="'+doc.output('bloburi')+'" type="application/pdf" width="90%" height="550"></object>';
		$.ajax({
			type: "GET",
			url: 'reporthistory.php?types=salesperson&titles=Sales Person&printdownpdfcsv=Printed PDF&fromto='+fromtopdf+'',
			success: function (result) {
				console.log(result);
			},
			error: function (error) {
...
function downloadaspdf() {
	...
	var finaltimer = setTimeout(function(){
		...
		$.ajax({
			type: "GET",
			url: 'reporthistory.php?types=salesperson&titles=Sales Person&printdownpdfcsv=Downloaded PDF&fromto='+fromtopdf+'',
			success: function (result) {
				console.log(result);
			},
			error: function (error) {
		...
		doc.save('Sales-Person-<?=$datefrom?>-To-<?=$dateto?>');
...
function downloadascsv() {
	...
	var finaltimer = setTimeout(function(){
		...
		link.setAttribute("download", "Sales-Person-<?=$datefrom?>-To-<?=$dateto?>");
		document.body.appendChild(link);
		link.click();
		$.ajax({
			type: "GET",
			url: 'reporthistory.php?types=salesperson&titles=Sales Person&printdownpdfcsv=Downloaded CSV&fromto='+fromtopdf+'',
			success: function (result) {
				console.log(result);
			},
			error: function (error) {
...
document.addEventListener('DOMContentLoaded', function() {
	...
	function makeAjaxRequest() {
		ajaxRequest = $.ajax({
			type: "GET",
			url: 'check.php?term=salesperson&datefrom=<?=$datefrom?>&dateto=<?=$dateto?>&preparedby=<?=$_GET['preparedby']?>&checkedby=<?=$_GET['checkedby']?>&limitfrom=0&limitto=10000',
			success: function (result) {
				if (result != mytabprev) {
