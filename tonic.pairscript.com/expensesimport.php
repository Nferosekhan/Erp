...
if(!empty($_FILES['expensesfile'] ['name']) && in_array($_FILES['expensesfile'] ['type'], $csvMimes)){
if(is_uploaded_file($_FILES['expensesfile'] ['tmp_name'])){
$csvFile=fopen($_FILES['expensesfile'] ['tmp_name'],'r');
fgetcsv($csvFile);
while(($line=fgetcsv($csvFile))!==FALSE){
$id=mysqli_real_escape_string($con, $line[0]);
$year=mysqli_real_escape_string($con, $line[1]);
$username=mysqli_real_escape_string($con, $line[2]);
$compusername=mysqli_real_escape_string($con, $line[3]);
$businesstype=mysqli_real_escape_string($con, $line[4]);
$mtype=mysqli_real_escape_string($con, $line[5]);
$createdon=mysqli_real_escape_string($con, $line[6]);
$mdate=mysqli_real_escape_string($con, $line[7]);
$mno=mysqli_real_escape_string($con, $line[8]);
$mcategory=mysqli_real_escape_string($con, $line[9]);
$details=mysqli_real_escape_string($con, $line[10]);
$amount=mysqli_real_escape_string($con, $line[11]);
$tdelete=mysqli_real_escape_string($con, $line[12]);
if ($id!='') {
$sqlexp = mysqli_query($con,"insert into pairexpenses set createdid='$companymainid', createdby='".$_SESSION["unqwerty"]."', franchisesession='".$_SESSION["franchisesession"]."', createdon='$createdon',  term='RECEIPT', headername='$mcategory', receiptno='$mno', receiptdate='$mdate', amount='$amount', paymentmode='$mtype', notes='$details',cancelstatus='$tdelete'");
}

}
if($sqlexp){
header("Location: expenses.php?remarks=Imported Successfully");
}
else{
header("Location: expenses.php?error=".mysqli_error($con));
}
}
}
else{
// $all="invalid expense type";
}
}
?>
<!DOCTYPE html>
...
<div class="card-body p-3">
 ...
 <p class="mb-3" style="color:black;font-size:20px;margin-top: -8px;"> <i class="fa fa-file-import"></i> Expense Import</p>

<div class="alert alert-info text-white">
Kindly Note that the New Expense List should contains existing Expense Name, otherwise it has been Overrided.
</div>
...
<div class="col-sm-4">
<label for="expensesfile" class="custom-label"><span class="text-danger">Expenses CSV File *</span></label>
</div>
<div class="col-sm-8">
<input type="file" class="form-control  form-control-sm" required id="expensesfile" name="expensesfile" accept=".csv">
</div>
</div>
</div>
</div>
...
<input class="btn btn-primary btn-sm btn-custom" type="submit" name="expensessubmit" value="Save">  <a class="btn btn-primary btn-sm btn-custom-grey" href="expenses.php">Cancel</a>
</div>
</div>
</form>
