...
$data1=mysqli_query($con, "select adjustment, adjustmentprefix, (adjustmentsuffix+1) as adjustmentsuffix from pairfranchises where tdelete='0' and id='".$_SESSION['franchisesession']."' order by id desc");
$info1=mysqli_fetch_array($data1);
// $sqlismodulespublicname=mysqli_query($con, "select * from pairmodules where moduletype='Inventory Adjustments' order by id  asc");
// $infomodulespublicname=mysqli_fetch_array($sqlismodulespublicname);
// $sqlismainaccesspublicname=mysqli_query($con, "select * from pairmainaccess where createdid='$companymainid' and moduletype='Inventory Adjustments' and franchiseid='".$_SESSION['franchisesession']."' order by id  asc");
// $infomainaccesspublicname=mysqli_fetch_array($sqlismainaccesspublicname);
// $sqlcode=mysqli_query($con,"select count(adjustmentno) from pairadjustments");
// $anscode=mysqli_fetch_array($sqlcode);
// $oldcode=$anscode[0];
// $adjustmentno=$oldcode+1;
// $publicsql=mysqli_query($con,"select count(publicid) from pairadjustments where createdid='$companymainid'");
...
// $privatesql=mysqli_query($con,"select count(privateid) from pairadjustments where createdid='$companymainid' and franchisesession='".$_SESSION['franchisesession']."'");
...
$sqlismainaccesspublicname=mysqli_query($con, "select * from pairmainaccess where createdid='$companymainid' and moduletype='Inventory Adjustments' and franchiseid='".$_SESSION['franchisesession']."' order by id  asc");
$infomainaccesspublicname=mysqli_fetch_array($sqlismainaccesspublicname);
$adjustmentno = str_pad(($infomainaccessuser['modulesuffix']+1), 0, "0", STR_PAD_LEFT);
$publiccode = $infomodulespublicname['publiccolumn'].str_pad(($infomainaccessuser['modulesuffix']+1), 0, "0", STR_PAD_LEFT);
$privatecode = ($infomainaccessuser['moduleprefix']).str_pad(($infomainaccessuser['modulesuffix']+1), 0, "0", STR_PAD_LEFT);
...
$sql2=mysqli_query($con, "select id from pairadjustments where franchisesession='".$_SESSION['franchisesession']."' and createdid='$companymainid' and adjustmentno='$adjustmentno' and adjustmentdate='$adjustmentdate'");
...
//     $expdate='';
// }
    // chartaccountname='$chartaccountname',chartaccountid='$chartaccountid',
if($productname!='')
{
...
// if($expdate!='')
// {
$sql5=mysqli_query($con, "update pairbatch set quantity=quantity+$newquantity where productid='$productid' and (batch='$batch' and (expdate='$expdate' OR expdate IS NULL))");
$sqluse=mysqli_query($con, "insert into pairusehistory set usetype='ADJUSTMENTS', createdon='$times',  createdby='".$_SESSION["unqwerty"]."', useid='$adjustmentno', uniqueid='".$salesid."', useremarks='INVENTORY ADJUSTED' ");
// }
// }
}
else
...
$sql3=mysqli_query($con, "update pairfranchises set adjustmentsuffix=adjustmentsuffix+1 where tdelete='0' and id='".$_SESSION['franchisesession']."'");
$sqluse=mysqli_query($con, "update pairusehistory set uniqueid='".$salesid."' WHERE usetype='ADJUSTMENTS' AND createdby='".$_SESSION["unqwerty"]."' AND useid='$adjustmentno' AND useremarks='INVENTORY ADJUSTED' ");
if($sql3)
{
 if(isset($_POST['submit1']))
{
echo '<script> window.open("adjustmentprint.php?adjustmentno='.$adjustmentno.'&adjustmentdate='.$adjustmentdate.'", "_blank");</script>';
echo '<script> window.location.href="adjustments.php?remarks=Added Successfully";</script>'; 
}
else
{
echo '<script> window.location.href="adjustments.php?remarks=Added Successfully";</script>';
...
if($sql)
{
$sql3=mysqli_query($con, "update pairmainaccess set modulesuffix=modulesuffix+1 where franchiseid='".$_SESSION['franchisesession']."' and moduletype='Inventory Adjustments'");
}
}
?>
<!DOCTYPE html>
...
<!--------------------------------------------- product add end --------------------------------------------->
<!------------------------------------------ Bulk Add Start ------------------------------------------>
<div class="modal fade" id="AddBulkProduct" tabindex="-1" role="dialog">
<div class="modal-dialog modal-lg" role="document" style="padding: 15px;">
<div class="modal-content">
<div class="modal-header" style="padding: 0.5rem 1rem;">
<h5 class="modal-title" id="exampleModalLabel">Add Items in Bulk</h5>
<span type="button" onclick="funesbulkproduct()" class="close" data-dismiss="modal" aria-label="Close">
<span aria-hidden="true" id="procloseicon">&times;</span>
</span>
</div>
<!-- <form action="" method="post" enctype="multipart/form-data" class="form-horizontal mt-2" role="form"> -->
<div class="modal-body mbsub" id="bulkpromodal">
<div class="row" style="padding: 0px 30px;">
   <div class="col-lg-6" style="margin-bottom:1rem;border: 1px solid #ced4da;">
    <style type="text/css">
        #bulkprosearch:focus{
            box-shadow: 0 2px 4px 0 rgba(26,42,88,.11);
            border: 1px solid #0f7aff;
        }
    </style>
    <input type="text" name="bulkprosearch" id="bulkprosearch" class="form-control form-control-sm" style="margin-top: 6px;border: 1px solid #ced4da;height: 34px;margin-bottom: 6px;" placeholder="Search Your Products">
    <div class="mobscrolls" style="max-height: 370px;overflow-y: auto;overflow-x: hidden;" id="proscrolldiv">
<?php
$count=1;
$sqli=mysqli_query($con, "select t1.category,t1.itemmodule,t1.productname,t1.id,t1.openingstock,t2.batch,t2.expdate,t2.quantity from pairproducts t1,pairbatch t2 where t1.createdid='$companymainid' and ((t1.franchisesession='".$_SESSION["franchisesession"]."' and t1.pvisiblity='PRIVATE') or t1.pvisiblity='PUBLIC') and t1.itemmodule='Products' and t2.productid=t1.id and t1.isactive='0' order by t1.productname asc limit 15");
...
document.getElementById("selected_items").innerHTML = '<div id="addinforms" style="position:relative;height:370px;"><div style="display:inline-table;top:50%;transform:translate(-50%,-50%);left:50%;position:relative;text-align:center;">Click the item names from the left pane to select them</div><div>';
var additionalinfos = 0;
$('.ticoruntic').click(function(){
    console.log("check1");
if (document.getElementById("ticoruntic"+this.id+"").style.color === 'rgb(27, 188, 155)') {
    console.log("check2");
document.getElementById("ticoruntic"+this.id+"").style.color = 'darkgrey';
document.getElementById("ticoruntic"+this.id+"").style.visibility = 'hidden';
// total_selected_items-=1;
document.getElementById("total_selected_items").innerHTML = parseInt(document.getElementById("total_selected_items").innerHTML)-1;
var parofthis = 'def'+this.id.replace(/[\/,.\-)]/g, '')+'row'+this.id.split('adjust')[0]+'';
document.getElementById(""+parofthis+"").remove();
document.getElementById(this.id+"").style.backgroundColor = '#fff';
if(document.querySelectorAll(".mybulkqty").length>0){
document.getElementById("addinforms").style.display = 'none';
}
else{
document.getElementById("addinforms").style.display = 'block';
}
}
else {
    console.log("check3");
document.getElementById("ticoruntic"+this.id+"").style.color = 'rgb(27, 188, 155)';
document.getElementById("ticoruntic"+this.id+"").style.visibility = 'visible';
// total_selected_items+=1;
document.getElementById("total_selected_items").innerHTML = parseInt(document.getElementById("total_selected_items").innerHTML)+1;
var newDiv = document.createElement('div');
var nxtid = this.id.split('adjust')[0];
var delfulid = this.id;
document.getElementById(this.id+"").style.backgroundColor = 'rgb(241, 244, 249)';
    console.log("id"+nxtid);
if (document.getElementById("selected_items").innerHTML.includes("row"+nxtid+"")) {
additionalinfos+=1;
newDiv.innerHTML = ('<div class="row mybulkqty" batch="'+$(this).attr("forbat")+'" expdate="'+$(this).attr("forexp")+'" delidatt="'+delfulid+'" id="def'+delfulid.replace(/[\/,.\-)]/g, '')+'row'+nxtid+'" style="margin-bottom:15px !important;"><div class="col-sm-5" style="padding-bottom:10px;line-height: 15px;">'+$(this).attr("pnamesshow")+'<br>(Stock on Hand : '+$(this).attr("forstock")+')</div><div class="col-sm-5 col-9" style="padding-bottom:10px;">    <div class="input-group mb-3"><div class="input-group-prepend decrementofadjust" onclick="decrementqytval('+nxtid+''+additionalinfos+')" style="border: 1px solid lightgrey;"><span class="input-group-text" style="font-size: 27px;color:#a3a7aa;">-</span></div><input type="number" class="form-control form-control-sm" placeholder="1" style="max-width: 60px;border: 1px solid lightgrey;height: 27px;text-align:center;" value="1" id="qtyval'+nxtid+''+additionalinfos+'"><div class="input-group-append incrementofadjust" onclick="incrementqytval('+nxtid+''+additionalinfos+')" style="border: 1px solid lightgrey;"><span class="input-group-text" style="font-size: 23px;color:#a3a7aa;">+</span></div></div></div><div class="col-sm-2 col-3" style="padding-bottom:10px;" id="close'+nxtid+'" onclick="closeselectedpro('+nxtid+')"><img src="assets/img/delete-row.png" width="15" height="15" style="border-radius: 10px;position:relative;top:3px;"></div></div>');
}
else{
newDiv.innerHTML = ('<div class="row mybulkqty" batch="'+$(this).attr("forbat")+'" expdate="'+$(this).attr("forexp")+'" delidatt="'+delfulid+'" id="def'+delfulid.replace(/[\/,.\-)]/g, '')+'row'+nxtid+'" style="margin-bottom:15px !important;"><div class="col-sm-5" style="padding-bottom:10px;line-height: 15px;">'+$(this).attr("pnamesshow")+'<br>(Stock on Hand : '+$(this).attr("forstock")+')</div><div class="col-sm-5 col-9" style="padding-bottom:10px;">    <div class="input-group mb-3"><div class="input-group-prepend decrementofadjust" onclick="decrementqytval('+nxtid+')" style="border: 1px solid lightgrey;"><span class="input-group-text" style="font-size: 27px;color:#a3a7aa;">-</span></div><input type="number" class="form-control form-control-sm" placeholder="1" style="max-width: 60px;border: 1px solid lightgrey;height: 27px;text-align:center;" value="1" id="qtyval'+nxtid+'"><div class="input-group-append incrementofadjust" onclick="incrementqytval('+nxtid+')" style="border: 1px solid lightgrey;"><span class="input-group-text" style="font-size: 23px;color:#a3a7aa;">+</span></div></div></div><div class="col-sm-2 col-3" style="padding-bottom:10px;" id="close'+nxtid+'" onclick="closeselectedpro('+nxtid+')"><img src="assets/img/delete-row.png" width="15" height="15" style="border-radius: 10px;position:relative;top:3px;"></div></div>');
}
document.getElementById("selected_items").appendChild(newDiv);
if(document.querySelectorAll(".mybulkqty").length>0){
document.getElementById("addinforms").style.display = 'none';
}
else{
...
// $(".decrementofadjust").click(function(){
//     var idofthis = this.id.split('decrement')[1];
//     var ourInput = $("#qtyval"+idofthis+"");
//     var currentValue = parseInt(ourInput.val());
//     ourInput.val(currentValue - 1);
// });
// $(".incrementofadjust").click(function(){
//     var idofthis = this.id.split('increment')[1];
//     var ourInput = $("#qtyval"+idofthis+"");
//     var currentValue = parseInt(ourInput.val());
//     ourInput.val(currentValue + 1);
// });
// document.getElementById("total_selected_items").innerHTML = total_selected_items;
});
});
function incrementqytval(incval) {
var currentVal = $("#qtyval"+incval+"").val();
$("#qtyval"+incval+"").val(parseInt($("#qtyval"+incval+"").val())+1);
}
function decrementqytval(decval) {
var currentVal = $("#qtyval"+decval+"").val();
$("#qtyval"+decval+"").val(parseInt($("#qtyval"+decval+"").val())-1);
}
function closeselectedpro(closeval) {
var parentidval = document.getElementById("close"+closeval+"").parentNode.id;
var yes_or_no = confirm('Delete this item?');
if (yes_or_no) {
var remove_the_count = 'ticoruntic'+$("#"+parentidval).attr("delidatt");
var remove_the_counts = $("#"+parentidval).attr("delidatt");
document.getElementById(""+remove_the_count+"").style.color = 'darkgrey';
document.getElementById(""+remove_the_count+"").style.visibility = 'hidden';
document.getElementById("total_selected_items").innerHTML = parseInt(document.getElementById("total_selected_items").innerHTML)-1;
document.getElementById(""+parentidval+"").remove();
document.getElementById(remove_the_counts+"").style.backgroundColor = '#fff';
}
}
</script>
    </div>
   ...
   <div class="col-lg-6" style="margin-bottom:1rem;">
    <h3 style="font-size: 20px;font-weight: 400;border-bottom: 1px solid lightgray;padding: 0px 0 6px 15px;border-bottom: 1px solid #eee;">Selected Items <span id="total_selected_items" style="border:1px solid #bac8dc;background-color: #fff;color: #000;min-width: 40px;border-radius: 0px;padding: 5px 13px;margin-left: 4px;letter-spacing: 1.4px;font-size: 14px;display: inline-block;text-align: center;white-space: nowrap;max-width: 100%;position: relative;top: -3px;">0</span></h3>
    <div class="mobscrolls" style="max-height: 370px;overflow-y: auto;overflow-x: hidden;">
...
<div class="modal-footer mfsub">
<div class="col">
<button onclick="funaddbulkproduct()" class="btn btn-primary btn-sm btn-custom arlina-button expand-left" type="submit"  name="submitbulkproduct" id="submitbulkproduct" value="Submit">
<span class="label">Add Items</span> <span class="spinner"></span>
</button>
<button type="button" class="btn btn-primary btn-sm btn-custom-grey" onclick="funesbulkproduct()">Cancel</button> </div>
</div>
<!-- </form> -->
</div>
</div>
...
<script type="text/javascript">
$("#bulkprosearch").on("keyup", function() {
    var value = $(this).val().toLowerCase();
// ajax for get
$.ajax({
type: "GET",
url: 'adjustprosearch.php?term=0&type=typing&val='+ value +'',
success: function (result) {
$("#proscrolldiv").html(result);
if (result == ''){
$.ajax({
type: "GET",
url: 'adjustprosearch.php?term=0&type=scroll',
success: function (results) {
$("#proscrolldiv").html(results);},
...
$('#proscrolldiv').on('scroll', function() {
...
$.ajax({
type: "GET",
url: 'adjustprosearch.php?term=' + sIndex + '&type=scroll',
success: function (result) {
$("#proscrolldiv").append(result);
var additionalinfos = 0;
$('.ticoruntic').click(function(){
    console.log("check1");
if (document.getElementById("ticoruntic"+this.id+"").style.color === 'rgb(27, 188, 155)') {
...
function funaddbulkproduct() {
$("#purchasetable tbody").html(' ');
var selectedItems = document.querySelector("#selected_items");
if (selectedItems.innerHTML!='') {
var mybulkqtyDivs = selectedItems.querySelectorAll(".mybulkqty");
var totalCount = mybulkqtyDivs.length;
if (totalCount>0) {
for (var i = 0; i < totalCount; i++) {
addnewrow(i+1);
var idforselected = mybulkqtyDivs[i].id;
var selectnxtlevel = idforselected.split('row')[1];
var selectproname = mybulkqtyDivs[i].children[0].innerHTML;
$('.adjustingproduct1').append('<option value="' + selectnxtlevel + '">' + selectproname + '</option>');
$('select[name^="product'+(i+1)+'"] option[value="' + selectnxtlevel + '"]').attr("selected","selected");
$('#product'+(i+1)+'').val(selectnxtlevel).change();
$('#newquantity'+(i+1)).val(document.getElementById(""+idforselected+"").querySelector("input[type='number']").value);
$('#batch'+(i+1)).val($("#"+idforselected).attr("batch"));
$('#productexpdateval'+(i+1)).html($("#"+idforselected).attr("expdate"));
$('#expdate'+(i+1)).val($("#"+idforselected).attr("expdate").split('/')[2]+'-'+$("#"+idforselected).attr("expdate").split('/')[1]+'-'+$("#"+idforselected).attr("expdate").split('/')[0]);
addnewrow(i+2);
}
}
else{
...
$("#AddBulkProduct").modal("hide");
}
function funesbulkproduct() {
$("#AddBulkProduct").modal("hide");
}
</script>
<!--------------------------------------------- Bulk Add End --------------------------------------------->
...
<input type="hidden" id="prooriginpage">
     <?php
     // if ((in_array('Inventory Adjustment Information', $fieldadd))) {
        ?>
<div class="row">
...
<div class="col-lg-7">
<input type="text" class="form-control  form-control-sm" id="adjustmentno" name="adjustmentno" required value="<?=str_pad(($infomainaccessuser['modulesuffix']+1), 0, "0", STR_PAD_LEFT)?>" readonly>
</div>
</div>
</div>
</div>
<div class="row" <?=(in_array('Public Id', $fieldadd))?'':'style="display:none !important;"'?>>
<div class="col-lg-12">
<div class="form-group row">
<div class="col-lg-5">
...
<div class="col-lg-7">
<input type="text" class="form-control  form-control-sm" id="publicid" name="publicid" required value="<?=$infomodulespublicname['publiccolumn'].str_pad(($infomainaccessuser['modulesuffix']+1), 0, "0", STR_PAD_LEFT)?>" readonly>
</div>
</div>
</div>
</div>
<div class="row" <?=(in_array('Private Id', $fieldadd))?'':'style="display:none !important;"'?>>
<div class="col-lg-12">
<div class="form-group row">
<div class="col-lg-5">
...
<div class="col-lg-7">    
<input type="text" class="form-control  form-control-sm" id="privateid" name="privateid" required value="<?=($infomainaccessuser['moduleprefix']).str_pad(($infomainaccessuser['modulesuffix']+1), 0, "0", STR_PAD_LEFT)?>" readonly>
</div>
</div>
</div>
</div>
...
<div class="col-lg-7">
<input type="date" class="form-control  form-control-sm" id="adjustmentdate" name="adjustmentdate" required value="<?=date('Y-m-d')?>">
</div>
</div>
</div>
</div>
...
<div class="col-lg-7">
<input type="hidden" name="chartaccountid" id="chartaccountid" required>
<input type="text" class="form-control  form-control-sm" id="chartaccountname" name="chartaccountname" required>

</div>
</div>
</div>
</div> -->
<div class="row" <?=(in_array('Reason', $fieldadd))?'':'style="display:none !important;"'?>>
<div class="col-lg-12">
<div class="form-group row">
<div class="col-lg-5" style="padding-right:0px">
...
while ($info = mysqli_fetch_array($sqli)) {
?>
<option value="<?= $info['reason'] ?>"><?= $info['reason'] ?></option>
<?php
}
?>
</select>
...
<div class="row" <?=(in_array('Reference Number', $fieldadd))?'':'style="display:none !important;"'?>>
<div class="col-lg-12">
<div class="form-group row">
<div class="col-lg-5">
...
<div class="row" <?=(in_array('Description', $fieldadd))?'':'style="display:none !important;"'?>>
<div class="col-lg-12">
<div class="form-group row">
<div class="col-lg-5">
...
<div class="col-lg-7">
<textarea class="form-control  form-control-sm" id="description" name="description" style="height:45px !important;"></textarea>
</div>
</div>
</div>
</div>
...
<th></th>
<th width="28%" <?=(in_array('Item Details', $fieldadd))?'':'style="display:none !important;"'?>>ITEM DETAILS<span class="text-danger"> *</span></th>
     <?php
...
?>
<th <?=(in_array('Quantity Available', $fieldadd))?'':'style="display:none !important;"'?>>Quantity Available<span class="text-danger"> *</span></th>
<th <?=(in_array('New Quantity On Hand', $fieldadd))?'':'style="display:none !important;"'?>>New Quantity On Hand<span class="text-danger"> *</span></th>
<th <?=(in_array('Quantity Adjusted', $fieldadd))?'':'style="display:none !important;"'?>>Quantity Adjusted</th>
<th></th>
</tr>
</thead>
<tbody>
<?php
if (isset($_GET['types'])) {
    if ($_GET['types']=='zerobyprefer') {
        if ($_GET['status']=='enabled') {
            $activatecheck = "pairproducts.isactive='0'";
        }
        elseif($_GET['status']=='disabled'){
            $activatecheck = "pairproducts.isactive='1'";
        }
$expiryproducts = mysqli_query($con,"select pairbatch.productid,pairbatch.productname,pairbatch.quantity,pairbatch.batch,pairbatch.expdate from pairbatch join pairproducts on pairproducts.id=pairbatch.productid and ".$activatecheck." where pairbatch.createdid='$companymainid' and pairbatch.franchisesession='".$_SESSION["franchisesession"]."' and (pairbatch.quantity>0 or pairbatch.quantity<0) GROUP BY pairbatch.batch,pairbatch.expdate,pairbatch.productname order by pairbatch.expdate asc,pairbatch.quantity desc limit ".$access['expinvadj']."");
$inczero = 1;
if (mysqli_num_rows($expiryproducts)>0) {
while($expiryprofetch = mysqli_fetch_array($expiryproducts)){
$profet = mysqli_query($con,"select itemmodule from pairproducts where id='".$expiryprofetch['productid']."'");
$profetching = mysqli_fetch_array($profet);
?>
<tr>
<td class="priority" style="display:none"> 1</td>
<td class="tdmove for1per"><svg version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" x="0" y="0" viewBox="0 0 512 512" xml:space="preserve" class="icon icon-drag align-text-bottom" style="color:#cccccc"><circle cx="153.6" cy="451" r="61"></circle><circle cx="153.6" cy="256" r="61"></circle><circle cx="153.6" cy="61" r="61"></circle><circle cx="358.4" cy="256" r="61"></circle><circle cx="358.4" cy="61" r="61"></circle><circle cx="358.4" cy="451" r="61"></circle></svg></td>
<td data-label="ITEM DETAILS" style="padding-top: 1px !important; <?=(in_array('Item Details', $fieldadd))?'':'display:none !important;'?>" class="iteminfo"><input type="hidden" name="productid[]" id="productid<?=$inczero?>" value="<?=$expiryprofetch['productid']?>">
<input type="hidden" name="productname[]" id="productname<?=$inczero?>" value="<?=$expiryprofetch['productname']?>">
<div onclick="andus()" id="selecttheproduct">
<select class="form-control  form-control-sm product proitemselect adjustingproduct1" name="product[]" id="product<?=$inczero?>" required onChange="productchange(<?=$inczero?>)">
<option value="<?=$expiryprofetch['productid']?>" selected><?=$expiryprofetch['productname']?></option>
</select>
</div>
<span class="badge" style="display:inline-block; width:75px; padding:3px; margin:5px 3px; background-color: #57b729; font-size:75%;border-radius: 0px !important;" id="itemmodulespan<?=$inczero?>"><?=$profetching['itemmodule']?></span>
</td>
<td data-label="BATCH" <?=($access['batchexpiryval']==1)?'':'style="display:none !important;"'?> class="batchinfo">
<div>
<input autocomplete="off" type="text" name="batch[]" id="batch<?=$inczero?>" onClick="batchget(<?=$inczero?>);" onFocus="batchget(<?=$inczero?>);"  class="form-control form-control-sm proitemselect productselectwidth" style="margin-bottom: 3px !important;padding: 0px !important;" value="<?=$expiryprofetch['batch']?>"></div>
<span id="productexpdatespan<?=$inczero?>" style="display:inline-block; font-size:11px;">EXPIRY:</span>
<input autocomplete="off" type="date" name="expdate[]" id="expdate<?=$inczero?>" class="form-control form-control-sm proitemselect" style="display:none;width: 94px !important;padding: 0px !important;height: 18px !important;" value="<?=$expiryprofetch['expdate']?>"> <span id="productexpdateval<?=$inczero?>" style=" font-size:11px;" class="text-primary"><?=(($expiryprofetch['expdate']!='')?date($datemainphp,strtotime($expiryprofetch['expdate'])):'')?></span>
<span id="productexpdateedit<?=$inczero?>" style="display:inline-block; font-size:11px; cursor:pointer" class="text-blue" onclick="editexpdate(<?=$inczero?>)"><i class="fa fa-edit"></i></span>
<span id="productexpdateupdate<?=$inczero?>" style="display:none; font-size:11px; cursor:pointer" class="text-success" onclick="changeexpdate(<?=$inczero?>)"><i class="fa fa-save"></i></span>
<div>
<div id="outfordone<?=$inczero?>" class="dvi" style="display:none;width: 250px;"></div>
<input type="text" id="errbatch<?=$inczero?>" style="display:none;">
</div>
</td>
<td data-label="Quantity Available" class="qtyinfo" <?=(in_array('Quantity Available', $fieldadd))?'':'style="display:none !important;"'?>><input type="number" step="0.01" name="quantity[]" required id="quantity<?=$inczero?>" class="form-control form-control-sm bordernoneinput productselectwidth" onChange="productcalc(<?=$inczero?>)" readonly value="<?=$expiryprofetch['quantity']?>"></td>
<td data-label="New Quantity On Hand" class="newqtyinfo" <?=(in_array('New Quantity On Hand', $fieldadd))?'':'style="display:none !important;"'?>><input type="number" step="0.01" name="newquantity[]" placeholder="Eg. +10, -10" required id="newquantity<?=$inczero?>" class="form-control form-control-sm bordernoneinput productselectwidth" onChange="productcalc(<?=$inczero?>)" value="<?=($expiryprofetch['quantity'] > 0) ? "-" . $expiryprofetch['quantity'] : ltrim($expiryprofetch['quantity'], "-")?>"></td>
<td data-label="Quantity Adjusted" class="qtyadjinfo" <?=(in_array('Quantity Adjusted', $fieldadd))?'':'style="display:none !important;"'?>><input type="number" step="0.01" name="adjustment[]" required id="adjustment<?=$inczero?>" class="form-control form-control-sm bordernoneinput productselectwidth" onChange="productcalc(<?=$inczero?>)" readonly></td>
<td class="for1per"><a class="btn-delete" style="cursor:pointer"><img src="assets/img/delete-row.png" width="15" height="15" style="border-radius: 10px;"></a></td>
</tr>
<?php
$inczero++;
}
...
function productchangetypes(lineNo)
{
...
<td class="priority" style="display:none"> 1</td>
<td class="tdmove for1per"><svg version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" x="0" y="0" viewBox="0 0 512 512" xml:space="preserve" class="icon icon-drag align-text-bottom" style="color:#cccccc"><circle cx="153.6" cy="451" r="61"></circle><circle cx="153.6" cy="256" r="61"></circle><circle cx="153.6" cy="61" r="61"></circle><circle cx="358.4" cy="256" r="61"></circle><circle cx="358.4" cy="61" r="61"></circle><circle cx="358.4" cy="451" r="61"></circle></svg></td>
<td data-label="ITEM DETAILS" style="padding-top: 1px !important; <?=(in_array('Item Details', $fieldadd))?'':'display:none !important;'?>" class="iteminfo"><input type="hidden" name="productid[]" id="productid<?=$inc?>" value="<?=$expiryprofetch['productid']?>">
<input type="hidden" name="productname[]" id="productname<?=$inc?>" value="<?=$expiryprofetch['productname']?>">
<div onclick="andus()" id="selecttheproduct">
<select class="form-control  form-control-sm product proitemselect adjustingproduct1" name="product[]" id="product<?=$inc?>" required onChange="productchange(<?=$inc?>)">
<option value="<?=$expiryprofetch['productid']?>" selected><?=$expiryprofetch['productname']?></option>
</select>
</div>
<span class="badge" style="display:inline-block; width:75px; padding:3px; margin:5px 3px; background-color: #57b729; font-size:75%;border-radius: 0px !important;" id="itemmodulespan<?=$inc?>"><?=$profetching['itemmodule']?></span>
</td>
<td data-label="BATCH" <?=($access['batchexpiryval']==1)?'':'style="display:none !important;"'?> class="batchinfo">
<div>
<input autocomplete="off" type="text" name="batch[]" id="batch<?=$inc?>" onClick="batchget(<?=$inc?>);" onFocus="batchget(<?=$inc?>);"  class="form-control form-control-sm proitemselect productselectwidth" style="margin-bottom: 3px !important;padding: 0px !important;" value="<?=$expiryprofetch['batch']?>"></div>
<span id="productexpdatespan<?=$inc?>" style="display:inline-block; font-size:11px;">EXPIRY:</span>
<input autocomplete="off" type="date" name="expdate[]" id="expdate<?=$inc?>" class="form-control form-control-sm proitemselect" style="display:none;width: 94px !important;padding: 0px !important;height: 18px !important;" value="<?=$expiryprofetch['expdate']?>"> <span id="productexpdateval<?=$inc?>" style=" font-size:11px;" class="text-primary"><?=(($expiryprofetch['expdate']!='')?date($datemainphp,strtotime($expiryprofetch['expdate'])):'')?></span>
<span id="productexpdateedit<?=$inc?>" style="display:inline-block; font-size:11px; cursor:pointer" class="text-blue" onclick="editexpdate(<?=$inc?>)"><i class="fa fa-edit"></i></span>
<span id="productexpdateupdate<?=$inc?>" style="display:none; font-size:11px; cursor:pointer" class="text-success" onclick="changeexpdate(<?=$inc?>)"><i class="fa fa-save"></i></span>
<div>
<div id="outfordone<?=$inc?>" class="dvi" style="display:none;width: 250px;"></div>
<input type="text" id="errbatch<?=$inc?>" style="display:none;">
</div>
</td>
<td data-label="Quantity Available" class="qtyinfo" <?=(in_array('Quantity Available', $fieldadd))?'':'style="display:none !important;"'?>><input type="number" step="0.01" name="quantity[]" required id="quantity<?=$inc?>" class="form-control form-control-sm bordernoneinput productselectwidth" onChange="productcalc(<?=$inc?>)" readonly value="<?=$expiryprofetch['quantity']?>"></td>
<td data-label="New Quantity On Hand" class="newqtyinfo" <?=(in_array('New Quantity On Hand', $fieldadd))?'':'style="display:none !important;"'?>><input type="number" step="0.01" name="newquantity[]" placeholder="Eg. +10, -10" required id="newquantity<?=$inc?>" class="form-control form-control-sm bordernoneinput productselectwidth" onChange="productcalc(<?=$inc?>)"></td>
<td data-label="Quantity Adjusted" class="qtyadjinfo" <?=(in_array('Quantity Adjusted', $fieldadd))?'':'style="display:none !important;"'?>><input type="number" step="0.01" name="adjustment[]" required id="adjustment<?=$inc?>" class="form-control form-control-sm bordernoneinput productselectwidth" onChange="productcalc(<?=$inc?>)" readonly></td>
<td class="for1per"><a class="btn-delete" style="cursor:pointer"><img src="assets/img/delete-row.png" width="15" height="15" style="border-radius: 10px;"></a></td>
</tr>
<?php
$inc++;
...
elseif ($_GET['types']=='pro') {
$proid = $_GET['proid'];
$sqlstock=mysqli_query($con,"SELECT * FROM pairbatch where createdid='$companymainid' and franchisesession='".$_SESSION["franchisesession"]."' and productid='".$proid."'");
$incpro = 1;
while($infostock=mysqli_fetch_array($sqlstock)){
$profet = mysqli_query($con,"select itemmodule from pairproducts where id='".$infostock['productid']."'");
$profetching = mysqli_fetch_array($profet);
$expdate = $infostock['expdate']; // Example: '2025-04'

// Check if the input matches YYYY-MM format
if (preg_match('/^\d{4}-\d{2}$/', $expdate)) {
    // Parse the date and determine the last day of the month
    $dateObj = DateTime::createFromFormat('Y-m', $expdate);
    $finalExpDate = $dateObj->format('Y-m-t'); // Get the full date with the last day of the month (YYYY-MM-DD)
} else {
    // If input is already in correct format or invalid, return it as-is
    $finalExpDate = $expdate;
}
?>
<tr>
<td class="priority" style="display:none"> 1</td>
<td class="tdmove for1per"><svg version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" x="0" y="0" viewBox="0 0 512 512" xml:space="preserve" class="icon icon-drag align-text-bottom" style="color:#cccccc"><circle cx="153.6" cy="451" r="61"></circle><circle cx="153.6" cy="256" r="61"></circle><circle cx="153.6" cy="61" r="61"></circle><circle cx="358.4" cy="256" r="61"></circle><circle cx="358.4" cy="61" r="61"></circle><circle cx="358.4" cy="451" r="61"></circle></svg></td>
<td data-label="ITEM DETAILS" style="padding-top: 1px !important; <?=(in_array('Item Details', $fieldadd))?'':'display:none !important;'?>" class="iteminfo"><input type="hidden" name="productid[]" id="productid<?=$incpro?>" value="<?=$infostock['productid']?>">
<input type="hidden" name="productname[]" id="productname<?=$incpro?>" value="<?=$infostock['productname']?>">
<div onclick="andus()" id="selecttheproduct">
<select class="form-control  form-control-sm product proitemselect adjustingproduct1" name="product[]" id="product<?=$incpro?>" required onChange="productchange(<?=$incpro?>)">
<option value="<?=$infostock['productid']?>" selected><?=$infostock['productname']?></option>
</select>
</div>
<span class="badge" style="display:inline-block; width:75px; padding:3px; margin:5px 3px; background-color: #57b729; font-size:75%;border-radius: 0px !important;" id="itemmodulespan<?=$incpro?>"><?=$profetching['itemmodule']?></span>
</td>
<td data-label="BATCH" <?=($access['batchexpiryval']==1)?'':'style="display:none !important;"'?> class="batchinfo">
<div>
<input autocomplete="off" type="text" name="batch[]" id="batch<?=$incpro?>" onClick="batchget(<?=$incpro?>);" onFocus="batchget(<?=$incpro?>);"  class="form-control form-control-sm proitemselect productselectwidth" style="margin-bottom: 3px !important;padding: 0px !important;" value="<?=$infostock['batch']?>"></div>
<span id="productexpdatespan<?=$incpro?>" style="display:inline-block; font-size:11px;">EXPIRY:</span>
<input autocomplete="off" type="date" name="expdate[]" id="expdate<?=$incpro?>" class="form-control form-control-sm proitemselect" style="display:none;width: 94px !important;padding: 0px !important;height: 18px !important;" value="<?=$finalExpDate?>"> <span id="productexpdateval<?=$incpro?>" style=" font-size:11px;" class="text-primary"><?=(($finalExpDate!='')?date($datemainphp,strtotime($finalExpDate)):'')?></span>
<span id="productexpdateedit<?=$incpro?>" style="display:inline-block; font-size:11px; cursor:pointer" class="text-blue" onclick="editexpdate(<?=$incpro?>)"><i class="fa fa-edit"></i></span>
<span id="productexpdateupdate<?=$incpro?>" style="display:none; font-size:11px; cursor:pointer" class="text-success" onclick="changeexpdate(<?=$incpro?>)"><i class="fa fa-save"></i></span>
<div>
<div id="outfordone<?=$incpro?>" class="dvi" style="display:none;width: 250px;"></div>
<input type="text" id="errbatch<?=$incpro?>" style="display:none;">
</div>
</td>
<td data-label="Quantity Available" class="qtyinfo" <?=(in_array('Quantity Available', $fieldadd))?'':'style="display:none !important;"'?>><input type="number" step="0.01" name="quantity[]" required id="quantity<?=$incpro?>" class="form-control form-control-sm bordernoneinput productselectwidth" onChange="productcalc(<?=$incpro?>)" readonly value="<?=$infostock['quantity']?>"></td>
<td data-label="New Quantity On Hand" class="newqtyinfo" <?=(in_array('New Quantity On Hand', $fieldadd))?'':'style="display:none !important;"'?>><input type="number" step="0.01" name="newquantity[]" placeholder="Eg. +10, -10" required id="newquantity<?=$incpro?>" class="form-control form-control-sm bordernoneinput productselectwidth" onChange="productcalc(<?=$incpro?>)"></td>
<td data-label="Quantity Adjusted" class="qtyadjinfo" <?=(in_array('Quantity Adjusted', $fieldadd))?'':'style="display:none !important;"'?>><input type="number" step="0.01" name="adjustment[]" required id="adjustment<?=$incpro?>" class="form-control form-control-sm bordernoneinput productselectwidth" onChange="productcalc(<?=$incpro?>)" readonly></td>
<td class="for1per"><a class="btn-delete" style="cursor:pointer"><img src="assets/img/delete-row.png" width="15" height="15" style="border-radius: 10px;"></a></td>
</tr>
<?php
$incpro++;
...
<td class="priority" style="display:none"> 1</td>
<td class="tdmove for1per"><svg version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" x="0" y="0" viewBox="0 0 512 512" xml:space="preserve" class="icon icon-drag align-text-bottom" style="color:#cccccc"><circle cx="153.6" cy="451" r="61"></circle><circle cx="153.6" cy="256" r="61"></circle><circle cx="153.6" cy="61" r="61"></circle><circle cx="358.4" cy="256" r="61"></circle><circle cx="358.4" cy="61" r="61"></circle><circle cx="358.4" cy="451" r="61"></circle></svg></td>
<td data-label="ITEM DETAILS" style="padding-top: 1px !important; <?=(in_array('Item Details', $fieldadd))?'':'display:none !important;'?>" class="iteminfo"><input type="hidden" name="productid[]" id="productid1">
<input type="hidden" name="productname[]" id="productname1">
<div onclick="andus()" id="selecttheproduct">
<select class="form-control  form-control-sm product proitemselect adjustingproduct1" name="product[]" id="product1" required onChange="productchange(1)">
<option value="" selected disabled>Select</option>
</select>
</div>
<span class="badge" style="display:none; width:75px; padding:3px; margin:5px 3px; background-color: #57b729; font-size:75%;border-radius: 0px !important;" id="itemmodulespan1"></span>
</td>
<td data-label="BATCH" <?=($access['batchexpiryval']==1)?'':'style="display:none !important;"'?> class="batchinfo">
<div>
<input autocomplete="off" type="text" name="batch[]" id="batch1" onClick="batchget(1);" onFocus="batchget(1);"  class="form-control form-control-sm proitemselect productselectwidth" style="margin-bottom: 3px !important;padding: 0px !important;"></div>
<span id="productexpdatespan1" style="display:none; font-size:11px;">EXPIRY:</span>
<input autocomplete="off" type="date" name="expdate[]" id="expdate1" class="form-control form-control-sm proitemselect" style="display:none;width: 94px !important;padding: 0px !important;height: 18px !important;"> <span id="productexpdateval1" style=" font-size:11px;" class="text-primary"></span>
...
<div>
<div id="outfordone1" class="dvi" style="display:none;width: 250px;"></div>
<input type="text" id="errbatch1" style="display:none;">
</div>
</td>
<td data-label="Quantity Available" class="qtyinfo" <?=(in_array('Quantity Available', $fieldadd))?'':'style="display:none !important;"'?>><input type="number" step="0.01" name="quantity[]" required id="quantity1" class="form-control form-control-sm bordernoneinput productselectwidth" onChange="productcalc(1)" readonly></td>
<td data-label="New Quantity On Hand" class="newqtyinfo" <?=(in_array('New Quantity On Hand', $fieldadd))?'':'style="display:none !important;"'?>><input type="number" step="0.01" name="newquantity[]" placeholder="Eg. +10, -10" required id="newquantity1" class="form-control form-control-sm bordernoneinput productselectwidth" onChange="productcalc(1)"></td>
<td data-label="Quantity Adjusted" class="qtyadjinfo" <?=(in_array('Quantity Adjusted', $fieldadd))?'':'style="display:none !important;"'?>><input type="number" step="0.01" name="adjustment[]" required id="adjustment1" class="form-control form-control-sm bordernoneinput productselectwidth" onChange="productcalc(1)" readonly></td>
<td class="for1per"><a class="btn-delete" style="cursor:pointer"><img src="assets/img/delete-row.png" width="15" height="15" style="border-radius: 10px;"></a></td>
</tr>
<?php
}
...
<div class="col-lg-6 col-md-6 col-sm-12 col-12" style=" <?=(in_array('Add Items in Bulk', $fieldadd))?'':'display:none !important;'?>">
<a class="btn btn-primary btn-sm btn-custom-grey" style="background-color: #e9ecef;padding: 2px 4.5px 1px 4.5px !important; margin-bottom:0.25rem;" id="bulkproductadd" data-bs-toggle="modal" data-bs-target="#AddBulkProduct"><i style="font-size: 14px;color:#0066cc;" class="fa fa-plus-circle"></i> Add Items in Bulk</a>
</div>
</div>
</p>
</div>
...
<script>
$(document).ready(function () {
let lineNo = parseInt($("#totalProCount").html()) + 1;
// alert(lineNo);
...
console.log(obj[0]);
$("#chartaccountname").val(obj[0].chartaccountname);
$("#chartaccountid").val(obj[0].id);
...
markup =
'<tr><td style="width:3%;"><svg version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" x="0" y="0" viewBox="0 0 512 512" xml:space="preserve" class="icon icon-drag align-text-bottom" style="color:#cccccc"><circle cx="153.6" cy="451" r="61"></circle><circle cx="153.6" cy="256" r="61"></circle><circle cx="153.6" cy="61" r="61"></circle><circle cx="358.4" cy="256" r="61"></circle><circle cx="358.4" cy="61" r="61"></circle><circle cx="358.4" cy="451" r="61"></circle></svg></td><td style="width:18%;"><input type="hidden" name="productid[]" id="productid1"><input type="text" name="proproductname[]" id="proproductname1' +lineNo +
'" required class="form-control form-control-sm bordernoneinput bor" style="height:21px;padding: 0px;" oninput="title(this)" data-toggle="tooltip" title=""></td><td style="width:11%;"><div class="input-group mb-3 input-group-sm"><div class="input-group-prepend"><span class="input-group-text" style="color: #495057;padding: 8px 3.75px;height:21px;"><?php echo $rescurrency[0]; ?></span></div><input type="age" name="quantity[]" required id="quantity1' +
lineNo +
'" class="form-control form-control-sm bordernoneinput bor" style="height:21px;width: 24px;text-align: right;padding: 0px;" onChange="productcalc(1)"></div><td style="width: 6%;"><div class="input-group"><div class="input-group-prepend"><div class="input-group-text"  style="color: #495057;padding: 8px 3.75px;height:21px;"><?php echo $rescurrency[0]; ?></div></div><input  oninput="increaseWidth(this)" style="height:21px;width: 24px;text-align: right;padding: 0px;" placeholder="0.00" type="age" name="productrate[]"  required id="productrate1' +
lineNo +
'" class="form-control form-control-sm bordernoneinput rup" onChange="productcalc(1)"></div></td><td style="width:18%;"><input type="number" step="0.01" name="vat[]" id="vat1' +
lineNo +
...
alert('Please Enter New Adjustment Reason Name');
...
$("#adjustmentreason").on("select2:open", function() {
document.getElementById("configureunits").innerHTML = "New Adjustment Reason";
});
</script>
<!--term end-->
...
<script>
$( "#chartaccountname" ).autocomplete({
   source: 'chartaccountsearch.php', select: function (event, ui) { $("#chartaccountname").val(ui.item.chartaccountname); $("#chartaccountid").val(ui.item.id);}, minLength: 0
 });
</script>
