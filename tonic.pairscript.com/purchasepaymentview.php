...
$infomainaccessuserven=mysqli_fetch_array($sqlismainaccessuserven);
     if(isset($_GET['id']))
{
$sql=mysqli_query($con,"select id, createdon, term, receiptdate, receiptno, cancelstatus, vendorid, vendorname, paymentmode, amount, notes, type, publicid, privateid from pairpurchasepayments where franchisesession='".$_SESSION['franchisesession']."' and createdid='$companymainid' and id='".$_GET['id']."'");
...
$sqlismainaccessbill=mysqli_query($con, "select * from pairmainaccess where (userid='$companymainid' and createdid='0') and moduletype='Bills' order by id  asc");
...
$sql=mysqli_query($con,"select module, id, createdon, term, receiptdate, receiptno, cancelstatus, vendorid, vendorname, paymentmode, amount, notes, publicid, privateid from pairpurchasepayments where franchisesession='".$_SESSION['franchisesession']."' and createdid='$companymainid' and id='".$_GET['id']."'");
...
?>
<a class="btn btn-primary btn-sm btn-custom-grey" href="purchasepaymentedit.php?id=<?=$id?>" id="btngopage"><i class="fa fa-pencil-alt"></i> <span class="mobreswords "> Edit</span></a>
  <?php 
}
?>
  </span>
...
$franchisesession = $_SESSION['franchisesession'];
$payid = $_GET['id'];
$sql = "SELECT billno, billdate FROM pairpurchasepayhistory WHERE franchisesession = ? AND createdid = ? AND vendorid = ? AND paymentid = ?";
$stmt = $con->prepare($sql);
$stmt->bind_param("ssss", $franchisesession, $companymainid, $vendorid, $payid);
$stmt->execute();
...
$sqls=mysqli_query($con,"select invnumber,billno, billdate, grandtotal, paidstatus, vendorid, vendorname from pairbills where franchisesession='".$_SESSION['franchisesession']."' and createdid='$companymainid' and vendorid='$vendorid' and  billno='".$bill['billno']."' and billdate='".$bill['billdate']."' GROUP BY billdate, billno  order by billdate asc, billno asc");
while($infos=mysqli_fetch_array($sqls))
{
$s=0;
if($infos['paidstatus']=='1')
{
$sqlse=mysqli_query($con,"select paymentid from pairpurchasepayhistory where franchisesession='".$_SESSION['franchisesession']."' and createdid='$companymainid' and  billno='".$infos['billno']."' and billdate='".$infos['billdate']."' and vendorid='$vendorid' and paymentid='".$_GET['id']."'");
...
$p='';
$p1=0;
$sqlse=mysqli_query($con,"select amount, paymentid from pairpurchasepayhistory where franchisesession='".$_SESSION['franchisesession']."' and createdid='$companymainid' and  billno='".$infos['billno']."' and billdate='".$infos['billdate']."' and vendorid='$vendorid'");
...
$totpros++;
    ...
    while($infos=mysqli_fetch_array($sqls))
    {
        ...
        else
        {
            $sta='2';
                        ...
                        $sqlijournals=$con->prepare("SELECT customerid, chartaccountname, customername, sum(ledgerdebit) AS ledgerdebit, sum(ledgercredit) AS ledgercredit, notes,ledgerdate, ledgerno, referenceno, referencedate, totalledgerdebit, totalledgercredit, id, type FROM pairledgers WHERE franchisesession=? AND createdid=? AND ledgerno=? AND ledgerdate=? AND type='bill payment' GROUP BY type,chartaccountid,ledgerdate,ledgerno ORDER BY ledgerdate,ledgerno,type ASC");
                          $sqlijournals->bind_param("ssss", $_SESSION['franchisesession'], $companymainid, $infos['billno'], $infos['billdate']);
    ...
    <div class="tab-pane fade" id="nav-profile" role="tabpanel" aria-labelledby="nav-profile-tab">
      ...
      <tbody>
        <?php
      $sqluse=mysqli_query($con, "select * from pairusehistory where usetype='PURCHASEPAY' and useid='$id' order by createdon desc");
...
<?php
if($infofranch['purchasepaymenttemplate']=='1')
{
?>
<div id="printalble">
...
<td>
<table width="100%">
<tr style="<?=($access['purpaymadebranchphone']=='0')?'display:none;':''?>">
...
<tr>
<td width="10%" style="font-size:12px;padding: 0px !important;line-height: 15px !important;<?=($access['purpaymadebranchemail']=='0')?'visibility:hidden;':''?>">E-mail </td>
<td width="90%" style="font-size:12px;padding: 0px !important;line-height: 15px !important;"><input type="text" readonly value=": <?=$infofranch['email']?>" style="padding:0px;color:black;width: 206px !important;height: 18px !important;border: none !important;outline: none !important;display: inline-flex;font-size: 12px;<?=($access['purpaymadebranchemail']=='0')?'visibility:hidden;':''?>" class="form-control form-control-sm"><span style="<?=(($access['purpaymadeprintdlno20']=='1')?'display:inline-flex;':'display:none;')?>white-space: nowrap;width: 169px;overflow: hidden;float: right;margin-right: 9px;">DL No 20 &nbsp; : <?=$infofranch['dlno20']?></span></td>
</tr>
<tr>
<td width="10%" style="font-size:12px;padding: 0px !important;line-height: 15px !important;<?=($access['purpaymadebranchgstin']=='0')?'visibility:hidden;':''?>">GSTIN </td>
<td width="90%" style="font-size:12px;padding: 0px !important;line-height: 15px !important;"><span style="display: inline-flex;white-space: nowrap;width: 134px;overflow: hidden;<?=($access['purpaymadebranchgstin']=='0')?'visibility:hidden;':''?>">: <?=$infofranch['gstno']?></span><span style="<?=(($access['purpaymadeprintdlno21']=='1')?'display:inline-flex;':'display:none;')?>white-space: nowrap;width: 169px;overflow: hidden;float: right;margin-right: 9px;">DL No 21 &nbsp; : <?=$infofranch['dlno21']?></span></td>
</tr>
</table>
</td>
</tr>
...
<?php
// if (($infomainaccessuser['billprintgstin']=='show')||($sqlcusfetch['gstno']!='')&&($infomainaccessuser['billprintgstin']!='hide')) {
?>
...
<th style="border:1px solid #ddd !important;vertical-align:middle !important;padding: 0px 6px !important;height: 13px !important;"><span style="display:inline-block;width:100%;color:black !important;font-size:12px !important;"><?=strtoupper($infomainaccessbill['modulename'])?> DATE</span></th>
<th style="border:1px solid #ddd !important;vertical-align:middle !important;padding: 0px 6px !important;height: 13px !important;"><span style="display:inline-block;width:100%;color:black !important;font-size:12px !important;"><?=strtoupper($infomainaccessbill['modulename'])?> NUMBER</span></th>
<th style="border:1px solid #ddd !important;vertical-align:middle !important;padding: 0px 6px !important;height: 13px !important;"><span style="display:inline-block;width:100%;color:black !important;font-size:12px !important;">INVOICE NUMBER</span></th>
...
<th style="border:1px solid #ddd !important;vertical-align:middle !important;padding: 0px 6px !important;height: 13px !important;"><span style="display:inline-block;width:100%;color:black !important;font-size:12px !important;text-align:right;"><?=strtoupper($infomainaccessbill['modulename'])?> AMOUNT</span></th>
...
$totpros++;
    ...
    while($infos=mysqli_fetch_array($sqls))
    {
        ...
        echo '<input type="hidden" name="nos[]" id="bill'.$i.'" value="'.$infos['billno'].'">
        <input type="hidden" name="dates[]" id="date'.$i.'" value="'.$infos['billdate'].'">
        ...
        echo '<td data-label="'. strtoupper($infomainaccessbill['modulename']) .' DATE" style="font-size:12px !important;vertical-align:middle !important;padding: 1px 6px !important;text-align:left;"><input type="text" readonly value="'.date('d/m/Y',strtotime($infos['billdate'])).'" style="text-align: left;padding:0px;color:black;height: 18px !important;border: none !important;outline: none !important;display: inline-flex;font-size: 12px;" class="form-control form-control-sm"></td>';
        echo '<td data-label="'. strtoupper($infomainaccessbill['modulename']) .' NUMBER" style="font-size:12px !important;vertical-align:middle !important;padding: 1px 6px !important;text-align:left;"><input type="text" readonly value="'.$infos['billno'].'" style="text-align: left;padding:0px;color:black;height: 18px !important;border: none !important;outline: none !important;display: inline-flex;font-size: 12px;" class="form-control form-control-sm"></td>';
        echo '<td data-label="INVOICE NUMBER" style="font-size:12px !important;vertical-align:middle !important;padding: 1px 6px !important;text-align:left;"><input type="text" readonly value="'.$infos['invnumber'].'" style="text-align: left;padding:0px;color:black;height: 18px !important;border: none !important;outline: none !important;display: inline-flex;font-size: 12px;" class="form-control form-control-sm"></td>';
        // echo '<td data-label="NAME" style="font-size:12px !important;vertical-align:middle !important;padding: 1px 6px !important;text-align:left;"><input type="text" readonly value="'.(($infos['vendorid']!='')?$infos['vendorname']:$infos['vendorname']).'" style="text-align: left;padding:0px;color:black;height: 18px !important;border: none !important;outline: none !important;display: inline-flex;font-size: 12px;" class="form-control form-control-sm"></td>';
        echo '<td data-label="'. strtoupper($infomainaccessbill['modulename']) .' AMOUNT" style="font-size:12px !important;vertical-align:middle !important;padding: 1px 6px !important;text-align:right;"><span >'.$rescurrency[0].'</span><input type="text" readonly value="'.(number_format((float)$infos['grandtotal'],2,'.','')).'" style="text-align: right;padding:0px;color:black;height: 18px !important;border: none !important;outline: none !important;display: inline-flex;font-size: 12px;width:90%;" class="form-control form-control-sm"></td>';
...
<thead style="background-color: #eee;">
<tr class="info">
<th style="border:1px solid #ddd !important;vertical-align:middle !important;padding: 0px 6px !important;height: 13px !important;"><span style="display:inline-block;width:100%;color:black !important;font-size:12px !important;">DESCRIPTION</span></th>
...
<td data-label="DESCRIPTION" style="font-size:12px !important;vertical-align:middle !important;padding: 1px 6px !important;text-align:left;"><input type="text" readonly value="<?=strtoupper($infomainaccessuserven['modulename'])?> ADVANCE RECEIVED" style="text-align: left;padding:0px;color:black;height: 18px !important;border: none !important;outline: none !important;display: inline-flex;font-size: 12px;" class="form-control form-control-sm"></td>
<td data-label="AMOUNT" style="font-size:12px !important;vertical-align:middle !important;padding: 1px 6px !important;text-align:left;"><input type="text" readonly value="<?=number_format((float)$info['amount'],2,'.','')?>" style="text-align: right;padding:0px;color:black;height: 18px !important;border: none !important;outline: none !important;display: inline-flex;font-size: 12px;" class="form-control form-control-sm"></td>
</tr>
<tr>
<td>&nbsp;</td>
<td>&nbsp;</td>
...
<tr>
<td width="43%">Total &nbsp;</td>
<td width="57%" style="font-size: 13px !important;"> : <b><span style="margin-right:-3px position:relative;top:1px;!important;" class="rupeeforprint"><?=$rescurrency[0]?></span><input type="text" readonly value="<?=(number_format((float)$info['amount'],2,'.',''))?>" style="color:black;width: 86% !important;float: right;text-align: right;font-weight: 700 !important;font-size: 13px !important;height: 18px !important;border: none !important;outline: none !important;padding: 0px 6px !important;" class="form-control form-control-sm"></b></td>
</tr>
</table>
</td>
</tr>
...
<p align="right" class="mt-3" style="margin-right:-25px; cursor:pointer" id="templatetext">Template: '<?=($infofranch['purchasepaymenttemplate']=='0')?'Standard A5 Landscape':''?><?=($infofranch['purchasepaymenttemplate']=='1')?'Standard A4 Portrait':''?> ' <a data-bs-toggle="modal" data-bs-target="#changepurchasepaymentModal" class="text-blue">Change</a></p>
</div>
 <!-- Modal -->
<div class="modal fade" id="changepurchasepaymentModal" tabindex="-1" role="dialog" aria-labelledby="changepurchasepaymentModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="changepurchasepaymentModalLabel" style="font-weight:normal;">Choose Template</h5>
        <button type="button" class="btn-close text-dark" data-bs-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
...
if (r == true) {
    window.location.href='changetemplatereceipt.php?template=purchasepayment&id=<?=$id?>&publicid=<?=$info['publicid']?>&receiptno=<?=$_GET['receiptno']?>&receiptdate=<?=$info['receiptdate']?>&temptype=0&submt=submt';
} else {
...
var r = confirm('Are you sure want to Change this Template?');
if (r == true) {
    window.location.href='changetemplatereceipt.php?template=purchasepayment&id=<?=$id?>&publicid=<?=$info['publicid']?>&receiptno=<?=$_GET['receiptno']?>&receiptdate=<?=$info['receiptdate']?>&temptype=1&submt=submt';
} else {
...
$.ajax({
type: "GET",
url: 'purchasepaydompdf.php?term=purchasepayment&names=<?= $infomainaccessuser['modulename'] ?>-<?=$info['privateid']?>-<?=$_GET['receiptdate']?>&receiptno=<?=$_GET['receiptno']?>&receiptdate=<?=$_GET['receiptdate']?>&sizes=a4&id=<?=$_GET['id']?>',
...
if($infofranch['purchasepaymenttemplate']=='0'){
?>
<script>
function converHTMLFileToPDF() {
$('#pdfObj').html('<div class="text-center"><span id="timer">Please wait <span id="time"></span> Seconds</span></div><br><img src="loading.gif" width="100%">');
...
$.ajax({
type: "GET",
url: 'purchasepaydompdf.php?term=purchasepayment&names=<?= $infomainaccessuser['modulename'] ?>-<?=$info['privateid']?>-<?=$_GET['receiptdate']?>&receiptno=<?=$_GET['receiptno']?>&receiptdate=<?=$_GET['receiptdate']?>&sizes=a5&id=<?=$_GET['id']?>',
success: function (result) {
if (result && result.length > 0) {       
result=$.parseJSON( result );
Timer=result.timer;
...
header("Location:purchasepayments.php?error=No Information Found");
}
}
else{
header("Location:purchasepayments.php?error=No Information Found");  
