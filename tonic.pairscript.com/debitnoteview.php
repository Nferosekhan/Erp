...
if((isset($_GET['debitnoteno']))&&(isset($_GET['debitnotedate']))){
	$debitnoteno=mysqli_real_escape_string($con, $_GET['debitnoteno']);
	$debitnotedate=mysqli_real_escape_string($con, $_GET['debitnotedate']);
...
<?php
	...
	$datefetch=$dateformat->fetch_array();
	if ($datefetch['date']=='DD/MM/YYYY') {
		if (($access['debitnoteprintexpdate']=='0')&&($access['debitnoteprintexpmonth']=='0')&&($access['debitnoteprintexpyear']=='0')) {
			$dateforexp = '';
		}
		if (($access['debitnoteprintexpdate']=='1')&&($access['debitnoteprintexpmonth']=='1')&&($access['debitnoteprintexpyear']=='1')) {
			$dateforexp = 'd/m/Y';
		}
		if (($access['debitnoteprintexpdate']=='0')&&($access['debitnoteprintexpmonth']=='1')&&($access['debitnoteprintexpyear']=='1')) {
			$dateforexp = 'm/Y';
		}
		if (($access['debitnoteprintexpdate']=='1')&&($access['debitnoteprintexpmonth']=='0')&&($access['debitnoteprintexpyear']=='1')) {
			$dateforexp = 'd/Y';
		}
		if (($access['debitnoteprintexpdate']=='1')&&($access['debitnoteprintexpmonth']=='1')&&($access['debitnoteprintexpyear']=='0')) {
			$dateforexp = 'd/m';
		}
		if (($access['debitnoteprintexpdate']=='1')&&($access['debitnoteprintexpmonth']=='0')&&($access['debitnoteprintexpyear']=='0')) {
			$dateforexp = 'd';
		}
		if (($access['debitnoteprintexpdate']=='0')&&($access['debitnoteprintexpmonth']=='1')&&($access['debitnoteprintexpyear']=='0')) {
			$dateforexp = 'm';
		}
		if (($access['debitnoteprintexpdate']=='0')&&($access['debitnoteprintexpmonth']=='0')&&($access['debitnoteprintexpyear']=='1')) {
			$dateforexp = 'Y';
		}
	}
	//FOR DATE FORMAT
...
?>
	...
	<?php
		...
		if((isset($_GET['debitnoteno']))&&(isset($_GET['debitnotedate']))){
			$debitnoteno=mysqli_real_escape_string($con, $_GET['debitnoteno']);
			$debitnotedate=mysqli_real_escape_string($con, $_GET['debitnotedate']);
			$sqls=$con->prepare("SELECT * FROM pairdebitnotes WHERE franchisesession=? AND createdid=? AND debitnoteno=? AND debitnotedate=? ORDER BY id ASC");
			$sqls->bind_param("ssss", $_SESSION['franchisesession'], $companymainid, $debitnoteno, $debitnotedate);
			$sqls->execute();
				$sql = $sqls->get_result();
			$count=1;
			if($sql->num_rows>0){
					...
					<div class="card mb-4 mt-5">
						<div class="card-body p-3" style="font-family:'Myriad Set Pro','Helvetica Neue',Helvetica,Arial,sans-serif;">
							<div class="row">
								...
								<div class="col-lg-6">
									<span style="float:right">
										...
										<a style="margin:4.5px 4.5px !important;" class="btn btn-primary btn-sm btn-custom-grey" href="vendorrefundadd.php?no=<?=$debitnoteno?>&date=<?=$debitnotedate?>&type=debitnote" id="recordpaymentlink">
									...
									?>
										<a class="btn btn-primary btn-sm btn-custom-grey" href="debitnoteedit.php?id=<?=$_GET['id']?>&debitnoteno=<?=$debitnoteno?>&debitnotedate=<?=$debitnotedate?>" style="margin:4.5px 4.5px !important;">
									...
									<?php
										...
										if(($rows[0]['cancelstatus']!="1")&&(($paidamount - $paidamountdr)<=0)){
									?>
										<ul class="dropdown-menu  dropdown-menu-end customdropdown me-sm-n4 mt-1" aria-labelledby="dropdownMenuButton" style="display: none;">   
											<div style="background-color: #3c3c46;margin-top: 0px !important;cursor: pointer;"> 
												...
												<li class="nav-item" style="margin-top:-22px !important;">
													<a class="nav-link" href="debitnotecancel.php?debitnoteno=<?=$debitnoteno?>&debitnotedate=<?=$debitnotedate?>&cancelstatus=<?=($rows[0]['cancelstatus']=='0')?'1':'0'?>" onclick="return confirm('Are you sure want to <?=($rows[0]['cancelstatus']=='0')?'Cancel':'Enable'?> this Bill?')">
							...
							<div class="tab-content" id="nav-tabContent">
								<div class="tab-pane fade" id="nav-journal" role="tabpanel" aria-labelledby="nav-journal-tab">
									<div class="table-responsive m-3" id="journaltables">
										<table class="table table-bordered">
											<tbody>
											<?php
												$sqlijournals=$con->prepare("SELECT customerid, chartaccountname, customername, sum(ledgerdebit) AS ledgerdebit, sum(ledgercredit) AS ledgercredit, notes,ledgerdate, ledgerno, referenceno, referencedate, totalledgerdebit, totalledgercredit, id, type FROM pairledgers WHERE franchisesession=? AND createdid=? AND ledgerno=? AND ledgerdate=? AND (type='debitnote' OR type='debitnote payment') GROUP BY type,chartaccountid,ledgerdate,ledgerno ORDER BY ledgerdate,ledgerno,type ASC");
													$sqlijournals->bind_param("ssss", $_SESSION['franchisesession'], $companymainid, $debitnoteno, $debitnotedate);
								...
								<div class="tab-pane fade" id="nav-profile" role="tabpanel" aria-labelledby="nav-profile-tab">
									<div class="table-responsive m-3" id="histables">
										<table class="table table-bordered">
											...
											<tbody>
											<?php
												$sqluse=$con->prepare("SELECT * FROM pairusehistory WHERE (usetype = 'DEBITNOTES' AND (useid = ? AND (uniqueid = ? OR uniqueid = ?))) ORDER BY createdon DESC");
													$sqluse->bind_param("sss", $debitnoteno, $_GET['id'], $debitnoteno);
													$sqluse->execute();
											...
											?>
												<tr>
													...
													<td data-label="DETAILS">
														...
														<br>
														<span>
															<?=((str_contains($infouse['useremarks'],'DEBITNOTE CREATED'))?'Created By':'Changed By')?>
														</span>
								...
								<?php
									$sqlismainaccessusers=$con->prepare("SELECT moduleno, moduleprefix, modulesuffix,debitnoteprintgstin,modulename,debitnoteprintpos,viewprintdlno20,viewprintdlno21 FROM pairmainaccess WHERE franchiseid = ? AND moduletype = 'Debit Notes' ORDER BY id ASC");
											...
											<div class="row d-flex justify-content-center">
												<div class="col-lg-8 col-md-12 justify-content-center">
																					...
																					<td style="text-align:center;height: 80px !important;">
																						<table width="100%">
																							<tr>
																								...
																								<td width="51.5%" style="text-align:left;vertical-align: top;padding-left: 10px !important;border-right: 0px solid #cccccc;">
																									<table width="100%">
																										...
																										<tr>
																											<td>
																												<table width="100%">
																													<tr style="<?=($access['debitnotebranchphone']=='0')?'display:none;':''?>">
																													...
																													<tr>
																														<td width="10%" style="font-size:12px;padding: 0px !important;line-height: 15px !important;<?=($access['debitnotebranchemail']=='0')?'visibility:hidden;':''?>">
																															E-mail
																														</td>
																														<td width="90%" style="font-size:12px;padding: 0px !important;line-height: 15px !important;">
																															<input type="text" readonly value=": <?=$info['email']?>" style="padding:0px;color:black;width: 206px !important;height: 18px !important;border: none !important;outline: none !important;display: inline-flex;font-size: 12px;<?=($access['debitnotebranchemail']=='0')?'visibility:hidden;':''?>" class="form-control form-control-sm">
																															<span style="<?=(($access['debitnoteprintdlno20']=='1')?'display:inline-flex;':'display:none;')?>white-space: nowrap;width: 169px;overflow: hidden;float: right;margin-right: 9px;">
																																DL No 20 &nbsp; : <?=$info['dlno20']?>
																															</span>
																														</td>
																													</tr>
																													<tr>
																														<td width="10%" style="font-size:12px;padding: 0px !important;line-height: 15px !important;<?=($access['debitnotebranchgstin']=='0')?'visibility:hidden;':''?>">
																															GSTIN
																														</td>
																														<td width="90%" style="font-size:12px;padding: 0px !important;line-height: 15px !important;">
																															<span style="display: inline-flex;white-space: nowrap;width: 134px;overflow: hidden;<?=($access['debitnotebranchgstin']=='0')?'visibility:hidden;':''?>">
																																: <?=$info['gstno']?>
																															</span>
																															<span style="<?=(($access['debitnoteprintdlno21']=='1')?'display:inline-flex;':'display:none;')?>white-space: nowrap;width: 169px;overflow: hidden;float: right;margin-right: 9px;">
																								...
																								<td width="34.5%">
																									<table width="100%">
																										<tr <?=(($access['debitnotebank']=='1')?'':'style="display:none;"')?>>
																										...
																										<tr <?=(($access['debitnotename']=='1')?'':'style="display:none;"')?>>
																											<td style="padding: 0px 6px !important;font-size: 11px !important;line-height: 15px !important;text-align: left;" width="35%">
																										...
																										<tr <?=(($access['debitnoteaccnumber']=='1')?'':'style="display:none;"')?>>
																											<td style="padding: 0px 6px !important;font-size: 11px !important;line-height: 15px !important;text-align: left;" width="35%">
																										...
																										<tr <?=(($access['debitnoteifsccode']=='1')?'':'style="display:none;"')?>>
																											<td style="padding: 0px 6px !important;font-size: 11px !important;line-height: 15px !important;text-align: left;" width="35%">
																										...
																										<tr <?=(($access['debitnotebranchandcity']=='1')?'':'style="display:none;"')?>>
																											<td style="padding: 0px 6px !important;font-size: 11px !important;line-height: 15px !important;text-align: left;" width="35%">
																								...
																								<td style="padding: 0px !important;">
																									<table width="100%">
																										...
																										<tr <?=((in_array('GSTIN', $fieldview))?'':'style="display:none;"')?>>
																											<td width="38%" style="<?=((($infomainaccessuser['debitnoteprintgstin']=='show')||($rows[0]['gstno']!='')&&($infomainaccessuser['debitnoteprintgstin']!='hide'))?'':'display:none;')?>font-size:12px;padding:0px 6px !important;line-height: 15px !important;">
																												GSTIN
																											</td>
																											<td width="62%" style="<?=((($infomainaccessuser['debitnoteprintgstin']=='show')||($rows[0]['gstno']!='')&&($infomainaccessuser['debitnoteprintgstin']!='hide'))?'':'display:none;')?>font-size:12px;padding: 0px !important;line-height: 15px !important;">
																												: 
																											<?php
																												if (($infomainaccessuser['debitnoteprintgstin']=='show')||($rows[0]['gstno']!='')&&($infomainaccessuser['debitnoteprintgstin']!='hide')) {
																											?>
																					...
																					<td width="33%" style="white-space: normal !important;vertical-align: middle;padding: 0px !important;">
																						...
																						?>
																							...
																							?>
																								<td style="padding: 0px !important;font-size: 14px !important;line-height: 15px !important;text-align: left !important;" width="45%">
																									:   <b>
																											<?=date($date,strtotime($rows[0]['debitnotedate']))?>
																										</b>
																								</td>
																							</tr>
																							<tr>
																							...
																							<table width="100%" style="border:1px solid #cccccc 0px 0px 0px !important;line-height: 13px !important;height: 226px;">
																								...
																								?>
																									...
																									<?php
																										...
																										$pos ='';
																										if (($infomainaccessuser['debitnoteprintpos']=='show')||($rows[0]['pos']!='')&&($infomainaccessuser['debitnoteprintpos']!='hide')) {
																								...
																								<td style="padding: 0px !important;">
																									<table id="gsttable" width="100%" style="line-height: 13px !important;padding: 0px !important;border: 1px solid #cccccc;border-collapse: collapse;">
																										...
																										<?php
																											...
																											$sqlitaxess=$con->prepare("SELECT * FROM pairdebitnotes WHERE franchisesession = ? AND createdid = ? AND debitnoteno=? and debitnotedate=? ORDER BY id ASC");
																												$sqlitaxess->bind_param("ssss", $_SESSION['franchisesession'], $companymainid, $debitnoteno, $debitnotedate);
																												$sqlitaxess->execute();
													...
													<?php
														...
														elseif($info['debitnotetemplate']=='0'){
																											...
																											<td>
																												<table width="100%">
																													...
																													<tr>
																														...
																														<td width="90%" style="font-size:12px;padding: 0px !important;line-height: 15px !important;">
																															<input type="text" readonly value=": <?=$info['email']?>" style="padding:0px;color:black;width: 186px !important;height: 18px !important;border: none !important;outline: none !important;display: inline-flex;font-size: 12px;<?=($access['debitnotebranchemail']=='0')?'visibility:hidden;':''?>" class="form-control form-control-sm">
																															<span style="<?=(($access['debitnoteprintdlno20']=='1')?'display:inline-flex;':'display:none;')?>white-space: nowrap;width: 139px;overflow: hidden;float: right;margin-right: 9px;">
																																DL No 20 &nbsp; : <?=$info['dlno20']?>
																															</span>
																														</td>
																													</tr>
																													<tr>
																														...
																														<td width="90%" style="font-size:12px;padding: 0px !important;line-height: 15px !important;">
																															...
																															<span style="<?=(($access['debitnoteprintdlno21']=='1')?'display:inline-flex;':'display:none;')?>white-space: nowrap;width: 139px;overflow: hidden;float: right;margin-right: 9px;">
																																DL No 21 &nbsp; : <?=$info['dlno21']?>
																															</span>
																														</td>
																													</tr>
																							...
																							<table width="100%" style="border:1px solid #cccccc 0px 0px 0px !important;line-height: 13px !important;height: 723px;">
																								...
																								?>
																									...
																									<?php
																										...
																										if ((($rows[0]['area']!='')||($rows[0]['city']!='')||($rows[0]['state']!='')||($rows[0]['pincode']!='')||($rows[0]['district']!=''))&&(in_array('Billing Address', $fieldview))) {
																											if ((($rows[0]['area']!='')||($rows[0]['city']!=''))) {
																												$debitnote1 = '<span style="font-size:12px;display: inline-flex;width: 254px;overflow: hidden;">'.(($rows[0]['area'])).' '.((($rows[0]['area']!='')&&($rows[0]['city']!=''))?',':'').' '.(($rows[0]['city'])).'</span><br>';
																											}
																											if ((($rows[0]['state']!='')||($rows[0]['pincode']!='')||($rows[0]['district']!=''))) {
																												$debitnote2 = '<span style="font-size: 12px;display: inline-flex;width: 254px;overflow: hidden;">'.$rows[0]['state'].' '.((($rows[0]['state']!='')&&($rows[0]['pincode']!=''))?',':'').' '.$rows[0]['pincode'].' '.((($rows[0]['state']!='')&&($rows[0]['district']!='')||($rows[0]['pincode']!='')&&($rows[0]['district']!=''))?',':'').' '.$rows[0]['district'].'</span>';
																											}
																										}
																										$gstin ='';
																										if (($infomainaccessuser['debitnoteprintgstin']=='show')||($rows[0]['gstno']!='')&&($infomainaccessuser['debitnoteprintgstin']!='hide')) {
													...
													<?php
														...
														elseif($info['debitnotetemplate']=='2'){
													...
													<p align="right" class="mt-3" style="margin-right:-25px; cursor:pointer" id="templatetext">
														Template: '<?=($info['debitnotetemplate']=='1')?'24.13cm X 15.75cm':''?><?=($info['debitnotetemplate']=='0')?'Standard A4 Portrait':''?><?=($info['debitnotetemplate']=='2')?'Standard A5 Landscape':''?> ' 
														<a data-bs-toggle="modal" data-bs-target="#changedebitnoteModal" class="text-blue">
															Change
														</a>
													</p>
												</div>
												<div class="modal fade" id="changedebitnoteModal" tabindex="-1" role="dialog" aria-labelledby="changedebitnoteModalLabel" aria-hidden="true">
													<div class="modal-dialog modal-dialog-centered" role="document">
														<div class="modal-content">
															<div class="modal-header">
																<h5 class="modal-title" id="changedebitnoteModalLabel">
																	Choose Template
																</h5>
																<button type="button" class="btn-close text-dark" data-bs-dismiss="modal" aria-label="Close">
																	<span aria-hidden="true">&times;</span>
															...
															<div class="modal-body">
																...
																function standandclick(){
																	...
																	setTimeout(function() {
																		...
																		if (r == true) {
																			window.location.href='changetemplate.php?template=debitnote&id=<?=$_GET['id']?>&no=<?=$debitnoteno?>&date=<?=$debitnotedate?>&temptype=0&submt=submt';
																		}
																	}, 100);
																}
																function standardafiveclick(){
																	...
																	setTimeout(function() {
																		var r = confirm('Are you sure want to Change this Template?');
																		if (r == true) {
																			window.location.href='changetemplate.php?template=debitnote&id=<?=$_GET['id']?>&no=<?=$debitnoteno?>&date=<?=$debitnotedate?>&temptype=2&submt=submt';
																		}
																	}, 100);
																}
																function advancedclick(){
																	...
																	setTimeout(function() {
																		var r = confirm('Are you sure want to Change this Template?');
																		if (r == true) {
																			window.location.href='changetemplate.php?template=debitnote&id=<?=$_GET['id']?>&no=<?=$debitnoteno?>&date=<?=$debitnotedate?>&temptype=1&submt=submt';
																		}
																	}, 100);
																}
															<?php
...
function converHTMLFileToPDF() {
	...
	$.ajax({
		type: "GET",
		url: 'debitnotedompdf.php?term=debitnote&icsgst='+finalforprint+'&names=Debit Notes-<?=str_replace('/', '-', $_GET['debitnoteno'])?>-<?=$_GET['debitnotedate']?>&debitnoteno=<?=$_GET['debitnoteno']?>&debitnotedate=<?=$_GET['debitnotedate']?>&sizes=a4',
		success: function (result) {
			...
			req.open("GET", "dompdf/Debit Notes-<?=str_replace('/', '-', $_GET['debitnoteno'])?>-<?=$_GET['debitnotedate']?>.pdf", true);
	...
	$.ajax({
		...
		success: function (result) {
			...
			req.onload = function (event) {
				...
				link.download="Debit Notes-<?=str_replace('/', '-', $_GET['debitnoteno'])?>-<?=$_GET['debitnotedate']?>.pdf";
				link.click();
			};
			req.send();
...
function converHTMLFileToPDF() {
	...
	$.ajax({
		type: "GET",
		url: 'debitnotedompdf.php?term=debitnote&icsgst='+finalforprint+'&names=Debit Notes-<?=str_replace('/', '-', $_GET['debitnoteno'])?>-<?=$_GET['debitnotedate']?>&debitnoteno=<?=$_GET['debitnoteno']?>&debitnotedate=<?=$_GET['debitnotedate']?>&sizes=a5',
		success: function (result) {
			if (result && result.length > 0) {       
				result=$.parseJSON( result );
				Timer=result.timer;
...
function converHTMLFileToPDF() {
	...
	$.ajax({
		type: "GET",
		url: 'debitnotedompdf.php?term=debitnote&icsgst='+finalforprint+'&names=Debit Notes-<?=str_replace('/', '-', $_GET['debitnoteno'])?>-<?=$_GET['debitnotedate']?>&debitnoteno=<?=$_GET['debitnoteno']?>&debitnotedate=<?=$_GET['debitnotedate']?>&sizes=dt',
		success: function (result) {
			if (result && result.length > 0) {       
				result=$.parseJSON( result );
				Timer=result.timer;
