...
<?php
	...
	$datefetch=$dateformat->fetch_array();
	if ($datefetch['date']=='DD/MM/YYYY') {
		if (($access['purreturnprintexpdate']=='0')&&($access['purreturnprintexpmonth']=='0')&&($access['purreturnprintexpyear']=='0')) {
			$dateforexp = '';
		}
		if (($access['purreturnprintexpdate']=='1')&&($access['purreturnprintexpmonth']=='1')&&($access['purreturnprintexpyear']=='1')) {
			$dateforexp = 'd/m/Y';
		}
		if (($access['purreturnprintexpdate']=='0')&&($access['purreturnprintexpmonth']=='1')&&($access['purreturnprintexpyear']=='1')) {
			$dateforexp = 'm/Y';
		}
		if (($access['purreturnprintexpdate']=='1')&&($access['purreturnprintexpmonth']=='0')&&($access['purreturnprintexpyear']=='1')) {
			$dateforexp = 'd/Y';
		}
		if (($access['purreturnprintexpdate']=='1')&&($access['purreturnprintexpmonth']=='1')&&($access['purreturnprintexpyear']=='0')) {
			$dateforexp = 'd/m';
		}
		if (($access['purreturnprintexpdate']=='1')&&($access['purreturnprintexpmonth']=='0')&&($access['purreturnprintexpyear']=='0')) {
			$dateforexp = 'd';
		}
		if (($access['purreturnprintexpdate']=='0')&&($access['purreturnprintexpmonth']=='1')&&($access['purreturnprintexpyear']=='0')) {
			$dateforexp = 'm';
		}
		if (($access['purreturnprintexpdate']=='0')&&($access['purreturnprintexpmonth']=='0')&&($access['purreturnprintexpyear']=='1')) {
			$dateforexp = 'Y';
		}
	}
	//FOR DATE FORMAT
...
?>
	...
	<?php
		...
		if((isset($_GET['purchasereturnno']))&&(isset($_GET['purchasereturndate']))){
			$purchasereturnno=mysqli_real_escape_string($con, $_GET['purchasereturnno']);
			$purchasereturndate=mysqli_real_escape_string($con, $_GET['purchasereturndate']);
			$sqls=$con->prepare("SELECT * FROM pairpurchasereturns WHERE franchisesession=? AND createdid=? AND purchasereturnno=? AND purchasereturndate=? ORDER BY id ASC");
			$sqls->bind_param("ssss", $_SESSION['franchisesession'], $companymainid, $purchasereturnno, $purchasereturndate);
			$sqls->execute();
				$sql = $sqls->get_result();
			$count=1;
			if($sql->num_rows>0){
					...
					<div class="card mb-4 mt-5">
						<div class="card-body p-3" style="font-family:'Myriad Set Pro','Helvetica Neue',Helvetica,Arial,sans-serif;">
							<div class="row">
								...
								<div class="col-lg-6">
									<span style="float:right">
										...
										<a style="margin:4.5px 4.5px !important;" class="btn btn-primary btn-sm btn-custom-grey" href="purchasepaymentadd.php?no=<?=$purchasereturnno?>&date=<?=$purchasereturndate?>&type=purchasereturn" id="recordpaymentlink">
									...
									?>
										<a class="btn btn-primary btn-sm btn-custom-grey" href="purchasereturnedit.php?id=<?=$_GET['id']?>&purchasereturnno=<?=$purchasereturnno?>&purchasereturndate=<?=$purchasereturndate?>" style="margin:4.5px 4.5px !important;">
									...
									<?php
										$paidamount=0;
										$currentbalance=0;
										$currentamount=(float)$rows[0]['purchasereturnamount'];
										$sqlpurchasepays=$con->prepare("SELECT amount FROM pairpurchasereturnpayhistory WHERE franchisesession=? AND createdid=? AND purchasereturnno=? AND purchasereturndate=? ORDER BY id DESC");
											$sqlpurchasepays->bind_param("ssss", $_SESSION['franchisesession'], $companymainid, $purchasereturnno, $purchasereturndate);
										...
										$currentbalance=((float)$rows[0]['purchasereturnamount']-$paidamount);
										...
										<ul class="dropdown-menu  dropdown-menu-end customdropdown me-sm-n4 mt-1" aria-labelledby="dropdownMenuButton">   
											<div style="background-color: #3c3c46;margin-top: 0px !important;cursor: pointer;"> 
												...
												<li class="nav-item" style="margin-top:-22px !important;">
													<a class="nav-link" href="purchasereturncancel.php?purchasereturnno=<?=$purchasereturnno?>&purchasereturndate=<?=$purchasereturndate?>&cancelstatus=<?=($rows[0]['cancelstatus']=='0')?'1':'0'?>" onclick="return confirm('Are you sure want to <?=($rows[0]['cancelstatus']=='0')?'Cancel':'Enable'?> this Bill?')">
							...
							<div class="tab-content" id="nav-tabContent">
								<div class="tab-pane fade" id="nav-journal" role="tabpanel" aria-labelledby="nav-journal-tab">
									<div class="table-responsive m-3" id="journaltables">
										<table class="table table-bordered">
											<tbody>
											<?php
												$sqlijournals=$con->prepare("SELECT customerid, chartaccountname, customername, sum(ledgerdebit) AS ledgerdebit, sum(ledgercredit) AS ledgercredit, notes,ledgerdate, ledgerno, referenceno, referencedate, totalledgerdebit, totalledgercredit, id, type FROM pairledgers WHERE franchisesession=? AND createdid=? AND ledgerno=? AND ledgerdate=? AND (type='purchasereturn' OR type='purchasereturn payment') GROUP BY type,chartaccountid,ledgerdate,ledgerno ORDER BY ledgerdate,ledgerno,type ASC");
													$sqlijournals->bind_param("ssss", $_SESSION['franchisesession'], $companymainid, $purchasereturnno, $purchasereturndate);
								...
								<div class="tab-pane fade" id="nav-profile" role="tabpanel" aria-labelledby="nav-profile-tab">
									<div class="table-responsive m-3" id="histables">
										<table class="table table-bordered">
											...
											<tbody>
											<?php
												$sqluse=$con->prepare("SELECT * FROM pairusehistory WHERE (usetype = 'PURCHASERETURNS' AND (useid = ? AND (uniqueid = ? OR uniqueid = ?))) ORDER BY createdon DESC");
													$sqluse->bind_param("sss", $purchasereturnno, $_GET['id'], $purchasereturnno);
													$sqluse->execute();
											...
											?>
												<tr>
													...
													<td data-label="DETAILS">
														...
														<br>
														<span>
															<?=((str_contains($infouse['useremarks'],'PURCHASERETURN CREATED'))?'Created By':'Changed By')?>
														</span>
								...
								<?php
									$sqlismainaccessusers=$con->prepare("SELECT moduleno, moduleprefix, modulesuffix,purreturnprintgsttreatment,purreturnprintgstin,modulename,purreturnprintpos,viewprintdlno20,viewprintdlno21 FROM pairmainaccess WHERE franchiseid = ? AND moduletype = 'Purchase Returns' ORDER BY id ASC");
											...
											<div class="row d-flex justify-content-center">
												<div class="col-lg-8 col-md-12 justify-content-center">
													...
													<?php
														...
														if($info['purchasereturntemplate']=='1'){
																					...
																					<td style="text-align:center;height: 80px !important;">
																						<table width="100%">
																							<tr>
																								...
																								<td width="51.5%" style="text-align:left;vertical-align: top;padding-left: 10px !important;border-right: 0px solid #cccccc;">
																									<table width="100%">
																										...
																										<tr>
																											<td>
																												<table width="100%">
																													<tr style="<?=($access['purreturnbranchphone']=='0')?'display:none;':''?>">
																													...
																													<tr>
																														<td width="10%" style="font-size:12px;padding: 0px !important;line-height: 15px !important;<?=($access['purreturnbranchemail']=='0')?'visibility:hidden;':''?>">
																															E-mail
																														</td>
																														<td width="90%" style="font-size:12px;padding: 0px !important;line-height: 15px !important;">
																															<input type="text" readonly value=": <?=$info['email']?>" style="padding:0px;color:black;width: 206px !important;height: 18px !important;border: none !important;outline: none !important;display: inline-flex;font-size: 12px;<?=($access['purreturnbranchemail']=='0')?'visibility:hidden;':''?>" class="form-control form-control-sm">
																															<span style="<?=(($access['purreturnprintdlno20']=='1')?'display:inline-flex;':'display:none;')?>white-space: nowrap;width: 169px;overflow: hidden;float: right;margin-right: 9px;">
																																DL No 20 &nbsp; : <?=$info['dlno20']?>
																															</span>
																														</td>
																													</tr>
																													<tr>
																														<td width="10%" style="font-size:12px;padding: 0px !important;line-height: 15px !important;<?=($access['purreturnbranchgstin']=='0')?'visibility:hidden;':''?>">
																															GSTIN
																														</td>
																														<td width="90%" style="font-size:12px;padding: 0px !important;line-height: 15px !important;">
																															<span style="display: inline-flex;white-space: nowrap;width: 134px;overflow: hidden;<?=($access['purreturnbranchgstin']=='0')?'visibility:hidden;':''?>">
																																: <?=$info['gstno']?>
																															</span>
																															<span style="<?=(($access['purreturnprintdlno21']=='1')?'display:inline-flex;':'display:none;')?>white-space: nowrap;width: 169px;overflow: hidden;float: right;margin-right: 9px;">
																								...
																								<td width="34.5%">
																									<table width="100%">
																										<tr <?=(($access['purreturnbank']=='1')?'':'style="display:none;"')?>>
																										...
																										<tr <?=(($access['purreturnname']=='1')?'':'style="display:none;"')?>>
																											<td style="padding: 0px 6px !important;font-size: 11px !important;line-height: 15px !important;text-align: left;" width="35%">
																										...
																										<tr <?=(($access['purreturnaccnumber']=='1')?'':'style="display:none;"')?>>
																											<td style="padding: 0px 6px !important;font-size: 11px !important;line-height: 15px !important;text-align: left;" width="35%">
																										...
																										<tr <?=(($access['purreturnifsccode']=='1')?'':'style="display:none;"')?>>
																											<td style="padding: 0px 6px !important;font-size: 11px !important;line-height: 15px !important;text-align: left;" width="35%">
																										...
																										<tr <?=(($access['purreturnbranchandcity']=='1')?'':'style="display:none;"')?>>
																											<td style="padding: 0px 6px !important;font-size: 11px !important;line-height: 15px !important;text-align: left;" width="35%">
																								...
																								<td style="padding: 0px !important;">
																									<table width="100%">
																										...
																										<tr <?=((in_array('GSTIN', $fieldview))?'':'style="display:none;"')?>>
																											<td width="38%" style="<?=((($infomainaccessuser['purreturnprintgstin']=='show')||($rows[0]['gstno']!='')&&($infomainaccessuser['purreturnprintgstin']!='hide'))?'':'display:none;')?>font-size:12px;padding:0px 6px !important;line-height: 15px !important;">
																												GSTIN
																											</td>
																											<td width="62%" style="<?=((($infomainaccessuser['purreturnprintgstin']=='show')||($rows[0]['gstno']!='')&&($infomainaccessuser['purreturnprintgstin']!='hide'))?'':'display:none;')?>font-size:12px;padding: 0px !important;line-height: 15px !important;">
																												: 
																											<?php
																												if (($infomainaccessuser['purreturnprintgstin']=='show')||($rows[0]['gstno']!='')&&($infomainaccessuser['purreturnprintgstin']!='hide')) {
																											?>
																					...
																					<td width="33%" style="white-space: normal !important;vertical-align: middle;padding: 0px !important;">
																						...
																						?>
																							...
																							?>
																								<td style="padding: 0px !important;font-size: 14px !important;line-height: 15px !important;text-align: left !important;" width="45%">
																									:   <b>
																											<?=date($date,strtotime($rows[0]['purchasereturndate']))?>
																										</b>
																								</td>
																							</tr>
																							<tr>
																							...
																							<table width="100%" style="border:1px solid #cccccc 0px 0px 0px !important;line-height: 13px !important;height: 226px;">
																								...
																								?>
																									...
																									<?php
																										...
																										$pos ='';
																										if (($infomainaccessuser['purreturnprintpos']=='show')||($rows[0]['pos']!='')&&($infomainaccessuser['purreturnprintpos']!='hide')) {
																								...
																								<td style="padding: 0px !important;">
																									<table id="gsttable" width="100%" style="line-height: 13px !important;padding: 0px !important;border: 1px solid #cccccc;border-collapse: collapse;">
																										...
																										<?php
																											...
																											$sqlitaxess=$con->prepare("SELECT * FROM pairpurchasereturns WHERE franchisesession = ? AND createdid = ? AND purchasereturnno=? and purchasereturndate=? ORDER BY id ASC");
																												$sqlitaxess->bind_param("ssss", $_SESSION['franchisesession'], $companymainid, $purchasereturnno, $purchasereturndate);
																												$sqlitaxess->execute();
													...
													<?php
														...
														elseif($info['purchasereturntemplate']=='0'){
																											...
																											<td>
																												<table width="100%">
																													...
																													<tr>
																														...
																														<td width="90%" style="font-size:12px;padding: 0px !important;line-height: 15px !important;">
																															<input type="text" readonly value=": <?=$info['email']?>" style="padding:0px;color:black;width: 186px !important;height: 18px !important;border: none !important;outline: none !important;display: inline-flex;font-size: 12px;<?=($access['purreturnbranchemail']=='0')?'visibility:hidden;':''?>" class="form-control form-control-sm">
																															<span style="<?=(($access['purreturnprintdlno20']=='1')?'display:inline-flex;':'display:none;')?>white-space: nowrap;width: 139px;overflow: hidden;float: right;margin-right: 9px;">
																																DL No 20 &nbsp; : <?=$info['dlno20']?>
																															</span>
																														</td>
																													</tr>
																													<tr>
																														...
																														<td width="90%" style="font-size:12px;padding: 0px !important;line-height: 15px !important;">
																															...
																															<span style="<?=(($access['purreturnprintdlno21']=='1')?'display:inline-flex;':'display:none;')?>white-space: nowrap;width: 139px;overflow: hidden;float: right;margin-right: 9px;">
																																DL No 21 &nbsp; : <?=$info['dlno21']?>
																															</span>
																														</td>
																													</tr>
																					...
																					<td style="padding: 0px !important;height:723px !important;">
																						<div style="max-height: 723px;min-height: 723px;overflow: hidden;">
																							<table width="100%" style="border:1px solid #cccccc 0px 0px 0px !important;line-height: 13px !important;height: 723px;">
																								...
																								?>
																									<tr style="height:13px !important;">
																										<td style="border: 1px solid #cccccc;padding:0px 3px !important;min-width:130px !important;max-width: 130px !important;overflow: hidden;line-height: 18px;white-space: normal !important;<?=(in_array('Item Details', $fieldview))?'':'display:none !important;'?>">
																									...
																									<?php
																										...
																										if ((($rows[0]['area']!='')||($rows[0]['city']!='')||($rows[0]['state']!='')||($rows[0]['pincode']!='')||($rows[0]['district']!=''))&&(in_array('Billing Address', $fieldview))) {
																											if ((($rows[0]['area']!='')||($rows[0]['city']!=''))) {
																												$purchasereturn1 = '<span style="font-size:12px;display: inline-flex;width: 254px;overflow: hidden;">'.(($rows[0]['area'])).' '.((($rows[0]['area']!='')&&($rows[0]['city']!=''))?',':'').' '.(($rows[0]['city'])).'</span><br>';
																											}
																											if ((($rows[0]['state']!='')||($rows[0]['pincode']!='')||($rows[0]['district']!=''))) {
																												$purchasereturn2 = '<span style="font-size: 12px;display: inline-flex;width: 254px;overflow: hidden;">'.$rows[0]['state'].' '.((($rows[0]['state']!='')&&($rows[0]['pincode']!=''))?',':'').' '.$rows[0]['pincode'].' '.((($rows[0]['state']!='')&&($rows[0]['district']!='')||($rows[0]['pincode']!='')&&($rows[0]['district']!=''))?',':'').' '.$rows[0]['district'].'</span>';
																											}
																										}
																										$gstin ='';
																										if (($infomainaccessuser['purreturnprintgstin']=='show')||($rows[0]['gstno']!='')&&($infomainaccessuser['purreturnprintgstin']!='hide')) {
													...
													<?php
														...
														elseif($info['purchasereturntemplate']=='2'){
													...
													<p align="right" class="mt-3" style="margin-right:-25px; cursor:pointer" id="templatetext">
														Template: '<?=($info['purchasereturntemplate']=='1')?'24.13cm X 15.75cm':''?><?=($info['purchasereturntemplate']=='0')?'Standard A4 Portrait':''?><?=($info['purchasereturntemplate']=='2')?'Standard A5 Landscape':''?> ' 
														<a data-bs-toggle="modal" data-bs-target="#changepurchasereturnModal" class="text-blue">
															Change
														</a>
													</p>
												</div>
												<div class="modal fade" id="changepurchasereturnModal" tabindex="-1" role="dialog" aria-labelledby="changepurchasereturnModalLabel" aria-hidden="true">
													<div class="modal-dialog modal-dialog-centered" role="document">
														<div class="modal-content">
															<div class="modal-header">
																<h5 class="modal-title" id="changepurchasereturnModalLabel">
																	Choose Template
																</h5>
																<button type="button" class="btn-close text-dark" data-bs-dismiss="modal" aria-label="Close">
																	<span aria-hidden="true">&times;</span>
															...
															<div class="modal-body">
																...
																function standandclick(){
																	...
																	setTimeout(function() {
																		...
																		if (r == true) {
																			window.location.href='changetemplate.php?template=purchasereturn&id=<?=$_GET['id']?>&no=<?=$purchasereturnno?>&date=<?=$purchasereturndate?>&temptype=0&submt=submt';
																		}
																	}, 100);
																}
																function standardafiveclick(){
																	...
																	setTimeout(function() {
																		var r = confirm('Are you sure want to Change this Template?');
																		if (r == true) {
																			window.location.href='changetemplate.php?template=purchasereturn&id=<?=$_GET['id']?>&no=<?=$purchasereturnno?>&date=<?=$purchasereturndate?>&temptype=2&submt=submt';
																		}
																	}, 100);
																}
																function advancedclick(){
																	...
																	setTimeout(function() {
																		var r = confirm('Are you sure want to Change this Template?');
																		if (r == true) {
																			window.location.href='changetemplate.php?template=purchasereturn&id=<?=$_GET['id']?>&no=<?=$purchasereturnno?>&date=<?=$purchasereturndate?>&temptype=1&submt=submt';
																		}
																	}, 100);
																}
															<?php
...
<?php
if($info['purchasereturntemplate']=='0'){
	//FOR Standard A4 Portrait PAPER
?>
<script>
function converHTMLFileToPDF() {
	...
	$.ajax({
		type: "GET",
		url: 'purchasereturndompdf.php?term=purchasereturn&icsgst='+finalforprint+'&names=Purchase Returns-<?=$_GET['purchasereturnno']?>-<?=$_GET['purchasereturndate']?>&purchasereturnno=<?=$_GET['purchasereturnno']?>&purchasereturndate=<?=$_GET['purchasereturndate']?>&sizes=a4',
		success: function (result) {
			...
			req.open("GET", "dompdf/Purchase Returns-<?=$_GET['purchasereturnno']?>-<?=$_GET['purchasereturndate']?>.pdf", true);
	...
	$.ajax({
		...
		success: function (result) {
			...
			req.onload = function (event) {
				...
				link.download="Purchase Returns-<?=$_GET['purchasereturnno']?>-<?=$_GET['purchasereturndate']?>.pdf";
				link.click();
			};
			req.send();
...
if($info['purchasereturntemplate']=='2'){
	//FOR Standard A5 Landscape PAPER
?>
<script>
function converHTMLFileToPDF() {
	...
	$.ajax({
		type: "GET",
		url: 'purchasereturndompdf.php?term=purchasereturn&icsgst='+finalforprint+'&names=Purchase Returns-<?=$_GET['purchasereturnno']?>-<?=$_GET['purchasereturndate']?>&purchasereturnno=<?=$_GET['purchasereturnno']?>&purchasereturndate=<?=$_GET['purchasereturndate']?>&sizes=a5',
		success: function (result) {
			if (result && result.length > 0) {       
				result=$.parseJSON( result );
				Timer=result.timer;
...
function converHTMLFileToPDF() {
	...
	$.ajax({
		type: "GET",
		url: 'purchasereturndompdf.php?term=purchasereturn&icsgst='+finalforprint+'&names=Purchase Returns-<?=$_GET['purchasereturnno']?>-<?=$_GET['purchasereturndate']?>&purchasereturnno=<?=$_GET['purchasereturnno']?>&purchasereturndate=<?=$_GET['purchasereturndate']?>&sizes=dt',
		success: function (result) {
			if (result && result.length > 0) {       
				result=$.parseJSON( result );
				Timer=result.timer;
