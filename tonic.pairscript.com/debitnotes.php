		  ...
		  <div class="col-12">
			 <div class="card mb-4 mt-5">
				<div class="card-body p-3" style="color:black;font-family:'Myriad Set Pro','Helvetica Neue',Helvetica,Arial,sans-serif;">
				  <div class="row">
					 ...
					 <div class="col-lg-3" style="margin-bottom: 57px !important;"> 
						<input class="form-control" id="debitnotesearch" type="text" placeholder="Search.." style="height:24px;">
					 </div> 
				  </div>
				<?php
				  $sqlismainaccessusers=$con->prepare("SELECT * FROM pairmainaccess WHERE userid=? AND moduletype='Debit Notes' ORDER BY id ASC");
				...
				?>
				  <div align="right" class=" p-2 addandmenuonlyforimports" style="margin-bottom: -30px;">
					 <div class="row" style="width:250px;">           
						<div class="col-8" style="<?=((in_array('New Debit Notes', $modulecolumns))?'':'visibility: hidden;')?>">  
						  <a href="debitnoteadd.php" class="btn btn-custom btn-sm p-2 add" style="font-size: 13px;height: 24px;margin-bottom:1rem;margin-top: -151px;margin-right:-39px;padding-right: 5px;">
						...
						<div class="col-4">
						  ...
						  ?>
							 <ul class="dropdown-menu dropdown-menu-end" aria-labelledby="dropdownMenuButton">
								<li>
								  <a class="dropdown-item" href="debitnoteimport.php">
				...
				<?php
				  ...
				  $sqlismainaccessusers=$con->prepare("SELECT * FROM pairmainaccess WHERE franchiseid=? AND moduletype='Debit Notes' ORDER BY id ASC");
					 $sqlismainaccessusers->bind_param("i", $_SESSION['franchisesession']);
					 $sqlismainaccessusers->execute();
					 $sqlismainaccessuser = $sqlismainaccessusers->get_result();
					 $infomainaccessuser=$sqlismainaccessuser->fetch_array();
					 //FOR GET MODULE NAME OF DEBITNOTE
...
<div style="visibility: visible;" id="arrowsalltabs">
	...
	<div class="tab-content" id="nav-tabContent" style="position:relative;top: -88px;">
		<div class="tab-pane fade show active p-3" id="nav-general" role="tabpanel" aria-labelledby="nav-general-tab">
					 ...
					 <table id="someTable" class="table table-bordered align-items-center mb-0" style="table-layout: fixed;">
						...
						<?php
						  ...
						  $totaldebitnotedate=array();
									$sqls=$con->prepare("SELECT MAX(id) AS id,debitnotedate, debitnoteno, vendorname, debitnoteterm, duedate, debitnoteamount, cancelstatus FROM pairdebitnotes WHERE franchisesession=? AND createdid=? ".$typesforlisting." GROUP BY debitnotedate, debitnoteno ORDER BY debitnotedate DESC,ordering DESC LIMIT ".(($access['debitnotepaypageload']=='pagenum')?'10':'15')."");
						  ...
						  while($info=$sql->fetch_array()){
							 ...
							 $paidamount=0;
							 $currentbalance=0;
										$sqlpurchasepays=$con->prepare("SELECT amount FROM pairdebitnotepayhistory WHERE franchisesession=? AND createdid=? AND debitnoteno=? AND debitnotedate=? ORDER BY id DESC");
								$sqlpurchasepays->bind_param("ssss", $_SESSION['franchisesession'], $companymainid, $info['debitnoteno'], $info['debitnotedate']);
						...
						<?php
						  ...
						  $sqltotlists=$con->prepare("SELECT COUNT(DISTINCT debitnoteno) FROM pairdebitnotes WHERE franchisesession=? AND createdid=? ".$typesforlisting."");
				  ...
				  <?php
					 if ($access['debitnotepaypageload']=='pageauto') {
				  ?>
					 <script type="text/javascript">
					 ...
					 $('.main-content').on('scroll', function() {
						...
						if (scrollTop + $(this).innerHeight() >= this.scrollHeight-50) {
						  if (isPreviousEventComplete && isDataAvailable) {
							 ...
							 $.ajax({
								type: "GET",
								url: 'listdebitnotesearch.php?term=' + sIndex + '&limitings=15&typesforlisting=<?=urlencode($typesforlisting)?>',
				  ...
				  ?>
					 ...
					 try{
						...
						const pagechanges = (page, pageSize) => {
						  ...
						  $.ajax({
							 type: "GET",
							 url: 'listdebitnotesearch.php?term='+perpages+'&limitings='+($('#limitforpagenum').val())+'&typesforlisting=<?=urlencode($typesforlisting)?>',
							 success: function (result) {
								$("#myTable").html(result);
							 },
							 error: function (error) {
...
function submitthesorting(value) {
  $.ajax({
	 type: "GET",
	 url: 'listingsortfilters.php?moduletype=Debit Notes&typesforlisting='+value+'',
	 success: function (result) {
		console.log(result);
		window.location = 'debitnotes.php?remarks='+result+'';
	 },
	 error: function (error) {
		console.log(error);
	 }
...
<script>
function deleteitem(debitnoteno,debitnotedate,cancelstatus){
  $('#deleteconfirm-adddelete').modal('show');
  $("#deleteconfirm-adddelete #deleteitem").attr("href","debitnotecancel.php?debitnoteno="+debitnoteno+"&debitnotedate="+debitnotedate+"&cancelstatus="+cancelstatus);
}
//FOR DELETE ITEM
</script>
</body>
