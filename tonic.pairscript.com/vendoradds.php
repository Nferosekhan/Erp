...
$venhead = $infomainaccesspublicnamecust['modulename'];
// $vendorids = mysqli_real_escape_string($con, $_POST['vendorid']);
...
$vendordname = mysqli_real_escape_string($con, $_POST['vendordname']);
...
$msg = "";
$msg_class = "";
if (($vendordname != "" || $vendordname == "")) {
$sqlcon = "SELECT id From paircustomers WHERE customername = '{$vendordname}' and franchisesession='" . $_SESSION["franchisesession"] . "' and  moduletype='Vendors'";
...
$tid = mysqli_insert_id($con);
                ...
                if($vendordname!='')
                {
                    if($ch!='')
                    {
                        $ch.='<br> '.$venhead.' Name<span style="color:green;" id="prohisfromtospan">( '.$vendordname.' ) </span>';
                    }
                    else
                    {
                        $ch.=''.$venhead.' Name<span style="color:green;" id="prohisfromtospan">( '.$vendordname.' ) </span>';
                    }                   
                }
                if($category!='')
                {
                ...
                if(in_array('Vendors Visibility', $cusfieldadd)){
                if($vvisibility!='')
                {
                    if($ch!='')
                    {
...
// } else {
// echo "This record is Already Found! Kindly check in All Vendors List|0";
// }
} else {
echo "Error Data|0";
}
